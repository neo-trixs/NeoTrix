    ...
    fn wrap_command(runtime: CloudRuntime, code: &str) -> (String, Vec<String>) {
        match runtime {
            ...
            CloudRuntime::RustStable => {
                // /tmp/ inside Docker container is ephemeral and isolated per-container — correct usage
                let script = format!(
...
impl CloudSandboxProvider for LocalDockerProvider {
    ...
    async fn execute(
        ...
        if has_uploads {
            // Host-side temp dir for ephemeral upload files — cleaned up after session, correct for temp data
            let tmpdir = std::env::temp_dir().join(format!("neotrix-upload-{}", session_id));
        ...
        if let Ok(mut bufs) = self.log_buffers.lock() {
            if bufs.len() >= MAX_SESSIONS {
                if let Some(oldest) = bufs.keys().next().cloned() {
                    bufs.remove(&oldest);
                }
            }
    ...
    async fn upload_file(&self, session_id: &str, path: &str, data: Vec<u8>) -> Result<(), String> {
        // Host-side temp dir for ephemeral sandbox uploads — no persistence needed
        let tmpdir = std::env::temp_dir().join(format!("neotrix-upload-{}", session_id));
        let tmpdir = tmpdir.to_string_lossy().to_string();
    ...
    fn stream_logs(&self, session_id: &str) -> BoxStream<'static, String> {
        let guard = match self.log_buffers.lock() {
            Ok(g) => g,
            Err(e) => {
                log::error!("docker stream_logs: mutex poisoned: {}", e);
                return stream::iter(Vec::new()).boxed();
            }
        };
        let logs = guard.get(session_id).cloned().unwrap_or_default();
