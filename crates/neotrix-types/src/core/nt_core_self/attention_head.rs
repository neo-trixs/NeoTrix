...
mod tests {
    ...
    fn test_attention_manager_decay_all() {
        ...
        let code_idx = AttentionDomain::all().iter().position(|d| *d == AttentionDomain::Code).expect("Code domain should exist");
        let plan_idx = AttentionDomain::all().iter().position(|d| *d == AttentionDomain::Planning).expect("Planning domain should exist");
