...
impl MetaMonitor {
    ...
    const MAX_CHECK_HISTORY: usize = 5000;
    ...
    pub fn run_check(&mut self) -> HealthCheck {
        ...
        if self.check_history.len() > Self::MAX_CHECK_HISTORY {
            self.check_history.drain(0..Self::MAX_CHECK_HISTORY / 5);
        }
        check
    }
    ...
    pub fn generate_alerts(&mut self) -> Vec<MetaAlert> {
        ...
        if self.alerts.len() > Self::MAX_ALERTS {
            self.alerts.drain(0..Self::MAX_ALERTS / 5);
        }
        new_alerts
    }
