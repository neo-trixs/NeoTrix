use super::self_model::{DebtSeverity, TechDebtItem, TechDebtKind, SelfModel};
...
impl WeaknessAnalyzer {
    ...
    fn detect_large_files(&self, model: &SelfModel) -> Vec<Weakness> {
        model.files.iter()
            ...
            .map(|f| Weakness {
                ...
                description: format!("File has {} lines (threshold: {})", f.lines, self.large_file_threshold),
    ...
    fn detect_missing_tests(&self, model: &SelfModel) -> Vec<Weakness> {
        model.modules.iter()
            .filter(|m| !m.has_tests && m.total_lines > self.missing_test_threshold && !m.name.contains("bin"))
            .map(|m| Weakness {
                ...
                description: format!("Module '{}' has {} lines but zero tests", m.name, m.total_lines),
                impact: "Changes in this module cannot be verified automatically".into(),
                suggestion: format!("Add #[cfg(test)] mod tests with unit tests for all public APIs in {}", m.name),
            })
            .collect()
    }
    ...
    fn detect_excess_unwrap(&self, model: &SelfModel) -> Vec<Weakness> {
        model.modules.iter()
            ...
            .map(|m| Weakness {
                ...
                suggestion: "Replace .unwrap() with .ok(), ? operator, or proper error handling".into(),
            })
            .collect()
    }
    ...
    fn detect_todo_leftovers(&self, model: &SelfModel) -> Vec<Weakness> {
        model.modules.iter()
            ...
            .map(|m| Weakness {
                ...
                line: None,
                severity: DebtSeverity::Minor,
                description: format!("Module '{}' has {} TODO/todo! markers", m.name, m.todo_count),
                impact: "Incomplete features or unresolved issues".into(),
                suggestion: "Review TODOs, implement remaining work or convert to tracked issues".into(),
            })
            .collect()
    }
    ...
    fn detect_orphan_modules(&self, model: &SelfModel) -> Vec<Weakness> {
        ...
        orphans.iter().map(|name| Weakness {
    ...
    fn detect_circular_deps(&self, model: &SelfModel) -> Vec<Weakness> {
        ...
        cycles.iter().map(|cycle| Weakness {
    ...
    fn detect_compilation_warnings(&self, model: &SelfModel) -> Vec<Weakness> {
        if model.compilation.warnings > 0 {
            model.modules.iter().map(|m| Weakness {
    ...
    fn detect_test_gaps(&self, model: &SelfModel) -> Vec<Weakness> {
        model.modules.iter()
            ...
            .map(|m| Weakness {
                ...
                line: None,
                severity: DebtSeverity::Minor,
                description: format!("Module '{}' has {} public APIs but {} tests", m.name, m.public_api_count, m.test_count),
    ...
    fn detect_tech_debt_accumulation(&self, model: &SelfModel) -> Vec<Weakness> {
        let critical = model.tech_debt.items.iter()
            .filter(|i| i.severity == DebtSeverity::Critical).count();
        if critical > 10 {
            vec![Weakness {
    ...
    pub fn to_tech_debt_items(&self, report: &WeaknessReport) -> Vec<TechDebtItem> {
        report.weaknesses.iter().map(|w| TechDebtItem {
...
mod tests {
    ...
    fn make_test_model() -> SelfModel {
        ...
        model.modules.push(ModuleInfo {
            name: "safe_module".into(), path: "src/safe/".into(),
            file_count: 1, total_lines: 100, test_count: 5, has_tests: true,
            unsafe_count: 0, unwrap_count: 2, todo_count: 1,
            public_api_count: 5, description: "".into(),
        });
        model.modules.push(ModuleInfo {
            name: "unsafe_module".into(), path: "src/unsafe/".into(),
            file_count: 1, total_lines: 500, test_count: 0, has_tests: false,
            unsafe_count: 10, unwrap_count: 30, todo_count: 5,
            public_api_count: 8, description: "".into(),
        });
        model.files.push(FileInfo {
            path: "src/unsafe/big.rs".into(), module: "unsafe_module".into(),
            lines: 900, is_test_file: false, has_unsafe: true, has_todos: true,
            pub_fns: 5, last_modified: chrono::Utc::now(),
        });
        model
    }
    ...
    fn test_analyzer_detects_large_files() {
        ...
        let large_files: Vec<_> = report.weaknesses.iter()
    ...
    fn test_analyzer_detects_missing_tests() {
        ...
        let model = make_test_model();
        let report = analyzer.analyze(&model);
        let no_tests: Vec<_> = report.weaknesses.iter()
    ...
    fn test_analyzer_detects_excess_unsafe() {
        ...
        let model = make_test_model();
        let report = analyzer.analyze(&model);
        let unsafe_issues: Vec<_> = report.weaknesses.iter()
    ...
    fn test_analyzer_detects_excess_unwrap() {
        ...
        let model = make_test_model();
        let report = analyzer.analyze(&model);
        let unwrap_issues: Vec<_> = report.weaknesses.iter()
    ...
    fn test_analyzer_detects_todos() {
        ...
        let model = make_test_model();
        let report = analyzer.analyze(&model);
        let todo_issues: Vec<_> = report.weaknesses.iter()
