...
mod tests {
    ...
    fn test_initialize_full_knowledge() {
        ...
        bank.initialize_with_coding_knowledge();
        bank.initialize_with_everos_knowledge();
        assert_eq!(bank.memories().len(), 7 + 10 + 5);
    }
    ...
    #[test]
    fn test_explain_topic_finds_explainers() {
        let mut bank = ReasoningBank::new(100);
        bank.initialize_with_explainers();
        let results = bank.explain_topic("borrow");
        assert!(!results.is_empty(), "Should find borrow checker explainer");
        for r in &results {
            assert_eq!(r.task_type, TaskType::MetaCognition);
        }
    }

    #[test]
    fn test_explain_topic_empty_returns_nothing() {
        let bank = ReasoningBank::new(100);
        let results = bank.explain_topic("");
        assert!(results.is_empty());
    }

    #[test]
    fn test_explain_topic_no_match() {
        let mut bank = ReasoningBank::new(100);
        bank.initialize_with_explainers();
        let results = bank.explain_topic("nonexistent_topic_xyz");
        assert!(results.is_empty());
    }

    #[test]
    fn test_explainers_count() {
        let mut bank = ReasoningBank::new(100);
        assert_eq!(bank.explainers_count(), 0);
        bank.initialize_with_explainers();
        assert_eq!(bank.explainers_count(), 6);
    }
}
