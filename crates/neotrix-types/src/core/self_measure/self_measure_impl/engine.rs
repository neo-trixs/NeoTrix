...
impl SelfMeasure {
    ...
    fn recompute_all(&mut self) {
        ...
        for series_i in series.iter() {
            let mean = series_i.iter().sum::<f64>() / n as f64;
            let centered: Vec<f64> = series_i.iter().map(|v| v - mean).collect();
            data.push(centered);
        }
        ...
        for i in 0..NUM_SUBSYSTEMS {
            for j in 0..NUM_SUBSYSTEMS {
                let mut s = 0.0;
                for (di, dj) in data[i].iter().zip(data[j].iter()).take(n) {
                    s += di * dj;
                }
        ...
        let mut count_syn = 0;
        for (i, syn_row) in syn_matrix.iter().enumerate().take(NUM_SUBSYSTEMS) {
            for syn_val in syn_row.iter().skip(i + 1) {
                total_syn += syn_val;
                count_syn += 1;
            }
        }
