...
use crate::core::nt_core_hex::{
    FullReasoningState, ReasoningHexagram, MetaState,
    optimal_starting_mode,
};
...
pub enum TrajectoryPattern {
    ...
    Oscillation { a: ReasoningHexagram, b: ReasoningHexagram, cycle_len: usize },
    /// Stuck in the same state for N consecutive steps
    Stuck { state: ReasoningHexagram, steps: usize },
    /// Productive monotonic progression through distinct states
    ProductiveWalk { states_visited: usize, coverage: f64 },
    /// Returned to a previously visited state (potential loop)
    LoopBack { from: ReasoningHexagram, to: ReasoningHexagram, dist: u32 },
    /// Efficient path — short path between start and resolution
    Efficient { steps: usize, optimal: usize, ratio: f64 },
    /// Inefficient path — wandered too much
    Inefficient { steps: usize, optimal: usize, ratio: f64 },
    /// Exploration phase followed by exploitation
    ExploreThenExploit { explore_len: usize, exploit_len: usize },
}
...
impl OneObserver {
    ...
    pub fn analyze(&mut self, trajectory: &[FullReasoningState], task_keywords: &[&str]) -> ObserverReport {
        ...
        let distinct: std::collections::HashSet<ReasoningHexagram> = modes.iter().copied().collect();
        ...
        let quality_score = Self::compute_quality(&modes, &patterns, &step_qualities, task_keywords);
    ...
    fn detect_patterns(modes: &[ReasoningHexagram]) -> Vec<TrajectoryPattern> {
        ...
        if modes.len() < 2 { return patterns; }
    ...
    fn detect_oscillation(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        ...
        if pairs >= 2 {
            ...
            patterns.push(TrajectoryPattern::Oscillation { a, b, cycle_len: effective_len });
        }
    }

    fn detect_stuck(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        if modes.len() < 3 { return; }
        let last = modes.last().copied().expect("modes.len() >= 3 checked above");
        let mut repeat = 0;
        for m in modes.iter().rev() {
            if *m == last { repeat += 1; } else { break; }
        }
        if repeat >= 3 {
            patterns.push(TrajectoryPattern::Stuck { state: last, steps: repeat });
        }
    }
    ...
    fn detect_efficiency(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        if modes.len() < 2 { return; }
        let start = modes.first().copied().expect("modes.len() >= 2 checked above");
        let end = modes.last().copied().expect("modes.len() >= 2 checked above");
        ...
        if ratio >= 0.8 {
            patterns.push(TrajectoryPattern::Efficient { steps: steps_taken, optimal, ratio });
        } else if ratio <= 0.3 && steps_taken >= 4 {
            patterns.push(TrajectoryPattern::Inefficient { steps: steps_taken, optimal, ratio });
        }
    }

    fn detect_explore_exploit(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        if modes.len() < 5 { return; }
    ...
    fn classify_steps(modes: &[ReasoningHexagram]) -> Vec<StepQuality> {
        ...
        if modes.is_empty() { return qualities; }
    ...
    fn compute_quality(
        ...
        for p in patterns {
            match p {
                ...
                TrajectoryPattern::Oscillation { cycle_len, .. } => score -= 0.05 * *cycle_len.min(&6) as f64,
        ...
        for p in patterns {
            match p {
                ...
                TrajectoryPattern::ProductiveWalk { coverage, .. } => score += 0.1 * coverage.min(1.0),
        ...
        if !step_qualities.is_empty() {
            let productive_ratio = step_qualities.iter().filter(|q| **q == StepQuality::Productive).count() as f64
    ...
    fn compute_capability_deltas(
        ...
        merged.into_iter().map(|(n, vals)| (n.to_string(), vals.iter().sum::<f64>() / vals.len() as f64)).collect()
    }
}
...
mod tests {
    ...
    fn short_trajectory(modes: &[u8]) -> Vec<FullReasoningState> {
        modes.iter().map(|&m| FullReasoningState::new(ReasoningHexagram(m), MetaState(0))).collect()
    }
    ...
    fn test_detects_oscillation() {
        ...
        let report = observer.analyze(&traj, &[]);
        let has_osc = report.patterns.iter().any(|p| matches!(p, TrajectoryPattern::Oscillation { .. }));
        assert!(has_osc, "should detect oscillation, patterns: {:?}", report.patterns);
    ...
    fn test_detects_stuck() {
        ...
        let report = observer.analyze(&traj, &[]);
        let has_stuck = report.patterns.iter().any(|p| matches!(p, TrajectoryPattern::Stuck { .. }));
        assert!(has_stuck);
    }
    ...
    fn test_efficient_trajectory() {
        ...
        let has_efficient = report.patterns.iter().any(|p| matches!(p, TrajectoryPattern::Efficient { .. }));
    ...
    fn test_inefficient_trajectory() {
        ...
        let report = observer.analyze(&traj, &[]);
        let has_inefficient = report.patterns.iter().any(|p| matches!(p, TrajectoryPattern::Inefficient { .. }));
    ...
    fn test_observer_recommends_meta_on_critical() {
        ...
        assert!(report.recommended_meta.expect("recommended_meta should be Some").is_reflecting());
    }
    ...
    fn test_capability_deltas_on_stuck() {
        ...
        let has_exploration = report.capability_deltas.iter().any(|(n, _)| *n == "exploration_bias");
    ...
    fn test_detect_explore_exploit() {
        ...
        let report = observer.analyze(&traj, &[]);
        let has_e_e = report.patterns.iter().any(|p| matches!(p, TrajectoryPattern::ExploreThenExploit { .. }));
