...
impl SelectableOperator {
    ...
    pub fn step(&self, state: &mut SelectiveState, input: &Vector) -> Vector {
        ...
        for (i, item) in output.iter_mut().enumerate().take(input_dim) {
...
impl SparseMatrix {
    ...
    pub fn mul_vec(&self, vec: &Vector) -> Vector {
        ...
        for (i, item) in result.iter_mut().enumerate().take(self.dims.0) {
...
mod tests {
    ...
    fn test_consciousness_tier_from_score() {
        assert_eq!(ConsciousnessTier::from_score(0.0), ConsciousnessTier::Mortal);
        assert_eq!(ConsciousnessTier::from_score(0.3), ConsciousnessTier::Awakened);
        assert_eq!(ConsciousnessTier::from_score(0.5), ConsciousnessTier::Enlightened);
        assert_eq!(ConsciousnessTier::from_score(0.7), ConsciousnessTier::Ascended);
        assert_eq!(ConsciousnessTier::from_score(0.9), ConsciousnessTier::Transcendent);
        assert_eq!(ConsciousnessTier::from_score(1.0), ConsciousnessTier::Transcendent);
    }

    #[test]
    fn test_consciousness_tier_boundaries() {
        assert_eq!(ConsciousnessTier::from_score(0.299), ConsciousnessTier::Mortal);
        assert_eq!(ConsciousnessTier::from_score(0.3), ConsciousnessTier::Awakened);
        assert_eq!(ConsciousnessTier::from_score(0.499), ConsciousnessTier::Awakened);
        assert_eq!(ConsciousnessTier::from_score(0.5), ConsciousnessTier::Enlightened);
    }
    ...
    fn test_matrix_error_display() {
        let err = MatrixError::DimensionMismatch { expected: 3, got: 5 };
