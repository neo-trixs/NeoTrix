...
pub enum BoundaryError {
    ...
    HookPanic { hook: String },
    ChainBreaks { hook: String, suppressed: Vec<String> },
}
...
impl BoundaryHook for AuditHook {
    ...
    fn after(
        ...
        match result {
            Ok(()) => {
                log::info!("[boundary] AFTER  {} | op={} | OK", ctx.operation_id, op.as_str())
            }
            Err(e) => {
                log::warn!(
                ...
                if let Ok(drift) = drift_str.parse::<f64>() {
                    if drift > DRIFT_THRESHOLD {
                        return Err(BoundaryError::HookRejected {
                            ...
                            reason: format!("drift {:.4} exceeds threshold {:.4}", drift, DRIFT_THRESHOLD),
                        });
                    }
                }
            }
            ...
            if let Some(coherence_str) = ctx.details.get("current_coherence") {
                if let Ok(coherence) = coherence_str.parse::<f64>() {
                    if coherence < self.min_coherence {
                        return Err(BoundaryError::HookRejected {
                            hook: self.name().to_string(),
                            reason: format!("coherence {:.4} below minimum {:.4}", coherence, self.min_coherence),
                        });
                    }
                }
            }
