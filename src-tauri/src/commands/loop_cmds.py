#!/usr/bin/env python3
"""Write loop_cmds.rs with proper fsync persistence."""
...
pub fn loop_execute_now(id: String) -> Result<String, String> {
    ...
    if let Some(run) = state.executions.iter_mut().find(|r| r.id == run_id) {
        run.status = if error.is_some() { "failed" } else { "completed" };
...
pub fn loop_tick() -> Result<Vec<String>, String> {
    ...
    for id in &due_ids {
        if let Some(sched) = state.schedules.iter().find(|s| s.id == *id) {
            let run_id = format!("exec-{}", short_uid());
            let started = now;
            ...
            let task_type = sched.task_type.clone();
            drop(state);
            ...
            executed.push(sched.name.clone());
        }
    }
...
path = "/Users/neo/Downloads/neotrix/src-tauri/src/commands/loop_cmds.rs"
