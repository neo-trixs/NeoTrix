...
#[command]
pub fn read_dir_recursive(path: String, max_depth: u32) -> Result<Vec<FileNode>, NeoTrixError> {
    let dir = std::path::Path::new(&path);
    if !dir.is_dir() {
        return Err(NeoTrixError::Path { path: std::path::PathBuf::from(&path), detail: "Not a directory".to_string() });
    }
    read_dir_inner(dir, 0, max_depth)
}

fn read_dir_inner(dir: &std::path::Path, depth: u32, max_depth: u32) -> Result<Vec<FileNode>, NeoTrixError> {
    if depth > max_depth { return Ok(Vec::new()); }
    let mut nodes = Vec::new();
    let entries = std::fs::read_dir(dir).map_err(|e| NeoTrixError::Io(e.to_string()))?;
...
pub fn read_file(path: String) -> Result<String, NeoTrixError> {
    std::fs::read_to_string(&path).map_err(|e| NeoTrixError::Io(e.to_string()))
}

#[command]
pub fn write_file(path: String, content: String) -> Result<(), NeoTrixError> {
    std::fs::write(&path, &content).map_err(|e| NeoTrixError::Io(e.to_string()))
}
...
#[command]
pub fn cmd_project_open(path: String) -> Result<String, NeoTrixError> {
    let dir = std::path::Path::new(&path);
    if !dir.exists() {
        return Err(NeoTrixError::Path { path: std::path::PathBuf::from(&path), detail: "Path does not exist".to_string() });
    }
    if !dir.is_dir() {
        return Err(NeoTrixError::Path { path: std::path::PathBuf::from(&path), detail: "Path is not a directory".to_string() });
    }
...
#[command]
pub fn cmd_scan_files(path: String, pattern: Option<String>) -> Result<Vec<String>, NeoTrixError> {
    let dir = std::path::Path::new(&path);
    if !dir.exists() {
        return Err(NeoTrixError::Path { path: std::path::PathBuf::from(&path), detail: "Path does not exist".to_string() });
    }
...
fn scan_files_recursive(dir: &std::path::Path, files: &mut Vec<String>, max_depth: u32) -> Result<(), NeoTrixError> {
