...
/// 从 Tauri 全局 GatewayV2 池子获取 LLM provider，注入 NeoCodexAgent。
fn get_gateway_provider() -> Option<std::sync::Arc<dyn neotrix::neotrix::nt_io_provider::LlmProvider>> {
    super::provider_cmds::get_gateway().ok().map(|gw| gw.clone() as std::sync::Arc<dyn neotrix::neotrix::nt_io_provider::LlmProvider>)
}
...
pub async fn neocodex_send_message_stream(
    ...
    let agent = match agent_guard.as_mut() {
        ...
        None => {
            ...
            if let Some(gw) = get_gateway_provider() { a.set_gateway_provider(gw); }
            *agent_guard = Some(a);
