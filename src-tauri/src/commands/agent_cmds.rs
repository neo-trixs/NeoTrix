...
#[command]
pub fn cmd_agent_stop() -> Result<(), NeoTrixError> {
    let mut state = AGENT_RUNNING.lock().map_err(|e| NeoTrixError::Brain(e.to_string()))?;
...
#[command]
pub fn cmd_agent_status() -> Result<AgentStatus, NeoTrixError> {
    let state = AGENT_RUNNING.lock().map_err(|e| NeoTrixError::Brain(e.to_string()))?;
