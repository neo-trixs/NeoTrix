...
impl WorkflowPluginImpl {
    ...
    fn save_workflows(&self) -> Result<(), DomainError> {
        ...
        std::fs::write(&self.db_path, serde_json::to_string_pretty(&*workflows).unwrap_or_default())
...
impl DomainPlugin for WorkflowPluginImpl {
    ...
    fn description(&self) -> &str { "工作流：DAG 编排、LLM 步骤、工具调用" }

    fn actions(&self) -> Vec<ActionSpec> {
        vec![
            ActionSpec { name: "list".into(), description: "列出所有工作流".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "get".into(), description: "获取工作流详情".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "create".into(), description: "创建工作流".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "update".into(), description: "更新工作流".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "delete".into(), description: "删除工作流".into(), params: vec![], returns: "Value".into() },
        ...
        match action {
            ...
            "get" => {
                ...
                match workflows.get(id) {
                    ...
                    None => Err(DomainError { code: "NOT_FOUND".into(), message: format!("Workflow {} not found", id), recoverable: true }),
                }
            }
            "create" => {
                let name = args.get("name").and_then(|v| v.as_str()).unwrap_or("Untitled");
                let description = args.get("description").and_then(|v| v.as_str()).unwrap_or("");
                let steps: Vec<WorkflowStep> = args.get("steps").and_then(|v| serde_json::from_value(v.clone()).ok()).unwrap_or_default();
            ...
            "update" => {
                ...
                let workflow = workflows.get_mut(id)
                    .ok_or_else(|| DomainError { code: "NOT_FOUND".into(), message: format!("Workflow {} not found", id), recoverable: true })?;
                ...
                if let Some(steps) = args.get("steps").and_then(|v| v.as_array()) {
                    workflow.steps = serde_json::from_value(serde_json::json!(steps)).unwrap_or_default();
                }
            ...
            "run" => {
                let workflow_id = args.get("workflow_id").and_then(|v| v.as_str())
                    .ok_or_else(|| DomainError { code: "INVALID_ARGS".into(), message: "missing 'workflow_id'".into(), recoverable: true })?;
                ...
                // 验证工作流存在
                {
                    ...
                    if !workflows.contains_key(workflow_id) {
                        return Err(DomainError { code: "NOT_FOUND".into(), message: format!("Workflow {} not found", workflow_id), recoverable: true });
                    }
                }
            ...
            "status" => {
                let run_id = args.get("run_id").and_then(|v| v.as_str())
                    .ok_or_else(|| DomainError { code: "INVALID_ARGS".into(), message: "missing 'run_id'".into(), recoverable: true })?;
                let runs = self.runs.lock().map_err(|e| DomainError {
                    code: "LOCK_ERROR".into(), message: e.to_string(), recoverable: true
                })?;
                match runs.get(run_id) {
                    Some(r) => Ok(serde_json::json!(r)),
                    None => Err(DomainError { code: "NOT_FOUND".into(), message: format!("Run {} not found", run_id), recoverable: true }),
                }
            }
            "cancel" => {
                let run_id = args.get("run_id").and_then(|v| v.as_str())
