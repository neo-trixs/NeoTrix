...
impl DomainPlugin for MemoryPlugin {
    ...
    fn actions(&self) -> Vec<ActionSpec> {
        vec![
            ActionSpec { name: "list".into(), description: "列出记忆".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "search".into(), description: "搜索记忆".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "clear".into(), description: "清空记忆".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "stats".into(), description: "统计信息".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "timeline".into(), description: "时间线".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "export".into(), description: "导出记忆".into(), params: vec![], returns: "Value".into() },
            ActionSpec { name: "import".into(), description: "导入记忆".into(), params: vec![], returns: "Value".into() },
        ]
    }
    ...
    fn call(&self, action: &str, args: serde_json::Value) -> Result<serde_json::Value, DomainError> {
        ...
        match action {
            ...
            "stats" => {
                ...
                let avg_confidence: f64 = conn.query_row("SELECT AVG(confidence) FROM memories", [], |row| row.get(0))
                    .unwrap_or(0.0);
                
                let total_size: i64 = conn.query_row("SELECT SUM(LENGTH(content)) FROM memories", [], |row| row.get(0))
                    .unwrap_or(0);
            ...
            "export" => {
                ...
                let memories: Vec<serde_json::Value> = stmt.query_map([], |row| {
                    Ok(serde_json::json!({
                        "id": row.get::<_, String>(0)?,
                        "kind": row.get::<_, String>(1)?,
                        "content": row.get::<_, String>(2)?,
            ...
            "import" => {
                let content = args.get("content")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| DomainError { code: "INVALID_ARGS".into(), message: "missing 'content'".into(), recoverable: true })?;
                
                let data: serde_json::Value = serde_json::from_str(content)
                    .map_err(|e| DomainError { code: "PARSE_ERROR".into(), message: format!("JSON 解析失败: {}", e), recoverable: true })?;
                
                let memories = data.get("memories")
                    .and_then(|v| v.as_array())
                    .ok_or_else(|| DomainError { code: "INVALID_DATA".into(), message: "missing 'memories' array".into(), recoverable: true })?;
                
                let mut imported = 0;
                for mem in memories {
                    let default_id = uuid::Uuid::new_v4().to_string();
                    let id = mem.get("id").and_then(|v| v.as_str()).unwrap_or(&default_id);
                    let kind = mem.get("kind").and_then(|v| v.as_str()).unwrap_or("unknown");
                    let content = mem.get("content").and_then(|v| v.as_str()).unwrap_or("");
                    let confidence = mem.get("confidence").and_then(|v| v.as_f64()).unwrap_or(0.5);
