...
use super::{DomainPlugin, DomainCall, DomainResponse, DomainInfo, DomainEvent, DomainError, ok, err};
...
impl DomainRegistry {
    ...
    /// 注册域插件 (带初始化)
    pub fn register_with_init(&mut self, mut plugin: Box<dyn DomainPlugin>) -> Result<(), DomainError> {
        plugin.init()?;
    ...
    pub fn call(&self, request: DomainCall) -> DomainResponse {
        let DomainCall { domain, action, args } = request;
    ...
    pub fn list(&self) -> Vec<DomainInfo> {
        self.plugins.iter().map(|p| DomainInfo {
