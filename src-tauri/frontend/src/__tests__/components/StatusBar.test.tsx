...
describe("StatusBar", () => {
  ...
  it("has a busy class on the container when agent is busy", () => {
    ...
    expect(container.querySelector('[data-testid="status-bar"]')).toHaveAttribute("data-busy", "true");
  });
  ...
  it("shows active class on terminal button when terminal is visible", () => {
    render(<StatusBar {...defaultProps} showTerminal={true} />);
    const terminalBtn = screen.getByRole("button", { name: "Toggle terminal" });
    expect(terminalBtn.className).toContain("active");
  });
});
