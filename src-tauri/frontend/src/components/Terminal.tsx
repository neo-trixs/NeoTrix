...
export function closeTerminal(sessionId: string) {
  ...
  if (!entry) return;
  if (entry.ptyId) {
    invoke("pty_close", { session_id: entry.ptyId }).catch(() => {});
  }
  entry.term.dispose();
...
export function resizeTerminal(sessionId: string, cols: number, rows: number) {
  const entry = terminalRegistry.get(sessionId);
  if (entry?.ptyId) {
    invoke("pty_resize", { session_id: entry.ptyId, cols, rows }).catch(() => {});
  }
}
    ...
    async function init() {
      ...
      invoke<string>("pty_spawn")
        .then((ptyId) => {
          ...
          term.onData((data) => {
            invoke("pty_write", { session_id: ptyId, data }).catch(() => {});
          });

          term.onResize(({ cols, rows }: { cols: number; rows: number }) => {
            invoke("pty_resize", { session_id: ptyId, cols, rows }).catch(() => {});
          });
