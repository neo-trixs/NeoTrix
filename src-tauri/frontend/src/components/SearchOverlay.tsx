...
function highlightText(text: string, query: string): React.ReactNode {
  ...
  while (pos !== -1) {
    ...
    parts.push(<mark key={pos} className={styles.searchMatch}>{text.slice(pos, pos + q.length)}</mark>);
    idx = pos + q.length;
...
const SearchOverlay: React.FC = () => {
      ...
      if (targetTimestamp) {
        for (const el of messageEls) {
          if (Number(el.getAttribute("data-message-timestamp")) === targetTimestamp) {
            ...
            (el as HTMLElement).style.outline = "2px solid var(--nt-primary)";
  ...
  useEffect(() => {
    ...
    const selectedEl = listRef.current.querySelector(`.${styles.resultItemSelected}`) as HTMLElement | null;
    if (selectedEl) {
    ...
    <div className={styles.overlay} onMouseDown={(e) => {
      ...
      <div className={`${styles.panel} glass-panel`}>
        <div className={styles.inputWrap}>
          ...
          />
          {searchQuery && (
            <button className={styles.clear} onClick={() => setSearchQuery("")} tabIndex={-1}>
          ...
          <span className={styles.count}>{totalCount > 0 ? `${totalCount} result${totalCount !== 1 ? "s" : ""}` : ""}</span>
        </div>

        <div className={styles.hints}>
          <span className={styles.hintKey}>↑↓</span> Navigate
          <span className={styles.hintKey} style={{ marginLeft: 12 }}>↵</span> Jump
          <span className={styles.hintKey} style={{ marginLeft: 12 }}>Esc</span> Close
        </div>
        ...
        <div className={styles.results} ref={listRef}>
          ...
          {groupedResults.map(([sessionIndex, group]) => (
            <div key={sessionIndex} className={styles.group}>
                ...
                return (
                  ...
                  >
                    <div className={styles.resultHeader}>
                      <span className={styles.resultRole} style={{ color: roleColor }}>
                        {SEARCH_ROLES[result.message.role] || result.message.role}
                      </span>
                      <span className={styles.resultSessionTag}>{group.sessionName}</span>
                    </div>
