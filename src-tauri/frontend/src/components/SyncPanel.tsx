...
const SyncPanel: React.FC = () => {
  ...
  const initSync = useCallback(async () => {
    try {
      await invoke("sync_init", { discovery_port: 42069, sync_port: 42070, local_root: localRoot || "~/.neotrix" });
      setStatus("ready");
    } catch (e) {
      setStatus(`error: ${e}`);
    }
  ...
  const discover = useCallback(async () => {
    setScanning(true);
    try {
      const result = await invoke<Peer[]>("sync_discover", { duration_ms: 3000 });
      setPeers(result);
    } catch (e) {
  ...
  const addPair = useCallback(async (peerId: string) => {
    try {
      await invoke("sync_add_pair", { peer_id: peerId, localPath: localRoot || "~/.neotrix" });
      await listPairs();
    } catch (e) {
  ...
  const removePair = useCallback(async (peerId: string) => {
    try {
      await invoke("sync_remove_pair", { peer_id: peerId });
      await listPairs();
    } catch (e) {
  ...
  const preview = useCallback(async (peerId: string) => {
    try {
      const result = await invoke<SyncDiff>("sync_preview", { peer_id: peerId });
      setDiff(result);
    } catch (e) {
  ...
  const startSync = useCallback(async (peerId: string) => {
    setSyncing(true);
    try {
      const bytes = await invoke<number>("sync_start", { peer_id: peerId });
