...
function renderTokens(tokens: Token[], findQuery?: string, activeIdx?: number): React.ReactNode[] {
  ...
  for (const token of tokens) {
    ...
    while ((idx = lower.indexOf(queryLower, start)) !== -1) {
      ...
      result.push(
        <mark key={globalKey++} className={`${styles.ceFindMark}${isActive ? ` ${styles.ceFindActive}` : ""}`}>
          {matchText}
        </mark>
      );
...
const CodeEditor: React.FC<CodeEditorProps> = ({ filePath, initialContent, language, onSave, onClose }) => {
  ...
  const renderedLines = useCallback(() => {
    ...
    return lines.current.map((line, i) => {
      ...
      return (
        <div key={i} className={styles.ceHighlightLine}>
          {guides.length > 0 && (
            <span className={styles.ceIndentGuides} aria-hidden="true">
              {guides.map((col) => (
                <span key={col} style={{ left: `${col}ch` }} className={styles.ceIndentGuide} />
              ))}
            </span>
          )}
          {tokensWithFind}
  ...
  return (
    <div className={`${styles.codeEditor} glass-panel`}>
      <div className={styles.ceHeader}>
        <div className={styles.ceFileInfo}>
          <span className={styles.ceFileIcon}>{"</>"}</span>
          <span className={styles.ceFileName} title={filePath}>
            {dirty && <span className={styles.ceDirty}>* </span>}
            {fileDir ? (
              <>
                <span className={styles.ceFileDir}>{fileDir}/</span>
                <span className={styles.ceFileBase}>{fileName}</span>
              </>
            ) : (
              fileName
            )}
          </span>
          {language && <span className={styles.ceLangBadge}>{language}</span>}
        </div>
        <div className={styles.ceActions}>
          ...
          <button
            className={`${styles.ceBtn} ${styles.ceBtnSave}`}
            onClick={handleSave}
          ...
          {onClose && (
            <button className={`${styles.ceBtn} ${styles.ceBtnClose}`} onClick={onClose} title="Close (Cmd+W)">
      ...
      {findOpen && (
        <div className={styles.ceFindBar}>
          <div className={styles.ceFindInputWrap}>
            <svg className={styles.ceFindIcon} width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
              <circle cx="5" cy="5" r="3.5" /><line x1="7.5" y1="7.5" x2="10.5" y2="10.5" />
            </svg>
            <input
              ref={findInputRef}
              className={styles.ceFindInput}
              type="text"
              value={findQuery}
          ...
          <span className={styles.ceFindCount}>
            {findQuery ? `${findCurrentIdx}/${findMatchCount}` : ""}
          </span>
          <div className={styles.ceFindActions}>
            <button className={styles.ceFindBtn} onClick={findPrev} disabled={!findQuery || findMatchCount === 0} title="Previous (Shift+Enter)">
              <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor"><path d="M5 8L0 2h10z" /></svg>
            </button>
            <button className={styles.ceFindBtn} onClick={findNext} disabled={!findQuery || findMatchCount === 0} title="Next (Enter)">
              <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor"><path d="M5 2l5 6H0z" /></svg>
            </button>
            <button className={`${styles.ceFindBtn} ${styles.ceFindBtnClose}`} onClick={() => { setFindOpen(false); textareaRef.current?.focus(); }} title="Close (Esc)">
      ...
      <div className={styles.ceBody}>
        <div className={styles.ceGutter} ref={gutterRef}>
        ...
        <div className={styles.ceCodeArea} ref={codeAreaRef}>
          <textarea
            ref={textareaRef}
            ...
            wrap="off"
          />
          <pre className={styles.ceHighlightLayer} aria-hidden="true">
            {renderedLines()}
          </pre>
        </div>
      </div>

      <div className={styles.ceStatusBar}>
        <span className={styles.ceStatusItem}>Ln {cursorLine}, Col {cursorCol}</span>
        <span className={styles.ceStatusItem}>Lines: {lineCount}</span>
        <span className={styles.ceStatusItem}>{language || "plain"}</span>
        <span className={`${styles.ceStatusItem} ${styles.ceStatusRight}`}>
