...
## 🚨 完整待修复清单 (2026-07-01, External Absorption Cycle 3 收官)

> 来源: `docs/0-ARCHITECTURE/BLIND_SPOT_CROSS_REFERENCE_2026-07-01.md`
> 3轮吸收循环: 30+领域, 40+来源, 28+盲点发现, 9层架构交叉分析
...
| ID | 问题 | 影响层 | 状态 | 说明 |
|----|------|--------|------|------|
| EA3-P1-01 | **Telemetry 连线关键路径** | L1-L9 | ⬜ | Tracer已存在但零消费者→插入GatewayV2/engine_core |
| EA3-P1-02 | **Adaptive RAG 连线KB** | L3 | ⬜ | 评分/重写/回退未接入KB search() |
| EA3-P1-03 | **数据合成接入SEAL** | L8 | 🔶 代码已写 | data_synthesis.rs(600行37测试)待接入pipeline |
| EA3-P1-04 | **编排效率基准** | L7 | ⬜ | nt_cap_orch_graph无可量化性能基线 |

### 🟨 P2 — 优化 (可推迟)

| ID | 问题 | 影响层 | 状态 | 说明 |
|----|------|--------|------|------|
| EA3-P2-01 | SkillForge动态技能路由 | L7 | ⬜ | 4阶段技能生命周期+11思考透镜 |
| EA3-P2-02 | DSPy式提示优化(GEPA/MIPROv2) | L8 | ⬜ | 依赖P1-03数据合成 |
| EA3-P2-03 | ANE直接编程路径(ANEForge) | L0 | ⬜ | Apple Core AI + 90μs fused dispatch |
| EA3-P2-04 | 黑板图模式整合E8 | L4/L7 | ⬜ | VulnClaw状态空间+证据门 |
| EA3-P2-05 | 宪法CVP价值电位 | L8 | ⬜ | 隐藏状态→优先级边界(AUROC 0.91) |

### 🟦 已知预存故障 (非回归, 32个)

| 类别 | 数量 | 根因 |
|------|------|------|
| VSA/HyperCube 浮点精度 | ~9 | FHRR余弦相似度环境差异 |
| SEAL pipeline 环境依赖 | ~5 | git/bash/shell 命令可用性 |
| GWT/意识/IIT/FEP 随机性 | ~4 | 蒙地卡洛随机种子敏感 |
| MCP/git/proxy 集成 | ~14 | 外部服务可用性 |

### ⬜ 阻塞的外部依赖

| 问题 | 阻碍者 |
|------|--------|
| macOS 代码签名/公证 | Apple Developer Program 账号 |
| Windows EV 签名 | EV Code Signing 证书 |
| CDN 部署 + updater 二进制托管 | 托管基础设施 |
```
