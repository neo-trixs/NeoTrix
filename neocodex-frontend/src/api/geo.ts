...
export function geoPoints(limit?: number, source?: string): Promise<GeoPoint[]> {
  return invoke('kb_geo_points', { limit: limit ?? null, source: source ?? null })
}
...
export function geoPointsPack(limit?: number, source?: string): Promise<GeoPoint[]> {
  return invoke('kb_geo_points_pack', { limit: limit ?? null, source: source ?? null })
}

/** 地理索引统计 (总数, 有国家数) */
export function geoStats(): Promise<[number, number]> {
  return invoke('kb_geo_stats', {})
}

/** 地图分层摘要 — 各数据源计数，前端据此决定加载策略 */
export function geoLayers(): Promise<GeoLayerSummary[]> {
  return invoke('kb_geo_layers', {})
}
...
export function geoElevations(limit?: number): Promise<GeoElevationPoint[]> {
  return invoke('kb_geo_elevations', { limit: limit ?? null })
}
...
export function trajectoryAdd(
  ...
  return invoke('kb_trajectory_add', { id, name, kind, points, west: bbox[0], south: bbox[1], east: bbox[2], north: bbox[3], distance_km })
}

/** 查询所有轨迹 */
export function trajectoryQuery(): Promise<TrajectoryRecord[]> {
  return invoke('kb_trajectory_query', {})
}
...
export function geoOfflinePack(
  ...
  return invoke('kb_geo_offline_pack', { bbox, name })
}