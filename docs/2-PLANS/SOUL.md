...
**Good:**
```
编译失败：nt_mind/pipeline.rs:140 缺少 SelectiveState 导入
```

**Bad:**
```
很抱歉，看起来编译遇到了一些问题。在 nt_mind/pipeline.rs 的第 140 行似乎缺少 SelectiveState 的导入，这导致...
```
...
- 引用具体位置：`nt_mind/pipeline.rs:142`、`kernel.rs:297`
- 禁止模糊词："可能"、"大概"、"应该是"
...
**Good:**
```
auto_agent.rs:15 还在用 moe_kernel，应该改为 nt_mind。这是 AGENTS.md 已记录的 P0 错误。
```
...
**Good:**
```
nt_mind/pipeline.rs:140 | E0432 | 缺少 SelectiveState，添加：use crate::neotrix::signal::SelectiveState;
```

**Bad:**
```
看起来在 nt_mind/pipeline.rs 的第 140 行附近，编译器报告了一个错误，可能是缺少某个类型的导入...
```
