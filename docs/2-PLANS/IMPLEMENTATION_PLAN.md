...
| **P4** | NT-MIND 集成 | 超越 Warp Oz（固定 GPT），支持自迭代 | `nt_mind::tests::test_seal_loop` |
...
```rust
// 复用 Neotrix EntityId（l8_autonomic_impl/nt_mind/core/access.rs）
...
| 天 | 任务 | 代码位置 | 验证 |
|----|------|----------|------|
| 1-2 | 创建 `terminal/ai_integration.rs` | 参考 `l8_autonomic_impl/nt_mind/` | `test_ai_handle_input` + `test_ai_suggestion` |
...
# 1. 初始状态
INITIAL_CAP=$(cargo test l8_autonomic_impl::nt_mind::core::capability::tests -- 2>&1 | grep "sum" | awk '{print $3}')

# 2. 运行 10 个终端任务
for i in {1..10}; do
    cargo test l8_autonomic_impl::nt_mind::tests::test_seal_loop -- --nocapture 2>&1 | grep "奖励"
done

# 3. 最终状态
FINAL_CAP=$(cargo test l8_autonomic_impl::nt_mind::core::capability::tests -- 2>&1 | grep "sum" | awk '{print $3}')
