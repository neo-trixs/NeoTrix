...
| 状态 | `pub mod moe_experts` 已注释 | `pub mod nt_mind` 活跃开发 |
...
**已实现功能**：
- `run_seal_loop()`：`l8_autonomic_impl/nt_mind/pipeline.rs`
- `ReasoningBank`：体验记忆存储（`l8_autonomic_impl/nt_mind/cortex_memory/`）
...
- **位置**：`l8_autonomic_impl/nt_mind/cortex_memory/`
...
- 当前实现：`l8_autonomic_impl/nt_mind/pipeline.rs`
...
```
src/neotrix/
├── l8_autonomic_impl/nt_mind/      # 推理大脑
│   ├── core/capability.rs          # CapabilityVector, KnowledgeSource, AbsorptionRecord
│   ├── brain_impl.rs               # SelfEdit, generate_tool_calls, infer_task_type
│   ├── pipeline.rs                 # ReasoningBrain, run_seal_loop(), apply_self_edit()
│   ├── cortex_memory/              # ReasoningBank 体验记忆
│   └── mod.rs                      # 模块导出
├── kernel/                 # 内核（11子模块）
│   ├── types.rs           # 核心类型定义
│   ├── guardrails.rs      # 安全护栏
...
- **2026-04-25**：`l8_autonomic_impl/nt_mind/` 拆分完成
