...
Located in `l8_autonomic_impl/nt_mind/pipeline.rs` — a 34-stage pipeline executed iteratively:
...
Located in `l8_autonomic_impl/nt_mind/attention_router.rs` — Global Workspace Theory-inspired attention routing with specialist modules competing for the global workspace through salience computation.
...
Located in `l8_autonomic_impl/nt_mind/benchmark.rs` — classifies agent reasoning trajectories into StrangeAttractor (chaotic), LimitCycle (oscillation), or NormalDiffusion (healthy convergence) dynamics.
