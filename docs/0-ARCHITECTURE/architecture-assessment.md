...
| l8_autonomic_impl/nt_mind/ | 62 | 20,770 | SEAL loop, goal loop, pipeline, capability vector, absorption, self-iteration, context artifacts | core, world_model |
...
| neotrix/orchestrator/ | 7 | 914 | Task decomposition, PlannerNode/WorkerNode/CriticNode pipeline | core, l8_autonomic_impl/nt_mind |
...
| world_model.rs | 1 | 732 | Latent state expert prediction, capability forecasting | core, l8_autonomic_impl/nt_mind |
...
1. **Massive `nt_mind/` module** — 62 files, 20,770 LOC (27% of codebase). This module is a monolith in all but file count: the SEAL loop, goal loop, pipeline, self-iteration, context artifacts, and engine are tightly coupled through shared `SelfIteratingBrain` state. No clear module boundary between "loop orchestration" and "iteration logic".
...
Extract the SEAL loop orchestrator from `SelfIteratingBrain` into a standalone `SealLoop` struct. Split `nt_mind/` into three independent subcrates:
