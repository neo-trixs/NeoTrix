...
NeoTrix has undergone substantial architectural modernization in the current cycle. The HC (HyperCube-Consciousness) series added 8 new core modules across the VSA engine, GWT routing, and hypercube bridge layers. R2 and R3 refactoring series cleaned up module splits, permissions, and dead code. The project is structurally sound at Layers 1-2 (core and l8_autonomic_impl/nt_mind) with well-defined zero-dependency boundaries.
...
Layer 2: l8_autonomic_impl/nt_mind/ (nt_mind subsystem, ~12,000+ LOC)
...
```
core/  ←  l8_autonomic_impl/nt_mind/  ←  agent/  ←  neotrix/*
  |           |                    |
  |           +-- goal_loop/       +-- MCP tools (rmcp 0.5)
  |           +-- self_iterating/  +-- sub-agent dispatch
  |           +-- memory/          +-- swarm modes (5)
  |
  +-- hypercube/  ──→  consciousness_bridge.rs  ──→  l8_autonomic_impl/nt_mind/
  +-- consciousness/  ──→  attention_router.rs  ──→  l8_autonomic_impl/nt_mind/
```

**Key properties:**
- Layer 1 (core/) has zero external runtime dependencies. This invariant is enforced by design and holds.
- Layer 2 depends only on core/ + tokio. No circular dependencies detected between core/ and l8_autonomic_impl/nt_mind/.
- Layer 3 (agent/) depends on core/ but does not import from l8_autonomic_impl/nt_mind/ directly -- it accesses brains through trait objects (BrainProvider, EngineProvider in core/traits.rs).
...
| l8_autonomic_impl/nt_mind/ | core/ | Internal struct/trait usage | None |
...
| 3 | `neotrix/l8_autonomic_impl/nt_mind/cortex_memory/` | 992 | nt_mind | MEDIUM -- single large file → split to dir |
| 4 | `core/knowledge.rs` | 964 | core | LOW -- data model, stable |
| 5 | `neotrix/l8_autonomic_impl/nt_mind/loop_impl/` | 959 | nt_mind | MEDIUM -- SEAL loop core logic |
| 6 | `neotrix/l8_autonomic_impl/nt_mind/brain_impl.rs` | 955 | nt_mind | LOW -- well-structured |
| 7 | `neotrix/l8_autonomic_impl/nt_mind/` | 913 | nt_mind | LOW |
| 8 | `neotrix/project_manager.rs` | 904 | neotrix | LOW -- declining usage |
| 9 | `main.rs` | 873 | — | LOW -- entry point, stable |
| 10 | `neotrix/l8_autonomic_impl/nt_mind/tests/` | 863 | nt_mind | MEDIUM -- test concentration |
...
legacy_kernel.rs consumers (from grep):
  ...
  neotrix/l8_autonomic_impl/nt_mind/ (nt_mind layer)
...
- Within `neotrix/`, three sub-clusters dominate:
  ...
  - `l8_autonomic_impl/nt_mind/`: 42 sub-modules, ~12,000+ LOC
...
2. `neotrix-ra` (nt_mind) -- largest brain logic. Dependent on core/ only.
...
| neotrix/l8_autonomic_impl/nt_mind/tests/ | 49 | Dedicated test directory |
...
| neotrix/l8_autonomic_impl/nt_mind/context_artifacts/mod.rs | 24 | Inline unit tests |
...
| `l8_autonomic_impl/nt_mind/code_graph.rs` | Large dependency graph resolution | Performance |
| `l8_autonomic_impl/nt_mind/code_graph_executor.rs` | Full SEAL loop execution | Performance |
...
Split 2: neotrix-reasoning (42 files, ~12K LOC)
  - Contains nt_mind/ and reasoning_kernel/
