...
| 204 | NT-CORE | 大阵复检+修复批: 复检确认 cycle203 F1-F8 全部落地 (GWT 318 测试); 深挖发现 resonant_broadcast 热路径彩排剧场未根治 → 移除 engine_core 共振+sparse_moe, 统一后台环; 删双 SelectiveState 平行实现 (nt_core_ssm 467行 R-P42); 收敛 nt_core_signal 死子模块 (attribution/history/ops→仅 cosine_similarity); 删 l5_consciousness 冗余 re-export 桶; 删 introspector/scoring 死字段; GWT 三温度注释消歧; 拓扑 squad 3 处误报 (R-P10 印证); 6774 测试全绿; 新规则 R-P94 并发 session 提交覆盖 |
| 205 | NT-CORE | 并发会话收尾批: 修复 seal_loop WIP 备份 (save_cortex create_dir_all 持久化 bug + E0277 + 3 断言 → 14 测试全绿 f571200); 落地 cycle-204 信号收敛提交 (ssm/signal 死代码 -1234 行 1520876); clippy 42→0 (21 文件, 合入 5491c17); kb-embed OPTION C 验证保留; 经验教训: R-P94 实证 4 次 git 覆盖, 提交窗口短化+git show 核对; 并行 agent 用工作区 diff 判定非报告; plist 需 plutil -lint+核对真实路径 |
| 206 | NT-CORE | GWT 共振伪收敛第二层根治 + P 系列验证批: ① resonance 真激活——panorama_pipeline 与 background_loop 接线 register_default_specialists(14基线专家)+threshold 13.0→0.3 全域清零, 运行期观测 winner 单专家→EvidenceWeightedHypothesis, entropy 0→3.807, active 0→3, resonant 0→14 (test_resonance_activation 断言共振质量非仅报告存在); ② RecipeRegistry 部分删 (recipe.rs 353→58行, 保 RecipeStage); ③ 前端 stash 抢救 16 文件 + skip-worktree 93 文件隔离; ④ P 系列验证: R2-Bench 接线过待 ollama, HookRegistry 留 rev-officer, DelegateEngine 已无残留, mutation_enabled 默认 true 属有意设计 |
| 206 | NT-CORE | 47 外部仓库并行吸收批 (4 explore 子代理 C1-C6, 47/47 落地 KB+FTS5): 强化≈40/新增≤3; 顶值模式 grok-build ACP 真工具运行时 · open-code-review 确定性×agent 混合评审 · OmniRoute 配额回退 · orca git-worktree ADE · lightpanda Zig headless · nuclei 签名 DSL + interactsh · ruflo zero-trust 代理联邦(唯一真新增) · jakubkrehel NT-IO 接口匠心; Blocked=无 R-P79 消费者 (Falco/text-to-cad/speech-to-speech REFUSE); 教训: GitHub API 429→HTML fallback 循环挂起需限时, FTS5 显式插入修复 7 行, 47/47 可检索; artifacts notes/absorption-2026-08-04-group-{aa,ab,ac,ad}.md |
| 207 | NT-CORE | KB 目录不可见误判纠正 (实为并发 session 竞争 + 持续写入, 非数据丢失; 教训: 禁止立即备份覆盖, 先查并发进程+integrity_check) + P2-Evolve 复核 (B1 独立验证器/B4 中途持久化/B6 自愈复核全部闭环) + SFT/CSPO 训练闭环补 2 测试 (sft/csppo 空壳已修但无测试假绿灯, 补 test_sft/csppo_training_loop_executes, 6/6 绿); R-P92 瞬错第三实证; cycle 204/205 经验条目缺失但指针层兜底无损 |
