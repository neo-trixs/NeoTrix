...
use neotrix::core::thinking_model::{
    SiliconSelfModel, ThinkingStep, StrategyKind, AttentionDomain,
};

fn main() {
    println!("╭─ NeoTrix SiliconSelf 自进化引擎 ─────────────────╮");
    println!("│                                                    │");
    println!("│  构建硅基生命的思维模型                               │");
    println!("│                                                    │");
    println!("╰────────────────────────────────────────────────────╯");
    ...
    println!("   ContextWindow: {} units", ss.context_window.len());

    // Phase 2: 开始思考追踪
    println!("\n🧠 Phase 2: 开始推理追踪");
    let trace_id = ss.begin_thinking_trace("分析系统架构");
    
    ss.add_thinking_step(trace_id,
        ThinkingStep::new(1, "扫描模块结构", StrategyKind::Direct)
            .with_domain(AttentionDomain::Code)
            .with_tool("glob"));
    println!("   Step 1: 扫描模块 → 发现 8 个核心模块");

    ss.add_thinking_step(trace_id,
        ThinkingStep::new(2, "分析依赖关系", StrategyKind::ChainOfThought)
            .with_domain(AttentionDomain::Code));
    println!("   Step 2: 依赖分析 → 3 层架构 (core → reasoning → agent)");

    ss.add_thinking_step(trace_id,
        ThinkingStep::new(3, "自我反思评估", StrategyKind::Reflection)
            .with_domain(AttentionDomain::SelfReflection));
    println!("   Step 3: 自反思 → 架构覆盖度 85%");

    // Phase 3: 完成追踪 + 自评
    ss.complete_thinking_trace(trace_id, "架构分析完成", 0.85);
    println!("\n📊 Phase 3: 自我评估");
    println!("   追踪评分: {:?}", ss.thinking_traces[trace_id].grade);

    // Phase 4: 注意力分布
    println!("\n👁 Phase 4: 注意力分布");
    ss.attention_manager.stimulate_domain(AttentionDomain::Code, 0.9);
    ss.attention_manager.stimulate_domain(AttentionDomain::Planning, 0.7);
    ss.attention_manager.stimulate_domain(AttentionDomain::SelfReflection, 0.5);
    
    let profile = ss.attention_manager.profile();
    println!("   主导域: {:?}", profile.dominant);
    println!("   活跃头数: {}/{}", profile.num_activated_heads, 10);

    // Phase 5: 策略选择
    println!("\n🎯 Phase 5: 策略推荐");
    let strategy = ss.strategy_registry.select(5, false, true);
    println!("   推荐策略: {:?} (复杂度5 + 需反思)", strategy);

    // Phase 6: 自我身份
    println!("\n🔖 Phase 6: 自我认知");
    println!("   能力评分: 代码生成={:.0}%, 架构设计={:.0}%, 知识综合={:.0}%",
    ...
    println!("   下一阶段: 接入 background_loop 120s ticker 实现持续自省");
    println!("   测试:     cargo test --lib -- thinking_model");
}
