...
fn main() {
    log::info!("╔════════════════════════════════════════════════════╗");
    log::info!("║  🧠 CortexMemory — 类人脑多维知识挖掘              ║");
    log::info!("║  存储架构: 时间线 × 7维度 × 多模态 × 联想关联     ║");
    log::info!("╚════════════════════════════════════════════════════╝");
    ...
    log::info!(
        "\n🧠 巩固: {} → 长期, 总数: {} 条",
        consolidated,
        cortex.stats().total_traces
    );

    // 报告
    log::info!("{}", cortex.report());
    ...
    for query in &queries {
        ...
        log::info!("  \"{}\" → {} 条", query, results.len());
        for (t, s) in &results {
            log::info!("    [{:.2}] {} [{}]", s, t.title, t.source_type);
        }
    }
    ...
    for cat in &chains {
        ...
        log::info!("  {:10} |{:<30}| {}", cat, bar, traces.len());
    }
