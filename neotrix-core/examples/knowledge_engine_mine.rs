...
fn main() {
    log::info!("╔══════════════════════════════════════════════════════════╗");
    log::info!("║  🧠 知识引擎 — 结构化长久知识库 + 文献搜索              ║");
    log::info!("║  架构: 知识条目 × 关系网络 × 多源文献搜索 × 持久化     ║");
    log::info!("╚══════════════════════════════════════════════════════════╝");
    ...
    log::info!("  ✅ 注入 {} 条核心知识", core_knowledge.len());
    ...
    // 因果关系
    if let Some(_agri) = core_ids.get(5) {
        if let Some(_indus) = core_ids.get(6) { // 实际 index 7 是工业革命
    ...
    for e in &wiki {
        log::info!("     • {} (重要度: {:.2})", e.title, e.importance);
    }
