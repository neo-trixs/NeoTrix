...
fn main() {
    log::info!("╔══════════════════════════════════════════════════════════════╗");
    log::info!("║  🔱 知识注入 — 从 JSON 批量加载 250+ 条知识                 ║");
    log::info!("╚══════════════════════════════════════════════════════════════╝");
    ...
    log::info!("📦 读取到 {} 条知识条目", entries.len());
    ...
    log::info!("\n📊 注入结果:");
    log::info!("  已存在(跳过): {}", skipped);
    log::info!("  新插入: {}", inserted);
    log::info!(
        "  累计: {} 条目, {} 关系",
    ...
    for i in 0..all.len().min(200) {
        for j in (i + 1)..all.len().min(i + 5) {
            // limit to 5 links per entry
    ...
    // Domain breakdown
    let domains = [
        "天界", "地界", "人界", "数学", "物理", "化学", "天文", "历史", "哲学", "生物", "地理",
        "科学", "文学", "艺术",
    ];
    log::info!("\n  标签统计:");
    for d in &domains {
