...
use neotrix::neotrix::stealth_net::config::load as cfg;
...
async fn main() {
    ...
    println!("╰──────────────────────────────────────────────╯");
    println!("  📄 配置: {}", neotrix::neotrix::stealth_net::config::config_file_path());

    // 1. 启动 Tor/Arti（后台安装）
    let tor = Arc::new(neotrix::neotrix::stealth_net::local_proxy::TorManager::new());
    ...
    let sys_proxy = Arc::new(neotrix::neotrix::stealth_net::system_proxy::SystemProxyManager::new());
    ...
    let mut engine = neotrix::neotrix::stealth_net::rules::RuleEngine::new();
    engine.load_rules(neotrix::neotrix::stealth_net::rules::china_bypass_rules());
    let proxy = neotrix::neotrix::stealth_net::local_proxy::LocalProxy::new()
    ...
    let pool = neotrix::neotrix::stealth_net::proxy_pool::global_pool();
    // 用户可在此处添加订阅地址
    ...
    tokio::spawn(async move {
        ...
        for _ in 0..3 {
            if neotrix::neotrix::stealth_net::local_proxy::TorManager::socks5_reachable().await {
                // 通过 Google 预热 Tor 电路
                let _ = neotrix::neotrix::stealth_net::local_proxy::tor_connect("www.google.com", 443).await;
                break;
            }
