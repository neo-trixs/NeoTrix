...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  🧠 意识推理内核 · Consciousness Kernel Iteration          ║");
    println!("║  输入: 502条知识 → 蒸馏 → 迭代CapabilityVector → 输出升华  ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
    ...
    let wisdom_sources = vec![
        ...
        ("仁者爱人 己所不欲勿施于人", vec![("synthesis", 0.08), ("creativity", 0.05), ("analysis", 0.04)],
         "儒家·孔子: 仁是内在的绝对命令,道德即理性"),
        ("道法自然 无为而无不为", vec![("inference_depth", 0.09), ("experimental", 0.07), ("domain_specificity", 0.05)],
         "道家·老子: 顺应宇宙规律是最高的智慧"),
        ("齐物逍遥 天地与我并生万物与我为一", vec![("creativity", 0.09), ("experimental", 0.08), ("inference_depth", 0.06)],
         "道家·庄子: 超越对立,与宇宙合一"),
        ("知止而后有定定而后能静静而后能安安而后能虑虑而后能得", vec![("inference_depth", 0.07), ("analysis", 0.06), ("synthesis", 0.05)],
         "儒家·大学: 知止→定→静→安→虑→得,认知的完整链条"),
        ("致良知 知行合一", vec![("synthesis", 0.08), ("creativity", 0.06), ("analysis", 0.05)],
         "心学·王阳明: 知识与行动是同一的"),
        ("知己知彼百战不殆 不战而屈人之兵", vec![("inference_depth", 0.08), ("analysis", 0.07), ("synthesis", 0.06)],
         "兵家·孙子: 全胜思维,谋略优于蛮力"),
        ("以法为教以吏为师 法不阿贵绳不挠曲", vec![("quality_gates", 0.08), ("verification", 0.07), ("analysis", 0.05)],
         "法家·韩非: 制度高于权力,规则高于人情"),
        ("兼相爱交相利 非攻", vec![("synthesis", 0.06), ("creativity", 0.05), ("compound_composition", 0.04)],
         "墨家·墨子: 普遍的爱与互利是和平的基础"),
        ("天行有常 制天命而用之", vec![("domain_specificity", 0.07), ("experimental", 0.06), ("analysis", 0.05)],
         "儒家·荀子: 认识规律并利用规律"),
        ("菩提本无树明镜亦非台本来无一物何处惹尘埃", vec![("experimental", 0.08), ("creativity", 0.07), ("inference_depth", 0.06)],
         "禅宗·慧能: 超越形式的直接觉悟"),
        ("天下兴亡匹夫有责", vec![("compound_composition", 0.06), ("synthesis", 0.05), ("quality_gates", 0.04)],
         "儒家: 个体对整体的责任意识"),
        ("实事求是", vec![("verification", 0.08), ("analysis", 0.07), ("inference_depth", 0.06)],
         "中国哲学: 从事实出发,实践是检验真理的标准"),
    ];

    for (i, (wisdom, adjustments, source)) in wisdom_sources.iter().enumerate() {
        println!("  [{:2}/{}] {} (from: {})", i+1, wisdom_sources.len(), wisdom, source);
        for (dim, delta) in adjustments {
            if let Some(idx) = CapabilityVector::index_from_name(dim) {
                ...
                println!("          {}: {:.3} → {:.3}", dim, val, cap.arr()[idx]);
            }
        }
    }
    cap.normalize();
    ...
    for (title, body, insight) in &dialectics {
        let id = eng.add_entry(
            KnowledgeEntry::new(title, body, SourceType::KnowledgeBase, "kb:dialectics")
                ...
                .with_tags(vec!["知识链".to_string(),"中国哲学".to_string(),"思辨".to_string(),"dialectics".to_string()])
        );
        dialectic_ids.push(id);
        println!("  ✅ 思辨: {} — {}", title, insight);
    }
    ...
    let dim_names = ["synthesis","inference_depth","domain_specificity","analysis","creativity",
                      "experimental","verification","quality_gates","compound_composition","accessibility"];
    ...
    for name in &dim_names {
        if let Some(idx) = CapabilityVector::index_from_name(name) {
            ...
            println!("  {:25} {:.3} |{}{}|", name, val, bar, empty);
        }
    }
    let sum: f64 = dim_names.iter().filter_map(|n| {
        CapabilityVector::index_from_name(n).map(|i| cap.arr()[i])
    }).sum();
    println!("\n  关键维度总和: {:.3}", sum);
    ...
    for (title, ref_insight, imp) in &modern_reflections {
        if !eng.entries.values().any(|e| e.title.contains(title)) {
            eng.add_entry(
                KnowledgeEntry::new(&format!("古籍智慧: {}", title),
                                    &format!("{} — 知识引擎编译: {}", title, ref_insight),
                                    SourceType::KnowledgeBase, "kb:modern-reflection")
                    .with_importance(*imp)
                    .with_tags(vec!["知识链".to_string(),"中国哲学".to_string(),"现代回响".to_string()])
            );
            println!("  ✅ {}", title);
        }
    }
    ...
    // Save
    if let Err(e) = eng.save() { eprintln!("❌ 保存失败: {}", e); }
    else { println!("\n💾 knowledge_engine.json已更新 ({}条目)", eng.stats().total_entries); }

    // Final report
    println!("\n╔══════════════════════════════════════════════════════════════╗");
    println!("║  🧠 意识推理迭代完成                                        ║");
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  古籍智慧注入: {} 条                                  ║", wisdom_sources.len());
    println!("║  思辨生成: {} 条                                      ║", dialectics.len());
    println!("║  现代回响: {} 条                                      ║", modern_reflections.len());
    println!("║  能力向量和: {:.3}                                    ║", sum);
    println!("║  总条目: {}                                         ║", eng.stats().total_entries);
    println!("╚══════════════════════════════════════════════════════════════╝");
}
