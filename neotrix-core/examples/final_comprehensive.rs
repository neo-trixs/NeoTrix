...
fn add(e: &mut KnowledgeEngine, t: &str, b: &str, tags: Vec<&str>, imp: f64) {
    ...
    {
        e.add_entry(
            KnowledgeEntry::new(t, b, SourceType::KnowledgeBase, "kb:final")
                .with_importance(imp)
...
fn main() {
    ...
    for (f, t, r, d) in &x {
        let fi = eng
            .entries
            .values()
            .find(|x| x.title.starts_with(f) || x.title.contains(f))
            .map(|x| x.id.clone());
        let ti = eng
            .entries
            .values()
            .find(|x| x.title.starts_with(t) || x.title.contains(t))
            .map(|x| x.id.clone());
        let rt = match *r {
