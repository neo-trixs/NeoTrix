...
pub fn demo_reasoning_bank() {
    ...
    for (task, reward) in &tasks {
        ...
        let memory = ReasoningMemory::new(task, TaskType::UIDesign, &[], *reward);
        bank.store(memory);
    ...
    // 检索相关记忆
    println!("\n检索与 \"UI 设计\" 相关的记忆:");
    let relevant = bank.retrieve_relevant("UI 设计", None, 3);
...
pub fn demo_rollback() {
    ...
    brain.brain.apply_self_edit(&negative_edit, None);
