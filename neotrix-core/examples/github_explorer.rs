...
fn main() {
    ...
    for (title, body, tags, imp) in &discovered {
        if !eng.entries.values().any(|x| x.title.contains(title) && title.len() > 8) {
            eng.add_entry(
                KnowledgeEntry::new(title, body, SourceType::KnowledgeBase, "kb:github-discovery")
                    .with_importance(*imp)
    ...
    println!("  ✅ 新增 {} 条GitHub资源发现", count);

    // Cross references
    let x = vec![
        ("chinese-poetry","全唐诗三百首","Related","唐诗大数据"),
        ("daozang-english","正统道藏三洞体系","Related","道藏英译项目"),
        ("my-TCM-textbook-website","八纲辨证","Related","中医教材数字版"),
        ("qimen-go","奇门遁甲预测法","Related","Go实现奇门排盘"),
        ("taisho-translation","大正藏结构详解","Related","大正藏英译"),
    ];
    for (f,t,r,d) in &x {
    ...
    println!("\n━━━ 发现的GitHub资源索引 ━━━");
    for (t,_,_,_) in &discovered { println!("  • {}","你可以在浏览器中打开查看项目详情"); }
}
