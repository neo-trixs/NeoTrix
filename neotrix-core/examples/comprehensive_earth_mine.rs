...
fn main() {
    ...
    log::info!(
        "  ✅ {} / {} 成功吸收",
        gh_result.success_count,
        gh_result.total_urls
    );
    ...
    let arxiv_result =
        arxiv_web_miner.mine_all(&arxiv_urls, &mut brain.brain, &mut brain.reasoning_bank);
    log::info!(
        "  ✅ {} / {} 成功吸收",
        arxiv_result.success_count,
        arxiv_result.total_urls
    );
    ...
    log::info!("  ✅ {} 个仓库深度分析", gh_deep_result.mined_count);
    ...
    for name in &tracked {
        if let Some(idx) = neotrix::neotrix::nt_mind::CapabilityVector::index_from_name(name) {
            ...
            log::info!("  {:20} {:5.3} |{}{}|", name, val, bar, empty);
        }
    }
...
fn kb_web_miner(
    work_dir: &PathBuf,
    brain: &mut neotrix::neotrix::nt_mind::self_iterating::ReasoningBrain,
