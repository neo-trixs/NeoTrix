...
fn add(eng: &mut KnowledgeEngine, t: &str, b: String, tags: Vec<&str>, imp: f64) {
    if !eng
        .entries
        .values()
        .any(|x| x.title.contains(t) && t.len() > 4)
    {
        eng.add_entry(
            KnowledgeEntry::new(t, &b, SourceType::KnowledgeBase, "kb:complete")
                .with_importance(imp)
...
fn main() {
    ...
    let nhx = vec![
        (
            ...
            format!(
                "倪海厦(1954-2012),美国汉唐中医学院院长,中医界最具影响力的经方家之一。\n\
        ...
        (
            ...
            format!(
                "天纪是倪海厦对\"天道规律\"的解读,包括易经占卜、紫微斗数命理、风水地理。\n\
          核心:人纪治已病,天纪治未病。\n\
    ...
    let gh = vec![
        (
            ...
            format!(
                "DaimaRuge/daozang-text 仓库包含明《正统道藏》及《续道藏》全文文本。\n\
    ...
    for (t, b) in gh {
        add(&mut eng, t, b, vec!["联网挖掘", "GitHub", "知识链"], 0.92);
    }
    log::info!("  ✅ GitHub开源古籍资源");
    ...
    for (f, t, r, d) in &x {
        ...
        if let (Some(ff), Some(tt)) = (fi, ti) {
            eng.add_relation(&ff, &tt, rt, 0.7, d);
        }
    }
    log::info!("  ✅ 交叉关系");
