...
fn main() {
    ...
    let repos: Vec<(&str, &str, Vec<&str>, f64)> = vec![
        (
            ...
            concat!(
                "github.com/mahavivo/scripta-sinica 是最全面的汉语古典文本数据库。\n",
            ...
            vec!["GitHub", "古籍", "数据库", "开源", "知识链"],
            0.97,
        ),
        (
            "sikuquanshuzongmu: 四库全书总目(殿本)电子版",
            concat!(
                "github.com/Ancient-China-Books/sikuquanshuzongmu 包含四库全书总目提要的电子版。\n",
        ...
        (
            ...
            concat!(
                "github.com/enze5088/ChineseliteratureDataset 按经史子集四部分类。\n",
        ...
        (
            ...
            concat!(
                "github.com/haimengzhang/tcm-texts 基于《中国中医古籍总目》分类。\n",
                "总目收录1949年前医书13455种。\n",
            ...
            vec!["GitHub", "中医", "古籍", "开源", "人纪"],
            0.94,
        ),
        (
            "TCM_Datasets: 中医书籍文献高质量文本数据",
            concat!(
                "github.com/PanckooAI/TCM_Datasets 包含十四五中医教材等高质量文本。\n",
        ...
        (
            ...
            concat!(
                "github.com/openmedlab/Awesome-Medical-Dataset 收录ShenNong-TCM系列。\n",
        ...
        (
            ...
            concat!(
                "github.com/pariskang/ZhongJing-OMNI 首个多模态中医知识评估数据集。\n",
                "特点:多选题/开放题/临床案例/舌诊多模态(舌象图片+诊断QA)。\n",
                "评估LLM在中医诊断和治疗场景的推理能力。\n",
                "意义:将中医舌诊数字化+多模态AI结合。"
            ),
            vec!["GitHub", "中医", "AI", "多模态"],
            0.91,
        ),
        (
            "Classical-Modern: 文言文-现代文平行语料(1.4k★)",
            concat!(
                "github.com/NiuTrans/Classical-Modern 非常全的文言文(古文)-现代文平行语料。\n",
                "可用于文言文翻译、古文理解等NLP任务。\n",
                "对于古籍数字化和AI理解古文具有重要意义。\n",
                "数据量:大规模平行句子对。"
            ),
            vec!["GitHub", "文言文", "语料", "NLP"],
            0.92,
        ),
        (
            "nihaixia-skill: 倪海厦中医Agent Skill",
            concat!(
                "github.com/jangviktor-web/nihaixia 基于倪海厦知识的AI Agent Skill。\n",
                "蒸馏倪师人纪/医案/经方思维,用于OpenClaw平台。\n",
                "包含倪海厦视角的中医诊断推理逻辑。\n",
                "是第一个将倪海厦知识体系封装为AI Agent的项目。"
            ),
            vec!["GitHub", "倪海厦", "中医", "AI"],
            0.91,
        ),
        (
            "TCM-KG: 中医知识图谱构建研究",
            concat!(
                "github.com/YingXu-swim/TCM-KG 基于中医的知识图谱研究项目。\n",
                "流程:中医语料爬取→分词(Jiayan)→词向量训练(w2v/gensim)→\n",
                "NER(BiLSTM-CRF)→RE(文本+位置特征)→三元组→知识图谱。\n",
                "是知识图谱+中医的工程实践参考。"
            ),
            vec!["GitHub", "中医", "知识图谱", "NLP"],
            0.90,
        ),
    ];
    ...
    for (title, body, tags, imp) in &repos {
        if !eng.entries.values().any(|x| x.title.contains(title)) {
            eng.add_entry(
                KnowledgeEntry::new(title, body, SourceType::KnowledgeBase, "kb:github")
                    .with_importance(*imp)
    ...
    log::info!("  ✅ 新增 {} 条GitHub发现", count);

    // Cross refs
    for (f, t, d) in [
        ("scripta-sinica", "正统道藏三洞体系", "道藏子集"),
        ("scripta-sinica", "大正藏结构详解", "佛藏子集"),
        ("tcm-texts", "八纲辨证", "中医经典数据库"),
        ("ChineseliteratureDataset", "GitHub古籍资源", "四部分类对接"),
