...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  🔬 知识蒸馏引擎 — 从 470 条知识提取核心逻辑链路           ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
    ...
    println!("📊 知识库: {} 条目, {} 关系", eng.stats().total_entries, eng.stats().total_relations);
    ...
    for (domain, count) in domain_list.iter().take(30) {
        ...
        println!("  {:20} |{}{}| {:>4}", domain, bar, empty, count);
    }
    ...
    for (title, body, imp) in &supplements {
        if !eng.entries.values().any(|e| e.title == *title) {
            eng.add_entry(KnowledgeEntry::new(title, body, SourceType::KnowledgeBase, "kb:distilled")
                .with_importance(*imp).with_tags(vec!["天界".to_string(),"科学".to_string(),"知识链".to_string()]));
            println!("  ✅ 补充: {}", title);
        }
    }
    ...
    let cosmos_refs = vec![
        ...
        ("银河系与太阳系","地球内部结构","Causes","太阳系→地球分层"),
        ("地核驱动地磁场","地球内部结构","Causes","地核产生磁场"),
    ];
    for (from, to, rel, desc) in &cosmos_refs {
        let rt = match *rel { "Causes"=>RelationType::Causes, _=>RelationType::Related };
        let f_id = eng.entries.values().find(|e|e.title.contains(from)).map(|e|e.id.clone());
        let t_id = eng.entries.values().find(|e|e.title.contains(to)).map(|e|e.id.clone());
    ...
    println!("\n📊 最终: {} 条目, {} 关系", eng.stats().total_entries, eng.stats().total_relations);

    // 输出 HTML
    let html = render_html_report(&chains);
    let html_path = PathBuf::from(&home).join(".neotrix").join("knowledge_chains.html");
...
fn render_html_report(chains: &[KnowledgeChain]) -> String {
    let mut html = String::from(r#"<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>知识逻辑链报告</title>
...
.tag-gold{background:#ffd43b33;color:#ffd43b;border:1px solid #ffd43b44}
    ...
    html.push_str(r#"<div class="footer">NeoTrix 知识蒸馏引擎 · 470 条目, 160 关系</div></body></html>"#);
    html
}
