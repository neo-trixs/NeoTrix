...
fn main() {
    ...
    eng.add_entry(
        KnowledgeEntry::new(&d_title, &d_body, SourceType::WebPage, "web:daozang")
            .with_importance(0.96)
            .with_tags(vec![
                "联网挖掘".to_string(),
                "道藏".to_string(),
                "知识链".to_string(),
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&f_title, &f_body, SourceType::WebPage, "web:tripitaka")
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&y_title, &y_body, SourceType::WebPage, "web:yijing")
            .with_importance(0.96)
            .with_tags(vec![
                "联网挖掘".to_string(),
                "易学".to_string(),
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&z_title, &z_body, SourceType::WebPage, "web:zhiwei")
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&x_title, &x_body, SourceType::WebPage, "web:xuanxue")
            .with_importance(0.93)
            .with_tags(vec![
                "联网挖掘".to_string(),
                "玄学".to_string(),
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&n_title, &n_body, SourceType::WebPage, "web:wuyun")
    ...
    .to_string();
    eng.add_entry(
        KnowledgeEntry::new(&q_title, &q_body, SourceType::WebPage, "web:qimen")
            .with_importance(0.95)
            .with_tags(vec![
                "联网挖掘".to_string(),
                "术数".to_string(),
    ...
    let urls = vec![
        (
            "https://raw.githubusercontent.com/cbeta-org/xml-p5/master/README.md",
            "CBETA XML-P5 电子佛典项目",
        ),
        (
            "https://raw.githubusercontent.com/wizardforcel/sicp-py-zh/master/README.md",
            "SICP 计算机程序构造与解释(中文)",
        ),
    ];
        ...
        match client.get(*url).send() {
            Ok(resp) => {
                if let Ok(text) = resp.text() {
                    ...
                    eng.add_entry(
                        KnowledgeEntry::new(label, &preview, SourceType::WebPage, url)
                            .with_importance(0.70)
                            .with_tags(vec!["联网挖掘".to_string(), "开源".to_string()]),
                    );
                    log::info!("✅ {}b", text.len());
                }
            }
    ...
    let mut cap = if let Ok(b) = neotrix::neotrix::nt_mind::ReasoningBrain::load() {
        ...
        neotrix::neotrix::nt_mind::CapabilityVector::default()
    };
    for (dim, delta) in &[
