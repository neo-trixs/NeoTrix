...
fn main() {
    ...
    if SelfEvolver::is_url(helmor_url) {
        log::info!("\n{}", "=".repeat(60));
        log::info!("处理 helmor 仓库: {}", helmor_url);
        log::info!("（已启用重试、超时、镜像fallback等容错机制）");

        match evolver.evolve_from_url(helmor_url) {
            Ok(reward) => {
                log::info!("✅ helmor 进化完成！奖励值: {:.4}", reward);
                log::info!("检查 ~/.neotrix/brain.json 查看变化");
            }
            Err(e) => {
                log::error!("❌ helmor 进化失败: {}", e);
                log::error!("提示: 请检查网络连接或尝试使用代理");
            }
        }
    }
    ...
    if SelfEvolver::is_url(arxiv_url) {
        log::info!("\n{}", "=".repeat(60));
        log::info!("处理 arXiv 论文: {}", arxiv_url);

        match evolver.evolve_from_url(arxiv_url) {
            Ok(reward) => {
                log::info!("✅ arXiv 进化完成！奖励值: {:.4}", reward);
                log::info!("检查 ~/.neotrix/brain.json 查看变化");
            }
            Err(e) => {
                log::error!("❌ arXiv 进化失败: {}", e);
            }
        }
    }
}
