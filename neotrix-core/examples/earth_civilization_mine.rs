...
fn main() {
    ...
    let urls = vec![
        ...
        (
            "https://en.wikipedia.org/wiki/Timeline_of_historical_events",
            "历史事件时间线",
        ),
        // 多维度时间概念
    ...
    for (i, (url, label)) in urls.iter().enumerate() {
        log::info!("[{}/{}] 🌍 {} <{}>", i + 1, urls.len(), label, url);
        ...
        if let Ok(content) = std::fs::read_to_string(&target_file) {
            ...
            if !edits.is_empty() {
                ...
                for (dim, delta) in &edits {
                    if let Some(idx) =
                        neotrix::neotrix::nt_mind::CapabilityVector::index_from_name(dim)
                    {
                ...
                log::info!("   ✅ 吸收 {} 条知识 (reward: {:.3})", edits.len(), reward);
    ...
    let memory = neotrix::neotrix::nt_mind::ReasoningMemory::new(
        ...
        neotrix::neotrix::nt_expert_routing::TaskType::CodeAnalysis,
        &[],
    ...
    // 显示结果
    log::info!("\n=== 挖掘报告 ===");
    log::info!("来源总数: {}", urls.len());
    log::info!("成功吸收: {}", success_count);
    log::info!("总奖励: {:.3}", total_reward);
    log::info!(
        "能力向量和: {:.3}",
        brain.brain.get_statistics().capability_sum
    );
