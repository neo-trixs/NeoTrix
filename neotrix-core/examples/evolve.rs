...
use neotrix::neotrix::nt_world_crawl::{
    CrawlStrategy, CrawlTopic, CrawlerConfig, SeedEntry, UnifiedCrawler,
};
use neotrix::{ReasoningBank, ReasoningBrain};

fn main() {
    log::info!("╔═══════════════════════════════════════════════════════════╗");
    log::info!("║     NeoTrix 自主进化守护进程 v0.1                         ║");
    log::info!("║     爬取 → 分类 → 吸收(brain+hypercube) → gap → 种子      ║");
    log::info!("║     Ctrl+C 停止                                          ║");
    log::info!("╚═══════════════════════════════════════════════════════════╝");
        ...
        if result.absorbed {
            ...
            if total_absorbed > 0 && total_absorbed % 20 == 0 {
                ...
                if !sparse.is_empty() {
                    ...
                    log::info!("  ↪ 自驱种子注入: {} 个稀疏域 → Wikipedia", sparse.len());
                    for t in &sparse {
                        log::info!("    - {}", t.name());
                    }
                }
            ...
            if empty_count >= 5 {
                ...
                if !sparse.is_empty() {
                    let seeds = seeds_for_sparse_topics(&sparse);
                    crawler.add_seeds(seeds);
                    log::info!("  ↪ frontier 空 → gap 驱动注入 {} 个种子", sparse.len());
                    empty_count = 0;
                } else {
                    log::info!("  ⏹ frontier 空, 无稀疏域可探索 → 停止");
                    break;
                }
            }
        }
        ...
        if result.is_heal_check {
            ...
            if !crawler.heal_history().is_empty() {
                let last = crawler
                    .heal_history()
                    .last()
                    .expect("heal history is empty");
                if last.applied {
                    log::info!("  动作: {}", last.action);
                }
            }
            log::info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        }
    ...
    log::info!("║  自主进化结束                                 ║");
    log::info!("╚═══════════════════════════════════════════════╝");
    log::info!("  运行时间:     {:.0}s ({:.1}min)", elapsed, elapsed / 60.0);
    log::info!("  总 cycle:     {}", cycle_count);
    log::info!("  总吸收:       {} 页", total_absorbed);
    log::info!("  自愈次数:     {}", crawler.heal_history().len());
    log::info!(
        "  能力向量和:   {:.4}",
        brain.capability.arr().iter().sum::<f64>()
    );
    log::info!(
        "  向量维度激活: {}/{}",
    ...
    log::info!("  超立方体条目: {}", router.bridge.hypercube.cell_count());
    log::info!("  Brain知识源:  {}", brain.custom_sources.len());
    log::info!("  Frontier:     {} 剩余", crawler.frontier_stats());

    // 自愈历史
    for (i, heal) in crawler.heal_history().iter().enumerate() {
        log::info!("  自愈 #{}: {}", i + 1, heal.action);
    }
...
fn civilization_seeds() -> Vec<SeedEntry> {
    vec![
        SeedEntry {
            url: "https://www.un.org/en/about-us/universal-declaration-of-human-rights".into(),
        ...
        SeedEntry {
            url: "https://plato.stanford.edu/entries/philosophy-law".into(),
            topic: CrawlTopic::PhilosophyAndEthics,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://plato.stanford.edu/entries/ethics".into(),
            topic: CrawlTopic::PhilosophyAndEthics,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://en.wikipedia.org/wiki/Rule_of_law".into(),
            topic: CrawlTopic::LawAndGovernance,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://en.wikipedia.org/wiki/Science".into(),
            topic: CrawlTopic::ScienceAndTechnology,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://en.wikipedia.org/wiki/History".into(),
            topic: CrawlTopic::HistoryAndArcheology,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://en.wikipedia.org/wiki/Economics".into(),
            topic: CrawlTopic::SocietyAndEconomics,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://en.wikipedia.org/wiki/Art".into(),
            topic: CrawlTopic::ArtsAndLiterature,
            depth: 1,
            enabled: true,
        },
        SeedEntry {
            url: "https://www.who.int/health-topics".into(),
        ...
        .map(|t| {
            let url = match t {
                ...
                CrawlTopic::PhilosophyAndEthics => {
                    "https://plato.stanford.edu/entries/moral-philosophy"
                }
...
fn print_current_state(
    ...
    log::info!("  时间:        {}s", elapsed);
    log::info!("  吸收:        {} 页", s.absorbed);
    log::info!("  Frontier:    {} 未处理", s.frontier_size);
    log::info!("  错误率:      {:.1}%", s.error_rate * 100.0);
    log::info!("  能力向量和:  {:.4}", cap_sum);
    log::info!(
        "  超立方体:    {} 条目",
    ...
    log::info!("  稀疏维度:    {}/8 (threshold >0.7)", sparse);
    for g in gaps.iter().filter(|g| g.sparsity_score > 0.7) {
        log::info!("    dim{}: sparsity={:.2}", g.dim_index, g.sparsity_score);
    }
}
