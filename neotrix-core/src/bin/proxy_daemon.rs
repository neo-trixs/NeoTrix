...
use neotrix::nt_shield_traffic::api_proxy::{ApiProxy, ApiProxyConfig};
...
async fn main() {
    ...
    // --- L7 API Proxy (traffic capture + GatewayV2 routing) ---
    let gateway = Arc::new(GatewayV2::new());
    let api_proxy = Arc::new(ApiProxy::new(ApiProxyConfig::default(), gateway));
    let ap = api_proxy.clone();
    tokio::spawn(async move {
        if let Err(e) = ap.start().await {
            eprintln!("[api-proxy] 退出: {}", e);
        }
    });
