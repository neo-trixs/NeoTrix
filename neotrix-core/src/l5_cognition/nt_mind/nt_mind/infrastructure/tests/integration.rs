...
mod tests {
    ...
    fn test_attention_to_gwt_to_hypercube_chain() {
        ...
        bridge.silicon.attention_manager.stimulate_domain(AttentionDomain::Code, 0.9);
        bridge.silicon.attention_manager.stimulate_domain(AttentionDomain::Planning, 0.7);
        ...
        let has_code = active_specialists.iter().any(|m| m.specialist_type == SpecialistType::CodeAnalyzer);
        let has_planner = active_specialists.iter().any(|m| m.specialist_type == SpecialistType::Planner);
        ...
        assert!(!items.is_empty(), "should recall items for active Code domain");
        assert!(items.iter().any(|item| item.domain == AttentionDomain::Code));
    }
    ...
    fn test_metacognition_quick_scan() {
        ...
        let bridge = crate::l5_cognition::nt_mind::nt_mind::distillation::MetaCognitionBridge::new(
            root.to_str().unwrap(),
        );
    ...
    fn test_metacognition_full_cycle() {
        let root = metacog_fixture();
        let mut bridge = crate::l5_cognition::nt_mind::nt_mind::distillation::MetaCognitionBridge::new(
            root.to_str().unwrap(),
        );
    ...
    fn test_metacognition_weakness_patterns() {
        ...
        model.modules.push(ModuleInfo {
            name: "big_mod".into(), path: "src/big/".into(),
            file_count: 1, total_lines: 2000, test_count: 0, has_tests: false,
            unsafe_count: 10, unwrap_count: 30, todo_count: 10,
            public_api_count: 20, description: "".into(),
        });
        let report = analyzer.analyze(&model);
        assert!(report.summary.total_count >= 4, "Large module + missing tests + unsafe + unwrap");
        assert!(report.summary.critical_count >= 1, "Should have critical issues");
    }
