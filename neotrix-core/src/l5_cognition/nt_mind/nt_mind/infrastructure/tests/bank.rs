...
mod tests {
    ...
    use crate::nt_mind::infrastructure::ReasoningBank;
    ...
    fn test_reasoning_bank_retrieve_by_embedding() {
        ...
        let mut memory1 = ReasoningMemory::new("design task 1", TaskType::UIDesign, &micro_edits, 0.9);
        memory1.embedding = Some(vec![1.0, 0.0, 0.0]);
        bank.store(memory1);

        let mut memory2 = ReasoningMemory::new("design task 2", TaskType::UIDesign, &micro_edits, 0.8);
    ...
    fn test_reasoning_bank_retrieve_fallback() {
        ...
        let memory = ReasoningMemory::new("design task keyword", TaskType::UIDesign, &micro_edits, 0.8);
        bank.store(memory);
    ...
    fn test_reasoning_bank_capacity() {
        ...
        for i in 0..4 {
            let memory = ReasoningMemory::new(&format!("task {}", i), TaskType::UIDesign, &micro_edits, 0.8);
            bank.store(memory);
        }
    ...
    fn test_coding_knowledge_initialization() {
        ...
        let code_review_mems = bank.retrieve_relevant("error handling", Some(TaskType::CodeReview), 10);
    ...
    fn test_embedder_with_knowledge_matching() {
        ...
        let texts: Vec<&str> = bank.memories().iter().map(|m| m.task_description.as_str()).collect();
        let mut embedder = crate::l5_cognition::nt_mind::nt_mind::embedding::TextEmbedder::new();
        ...
        assert!(sim > 0.0, "should match knowledge: sim={}, text={}", sim, text);
