...
pub struct CodeReviewEngine {
    ...
    pub linters_enabled: bool,
}

impl CodeReviewEngine {
    pub fn new(capability: CapabilityVector) -> Self {
        Self { capability, mode: None, linters_enabled: false }
    }

    pub fn with_mode(mode: AuditMode) -> Self {
        Self { capability: CapabilityVector::default(), mode: Some(mode), linters_enabled: false }
    }
    ...
    pub(crate) fn _load_sources(&self, path: &Path) -> Vec<String> {
        ...
        if let Ok(entries) = std::fs::read_dir(path) {
            for entry in entries.flatten() {
                let p = entry.path();
                if p.extension().is_some_and(|e| matches!(e.to_str(), Some("rs" | "py" | "js" | "ts" | "go"))) {
                    ...
                    if self.linters_enabled {
                        ...
                        if line.contains("api_key") || line.contains("password") {
                            issues.push(_AuditIssue {
                                ...
                                file: src.clone(),
                                line: (i + 1) as u32,
                                message: "Potential secret in code".into(),
                                severity: "high".into(),
                            });
                        }
                    }
...
mod tests {
    ...
    fn test_detect_unwrap() {
        ...
        cv.arr[crate::l5_cognition::nt_mind::nt_mind::core::IDX_ANALYSIS] = 0.8;
    ...
    fn test_detect_secrets() {
        let mut cv = CapabilityVector::default();
        cv.arr[crate::l5_cognition::nt_mind::nt_mind::core::IDX_ANALYSIS] = 0.8;
        cv.arr[crate::l5_cognition::nt_mind::nt_mind::core::IDX_VERIFICATION] = 0.8;
        let engine = CodeReviewEngine::new(cv);
