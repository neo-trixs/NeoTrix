...
impl ElementRegistry {
    ...
    fn visit_dfs(
        ...
        if visiting.contains(id) {
            return Err(ElementError::DependencyNotMet(
                format!("circular dependency detected involving '{}'", id),
            ));
        }
        ...
        if let Some(element) = self.elements.get(id) {
            for dep in element.depends_on() {
                if !self.elements.contains_key(dep) {
                    return Err(ElementError::DependencyNotMet(
                        format!("element '{}' depends on '{}' which is not registered", id, dep),
                    ));
                }
    ...
    pub fn get<T: Element>(&self, id: &str) -> Option<&T> {
        self.elements.get(id).and_then(|e| {
            e.as_ref().as_any().downcast_ref::<T>()
        })
    }

    pub fn get_mut<T: Element>(&mut self, id: &str) -> Option<&mut T> {
        self.elements.get_mut(id).and_then(|e| {
            e.as_mut().as_any_mut().downcast_mut::<T>()
        })
    }
...
mod tests {
    use super::*;
    use crate::l5_cognition::nt_mind::nt_mind::element::bus::{ElementBus, EventKind, EventPayload};
    use crate::l5_cognition::nt_mind::nt_mind::element::ElementType;
    ...
    fn test_register_and_bootstrap() {
        ...
        reg.register(e).expect("register first element should succeed");
    ...
    fn test_bootstrap_lifecycle() {
        ...
        reg.bootstrap(vec![e]).expect("bootstrap lifecycle element should start registry");
        assert_eq!(reg.state(), RegistryState::Started);

        let element = reg.get::<TestElement>("test.lifecycle").expect("test.lifecycle must exist after bootstrap");
        assert!(element.init_called);
        assert!(element.start_called);

        reg.shutdown().expect("shutdown running registry should succeed");
    ...
    fn test_shutdown_calls_stop_destroy() {
        ...
        reg.bootstrap(vec![e]).expect("bootstrap shutdown element should start registry");
        reg.shutdown().expect("shutdown should stop and clean up registry");

        let element = reg.get::<TestElement>("test.shutdown").expect("test.shutdown must exist after bootstrap");
    ...
    fn test_dependency_order() {
        ...
        let pos_b = order.iter().position(|id| id == "B").expect("element B must be in load_order");
    ...
    fn test_duplicate_registration_errors() {
        ...
        reg.register(a).expect("register duplicate element should succeed on first attempt");
    ...
    fn test_get_by_type() {
        ...
        reg.bootstrap(vec![e]).expect("bootstrap element for get-by-type test");
