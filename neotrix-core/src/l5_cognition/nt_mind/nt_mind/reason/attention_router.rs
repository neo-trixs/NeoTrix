...
fn specialist_query_coord(st: SpecialistType, context: &str) -> HyperCoord {
    ...
    match st {
        ...
        SpecialistType::EvidenceWeightedHypothesis => {
            coord.set(DimensionAxis::Certainty, 0.9);
            coord.set(DimensionAxis::Abstraction, 0.7);
            coord.set(DimensionAxis::Domain, 0.4);
        }
        SpecialistType::Orchestrator => {
            coord.set(DimensionAxis::Agency, 0.9);
            coord.set(DimensionAxis::Abstraction, 0.6);
...
impl SpecialistType {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            SpecialistType::EvidenceWeightedHypothesis => "evidence-weighted-hypothesis",
            SpecialistType::Orchestrator => "orchestrator",
        }
    }
    ...
    pub(crate) fn _short_name(&self) -> &'static str {
        match self {
            ...
            SpecialistType::EvidenceWeightedHypothesis => "EWH",
            SpecialistType::Orchestrator => "ORC",
        }
    }
}
...
mod tests {
    ...
    fn test_seed_knowledge_populates_hypercube() {
        ...
        log::info!("cell_count: before={} after={}", before, after);
