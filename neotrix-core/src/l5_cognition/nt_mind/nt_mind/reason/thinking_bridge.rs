...
impl ThinkingBridge {
    ...
    pub(crate) fn _observe_task(&mut self, task: &str) -> usize { self.silicon.observe(&format!("task:{}", task)) }
    pub(crate) fn _observe_tool_use(&mut self, tool: &str, result: &str) -> (usize, usize) { self.silicon.observe_tool_call(tool, result) }
    ...
    pub fn evaluate_cognitive_health(&mut self) -> &CognitiveHealthReport {
        ...
        let report = self.evaluator.latest_report()
    ...
    pub fn auto_crystallize(&mut self) -> String {
        ...
        let traces: Vec<ThinkingTrace> = self.silicon.recent_traces(10).iter()
            .map(|t| (*t).clone())
            .collect();
        let extracted = SkillBridge::_extract_from_recent(&mut self.skill_registry, &traces, iteration);
...
impl SkillBridge {
    ...
    pub(crate) fn _extract_from_recent(registry: &mut CrystalRegistry, traces: &[ThinkingTrace], iteration: usize) -> usize {
...
mod tests {
    ...
    #[test] fn test_observe_task() { let mut bridge = ThinkingBridge::new("/tmp"); let id = bridge._observe_task("analyze architecture"); assert!(id > 0 || id == 0); assert!(bridge.silicon.context_window.len() > 0); }
    #[test] fn test_observe_tool_use() { let mut bridge = ThinkingBridge::new("/tmp"); let (call, result) = bridge._observe_tool_use("grep", "found 3 matches"); assert!(call < result); }
    ...
    #[test] fn test_skill_extract_from_recent() { let mut reg = CrystalRegistry::new(); let mut good_trace = ThinkingTrace::new(0, "refactor api"); good_trace.grade = ReflectionGrade::Good; good_trace.steps.push(ThinkingStep::new(1, "analyze", StrategyKind::Reflection).with_domain(AttentionDomain::Code)); let mut poor_trace = ThinkingTrace::new(1, "failed attempt"); poor_trace.grade = ReflectionGrade::Poor; poor_trace.steps.push(ThinkingStep::new(1, "try", StrategyKind::Direct).with_domain(AttentionDomain::Code)); let extracted = SkillBridge::_extract_from_recent(&mut reg, &[good_trace, poor_trace], 1); assert_eq!(extracted, 1); assert_eq!(reg.crystals.len(), 1); }
    ...
    #[test] fn test_skill_auto_maintain_empty() { let mut reg = CrystalRegistry::new(); assert_eq!(SkillBridge::_auto_maintain_registry(&mut reg), "no maintenance needed"); }
}
