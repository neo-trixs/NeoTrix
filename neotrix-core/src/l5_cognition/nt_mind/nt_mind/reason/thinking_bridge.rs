...
impl ThinkingBridge {
    ...
    pub fn run_reflection_cycle(&mut self) -> _ThinkingCycleResult {
        ...
        self.silicon
            .complete_thinking_trace(trace_id, "reflection complete", grade);
        _ThinkingCycleResult {
    ...
    pub(crate) fn _observe_task(&mut self, task: &str) -> usize {
        self.silicon.observe(&format!("task:{}", task))
    }
    pub(crate) fn _observe_tool_use(&mut self, tool: &str, result: &str) -> (usize, usize) {
        self.silicon.observe_tool_call(tool, result)
    }
    ...
    pub fn attention_profile_summary(&self) -> String {
        ...
        let active_heads: Vec<String> = self
            .silicon
            .attention_manager
            .active_heads()
            .iter()
    ...
    pub fn auto_snapshot(&mut self) -> Option<usize> {
        if self.silicon.iteration > 0 && self.silicon.iteration.is_multiple_of(5) {
    ...
    pub fn check_self_repair_needed(&mut self) -> Option<String> {
        ...
        let poor_count = recent
            ...
            .filter(|t| {
                matches!(
                    t.grade,
                    ReflectionGrade::Adequate | ReflectionGrade::Poor | ReflectionGrade::Failed
                )
            })
            .count();
    ...
    pub fn trigger_self_repair(&mut self) -> String {
        ...
        let repaired = self
            ...
            .strategy_registry
            .strategies
            .get_mut(&StrategyKind::ChainOfThought);
    ...
    pub fn evaluate_cognitive_health(&mut self) -> &CognitiveHealthReport {
        ...
        let report = self
            .evaluator
            .latest_report()
            .expect("CognitiveEvaluator must produce a report after evaluate() is called");
        if self.monitor.auto_tune_enabled {
            let _ = self.monitor.auto_tune(&self.silicon);
        }
        self.monitor
            .record_plan("cognitive-eval", report.stability_score);
        report
    }
    ...
    pub fn auto_crystallize(&mut self) -> String {
        let iteration = self.silicon.iteration;
        let traces: Vec<ThinkingTrace> = self
            .silicon
            .recent_traces(10)
            .iter()
            .map(|t| (*t).clone())
            .collect();
        let extracted =
            SkillBridge::_extract_from_recent(&mut self.skill_registry, &traces, iteration);
        let maintain = SkillBridge::_auto_maintain_registry(&mut self.skill_registry);
    ...
    pub fn evolution_summary(&self) -> String {
        ...
        let health = self
            ...
            .map(|r| {
                format!(
                    "Stability={:.2} flags={} repairs={}",
                    r.stability_score,
                    r.flags.len(),
                    r.repair_suggestions.len()
                )
            })
...
impl SkillBridge {
    ...
    pub(crate) fn _extract_from_recent(
        registry: &mut CrystalRegistry,
    ...
    pub(crate) fn _auto_maintain_registry(registry: &mut CrystalRegistry) -> String {
        let mut parts = Vec::new();
        let pruned = registry.prune_weak(0.3);
        if pruned > 0 {
            parts.push(format!("pruned {} weak", pruned));
        }
        if registry.crystals.len() > registry.max_crystals {
            let auto = registry.auto_prune();
            if auto > 0 {
                parts.push(format!(
                    "auto-pruned {} over max({})",
                    auto, registry.max_crystals
                ));
            }
        }
        if parts.is_empty() {
            "no maintenance needed".to_string()
        } else {
            parts.join(", ")
        }
    }

    pub fn summary(registry: &CrystalRegistry) -> String {
        registry.summary()
    }
    ...
    pub fn recommend(
        ...
        registry.find_similar(strategy, domain).map(|c| {
            format!(
                "SkillCrystal #{}: {} (eff={:.3}, used={}x)",
                c.id, c.pattern, c.effectiveness, c.use_count
            )
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_bridge_new() {
        let bridge = ThinkingBridge::new("/tmp");
        assert_eq!(bridge.silicon.iteration, 0);
        assert!(bridge.status_summary().contains("SiliconSelf"));
    }
    #[test]
    fn test_reflection_cycle() {
        ...
        assert!(result.summary.contains("SiliconSelf"));
    }
    #[test]
    fn test_observe_task() {
        let mut bridge = ThinkingBridge::new("/tmp");
        let id = bridge._observe_task("analyze architecture");
        assert!(id > 0 || id == 0);
        assert!(bridge.silicon.context_window.len() > 0);
    }
    #[test]
    fn test_observe_tool_use() {
        let mut bridge = ThinkingBridge::new("/tmp");
        let (call, result) = bridge._observe_tool_use("grep", "found 3 matches");
        assert!(call < result);
    }
    #[test]
    fn test_attention_profile() {
        ...
        assert!(profile.contains("AttentionProfile"));
    }
    #[test]
    fn test_reset() {
        let mut bridge = ThinkingBridge::new("/tmp");
        bridge.run_reflection_cycle();
        assert!(bridge.silicon.iteration > 0);
        bridge.reset();
        assert_eq!(bridge.silicon.attention_manager.active_heads().len(), 0);
    }
    #[test]
    fn test_multiple_reflection_cycles() {
        let mut bridge = ThinkingBridge::new("/tmp");
        let r1 = bridge.run_reflection_cycle();
        let r2 = bridge.run_reflection_cycle();
        assert!(r2.iteration > r1.iteration);
    }
    #[test]
    fn test_self_repair_not_needed_initial() {
        let mut bridge = ThinkingBridge::new("/tmp");
        assert!(bridge.check_self_repair_needed().is_none());
        assert_eq!(bridge.consecutive_poor_traces, 0);
    }
    #[test]
    fn test_self_repair_triggered() {
        let mut bridge = ThinkingBridge::new("/tmp");
        for _ in 0..3 {
            let id = bridge.silicon.thinking_traces.len();
            bridge
                .silicon
                .add_thinking_trace(ThinkingTrace::new(id, "poor cycle"));
            bridge
                .silicon
                .complete_thinking_trace(id, "bad result", 0.3);
        }
        assert!(bridge.check_self_repair_needed().is_some());
    }
    #[test]
    fn test_self_repair_not_needed_good_traces() {
        let mut bridge = ThinkingBridge::new("/tmp");
        for _ in 0..3 {
            let id = bridge.silicon.thinking_traces.len();
            bridge
                .silicon
                .add_thinking_trace(ThinkingTrace::new(id, "good cycle"));
            bridge
                .silicon
                .complete_thinking_trace(id, "good result", 0.95);
        }
        assert!(bridge.check_self_repair_needed().is_none());
    }
    #[test]
    fn test_trigger_self_repair_resets_state() {
        let mut bridge = ThinkingBridge::new("/tmp");
        for _ in 0..3 {
            let id = bridge.silicon.thinking_traces.len();
            bridge
                .silicon
                .add_thinking_trace(ThinkingTrace::new(id, "poor"));
            bridge.silicon.complete_thinking_trace(id, "bad", 0.3);
        }
        let _ = bridge.check_self_repair_needed();
        let msg = bridge.trigger_self_repair();
        assert!(msg.contains("self-repair"));
    }
    #[test]
    fn test_intrinsic_motivation_initial() {
        let mut bridge = ThinkingBridge::new("/tmp");
        bridge.run_reflection_cycle();
        let mot = bridge.compute_motivation();
        assert!(mot.intrinsic_reward >= 0.0);
    }
    #[test]
    fn test_evolution_summary_contains_expected() {
        let mut bridge = ThinkingBridge::new("/tmp");
        bridge.run_full_evolution_cycle();
        let summary = bridge.evolution_summary();
        assert!(summary.contains("IntrinsicMotivation") || summary.contains("Stability"));
    }
    #[test]
    fn test_broadcast_attention_no_active_heads() {
        let bridge = ThinkingBridge::new("/tmp");
        let mut gwt = GlobalWorkspace::new(0.5);
        let results = bridge.broadcast_attention_to_gwt(&mut gwt);
        assert!(results.is_empty());
    }
    #[test]
    fn test_broadcast_attention_with_stimulated_heads() {
        let mut bridge = ThinkingBridge::new("/tmp");
        bridge
            .silicon
            .attention_manager
        ...
        bridge
            .silicon
            .attention_manager
            .stimulate_domain(AttentionDomain::Planning, 0.8);
        bridge
            .silicon
            .attention_manager
            .stimulate_domain(AttentionDomain::SelfReflection, 0.7);
        bridge.silicon.attention_manager.global_threshold = 0.5;
        let mut gwt = GlobalWorkspace::new(0.3);
        let results = bridge.broadcast_attention_to_gwt(&mut gwt);
        assert_eq!(results.len(), 3);
    }
    #[test]
    fn test_broadcast_attention_salience_above_gwt_threshold() {
        let mut bridge = ThinkingBridge::new("/tmp");
        bridge
            .silicon
            .attention_manager
            .stimulate_domain(AttentionDomain::ToolUse, 0.85);
        bridge.silicon.attention_manager.global_threshold = 0.3;
        let mut gwt = GlobalWorkspace::new(0.6);
        let results = bridge.broadcast_attention_to_gwt(&mut gwt);
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn test_skill_extract_from_recent() {
        let mut reg = CrystalRegistry::new();
        let mut good_trace = ThinkingTrace::new(0, "refactor api");
        good_trace.grade = ReflectionGrade::Good;
        good_trace.steps.push(
            ThinkingStep::new(1, "analyze", StrategyKind::Reflection)
                .with_domain(AttentionDomain::Code),
        );
        let mut poor_trace = ThinkingTrace::new(1, "failed attempt");
        poor_trace.grade = ReflectionGrade::Poor;
        poor_trace.steps.push(
            ThinkingStep::new(1, "try", StrategyKind::Direct).with_domain(AttentionDomain::Code),
        );
        let extracted = SkillBridge::_extract_from_recent(&mut reg, &[good_trace, poor_trace], 1);
        assert_eq!(extracted, 1);
        assert_eq!(reg.crystals.len(), 1);
    }
    #[test]
    fn test_skill_recommend_found() {
        let mut reg = CrystalRegistry::new();
        let mut trace = ThinkingTrace::new(0, "debug");
        trace.grade = ReflectionGrade::Excellent;
        trace.steps.push(
            ThinkingStep::new(1, "step", StrategyKind::ChainOfThought)
                .with_domain(AttentionDomain::Code),
        );
        reg.extract_from_trace(&trace, 1);
        let rec = SkillBridge::recommend(&reg, StrategyKind::ChainOfThought, AttentionDomain::Code);
        assert!(rec.is_some());
        assert!(rec.unwrap().contains("SkillCrystal"));
    }
    #[test]
    fn test_skill_recommend_not_found() {
        let reg = CrystalRegistry::new();
        let rec = SkillBridge::recommend(&reg, StrategyKind::Direct, AttentionDomain::Code);
        assert!(rec.is_none());
    }
    #[test]
    fn test_skill_auto_maintain_empty() {
        let mut reg = CrystalRegistry::new();
        assert_eq!(
            SkillBridge::_auto_maintain_registry(&mut reg),
