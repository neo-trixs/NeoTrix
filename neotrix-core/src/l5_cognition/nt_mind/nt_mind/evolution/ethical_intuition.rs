...
impl EthicalIntuition {
    ...
    pub fn judge(&self, scenario: &str, context: JudgmentContext) -> IntuitionJudgment {
        // 1. 检索相似案例
        let analogical_results = self
            .casebase
            .analogical_reasoning(scenario, self.config.max_analogical_chain);
        ...
        if analogical_results.is_empty() {
            return IntuitionJudgment {
                ...
                primary_case: EthicalCase {
                    ...
                    domain: String::new(),
                    conflict_type: ConflictType::Other("none".into()),
                    severity:
                        crate::l5_cognition::nt_mind::nt_mind::evolution::casebase::Severity::Low,
                    stakeholders: vec![],
                    values_in_conflict: vec![],
        ...
        let calibrated_confidence =
            self.calibrate_confidence(confidence, &analogical_results[0].case.conflict_type);
        ...
        IntuitionJudgment {
            ...
            counterarguments,
            uncertainty_factors,
            recommended_action: self.derive_recommendation(
                &analogical_results[0].case,
                &analogical_results[0].case.recommended_action,
            ),
    ...
    pub fn calibrate(
        &self,
        scenario: &str,
        actual_verdict: JudgmentVerdict,
        predicted: &IntuitionJudgment,
    ) {
        ...
        tracing::warn!(
            "Calibration algorithm not yet implemented (Platt scaling / Isotonic regression)"
        );
    }
    ...
    fn analyze_mapping(&self, case: &EthicalCase) -> String {
        format!(
            "价值观冲突: {:?}; 领域: {}; 严重度: {:?}",
            case.conflict_type, case.domain, case.severity
        )
    }
    ...
    fn build_analogical_chain(&self, results: &[AnalogicalResult]) -> Vec<AnalogicalLink> {
        results
            .iter()
            .take(self.config.max_analogical_chain)
            .map(|r| {
    ...
    fn synthesize_verdict(
        ...
        if let Some(first) = results.first() {
            match first.case.severity {
                Severity::Critical => {
                    *scores.entry(JudgmentVerdict::Impermissible).or_insert(0.0) += 0.3;
                }
                Severity::High => {
                    *scores
                        .entry(JudgmentVerdict::ConditionallyPermissible(
                            "需严格条件".into(),
                        ))
                        .or_insert(0.0) += 0.2;
                }
    ...
    fn calibrate_confidence(&self, confidence: f64, conflict_type: &ConflictType) -> f64 {
        ...
        let data = self
            ...
            .read()
            .unwrap_or_else(|e| e.into_inner());
        if let Some(cal) = data
            .conflict_calibration
            .get(&format!("{:?}", conflict_type))
        {
...
impl EthicalIntuitionRuntime {
    pub fn new(casebase: Arc<CaseBase>, config: IntuitionConfig) -> Self {
        Self {
            inner: Arc::new(EthicalIntuition::new(casebase, config)),
        }
    }
    ...
    pub fn calibrate(
        ...
        self.inner
            .calibrate(scenario, actual, &EthicalIntuitionRuntime::dummy(predicted))
    }
...
mod tests {
    ...
    fn test_intuition_judgment_trolley() {
        ...
        assert!(matches!(
            judgment.judgment,
            JudgmentVerdict::Impermissible | JudgmentVerdict::ConditionallyPermissible(_)
        ));
    ...
    fn test_counterfactual_reasoning() {
        let casebase = Arc::new(CaseBase::new(Default::default()));
        casebase.load_from_kb(&mem_conn()).unwrap();
        let intuition = EthicalIntuitionRuntime::new(
            casebase.clone(),
            IntuitionConfig {
                enable_counterfactual: true,
                ..Default::default()
