...
fn capability_write_and_retrieve() {
    let agent = mem_agent();
    let w = agent
        .capability_write("AgentTest", "NeoTrix agent capability wiring", "test")
        .unwrap();
...
fn capability_kind_domain_mapping() {
    ...
    assert_eq!(
        MemoryCapabilityKind::Write.attention_domain(),
...
fn route_learner_cold_start_uses_static() {
    ...
    assert_eq!(
        learner.route(AttentionDomain::SelfReflection, "verifier"),
        "verifier"
    );
...
fn route_learner_overrides_static_with_evidence() {
    ...
    assert_eq!(
        routed, "generalist",
        "learner should override static mapping"
    );
    // 诊断视图可用
...
fn route_learner_persist_load_round_trip() {
    ...
    assert_eq!(
        restored.route(AttentionDomain::SelfReflection, "verifier"),
        "generalist"
    );
}
...
fn route_learner_load_missing_archive_stays_cold() {
    ...
    assert!(!learner.has_enough_evidence(AttentionDomain::SelfReflection));
    assert_eq!(
        learner.route(AttentionDomain::PatternMatch, "explorer"),
        "explorer"
    );
}
...
fn dialogue_bridge_derive_vector_content_aware() {
    ...
    assert!(
        cv.semantic_layer() > 0.0,
        "semantic_layer should be boosted"
    );
...
fn dialogue_bridge_absorb_pending_moves_capability() {
    ...
    assert!(
        outcome.absorbed >= 1,
        "at least one dialogue experience should absorb"
    );
...
fn dialogue_bridge_absorbs_multiple_sessions() {
    ...
    assert_eq!(
        outcome.absorbed, 2,
        "two distinct sessions should both absorb"
    );
...
fn research_results() -> Vec<SearchResult> {
    vec![
        SearchResult {
            ...
            snippet:
                "web search method with evidence source citation for verification and synthesis"
                    .into(),
            evidence: None,
        },
        SearchResult {
...
fn research_bridge_derive_research_vector_content_aware() {
    ...
    assert!(
        cv.semantic_layer() > 0.0,
        "search/web keywords should boost semantic_layer"
    );
    assert!(
        cv.inference_depth() > 0.0,
        "paper/research keywords should boost inference_depth"
    );
    assert!(
        cv.verification() > 0.0,
        "evidence/source keywords should boost verification"
    );
...
fn dispatch_bridge_derive_dispatch_vector_content_aware() {
    ...
    assert!(
        cv.ai_native_states() > 0.0,
        "route/agent keywords should boost ai_native_states"
    );
    assert!(
        cv.compound_composition() > 0.0,
        "topology/repair keywords should boost compound_composition"
    );
    assert!(
        cv.analysis() > 0.0,
        "strategy/bandit keywords should boost analysis"
    );
    assert!(
        cv.semantic_layer() > 0.0,
        "retrieval/hit keywords should boost semantic_layer"
    );
}
...
fn dispatch_bridge_absorb_experiences_moves_capability_and_dedups() {
    ...
    assert!(
        outcome.absorbed > 0,
        "dispatch experiences should be absorbed"
    );
    // 水位推进 → 同一批不再重复吸收。
    ...
    assert_eq!(
        again.absorbed, 0,
        "watermark must dedup already-consumed experiences"
    );
...
fn research_bridge_findings_absorb_moves_capability() {
    ...
    let mut brain = SelfIteratingBrain::new();
    let outcome =
        bridge.absorb_research_findings(&mut brain, "neotrix search", &research_results());
    // 结论落 KB: 2 条 Source + 1 条 Insight = 3 节点。
    assert_eq!(
        outcome.absorbed, 3,
        "2 sources + 1 insight should be written to KB"
    );
...
fn research_source_registered_in_catalog() {
    ...
    assert!(
        cv.verification() > 0.0,
        "research source should carry a real vector"
    );
}
...
fn domains_for_goal_multi_domain_stimulation() {
    ...
    assert!(
        research
            .iter()
            .any(|(d, _)| *d == AttentionDomain::PatternMatch),
    ...
    assert!(
        review
            .iter()
            .any(|(d, _)| *d == AttentionDomain::RiskAssessment),
...
fn tree_branch_weakness_derived_from_health_fog_constellation() {
    ...
    assert!(
        w_healthy < 0.1,
        "healthy branch should be ~0, got {w_healthy}"
    );
...
fn tree_branch_stimuli_fuses_with_goal_dispatch() {
    ...
    let (agent, _) = shell
        .dispatch_and_execute(&probe, "general_dialogue_tick")
        .expect("dispatch");
...
fn branch_attention_domains_covers_all_kinds() {
    ...
    for kind in BranchKind::all() {
        ...
        assert!(
            !domains.is_empty(),
            "branch {kind:?} should map to >=1 attention domain"
        );
    }
}
...
fn topology_task_conditioned_init() {
    ...
    assert_eq!(
        research.agent_for(AttentionDomain::PatternMatch),
        "researcher"
    );
    assert_eq!(
        research.agent_for(AttentionDomain::SelfReflection),
        "verifier"
    );
...
fn topology_audit_proposes_repair_on_underperformance() {
    ...
    assert!(
        repairs_healthy.is_empty(),
        "healthy edge should not be repaired"
    );
}
...
fn topology_audit_with_experience_fuses_coevo_evidence() {
    ...
    assert!(
        no_reward.is_empty(),
        "insufficient evidence must not repair"
    );
    ...
    assert!(
        !repairs.is_empty(),
        "coevo failure warning + mastery gap should repair"
    );
    // 修复边界是规范的: 提议的 to_agent 落在规范档案内。
    for r in &repairs {
        assert!(
            canonical_agent_ok(r.to_agent),
            "repair target must be canonical"
        );
    }
...
fn canonical_agent_ok(s: &str) -> bool {
    matches!(
        s,
        "researcher" | "explorer" | "planner" | "generalist" | "verifier" | "watcher"
    )
}
...
fn topology_apply_repair_is_bounded() {
    ...
    assert_eq!(
        topology.revision, 1,
        "rejected repair must not bump revision"
    );
}
...
fn topology_persist_load_round_trip() {
    ...
    assert_eq!(
        restored.agent_for(AttentionDomain::PatternMatch),
        "researcher"
    );
    restored.load(&kb).expect("load");
    assert_eq!(
        restored.agent_for(AttentionDomain::Semantic),
        "planner",
        "repair survives restart"
    );
...
fn shell_audit_repair_uses_real_learner_trace() {
    ...
    shell
        .learner
        .record(AttentionDomain::Semantic, "planner", true);
    shell
        .learner
        .record(AttentionDomain::Semantic, "planner", true);
    shell
    ...
    assert!(
        repairs
            .iter()
            .any(|r| r.domain == AttentionDomain::Semantic),
        "Semantic edge should be repaired after underperformance"
    );
    assert_eq!(
        shell.topology.agent_for(AttentionDomain::Semantic),
        "planner"
    );
...
fn dispatch_and_execute_feeds_learner_with_real_result() {
    ...
    let (agent, outcome) = shell
        .dispatch_and_execute(&probe, "review the codebase")
        .expect("dispatch");
    ...
    assert_eq!(
        shell.learner.rates(AttentionDomain::SelfReflection).len(),
        1
    );
}
...
fn dispatch_and_execute_failure_does_not_reinforce() {
    ...
    shell.stimulate(AttentionDomain::SelfReflection, 0.9);
    let (agent, outcome) = shell
        .dispatch_and_execute(&probe, "audit")
        .expect("dispatch");
    assert_eq!(agent, "verifier");
    assert!(!outcome.is_success());
...
fn dispatch_drives_coevo_same_reward_stream() {
    ...
    assert_eq!(
        shell
            .coevo
            .mastery(AttentionDomain::SelfReflection, "verifier"),
        1.0
    );
...
fn coevo_persist_load_round_trip() {
    ...
    assert_eq!(
        restored.mastery(AttentionDomain::PatternMatch, "explorer"),
        0.5
    );
    ...
    assert_eq!(
        restored.graph.memories[2].id, 2,
        "monotonic id survives restart"
    );
}
...
fn dispatch_research_task_routes_to_researcher() {
    ...
    assert_eq!(
        agent, "researcher",
        "research text should route to researcher"
    );
    assert!(outcome.is_success());
    ...
    assert_eq!(
        agent, "explorer",
        "code text outside PatternMatch semantics falls back to explorer"
    );
}
...
fn production_executor_maps_profiles_to_real_actions() {
    ...
    match exec.execute("watcher", "health probe") {
        AgentExecutionOutcome::Success(s) => {
            assert!(s.contains("KB nodes"), "watcher should probe KB: {}", s)
        }
        other => panic!("watcher should succeed offline: {:?}", other),
    }
    // verifier → 证据溯源计数 (确定性)。
    match exec.execute("verifier", "evidence audit") {
        AgentExecutionOutcome::Success(s) => assert!(
            s.contains("evidence"),
            "verifier should trace evidence: {}",
            s
        ),
    ...
    match exec.execute("planner", "design plan") {
        AgentExecutionOutcome::NoOp(s) => {
            assert!(s.contains("planning"), "planner should be NoOp: {}", s)
        }
    ...
    match exec.execute("ghost", "whatever") {
        AgentExecutionOutcome::NoOp(s) => {
            assert!(s.contains("ghost"), "unknown agent should NoOp: {}", s)
        }
...
fn execution_outcome_summary_human_readable() {
    assert!(AgentExecutionOutcome::Success("done".into())
        .summary()
        .contains("success"));
    assert!(AgentExecutionOutcome::NoOp("none".into())
        .summary()
        .contains("noop"));
    assert!(AgentExecutionOutcome::Failure("boom".into())
        .summary()
        .contains("failed"));
