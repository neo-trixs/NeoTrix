...
impl RouterStats {
    pub fn from_cost_log(cost_log: &[super::reasoning_engine::CostRecord]) -> Self {
        ...
        let last_24h = cost_log.iter().filter(|c| c.timestamp > now - 86400).count() as u64;
        ...
        for record in cost_log {
            ...
            *tier_costs.entry(record.tier.clone()).or_insert(0.0) += record.cost_estimate_usd;
            tier_durations.entry(record.tier.clone()).or_default().push(record.duration_ms);
            total_cost += record.cost_estimate_usd;
        }

        let tier_avg_duration_ms = tier_durations.into_iter().map(|(t, ds)| {
            let avg = if ds.is_empty() { 0.0 } else { ds.iter().sum::<u64>() as f64 / ds.len() as f64 };
            (t, avg)
        }).collect();

        let distr = tier_counts.iter().map(|(t, c)| {
            (t.clone(), *c as f64 / total.max(1) as f64 * 100.0)
        }).collect();

        // 估算节约：如果全用 T4 的费用 vs 实际费用
        let if_all_t4 = total as f64 * ModelTier::T4.cost_multiplier() * 0.02;
