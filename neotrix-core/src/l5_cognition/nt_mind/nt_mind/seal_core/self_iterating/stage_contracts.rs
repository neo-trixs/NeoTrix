...
impl Default for _EnforcementPolicy {
    fn default() -> Self {
        Self {
            suggestion: _ActionOnViolation::Log,
            warning: _ActionOnViolation::Log,
            error: _ActionOnViolation::Block,
        }
    }
}

impl _EnforcementPolicy {
    pub fn strict() -> Self {
        Self {
            suggestion: _ActionOnViolation::Log,
            warning: _ActionOnViolation::Block,
            error: _ActionOnViolation::Block,
        }
    }
    ...
    pub fn permissive() -> Self {
        Self {
            suggestion: _ActionOnViolation::Ignore,
            warning: _ActionOnViolation::Log,
            error: _ActionOnViolation::Log,
        }
    }

    pub(crate) fn _action_for(&self, severity: _ContractSeverity) -> _ActionOnViolation {
        match severity {
            _ContractSeverity::Suggestion => self.suggestion,
            _ContractSeverity::Warning => self.warning,
            _ContractSeverity::Error => self.error,
        }
    }
}
...
pub(crate) trait _StageContract: Send + Sync {
    ...
    fn invariant_check(&self, brain: &SelfIteratingBrain) -> Vec<_ContractViolation>;
}
...
impl _StageContract for _ReadOnlyContract {
    ...
    fn pre_check(&self, _brain: &SelfIteratingBrain) -> Vec<_ContractViolation> { vec![] }
    fn post_check(&self, brain: &SelfIteratingBrain, before: &StageCheckpoint) -> Vec<_ContractViolation> {
        let mut violations = Vec::new();
        if brain.iteration != before.iteration {
            violations.push(_ContractViolation {
                stage_name: String::new(),
                severity: _ContractSeverity::Error,
        ...
        if brain._reward != before.reward {
            violations.push(_ContractViolation {
                stage_name: String::new(),
                severity: _ContractSeverity::Warning,
                message: format!("read_only contract violated: reward changed from {} to {}", before.reward, brain._reward),
            });
        }
        violations
    }
    fn invariant_check(&self, _brain: &SelfIteratingBrain) -> Vec<_ContractViolation> { vec![] }
}
...
impl Default for _CapabilityStableContract {
    fn default() -> Self { Self { max_degradation: 0.05 } }
}

impl _StageContract for _CapabilityStableContract {
...
impl Default for _ResourceBudgetContract {
    fn default() -> Self { Self { max_tool_calls_per_run: 5 } }
}

impl _StageContract for _ResourceBudgetContract {
...
impl DeliveryPromise {
    pub fn capability_led(quality_floor: f64) -> Self {
        Self { promise_type: _PromiseType::CapabilityLed, quality_floor, fallback_allowed: false }
    }

    pub(crate) fn _progress_led(quality_floor: f64) -> Self {
        Self { promise_type: _PromiseType::ProgressLed, quality_floor, fallback_allowed: true }
    }
}
...
impl _StageContract for DeliveryPromiseContract {
                ...
                match (before.champion_score, brain.champion.as_ref().map(|c| c.score)) {
                    (Some(before_score), Some(after_score)) => {
                        ...
                        if improvement < self.promise.quality_floor && reward_rose {
                            ...
                            violations.push(_ContractViolation {
                                stage_name: String::new(),
                                severity: if self.promise.fallback_allowed { _ContractSeverity::Warning } else { _ContractSeverity::Error },
                                message: msg,
                            });
                        }
                    }
...
pub struct ContractRegistry {
    contracts: Vec<(String, Box<dyn _StageContract>)>,
    pub policy: _EnforcementPolicy,
}
...
impl ContractRegistry {
    pub fn new() -> Self {
        Self {
            contracts: Vec::new(),
            policy: _EnforcementPolicy::default(),
        }
    }

    pub fn register(&mut self, stage_name: &str, contract: Box<dyn _StageContract>) {
        self.contracts.push((stage_name.to_string(), contract));
    }

    pub(crate) fn _contracts_for(&self, stage_name: &str) -> Vec<&dyn _StageContract> {
        self.contracts.iter()
    ...
    pub(crate) fn _all_contracts(&self) -> &[(String, Box<dyn _StageContract>)] {
        &self.contracts
    }

    pub(crate) fn _check_stage(&self, stage_name: &str, brain: &SelfIteratingBrain, checkpoint: &Option<StageCheckpoint>) -> Vec<_ContractViolation> {
        let mut all = Vec::new();
        for contract in self._contracts_for(stage_name) {
    ...
    pub(crate) fn _has_any_violations(&self, violations: &[_ContractViolation]) -> bool {
        violations.iter().any(|v| {
            matches!(self.policy._action_for(v.severity), _ActionOnViolation::Block)
        })
    }
}
...
pub struct ContractAwareStage {
    ...
    contracts: Vec<Box<dyn _StageContract>>,
}
...
impl ContractAwareStage {
    ...
    pub fn with_contract(mut self, contract: Box<dyn _StageContract>) -> Self {
...
impl BrainStage for ContractAwareStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        for v in &violations {
            match policy._action_for(v.severity) {
                _ActionOnViolation::Ignore => {}
                _ActionOnViolation::Log => {
                    log::warn!("[contract] pre-check FAIL: stage={} contract={} msg={:?}",
                        self.name(), contract_name_for(&self.contracts), v.message);
                }
                _ActionOnViolation::Block => {
...
fn contract_name_for(contracts: &[Box<dyn _StageContract>]) -> String {
...
pub mod presets {
    ...
    pub fn read_only() -> Vec<Box<dyn _StageContract>> {
        vec![
            Box::new(_ReadOnlyContract),
            Box::new(_NoSelfEditContract),
            Box::new(_ResourceBudgetContract { max_tool_calls_per_run: 1 }),
        ]
    }

    /// A stage that may modify state but must keep capability stable.
    pub(crate) fn _safe_mutation() -> Vec<Box<dyn _StageContract>> {
        vec![
            Box::new(_CapabilityStableContract::default()),
            Box::new(_ResourceBudgetContract::default()),
            Box::new(_MonotonicIterationContract),
        ]
    }

    /// A stage that performs self-modification (DGM-H etc) — minimal restrictions.
    pub fn self_modifying() -> Vec<Box<dyn _StageContract>> {
        vec![
            Box::new(_ResourceBudgetContract { max_tool_calls_per_run: 10 }),
            Box::new(_MonotonicIterationContract),
        ]
    }
    ...
    pub fn capability_led(quality_floor: f64) -> Vec<Box<dyn _StageContract>> {
        vec![
...
mod tests {
    ...
    fn test_read_only_contract_passes() {
        ...
        let contract = _ReadOnlyContract;
    ...
    fn test_capability_stable_contract() {
        ...
        let contract = _CapabilityStableContract::default();
        let violations = contract.post_check(&brain, &before);
    ...
    fn test_resource_budget_contract() {
        ...
        let contract = _ResourceBudgetContract { max_tool_calls_per_run: 3 };
        let violations = contract.post_check(&brain, &before);
    ...
    fn test_monotonic_iteration_contract() {
        ...
        let contract = _MonotonicIterationContract;
        let violations = contract.post_check(&brain, &before);
    ...
    fn test_contract_registry_routing() {
        ...
        registry.register("read_only_stage", Box::new(_ReadOnlyContract));
        registry.register("mutator_stage", Box::new(_CapabilityStableContract::default()));

        let read_contracts = registry._contracts_for("read_only_stage");
        assert_eq!(read_contracts.len(), 1);
        assert_eq!(read_contracts[0].name(), "read_only");

        let no_contracts = registry._contracts_for("nonexistent");
        assert!(no_contracts.is_empty());
    }

    #[test]
    fn test_enforcement_policy_default() {
        let policy = _EnforcementPolicy::default();
        assert_eq!(policy._action_for(_ContractSeverity::Suggestion), _ActionOnViolation::Log);
        assert_eq!(policy._action_for(_ContractSeverity::Warning), _ActionOnViolation::Log);
        assert_eq!(policy._action_for(_ContractSeverity::Error), _ActionOnViolation::Block);
    }

    #[test]
    fn test_enforcement_policy_strict() {
        let policy = _EnforcementPolicy::strict();
        assert_eq!(policy._action_for(_ContractSeverity::Warning), _ActionOnViolation::Block);
        assert_eq!(policy._action_for(_ContractSeverity::Error), _ActionOnViolation::Block);
    }
    ...
    fn test_no_self_edit_contract_detects_edits() {
        ...
        impl BrainStage for EditorStage {
            ...
            fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
                brain._micro_edits.push(crate::l5_cognition::nt_mind::nt_mind::self_edit::MicroEdit::AdjustDimension("test".into(), 0.5));
                Ok(StageDecision::Continue)
            }
        }
        ...
        let contract = _NoSelfEditContract;
        let violations = contract.post_check(&brain, &before);
    ...
    fn test_presets_safe_mutation_has_three_contracts() {
        let contracts = presets::_safe_mutation();
        assert_eq!(contracts.len(), 3);
    }
    ...
    fn test_delivery_promise_capability_led_blocks_surface_improvement() {
        ...
        assert_eq!(violations[0].severity, _ContractSeverity::Error, "silent fallback must block");
    }
    ...
    fn test_delivery_promise_champion_loss_always_blocks() {
        ...
        assert_eq!(violations[0].severity, _ContractSeverity::Error);
    }
    ...
    fn test_delivery_promise_progress_led_warns_on_regression() {
        ...
        let contract = DeliveryPromiseContract { promise: DeliveryPromise::_progress_led(0.0) };
        let violations = contract.post_check(&brain, &before);
        assert!(!violations.is_empty());
        assert_eq!(violations[0].severity, _ContractSeverity::Warning);
    }
