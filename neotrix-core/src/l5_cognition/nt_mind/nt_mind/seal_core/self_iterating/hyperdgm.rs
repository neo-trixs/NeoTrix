...
impl DGMMetaAgent {
    ...
    pub fn generate_edit(&self, archive: &HyperAgentArchive) -> LatentEdit {
        ...
        for x in &mut style { *x /= style_norm; }
        ...
        for x in &mut context { *x /= ctx_norm; }
    ...
    pub fn proposal_from_edit(&self, edit: &LatentEdit, archive: &HyperAgentArchive) -> SelfModificationProposal {
        ...
        } else if archive.best_score().is_none_or(|s| s < 0.6) {
        ...
        let impact = format!(
            ...
            entropy, magnitude, self.top_k, archive.len()
        );
...
impl GenerativeReplay {
    ...
    pub fn replay(&self, archive: &HyperAgentArchive) -> Vec<Vec<f64>> {
        ...
        let high_scorers: Vec<&HyperAgentRecord> = archive
            .records
            .iter()
            .filter(|r| r.score.is_some_and(|s| s >= self.min_score))
            .collect();
        ...
        for _ in 0..target_count {
            ...
            let mut latent: Vec<f64> = p1.iter()
                .zip(p2.iter())
                .map(|(a, b)| alpha * a + (1.0 - alpha) * b + rng.gen::<f64>() * noise_std * 2.0 - noise_std)
                .collect();
...
impl SelfReferentialCheck {
    ...
    fn approximate_spectral_radius(&self, archive: &HyperAgentArchive, query: &[f64]) -> f64 {
        let records: Vec<&[f64]> = archive.records.iter().map(|r| r.latent_snapshot.as_slice()).collect();
