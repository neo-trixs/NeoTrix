...
impl SelfIteratingBrain {
    ...
    pub(crate) fn _save_evidence(&self) -> NeoTrixResult<()> {
        ...
        let evidence = serde_json::json!({
            "iteration": self.iteration,
            "task": self._current_task,
            "escalation_streak": lvl,
            "escalation_level": lvl_name,
            "validity": self.stagnation.validity(),
            "loop_ratio": self.stagnation.loop_ratio(),
            "reward": self._reward,
        });
...
//            println!("  • {} [{}] — {}", t.title, t.source_type, dims.join(", "));
    ...
    pub fn init_reasoning_engine(&mut self) {
        ...
        let gateway = if cfg!(test) {
            ...
            crate::l1_action::nt_io::nt_io_provider::gateway::register_gateway_for_re_evaluation(&gw);
            Some(gw)
        };
