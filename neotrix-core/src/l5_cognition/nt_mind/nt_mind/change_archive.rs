...
mod tests {
    ...
    fn test_conflict_detection() {
        let mut archive = ChangeArchive::new();
        let edits_a = vec![MicroEdit::ModifiedDimension("analysis".to_string(), 0.5, 0.7)];
        let edits_b = vec![MicroEdit::ModifiedDimension("analysis".to_string(), 0.7, 0.9)];
    ...
    fn test_archive_report() {
        let mut archive = ChangeArchive::new();
        archive.record("test", "me", &[MicroEdit::AdjustDimension("test".to_string(), 0.1)]);
