...
impl _RecoveryVerifyService {
    ...
    pub(crate) fn _on_pattern_learned<F: Fn(&PatternUpdate) + Send + Sync + 'static>(&mut self, cb: F) {
        self.pattern_callbacks.push(Box::new(cb));
    }

    /// 注册陷阱晋升回调
    pub(crate) fn _on_trap_promoted<F: Fn(&str) + Send + Sync + 'static>(&mut self, cb: F) {
        self.trap_callbacks.push(Box::new(cb));
    }

    /// 核心验证入口 (T3 生产接线)
    pub(crate) fn _verify_recovery(
        &mut self,
...
impl Default for _RecoveryVerifyService {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_recovery_verify_service() {
        ...
        let mut svc = _RecoveryVerifyService::new();
