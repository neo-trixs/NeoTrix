...
pub trait GenCadRetrieval {
    /// 将 CAD 几何渲染为图像 (C0: 仅生成占位尺寸, 真实走 rasterizer)。
    fn render(&self, geo: &CadGeometry) -> RenderedImage;

    /// 将几何编码为向量 (C0: 基于命令 token 哈希的统计向量)。
    fn encode_geometry(&self, geo: &CadGeometry) -> Vec<f32>;

    /// 将图像编码为向量 (C0: 基于尺寸/通道的统计向量)。
    fn encode_image(&self, img: &RenderedImage) -> Vec<f32>;

    /// 将一对 (几何, 图像) 写入 KB FTS5 对比索引。
    fn index_into_kb(&self, pair: ContrastivePair) -> Result<(), String>;

    /// 给定查询几何, 在 KB 中检索最相似的图像向量对 (返回 alignment 降序)。
    fn retrieve(&self, query: &CadGeometry) -> Vec<ContrastivePair>;
}

/// C0 基础实现 — 全部逻辑本地可运行, 不依赖外部模型。
#[derive(Default)]
pub struct GenCadCore {
    store: std::collections::VecDeque<ContrastivePair>,
}
...
impl GenCadRetrieval for GenCadCore {
    fn render(&self, geo: &CadGeometry) -> RenderedImage {
    ...
    fn retrieve(&self, query: &CadGeometry) -> Vec<ContrastivePair> {
        ...
        let mut scored: Vec<(f32, ContrastivePair)> = self
            .store
            .iter()
...
#[derive(Default)]
pub struct GenCadSelfTest;

impl SelfTest for GenCadSelfTest {
    fn name(&self) -> &str {
        "nt_core_gencad"
    }

    fn self_test(&self) -> Result<(), Vec<String>> {
        let core = GenCadCore::new();
        let mut geo = CadGeometry::new(3);
        geo.push("EXTRUDE");
        geo.push("FILLET");
...
mod tests {
    ...
    fn test_render_dim_selects_size() {
        let core = GenCadCore::new();
        let img2d = core.render(&CadGeometry::new(2));
        let img3d = core.render(&CadGeometry::new(3));
    ...
    fn test_encode_geometry_empty_is_zero_vec() {
        let core = GenCadCore::new();
        let v = core.encode_geometry(&CadGeometry::new(3));
        assert_eq!(v.len(), 8);
    ...
    fn test_retrieve_orders_by_alignment() {
        let mut core = GenCadCore::new();
        // 直接注入 store 以验证排序 (C0 内存占位)。
        let mut geo = CadGeometry::new(3);
        geo.push("EXTRUDE");
        let gv = core.encode_geometry(&geo);
        core.store.push_back(ContrastivePair {
            geometry_id: "a".into(),
            geometry_vec: gv.clone(),
            image_vec: vec![0.0; 8],
        });
        let mut geo2 = CadGeometry::new(3);
        geo2.push("BOSS");
