...
pub fn select_forget_candidates(entries: &[KnowledgeEntry], config: &ForgettingConfig) -> Vec<usize> {
    entries.iter()
        .enumerate()
...
mod tests {
    ...
    fn test_decay_factor_high_importance_low() {
        ...
        assert!(imp_decay < triv_decay, "important entries should decay slower");
    }
