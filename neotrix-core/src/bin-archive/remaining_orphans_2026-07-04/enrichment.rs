...
use super::frontier::{DualQueueFrontier, UrlEntry, extract_domain, extract_links};
...
fn dimension_to_crawl_topics(_dim: &str) -> Vec<CrawlTopic> {
    vec![CrawlTopic::ScienceAndTechnology, CrawlTopic::NewsAndMedia, CrawlTopic::General]
}
...
impl BrainEnrichmentPipeline {
    ...
    pub fn run_cycle(
        ...
        self.cycle += 1;
        let gap_topics = self.analyze_gaps(cap);
        let gaps_found = gap_topics.len();
        let seeds = self.gaps_to_seeds(&gap_topics);
        ...
        for _ in 0..batch_size {
            ...
            if result.error.is_some() || !result.is_success() { continue; }
            let text = match &result.text { Some(t) => t, None => continue };
            let classified = self.classifier.classify(&entry.url, text);
            let mapped = self.mapper.map(&classified);
            self.mapper.apply_to_brain(&mapped, cap, bank);
            urls_absorbed += 1;
            if let Some(body) = &result.body {
                let new_links = extract_links(body, &result.url);
                for link in new_links {
                    let domain = extract_domain(&link);
                    self.frontier.push(UrlEntry { url: link, domain, depth: 0, priority: 1, topic: None });
                }
            }
        }
        let duration_ms = start.elapsed().as_millis() as u64;
        let message = if urls_absorbed > 0 {
            format!("enriched {} dimensions, absorbed {} URLs", gaps_found, urls_absorbed)
        } else if gaps_found > 0 {
            format!("{} gaps found but no URLs absorbed", gaps_found)
        } else {
            "no weak dimensions — brain well-enriched".into()
        };
        EnrichmentReport { cycle: self.cycle, gaps_found, seeds_generated, urls_crawled, urls_absorbed, duration_ms, message }
    }

    fn analyze_gaps(&self, cap: &CapabilityVector) -> Vec<GapTopic> {
        let mut gaps = Vec::new();
        let arr = cap.arr();
        for (idx, &val) in arr.iter().enumerate() {
            if val < self.config.weak_threshold {
                let name = FIELD_NAMES_DOMAIN.get(idx).copied().unwrap_or("general_knowledge");
                let domains = dimension_to_crawl_topics(name);
                for domain in domains {
                    gaps.push(GapTopic { dim_index: idx, dim_name: name.to_string(), current_value: val, topic: domain, priority: 1.0 - val });
                }
            }
        }
    ...
    fn gaps_to_seeds(&self, gaps: &[GapTopic]) -> Vec<GapSeed> {
        let mut seeds = Vec::new();
        for gap in gaps {
            let urls = match self.config.depth {
                EnrichmentDepth::Shallow => vec![format!("https://en.wikipedia.org/wiki/{}", gap.dim_name.replace('_', "_"))],
                EnrichmentDepth::Moderate => vec![format!("https://news.ycombinator.com/item?id={}", gap.dim_name)],
                EnrichmentDepth::Deep => vec![format!("https://arxiv.org/search/?query={}", gap.dim_name)],
            };
            for url in urls {
                seeds.push(GapSeed { url, topic: gap.topic.clone(), priority: gap.priority, dim_name: gap.dim_name.clone() });
            }
        }
        seeds
    }
...
mod tests {
    ...
    fn make_cap_with_weak_dim(dim: usize) -> CapabilityVector {
    ...
    fn test_gap_detection_finds_weak_dimensions() {
        let cap = make_cap_with_weak_dim(0);
        let pipeline = BrainEnrichmentPipeline::new(EnrichmentConfig::default());
        let gaps = pipeline.analyze_gaps(&cap);
    ...
    fn test_no_gaps_when_all_strong() {
        let mut cap = CapabilityVector::default();
        for v in cap.arr_mut().iter_mut() { *v = 0.9; }
        let pipeline = BrainEnrichmentPipeline::new(EnrichmentConfig::default());
        let gaps = pipeline.analyze_gaps(&cap);
    ...
    fn test_shallow_urls_generated() {
        let cap = make_cap_with_weak_dim(0);
        let pipeline = BrainEnrichmentPipeline::new(EnrichmentConfig { depth: EnrichmentDepth::Shallow, ..Default::default() });
        let gaps = pipeline.analyze_gaps(&cap);
        let seeds = pipeline.gaps_to_seeds(&gaps);
