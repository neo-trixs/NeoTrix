...
impl ExternalDataConnector {
    pub fn collect_all() -> Vec<DataSourceRecord> {
        ...
        if !errors.is_empty() {
            log::warn!("[data_connector] fetch failures: {}", errors.join("; "));
        }
        records
    }
    ...
    pub fn collect_from(source: DataSourceType) -> Vec<DataSourceRecord> {
        let result = match source {
            DataSourceType::HackerNews => Self::fetch_hackernews(),
            DataSourceType::ArXiv => Self::fetch_arxiv_latest(),
            DataSourceType::GitHubTrending => Self::fetch_github_trending(),
            DataSourceType::Wikipedia => Self::fetch_wikipedia_random(),
            DataSourceType::OpenLibrary => Self::fetch_openlibrary_trending(),
            DataSourceType::NewsRss => Self::fetch_news_rss(),
            DataSourceType::SemanticScholar => Self::fetch_semantic_scholar(),
        };
        match result {
            Ok(r) => r,
            Err(e) => {
                log::warn!("[data_connector] fetch {:?} failed: {}", source, e);
                vec![]
            }
        }
    }

    // TODO: inject via DI — pass reqwest::blocking::Client through DataConnector constructor
    fn http_client() -> &'static reqwest::blocking::Client {
        static CLIENT: std::sync::LazyLock<reqwest::blocking::Client> = std::sync::LazyLock::new(|| {
            reqwest::blocking::Client::builder()
                ...
                .no_proxy()
                .build()
                .expect("Failed to build HTTP client for DataConnector")
        });
        &CLIENT
    }
