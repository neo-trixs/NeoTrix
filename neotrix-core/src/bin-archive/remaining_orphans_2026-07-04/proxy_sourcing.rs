...
pub enum SourceFormat {
    ...
    PlainTxt,
    /// JSON array of {ip, port, protocol, ...}
    JsonArray,
...
impl ProxySourcing {
    pub fn new() -> Self {
        let client = reqwest::Client::builder()
            ...
            .build()
            .unwrap_or_else(|e| {
                log::warn!("Failed to build reqwest client: {}. Using default.", e);
                reqwest::Client::new()
            });
        Self {
...
pub fn parse_json_array(text: &str, _protocols: &[ProxyProtocol]) -> Vec<RawProxy> {
    ...
    if let Ok(values) = serde_json::from_str::<Vec<serde_json::Value>>(text) {
        for v in values {
            let ip = v["ip"].as_str().or_else(|| v["host"].as_str()).unwrap_or("");
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(text) {
        if let Some(data) = json["data"].as_array() {
            for entry in data {
                ...
                let protocols_str = entry["protocols"]
                    .as_str()
                    .unwrap_or("http")
                    .to_lowercase();
                if ip.is_empty() || port == 0 {
...
pub async fn validate_proxy(
    ...
    match protocol {
        ProxyProtocol::Socks4 | ProxyProtocol::Socks5 => {
            ...
            tokio::time::timeout(Duration::from_secs(timeout_secs), tokio::net::TcpStream::connect(addr))
                .await
                .is_ok_and(|r| r.is_ok())
        }
...
async fn connect_via_http_validate(
    ...
    if !resp.starts_with("HTTP/1.1 200") && !resp.starts_with("HTTP/1.0 200") {
        return Err(format!("http proxy rejected: {}", resp.lines().next().unwrap_or("")));
    }
    ...
    let futs: Vec<_> = proxies
        ...
        .map(|proxy| {
            ...
            async move {
                ...
                let result = validate_proxy_multi_target(
                    &p.to_proxy_url(),
                    p.protocol,
                    &t,
                    timeout_secs,
...
mod tests {
    ...
    fn test_dedup_proxies() {
        let proxies = vec![
            RawProxy { ip: "1.2.3.4".into(), port: 8080, protocol: ProxyProtocol::Http, source: None },
            RawProxy { ip: "1.2.3.4".into(), port: 8080, protocol: ProxyProtocol::Http, source: None },
            RawProxy { ip: "5.6.7.8".into(), port: 3128, protocol: ProxyProtocol::Http, source: None },
        ];
