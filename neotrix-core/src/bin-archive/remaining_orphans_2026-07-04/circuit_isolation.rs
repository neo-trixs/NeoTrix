...
// TODO: inject via DI — pass &CircuitIsolationManager through proxy subsystem constructor
pub fn global_circuit_manager() -> &'static CircuitIsolationManager {
    static CM: LazyLock<CircuitIsolationManager> = LazyLock::new(|| {
        CircuitIsolationManager::new(CircuitIsolationConfig::default(), None)
    });
    &CM
}
    ...
    pub async fn rotate(&self) {
        ...
        *pool = new_pool;
        ...
        if let Some(ref tm) = *self.tor_manager.read().await {
            let mut last = self.last_newnym.lock().unwrap_or_else(|e| { log::warn!("[circuit] mutex poisoned: {}", e); e.into_inner() });
