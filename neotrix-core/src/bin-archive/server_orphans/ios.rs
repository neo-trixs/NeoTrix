...
use crate::neotrix::nt_act_social::filter::{SelfControlledFilter, FilterContext, ScoredMoment};
...
impl From<VideoInfoSubmission> for VideoInfo {
    fn from(s: VideoInfoSubmission) -> Self {
        VideoInfo {
            ...
            author: s.author,
            published_at: s.published_at.map(|ts| {
...
async fn social_status_handler(
    State(state): State<IosAppState>,
) -> Json<Vec<SocialStatus>> {
...
async fn chat_handler(
    ...
    let reply = if let Some(ref mut engine) = agent.reasoning_engine {
        engine.reason(&req.message).unwrap_or_else(|e| format!("error: {}", e))
    } else {
...
async fn chat_stream_handler(
    ...
    let events: Vec<Result<Event, Infallible>> = reply.chars().map(|c| {
        Ok(Event::default().data(c.to_string()))
    }).collect();
