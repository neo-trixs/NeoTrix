...
// Stub types for modules that were removed/renamed
#[derive(Clone, Copy)]
enum DataSourceType {
    HackerNews, ArXiv, GitHubTrending, SemanticScholar, Wikipedia,
}
impl DataSourceType {
...
struct ExternalDataConnector;
impl ExternalDataConnector {
    fn collect_from(_source: DataSourceType) -> Vec<Record> { vec![] }
    fn ingest_to_kb(_records: &[Record], _kb: &KnowledgeBase) -> Result<usize, String> { Ok(0) }
}
...
impl Default for Metrics {
    fn default() -> Self { Self { accuracy: 0.5, avg_loss: 0.0, total_pairs: 0 } }
}

struct PredictionTrainer;
impl PredictionTrainer {
    fn new() -> Self { Self }
    fn train_on_kb(&self, _jepa: &mut JepaWorldModel, _kb: &KnowledgeBase) -> Metrics { Metrics::default() }
    fn prediction_quality(&self) -> f64 { 0.5 }
    fn accuracy_trend_slope(&self) -> f64 { 0.0 }
}

fn select_sources(round: usize, _trainer: &PredictionTrainer) -> Vec<DataSourceType> {
    let base = round % 4;
    match base {
