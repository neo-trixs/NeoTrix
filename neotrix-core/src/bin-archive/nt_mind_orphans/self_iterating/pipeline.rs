...
use super::compile_fix_stage::CompileFixStage;
use super::test_repair_stage::TestRepairStage;
...
impl BrainStage for GwtAbsorbStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        let prm_cum = brain._prm_cumulative_reward();
        let prm_count = brain._prm_step_rewards().len();
        let prm_summary = if prm_count > 0 {
            format!(", prm_steps={}, prm_cumulative={:.4}", prm_count, prm_cum)
        } else {
            String::new()
        };
        let summary = format!("task: {}, reward: {:.4}, source: {:?}{}", task, reward, source, prm_summary);
...
make_stage!(PrmRewardBridgeStage);
impl BrainStage for PrmRewardBridgeStage {
    fn name(&self) -> &str { "prm_reward_bridge" }
    fn frequency(&self) -> usize { 1 }
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        // Read step rewards from the reasoning engine's persisted buffer
        if let Some(ref engine) = brain.reasoning_engine {
            let step_rewards = &engine.last_step_rewards;
            if !step_rewards.is_empty() {
                let count = step_rewards.len();
                // Compute discounted cumulative reward
                let gamma: f64 = 0.95;
                let cumulative: f64 = step_rewards.iter().enumerate()
                    .map(|(i, (_, r))| r * gamma.powi(i as i32))
                    .sum();
                let avg = cumulative / count.max(1) as f64;
                brain._set_prm_step_rewards(step_rewards.clone());
                brain._set_prm_cumulative_reward(cumulative);

                // Blend PRM step reward into the brain's main reward signal
                let current = brain._reward();
                let blended = 0.7 * current + 0.3 * avg;
                brain._set_reward(blended);
                brain._set_reward_source(crate::neotrix::nt_mind::core::RewardSource::Internal);

                log::debug!(
                    "[prm-bridge] {} step rewards, cumulative={:.4}, avg={:.4}, blended_reward={:.4}",
                    count, cumulative, avg, blended,
                );
            }
        } else {
            log::trace!("[prm-bridge] no reasoning engine available, skipping");
        }
        Ok(StageDecision::Continue)
    }
}
...
impl BrainStage for E8ExperimentStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        // GRPO: group-relative policy optimization over recent outcomes
        // Uses last G=4 outcomes as a group for contrastive advantage estimation
        const GRPO_GROUP_SIZE: usize = 4;
        let recent_count = brain._transition_learner.outcomes.len();
        if recent_count >= GRPO_GROUP_SIZE {
            let trajectories: Vec<_> = brain._transition_learner.outcomes
                .iter()
                .rev()
                .take(GRPO_GROUP_SIZE)
                .map(|o| (o.mode, o.reward))
                .collect();
            let (avg_adv, kl) = brain._e8_policy.grpo_update(GRPO_GROUP_SIZE, &trajectories);
            if avg_adv > 0.1 || kl > 0.01 {
                log::debug!("[e8-grpo] avg_advantage={:.4}, kl={:.6}, group_size={}", avg_adv, kl, GRPO_GROUP_SIZE);
            }
        } else {
            // Fallback: basic TD update with step rewards if available
            let step_rewards = brain._take_prm_step_rewards();
            if !step_rewards.is_empty() {
                brain._e8_policy.update_with_step_rewards(reward, &step_rewards, 0.6);
                log::debug!(
                    "[e8-experiment] step-reward TD update with {} step rewards, reward={:.4}",
                    step_rewards.len(), reward,
                );
            } else {
...
// ── DPO Stage ──────────────────────────────────────────────────
make_stage!(DpoStageWrapper);
impl BrainStage for DpoStageWrapper {
    fn name(&self) -> &str { "dpo" }
    fn frequency(&self) -> usize { 3 }
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let _count = brain._dpo_stage.buffer.len();
        Ok(StageDecision::Continue)
    }
}

// ── Constitutional Self-Critique Stage ─────────────────────────
make_stage!(ConstitutionalStageWrapper);
impl BrainStage for ConstitutionalStageWrapper {
    fn name(&self) -> &str { "constitutional_self_critique" }
    fn frequency(&self) -> usize { 3 }
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let edit_deltas: Vec<(String, f64)> = brain._micro_edits.iter().filter_map(|e| {
            match e {
                crate::core::MicroEdit::AdjustDimension(name, val) => Some((name.clone(), *val)),
                crate::core::MicroEdit::AddedDimension(name, val) => Some((name.clone(), *val)),
                crate::core::MicroEdit::ModifiedDimension(name, _, val) => Some((name.clone(), *val)),
                crate::core::MicroEdit::BatchAdjust(pairs) => pairs.first().map(|(n, v)| (n.clone(), *v)),
                _ => None,
            }
        }).collect();
        let (_result, _adjusted, _reflect) = brain._constitutional_stage.process(&edit_deltas, brain._reward);
        Ok(StageDecision::Continue)
    }
}

// ── Safety Check Stage ─────────────────────────────────────────
make_stage!(SafetyStageWrapper);
impl BrainStage for SafetyStageWrapper {
    fn name(&self) -> &str { "safety_check" }
    fn frequency(&self) -> usize { 2 }
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let deltas: Vec<(String, f64)> = brain._micro_edits.iter().filter_map(|e| {
            match e {
                crate::core::MicroEdit::AdjustDimension(name, val) => Some((name.clone(), *val)),
                crate::core::MicroEdit::AddedDimension(name, val) => Some((name.clone(), *val)),
                crate::core::MicroEdit::ModifiedDimension(name, _, val) => Some((name.clone(), *val)),
        ...
        let (_result, check_result, _adjusted) = brain._safety_stage.evaluate(&deltas, brain._reward);
        match check_result {
            super::hypercore::SafetyCheckResult::Failed { reason } => {
                Ok(StageDecision::Skip(format!("safety_check: {}", reason)))
            }
            super::hypercore::SafetyCheckResult::NeedsHumanReview { .. } => {
                Ok(StageDecision::Continue)
            }
            super::hypercore::SafetyCheckResult::Passed => {
                Ok(StageDecision::Continue)
            }
        }
    }
...
pub fn seal_pipeline() -> BrainPipeline {
    BrainPipeline::with_stages(vec![
        ...
        Box::new(DpoStageWrapper::new()),
        Box::new(ConstitutionalStageWrapper::new()),
        Box::new(SafetyStageWrapper::new()),
        Box::new(crate::neotrix::nt_mind::self_iterating::skillopt::BoundedEditStage::new()),
        Box::new(ApplyEditsStage::new()),
        Box::new(CompileFixStage::new()),
        Box::new(TestRepairStage::new()),
        ...
        Box::new(PrmRewardBridgeStage::new()),
        Box::new(E8ExperimentStage::new()),
        Box::new(super::procedural_memory::ProceduralMemoryStage::new()),
...
mod tests {
    ...
    fn test_pipeline_stages_order() {
        ...
        assert_eq!(names, vec![
            ...
            "dpo", "constitutional_self_critique", "safety_check",
            "bounded_edit", "apply_edits",
            "compile_fix", "test_repair",
