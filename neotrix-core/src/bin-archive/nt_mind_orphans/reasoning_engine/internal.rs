...
use crate::neotrix::nt_mind::reasoning_engine::{ReasoningEngine, CostRecord, EngineMetrics, ReasoningStats};
...
impl ReasoningEngine {
    /// Run a future, respecting any existing tokio runtime context.
    /// Avoids panicking with "Cannot block from within a runtime" by detecting
    /// an active runtime handle and using it instead of the dedicated runtime.
    fn block_on_future<F: std::future::Future<Output = T>, T>(&self, f: F) -> T {
    ...
    pub(super) fn call_llm_with_ctx(&mut self, prompt: &str, _context_size: u32) -> NeoTrixResult<String> {
        let guard = PromptGuard::new();
        match guard.analyze(prompt).0 {
            RiskLevel::Dangerous => return Err(NeoTrixError::Brain("LLM 调用被阻止: 检测到注入模式".to_string())),
        ...
        let response = self.block_on_future(self.gateway.complete_with_selection(&request));
        ...
        match response {
            ...
            Err(e) => {
                for _fallback_tier in [ModelTier::T4, ModelTier::T3, ModelTier::T2] {
                    if let Some(mapping) = self.router.fallback(route.tier) {
                        ...
                        if let Ok(fb_resp) = self.block_on_future(self.gateway.complete_with_selection(&fb_request)) {
                            self.cost_log.push(CostRecord {
                                timestamp: Utc::now().timestamp(),
    ...
    pub fn cost_report(&self) -> serde_json::Value {
        ...
        let by_tier: std::collections::HashMap<String, Vec<&CostRecord>> = self.cost_log.iter()
    ...
    pub fn infer_reasoning_type(task: &str) -> ReasoningType {
        ...
        if t.contains("error") || t.contains("bug") || t.contains("fail") || t.contains("exception")
            || t.contains("panic") || t.contains("crash") || t.contains("错误") || t.contains("失败") {
            ReasoningType::ErrorDebugging
        } else if t.contains("knowledge") || t.contains("what is") || t.contains("explain")
            || t.contains("知识") || t.contains("什么") {
            ReasoningType::KnowledgeQuery
        } else if t.contains("task") || t.contains("implement") || t.contains("solve")
            || t.contains("任务") || t.contains("实现") || t.contains("解决") {
    ...
    pub(super) fn build_context(&mut self, query: &str, rtype: ReasoningType) -> String {
        ...
        results.iter()
            .map(|m| format!("  [{}] {}", if m.success { "SUCCESS" } else { "FAILURE" }, m.task_description))
            .collect::<Vec<_>>().join("\n")
    }
    ...
    pub fn build_artifact_context(&self, query: &str) -> String {
        ...
        for a in &artifacts {
            ...
            out.push_str(&format!("  [{}] ({})\n    {}\n", a.name, a.artifact_type.label(), preview));
        }
        out.push('\n');
        out
    }

    pub(super) fn record_trace(&mut self, rt: ReasoningType, task: &str, prompt: &str, response: &str, err: Option<&str>, score: f64) {
    ...
    pub fn learn_from_execution(&mut self, task: &str, exec_result: &Result<(String, String), String>) {
        ...
        self.brain.capability.arr_mut()[depth_idx] = (current + reward * 0.05).clamp(0.0, 1.0);
        ...
        match check_result {
            Ok(output) if output.status.success() => {
                ...
                if let Some(idx) = CapabilityVector::index_from_name("quality_gates") {
                    ...
                    self.brain.capability.arr_mut()[idx] = (cur + 0.05).min(1.0);
                }
            }
            Ok(output) => {
                ...
                self.anti_patterns.push(AntiPattern {
                    id: uuid::Uuid::new_v4().to_string(),
                    description: format!("Compile error: {}", &stderr.lines().next().unwrap_or("unknown")),
    ...
    pub fn traces_to_tree(&self, limit: usize) -> String {
        ...
        if traces.is_empty() { return "(无推理记录)".to_string(); }

        let mut out = String::new();
        out.push_str("因果链 (ReasoningTrace)\n");
        out.push_str("═══════════════════════\n");
        for (i, t) in traces.iter().enumerate() {
            let prefix = if i == traces.len() - 1 { " └─ " } else { " ├─ " };
            out.push_str(&format!("{}#{} [{}] {}\n", prefix, traces.len() - i,
                format!("{:?}", t.reasoning_type).to_lowercase(),
                t.task.chars().take(40).collect::<String>()));

            let method = t.reasoning_method.map(|m| format!("{:?}", m)).unwrap_or_default();
            let lens = t.perspective_lens.map(|l| format!("{:?}", l)).unwrap_or_default();
            let sub_prefix = if i == traces.len() - 1 { "    " } else { " │  " };
...
#[allow(dead_code)]
pub fn build_prd_context(_input: &super::prd_gen::PrdInput) -> String {
    String::new()
}
