...
fn main() {
    ...
    // Phase 1: Reset stuck "processing" items (method removed, skipping)
    // TODO: restore when reset_stuck_items is re-added
    println!("\n━━━ Phase 1: 重置卡住的 processing 条目 ━━━");
    println!("  ⏭ skipped (deprecated)");
    ...
    // Phase 2b: Purge skip domains — skip in full mode (method removed)
    // TODO: restore when purge_skip_domains is re-added
    if !fetch_links {
        println!("\n━━━ Phase 2b: 清除已知无用域名 ━━━");
        println!("  ⏭ skipped (deprecated)");
    }

    // Phase 2c: Quick connectivity validation — skip in full mode (method removed)
    // TODO: restore when validate_urls is re-added
    if !fetch_links {
        println!("\n━━━ Phase 2c: 连通性验证 (HEAD, 1s timeout) ━━━");
        println!("  ⏭ skipped (deprecated)");
    }

    // Phase 2d: Bulk-delete all skip-pattern URLs — skip in full mode (method removed)
    // TODO: restore when purge_all_skip_patterns is re-added
    if !fetch_links {
    ...
    for cycle in 1..=cycles {
        ...
        if remaining == 0 && cycle > 1 {
            ...
            match kb.run_crawl_cycle(batch) {
            Ok(report) => {
