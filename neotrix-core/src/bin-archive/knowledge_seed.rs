...
fn main() {
    ...
    // GitHub search — batch ingest (method removed, skipping)
    // TODO: restore when ingest_github_search is re-added
    for q in &["topic:book+stars:>100", "topic:ebook+stars:>50", "topic:open+library+books",
               "topic:chinese-literature", "topic:textbooks"] {
        println!("  ⏭ GitHub search '{}' — skipped (deprecated)", q);
    }
    ...
    // Direction 3: OpenLibrary — direct ingest (method removed, skipping)
    // TODO: restore when ingest_openlibrary_search is re-added
    println!("\n━━━ Direction 3: OpenLibrary 书籍注入 (直接 API — 已跳过) ━━━");
    for q in &["fiction","non_fiction","fantasy","history","science","ebook",
        ...
        println!("  ⏭ OpenLibrary '{}' — skipped (deprecated)", q);
    }
