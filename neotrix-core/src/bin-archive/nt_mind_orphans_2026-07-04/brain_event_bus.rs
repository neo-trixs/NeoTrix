...
// TODO: inject via DI — pass global relay through a shared context struct instead
...
impl BrainEventBus {
    ...
    pub fn emit(&self, event: BrainEvent) {
        if let Err(e) = self.sender.send(event.clone()) {
            log::warn!("[brain_event_bus] drop local event: {:?}", e);
        }
        if let Some(tx) = global_sender() {
            if let Err(e) = tx.send(event) {
                log::warn!("[brain_event_bus] drop global event: {:?}", e);
            }
        }
    }
...
mod tests {
    ...
    fn test_stage_event_roundtrip() {
        ...
        match parsed {
            BrainEvent::Stage { name, status, duration_ms } => {
