...
use crate::neotrix::nt_io_provider::{
    LlmRequest, LlmError, LlmProviderType,
    factory::create_gateway, GatewayV2,
};
...
/// 后备 LLM 供应商路由表 — 基于 GatewayV2 自动选择
...
impl ProviderRouter {
    ...
    pub fn gateway(&self) -> &Arc<GatewayV2> {
        &self.gateway
    }

    pub fn register_gateway(&mut self, gateway: Arc<GatewayV2>) {
        self.gateway = gateway;
    }

    pub async fn complete(&self, prompt: &str) -> Result<crate::neotrix::nt_io_provider::LlmResponse, LlmError> {
...
impl ConsciousnessReasoner {
    ...
    pub async fn reason(&self, prompt: &str) -> ReasonerResponse {
        ...
        if internal.confidence >= 0.6 {
            return ReasonerResponse {
                ...
                duration_ms: elapsed,
                model_used: "NeoTrix Consciousness (E8)".into(),
            };
        }

        let llm_start = std::time::Instant::now();
        let provider = self.provider.lock().unwrap_or_else(|e| e.into_inner());
        let model_name = provider.current_name().to_string();
    ...
    fn internal_reason(&self, prompt: &str) -> InternalResult {
        ...
        InternalResult { content, confidence: calibrated }
    }
    ...
    pub fn get_info(&self) -> ReasonerInfo {
        ...
        ReasonerInfo {
            default_model: "NeoTrix Consciousness (E8)".into(),
    ...
    pub fn available_providers() -> Vec<ProviderInfo> {
        ProviderRouter::available().into_iter().map(|t| {
            let name = match t {
