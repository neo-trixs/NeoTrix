...
impl DiscoveryAgent {
    ...
    fn search_arxiv(&self) -> Vec<DiscoveryResult> {
        ...
        for query in SEARCH_QUERIES.iter() {
            ...
            for entry in resp.split("<entry>").skip(1) {
                ...
                let clean_summary: String = summary.replace('\n', " ").chars().take(300).collect::<String>();
    ...
    fn search_github(&self) -> Vec<DiscoveryResult> {
        ...
        for q in &queries {
            let url = format!("https://api.github.com/search/repositories?q={}&sort=stars&order=desc&per_page=5", q);
            let resp = match client.get(&url).send().and_then(|r| r.text()) {
                Ok(text) => text,
                Err(_) => continue,
            };
    ...
    fn score_relevance(&self, title: &str, summary: &str) -> f64 {
        let keywords = [
            ("agent", 0.3), ("llm", 0.25), ("rust", 0.2), ("autonomous", 0.25),
            ("self", 0.2), ("evolv", 0.25), ("orchestrat", 0.2), ("framework", 0.15),
            ("reasoning", 0.2), ("multi", 0.15), ("code", 0.15), ("tool", 0.15),
            ("chain", 0.1), ("pipeline", 0.1), ("memory", 0.15), ("learn", 0.1),
            ("mcp", 0.2), ("protocol", 0.1), ("language model", 0.2),
        ];
    ...
    fn matched_dims(&self, title: &str, summary: &str) -> Vec<String> {
        ...
        let dim_keywords: [(&str, &[&str]); 7] = [
            ("agent_orchestration", &["agent", "orchestrat", "multi-agent", "swarm"]),
            ("code_understanding", &["code", "program", "compil", "static analysis"]),
            ("self_evolution", &["evolv", "self-improve", "adapt", "learn"]),
            ("knowledge_integration", &["knowledge", "memory", "retriev", "rag"]),
            ("multi_modal", &["multimodal", "vision", "audio", "image"]),
            ("reasoning_depth", &["reason", "infer", "think", "chain-of-thought"]),
    ...
    fn extract_xml_tag(xml: &str, tag: &str) -> Option<String> {
        ...
        xml.find(&open).and_then(|start| {
            ...
            xml[start..].find(&close).map(|end| xml[start..start + end].to_string())
        })
    }
}
