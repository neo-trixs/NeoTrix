...
impl PanoramicBrain {
    ...
    pub fn route_task(&self, task: &str, task_type: Option<TaskType>, use_rubric: bool) -> FrameworkRoute {
        ...
        let mut weights: Vec<(EarthEpoch, f64)> = EarthEpoch::all().iter().filter_map(|epoch| {
    ...
    pub fn epoch_report(&self) -> Vec<(EarthEpoch, f64, f64, usize)> {
        let mut report: Vec<(EarthEpoch, f64, f64, usize)> = EarthEpoch::all().iter().filter_map(|epoch| {
            let fw = self.frameworks.get(epoch)?;
            let avg_state = fw.state.iter().sum::<f64>() / fw.state.len() as f64;
            Some((fw.epoch, avg_state, fw.effective_weight(), fw.activation_count as usize))
        }).collect();
