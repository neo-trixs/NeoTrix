...
fn main() {
    ...
    for (title, body, tags, imp) in &repos {
        if !eng.entries.values().any(|x| x.title.contains(title)) {
            eng.add_entry(KnowledgeEntry::new(title, body, SourceType::KnowledgeBase, "kb:github3")
    ...
    for (f, t, d) in [("TCM-Ancient-Books","tcm-texts","中医古籍集合"),("Ancient-China-Books","sikuquanshuzongmu","四库组织"),
        ("ChuangTzu-text","正统道藏三洞体系","道藏庄子"),("TCM-Ancient-Books","八纲辨证","中医古籍→辨证")] {
    ...
    println!("\n💾 知识引擎: {}条目(+{}), {}关系\nGitHub合计: {}条",
        eng.stats().total_entries, eng.stats().total_entries - before,
        eng.stats().total_relations, eng.search_by_tag("GitHub", 1000).len());
}
