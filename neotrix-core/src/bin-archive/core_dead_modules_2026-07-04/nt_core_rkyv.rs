...
// TODO: inject via DI — pass storage path through RkyvStorage constructor
...
impl<K, V> RkyvStorage<K, V> {
    ...
    where
        ...
        V: rkyv::Serialize<
            rkyv::api::high::HighSerializer<
                ...
                rkyv::ser::allocator::ArenaHandle<'_>,
                rkyv::rancor::Error,
            >,
        >,
    {
    ...
    where
        K: Hash,
        V: rkyv::Archive,
        V::Archived: rkyv::Deserialize<V, rkyv::de::Pool>
            + for<'a> rkyv::bytecheck::CheckBytes<
                rkyv::validation::Validator<'a, rkyv::validation::shared::SharedContext>,
            >,
    {
        let path = self.key_path(key);
        if !path.exists() {
...
mod tests {
    ...
    fn test_store_and_load() {
        ...
        let loaded = store.load_deserialized(&"key1".to_string()).expect("load_deserialized should return stored value");
    ...
    fn test_exists() {
        ...
        store.store(&"exist_key".to_string(), &val).expect("store should succeed for exists test");
    ...
    fn test_delete() {
        ...
        store.store(&"del_key".to_string(), &val).expect("store should succeed for delete test");
