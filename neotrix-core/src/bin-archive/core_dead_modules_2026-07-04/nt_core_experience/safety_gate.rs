...
impl SafetyGate {
    ...
    pub fn check_all(
        ...
        let checks = vec![
            self.check_self_consistency(self_compile_ok),
            self.check_backward_compatibility(backward_compat_scores),
            self.check_behavior_preservation(vsa_primitives_current),
            self.check_negentropy(negentropy_delta),
            self.check_meta_accuracy(meta_predicted, meta_actual),
        ];
...
mod tests {
    ...
    fn behavior_preservation_fails_on_mismatch() {
        let gate = make_ref();
        let gate = SafetyGate { meta_accuracy_tolerance: 0.01, ..gate };
        ...
        assert!(!report.all_passed, "safety gate should reject mismatched primitives: {:?}", report);
    }
