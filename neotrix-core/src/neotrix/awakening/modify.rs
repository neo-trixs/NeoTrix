...
use neotrix_types::capability::CapabilityVector;
...
impl SelfModify {
    ...
    pub fn apply(&mut self, intervention: &Intervention, brain: &mut ReasoningBrain) -> Result<(), String> {
        match intervention {
            Intervention::CapabilityDelta { index, delta } => {
                if *index >= brain.capability.arr.len() {
                    return Err(format!("capability index {} out of bounds ({})", index, brain.capability.arr.len()));
                }
                brain.capability.arr[*index] = (brain.capability.arr[*index] + delta).clamp(0.0, 1.0);
    ...
    pub fn apply_batch(
        ...
        InterventionResult {
            ...
            rolled_back: false,
            message: format!("applied {} interventions, Δcapability={:.4}", interventions.len(), after - before),
        }
    }
...
mod tests {
    ...
    fn test_apply_capability_delta() {
        ...
        let result = modifier.apply_batch(
            &[Intervention::CapabilityDelta { index: 0, delta: 0.3 }],
            &mut brain,
        );
    ...
    fn test_bounds_checking() {
        ...
        let mut modifier = SelfModify::new();
        let result = modifier.apply_batch(
            &[Intervention::CapabilityDelta { index: 99, delta: 0.1 }],
            &mut brain,
        );
        assert!(!result.applied);
        assert!(result.rolled_back);
