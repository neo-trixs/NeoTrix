...
pub fn aes_ige_encrypt(key: &[u8; 32], iv: &[u8; 32], data: &[u8]) -> Vec<u8> {
    let cipher =
        Aes256::new_from_slice(key).expect("AES-256 key is &[u8; 32], guaranteed by type system");
    ...
    for chunk in blocks {
        let plain_block = <&[u8; AES_BLOCK]>::try_from(chunk)
            .expect("chunks_exact yields exactly AES_BLOCK=16 bytes per chunk");
...
pub fn aes_ige_decrypt(key: &[u8; 32], iv: &[u8; 32], data: &[u8]) -> Vec<u8> {
    ...
    for chunk in blocks {
        let original_c = *<&[u8; AES_BLOCK]>::try_from(chunk)
            .expect("chunks_exact yields exactly AES_BLOCK=16 bytes per chunk");
...
pub fn parse_biguint_hex(hex_str: &str) -> BigUint {
    BigUint::parse_bytes(hex_str.as_bytes(), 16)
        .expect("DH_PRIME is a compile-time valid hex string constant")
}
...
mod tests {
    ...
    fn test_server_client_key_differs() {
        ...
        assert_ne!(
            sk, ck,
            "server/client keys must differ with non-uniform auth_key"
        );
    }
}
