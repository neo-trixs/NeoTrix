...
impl MtpTransport {
    ...
    pub async fn send_handshake(&mut self) -> Result<(), String> {
        match self.protocol {
            TransportProtocol::Abridged => self
                .stream
                .write_all(&ABRIDGED_HEADER)
                .await
                .map_err(|e| e.to_string()),
            TransportProtocol::Intermediate => self
                .stream
                .write_all(&[0xee, 0xee, 0xee, 0xee])
                .await
                .map_err(|e| e.to_string()),
        }
    }
    ...
    pub async fn read_handshake(&mut self) -> Result<TransportProtocol, String> {
        ...
        if first[0] == 0xef {
            ...
            self.stream
                .read_exact(&mut rest)
                .await
                .map_err(|e| format!("read int handshake: {}", e))?;
...
mod tests {
    ...
    async fn test_abridged_roundtrip() {
        ...
        t.write_packet(payload)
            .await
            .expect("client should write packet");
