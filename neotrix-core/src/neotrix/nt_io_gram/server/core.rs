...
use crate::neotrix::nt_io_gram::transport::{MtpTransport, TransportProtocol};
    ...
    loop {
        ...
        let (decrypted, _computed_msg_key) = {
            let auth_key = session
                .auth_key
                .as_ref()
                .ok_or_else(|| "no auth key".to_string())?;
            let msg_key: [u8; 16] = {
                let mut k = [0u8; 16];
            ...
            let (aes_key, aes_iv) = crypto::generate_aes_key_iv(auth_key, &msg_key, true);
            let decrypted = crypto::aes_ige_decrypt(&aes_key, &aes_iv, encrypted_data);
...
async fn perform_key_exchange(transport: &mut MtpTransport) -> Result<[u8; 256], String> {
    let packet = transport.read_packet().await?;
    ...
    let client_inner =
        key_exchange::decrypt_client_dh_inner_data(&server_nonce, &set_params.encrypted_data)?;
