...
use super::proxy_pool::{ProxyNode, ProxyPool};
...
impl ProxyHeartbeatEngine {
    ...
    pub async fn summary(&self) -> HeartbeatSummary {
        ...
        let (last_proxy_url, last_proxy_geo, last_fingerprint_id) = match last {
            Some(r) => (
                r.proxy_url.clone(),
                r.proxy_geo.clone().unwrap_or_default(),
                r.fingerprint_id,
            ),
        ...
        let recent_success = history
            .iter()
            .rev()
            .take(recent_count)
            .filter(|r| r.success)
            .count();
        let recent_success_rate = if recent_count > 0 {
        ...
        let avg_interval = if history.len() >= 2 {
            ...
            if history.len() > 1 {
                duration / (history.len() - 1) as f64 * 1000.0
            } else {
                0.0
            }
        } else {
        ...
        let rotation_count_last_hour = history
            .iter()
            .rev()
...
mod tests {
    use super::*;
    use crate::neotrix::nt_io_stealth_net::proxy_pool::ProxyPool;
