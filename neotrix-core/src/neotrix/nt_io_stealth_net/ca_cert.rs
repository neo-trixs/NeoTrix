...
impl CaCertManager {
    ...
    pub fn install_macos(&self) -> Result<(), String> {
        ...
        log::info!("[ca] CA certificate installed in macOS trust store");
        Ok(())
    }
    ...
    pub fn uninstall_macos(&self) -> Result<(), String> {
        ...
        if let Some(fp) = sha1.split('=').nth(1) {
            ...
            log::info!("[ca] CA certificate removed from macOS trust store");
        }
        Ok(())
    }
...
pub fn build_client_tls_config(
    ...
    let suites: Vec<SupportedCipherSuite> = rustls::ALL_CIPHER_SUITES
        .iter()
        .copied()
        .filter(|cs| match cs {
            SupportedCipherSuite::Tls13(s) => cipher_order
                .iter()
                .any(|name| cipher_suite_name_match(&s.common.suite, name)),
            _ => false,
        })
        .collect();
