...
use super::config::{
    self, stealth_user_agent, ProxyConfig, DEFAULT_TIMEOUT_SECS, MAX_REDIRECTS,
    STEALTH_CONNECT_TIMEOUT_SECS, STEALTH_POOL_IDLE_TIMEOUT_SECS, STEALTH_POOL_MAX_IDLE,
};
...
impl StealthHttpClient {
    pub(super) fn build_client_for_proxy(
        ...
        let use_insecure = tls_insecure
            && variant != TlsVariant::StrictVerify
            && variant != TlsVariant::LegacyStrict;
        ...
        let mut builder = reqwest::Client::builder()
            ...
            .resolve_to_addrs(
                ...
                &[
                    ...
                    std::net::SocketAddr::from(([8, 8, 4, 4], 443)),
                ],
            )
            .resolve_to_addrs(
                "cloudflare-dns.com",
                &[
                    ...
                    std::net::SocketAddr::from(([1, 0, 0, 1], 443)),
                ],
            )
            .resolve_to_addrs(
                "dns.quad9.net",
                &[
                    std::net::SocketAddr::from(([9, 9, 9, 9], 443)),
                    std::net::SocketAddr::from(([149, 112, 112, 112], 443)),
                ],
            );
    ...
    pub(super) async fn get_or_create_client(
        &self,
        proxy_url: &str,
    ) -> Result<reqwest::Client, String> {
        ...
        if let Some(entry) = pool
            .iter()
            .find(|e| e.proxy_url == proxy_url && e.created_at.elapsed() < Duration::from_secs(9))
        {
    ...
    pub(super) async fn current_geo(&self) -> Option<String> {
        ...
        match &proxy_config {
            ProxyConfig::DynamicChain(chain) => chain.current_exit_geo().await,
            _ => None,
        }
    }
...
mod tests {
    ...
    fn test_build_client_no_proxy() {
        let client =
            StealthHttpClient::build_client_for_proxy(&ProxyConfig::None, false, None, None);
        assert!(client.is_ok());
    }
    ...
    fn test_build_client_static_http_proxy() {
        let client = StealthHttpClient::build_client_for_proxy(
            &ProxyConfig::Static("http://127.0.0.1:8080".into()),
            false,
            None,
            None,
        );
    ...
    fn test_build_client_tor() {
        let client =
            StealthHttpClient::build_client_for_proxy(&ProxyConfig::Tor, false, None, None);
        assert!(client.is_ok());
    }
    ...
    fn test_build_client_with_local_address() {
        ...
        let client =
            StealthHttpClient::build_client_for_proxy(&ProxyConfig::None, false, None, Some(addr));
        assert!(client.is_ok());
    }
    ...
    fn test_build_client_https_proxy_insecure() {
        let client = StealthHttpClient::build_client_for_proxy(
            &ProxyConfig::Static("https://proxy:8443".into()),
            true,
            None,
            None,
        );
    ...
    fn test_client_entry_construction() {
        ...
        let entry = ClientEntry {
            client: raw.clone(),
            proxy_url: format!("socks5://{}", TOR_SOCKS_ADDR),
            created_at: std::time::Instant::now(),
        };
        assert_eq!(entry.proxy_url, format!("socks5://{}", TOR_SOCKS_ADDR));
    }
}
