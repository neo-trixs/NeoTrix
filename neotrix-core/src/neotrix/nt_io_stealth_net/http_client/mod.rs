...
use super::bandit::{ComboArm, FingerprintBandit};
...
use super::system_fingerprint::{
    SystemFingerprint, SystemFingerprintConfig, SystemFingerprintGenerator,
};
...
pub use config::{stealth_user_agent, EntropyBudget, ProxyConfig};
...
impl StealthHttpClient {
    ...
    fn build_or_fallback(proxy: ProxyConfig, label: &str) -> Self {
        match Self::build(proxy) {
            ...
            Err(e) => {
                log::warn!(
                    "StealthHttpClient::{} failed (fallback to no-proxy): {}",
                    label,
                    e
                );
                Self::build(ProxyConfig::None).unwrap_or_else(|_| {
                    let client = build_async_client();
                    let fp = SystemFingerprintGenerator::new()
    ...
    pub async fn rotate_tls_variant_for_host(&self, host: Option<&str>) {
        ...
        *self.current_combo_arm.write().await = new_arm.clone();
        ...
        if let Some(ref coord) = *self.coordinator.read().await {
            ...
            let min_factor = 0.5;
            let effective_mean =
                base_mean * (min_factor + conf * (1.0 - min_factor)).max(min_factor);
    ...
    pub async fn bind_lan_router(&self, router: Arc<LanRouter>) {
        *self.lan_router.write().await = Some(router.clone());
        tokio::spawn(async move {
            ...
            .await
            {
                log::error!("[http_client] LAN router rotation loop panic: {:?}", panic);
            }
        });
    }
    ...
    pub async fn bind_ip_privacy(&self, mgr: Arc<IpPrivacyManager>) {
        *self.ip_privacy.write().await = Some(mgr.clone());
        tokio::spawn(async move {
            ...
            .await
            {
                log::error!("[http_client] IP privacy rotation loop panic: {:?}", panic);
            }
        });
    }
...
mod tests {
    ...
    fn test_client_with_socks5() {
        let client = StealthHttpClient::with_socks5(&format!(
            "socks5://{}",
            crate::neotrix::nt_io_stealth_net::local_proxy::TOR_SOCKS_ADDR
        ));
        assert!(client.check_tracker);
    }
    ...
    fn test_response_text() {
        ...
        assert_eq!(
            resp.text().expect("value should be ok in test"),
            "hello world"
        );
    }
