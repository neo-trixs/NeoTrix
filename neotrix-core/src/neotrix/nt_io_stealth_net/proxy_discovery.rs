...
use super::proxy_sourcing::{self, batch_validate, ProxyProtocol, RawProxy};
...
impl DiscoveryTargetHealth {
    ...
    fn record_failure(&mut self) {
        ...
        if self.consecutive_failures >= MAX_TARGET_FAILURES {
            self.cooldown_until = Some(Instant::now() + Duration::from_secs(TARGET_COOLDOWN_SECS));
        }
    }
...
impl ProxyDiscoveryEngine {
    ...
    async fn crawl_custom_target(&self, url: &str) -> Result<Vec<RawProxy>, String> {
        ...
        if text.contains('\n') && text.lines().count() > 2 {
            let proxies = proxy_sourcing::parse_plain_text(
                &text,
                &[
                    ProxyProtocol::Http,
                    ProxyProtocol::Https,
    ...
    async fn validate_and_feed(&self, target: &DiscoveryTarget, proxies: Vec<RawProxy>) -> usize {
        ...
        let validated = batch_validate(&proxies, 50, 5, DEFAULT_VALIDATION_TARGETS).await;

        let valid_count = validated.iter().filter(|(_, r)| r.connect_ok).count();
    ...
    pub fn add_discovery_target(&self, url: &str) {
        let mut targets = tokio::task::block_in_place(|| self.discovered_targets.blocking_write());
    ...
    pub async fn target_health_snapshot(&self) -> Vec<(&'static str, DiscoveryTargetHealth)> {
        ...
        DISCOVERY_TARGETS
            .iter()
            .map(|t| (t.name, health.get(t.name).cloned().unwrap_or_default()))
            .collect()
    }

    pub async fn last_run_elapsed(&self) -> Option<Duration> {
        self.last_run.read().await.map(|t| t.elapsed())
    }
...
fn extract_proxies_from_html(html: &str, hint_protocol: Option<ProxyProtocol>) -> Vec<RawProxy> {
...
fn detect_protocol(line: &str, hint: Option<ProxyProtocol>, _port: u16) -> ProxyProtocol {
...
mod tests {
    ...
    fn test_detect_protocol_from_text() {
        ...
        assert_eq!(detect_protocol(line, None, 1080), ProxyProtocol::Socks5);

        let line = "socks4://1.2.3.4:1080";
        assert_eq!(detect_protocol(line, None, 1080), ProxyProtocol::Socks4);

        let line = "https://1.2.3.4:443";
        assert_eq!(detect_protocol(line, None, 443), ProxyProtocol::Https);
    }
