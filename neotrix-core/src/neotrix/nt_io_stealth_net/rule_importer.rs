...
pub fn import_shadowrocket_rules(
    text: &str,
) -> Result<Vec<(RuleCondition, OutboundAction)>, String> {
...
mod tests {
    ...
    fn test_parse_domain_suffix() {
        let result = parse_shadowrocket_line("DOMAIN-SUFFIX,google.com,Proxy")
            .expect("parse_shadowrocket_line should succeed for valid DOMAIN-SUFFIX");
        assert_eq!(
            result.0,
            RuleCondition::DomainSuffix("google.com".to_string())
        );
    ...
    fn test_parse_domain_keyword() {
        ...
        let result = parse_shadowrocket_line("DOMAIN-KEYWORD,google,Proxy")
            .expect("parse_shadowrocket_line should succeed for valid DOMAIN-KEYWORD");
    ...
    fn test_parse_domain_exact() {
        let result = parse_shadowrocket_line("DOMAIN,example.com,Proxy")
            .expect("parse_shadowrocket_line should succeed for valid DOMAIN");
        assert_eq!(
            result.0,
            RuleCondition::DomainExact("example.com".to_string())
        );
        assert!(matches!(result.1, OutboundAction::Proxy(_)));
    }
    ...
    fn test_parse_ip_cidr() {
        let result = parse_shadowrocket_line("IP-CIDR,10.0.0.0/8,DIRECT")
            .expect("parse_shadowrocket_line should succeed for valid IP-CIDR");
    ...
    fn test_parse_ip_cidr6() {
        let result = parse_shadowrocket_line("IP-CIDR6,::1/128,DIRECT")
            .expect("parse_shadowrocket_line should succeed for valid IP-CIDR6");
        assert!(matches!(result.0, RuleCondition::Cidr(..)));
        assert!(matches!(result.1, OutboundAction::Direct));
    }
    ...
    #[test]
    fn test_parse_final() {
        let result = parse_shadowrocket_line("FINAL,DIRECT")
            .expect("parse_shadowrocket_line should succeed for valid FINAL");
    ...
    fn test_parse_action_case_insensitive() {
        let r1 = parse_shadowrocket_line("DOMAIN-SUFFIX,test.com,proxy")
            .expect("parse_shadowrocket_line should succeed for lowercase proxy action");
        assert!(matches!(r1.1, OutboundAction::Proxy(_)));
        let r2 = parse_shadowrocket_line("DOMAIN-SUFFIX,test.com,direct")
            .expect("parse_shadowrocket_line should succeed for lowercase direct action");
        assert!(matches!(r2.1, OutboundAction::Direct));
        let r3 = parse_shadowrocket_line("DOMAIN-SUFFIX,test.com,reject")
            .expect("parse_shadowrocket_line should succeed for lowercase reject action");
        assert!(matches!(r3.1, OutboundAction::Block));
    }

    #[test]
    fn test_parse_reject_as_block() {
        let result = parse_shadowrocket_line("DOMAIN-SUFFIX,evil.com,REJECT")
            .expect("parse_shadowrocket_line should succeed for uppercase REJECT action");
...
GEOIP,CN,DIRECT
FINAL,DIRECT";
        let rules = import_shadowrocket_rules(text)
            .expect("import_shadowrocket_rules should succeed for valid Shadowrocket config");
    ...
    fn test_domain_suffix_matches_contains() {
        ...
        let url = Url::parse("https://fonts.googleapis.com/path")
            .expect("Url::parse of hardcoded test URL should never fail");
        assert!(cond.matches(&url));
    ...
    fn test_parse_proxy_uses_default_tag() {
        let result = parse_shadowrocket_line("DOMAIN-SUFFIX,test.com,Proxy")
            .expect("parse_shadowrocket_line should succeed for Proxy action validation");
        assert!(
