...
use crate::neotrix::nt_io_http_factory::{H2SettingsProfile, TlsVariant};
...
impl FingerprintBandit {
    ...
    pub fn new() -> Self {
        let arms = ComboArm::all()
            .iter()
            .map(|arm| {
                (
                    arm.clone(),
                    ArmStats {
                        success: AtomicU64::new(1),
    ...
    pub fn save(&self) {
        ...
        let data: Vec<(ComboArm, u64, u64)> = self
            .arms
            .iter()
            .map(|(a, s)| {
                (
    ...
    pub fn update(&self, arm: ComboArm, reward: f64) {
        ...
        for (a, stats) in &self.arms {
            if *a == arm {
                stats
                    .success
                    .fetch_add((reward * k as f64).round() as u64, Ordering::Relaxed);
                stats.fail.fetch_add(
                    ((1.0 - reward) * k as f64).round() as u64,
                    Ordering::Relaxed,
                );
                self.save();
                return;
    ...
    pub fn similarity(&self, other: &FingerprintBandit) -> f64 {
        let self_rates: Vec<f64> = self
            .arms
            .iter()
            .map(|(_, s)| {
                s.success.load(Ordering::Relaxed) as f64
                    / (s.success.load(Ordering::Relaxed) + s.fail.load(Ordering::Relaxed)) as f64
            })
            .collect();
        let other_rates: Vec<f64> = other
            .arms
            .iter()
            .map(|(_, s)| {
                s.success.load(Ordering::Relaxed) as f64
...
mod tests {
    use super::*;
    use crate::neotrix::nt_io_http_factory::{H2SettingsProfile, TlsVariant};
    use crate::neotrix::nt_io_stealth_net::system_fingerprint::Platform;

    fn test_arm(tls: TlsVariant, platform: Platform) -> ComboArm {
        ComboArm {
            tls,
            platform,
            h2_profile: H2SettingsProfile::ChromeDefault,
            geo_tag: String::new(),
        }
    }
    ...
    fn test_update_affects_stats() {
        ...
        assert!(!after.is_empty(), "arm not found after update");
    }
    ...
    fn test_bandit_converges_to_best_arm() {
        ...
        for tls in crate::neotrix::nt_io_http_factory::TlsVariant::all() {
            for plat in crate::neotrix::nt_io_stealth_net::system_fingerprint::Platform::all()
                .iter()
                .take(3)
            {
                for h2 in H2SettingsProfile::all() {
                    let cnt = if *tls == TlsVariant::LegacyHttp11 {
                        train_arms.iter().filter(|a| a.tls == *tls).count() as u64
                    } else {
                        0
                    };
                    if cnt == 0 {
        ...
        for a in &train_arms {
            let reward = if a.tls == TlsVariant::LegacyHttp11 {
                0.9
            } else {
                0.1
            };
            for _ in 0..50 {
                bandit.update(a.clone(), reward);
            }
        }
        ...
        let champ = counts
            ...
            .max_by_key(|(_, c)| *c)
            .map(|(v, _)| v)
            .expect("bandit should have a winner");
        assert_eq!(
            champ.tls,
            TlsVariant::LegacyHttp11,
            "best tls should be LegacyHttp11"
        );
        assert_eq!(
            champ.platform,
            Platform::MacOS,
            "best platform should be MacOS"
        );
    }

    #[test]
    fn test_save_load_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 手工保存到临时路径
        let data: Vec<(ComboArm, u64, u64)> = bandit
            .stats()
            .into_iter()
            .map(|(a, s, f, _)| (a, s, f))
            .collect();
        ...
        // 反序列化验证
        let content = std::fs::read_to_string(&tmp).map_err(|e| format!("read: {}", e))?;
        let loaded: Vec<(ComboArm, u64, u64)> =
            serde_json::from_str(&content).map_err(|e| format!("deserialize: {}", e))?;
        ...
        let _ = std::fs::remove_file(&tmp);
        assert!(!loaded.is_empty(), "arm not found after load");
        Ok(())
    }
