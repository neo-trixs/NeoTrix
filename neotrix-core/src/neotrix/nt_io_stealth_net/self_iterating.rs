...
impl FingerprintManager {
    ...
    fn ua_from_fingerprint(&self, _fp: &SystemFingerprint) -> String {
        UserAgentRotation::default().next().to_string()
    }
    ...
    pub fn spawn_variant(&mut self) {
        ...
        let compat_nt_world_browses: Vec<&Browser> = nt_world_browses
            .iter()
            .filter(|b| b.compatible_platforms().contains(&platform))
            .collect();
        let nt_world_browse =
            *compat_nt_world_browses[rng.gen_range(0..compat_nt_world_browses.len())];
...
mod tests {
    ...
    fn test_fingerprint_persistence() {
        ...
        let path = td.path().join("test_fp_persist.json");
        {
            let mut mgr = FingerprintManager::new();
