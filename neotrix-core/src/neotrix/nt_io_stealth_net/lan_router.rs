...
impl LanRouter {
    ...
    fn discover_sync(&self) {
        ...
        if interfaces.is_empty() {
            if let Ok(output) = std::process::Command::new("ifconfig").args(["-l"]).output() {
                let text = String::from_utf8_lossy(&output.stdout);
                for iface in text.split_whitespace() {
                    if iface == "lo0" || iface.starts_with("lo") {
                        continue;
                    }
                    let mut ips = Vec::new();
                    // 获取该接口的 IP
            ...
            if let Ok(output) = std::process::Command::new("ipconfig").output() {
                ...
                for line in text.lines() {
                    ...
                    if trimmed.starts_with("Ethernet")
                        || trimmed.starts_with("Wi-Fi")
                        || trimmed.starts_with("本地连接")
                    {
                        current_name = trimmed.to_string();
                    }
                    if let Some(ip_str) = trimmed.strip_prefix("IPv4 Address") {
                        if let Some(ip) = ip_str.split(':').nth(1).and_then(|s| {
                            s.trim()
                                .parse::<Ipv4Addr>()
                            ...
                            if let Some(iface) =
                                interfaces.iter_mut().find(|i| i.name == current_name)
                            {
    ...
    pub async fn start_rotation_loop(self: Arc<Self>) {
        ...
        loop {
            ...
            if use_coord {
                let coord = self.coordinator.read().await.clone().expect("LanRouter: coordinator verified Some by is_some() check above");
                // IP 轮转
                ...
                sleep(Duration::from_secs(
                    self.rotation_interval_secs.load(Ordering::Relaxed),
                ))
                .await;
                self.rotate_ip().await;
                self.rotate_wifi_fingerprint().await;
...
mod tests {
    ...
    async fn test_auto_discovery_has_at_least_localhost_excluded() {
        ...
        for ip in &ips {
            assert!(
                !ip.to_string().starts_with("127."),
                "127.x.x.x should be excluded"
            );
        }
    }
