...
impl Identity {
    ...
    pub fn record_use(&mut self, success: bool) {
        ...
        self.success_rate =
            ((self.success_rate * (total - 1.0)) + if success { 1.0 } else { 0.0 }) / total;
    }
...
impl IdentityPool {
    ...
    fn initialize(&mut self) {
        for i in 0..self.pool_size {
            ...
            identity.user_agent = UserAgentRotation::default().next().to_string();
    ...
    pub fn select(&mut self, target_tags: &[String]) -> Option<&mut Identity> {
        ...
        let idx = match self.rotation_strategy {
            ...
            RotationStrategy::HighestConfidence => candidates
                .into_iter()
                .max_by(|a, b| {
                    self.identities[*a]
                        .confidence
            ...
            RotationStrategy::Targeted => candidates
                .into_iter()
                .max_by(|a, b| {
    ...
    pub fn replenish(&mut self) -> usize {
        ...
        for i in 0..needed {
            ...
            let mut identity =
                Identity::new(start + i, &format!("identity_{}", start + i), profile);
...
impl BehaviorSimulator {
    ...
    pub fn total_typing_time(&self, text: &str, profile: &BehaviorProfile) -> u64 {
        self.simulate_typing(text, profile)
            .iter()
            .map(|(_, d)| d)
            .sum()
    }
...
impl StealthManager {
    ...
    fn rotate_pool(&mut self) {
        let low_confidence: Vec<usize> = self
            .pool
            .active_ids
            .iter()
