...
use super::rules::{OutboundAction, OutboundRule, RuleCondition, RuleEngine};

fn dot_neotrix_path(name: &str) -> String {
    let home = nt_core_util::home_dir().to_string_lossy().to_string();
    let path = format!("{}/.neotrix/{}", home, name);
...
impl FirewallRule {
    fn to_pf_rule(&self) -> String {
        ...
        format!(
            "{} proto {} {} label \"neotrix:{}\"",
            action_str, self.protocol, dst, self.label
        )
    }
}
...
impl FirewallManager {
    pub fn new() -> Self {
        ...
        log::info!("[fw] detected firewall: {:?}", fw_type);
        let mgr = Self {
    ...
    pub fn is_available(&self) -> bool {
        self.firewall_type != FirewallType::Unsupported
    }
    ...
    fn ensure_pf_loaded() -> Result<(), String> {
        ...
        let anchor_rules = format!(
            ...
            PF_ANCHOR_PATH,
            PF_ANCHOR_PATH,
            dot_neotrix_path("neotrix_pf_anchor.conf")
        );
        let _ = std::fs::write(dot_neotrix_path("neotrix_pf_main.conf"), &anchor_rules);
        let output = std::process::Command::new("pfctl")
            .args(["-f", &dot_neotrix_path("neotrix_pf_main.conf"), "-q"])
            .output();
        match output {
            Ok(o) if o.status.success() => {
                let _ = std::process::Command::new("pfctl")
                    .args(["-a", PF_ANCHOR_PATH, "-F", "all", "-q"])
                    .output();
                Ok(())
            }
            Ok(o) => {
                ...
                if stderr.contains("Permission denied") {
                    Err(
                        "pfctl: Permission denied (run with sudo or add user to _pfd_admins)"
                            .to_string(),
                    )
                } else {
    ...
    fn write_pf_rules(rules: &[FirewallRule]) -> Result<(), String> {
        ...
        pf_text.push_str(&format!(
            "pass out quick proto tcp from any to any divert-to 127.0.0.1 port {} no-state\n",
            DIVERT_PORT
        ));
        pf_text.push_str(
            "pass out quick proto udp from any to any port 53 divert-to 127.0.0.1 port 11053\n",
        );
        for r in rules {
        ...
        let _ = std::fs::write(dot_neotrix_path("neotrix_pf_anchor.conf"), &pf_text);
        let output = std::process::Command::new("pfctl")
            .args([
                "-a",
                PF_ANCHOR_PATH,
    ...
    fn write_nftables_rules(rules: &[FirewallRule]) -> Result<(), String> {
        ...
        nft.push_str(&format!(
            "add rule inet neotrix divert tcp dport != 11080 divert-to 127.0.0.1:{}\n",
            DIVERT_PORT
        ));
        ...
        for r in rules {
            if r.action == FirewallAction::Block {
                ...
                nft.push_str(&format!(
                    "add rule inet neotrix output {} drop comment \"{}\"\n",
                    dst, r.label
                ));
            }
        }
        if let Err(e) = std::fs::write(dot_neotrix_path("neotrix_nftables.conf"), &nft) {
            log::warn!("[fw] failed to write nftables config: {}", e);
        }
        let child = std::process::Command::new("nft")
            .args(["-f", &dot_neotrix_path("neotrix_nftables.conf")])
            .output();
        match child {
            Ok(o) if o.status.success() => Ok(()),
        ...
        match fw_type {
            FirewallType::Pf => {
                let _ = std::process::Command::new("pfctl")
                    .args(["-a", PF_ANCHOR_PATH, "-F", "all", "-q"])
                    .output();
                if let Err(e) = std::fs::write(dot_neotrix_path("neotrix_pf_anchor.conf"), "") {
            ...
            FirewallType::Nftables => {
                let _ = std::process::Command::new("nft")
                    .args(["delete", "table", "inet", "neotrix"])
                    .output();
                Ok(())
            }
    ...
    pub async fn sync_rules(&self, rules: &[OutboundRule]) -> Result<(), String> {
        if !self.is_available() {
            return Err("No firewall available".into());
        }
        ...
        *self.active_rules.write().await = fw_rules.clone();
        ...
        match &result {
            Ok(_) => {
                ...
                log::info!(
                    "[fw] synced {} CIDR rules (total syncs: {})",
                    count,
    ...
    pub async fn start_auto_sync(
        ...
        log::info!("[fw] auto-sync started (interval: {}s)", SYNC_INTERVAL_SECS);
        loop {
            tokio::select! {
                _ = sleep(Duration::from_secs(SYNC_INTERVAL_SECS)) => {},
                _ = shutdown.wait_shutdown() => {
                    log::info!("[fw] auto-sync stopped");
                    return;
                }
            }
...
/// 全局防火墙管理器单例（惰性初始化）
pub fn global_firewall_manager() -> Arc<FirewallManager> {
...
mod tests {
    use super::*;
    use crate::neotrix::nt_io_stealth_net::rules::{OutboundAction, OutboundRule, RuleCondition};
    ...
    fn test_derive_firewall_rules_from_outbound() {
        let rules = vec![
            OutboundRule::new(
                "test-block",
                RuleCondition::Cidr("10.0.0.0".parse().expect("value should be ok in test"), 8),
    ...
    fn test_derive_firewall_rules_sorts_by_priority() {
        let rules = vec![
            ...
            OutboundRule::new(
                "high",
                RuleCondition::Cidr(
                    "192.168.0.0".parse().expect("value should be ok in test"),
                    16,
                ),
                OutboundAction::Block,
            )
