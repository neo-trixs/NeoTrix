...
pub fn global_circuit_manager() -> &'static CircuitIsolationManager {
    &**global_circuit_manager_arc_inner()
}

pub fn global_circuit_manager_arc() -> Arc<CircuitIsolationManager> {
    global_circuit_manager_arc_inner().clone()
}

fn global_circuit_manager_arc_inner() -> &'static Arc<CircuitIsolationManager> {
    static CM: OnceLock<Arc<CircuitIsolationManager>> = OnceLock::new();
    CM.get_or_init(|| {
        Arc::new(CircuitIsolationManager::new(
...
impl CircuitIsolationManager {
    pub fn new(config: CircuitIsolationConfig, tor_manager: Option<Arc<TorManager>>) -> Self {
        let tokens: Vec<IsolationToken> = (0..config.pool_size)
    ...
    pub async fn rotate(&self) {
        ...
        *pool = new_pool;
        ...
        if let Some(ref _tm) = *self.tor_manager.read().await {
            let mut last = self.last_newnym.lock().unwrap_or_else(|e| e.into_inner());
            if last.elapsed() >= self.config.newnym_cooldown {
                let _pool_size = self.config.pool_size;
                tokio::spawn(async move {
                    log::info!("[circuit] rotated {} tokens", _pool_size);
                });
                *last = Instant::now();
            }
        }
...
mod tests {
    ...
    async fn test_rotate_generates_new_tokens() {
        ...
        for t in &before {
            assert!(
                !after.contains(t),
                "all tokens should be new after rotation"
            );
        }
    }
