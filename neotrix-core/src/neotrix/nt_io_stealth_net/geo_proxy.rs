...
pub fn global_geo() -> Arc<RwLock<GeoDatabase>> {
    GLOBAL_GEO
        .get_or_init(|| Arc::new(RwLock::new(GeoDatabase::new())))
        .clone()
}

pub fn global_domains() -> Arc<RwLock<DomainRules>> {
    GLOBAL_DOMAINS
        .get_or_init(|| Arc::new(RwLock::new(DomainRules::new())))
        .clone()
}
        ...
        while !self.shutdown.load(Ordering::Relaxed) {
            ...
            if !report.errors.is_empty() {
                ...
                log::info!(
                    ...
                    report.china_ip_count,
                    report.direct_domain_count,
                    report.proxy_domain_count
                );
            }
        }
    }
...
pub async fn domain_resolves_to_china(domain: &str) -> Result<bool, String> {
    domain_resolves_to_china_cached(domain, None).await
}

/// DNS 解析 + IP 归属判断，可选 VsaDnsCache
pub async fn domain_resolves_to_china_cached(
    domain: &str,
    ...
    let addrs = tokio::time::timeout(Duration::from_secs(5), task)
        .await
    ...
    // Cache the resolved IP in VsaDnsCache
...
const CHINA_V4_FALLBACK: &[(u32, u32)] = &[
    ...
    (0x2A000000, 0x2AFFFFFF),
    (0x31000000, 0x31FFFFFF),
    (0x3A000000, 0x3AFFFFFF),
    (0x3B000000, 0x3BFFFFFF),
    (0x3C000000, 0x3CFFFFFF),
    (0x3D000000, 0x3DFFFFFF),
    (0x65000000, 0x65FFFFFF),
    (0x67000000, 0x67FFFFFF),
    (0x6A000000, 0x6AFFFFFF),
    (0x6E000000, 0x6EFFFFFF),
    (0x6F000000, 0x6FFFFFFF),
    (0x70000000, 0x70FFFFFF),
    (0x71000000, 0x71FFFFFF),
    (0x72000000, 0x72FFFFFF),
    (0x73000000, 0x73FFFFFF),
    (0x74000000, 0x74FFFFFF),
    (0x75000000, 0x75FFFFFF),
    (0x76000000, 0x76FFFFFF),
    (0x77000000, 0x77FFFFFF),
    (0x78000000, 0x78FFFFFF),
    (0x79000000, 0x79FFFFFF),
    (0x7A000000, 0x7AFFFFFF),
    (0x7B000000, 0x7BFFFFFF),
    (0x7C000000, 0x7CFFFFFF),
    (0x7D000000, 0x7DFFFFFF),
    (0xAF000000, 0xAFFFFFFF),
    (0xB4000000, 0xB4FFFFFF),
    (0xB6000000, 0xB6FFFFFF),
    (0xB7000000, 0xB7FFFFFF),
    (0xCA000000, 0xCAFFFFFF),
    (0xD2000000, 0xD2FFFFFF),
    (0xD3000000, 0xD3FFFFFF),
    (0xDA000000, 0xDAFFFFFF),
    (0xDB000000, 0xDBFFFFFF),
...
mod tests {
    ...
    fn test_is_china_ip() {
        assert!(is_china_ip(
            &"114.114.114.114".parse().expect("parse 114.114.114.114")
        ));
        assert!(is_china_ip(
            &"180.76.76.76".parse().expect("parse 180.76.76.76")
        ));
        assert!(!is_china_ip(&"8.8.8.8".parse().expect("parse 8.8.8.8")));
        assert!(!is_china_ip(&"1.1.1.1".parse().expect("parse 1.1.1.1")));
        assert!(!is_china_ip(
            &"140.82.121.3".parse().expect("parse 140.82.121.3")
        ));
    }
    ...
    #[test]
    fn test_fallback_ip() {
        assert!(fallback_is_china_ip(
            &"14.0.0.1".parse().expect("parse 14.0.0.1")
        ));
        assert!(!fallback_is_china_ip(
    ...
    async fn test_fetch_direct_domains() {
        ...
        if report.direct_domain_count > 0 {
            assert!(
                report.direct_domain_count > 10000,
                "expected >= 10k domains"
            );
            let dom_arc = global_domains();
