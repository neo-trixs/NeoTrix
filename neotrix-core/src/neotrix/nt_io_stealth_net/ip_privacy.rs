...
impl IpPrivacyManager {
    ...
    fn generate_fake_config(_network: &str) -> FakeIpConfig {
        ...
        let (city, country, continent, lat, lon, tz) = match subnet {
            ...
            IpSubnet::PrivateClassB => {
                ("Private-B", "RFC1918", "Global", 34.05, -118.24, "Etc/UTC")
            }
        ...
        loop {
            ...
            if let Some(coord) = self.coordinator.read().await.as_ref() {
                ...
                sleep(Duration::from_secs(
                    self.rotation_interval.load(Ordering::Relaxed),
                ))
                .await;
                self.rotate_identity().await;
            }
...
mod tests {
    ...
    async fn test_public_ip_range() {
        ...
        assert!(
            found_public,
            "Should generate at least one public IP out of 20"
        );
    }
}
