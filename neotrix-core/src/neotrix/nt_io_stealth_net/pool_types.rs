...
#[non_exhaustive]
pub enum NodeRole {
    /// Tier 1: low-latency relay nodes for first hop
    Relay,
    /// Tier 2: high-diversity obfuscation nodes for exit hop
    Obfuscation,
    /// Can serve as either relay or obfuscation
    Mixed,
}

impl NodeRole {
    pub fn is_relay(&self) -> bool {
        matches!(self, NodeRole::Relay | NodeRole::Mixed)
    }

    pub fn is_obfuscation(&self) -> bool {
        matches!(self, NodeRole::Obfuscation | NodeRole::Mixed)
    }
}
...
impl ProxyNode {
    pub(crate) fn is_stale(&self) -> bool {
        self.last_success
            .is_none_or(|t| t.elapsed().as_secs() > PROXY_STALE_SECS)
    }
    ...
    pub fn diversity_score(&self) -> f64 {
        ...
        if self.from_subscription {
            score += 1.0;
        }
        if let Some(ms) = self.latency_ms {
            score += (ms / 1000.0).min(3.0);
        }
        score
    }
