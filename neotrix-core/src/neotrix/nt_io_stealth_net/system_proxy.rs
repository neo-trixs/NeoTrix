...
/// Serializer for `std::env::set_var` / `remove_var` calls.
///
/// `std::env::set_var` is documented as not thread-safe — it mutates
/// process-global state without internal synchronization.  All env var
/// writes in production code paths go through these wrappers.
///
/// The lock is held for <1µs per call (just the hashmap insertion in libc),
/// so blocking the async runtime is negligible.
static ENV_MUTEX: Mutex<()> = Mutex::new(());

fn set_env_var_safe(key: &str, val: &str) {
...
const DEFAULT_PROXY_URL: &str = "http://127.0.0.1:11080";
...
impl SystemProxyManager {
    ...
    async fn current_proxy_url(&self) -> String {
        if let Some(ref chain) = *self.dynamic_chain.read().await {
            if let Some(exit_url) = chain.current_exit_url().await {
                if let Ok(parsed) = url::Url::parse(&exit_url) {
                    ...
                    let port = parsed.port().unwrap_or(TOR_SOCKS_PORT);
        ...
        config
            ...
            .unwrap_or_else(|| DEFAULT_PROXY_URL.into())
    }
    ...
    pub async fn enable(&self) -> Result<(), String> {
        ...
        if is_socks {
            set_env_var_safe("ALL_PROXY", &proxy_url);
            set_env_var_safe("all_proxy", &proxy_url);
            set_env_var_safe("SOCKS5_PROXY", &proxy_url);
            set_env_var_safe("socks5_proxy", &proxy_url);
            remove_env_var_safe("HTTP_PROXY");
            remove_env_var_safe("HTTPS_PROXY");
            remove_env_var_safe("http_proxy");
            remove_env_var_safe("https_proxy");
        } else {
            set_env_var_safe("HTTP_PROXY", &proxy_url);
            set_env_var_safe("HTTPS_PROXY", &proxy_url);
            set_env_var_safe("http_proxy", &proxy_url);
            set_env_var_safe("https_proxy", &proxy_url);
        }
        set_env_var_safe("NO_PROXY", &no_proxy_str);
        set_env_var_safe("no_proxy", &no_proxy_str);
        ...
        if reachable {
            ...
            log::info!(
                "[proxy] system proxy enabled via networksetup ({})",
                proxy_url
            );
        } else {
            log::warn!("[proxy] proxy backend {host}:{port} unreachable, env vars set (networksetup skipped)");
    ...
    pub async fn retry_enable(&self) -> bool {
        ...
        if let Ok(parsed) = url::Url::parse(&proxy_url) {
            ...
            {
                log::info!("[proxy] system proxy activated via retry ({})", proxy_url);
                return true;
            }
        }
        false
        ...
        match self.os {
            OsType::MacOS => {
                let parsed =
                    url::Url::parse(proxy_url).map_err(|e| format!("Invalid proxy URL: {}", e))?;
                let host = parsed.host_str().unwrap_or("127.0.0.1");
                let port = &parsed.port().unwrap_or(TOR_SOCKS_PORT).to_string();
                let scheme = parsed.scheme();
                ...
                for service in &services {
                    // 跳过 VPN/代理接口，只改物理网络
                    if service.contains("Shadowrocket")
                        || service.contains("VPN")
                        || service.contains("Tunnel")
                    {
                        continue;
                    }
                    match scheme {
            ...
            if self.enabled.load(Ordering::Relaxed) {
                if let Some(ref chain) = *self.dynamic_chain.read().await {
                    ...
                    if let Some(url) = exit_url {
                        ...
                        set_env_var_safe("HTTP_PROXY", &safe_url);
                        set_env_var_safe("HTTPS_PROXY", &safe_url);
                        set_env_var_safe("ALL_PROXY", &safe_url);
                        chain.rotate_all().await;
                    }
                }
            }
...
mod tests {
    ...
    async fn test_enable_disable_proxy() {
        with_env_lock(async {
            ...
            mgr.enable()
                .await
                .expect("enable proxy in test should succeed");
            assert!(mgr.is_enabled());
            mgr.disable()
                .await
                .expect("disable proxy in test should succeed");
            assert!(!mgr.is_enabled());
        })
        .await;
    }
    ...
    async fn test_proxy_env_vars_set_on_enable() {
        with_env_lock(async {
            ...
            mgr.enable()
                .await
                .expect("enable proxy in test_proxy_env_vars should succeed");
            // 默认 HTTP 代理 → 设 HTTP_PROXY + HTTPS_PROXY
            assert!(std::env::var("HTTP_PROXY").is_ok());
            assert!(std::env::var("HTTPS_PROXY").is_ok());
            mgr.disable()
                .await
                .expect("disable proxy in test_proxy_env_vars should succeed");
        })
        .await;
    }
    ...
    async fn test_env_restored_on_disable() {
        with_env_lock(async {
            ...
            mgr.enable()
                .await
                .expect("enable proxy in test_env_restored should succeed");
            mgr.disable()
                .await
                .expect("disable proxy in test_env_restored should succeed");
            assert_eq!(
                std::env::var("HTTP_PROXY")
                    .expect("HTTP_PROXY should be restored to original value"),
                "http://original:8080"
            );
