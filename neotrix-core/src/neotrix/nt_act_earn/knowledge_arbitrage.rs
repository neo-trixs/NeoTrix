...
pub struct InformationSource {
    pub name: String,
    pub source_type: ArbitrageSourceType,
    ...
    fn default() -> Self {
        Self {
            sources: vec![
                InformationSource {
                    name: "ArXiv ML".into(),
                    source_type: ArbitrageSourceType::ArxivPaper,
                    signal_quality: 0.85,
                    update_frequency_secs: 86400,
                    access_cost_monthly: 0.0,
                    exclusive: false,
                    last_fetch: None,
                },
                InformationSource {
                    name: "SEC EDGAR".into(),
                    source_type: ArbitrageSourceType::RegulatoryFiling,
                    signal_quality: 0.95,
                ...
                InformationSource {
                    ...
                    source_type: ArbitrageSourceType::GithubActivity,
                    signal_quality: 0.7,
                    update_frequency_secs: 3600,
                    access_cost_monthly: 0.0,
                    exclusive: false,
                ...
                InformationSource {
                    name: "Patents".into(),
                    source_type: ArbitrageSourceType::PatentDatabase,
                    signal_quality: 0.8,
                    update_frequency_secs: 604800,
                    access_cost_monthly: 200.0,
                    exclusive: false,
                    last_fetch: None,
                },
                InformationSource {
                    name: "OnChain Alpha".into(),
                    source_type: ArbitrageSourceType::OnChain,
                    signal_quality: 0.9,
                    update_frequency_secs: 60,
                    access_cost_monthly: 50.0,
                    exclusive: true,
                    last_fetch: None,
                },
            ],
            ...
            attention_assets: vec![
                AttentionAsset {
                    platform: "X/Twitter Tech".into(),
                    followers: 10000,
                    engagement_rate: 0.03,
                    attention_value_daily: 50.0,
                    niche: "AI/ML Engineering".into(),
                    monetization_method: MonetizationMethod::SponsoredContent,
                },
                AttentionAsset {
                    platform: "YouTube Dev".into(),
                    followers: 5000,
                    engagement_rate: 0.05,
                    attention_value_daily: 30.0,
...
impl KnowledgeArbitrageEngine {
    ...
    pub fn scan_opportunities(&mut self) -> Vec<ArbitrageOpportunity> {
        ...
        let has_arxiv = self
            .sources
            .iter()
            .any(|s| matches!(s.source_type, ArbitrageSourceType::ArxivPaper));
        let has_patents = self
            .sources
            .iter()
            .any(|s| matches!(s.source_type, ArbitrageSourceType::PatentDatabase));
        ...
        let has_sec = self
            .sources
            .iter()
            .any(|s| matches!(s.source_type, ArbitrageSourceType::RegulatoryFiling));
        if has_sec {
            opportunities.push(ArbitrageOpportunity {
                id: format!("opp-{}", opportunities.len() + 1),
        ...
        let has_github = self
            .sources
            .iter()
            .any(|s| matches!(s.source_type, ArbitrageSourceType::GithubActivity));
        if has_github {
            opportunities.push(ArbitrageOpportunity {
                id: format!("opp-{}", opportunities.len() + 1),
    ...
    pub fn add_entity(&mut self, entity: KnowledgeEntity) {
        ...
        self.knowledge_graph.avg_connectivity = if !self.knowledge_graph.relations.is_empty() {
            self.knowledge_graph.relations.len() as f64
                / self.knowledge_graph.entities.len().max(1) as f64
        } else {
            0.0
        };
    }
    ...
    pub fn add_relation(&mut self, relation: EntityRelation) {
        ...
        if !self.knowledge_graph.entities.is_empty() {
            self.knowledge_graph.avg_connectivity = self.knowledge_graph.relations.len() as f64
                / self.knowledge_graph.entities.len() as f64;
        }
    }

    /// 注意力估值的总和
    pub fn total_attention_value(&self) -> f64 {
        self.attention_assets
            .iter()
            .map(|a| a.attention_value_daily)
            .sum()
    }
    ...
    pub fn arbitrage_insight(&self) -> Vec<String> {
        ...
        insights.push(format!(
            "活跃套利机会: {}个, 预期总价值: ${:.0}",
            self.arbitrage_opportunities.len(),
            self.expected_arbitrage_return()
        ));
    ...
    pub fn suggest_best_arbitrage(&self) -> Vec<&ArbitrageOpportunity> {
        let mut ranked: Vec<&ArbitrageOpportunity> = self
            .arbitrage_opportunities
            .iter()
    ...
    pub fn extract_knowledge_capital(&self) -> f64 {
        let arbitrage_capital: f64 = self
            .arbitrage_opportunities
            .iter()
...
mod tests {
    ...
    fn test_signal_processor_accuracy_range() {
        for sp in &KnowledgeArbitrageEngine::default().signal_processors {
            assert!(
                sp.accuracy >= 0.0 && sp.accuracy <= 1.0,
                "Processor {} has invalid accuracy",
                sp.name
            );
        }
    }
