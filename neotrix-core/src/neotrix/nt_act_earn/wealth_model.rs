...
impl WealthModel {
    ...
    pub fn analyze_strategy(&self, mechanism: &WealthMechanism) -> f64 {
        match mechanism {
            ...
            WealthMechanism::RegulatoryCapture {
                compliance_cost, ..
            ...
            WealthMechanism::AttentionMonopoly {
                attention_revenue_per_user,
                user_base,
                ..
            } => *user_base as f64 * attention_revenue_per_user * 0.001,
            WealthMechanism::AlgorithmicArbitrage {
                capacity_per_day, ..
    ...
    pub fn compound_day(&mut self, labor_income: f64, capital_return_rate: f64) {
        ...
        let record = DailyWealthRecord {
            ...
            leverage_contribution: self.capital_layers.leveraged_capital * capital_return_rate
                / 365.0,
        };
        ...
        self.compounding_tracker.capital_to_labor_ratio =
            if self.compounding_tracker.labor_income > 0.0 {
                self.compounding_tracker.capital_income / self.compounding_tracker.labor_income
            } else {
                f64::MAX
            };
        ...
        if total_income > 0.0 {
            self.compounding_tracker.compounding_rate = (self.compounding_tracker.capital_income
                / self.compounding_tracker.labor_income.max(1.0))
            .min(100.0);
        }
    }
    ...
    pub fn suggest_optimal_strategy(&self) -> Vec<WealthMechanism> {
        let mut ranked: Vec<(f64, &WealthMechanism)> = self
            .mechanisms
            .iter()
