...
use super::content::{ContentPlan, VideoScene, VideoScript};
...
impl EdgeTtsBackend {
    pub fn new(nt_act_voice: &str) -> Self {
        Self {
            nt_act_voice: nt_act_voice.to_string(),
            timeout_secs: 60,
        }
    }
...
impl TtsEngine for EdgeTtsBackend {
    fn synthesize(
        ...
        let nt_act_voice = if nt_act_voice.is_empty() {
            &self.nt_act_voice
        } else {
            nt_act_voice
        };
...
impl PexelsSource {
    pub fn new(api_keys: Vec<String>) -> Self {
        ...
        Self {
            api_keys,
            key_index: std::sync::atomic::AtomicUsize::new(0),
            client,
        }
    }
...
impl MediaSource for PexelsSource {
    fn search_videos(
        ...
        let orient = if orientation == "9:16" {
            "portrait"
        } else {
            "landscape"
        };
        ...
        let json: serde_json::Value = resp
            .json()
            .map_err(|e| format!("Pexels parse failed: {}", e))?;
        let clips = json
            .get("videos")
    ...
    fn download(&self, clip: &MediaClip, output_path: &str) -> Result<String, String> {
        ...
        let tmp = std::path::Path::new(output_path).with_extension("tmp");
        std::fs::write(&tmp, &bytes).map_err(|e| format!("Write failed: {}", e))?;
        std::fs::rename(&tmp, output_path).map_err(|e| format!("Rename failed: {}", e))?;
        Ok(output_path.to_string())
    }
...
impl MediaSource for LocalMediaSource {
    fn search_videos(
        ...
        let entries: Vec<_> = std::fs::read_dir(&self.media_dir)
            ...
            .filter(|e| {
                e.path()
                    .extension()
                    .map(|ext| matches!(ext.to_str(), Some("mp4" | "mov" | "avi" | "webm")))
                    .unwrap_or(false)
            })
            .take(count as usize)
            .map(|e| MediaClip {
    ...
    fn download(&self, clip: &MediaClip, output_path: &str) -> Result<String, String> {
        std::fs::copy(&clip.url, output_path).map_err(|e| format!("Copy failed: {}", e))?;
        Ok(output_path.to_string())
    }
...
impl FfmpegRenderer {
    ...
    pub fn composite(
        ...
        // 生成 concat 文件列表
        let concat_content: String = scene_paths
            .iter()
            .map(|p| format!("file '{}'\n", p.replace('\'', "'\\\\''")))
            .collect();
        let concat_file = format!("{}.concat.txt", output_path);
        let concat_path = std::path::Path::new(&concat_file);
        let tmp = concat_path.with_extension("tmp");
        std::fs::write(&tmp, &concat_content)
            .map_err(|e| format!("Failed to write concat file: {}", e))?;
        std::fs::rename(&tmp, concat_path)
            .map_err(|e| format!("Failed to rename concat file: {}", e))?;
        ...
        if let Some(bgm) = bgm_path {
            ...
            cmd.args([
                "-filter_complex",
                &format!(
                    "[1:a][2:a]amix=inputs=2:duration=first:weights=1 {}[aout]",
        ...
        let output = cmd
            .output()
            .map_err(|e| format!("ffmpeg composite failed: {}", e))?;
    ...
    pub fn render_script(
        ...
        for (i, scene) in script.scenes.iter().enumerate() {
            ...
            let spath = self
                .render_scene(scene, i, audio_path, mat, output_dir, output_dir)
...
impl VideoPipeline {
    ...
    pub fn with_tts(mut self, tts: Box<dyn TtsEngine>) -> Self {
        self.tts = tts;
        self
    }
    pub fn with_media(mut self, media: Box<dyn MediaSource>) -> Self {
        self.media_source = media;
        self
    }
    pub fn with_render_config(mut self, config: RenderConfig) -> Self {
        self.renderer = FfmpegRenderer::new(config);
        self
    }
    pub fn with_bgm_dir(mut self, dir: &str) -> Self {
        self.bgm_dir = Some(PathBuf::from(dir));
        self
    }
    pub fn with_nt_act_voice(mut self, nt_act_voice: &str) -> Self {
        self.nt_act_voice = nt_act_voice.to_string();
        self
    }
    pub fn with_orientation(mut self, orient: &str) -> Self {
        self.orientation = orient.to_string();
        self
    }
    ...
    pub fn produce(&self, plan: &ContentPlan) -> Result<String, String> {
        ...
        // Step 1: TTS — 合成全片旁白音频
        let full_narration: Vec<&str> =
            script.scenes.iter().map(|s| s.narration.as_str()).collect();
        ...
        for (i, scene) in script.scenes.iter().enumerate() {
            ...
            for kw in &keywords {
                ...
                if let Some(clip) = clips.first() {
                    ...
                    self.media_source
                        .download(clip, &mat_path.to_string_lossy())?;
...
mod tests {
    ...
    fn test_synthesize_speech_fails_without_cli() {
        ...
        let _guard = crate::core::nt_core_util::GLOBAL_TEST_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let td = crate::core::nt_core_util::TestDir::new();
        let out_path = td.path().join("test.mp3").to_string_lossy().to_string();
        let result = synthesize_speech("Hello", "en-US-JennyNeural", &out_path);
    ...
    fn test_ffmpeg_renderer_check() {
        ...
        if !available {
            log::warn!("ffmpeg not installed — skipping render tests");
        }
    }
