...
use super::publisher::{ContentMeta, ContentType, PublisherRegistry, PublishResult};
...
impl EarnPipeline {
    pub fn new(config: StrategyConfig, work_dir: &str) -> Self {
        Self {
    ...
    pub fn tracker_mut(&mut self) -> &mut EarnTracker { &mut self.tracker }
    ...
    pub fn execute_cycle(&mut self, best_platform: Option<&str>) -> CycleReport {
        ...
        let video_path = if plan.content_type == ContentType::Video {
            self.video_pipeline.as_ref().and_then(|vp| vp.produce(&plan).ok())
        } else {
            None
        };
    ...
    pub fn run_batch(&mut self, count: u32, best_platform: Option<&str>) -> Vec<CycleReport> {
        (0..count).map(|_| self.execute_cycle(best_platform)).collect()
    }
}
