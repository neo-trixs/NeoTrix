...
pub struct VideoScriptPlanner {
    llm: Option<(
        ...
        Box<dyn crate::neotrix::nt_io_provider::types::LlmProvider>,
    )>,
}
        ...
        match runtime.block_on(provider.complete(&request)) {
            Ok(resp) => {
                let sanitized = crate::neotrix::nt_shield_prompt::default_output_screener()
                    .sanitize(&resp.content);
                self.parse_json_script(&sanitized, topic, duration_secs)
            }
    ...
    fn parse_json_script(&self, json: &str, topic: &str, duration_secs: f64) -> VideoScript {
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(cleaned) {
            ...
            let scenes = parsed
                ...
                .unwrap_or_else(|| {
                    vec![VideoScene {
                        narration: format!("Exploring {}", topic),
                        visual_desc: topic.to_string(),
                        duration_secs,
                        search_keywords: vec![topic.to_string()],
                    }]
                });
            return VideoScript {
                title,
        ...
        let scenes = (0..(duration_secs / per_scene).ceil() as usize)
            .map(|i| {
                ...
                VideoScene {
                    narration: format!(
                        "{}: {} — {}",
                        topic, angle, "built with Rust, evolving every cycle"
                    ),
...
impl ContentPlanner {
    ...
    pub fn script_planner(&self) -> &VideoScriptPlanner {
        &self.script_planner
    }

    /// 生成下一个内容计划
    pub fn plan_next(&mut self, best_platform: Option<&str>) -> ContentPlan {
        let platform = best_platform.filter(|p| !p.is_empty()).unwrap_or("twitter");
        ...
        *self
        ...
        let is_video_platform = platform == "youtube"
            || platform == "bilibili"
            || platform == "douyin"
            || platform == "kuaishou"
        ...
        let (title, body) = match (platform, count) {
            ...
            ("wechat", _) => (
                ...
                format!("{} — {}\n\n## Why all AI coding tools are wrong\nAgents that cannot examine their own reasoning are tools, not agents.\n\n## The self-improvement loop\nEvery output is measured, absorbed, or corrected.", self.config.brand_name, self.config.brand_tagline),
            ),
            _ if is_video_platform => (
                format!("{} — {}", self.config.brand_name, ["Architecture Deep Dive", "Self-Improvement Loop", "Building Self-Improving AI", "Rust Cognitive OS"][count as usize % 4]),
                format!("In this video: {} — {}. Full architecture breakdown: self-improving reasoning, knowledge representation, and adaptive attention routing.", self.config.brand_name, self.config.brand_tagline),
            ),
            _ => (
        ...
        let video_script = if is_video_platform {
            let platform_label = if platform == "bilibili" || platform == "douyin" {
                "short"
            } else {
                "youtube"
            };
            Some(self.script_planner.generate(
                &title,
                platform_label,
                self.config.default_video_duration_secs,
            ))
        } else {
            None
        };
    ...
    /// 从已有的内容日历中选择下一个
    pub fn next_from_calendar(
        &self,
        calendar: &[ContentTopic],
        video: bool,
    ) -> Option<ContentPlan> {
        calendar.first().map(|topic| {
            let is_video = video
                || topic
                    .target_platforms
                    .iter()
                    .any(|p| matches!(p.as_str(), "youtube" | "bilibili" | "douyin" | "tiktok"));
            ContentPlan {
                title: format!("{} — {}", self.config.brand_name, topic.angle),
                body: format!(
                    "Exploring {} from the perspective of {}.",
                    topic.pillar, self.config.brand_name
                ),
...
mod tests {
    ...
    fn test_content_type_mapping() {
        let mut planner = ContentPlanner::new(StrategyConfig::default());
        assert_eq!(
            planner.plan_next(Some("youtube")).content_type,
            ContentType::Video
        );
        assert_eq!(
            planner.plan_next(Some("douyin")).content_type,
            ContentType::Video
        );
        assert_eq!(
            planner.plan_next(Some("instagram")).content_type,
            ContentType::Image
        );
        assert_eq!(
            planner.plan_next(Some("twitter")).content_type,
            ContentType::Text
        );
    }
