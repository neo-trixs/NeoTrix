...
impl KnowledgePool {
    ...
    pub fn publish(&mut self, packet: KnowledgePacket, sva_gate: Option<&mut SvaGate>) -> f64 {
        ...
        self.by_domain.entry(domain).or_default().push(pid);
        ...
        let mut results: Vec<&StoredPacket> = self
            ...
            .filter(|sp| {
                ...
                if !subscription.domain_filter.is_empty()
                    ...
                    && !subscription.domain_filter.contains(&sp.packet.domain)
                {
                    return false;
                }
                true
...
mod tests {
    ...
    fn test_novelty_scoring() {
        let mut pool = KnowledgePool::new(100);
        pool.publish(
            dummy_packet("code", "extract method refactoring", 0.7),
            None,
        );
        ...
        assert!(
            score2 < 0.6,
            "duplicate should have low novelty score, got {}",
            score2
        );
    }
    ...
    fn test_low_score_rejected() {
        ...
        assert!(
            score < 0.3,
            "low negentropy should get low score, got {}",
            score
        );
    ...
    fn test_sva_gate_modulates_low_value() {
        ...
        assert!(
            score < 0.5,
            "low value with SVAF should score < 0.5, got {}",
            score
        );
    }
