...
mod tests {
    ...
    fn test_sub_hive_absorbs_peer_knowledge() {
        ...
        assert!(
            hive.negentropy() > 0.0,
            "negentropy should increase after absorbing"
        );
    }
    ...
    fn test_sub_hive_dies_after_budget() {
        let mut pool = test_pool();
        let spec = SubHiveSpec::new("short lived", "test").with_budget(3);
        let mut hive = SubHiveInstance::new(HiveId::new(1), spec);
    ...
    fn test_registry_with_sva_gate() {
        ...
        let mut registry = SubHiveRegistry::new().with_sva_gate(gate);
    ...
    fn test_distributed_evolution_convergence() {
        ...
        assert!(
            ...
            "knowledge should have been published"
        );
        assert!(
