...
impl SvaGate {
    pub fn new(_vsa_dim: usize, _aligner_seed: u64) -> Self {
        SvaGate {
...
mod tests {
    ...
    fn test_high_negentropy_packet_accepted() {
        ...
        let results = g.evaluate(&packet);
        assert!(
            g.should_absorb(&results),
            "high negentropy should be absorbed"
        );
    }
    ...
    fn test_low_value_packet_rejected() {
        ...
        assert!(
            !g.should_absorb(&results),
            "low value packet should not be absorbed"
        );
    }
    ...
    fn test_intent_field_detection() {
        ...
        let intent = results
            .iter()
            .find(|r| r.field == SvaField::Intent)
            .unwrap();
        assert!(
    ...
    fn test_weighted_score_range() {
        ...
        assert!(
            ws >= 0.0 && ws <= 1.0,
            "weighted score should be in [0,1], got {}",
            ws
        );
    }
    ...
    fn test_mood_floor_enforcement() {
        ...
        assert!(
            (sum - 1.0).abs() < 0.01,
            "weights must sum to 1 after floor"
        );
    }
    ...
    fn test_mood_va_distance() {
        ...
        assert!(
            (d - 1.0).abs() < 0.01,
            "opposite VA should have distance ~1.0, got {}",
            d
        );
    }
    ...
    fn test_mood_va_zero_distance() {
        ...
        assert!(
            (d - 0.0).abs() < 0.01,
            "identical VA should have distance 0.0, got {}",
            d
        );
    }
