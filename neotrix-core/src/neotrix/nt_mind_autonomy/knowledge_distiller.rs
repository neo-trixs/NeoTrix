...
impl KnowledgeDistiller {
    ...
    fn generate_principles(&self, session: &SessionRecord) -> Vec<Principle> {
        ...
        if let Some(ref task_type) = session.task_type {
            ...
            if (lower_tt.contains("code") || lower_tt.contains("debug"))
                && session.reward_signal > 0.5
            {
                principles.push(Principle {
        ...
        for et in &session.edit_types {
            ...
            if lower_et.contains("normalize") && session.reward_signal > 0.4 {
                principles.push(Principle {
                    ...
                    description: format!(
                        "Normalization beneficial (reward={:.2})",
                        session.reward_signal
                    ),
                    confidence: session.reward_signal.min(1.0),
        ...
        if let Some(mode) = session.e8_mode {
            if session.reward_signal.abs() > 0.5 {
                let cat = if session.reward_signal > 0.0 {
                    PrincipleCategory::Insight
                } else {
                    PrincipleCategory::AntiPattern
                };
                let dir = if session.reward_signal > 0.0 {
                    "effective"
                } else {
                    "ineffective"
                };
                principles.push(Principle {
                    ...
                    delta: if session.reward_signal > 0.0 {
                        0.1
                    } else {
                        -0.05
                    },
    ...
    pub fn summary(&self) -> String {
        ...
        let patterns = self
            .principles
            .iter()
            .filter(|p| matches!(p.category, PrincipleCategory::Pattern))
            .count();
        let anti = self
            .principles
            .iter()
            .filter(|p| matches!(p.category, PrincipleCategory::AntiPattern))
            .count();
        let opts = self
            .principles
            .iter()
            .filter(|p| matches!(p.category, PrincipleCategory::Optimization))
            .count();
        let insights = self
            .principles
            .iter()
            .filter(|p| matches!(p.category, PrincipleCategory::Insight))
            .count();
        format!(
...
mod tests {
    ...
    fn test_distill_positive_session() {
        ...
        assert!(
            !principles.is_empty(),
            "positive session should produce principles"
        );
        assert!(principles
            .iter()
            .all(|p| p.category == PrincipleCategory::Pattern));
    ...
    fn test_distill_negative_session() {
        ...
        assert!(
            !principles.is_empty(),
            "negative session should produce principles"
        );
        assert!(principles
            .iter()
            .all(|p| p.category == PrincipleCategory::AntiPattern));
    }
    ...
    fn test_keyword_should_always() {
        ...
        assert_eq!(
            principles.len(),
            1,
            "should always keyword should produce one pattern"
        );
    ...
    fn test_empty_session_no_principles() {
        ...
        assert!(
            principles.is_empty(),
            "empty session should produce no principles"
        );
    }
    ...
    fn test_optimization_keyword() {
        ...
        assert!(principles
            .iter()
            .all(|p| p.category == PrincipleCategory::Optimization));
    }
    ...
    fn test_absorb_clears_principles() {
        ...
        assert!(
            d.principles().is_empty(),
            "principles should be cleared after absorb"
        );
    }
}
