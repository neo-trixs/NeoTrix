...
impl CrossSessionMemory {
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        std::fs::rename(&tmp, &self.storage_path).map_err(|e| format!("rename error: {}", e))
    }
    ...
    pub fn auto_save(&self) {
        if let Err(e) = self.save() {
            log::error!("[cross-session-memory] auto-save failed: {}", e);
        }
    }
...
// ── CrossSessionMemoryReader trait impl (for ND-02 architecture boundary fix) ──

impl crate::core::nt_core_consciousness::default_mode_network::MemoryEntryReader for MemoryEntry {
    fn key(&self) -> &str {
        &self.key
    }
    ...
    fn vsa_bytes(&self) -> Option<&[u8]> {
        self.vsa.as_ref().map(|v| v.as_bytes())
    }
}

impl crate::core::nt_core_consciousness::default_mode_network::CrossSessionMemoryReader
    for CrossSessionMemory
{
