use crate::neotrix::nt_mind_autonomy::awareness_monitor::AwarenessReport;
...
mod tests {
    ...
    fn no_large_modules() {
        ...
        let files = vec![make_file("src/main.rs", 100), make_file("src/lib.rs", 50)];
    ...
    fn detects_large_module() {
        let optimizer = SelfArchitectureOptimizer::new();
        let files = vec![make_file("src/main.rs", 100), make_file("src/huge.rs", 900)];
        let report = optimizer.analyze(&files, None);
