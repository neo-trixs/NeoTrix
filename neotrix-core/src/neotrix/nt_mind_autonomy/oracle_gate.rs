use crate::neotrix::nt_mind_autonomy::awareness_monitor::{AwarenessReport, GapSeverity};

#[derive(Debug, Clone, PartialEq)]
pub enum OracleReason {
    CriticalCapabilityGap {
        dimension: String,
        gap: f64,
    },
    UnknownTask {
    ...
    pub fn evaluate(
        ...
        if report.critical_count > self.max_critical_gaps_before_oracle {
            let gap = report
                .gaps
                .iter()
                .find(|g| g.severity == GapSeverity::Critical);
        ...
        if complex_keywords
            .iter()
            .any(|kw| _task_description.to_lowercase().contains(kw))
        {
            return OracleDecision {
                needs_oracle: true,
    ...
    pub fn evaluate_failure(&mut self, attempt_count: u32, dimension: &str) -> OracleDecision {
        ...
        OracleDecision {
            ...
            suggested_action: format!(
                "Retry autonomously (attempt {}/{})",
                attempt_count, self.max_attempts_before_oracle
            ),
        }
    }
...
mod tests {
    use super::*;
    use crate::neotrix::nt_mind_autonomy::awareness_monitor::{
        AwarenessReport, CapabilityGap, GapSeverity,
    };
    ...
    fn test_critical_gap_triggers_oracle() {
        ...
        assert!(matches!(
            decision.reason,
            Some(OracleReason::CriticalCapabilityGap { .. })
        ));
        assert_eq!(
            decision.request.as_ref().unwrap().urgency,
            OracleUrgency::Immediate
        );
    }
    ...
    fn test_complex_task_triggers_oracle() {
        ...
        assert!(matches!(
            decision.reason,
            Some(OracleReason::UnknownTask { .. })
        ));
        assert_eq!(
            decision.request.as_ref().unwrap().urgency,
            OracleUrgency::Normal
        );
    }
    ...
    fn test_repeated_failure_triggers_oracle() {
        ...
        assert!(matches!(
            decision.reason,
            Some(OracleReason::RepeatedFailure {
                attempt_count: 3,
                ..
            })
        ));
    ...
    fn test_architecture_always_triggers() {
        ...
        assert!(matches!(
            decision.reason,
            Some(OracleReason::ArchitectureDecision { .. })
        ));
        assert_eq!(
            decision.request.as_ref().unwrap().urgency,
            OracleUrgency::Immediate
