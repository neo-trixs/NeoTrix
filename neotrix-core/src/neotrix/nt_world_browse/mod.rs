...
impl BrowserCircuit {
    ...
    pub fn browse(&self, url: &str) -> Result<String, String> { self.session.lock().expect("result").fetch(url) }
    pub fn login(&self, url: &str) -> Result<(), String> { self.session.lock().expect("result").login(url) }
}
...
impl ReasoningCircuit for BrowserCircuit {
    ...
    fn process(&self, input: &ReasoningInput) -> ReasoningOutput {
        let session = self.session.lock().expect("result");
