    ...
    fn clean(html: &str) -> String {
        let re =
            regex::Regex::new(r"(?is)<script[^>]*>.*?</script>|<style[^>]*>.*?</style>|<[^>]+>")
                .expect("DumpDomFetcher::clean: static regex pattern is valid");
        let text = re.replace_all(html, " ");
        let ws = regex::Regex::new(r"\s+").expect("DumpDomFetcher::clean: static regex pattern is valid");
...
impl BrowserSession {
    pub fn new() -> Self {
        ...
        Self {
            fingerprint: super::stealth::Fingerprint::random(),
            profile: profile_dir(),
            fetchers,
        }
    }
    ...
    pub fn login(&self, url: &str) -> Result<(), String> {
        log::info!("[nt_world_browse] Opening {} for login (headed)...", url);
        let out = std::process::Command::new(chrome_path())
            .args([
        ...
        log::info!(
            "[nt_world_browse] Login session saved to {}",
    ...
    pub fn fetch(&self, url: &str) -> Result<String, String> {
        tokio::runtime::Handle::current().block_on(tokio::time::sleep(Duration::from_millis(200)));
        let http = self.fetch_http(url);
        match http {
            Ok(ref t) if t.len() > 500 => return Ok(t.clone()),
            _ => {}
        }
...
mod tests {
    ...
    fn test_nt_world_browse_session_new_creates_session() {
        ...
        assert!(
            !session.fingerprint.user_agent.is_empty(),
