...
use super::nt_core_fep_iit::FEPIITBridge;
use super::nt_mind_autofixer::AutoFixer;
use super::nt_mind_self_diagnose::{ActionExecutor, DiagnosticItem, PriorityQueue, SelfDiagnose};
...
impl EvolutionLoop {
    ...
    /// 运行一次完整进化周期
    pub fn run_cycle(&mut self, world_fe: Option<f64>, world_phi: Option<f64>) -> EvolutionReport {
        self.cycle += 1;
    ...
    pub fn autofix_cycle(
        ...
        let final_report =
            self.run_cycle(Some(initial_report.free_energy), Some(initial_report.phi));
    ...
    pub fn needs_human_intervention(&self) -> bool {
        self.consecutive_stagnant >= STAGNATION_LIMIT || self.issues.iter().any(|i| i.severity >= 9)
    }
...
mod tests {
    ...
    fn test_high_free_energy_detected() {
        ...
        assert!(report
            .issues_found
            .iter()
            .any(|i| i.issue_type == IssueType::HighFreeEnergy));
    }
    ...
    fn test_low_phi_detected() {
        ...
        assert!(report
            .issues_found
            .iter()
            .any(|i| i.issue_type == IssueType::LowPhi));
    }
