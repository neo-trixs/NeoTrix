...
impl ArchitectureDesigner {
                ...
                "CIRCULAR_DEP" => {
                    ...
                    if let Some(ref module) = plan.weakness.target_module {
                        let blueprint = ModuleBlueprint {
                            ...
                            description: format!(
                                "Shared interface extracted from {} to break circular dependency",
                                module
                            ),
                        };
    ...
    fn design_split(&self, weakness: &Weakness, model: &SelfModel) -> Option<RefactoringPlan> {
        ...
        Some(RefactoringPlan {
            ...
            description: format!(
                "Split {} ({}.{} lines) into {} sub-modules",
                module.name,
                module.name,
                module.total_lines,
                sub_names.len()
            ),
            actions: vec![CodeAction::SplitModule {
                new_sub_modules: sub_names,
            }],
        })
    }
    ...
    fn design_tests(&self, weakness: &Weakness) -> Option<RefactoringPlan> {
        ...
        Some(RefactoringPlan {
            ...
            actions: vec![CodeAction::AddTests {
                ...
                test_functions: vec!["test_sanity".to_string(), "test_edge_cases".to_string()],
            }],
        })
    }
    ...
    fn design_extract_interface(
        &self,
        _weakness: &Weakness,
        _model: &SelfModel,
    ) -> Option<RefactoringPlan> {
        None
    }
...
mod tests {
    ...
    fn test_large_file_triggers_split_design() {
        ...
        model
            .modules
            .push(crate::core::nt_core_meta::self_model::ModuleInfo {
    ...
    fn test_circular_dep_creates_interface_module() {
        ...
        let plan = PlannedEvolution {
            ...
            estimated_impact: crate::core::nt_core_meta::planner::ImpactEstimate {
                files_affected: 5,
                risk: crate::core::nt_core_meta::planner::RiskLevel::High,
            },
            dependencies: vec![],
        };
        let report = WeaknessReport {
        ...
        let model = SelfModel::new();
        let design = designer.design(&report, &[plan], &model);
        let interface_mods: Vec<_> = design
            .new_modules
            .iter()
            .filter(|m| m.name.contains("interface"))
            .collect();
