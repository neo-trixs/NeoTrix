...
use crate::neotrix::nt_mind::core::{AccessContext, CapabilityVector, PerformanceEvaluator};
...
pub fn heavy_pass_at_k(scores: &[f64], threshold: f64) -> HeavyPassResult {
    ...
    let diversity_bonus = if top_k.len() >= 2 {
        let var: f64 =
            top_k.iter().map(|s| (s - mean_at_k).powi(2)).sum::<f64>() / top_k.len() as f64;
...
impl CriticNode {
    ...
    pub fn evaluate_from_response(&self, response: &str) -> f64 {
        ...
        let has_steps =
            if response.contains("1)") || response.contains("Step") || response.contains("步骤") {
                0.3
            } else {
                0.0
            };
    ...
    pub fn cross_validate(&self, capability: &CapabilityVector) -> Vec<String> {
        ...
        if capability.verification() > 0.9 && capability.analysis() < 0.5 {
            issues
                .push("verification > 0.9 但 analysis < 0.5：验证能力与分析能力不匹配".to_string());
        }
        if capability.quality_gates() > 0.9 && capability.verification() < 0.5 {
            issues.push(
                "quality_gates > 0.9 但 verification < 0.5：门控与验证能力不匹配".to_string(),
            );
        }
        issues
    }
