...
impl QualityGate {
    pub fn new(
        ...
        description: &str,
        required: bool,
        check_fn: Box<dyn Fn(&str) -> bool + Send + Sync>,
    ) -> Self {
        Self {
            name: name.to_string(),
...
impl PMNode {
    ...
    pub fn all_required_pass(&self) -> bool {
        self.quality_gates.iter().filter(|g| g.required).all(|g| {
            self.gate_results
                .iter()
    ...
    pub fn weighted_score(&self) -> f64 {
        ...
        let total_weight: f64 = self
            .quality_gates
            .iter()
            .map(|g| if g.required { 3.0 } else { 1.0 })
            .sum();
        let earned: f64 = self
            .quality_gates
            .iter()
            .zip(self.gate_results.iter())
    ...
    fn default_gates(workflow: PMWorkflowType) -> Vec<QualityGate> {
        match workflow {
            ...
            PMWorkflowType::UxAudit => vec![
                QualityGate::new(
                    "has_accessibility",
                    "UX audit must check accessibility",
                    true,
                    Box::new(|o| {
                        o.contains("accessibility")
                            || o.contains("Accessibility")
                            || o.contains("WCAG")
                    }),
                ),
                QualityGate::new(
                    "has_heuristics",
                    "UX audit should use heuristic evaluation",
                    false,
                    Box::new(|o| o.contains("heuristic") || o.contains("Nielsen")),
                ),
                QualityGate::new(
                    "has_recommendations",
                    "UX audit should have recommendations",
                    false,
                    Box::new(|o| {
                        o.contains("recommend") || o.contains("Recommend") || o.contains("suggest")
                    }),
                ),
            ],
            PMWorkflowType::CompetitiveAnalysis => vec![
                QualityGate::new(
                    "has_comparison",
                    "Must include comparison data",
                    true,
                    Box::new(|o| {
                        o.contains("comparison")
                            || o.contains("Comparison")
                            || o.contains("vs")
                            || o.contains("VS")
                    }),
            ...
            PMWorkflowType::ExperimentDesign => vec![
                ...
                QualityGate::new(
                    "has_sample_size",
                    "Should estimate sample size",
                    false,
                    Box::new(|o| o.contains("sample") || o.contains("Sample")),
                ),
            ],
            _ => vec![QualityGate::new(
                "completeness",
