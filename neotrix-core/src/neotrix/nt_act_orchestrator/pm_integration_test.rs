...
use crate::neotrix::nt_mind::experiment::{ExperimentDesigner, Hypothesis};
use crate::neotrix::nt_mind::goal_loop::priority::{MoscowClass, PriorityEngine};
...
fn test_state_graph_pm_plan_topology() {
    ...
    let pos_research = sorted
        .iter()
        .position(|id| id == "research")
        .expect("research node should exist in sorted order");
    let pos_draft = sorted
        .iter()
        .position(|id| id == "draft")
        .expect("draft node should exist in sorted order");
    let pos_review = sorted
        .iter()
        .position(|id| id == "review")
        .expect("review node should exist in sorted order");
    let pos_refine = sorted
        .iter()
        .position(|id| id == "refine")
        .expect("refine node should exist in sorted order");
...
fn test_experiment_to_priority_ranking() {
    let h = Hypothesis::new(
        "H-001",
        "New feature improves retention",
        vec!["retention".to_string()],
        0.5,
    );
...
fn test_full_pm_chain_in_dag() {
    ...
    graph
        .mark_done("research")
        .expect("research node should exist");
