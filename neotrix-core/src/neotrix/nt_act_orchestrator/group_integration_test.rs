...
fn test_planner_enriches_cross_repo_tasks() {
    ...
    assert!(
        has_cross_repo,
        "Expected at least one task with cross-repo annotation"
    );
}
...
fn test_planner_with_group_manager_enriches_only_known_contracts() {
    ...
    assert_eq!(
        cross_repo_count, 0,
        "Design plan shouldn't match cross-repo contracts named 执行任务"
    );
}
...
fn test_decompose_cross_repo_info_in_task_name() {
    ...
    if let Some(task) = enriched {
        ...
        assert!(
            decoded.contains("->"),
            "Should reference a repo in cross-repo info"
        );
    }
}
