#[cfg(test)]
use crate::neotrix::nt_act_orchestrator::flow_state::FlowStateId;
use crate::neotrix::nt_act_orchestrator::flow_state::{ConfigState, StateManager};
...
impl<S: ConfigState> FlowRuntime<S> {
        ...
        for step in &self.flow.steps {
            match &step.trigger {
                ...
                FlowTrigger::StepComplete(after) => {
                    if self.completed_steps.contains(after)
    ...
    pub fn is_complete(&self) -> bool {
        self.flow
            .steps
            .iter()
            .all(|s| self.completed_steps.contains(&s.id))
    }
}
...
mod tests {
    ...
    fn test_flow_is_complete() {
        let flow = Flow::new("test", "").start("s1", "Only step", "do_it");
        let state = SimpleState {
            id: FlowStateId::new(),
            value: 0,
        };
    ...
    fn test_flow_with_flowstate() {
        let flow = Flow::new("stateful", "Flow with state").start("init", "Initialize", "setup");
        let state = SimpleState {
            id: FlowStateId::new(),
            value: 42,
        };
