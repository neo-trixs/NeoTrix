...
impl FnGuardrail {
    ...
    pub fn with_input_check(
        mut self,
        f: Arc<dyn Fn(&str, &str) -> GuardrailResult + Send + Sync>,
    ) -> Self {
        self.input_check = Some(f);
        self
    }

    pub fn with_output_check(
        mut self,
        f: Arc<dyn Fn(&str, Option<&str>, &str) -> GuardrailResult + Send + Sync>,
    ) -> Self {
...
impl GuardrailChain {
    ...
    pub fn validate_input(&self, task: &str, input: &str) -> Vec<(&str, GuardrailResult)> {
        self.guardrails
            .iter()
            .map(|g| (g.name(), g.validate_input(task, input)))
            .collect()
    }
    ...
    pub fn validate_output(
        ...
        expected: Option<&str>,
        actual: &str,
    ) -> Vec<(&str, GuardrailResult)> {
        self.guardrails
            .iter()
            .map(|g| (g.name(), g.validate_output(task, expected, actual)))
            .collect()
    }
...
pub mod builtins {
    ...
    pub fn non_empty_output() -> FnGuardrail {
        FnGuardrail::new("non_empty_output").with_output_check(Arc::new(
            |_task, _expected, actual| {
    ...
    pub fn max_input_length(max_chars: usize) -> FnGuardrail {
        FnGuardrail::new("max_input_length").with_input_check(Arc::new(move |_task, input| {
    ...
    pub fn contains_keywords(keywords: Vec<String>) -> FnGuardrail {
        ...
        FnGuardrail::new("contains_keywords").with_output_check(Arc::new(
            move |_task, _expected, actual| {
...
mod tests {
    ...
    fn test_fn_guardrail_input_only() {
        let g =
            FnGuardrail::new("input_only").with_input_check(Arc::new(|_, _| GuardrailResult::Pass));
        assert!(matches!(g.validate_input("t", "x"), GuardrailResult::Pass));
        assert!(matches!(
            g.validate_output("t", None, "x"),
            GuardrailResult::Pass
        ));
    }
