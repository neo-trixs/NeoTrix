...
impl Perspective {
    ...
    fn keywords(&self) -> &'static [&'static str] {
        match self {
            Perspective::Security => &["password", "token", "unsafe", "sql", "xss", "injection", "auth", "secret"],
            Perspective::Performance => &["loop", "recursion", "O(n", "clone", "alloc", "nested", "vector"],
            Perspective::Correctness => &["unwrap", "panic", "todo!", "unimplemented", "unreachable"],
            Perspective::Completeness => &[], // checked by length, not keywords
            Perspective::EdgeCase => &["0", "-1", "null", "empty", "boundary", "overflow", "underflow"],
        }
    }
}
...
impl AdversarialVerifier {
    ...
    pub fn verify(&self, _task: &str, result: &str) -> Vec<AdversarialFinding> {
        ...
        for perspective in &self.enabled_perspectives {
            ...
            let matches: Vec<&str> = keywords.iter().filter(|kw| result_lower.contains(*kw)).copied().collect();
            ...
            if !matches.is_empty() {
                ...
                let finding_str = format!("Found {} risk indicators: {}", match perspective {
        ...
        if self.enabled_perspectives.iter().any(|p| matches!(p, Perspective::Completeness)) {
        ...
        for i in 0..findings.len() {
            for j in 0..findings.len() {
                ...
                let matched: Vec<&str> = other_kws.iter().filter(|kw| finding_text.contains(*kw)).copied().collect();
                if !matched.is_empty() {
...
//            return "No findings — all perspectives passed.".to_string();
        ...
        lines.push(format!("Perspectives used: {}", self.enabled_perspectives.len()));
        lines.push(format!("Min agreement: {}", self.min_agreement));
        lines.push(String::new());

        let upheld_count = findings.iter().filter(|f| f.upheld).count();
        let total = findings.len();
        lines.push(format!("Findings: {} total, {} upheld (survived cross-check)", total, upheld_count));
        lines.push(String::new());
        ...
        for (i, finding) in findings.iter().enumerate() {
            ...
            let status = if finding.upheld { "UPHELD" } else { "DISMISSED" };
            lines.push(format!("{}. [{}] [{}] [{}] {}", i + 1, perspective_name, severity_name, status, finding.finding));
            if !finding.challenged_by.is_empty() {
                lines.push(format!("   Challenged by: {}", finding.challenged_by.join(", ")));
            }
            lines.push(String::new());
        }
...
mod tests {
    ...
    fn test_perspective_descriptions_non_empty() {
        ...
        for p in perspectives {
            assert!(!p.description().is_empty(), "description should be non-empty");
            assert!(p.description().len() > 10, "description should be substantial");
        }
    }
    ...
    fn test_verifier_default_state() {
        ...
        assert!(v.enabled_perspectives.iter().any(|p| matches!(p, Perspective::Correctness)));
        assert!(v.enabled_perspectives.iter().any(|p| matches!(p, Perspective::Completeness)));
    ...
    fn test_verify_nt_shield_result() {
        ...
        let findings = v.verify("login", "The function takes a password and token, then executes an unsafe SQL query");
        let nt_shield_findings: Vec<_> = findings.iter().filter(|f| matches!(f.perspective, Perspective::Security)).collect();
        assert!(!nt_shield_findings.is_empty(), "should find nt_shield issues");
        assert!(nt_shield_findings[0].finding.contains("password") || nt_shield_findings[0].finding.contains("nt_shield"));
    }

    #[test]
    fn test_verify_performance_result() {
        let v = AdversarialVerifier::with_all_perspectives();
        let findings = v.verify("sort", "This O(n^2) loop contains a nested clone allocation");
        let perf_findings: Vec<_> = findings.iter().filter(|f| matches!(f.perspective, Perspective::Performance)).collect();
    ...
    fn test_verify_with_all_perspectives() {
        let v = AdversarialVerifier::with_all_perspectives();
        let findings = v.verify("auth", "password token unsafe loop O(n) unwrap todo! empty 0 -1");
        // Should trigger nt_shield, performance, correctness, and edge case at minimum
        let unique_perspectives: std::collections::HashSet<&str> = findings.iter().map(|f| match f.perspective {
            Perspective::Security => "nt_shield",
            Perspective::Performance => "performance",
            Perspective::Correctness => "correctness",
            Perspective::Completeness => "completeness",
        ...
        assert!(unique_perspectives.len() >= 4, "should trigger at least 4 perspectives");
    }
    ...
    fn test_verify_empty_result() {
        ...
        let non_completeness: Vec<_> = findings.iter().filter(|f| !matches!(f.perspective, Perspective::Completeness)).collect();
        assert!(non_completeness.is_empty(), "empty result should not trigger non-completeness findings");
    }
    ...
    fn test_min_agreement_threshold() {
        ...
        for finding in &findings {
            if finding.challenged_by.len() < 4 {
                assert!(!finding.upheld, "finding should not be upheld with min_agreement=5");
            }
        }
    }
