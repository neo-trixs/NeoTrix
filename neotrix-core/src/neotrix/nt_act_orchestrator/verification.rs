...
#[derive(Debug, Clone)]
pub struct OrchestrationVerificationResult {
    pub passed: bool,
...
impl VerificationAware for AdversarialVerificationIntegration {
    fn verify_result(
        ...
        match stage {
            VerificationStage::None => OrchestrationVerificationResult {
                passed: true,
                consensus_score: 1.0,
                perspectives: vec![],
                issues: vec![],
            ...
            VerificationStage::Adversarial => {
                ...
                let perspectives: Vec<String> = {
                    ...
                    if seen.is_empty() {
                        seen = vec!["Correctness".to_string(), "Completeness".to_string()];
                    }
                    seen
                };
                ...
                let consensus_score = if total == 0 {
                    ...
                    1.0 - (upheld_count as f64 / total as f64)
                };
                OrchestrationVerificationResult {
...
mod tests {
    ...
    fn test_basic_verification_passes_clean_output() {
        ...
        let result = integration.verify_result(
            "test",
            "clean output with sufficient content",
            VerificationStage::Basic,
        );
    ...
    fn test_adversarial_verification_detects_issues() {
        ...
        assert!(
            !result.perspectives.is_empty(),
            "should report which perspectives flagged"
        );
    }
    ...
    fn test_adversarial_perspectives_non_empty() {
        ...
        assert!(
            !result.perspectives.is_empty(),
            "adversarial should have perspective list"
        );
    }
    ...
    fn test_issues_list_for_failed_verification() {
        ...
        assert!(
            !result.passed,
            "empty output should fail basic verification"
        );
    ...
    fn test_empty_input_handling() {
        ...
        assert!(
            result.passed,
            "empty input should pass adversarial (no findings)"
        );
        assert_eq!(result.consensus_score, 1.0);
    }
    ...
    fn test_high_consensus_score_passed() {
        ...
        let result = integration.verify_result(
            "clean task",
            "perfect output without any risk indicators",
            VerificationStage::Adversarial,
        );
    ...
    fn test_verification_result_types() {
        let vr = OrchestrationVerificationResult {
            passed: true,
