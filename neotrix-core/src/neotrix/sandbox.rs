...
impl Sandbox {
    ...
    fn run_wasm_module(&self, input: &str) -> NeoTrixResult<String> {
        ...
        let module = Module::new(&engine, &module_bytes)
            .map_err(|e| NeoTrixError::General { msg: format!("WASM compile: {}", e), backtrace: None })?;
        if module.exports().any(|e| e.name() == "main") {
            let mut store: Store<()> = Store::new(&engine, ());
            let instance = Instance::new(&mut store, &module, &[])
                .map_err(|e| NeoTrixError::General { msg: format!("WASM instantiate: {}", e), backtrace: None })?;
            let func = instance.get_typed_func::<(), ()>(&mut store, "main")
                .map_err(|e| NeoTrixError::General { msg: format!("WASM func: {}", e), backtrace: None })?;
            func.call(&mut store, ()).map_err(|e| NeoTrixError::General { msg: format!("WASM exec: {}", e), backtrace: None })?;
        }
    ...
    fn load_wasm_module(&self, name: &str) -> NeoTrixResult<Vec<u8>> {
        ...
        std::fs::read(&path).map_err(|e| NeoTrixError::General { msg: format!("WASM load {}: {}", name, e), backtrace: None })
    }
