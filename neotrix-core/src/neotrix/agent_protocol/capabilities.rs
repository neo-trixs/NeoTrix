...
impl CapabilityRouter {
    pub fn new() -> Self {
        Self { local: HashMap::new(), remote: HashMap::new() }
    }
    ...
    pub fn route_by_hexagram<'a>(&self, hexagram: ReasoningHexagram, agents: &'a [AgentInfo]) -> Vec<&'a AgentInfo> {
        let mut candidates: Vec<&'a AgentInfo> = agents.iter().filter(|a| a.hexagram != 0).collect();
    ...
    pub fn find_hexagram_affinity<'a>(&self, hexagram: ReasoningHexagram, agents: &'a [AgentInfo], top_n: usize) -> Vec<&'a AgentInfo> {
        let mut candidates: Vec<&'a AgentInfo> = agents.iter().filter(|a| a.hexagram != 0).collect();
        candidates.sort_by_key(|a| hexagram.hamming_dist(&ReasoningHexagram(a.hexagram)));
    ...
    pub fn summary(&self) -> String {
        ...
        format!("{} local, {} remote agents ({} caps)", local, remote_agents, remote_caps)
    }
}
...
mod tests {
    ...
    fn test_agent_info_hexagram_roundtrip() {
        ...
        let deserialized: AgentInfo = serde_json::from_str(&json).expect("value should be ok in test");
