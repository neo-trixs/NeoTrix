...
use crate::neotrix::nt_mind_goal::goal_generator::{EvolutionGoal, GoalCategory, GoalPriority};
...
impl GoalConflictResolver {
        ...
        for conflict in conflicts {
            ...
            match (pos_a, pos_b) {
                (Some(ia), Some(ib)) => {
                    ...
                    // 移除较低优先级的目标
                    let (keep_idx, remove_idx) =
                        if priority_score(goal_a.priority) >= priority_score(goal_b.priority) {
                            (ia, ib)
                        } else {
                            (ib, ia)
                        };
                    ...
                    // 更新保留的目标为合并结果
                    let keep_pos = if remove_idx < keep_idx {
                        keep_idx - 1
                    } else {
                        keep_idx
                    };
    ...
    fn detect_file_overlaps(&self, goals: &[EvolutionGoal]) -> Vec<GoalConflict> {
        ...
        for goal in goals {
            if let Some(ref file) = goal.target_file {
                if !seen.contains(file) {
                    ...
                    let group: Vec<&EvolutionGoal> = goals
                        .iter()
                        .filter(|g| g.target_file.as_deref() == Some(file))
                        .collect();
                    if group.len() > 1 {
        ...
        for group in file_groups {
            if group.len() > self.max_goals_per_file {
                // 超过阈值，检查优先级冲突
                let has_critical = group
                    .iter()
                    .any(|g| matches!(g.priority, GoalPriority::Critical | GoalPriority::High));
                let has_low = group
                    .iter()
                    .any(|g| matches!(g.priority, GoalPriority::Medium | GoalPriority::Low));

                if has_critical && has_low {
                    // 找到一对矛盾优先级
                    let critical_goal = group
                        .iter()
                        .find(|g| matches!(g.priority, GoalPriority::Critical | GoalPriority::High))
                        .expect("has_critical guard guarantees at least one critical/high priority goal in group");
                    let low_goal = group
                        .iter()
                        .find(|g| matches!(g.priority, GoalPriority::Medium | GoalPriority::Low))
                        .expect("has_low guard guarantees at least one medium/low priority goal in group");

                    conflicts.push(GoalConflict {
                        goal_a_id: critical_goal.id.clone(),
                        goal_b_id: low_goal.id.clone(),
                        reason: ConflictReason::SameFile {
                            file: critical_goal.target_file.clone().expect("group filtered by target_file != None, all members have Some(file)"),
                        },
    ...
    fn detect_category_overlaps(&self, goals: &[EvolutionGoal]) -> Vec<GoalConflict> {
        ...
        for category in &[
            ...
            let group: Vec<&EvolutionGoal> =
                goals.iter().filter(|g| g.category == *category).collect();
            ...
            for dep_id in &goal.dependencies {
                if let Some(dep_goal) = goals.iter().find(|g| g.id == *dep_id) {
                    if goal.priority == GoalPriority::Critical
                        && dep_goal.priority == GoalPriority::Medium
                    {
                        conflicts.push(GoalConflict {
    ...
    pub fn merge_goals(a: &EvolutionGoal, b: &EvolutionGoal) -> EvolutionGoal {
        ...
        EvolutionGoal {
            ...
            category: if a.category == b.category {
                a.category
            } else {
                GoalCategory::Architecture
            },
...
mod tests {
    ...
    fn test_priority_clash_detected() {
        let resolver = GoalConflictResolver::new(3);
        let dep_goal = make_goal(
            "dep-g",
            GoalPriority::Medium,
            GoalCategory::CodeHealth,
            None,
        );
        let mut main_goal = make_goal(
            "main",
            GoalPriority::Critical,
            GoalCategory::TestCoverage,
            None,
        ...
        assert!(conflicts
            .iter()
            .any(|c| matches!(c.severity, ConflictSeverity::Info)));
    }
    ...
    fn test_different_files_no_conflict() {
        ...
        let file_conflicts: Vec<_> = conflicts
            .iter()
            .filter(|c| matches!(c.reason, ConflictReason::SameFile { .. }))
            .collect();
