...
impl CloudSandboxProvider for RemoteApiProvider {
        ...
        let mut req = self
            ...
            .multipart(
                reqwest::multipart::Form::new()
                    .part("file", reqwest::multipart::Part::bytes(data).file_name(path.to_string())),
            );
    ...
    async fn download_result(&self, session_id: &str) -> Result<CloudResult, String> {
        ...
        let resp = req.send().await.map_err(|e| format!("download result: {}", e))?;
        let status = resp.status();
        let body: serde_json::Value = resp
            .json()
            .await
        ...
        if !status.is_success() {
            return Err(body["error"].as_str().unwrap_or("download failed").to_string());
        }
                ...
                Ok(resp) => {
                    if let Ok(body) = resp.text().await {
                        for line in body.lines() {
                            let _ = tx.unbounded_send(line.to_string());
                        }
                    }
                }
                Err(e) => {
    ...
    async fn cancel(&self, session_id: &str) -> Result<(), String> {
        ...
        let resp = req.send().await.map_err(|e| format!("cancel failed: {}", e))?;
        if !resp.status().is_success() {
            let body: serde_json::Value = resp.json().await.unwrap_or_default();
