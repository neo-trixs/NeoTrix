...
impl SessionDistiller {
    ...
    pub fn extract_patterns(&mut self, logs: &[(String, String)]) -> Vec<SessionPattern> {
        ...
        for (session_id, content) in logs {
            ...
            if lower.contains("同步执行") || lower.contains("parallel") {
                pattern_map
                    .entry("parallel_dispatch".to_string())
                    .or_insert((
                        "用户使用「同步执行」模式，要求并行分派独立任务".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
            ...
            {
                pattern_map
                    .entry("bottleneck_analysis".to_string())
                    .or_insert((
                        "用户周期性询问瓶颈分析，期望结构化路线图".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
            ...
            {
                pattern_map
                    .entry("architecture_first".to_string())
                    .or_insert((
                        "用户期望先收到架构全景/流程图的宏观概述，再进入执行".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
            ...
            if content.contains("|------|") || content.contains("| 等级 |") {
                pattern_map
                    .entry("table_format".to_string())
                    .or_insert((
                        "沟通输出偏好表格格式，便于快速对比优先级".to_string(),
                        Vec::new(),
                        false,
                    ))
                    .1
            ...
            if lower.contains("继续完善") || lower.contains("全部") || lower.contains("all") {
                pattern_map
                    .entry("batch_approval".to_string())
                    .or_insert((
                        "用户偏好一次性批准全部路线图，而非逐个选择".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
            ...
            {
                pattern_map
                    .entry("god_view_review".to_string())
                    .or_insert((
                        "用户定期要求从上帝视角全流程审视，识别系统级缺口".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
            ...
            if lower.contains("auto_goal")
                || lower.contains("自动制定")
                || lower.contains("pursue_auto")
            {
                pattern_map
                    .entry("auto_goal_loop".to_string())
                    .or_insert((
                        "核心工作模式：GoalLoop 自动目标追求 + BackgroundLoop".to_string(),
                        Vec::new(),
                        true,
                    ))
                    .1
    ...
    pub fn generate_suggestions(&self, patterns: &[SessionPattern]) -> Vec<String> {
        ...
        for p in patterns {
            ...
            match p.name.as_str() {
                "parallel_dispatch" => {
                    suggestions.push(
                        "用户说「同步执行」时，立即用 Task tool 并行分派独立任务，不等确认。"
                            .to_string(),
                    );
                }
                "bottleneck_analysis" => {
                    suggestions.push(
                        "每次完成一轮后主动提供「还有需要进化的路线吗」结构的路线图。".to_string(),
                    );
                }
                "architecture_first" => {
                ...
                "batch_approval" => {
                    suggestions
                        .push("给多选项时一次性列出全部路线，用户会选「同步执行」。".to_string());
                }
                "god_view_review" => {
                    suggestions.push(
                        "定期执行全流程审视：cargo check → 列出缺口 → 按等级排优先级。".to_string(),
                    );
                }
                _ => {}
            }
    ...
    pub fn update_agents_md(&self) -> Result<(), String> {
        ...
        if self.patterns.is_empty() {
            ...
            for pattern in &self.patterns {
                content.push_str(&format!(
                    ...
                    pattern.name, pattern.description, pattern.frequency, pattern.actionable
                ));
            }
        }
        ...
        content.push_str("\n## Guidelines\n\n");
        content.push_str(
            "- Follow existing code conventions (snake_case for Rust, camelCase for TS)\n",
        );
