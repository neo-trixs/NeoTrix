...
impl ContentClassifier {
    pub fn new() -> Self {
        ...
        format_heuristics.insert(
            CrawlFormat::LegalDocument,
            vec![
                "§".into(),
                "article".into(),
                "subsection".into(),
    ...
    pub async fn try_llm_classify(
        ...
        let request = LlmRequest::new("default", &prompt).with_max_tokens(200);
        ...
        let topic = CrawlTopic::all()
            .into_iter()
            .find(|t| t.name() == llm.topic)?;
    ...
    fn classify_topic(&self, text: &str, url: &str) -> CrawlTopic {
        ...
        if url_lower.contains("legislation")
            || url_lower.contains("law")
            || url_lower.contains("court")
            || url_lower.contains("juris")
            || url_lower.contains("constitut")
        {
            return CrawlTopic::LawAndGovernance;
        }
        if url_lower.contains("policy")
            || url_lower.contains("govern")
            || url_lower.contains("regulation")
        {
            return CrawlTopic::PolicyAndRegulation;
        }
        if url_lower.contains("arxiv")
            || url_lower.contains("nature.com")
            || url_lower.contains("science")
        {
        ...
        if url_lower.contains("who.int")
            || url_lower.contains("health")
            || url_lower.contains("medic")
        {
    ...
    fn classify_format(&self, text: &str, url: &str, _topic: &CrawlTopic) -> CrawlFormat {
        ...
        if url_lower.contains("wikipedia")
            || url_lower.contains("britannica")
        ...
        for (format, heuristics) in &self.format_heuristics {
            ...
            let score = heuristics
                .iter()
                .filter(|h| text.contains(&h.to_lowercase()))
                .count();
            if score > best_score {
                best_score = score;
                best_format = *format;
    ...
    fn extract_title(&self, text: &str, url: &str) -> String {
        ...
        if let Some(title_start) = text.find("# ") {
            ...
            return text[title_start + 2..title_start + 2 + end]
                .trim()
                .to_string();
        }
...
mod tests {
    ...
    fn test_classify_science_text() {
        let mut classifier = ContentClassifier::new();
        let science_text =
            "The experiment data shows algorithm machine learning quantum physics results";
    ...
    fn test_classify_by_url() {
        ...
        let result = classifier.classify(
            ...
            "some law text about constitution",
        );
        assert_eq!(result.topic, CrawlTopic::LawAndGovernance);
    }
    ...
    fn test_format_detection() {
        let mut classifier = ContentClassifier::new();
        let result = classifier.classify(
            "https://www.gov.uk/law",
            "official government act pursuant to section 5",
        );
    ...
    async fn test_try_llm_classify_returns_none_when_no_provider() {
        ...
        let result = classifier
            .try_llm_classify("https://example.com", "Test", "some content")
            .await;
    ...
    async fn test_classify_fallback_to_keyword() {
        ...
        let result = classifier
            .classify_with_llm_fallback(
                ...
                "Law Article",
                "This statute law article constitution amendment",
            )
            .await;
        assert_eq!(result.topic, CrawlTopic::LawAndGovernance);
    ...
    impl LlmProvider for MockLlmProvider {
        async fn complete(
            ...
            crate::neotrix::nt_io_provider::LlmResponse,
            crate::neotrix::nt_io_provider::LlmError,
        > {
