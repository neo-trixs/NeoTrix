...
use crate::neotrix::nt_shield_stealth_net::system_fingerprint::BrowserFingerprintProfile;
...
impl StealthBrowser {
    ...
    async fn inject_fingerprint_js(&self, page: &Page) -> Result<(), String> {
        let js = {
            ...
            fp_guard.as_ref().map(|fp| fp.to_cdp_js())
        };
        if let Some(js) = js {
            page.evaluate(js)
                .await
    ...
    pub async fn launch(&self) -> Result<(), String> {
        ...
        self.cdp_handler = Some(tokio::spawn(async move {
        ...
        if let Err(e) = page.close().await {
            log::warn!("[stealth] launch close page failed: {}", e);
        }
    ...
    async fn new_page(&self) -> Result<Page, String> {
        let browser = {
            ...
            guard.take().ok_or("Browser not launched")?
        };
        ...
        page.enable_stealth_mode()
            ...
            .map_err(|e| format!("Enable stealth: {}", e))?;
        self.inject_fingerprint_js(&page).await?;
        *self.nt_world_browse.lock().await = Some(browser);
        Ok(page)
    }
    ...
    pub async fn fetch(&self, url: &str) -> Result<String, String> {
        ...
        if let Err(e) = page.close().await {
            log::warn!("[stealth] fetch close page failed: {}", e);
        }
        Ok(content)
    }
    ...
    pub async fn title(&self, url: &str) -> Result<String, String> {
        ...
        let title = result
            ...
            .to_string();
        if let Err(e) = page.close().await {
            log::warn!("[stealth] title close page failed: {}", e);
        }
        Ok(title)
    }
    ...
    pub async fn close(&self) {
        if let Some(h) = self.cdp_handler.as_ref() {
            h.abort();
        }
        let mut guard = self.nt_world_browse.lock().await;
