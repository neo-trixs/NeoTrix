...
fn test_spider() -> CheckpointSpider {
    let dir = test_dir();
    CheckpointSpider::new("test_spider", vec!["https://example.com".to_string()], dir)
}
...
fn test_next_request_priority_order() {
    ...
    let first = spider
        .next_request()
        .expect("expected a next request (highest priority)");
    assert_eq!(first.url, "https://example.com/high");
    assert_eq!(first.priority, 1);

    let second = spider
        .next_request()
        .expect("expected a next request (medium priority)");
    assert_eq!(second.url, "https://example.com/medium");
    assert_eq!(second.priority, 5);

    let third = spider
        .next_request()
        .expect("expected a next request (lowest priority)");
...
#[tokio::test]
async fn test_should_checkpoint_after_interval() {
    let mut spider = test_spider();
...
fn test_load_checkpoint_restores_state() {
    ...
    let loaded =
        CheckpointSpider::load_checkpoint(&path).expect("should load checkpoint from file");
...
fn test_load_checkpoint_restores_pending_queue() {
    ...
    let first = loaded
        .next_request()
        .expect("expected a next request from loaded checkpoint");
    assert_eq!(first.url, "https://example.com/second");
    assert_eq!(first.priority, 5);

    let second = loaded
        .next_request()
        .expect("expected second next request from loaded checkpoint");
...
fn test_checkpoint_file_path() {
    ...
    assert_eq!(
        path,
        PathBuf::from("/tmp/neotrix/my_nt_world_crawl_checkpoint.json")
    );
}
...
fn test_resume_from_checkpoint() {
    ...
    let mut resumed =
        CheckpointSpider::resume_from_checkpoint(&cp_path).expect("should resume from checkpoint");
    ...
    let next = resumed
        .next_request()
        .expect("expected a next request after resume");
