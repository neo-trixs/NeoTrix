        ...
        self.nt_world_browse_client.get_or_insert_with(|| {
            Client::builder()
                ...
                .default_headers({
                    ...
                    headers.insert(reqwest::header::ACCEPT, "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8".parse().expect("valid accept header"));
                    headers.insert(reqwest::header::ACCEPT_LANGUAGE, "en-US,en;q=0.9".parse().expect("valid accept-language header"));
                    headers.insert(reqwest::header::DNT, "1".parse().expect("valid dnt header"));
                    headers.insert(reqwest::header::CONNECTION, "keep-alive".parse().expect("valid connection header"));
                    headers.insert(reqwest::header::UPGRADE_INSECURE_REQUESTS, "1".parse().expect("valid upgrade-insecure-requests header"));
                    headers
                })
                .build()
...
impl std::fmt::Display for FetcherSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.total_requests, self.total_success, self.total_timeouts, self.total_blocked,
            self.total_bytes, self.error_rate * 100.0,
        )
    }
}
