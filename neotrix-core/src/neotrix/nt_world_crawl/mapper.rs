...
use crate::core::nt_core_knowledge::{RewardSource, TaskType};
...
impl KnowledgeMapper {
    pub fn new() -> Self {
        ...
        topic_edit_map.insert(
            CrawlTopic::General,
            vec![("synthesis".into(), 0.04), ("analysis".into(), 0.04)],
        );
    ...
    pub fn map(&mut self, classified: &ClassifiedContent) -> MappedKnowledge {
        ...
        let insights = vec![
            format!(
                "{} content from {}",
                classified.topic.name(),
                classified.url
            ),
            format!(
                "format: {}, confidence: {:.2}",
                classified.format.name(),
    ...
    pub fn apply_to_brain(
        ...
        let micro_edits: Vec<MicroEdit> = edits
            .iter()
        ...
        let mem = ReasoningMemory {
            ...
            timestamp: std::time::SystemTime::now()
                ...
                .expect("MapperEntry: system clock is always after UNIX_EPOCH")
                .as_secs() as i64,
            embedding: None,
...
mod tests {
    ...
    fn test_map_legal_content() {
        ...
        let classified = classifier.classify(
            ...
            "statute law constitution amendment rights act",
        );
    ...
    fn test_map_philosophy_content() {
        ...
        let classified = classifier.classify(
            "https://plato.stanford.edu/ethics",
            "philosophy ethics moral reasoning logic virtue",
        );
        let mapped = mapper.map(&classified);
