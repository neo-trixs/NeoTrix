...
use super::config::{CrawlStrategy, CrawlTopic, CrawlerConfig, SeedEntry};
use super::fetcher::{FetchError, FetcherPool, FetcherProtocol};
...
impl UnifiedCrawler {
    ...
    pub fn run_cycle(
        ...
        let now = std::time::SystemTime::now()
            ...
            .expect("UnifiedCrawler: system clock is always after UNIX_EPOCH")
            .as_secs();
        ...
        let ret = 'cycle: {
            ...
            let result = self
                .fetcher
                .fetch_with_retry(&url_entry.url, self.config.max_retries);
            ...
            if let Some(ref err) = result.error {
                ...
                break 'cycle CycleResult {
                    ...
                    message: format!(
                        "fetch error: {}",
                        result.error.as_ref().map_or("", |e| &e.message)
                    ),
                    is_heal_check: self.should_heal(),
                };
            }
            ...
            let mapped_coord = KnowledgeMapper::absorb_to_hypercube(
                &mapped,
                classified.topic,
            ...
            self.bridge
                .hypercube
                .insert(&mapped_coord, &mapped.url, &mapped.title);
        ...
        if backoff_count >= 5 {
            self.config.strategy = match self.config.strategy {
                ...
                CrawlStrategy::Polite => {
                    log::warn!("[nt_world_crawl] ⏹ 连续 minor errors, 策略已最保守, 停止爬取");
                    return true;
                }
            };
            self.fetcher.adjust_strategy(self.config.strategy);
            log::warn!(
                "[nt_world_crawl] ⏸ minor errors backoff: → {:?}",
    ...
    fn run_self_healing(&mut self) {
        ...
        if errors.is_empty() {
            ...
            let sparse_dims = gap_reports
                .iter()
                .filter(|r| r.sparsity_score > 0.7)
                .count();
            if sparse_dims > 2 {
                if let Some(last) = self.heal_history.last_mut() {
                    let seed_msg = format!(
                        "gap analysis: {} sparse dimensions → auto-generated seeds queued",
                        sparse_dims
                    );
        ...
        let timeout_count = errors
            .iter()
            .filter(|e| e.duration_ms > self.config.fetch_timeout_secs * 1000)
            .count();
        let blocked_count = errors
            .iter()
            .filter(|e| e.status_code == 403 || e.status_code == 401)
            .count();
        ...
        let action = if error_rate > 0.5 {
            ...
            format!(
                "too many timeouts → increased timeout to {}s",
    ...
    pub fn add_seeds(&mut self, seeds: Vec<SeedEntry>) {
        let entries: Vec<UrlEntry> = seeds
            .iter()
            .filter(|s| s.enabled)
            .map(|s| UrlEntry {
                url: s.url.clone(),
...
impl std::fmt::Display for CycleResult {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let heal_tag = if self.is_heal_check {
            " [heal check]"
        } else {
            ""
        };
...
mod tests {
    ...
    fn test_heal_with_errors() {
        ...
        assert!(
            nt_world_crawl.heal_history[0].action.contains("downgraded")
                || nt_world_crawl.heal_history[0].action.contains("blocked")
        );
    }
