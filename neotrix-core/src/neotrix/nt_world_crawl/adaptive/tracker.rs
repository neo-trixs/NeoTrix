...
use super::helpers::{
    build_selector, fingerprint_from_dom_element, jaccard_str, levenshtein_normalized,
};
...
impl AdaptiveTracker {
    ...
    pub fn locate(&mut self, name: &str, dom_snapshot: &DomSnapshot) -> Option<FuzzyMatch> {
    ...
    pub fn smart_selector(saved: &SavedElement, dom_snapshot: &DomSnapshot) -> String {
        ...
        candidates
            .first()
            .map(|(s, _)| s.clone())
            .unwrap_or_else(|| saved.original_selector.clone())
    }

    fn selector_stability(fp_a: &ElementFingerprint, fp_b: &ElementFingerprint) -> f64 {
        let id_stable = if fp_a
            .data_attrs
            .keys()
            .any(|k| k == "id" || k.starts_with("data-"))
        {
            0.4
        } else {
            0.0
        ...
        let parent_stable = if fp_a.parent_tag == fp_b.parent_tag {
            0.1
        } else {
            0.0
        };
