...
const FIELD_SEARCH_TERMS: &[&str; 23] = &[
    "compound+component+composition+design+system",
    "tailwind+css+utility+framework",
    "web+accessibility+a11y+WCAG",
    "react+aria+accessible+components",
    "AI+native+user+interface+adaptive+UX",
    "semantic+layer+design+token+system",
    "verification+testing+formal+validation+quality",
    "quality+gates+CI+pipeline+automation",
    "video+rendering+HTML+snapshot+capture",
    "html+composition+layout+engine",
    "secret+detection+key+leak+nt_shield",
    "nt_shield+audit+web+vulnerability+scanning",
    "vulnerability+knowledge+base+CVE+exploit",
    "anti+detection+nt_world_browse+fingerprinting+stealth",
    "web+scraping+nt_world_crawl+automation+extraction",
    "react+lint+static+analysis+code+quality",
    "health+scoring+codebase+quality+metrics",
    "vector+design+canvas+graphics+editor",
    "MCP+tool+server+model+context+protocol",
    "agent+trading+quantitative+finance+algorithm",
    "signal+synchronization+real+time+data+stream",
    "ESP32+firmware+embedded+IoT+microcontroller",
...
impl ResearchScanner {
    ...
    pub fn run_scan(&mut self, dims: &[f64], weak_threshold: f64) -> Vec<DiscoResult> {
        self.cycle += 1;
        let weak_dims: Vec<(usize, &str)> = FIELD_NAMES_DOMAIN
            .iter()
            .enumerate()
        ...
        if self.scan_history.len() > self.max_history {
            self.scan_history
                .drain(..self.scan_history.len() - self.max_history);
        }

        let elapsed = start.elapsed();
        log::info!(
            "[disco] cycle {}: {} results in {}ms",
            self.cycle,
            results.len(),
            elapsed.as_millis()
        );
