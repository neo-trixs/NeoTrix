...
const DOMAIN_STALE_SECS: u64 = 7 * 24 * 3600; // 7 days
...
impl DualQueueFrontier {
    ...
    fn prune_old_entries(&mut self, now_secs: u64) {
        if self.seen.len() > MAX_SEEN {
            let drain_count = self.seen.len() / 2;
            let to_remove: Vec<String> = self.seen.iter().take(drain_count).cloned().collect();
            for url in &to_remove {
                self.seen.remove(url);
            }
        }
        let cutoff = now_secs.saturating_sub(DOMAIN_STALE_SECS) * 1000;
        self.domain_last_access
            .retain(|_, &mut last| last >= cutoff);
        self.domain_pushed_count
            .retain(|d, _| self.domain_last_access.contains_key(d));
        self.domain_visited_count
            .retain(|d, _| self.domain_last_access.contains_key(d));
    }

    pub fn push(&mut self, entry: UrlEntry) -> bool {
        self.prune_old_entries(0);
    ...
    pub fn pop(&mut self, now_secs: u64, min_interval_ms: u64) -> Option<UrlEntry> {
        self.prune_old_entries(now_secs);
        for tier in (0..self.max_priority_tiers).rev() {
            while let Some(entry) = self.front_queues[tier].pop_front() {
                ...
                self.domain_visited_count
                    ...
                    .or_insert(1);
                self.domain_last_access
                    .insert(entry.domain.clone(), now_secs * 1000);
                self.total_popped += 1;
                return Some(entry);
            }
        }
...
mod tests {
    ...
    fn test_push_and_pop() {
        ...
        assert_eq!(
            popped.expect("popped should be ok in test").url,
    ...
    fn test_priority_tiers() {
        ...
        assert_eq!(
            first.expect("first should be ok in test").url,
            "https://high.com"
        );
    }
