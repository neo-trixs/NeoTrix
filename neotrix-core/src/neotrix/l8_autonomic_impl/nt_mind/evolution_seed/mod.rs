...
use crate::core::nt_core_meta::planner::{EvolutionPlanner, PlannedEvolution, ImpactEstimate, RiskLevel};
...
use super::exploration_pipeline::{ExploreDomain, ExplorationPipeline};
...
impl SeedRegistry {
    ...
    pub fn priority_queue(&self) -> Vec<&EvolutionTarget> {
        let mut sorted: Vec<&EvolutionTarget> = self.targets.iter().filter(|t| !t.absorbed).collect();
    ...
    pub fn enqueue_all(&self, pipeline: &mut ExplorationPipeline) -> usize {
        ...
        for target in &self.targets {
            if !self.absorbed.contains_key(&target.url) {
                pipeline.ingest(&target.url, Some(target.domain));
                count += 1;
            }
        }
        count
    ...
    pub fn generate_plans(&self, planner: &mut EvolutionPlanner) -> Vec<PlannedEvolution> {
        ...
        for target in &self.targets {
            ...
            let weakness = Weakness {
                ...
                impact: format!("Missing {} capability (priority {})", target.name, target.priority),
            ...
            let plan = PlannedEvolution {
                ...
                estimated_impact: ImpactEstimate {
                    files_affected: 3,
                    risk: if target.priority >= 4 { RiskLevel::Low } else { RiskLevel::Medium },
                },
                dependencies: vec![],
            };
            plans.push(plan);
    ...
    pub fn targets_by_domain(&self, domain: &ExploreDomain) -> Vec<&EvolutionTarget> {
        self.targets.iter().filter(|t| t.domain == *domain).collect()
    }
...
mod tests {
    ...
    fn test_seed_registry_has_all_targets() {
        ...
        assert_eq!(reg.targets.len(), 16, "should have 16 evolution targets (15 repos + arnold)");
    }
    ...
    fn test_priority_5_targets_exist() {
        let reg = SeedRegistry::new();
        let priority_5: Vec<&EvolutionTarget> = reg.targets.iter().filter(|t| t.priority == 5).collect();
        assert_eq!(priority_5.len(), 3, "context-mode, cybernt_shield-skills, arnold");
    }
    ...
    fn test_seed_driver_has_crystalizer() {
        let reg = SeedRegistry::new();
        let pipeline = ExplorationPipeline::new(PathBuf::from("/tmp/neotrix-test"));
        let mut brain = ReasoningBrain::new();
        let mut bank = ReasoningBank::new(100);
        let crystallizer = AutoCrystallizer::new();
        let driver = SeedDriver::new(
            reg,
            pipeline,
            &mut brain,
            &mut bank,
            crystallizer,
            PathBuf::from("/tmp/neotrix-test"),
        );
    ...
    fn test_crystallize_all_domain_targets() {
        ...
        assert!(crystallizer.registry.summary().contains(&format!("{} crystals", domains.len())));
    }
    ...
    fn test_crystal_registry_persist_cycle() {
        ...
        assert_eq!(loaded.crystals[0].strategy, crate::core::nt_core_self::StrategyKind::Reflection);
        assert_eq!(loaded.crystals[0].domain, crate::core::nt_core_self::AttentionDomain::Code);
