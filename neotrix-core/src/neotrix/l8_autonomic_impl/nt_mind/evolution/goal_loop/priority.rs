...
impl RICEScore {
    pub fn new(reach: f64, impact: f64, confidence: f64, effort: f64) -> Self {
        Self { reach, impact, confidence, effort }
    }
...
impl MoscowClass {
    pub fn from_score(score: f64) -> Self {
        if score >= 8.0 { MoscowClass::MustHave }
        else if score >= 5.0 { MoscowClass::ShouldHave }
        else if score >= 2.0 { MoscowClass::CouldHave }
        else { MoscowClass::WontHave }
    }
    pub fn rank(&self) -> u8 {
        match self {
...
impl PriorityEngine {
    pub fn new(framework: PriorityFramework) -> Self {
        Self { framework, ..Default::default() }
    }

    pub fn evaluate(&self, description: &str, complexity: f64) -> f64 {
        let word_count = description.split_whitespace().count() as f64;
        let has_critical = description.to_lowercase().contains("critical") || description.to_lowercase().contains("nt_shield");
        let has_urgent = description.to_lowercase().contains("urgent") || description.to_lowercase().contains("blocker");
        let has_improve = description.to_lowercase().contains("improve") || description.to_lowercase().contains("optimize");

        let reach = if has_critical { 9.0 } else if has_urgent { 7.0 } else { 4.0 };
        let impact = if has_critical { 9.0 } else if has_improve { 3.0 } else { 5.0 };
        ...
        match self.framework {
            ...
            PriorityFramework::Hybrid => {
                (rice * self.weights[0] + ice * self.weights[1]) / (self.weights[0] + self.weights[1])
            }
        }
    }

    pub fn rank(&self, goals: &[(String, f64)]) -> Vec<usize> {
        let scores: Vec<f64> = goals.iter().map(|(desc, complexity)| self.evaluate(desc, *complexity)).collect();
