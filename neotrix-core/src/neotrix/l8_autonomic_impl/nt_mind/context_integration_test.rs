...
fn test_engine_with_artifact_indexer_produces_artifact_context() {
    ...
    store.store(
        Artifact::new("users_db", ArtifactType::DatabaseSchema, "CREATE TABLE users (id INT)")
            .with_tags(&["database"]),
    );
    store.store(
        Artifact::new("payment_api", ArtifactType::ApiSpec, "POST /payments")
            .with_tags(&["api"]),
    );
    ...
    *indexer.store_mut() = store;
    ...
    let engine = ReasoningEngine::from_parts(Box::new(brain), bank)
        .with_artifact_indexer(indexer);

    let context = engine.build_artifact_context("payment");
    if context.is_empty() {
        return;
    }
    assert!(context.contains("Relevant artifacts"));
