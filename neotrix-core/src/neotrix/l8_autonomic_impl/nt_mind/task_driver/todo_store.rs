...
fn parse_items(v: &serde_yaml::Value) -> Result<Vec<TodoItem>, String> {
    ...
    for (i, item) in arr.iter().enumerate() {
        ...
        let files: Vec<String> = match &item["files"] {
            serde_yaml::Value::Sequence(seq) => {
                seq.iter().filter_map(|v: &serde_yaml::Value| v.as_str().map(String::from)).collect()
            }
            _ => Vec::new(),
        };
        let depends_on: Vec<String> = match &item["depends_on"] {
            serde_yaml::Value::Sequence(seq) => {
                seq.iter().filter_map(|v: &serde_yaml::Value| v.as_str().map(String::from)).collect()
            }
            _ => Vec::new(),
