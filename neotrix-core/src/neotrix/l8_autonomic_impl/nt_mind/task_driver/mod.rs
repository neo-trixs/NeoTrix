...
pub use todo_store::{TodoStore, TodoItem, ItemStatus};
pub use verifier::CargoVerifier;
...
impl TaskDriver {
    ...
    pub fn ready_task_ids(&self) -> Vec<String> {
        let done_ids: HashSet<String> = self.todo.items.iter()
            .filter(|i| i.status == ItemStatus::Done)
            .map(|i| i.id.clone())
            .collect();
        let mut ready: Vec<String> = self.todo.items.iter()
    ...
    pub fn execute_single_task(&mut self, task: &TodoItem) -> TaskResult {
        ...
        if all_pass {
            ...
            let _ = self.todo.update_status(&task.id, ItemStatus::Blocked);
            let err_first: String = verify.iter()
                .find(|(_, r)| !r.passed)
    ...
    fn exec_impl(&self, task: &TodoItem) -> Result<(), String> {
        if let Some(ref engine) = self.engine {
            ...
            match task_type {
                TaskType::Design => { eng.reason_task(&prompt).map_err(|e| e.to_string())?; }
                TaskType::CodeGeneration => { eng.reason_task(&prompt).map_err(|e| e.to_string())?; }
                TaskType::CodeReview => { eng.reason_task(&prompt).map_err(|e| e.to_string())?; }
                _ => { eng.reason_task(&prompt).map_err(|e| e.to_string())?; }
            }
        }
        Ok(())
    }
    ...
    pub fn run_autonomous(&mut self, max_tasks: usize) -> (BatchResult, String) {
        ...
        for r in &result.results {
            ...
            log.push_str(&format!("{} {}: {}\n", icon, r.task_id, r.error.as_deref().unwrap_or("ok")));
        }
    ...
    pub fn task_queue_status(&self) -> String {
        ...
        let done = self.todo.items.iter().filter(|i| i.status == ItemStatus::Done).count();
        let pending = self.todo.items.iter().filter(|i| i.status == ItemStatus::Pending).count();
        let blocked = self.todo.items.iter().filter(|i| i.status == ItemStatus::Blocked).count();
...
fn infer_task_type(task: &TodoItem) -> TaskType {
    ...
    if title_lower.contains("design") || title_lower.contains("设计") {
        TaskType::Design
    } else if title_lower.contains("code") || title_lower.contains("实现") || title_lower.contains("implement") {
        TaskType::CodeGeneration
    } else if title_lower.contains("review") || title_lower.contains("分析") || title_lower.contains("安全") {
        TaskType::CodeReview
    } else {
        TaskType::General
    }
...
mod tests {
    ...
    fn project_root_todo() -> String {
        ...
        manifest.parent().expect("value should be ok in test").join("TODO.yml").to_string_lossy().to_string()
    }
    ...
    fn test_infer_task_type_design() {
        let item = TodoItem {
            id: "x".into(), title: "Design HTTP Proxy".into(),
            status: ItemStatus::Pending, priority: ItemPriority::High,
            created: "2026-01-01".into(), updated: "2026-01-01".into(),
            session: None, files: vec![], depends_on: vec![], notes: "".into(),
        };
    ...
    fn test_infer_task_type_code() {
        let item = TodoItem {
            id: "y".into(), title: "Implement PoC Engine".into(),
            status: ItemStatus::Pending, priority: ItemPriority::High,
            created: "2026-01-01".into(), updated: "2026-01-01".into(),
            session: None, files: vec![], depends_on: vec![], notes: "".into(),
        };
    ...
    fn test_batch_result_default() {
        let r = BatchResult {
            attempted: 0, succeeded: 0, failed: 0,
            results: vec![], verify_summary: "".into(),
        };
