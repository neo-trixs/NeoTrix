...
impl DimensionTag {
    pub fn all() -> Vec<DimensionTag> {
        ...
        vec![
            ...
            CivilizationRise, CivilizationFall, CivilizationTheory,
            TechAgriculture, TechIndustrial, TechInformation, TechSpace, TechAI,
            SpeciesEvolution, SpeciesExtinction, SpeciesHuman,
            GeoGeology, GeoClimate, GeoEcosystem,
            CosmoSpacetime, CosmoMultiverse, CosmoDimension,
            KnowledgePhilosophy, KnowledgeScience, KnowledgeCulture,
        ]
    }
    ...
    pub fn category(&self) -> &'static str {
        use DimensionTag::*;
        match self {
            TimelineGeology | TimelineLife | TimelineHuman | TimelineCivilization | TimelineFuture => "时间链",
    ...
    pub fn detect(title: &str, text: &str) -> Vec<DimensionTag> {
        ...
        if b.contains("billion years") || b.contains("million years") || b.contains("geologic") || b.contains("era") {
            dims.push(DimensionTag::TimelineGeology);
        }
        if b.contains("evolution") || b.contains("natural selection") || b.contains("common descent") || b.contains("life") {
            dims.push(DimensionTag::TimelineLife);
        }
        if b.contains("human evolution") || b.contains("hominin") || b.contains("australopithecus") || b.contains("homo") {
            dims.push(DimensionTag::TimelineHuman);
        }
        if b.contains("civilization") || b.contains("empire") || b.contains("dynasty") || b.contains("kingdom") {
            dims.push(DimensionTag::TimelineCivilization);
        }
        if b.contains("future") || b.contains("prediction") || b.contains("scenario") || b.contains("2050") {
        ...
        if b.contains("industrial") || b.contains("steam") || b.contains("factory") || b.contains("manufacturing") {
            dims.push(DimensionTag::TechIndustrial);
        }
        if b.contains("information") || b.contains("computer") || b.contains("digital") || b.contains("internet") {
            dims.push(DimensionTag::TechInformation);
        }
        if b.contains("space") || b.contains("rocket") || b.contains("satellite") || b.contains("nasa") {
            dims.push(DimensionTag::TechSpace);
        }
        if b.contains("artificial intelligence") || b.contains("machine learning") || b.contains("neural") {
        ...
        if b.contains("plate") || b.contains("tectonic") || b.contains("continent") || b.contains("ocean") {
        ...
        if b.contains("ecosystem") || b.contains("habitat") || b.contains("organism") || b.contains("species") {
        ...
        if b.contains("science") || b.contains("physics") || b.contains("biology") || b.contains("chemistry") {
            dims.push(DimensionTag::KnowledgeScience);
        }
        if b.contains("culture") || b.contains("religion") || b.contains("art") || b.contains("language") {
...
pub enum Modality {
    Text,
    Image { format: String, caption: String },
    Video { format: String, duration_secs: Option<f64> },
    Audio { format: String, duration_secs: Option<f64> },
    WebPage,
    StructuredData,
    KnowledgeSource,
    ReasoningTrace,
...
impl MemoryTrace {
    pub fn new(title: &str, source: &str, summary: &str, modality: Modality, dimensions: Vec<DimensionTag>) -> Self {
        Self {
    ...
    pub(crate) fn detect_source_type(source: &str) -> String {
        ...
        if s.contains("wikipedia") { "wikipedia".to_string() }
        else if s.contains("arxiv") { "arxiv".to_string() }
        else if s.contains("github") { "github".to_string() }
        else if s.contains(".pdf") || s.contains("paper") { "paper".to_string() }
        else if s.contains("youtube") || s.contains("video") { "video".to_string() }
        else if s.contains("image") || s.contains("photo") { "image".to_string() }
        else { "web".to_string() }
    }
...
impl MemoryLayer {
    pub fn all() -> Vec<MemoryLayer> {
        vec![MemoryLayer::Sensory, MemoryLayer::Topic, MemoryLayer::Event, MemoryLayer::Fact]
    }
