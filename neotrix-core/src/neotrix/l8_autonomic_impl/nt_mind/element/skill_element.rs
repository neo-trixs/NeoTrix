...
impl SkillElement {
    ...
    pub fn recommend(&self, strategy: StrategyKind, domain: AttentionDomain) -> Option<String> {
        self.registry.find_similar(strategy, domain)
...
impl Element for SkillElement {
    fn id(&self) -> &str { "element.skill" }
    fn name(&self) -> &str { "Skill Crystalization" }
    fn version(&self) -> &str { "0.2.0" }
    fn element_type(&self) -> ElementType { ElementType::Feature }
