...
use crate::core::nt_core_bank::{
    ReasoningMemory, ReasoningBank, PipelineConfig, PipelineState,
    OffloadManager, L1Memory, SceneBlock, Persona,
};
...
impl PipelineScheduler {
    ...
    pub async fn trigger_l1(&self, bank: &mut ReasoningBank) {
        ...
        let l1_results: Vec<L1Memory> = memories.iter().filter_map(|m| {
            ...
            let mem_type = if m.task_description.contains("nt_shield") || m.task_description.contains("safety") {
                "instruction"
        ...
        for mem in &l1_memories {
            ...
            if let Some(scene) = existing {
                ...
                scenes.push(SceneBlock {
                    ...
                    summary: if mem.content.len() > 60 { mem.content[..60].to_string() } else { mem.content.clone() },
    ...
    pub async fn trigger_l3(&self) {
        ...
        if let Some(ref existing_str) = existing {
            for line in existing_str.lines() {
                ...
                if trimmed.starts_with("## Base Anchors") { persona.base_anchors.push(trimmed.to_string()); }
                if trimmed.starts_with("## Interest Map") { persona.interest_map.push(trimmed.to_string()); }
                if trimmed.starts_with("## Interaction Protocol") { persona.interaction_protocol.push(trimmed.to_string()); }
                if trimmed.starts_with("## Cognitive Kernel") { persona.cognitive_kernel.push(trimmed.to_string()); }
            }
        }

        // 更新追加
        persona.base_anchors.push(format!("Last pipeline run: {}", chrono::Utc::now().timestamp()));
        persona.interest_map.push("Task types observed: General, Design, CodeAnalysis, Security".to_string());
        persona.interaction_protocol.push("Prefers structured output with reward signals".to_string());
        persona.cognitive_kernel.push("Adaptive learning via capability vector updates".to_string());
