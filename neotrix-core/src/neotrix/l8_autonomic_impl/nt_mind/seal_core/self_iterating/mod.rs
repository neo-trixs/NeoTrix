...
pub use hyperarchive::{ParentSelection, SelectionConfig, HyperAgentRecord, HyperAgentArchive};
pub use hyperdgm::{DGMMetaAgent, LatentEdit, GenerativeReplay, SelfReferentialCheck, SelfRefCheckResult};
pub use hyperstage::{MetaEvolveStage, DGMMetaEvolveStage};
...
pub use brain_dgm::{DgmSelfEditStrategy, EditCritic, EditContext, DgmEditOrchestrator};

// SkillOpt re-exports
pub use skillopt::{LrScheduler, EditBudget, ValidationGate, RejectedEditBuffer, RejectedEdit,
...
pub use goal_contract::{
    GoalContract, GoalPhase, GoalVerificationReport, PhaseEvidence, EvidenceType,
    RecoveryAction, VerificationAxis, AxisSeverity, AxisVerdict, VerdictVector, RepairTarget,
    GoalContractStage, EvidenceCaptureStage, NarrowRecoveryStage, FinalVerificationStage,
    GoalTerminatorStage, ExternalVerifierStage, SemanticRecallStage,
    analyze_failure, default_verification_axes, run_verification_axis, write_journal, should_stop_seal_loop,
};
...
mod tests {
    ...
    fn test_rollback_on_negative_reward() {
        ...
        if let Err(e) = brain.run_seal_loop(task, None, None) {
            eprintln!("SEAL loop returned error: {}", e);
        }

        // 即使奖励为负，brain 状态应该被回滚
        // 注意：由于噪声和初始化，这个测试可能会不稳定
        println!("学习率: {}", brain.brain.learning_rate);
    }
    ...
    fn test_reasoning_bank_integration() {
        ...
        if let Err(e) = brain.run_seal_loop("设计 UI 组件", None, None) {
            eprintln!("First SEAL loop error: {}", e);
        }

        // 第二次运行，应该能检索到第一次的记忆
        if let Err(e) = brain.run_seal_loop("设计另一个 UI 组件", None, None) {
            eprintln!("Second SEAL loop error: {}", e);
        }
    ...
    fn test_full_seal_algorithm2() {
        ...
        // 运行批量 SEAL 循环（包含基准测试）
        let avg_reward = brain.run_seal_loop_batch(&task_batch).expect("value should be ok in test");

        println!("SEAL Algorithm 2 完成:");
        println!("  平均奖励: {}", avg_reward);
        println!("  迭代次数: {}", brain.iteration);
        println!("  最终学习率: {}", brain.brain.learning_rate);

        let report = brain.get_brain_report();
        println!("  Brain 报告: {:?}", report);

        let bank_stats = brain.reasoning_bank.stats();
        println!("  ReasoningBank 统计: {:?}", bank_stats);
