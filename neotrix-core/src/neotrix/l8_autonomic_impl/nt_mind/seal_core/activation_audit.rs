//! ActivationAuditStage — 论文驱动进化 (arXiv:2605.30621 Harness Evolution) 的量化审计 stage。
//!
//! 论文核心发现：
...
pub struct ActivationAuditSample {
...
impl ActivationAuditSample {
    /// 单样本的三项比率（分母为 0 时返回 0.0，避免除零）。
    pub fn rates(&self) -> AuditRates {
        AuditRates {
...
/// ActivationAuditStage — 累计追踪每次 harness 进化的激活/跟随/阶段依从。
#[derive(Debug, Clone, Default)]
pub struct ActivationAuditStage {
    samples: Vec<ActivationAuditSample>,
    total: ActivationAuditSample,
}

impl ActivationAuditStage {
    pub fn new() -> Self {
        Self::default()
    }

    /// 记录一次 harness 进化样本，并累加到总计。
    pub fn record(&mut self, sample: ActivationAuditSample) {
    ...
    /// 最近一次样本的比率（无样本时 None）。
    pub fn latest_rates(&self) -> Option<AuditRates> {
        self.samples.last().map(|s| s.rates())
    }

    /// 跨所有样本的累计比率。
    pub fn cumulative_rates(&self) -> AuditRates {
        self.total.rates()
    }
    ...
    pub fn report(&self) -> ActivationAuditReport {
        ...
        ActivationAuditReport {
            n_cycles: self.samples.len() as u64,
            slr: r.slr,
            hfr: r.hfr,
...
mod tests {
    ...
    fn test_empty_sample_rates_are_zero() {
        let r = ActivationAuditSample::default().rates();
        assert_eq!(r.slr, 0.0);
        assert_eq!(r.hfr, 0.0);
    ...
    fn test_cumulative_rates_across_cycles() {
        let mut stage = ActivationAuditStage::new();
        stage.record(ActivationAuditSample {
    ...
    fn test_latest_rates_is_last_sample() {
        ...
        let latest = stage.latest_rates().unwrap();
        assert!((latest.slr - 0.9).abs() < 1e-9);
    }

    #[test]
    fn test_report_health_thresholds() {
        let mut crit = ActivationAuditStage::new();
        crit.record(ActivationAuditSample {
            activation_attempts: 100,
            activation_successes: 25,
            ..Default::default()
        });
        assert_eq!(crit.report().health, AuditHealth::Critical);

        let mut deg = ActivationAuditStage::new();
        deg.record(ActivationAuditSample {
            activation_attempts: 100,
            activation_successes: 60,
            ..Default::default()
        });
        assert_eq!(deg.report().health, AuditHealth::Degraded);

        let mut ok = ActivationAuditStage::new();
        ok.record(ActivationAuditSample {
            activation_attempts: 100,
            activation_successes: 95,
            ..Default::default()
        });
        assert_eq!(ok.report().health, AuditHealth::Healthy);
    }

    #[test]
    fn test_report_zero_cycle_healthy_default() {
        let stage = ActivationAuditStage::new();
        let rep = stage.report();
        assert_eq!(rep.n_cycles, 0);
        assert_eq!(rep.health, AuditHealth::Healthy);
    }
}
