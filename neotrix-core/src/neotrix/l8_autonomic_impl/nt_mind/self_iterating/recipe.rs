...
impl Recipe {
    ...
    pub fn execute(&self, brain: &mut SelfIteratingBrain) -> Result<(), NeoTrixError> {
        if self.config.min_permission != brain.permission
            && (brain.permission as i32) < (self.config.min_permission as i32) {
        ...
        if self.config.min_autonomy != brain.autonomy
            && (brain.autonomy as i32) < (self.config.min_autonomy as i32) {
...
pub fn preset_standard() -> Recipe {
    let stages: Vec<RecipeStage> = vec![
        RecipeStage::new(Box::new(crate::neotrix::l8_autonomic_impl::nt_mind_ingestion::pipeline_stages::StreamHygieneStage::new())),
        ...
        RecipeStage::new(Box::new(AntiDistillationStage::new())).with_frequency(5),
    ];
