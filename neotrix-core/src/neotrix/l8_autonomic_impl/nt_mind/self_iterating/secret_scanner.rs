...
impl SecretScanner {
    ...
    pub fn scan_with_context(&self, task: &str, code_context: &str) -> ScanResult {
        ...
        let count_by_severity = {
            let mut map: HashMap<Severity, usize> = HashMap::new();
            for f in &findings {
                *map.entry(f.severity).or_insert(0) += 1;
            }
            map
        };
        ScanResult { findings, max_severity, count_by_severity }
    }
}
...
impl ScanResult {
    ...
    pub fn risk_score(&self) -> f64 {
        ...
        let base = self.findings.iter().map(|f| match f.severity {
            Severity::Low => 0.1,
            Severity::Medium => 0.3,
            Severity::High => 0.6,
...
mod tests {
    ...
    fn test_scanner_detects_jwt() {
        let scanner = SecretScanner::new();
        let findings = scanner.scan("eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.dozjgNryP4J3j3k8JkLxJ8g");
    ...
    fn test_detects_training_data_extraction() {
        ...
        assert!(!findings.is_empty(), "should detect training data extraction");
    }
