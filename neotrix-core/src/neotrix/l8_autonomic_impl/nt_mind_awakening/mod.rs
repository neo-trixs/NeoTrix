...
use neotrix_types::self_measure::{
    SelfMeasure, SystemSnapshot, AwakeningReport, SubsystemId,
};
use neotrix_types::self_model::{
    SelfRepresentation, CausalPredictor, InterventionOutcome,
};
use modify::{SelfModify, Intervention, InterventionResult};
...
pub struct AwakeningEngine {
    ...
    tick: u32,
    #[allow(dead_code)] // kept for future refit scheduling analysis
    last_refit_tick: u32,
...
impl AwakeningEngine {
    ...
    pub fn tick(&mut self, sib: &mut SelfIteratingBrain) -> AwakeningTickResult {
        ...
        if self.tick.is_multiple_of(self.config.refit_interval) && self.measure.tick_count() >= 10 {
        ...
        if self.tick.is_multiple_of(self.config.auto_modify_interval)
                    ...
                    if let Some(best) = hyps.first() {
                            ...
                            && best.confidence > 0.3
                        {
                            ...
                            let interventions = vec![
                                Intervention::CapabilityDelta { index: cap_idx, delta },
                            ];
    ...
    pub fn suggestions(&self) -> Vec<InterventionOutcome> {
        self.predictor.as_ref().and_then(|p| {
            self.last_report.as_ref().map(|r| p.generate_hypotheses(r))
        }).unwrap_or_default()
    }
