...
pub enum SyncMessage {
    ...
    FileContent { relative_path: String, size: u64, modified: i64, checksum: String },
    /// Push file content (sender sends raw bytes after this)
    PutFile { relative_path: String, size: u64, modified: i64, checksum: String },
    /// Acknowledge receipt
    Ack { message: String },
    /// Error
...
impl SyncPeer {
    pub fn new(id: String, name: String, host: String, tcp_port: u16) -> Self {
        Self { id, name, host, tcp_port, last_seen: 0, is_connected: false }
    }
}
...
impl FileIndex {
    pub fn empty(root: String) -> Self {
        Self { root, files: Vec::new(), total_size: 0, file_count: 0 }
    }
}
...
impl SyncDiff {
    ...
    pub fn compute(local: &FileIndex, remote: &FileIndex) -> Self {
        let local_map: HashMap<&str, &FileEntry> = local.files.iter().map(|f| (f.relative_path.as_str(), f)).collect();
        let remote_map: HashMap<&str, &FileEntry> = remote.files.iter().map(|f| (f.relative_path.as_str(), f)).collect();
        ...
        for (path, remote_file) in &remote_map {
            match local_map.get(path) {
                Some(local_file) => {
                    if remote_file.modified > local_file.modified && remote_file.checksum != local_file.checksum {
        ...
        for (path, local_file) in &local_map {
            match remote_map.get(path) {
                Some(remote_file) => {
                    if local_file.modified > remote_file.modified && local_file.checksum != remote_file.checksum {
        ...
        let total_bytes: u64 = to_send.iter().chain(to_receive.iter()).map(|f| f.size).sum();

        Self { to_send, to_receive, to_delete_local, to_delete_remote, total_bytes }
    }
}
