...
pub fn scan_directory(root: &Path, include_patterns: &[String], exclude_patterns: &[String]) -> Result<FileIndex, String> {
    if !root.is_dir() {
        return Err(format!("Not a directory: {}", root.display()));
    }

    let canon_root = root.canonicalize().map_err(|e| format!("Canonicalize: {}", e))?;
    let mut files = Vec::new();
    let mut total_size = 0u64;

    visit_dirs(&canon_root, &canon_root, include_patterns, exclude_patterns, &mut files, &mut total_size)?;
...
fn visit_dirs(
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            visit_dirs(root, &path, include_patterns, exclude_patterns, files, total_size)?;
            ...
            let metadata = fs::metadata(&path).map_err(|e| format!("Metadata {}: {}", relative, e))?;
            ...
            files.push(FileEntry::new(PathBuf::from(&relative), size, modified, checksum));
            *total_size += size;
        }
    }
    Ok(())
