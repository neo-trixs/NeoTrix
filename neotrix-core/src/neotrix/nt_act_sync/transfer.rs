...
pub struct SyncServer {
    ...
    port: u16,
    local_index_fn: Box<dyn Fn(&str) -> Option<FileIndex> + Send + Sync>,
}
...
impl SyncServer {
    ...
    where
        F: Fn(&str) -> Option<FileIndex> + Send + Sync + 'static,
    {
                ...
                match std::fs::write(&tmp, &data) {
                    Ok(_) => {
                        ...
                        let _ = filetime::set_file_mtime(
                            target,
                            filetime::FileTime::from_system_time(ftime),
                        );
                        write_message(
                            stream,
                            &SyncMessage::Ack {
                                message: format!("received: {}", relative_path),
                            },
                        )
                    }
                    Err(e) => write_message(
...
impl SyncClient {
    ...
    pub fn request_index(host: &str, port: u16, path: &str) -> Result<FileIndex, String> {
        ...
        write_message(
            &mut stream,
            &SyncMessage::IndexRequest { path: path.into() },
        );
    ...
    pub fn pull_file(
        ...
        match read_message(&mut stream) {
            Some(SyncMessage::FileContent {
                ...
                let ftime = UNIX_EPOCH + Duration::from_secs(modified as u64);
                let _ =
                    filetime::set_file_mtime(&target, filetime::FileTime::from_system_time(ftime));
                Ok(())
            }
            Some(SyncMessage::Error { message }) => Err(message),
            _ => Err("unexpected response".into()),
...
mod tests {
    ...
    fn test_sync_message_serde() {
        let msg = SyncMessage::IndexRequest {
            path: "/tmp".into(),
        };
