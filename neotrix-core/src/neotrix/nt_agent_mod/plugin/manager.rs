...
mod tests {
    ...
    fn setup_test_plugin(dir: &Path, name: &str) {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&manifest).expect("failed to serialize plugin manifest"),
        )
    ...
    fn test_scan_discovers_plugins() {
        ...
        let mut mgr = PluginManager::new().with_scan_dir(dir.clone());
    ...
    fn test_enable_disable() {
        ...
        mgr.plugins
            .push(Plugin::new(manifest, PathBuf::from("/tmp/test")));

        assert_eq!(mgr.active_plugins().len(), 1);
        mgr.disable("test")
            .expect("disable should succeed for active plugin");
        assert_eq!(mgr.active_plugins().len(), 0);
        mgr.enable("test")
            .expect("enable should succeed for disabled plugin");
        assert_eq!(mgr.active_plugins().len(), 1);
    }
    ...
    fn test_plugin_count() {
        ...
        mgr.plugins
            .push(Plugin::new(manifest, PathBuf::from("/tmp/p1")));
        assert_eq!(mgr.len(), 1);
    }
}
