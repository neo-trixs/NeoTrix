use crate::neotrix::nt_core_kernel::ReasoningKernel;
...
impl AutonomousAgent {
    pub fn new(stage: usize) -> Self {
        Self {
            kernel: ReasoningKernel::new(stage.min(18)),
            nt_world_browse: BrowserCircuit::new(),
        }
    }

    pub fn research(&mut self, query: &str) -> Result<String, String> {
        let query_vec =
            crate::neotrix::nt_io_standalone::text_to_vector(query, self.kernel.state.len());
        ...
        for (i, f) in findings.iter().enumerate() {
            ctx.insert(
                format!("src_{}", i),
                crate::neotrix::nt_io_standalone::text_to_vector(f, self.kernel.state.len()),
            );
        }
        let out = self.kernel.reason(&query_vec, Some(ctx));
        let e: f64 = out.state_delta.iter().map(|x| x.abs()).sum::<f64>()
            / out.state_delta.len().max(1) as f64;
