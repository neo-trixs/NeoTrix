...
impl SocialMediaPlatform for InstagramConnector {
    ...
    async fn search(
        ...
        let fallback_resp: IgMediaResponse = self
            .api_get(&fallback_url, &token, &fallback_params)
            .await?;
    ...
    async fn trending(&self, max_results: u32) -> Result<Vec<VideoInfo>, PlatformError> {
        ...
        let videos: Vec<VideoInfo> = resp.data.into_iter().map(Self::into_video_info).collect();
        Ok(videos)
    }
