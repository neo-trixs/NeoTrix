...
impl TwitterConnector {
    ...
    fn build_video_info(
        ...
        VideoInfo {
            ...
            comment_count: tweet.public_metrics.as_ref().and_then(|m| m.reply_count),
            author,
            published_at: tweet
                .created_at
                .as_deref()
    ...
    async fn call_api<T: for<'de> Deserialize<'de>>(&self, url: Url) -> Result<T, PlatformError> {
        ...
        serde_json::from_str(&body).map_err(|e| PlatformError::ParseError(e.to_string()))
    }
}
...
impl SocialMediaPlatform for TwitterConnector {
    ...
    async fn get_home_timeline(
        &self,
        max_results: u32,
    ) -> Result<Vec<crate::neotrix::nt_world_social::connector::TimelineTweet>, PlatformError> {
        let clamped = max_results.min(100);
        let url = Url::parse_with_params(
            "https://api.twitter.com/2/tweets/search/recent",
            &[
            ...
            .map(|t| {
                let username = users.and_then(|u| {
                    t.author_id.as_ref().and_then(|aid| {
                        u.iter()
                            .find(|u2| u2.id == *aid)
                            .map(|u2| u2.username.clone())
                    })
                });
                crate::neotrix::nt_world_social::connector::TimelineTweet {
                    id: t.id.clone(),
                    text: t.text.clone(),
    ...
    async fn get_user_tweets(
        ...
        let tweets = resp
            ...
            .iter()
            .map(
                |t| crate::neotrix::nt_world_social::connector::TimelineTweet {
                    id: t.id.clone(),
                    text: t.text.clone(),
                    created_at: t.created_at.clone(),
                    author_id: t.author_id.clone(),
