...
impl StateGraph {
    ...
    pub fn topological_sort(&self) -> Result<Vec<String>, String> {
        ...
        if sorted.len() != self.nodes.len() {
            let missing: Vec<String> = self.nodes.keys()
                .filter(|id| !sorted.contains(id)).cloned()
                .collect();
    ...
    pub fn ready_nodes(&self) -> Vec<&ArtifactNode> {
        ...
        for node in self.nodes.values() {
            ...
            let all_deps_done = self.edges.iter()
                .filter(|e| e.to == node.id)
                .all(|e| self.nodes.get(&e.from)
    ...
    pub fn mark_done(&mut self, id: &str) -> Result<(), String> {
        ...
        let to_update: Vec<String> = self.edges.iter()
            .filter(|e| e.from == id)
            .map(|e| e.to.clone())
            .collect();

        for dep_id in &to_update {
            let from_ids: Vec<String> = self.edges.iter()
                .filter(|e| e.to == *dep_id)
                .map(|e| e.from.clone())
                .collect();
            let all_deps_done = from_ids.iter()
                .all(|fid| self.nodes.get(fid)
                    .map(|n| n.state == ArtifactState::Done)
                    .unwrap_or(false));
    ...
    pub fn build_plan(&mut self, goal: &str, task_count: usize) {
        ...
        self.add_edge(&format!("{}.proposal", goal_slug), &format!("{}.spec", goal_slug));
    ...
    pub fn build_pm_plan(&mut self, goal: &str, _task_count: usize) {
        let research = ArtifactNode::new("research", ArtifactType::Prd, &format!("Research: {}", goal));
        let draft = ArtifactNode::new("draft", ArtifactType::Prd, &format!("Draft: {}", goal));
        let review = ArtifactNode::new("review", ArtifactType::Review, &format!("Review: {}", goal));
    ...
    pub fn summary(&self) -> String {
        ...
        let done = self.nodes.values().filter(|n| n.state == ArtifactState::Done).count();
        let ready = self.nodes.values().filter(|n| n.state == ArtifactState::Ready).count();
        let in_progress = self.nodes.values().filter(|n| n.state == ArtifactState::InProgress).count();
        format!("DAG: {}/{} done, {} ready, {} in-progress", done, total, ready, in_progress)
    }
}
...
mod tests {
    ...
    fn test_mark_done_unlocks_dependents() {
        ...
        assert_eq!(g.node("a").expect("node 'a' exists").state, ArtifactState::Done);
        assert_eq!(g.node("b").expect("node 'b' exists").state, ArtifactState::Ready);

        g.mark_done("b").expect("mark_done should succeed");
        assert_eq!(g.node("c").expect("node 'c' exists").state, ArtifactState::Ready);
    }
    ...
    fn test_pm_artifacts_mark_done() {
        ...
        graph.mark_done("research").expect("mark_done should succeed");
        assert_eq!(graph.node("draft").expect("node 'draft' exists").state, ArtifactState::Ready);
        graph.mark_done("draft").expect("mark_done should succeed");
        assert_eq!(graph.node("review").expect("node 'review' exists").state, ArtifactState::Ready);
        graph.mark_done("review").expect("mark_done should succeed");
        assert_eq!(graph.node("refine").expect("node 'refine' exists").state, ArtifactState::Ready);
    }
}
