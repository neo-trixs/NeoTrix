...
impl<T: ConfigState> StateManager<T> {
    ...
    pub fn push(&mut self, new_state: T) {
        ...
        self.history.push((self.current_id.clone(), self.current.clone()));
...
mod tests {
    ...
    fn test_state_serialize() {
        ...
        let deserialized: TestState = serde_json::from_str(&json).expect("state should deserialize");
    ...
    fn test_state_manager_history() {
        ...
        assert_eq!(mgr.at_index(0).expect("state at index 0 should exist").value, 0);
        assert_eq!(mgr.at_index(1).expect("state at index 1 should exist").value, 10);
        assert_eq!(mgr.at_index(2).expect("state at index 2 should exist").value, 20);
