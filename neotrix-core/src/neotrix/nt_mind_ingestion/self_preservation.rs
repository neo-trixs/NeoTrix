...
impl SelfPreservation {
    ...
    pub fn protect(&self, usage: &ResourceUsage, memory_limit_mb: u64) -> Option<String> {
        ...
        if usage.memory_mb > memory_limit_mb {
            return Some(format!("memory {}MB exceeds limit {}MB", usage.memory_mb, memory_limit_mb));
        }
...
pub fn safe_recovery(reason: &str) -> String {
    format!("[recovery] degraded after: {}. preserving core state.", reason)
}
...
mod tests {
    ...
    fn test_resource_guard_disabled_by_default() {
        ...
        let usage = ResourceUsage { memory_mb: 99999, stage_count: 0, pipeline_depth: 0, cpu_seconds: 0.0 };
    ...
    fn test_resource_guard_trigger_memory() {
        ...
        let usage = ResourceUsage { memory_mb: 2000, stage_count: 5, pipeline_depth: 3, cpu_seconds: 10.0 };
    ...
    fn test_resource_guard_trigger_stage_count() {
        let mut sp = SelfPreservation::new(5);
        sp.enable_resource_guard();
        let usage = ResourceUsage { memory_mb: 50, stage_count: 150, pipeline_depth: 10, cpu_seconds: 5.0 };
    ...
    fn test_resource_guard_passes_clean() {
        let mut sp = SelfPreservation::new(5);
        sp.enable_resource_guard();
        let usage = ResourceUsage { memory_mb: 50, stage_count: 10, pipeline_depth: 3, cpu_seconds: 1.0 };
