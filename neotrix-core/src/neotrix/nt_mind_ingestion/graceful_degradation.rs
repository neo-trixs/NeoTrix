...
pub fn degrade_on_error(
    stage_name: &str,
    result: Result<(), crate::neotrix::nt_core_error::NeoTrixError>,
) -> Result<(), crate::neotrix::nt_core_error::NeoTrixError> {
    match result {
        Ok(()) => Ok(()),
        Err(e) => {
            log::warn!(
                "[degradation] stage '{}' failed, degrading: {}",
                stage_name,
                e
            );
            Ok(())
...
mod tests {
    ...
    fn test_all_available_full() {
        ...
        assert_eq!(
            DegradationLevel::from_capabilities(&status),
