...
pub fn classify_pattern(diag: &MetaDiagnostics) -> ImprovementPattern {
    if diag.duplicate_rate > 0.3 {
        ImprovementPattern::HighDuplicates
    } else if diag.throughput_vsa_per_ms < 10.0 {
        ImprovementPattern::LowActivation
    } else if diag.keep_rate < 0.5 {
        ImprovementPattern::LowKeepRate
    } else {
        ImprovementPattern::Healthy
    }
}
...
impl MetaAgent {
    ...
    pub fn log_self_edit(
        &mut self,
        edit_type: MetaSelfEditType,
        description: String,
        before_value: String,
        after_value: String,
    ) -> &MetaSelfEdit {
    ...
    pub fn observe_and_act(
    ...
    ) -> (MetaAction, Option<MetaSelfEditType>) {
        ...
        if let Some(ref mut orch) = self.orchestrator {
            ...
            let tasks = vec![
                agent_orchestrator::AgentTask {
                    ...
                    priority: if diag.throughput_vsa_per_ms < 10.0 {
                        2
                    } else {
                        1
                    },
            ...
            for task in tasks {
                if let Err(e) = orch.dispatch(task) {
                    log::error!("[meta] dispatch task failed: {e}");
                }
            }
            let _ = orch.tick();
        }
        ...
        let action = match pattern {
            ...
            ImprovementPattern::LowKeepRate => {
                if !self
                    .created_stages
                    .contains(&"meta_quality_gate".to_string())
                {
                    MetaAction::CreateStage {
...
impl crate::neotrix::nt_mind::self_iterating::pipeline::BrainStage for DynamicStage {
    ...
    fn process(
        &self,
        brain: &mut crate::neotrix::nt_mind::self_iterating::SelfIteratingBrain,
    ) -> Result<
        crate::neotrix::nt_mind::self_iterating::pipeline::StageDecision,
        crate::neotrix::nt_core_error::NeoTrixError,
    > {
        use crate::core::nt_core_hcube::vsa_quantized::QuantizedVSA;
        match self.name.as_str() {
            "meta_dedup" => {
                let before = brain.consciousness_state.consciousness_stream.len();
                ...
                let (important, mut candidates): (Vec<_>, Vec<_>) =
                    vec.into_iter().partition(|e| {
                        e.retention_score() >= salience_threshold || e.confidence >= 0.9
                    });
                candidates.dedup_by(|a, b| QuantizedVSA::similarity(&a.vector, &b.vector) > 0.92);
                // Reconstruct: important first, then deduped candidates
                brain.consciousness_state.consciousness_stream.clear();
                for entry in important {
                    brain.consciousness_state.consciousness_stream.push(entry);
                }
                for entry in candidates {
                    brain.consciousness_state.consciousness_stream.push(entry);
                }
                let removed = before - brain.consciousness_state.consciousness_stream.len();
                if removed > 0 {
                    log::info!(
                        "[meta_dedup] removed {} duplicates (preserved high-salience)",
                        removed
                    );
                }
            }
            "meta_activation" => {
                let before = brain.consciousness_state.consciousness_stream.len();
                brain.consciousness_state.consciousness_stream.retain(
                    |entry: &crate::core::nt_core_consciousness::VsaTagged| {
            ...
            "meta_quality_gate" => {
                let low_conf = brain
                    ...
                    .count();
                let high_salience_low_conf = brain
                    .consciousness_state
                    .consciousness_stream
                    .iter()
                ...
                if high_salience_low_conf > 0 {
                    ...
                    log::info!(
                        "[meta_quality_gate] boosted confidence of {} high-salience entries",
                        repaired
                    );
                }
            }
...
mod tests {
    ...
    fn sample_diag(iteration: u64, dup_rate: f64, throughput: f64, keep: f64) -> MetaDiagnostics {
        MetaDiagnostics {
            iteration,
            throughput_vsa_per_ms: throughput,
    ...
    fn test_meta_history_capacity() {
        ...
        for i in 0..5 {
            hist.record(sample_diag(i, 0.1, 50.0, 0.9));
        }
    ...
    fn test_observe_and_act_applies_self_mod_when_enabled() {
        ...
        assert_eq!(
            self_edit,
            Some(MetaSelfEditType::ModifyImprovementThreshold)
        );
