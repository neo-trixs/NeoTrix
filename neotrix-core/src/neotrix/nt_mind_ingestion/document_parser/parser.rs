...
fn get_section_mut<'a>(root: &'a mut Vec<Section>, stack: &[usize]) -> &'a mut Section {
    ...
    let last = *stack.last().expect("stack non-empty in get_section_mut");
    &mut current[last]
}
            ...
            match ch {
                ...
                '>' if in_tag => {
                    ...
                    if tag_lower.starts_with("script") || tag_lower.starts_with("/script") {
                        in_script =
                            tag_lower.starts_with("script") && !tag_lower.starts_with("/script");
                    }
                    if tag_lower.starts_with("style") || tag_lower.starts_with("/style") {
                        in_style =
                            tag_lower.starts_with("style") && !tag_lower.starts_with("/style");
                    }
...
mod tests {
    ...
    fn test_markdown_parser_hierarchy() {
        ...
        assert_eq!(
            first.subsections[1].subsections[0].heading.as_deref(),
            Some("B1")
        );
    }
