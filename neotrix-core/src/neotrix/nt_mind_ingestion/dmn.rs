...
pub fn detect_knowledge_gaps(
    ...
    if let Some(router) = attention_router {
        ...
        for r in &reports {
            if r.gap > 0.3 {
                gaps.push(format!(
                    "dim[{}] sparsity={:.2}, gap={:.3}",
                    r.dim_index, r.sparsity_score, r.gap
                ));
            }
        }
    }
...
pub fn build_integration(associations: &[String], gaps: &[String], retrieved: &[String]) -> String {
    use std::fmt::Write;
...
mod tests {
    ...
    fn test_discover_associations_with_content() {
        ...
        stream.push(VsaTagged::new(
            v1.clone(),
            crate::core::nt_core_consciousness::VsaOrigin::Self_(VsaSelfCategory::Thought),
        ));
        stream.push(VsaTagged::new(
            v1,
            crate::core::nt_core_consciousness::VsaOrigin::Self_(VsaSelfCategory::Thought),
        ...
        stream.push(VsaTagged::new(
            v2,
            crate::core::nt_core_consciousness::VsaOrigin::World(
                crate::core::nt_core_consciousness::VsaWorldCategory::UserInput,
            ),
        ));
        let specious = SpeciousPresent::new(5);
        let assoc = discover_associations(&stream, &specious);
        assert!(
            assoc.len() >= 1,
            "should find at least one association between identical vectors"
        );
    }
