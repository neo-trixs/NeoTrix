...
impl WorkerAgent {
    ...
    pub fn assign(&mut self, task: AgentTask) {
        if self.task_queue.len() >= MAX_PENDING_TASKS {
    ...
    pub fn tick(&mut self) -> Option<AgentOrchestratorResult> {
        let task = self.task_queue.pop_front()?;
        self.is_busy = !self.task_queue.is_empty();
        Some(AgentOrchestratorResult {
            task_id: task.id,
...
impl ManagerOrchestrator {
    ...
    pub fn tick(&mut self) -> Vec<AgentOrchestratorResult> {
    ...
    pub fn collect_results(&mut self) -> Vec<AgentOrchestratorResult> {
        self.tick()
    }
    ...
    pub fn workers_by_specialty(&self, specialty: &str) -> Vec<&WorkerAgent> {
        self.workers
            .iter()
            .filter(|w| w.specialty == specialty)
            .collect()
    }
}
