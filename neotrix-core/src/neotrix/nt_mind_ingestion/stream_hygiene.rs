...
use crate::core::nt_core_consciousness::{ConsciousnessStream, VsaOrigin, VsaTagged};
...
pub fn clean_orphan_vsa_vectors(stream: &mut ConsciousnessStream) -> usize {
    ...
    inner.retain(|tagged| matches!(tagged.tag, VsaOrigin::Self_(_) | VsaOrigin::World(_)));
...
pub fn repair_corrupted_tags(stream: &mut ConsciousnessStream) -> usize {
    ...
    stream.clear();
    let before = inner.len();
    inner.retain(|tagged| !tagged.vector.is_empty());
    let removed = before - inner.len();
    for item in inner {
        stream.push(item);
    }
...
pub fn fold_duplicate_vectors(stream: &mut ConsciousnessStream, window: usize) -> usize {
    ...
    for (i, item) in inner.into_iter().enumerate() {
        ...
        let is_dup = result
            .iter()
            .skip(window_start)
            .any(|existing| existing.vector == item.vector && existing.tag == item.tag);
        if !is_dup {
