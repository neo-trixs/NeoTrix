...
impl InformationSource {
    pub fn new(name: &str, source_type: SourceType, url: &str) -> Self {
        Self {
            name: name.to_string(),
            source_type,
            url: url.to_string(),
...
impl SourceRegistry {
    pub fn new() -> Self {
        Self {
            sources: vec![
                InformationSource::new("Etherscan", SourceType::OnChain, "https://etherscan.io"),
                InformationSource::new("DefiLlama", SourceType::Website, "https://defillama.com"),
                InformationSource::new("CoinGecko", SourceType::News, "https://coingecko.com"),
            ],
        }
    }
...
impl OpportunityScanner {
        ...
        self.known_airdrops
            ...
            .map(|a| {
                ...
                Opportunity {
                    ...
                    confidence: 0.6,
                    action: a.claim_url.clone().unwrap_or_else(|| {
                        format!("interact with contract {}", a.contract)
                    }),
    ...
    pub fn on_chain_scan(
        &self,
        address: &str,
        evm_clients: &MultiEvmClient,
    ) -> Vec<Opportunity> {
        let mut results = Vec::new();

        for client in evm_clients.clients().values() {
    ...
    pub fn rank_opportunities(&self, opportunities: &[Opportunity], min_value: f64) -> Vec<Opportunity> {
    ...
    pub fn best_opportunity(&self, opportunities: &[Opportunity], min_value: f64) -> Option<Opportunity> {
        let ranked = self.rank_opportunities(opportunities, min_value);
        ranked.into_iter().next()
    }

    pub fn learn_from_execution(&mut self, success: bool, opp_type: &OpportunityType, value_usd: f64, gas_usd: f64) {
    ...
    pub fn strategy_insights(&self) -> Vec<String> {
        ...
        insights.push(format!("  总收益: ${:.2}, 总Gas: ${:.2}", self.total_value_claimed, self.total_gas_spent));
        insights
    }
...
mod tests {
    ...
    #[test]
    fn test_risk_levels() {
        assert!(OpportunityType::FaucetClaim.risk_level() < OpportunityType::Arbitrage.risk_level());
    }
