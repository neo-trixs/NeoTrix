...
fn test_git_list_branches() {
    ...
    if Command::new("git").args(["init", &dir.to_string_lossy()]).output().map_or(true, |o| !o.status.success()) {
        let _ = fs::remove_dir_all(&dir); return;
    }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "config", "user.email", "test@test.com"]).output().map_or(false, |o| o.status.success()) { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "config", "user.name", "Test"]).output().map_or(false, |o| o.status.success()) { return; }
    if fs::write(dir.join("README.md"), "# test").is_err() { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "add", "."]).output().map_or(false, |o| o.status.success()) { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "commit", "-m", "init"]).output().map_or(false, |o| o.status.success()) { return; }
    let branches = match GitIntegration::list_branches(&dir) {
        Ok(b) => b,
        Err(_) => { let _ = fs::remove_dir_all(&dir); return; }
    };
    assert!(branches.contains(&"main".to_string()));
    if GitIntegration::create_branch(&dir, "feature-x").is_err() { return; }
    let branches = match GitIntegration::list_branches(&dir) {
        Ok(b) => b,
        Err(_) => { let _ = fs::remove_dir_all(&dir); return; }
    };
...
fn test_git_switch_branch() {
    ...
    if Command::new("git").args(["init", &dir.to_string_lossy()]).output().map_or(true, |o| !o.status.success()) { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "config", "user.email", "test@test.com"]).output().map_or(false, |o| o.status.success()) { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "config", "user.name", "Test"]).output().map_or(false, |o| o.status.success()) { return; }
    if fs::write(dir.join("README.md"), "# test").is_err() { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "add", "."]).output().map_or(false, |o| o.status.success()) { return; }
    if !Command::new("git").args(["-C", &dir.to_string_lossy(), "commit", "-m", "init"]).output().map_or(false, |o| o.status.success()) { return; }
    if GitIntegration::create_branch(&dir, "feature-y").is_err() { return; }
    let current = match GitIntegration::current_branch(&dir) {
        Ok(c) => c,
        Err(_) => return,
    };
    assert_eq!(current, "feature-y");
    if GitIntegration::switch_branch(&dir, "main").is_err() { return; }
    let current = match GitIntegration::current_branch(&dir) {
        Ok(c) => c,
        Err(_) => return,
    };
...
#[test]
#[ignore = "requires git working tree"]
