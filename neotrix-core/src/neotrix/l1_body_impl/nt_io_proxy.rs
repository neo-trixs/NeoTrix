...
pub fn resolve_daemon_path() -> Result<PathBuf, String> {
    ...
    Err("neotrix-proxy-daemon not found. Set NEOTRIX_PROXY_DAEMON_PATH or place it next to the neotrix binary.".to_string())
}
        ...
        loop {
            if !self.is_alive() {
                if restart_count >= self.max_restarts {
                    eprintln!(
                        "[proxy-wrapper] ProxyDaemon '{}' exceeded max restarts ({})",
