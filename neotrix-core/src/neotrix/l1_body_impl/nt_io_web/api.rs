...
pub async fn session_import_handler(
    ...
    let mut lock = state.sessions.lock().unwrap_or_else(|e| e.into_inner());
...
pub async fn agent_start_handler(
    ...
    let mut running = state.agent_running.lock().unwrap_or_else(|e| e.into_inner());
    *running = AgentStatus {
        running: true,
...
pub async fn permission_approve_handler(
    ...
    let mut perms = state
        .pending_permissions
        .lock()
        .map_err(|e| json_err(&format!("lock poisoned: {}", e)))?;
    let len = perms.len();
