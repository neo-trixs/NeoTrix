...
fn print_result(result: &super::CloudResult) {
    println!("[sandbox] exit_code={}", result.exit_code);
    println!("[sandbox] execution_time={:?}", result.execution_time);
    ...
    match code {
        ...
        None => {
            ...
            if std::io::stdin().lock().read_to_string(&mut buf).is_ok() && !buf.trim().is_empty() {
                println!("[sandbox] executing {} bytes from stdin...", buf.len());
                match sandbox.run_code(&buf, rt).await {
                    Ok(result) => print_result(&result),
                    Err(e) => eprintln!("[sandbox] error: {}", e),
                }
            } else {
                eprintln!("[sandbox] error: no code provided. Pipe code via stdin or pass as argument.");
            }
        }
    }
}
...
pub fn handle_list() {
    ...
    if sessions.is_empty() {
        println!("[sandbox] no active sessions (sessions are in-memory)");
        return;
    }
    println!("[sandbox] active sessions:");
    for s in sessions {
        println!(
            "  {} | runtime={} | status={:?}",
            s.session_id,
...
pub fn handle_cancel(session_id: &str) {
    ...
    match sandbox.cancel_session(session_id) {
        Ok(()) => println!("[sandbox] session {} cancelled", session_id),
        Err(e) => eprintln!("[sandbox] cancel failed: {}", e),
    }
}
...
pub async fn handle_upload(path: &str, session_id: &str) {
    let data = match tokio::fs::read(path).await {
        Ok(d) => d,
        Err(e) => {
            eprintln!("[sandbox] read file '{}' failed: {}", path, e);
            return;
        }
    };
    ...
    match sandbox.get_session_mut(sid_ref) {
        Some(session) => match session.upload_file(path, data).await {
            Ok(()) => println!("[sandbox] uploaded '{}' to session {}", path, sid_ref),
            Err(e) => eprintln!("[sandbox] upload failed: {}", e),
        },
        None => {
            eprintln!(
