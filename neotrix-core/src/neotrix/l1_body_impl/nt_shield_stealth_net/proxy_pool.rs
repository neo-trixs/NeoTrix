...
use crate::neotrix::l1_body_impl::nt_l1_shared_types::KbProvider;
...
const SUBS_PLAINTEXT_FILE: &str = "subscriptions.json";
...
async fn check_proxy(url: &str) -> bool {
    ...
    let addr = if let Some(at) = stripped.rfind('@') {
        stripped[at + 1..].trim()
    } else {
        stripped.trim()
    };

    // L4 TCP connectivity check
    tokio::time::timeout(
...
impl ProxyPool {
    pub fn new() -> Self {
        ...
        let subs_file = home.join(".neotrix").join(SUBS_PLAINTEXT_FILE);
    ...
    pub fn ensure_subs_file(&self) {
        if !self.subs_file.exists() {
            ...
            let _ = std::fs::write(&self.subs_file, serde_json::to_string_pretty(&defaults).unwrap_or_else(|e| { eprintln!("[proxy-pool] serialize defaults: {e}"); String::new() }));
        }
    }
    ...
    async fn add_batch_with_source(&self, proxies: &[(String, String)], from_subscription: bool) {
        ...
        let mut existing: HashSet<String> = nodes.iter().map(|n| {
            hex::encode(Sha256::digest(n.url.as_bytes()))
        }).collect();
        for (url, tag) in proxies {
            if nodes.len() >= MAX_POOL_SIZE { break; }
            let url_hash = hex::encode(Sha256::digest(url.as_bytes()));
            if existing.contains(&url_hash) { continue; }
            existing.insert(url_hash);
    ...
    /// Start the unified `PoolSupervisor` with auto-replenish.
    /// This replaces `start_health_loop` by also fetching subscriptions
    /// when node count drops below `min_nodes`.
    pub async fn start_supervisor(self: Arc<Self>, health_interval_secs: u64) {
        let supervisor = PoolSupervisor::new("proxy-pool")
            .with_health_interval(health_interval_secs)
            .with_replenish_interval(300);
        let weak = Arc::downgrade(&self);
        let replenish = move || {
            let w = weak.clone();
            Box::pin(async move {
                if let Some(pool) = w.upgrade() {
                    let count = pool.total_count().await;
                    if count < pool.min_nodes as usize {
                        let subs = pool.subscriptions.read().await.clone();
                        for url in &subs {
                            let _ = pool.fetch_subscription(url).await;
                        }
                    }
                }
            }) as futures::future::BoxFuture<'static, ()>
        };
        supervisor.run(self, Some(Box::new(replenish))).await;
    }
    ...
    pub async fn fetch_subscription(&self, sub_url: &str) -> Result<usize, String> {
        ...
        ).await.map_err(|_| "subscription fetch timeout (15s)".to_string())?
          .map_err(|e| e.to_string())?;

        let text = tokio::time::timeout(
            Duration::from_secs(10),
            resp.text(),
        ).await.map_err(|_| "subscription body read timeout".to_string())?
          .map_err(|e| e.to_string())?;

        if text.len() > 10_485_760 {
            return Err("subscription body exceeds 10MB limit".to_string());
        }
...
// ── AnyPool (type-erased registry support) ──

#[async_trait]
impl AnyPool for ProxyPool {
    ...
    async fn feed(&self, resources: &[DiscoveredResource]) -> usize {
        ...
        for r in resources {
            self.add(&r.resource_id, &r.label).await;
        }
        count
    }
}

// ── ResourcePool trait implementation ──

impl ResourcePool for ProxyPool {
    type Resource = ProxyNode;
    ...
    async fn register(&self, resource: Self::Resource) {
        self.add(&resource.url, &resource.tag).await;
    }

    async fn select_for(&self, context: Option<&str>) -> Option<Self::Resource> {
        match context {
            Some(host) => self.select_node_for_host(host).await,
            None => self.select_node().await,
        }
    }

    async fn report_success(&self, resource_id: &str) {
        let mut nodes = self.nodes.write().await;
        if let Some(node) = nodes.iter_mut().find(|n| n.url == resource_id) {
            node.success_count += 1;
            node.last_success = Some(Instant::now());
        }
    }
    ...
    async fn health_check(&self) -> PoolHealthReport {
        let start = Instant::now();
        self.health_check().await;
        let after = self.available_count().await;
        let total = self.total_count().await;
        PoolHealthReport {
            total_checked: total,
            healthy: after,
            failed: total.saturating_sub(after),
    ...
    async fn snapshot(&self) -> PoolSnapshot {
        let total = self.total_count().await;
        let effective = self.available_count().await;
        PoolSnapshot {
            total,
            effective,
            strategy: PoolSelectionStrategy::Fastest,
            healthy_count: effective,
            stale_count: total.saturating_sub(effective),
        }
    }
}
...
mod tests {
    ...
    fn test_strategy_from_name_auto() {
        ...
        assert!(s == NodeSelectionStrategy::Auto);
    }
