...
const DEFAULT_TOR_CONTROL_ADDR: &str = "127.0.0.1:9051";
...
impl Default for TorConfig {
    fn default() -> Self {
        Self {
            ...
            control_addr: DEFAULT_TOR_CONTROL_ADDR.to_string(),
...
impl TorClient {
    ...
    pub async fn socks5_connect(&self, target: &str, port: u16) -> Result<tokio::net::TcpStream, String> {
        use tokio::io::AsyncWriteExt;
        let mut stream = tokio::net::TcpStream::connect(&self.config.socks_addr)
            .await
    ...
    /// 启动 Tor 进程
    pub async fn start(&self) -> L1Result<()> {
        let mut proc = self.process.lock().await;
        if proc.is_some() {
            return Ok(());
        }
        let mut cmd = Command::new("tor");
        cmd.args(["--SocksPort", &self.config.socks_addr,
                    "--ControlPort", &self.config.control_addr])
        ...
        let child = cmd.spawn()
            .map_err(|e| L1Error::Command { cmd: "tor".into(), exit_code: None, stderr: format!("Failed to start Tor: {} — is 'tor' installed?", e) })?;
        *proc = Some(child);
    ...
    /// 停止 Tor 进程
    pub async fn stop(&self) -> L1Result<()> {
        let mut proc = self.process.lock().await;
        if let Some(mut child) = proc.take() {
            child.kill().map_err(|e| L1Error::Io(e.to_string()))?;
    ...
    pub async fn new_circuit(&self) -> L1Result<()> {
    ...
    ///   3. 若 250 OK → 正常
    async fn send_control_command(&self, cmd: &str) -> L1Result<()> {
        ...
        if let Some(safecookie_resp) = auth_result {
            write_half.write_all(safecookie_resp.as_bytes()).await
        ...
        // Step 4: 读命令响应
        if !Self::read_until_ok(&mut reader).await? {
            return Err(L1Error::Command { cmd: cmd.trim().into(), exit_code: None, stderr: "command rejected by Tor".into() });
        }
        Ok(())
    }
    ...
    async fn read_auth_response(
        ...
        loop {
            ...
            if trimmed.starts_with("5") && trimmed.len() >= 4 {
                return Err(L1Error::Network(format!("Tor auth error: {}", trimmed)));
            }
    ...
    fn handle_safecookie(b64_data: &str, config: &TorConfig) -> L1Result<Option<String>> {
        use hmac::{Hmac, Mac};
        use sha2::Sha256;
        ...
        let cookie = std::fs::read(&cookie_path)
            .map_err(|e| format!("Cannot read Tor auth cookie {:?}: {} (try chmod 644)", cookie_path, e))?;
        if cookie.len() != 32 {
            return Err(L1Error::Config(format!("Invalid auth cookie size: {} (expected 32)", cookie.len())));
        }

        // 解码 server nonce (base64)
        let server_nonce = base64::Engine::decode(
            &base64::engine::general_purpose::STANDARD,
            b64_data.trim()
        ).map_err(|e| format!("Invalid AUTHCHALLENGE base64: {}", e))?;
        if server_nonce.len() != 32 {
            return Err(L1Error::Network(format!("Invalid server nonce size: {}", server_nonce.len())));
        }

        // 生成 client nonce
        let client_nonce: [u8; 32] = rand::random();
        let client_b64 = base64::Engine::encode(
            &base64::engine::general_purpose::STANDARD,
            client_nonce
        );
    ...
    async fn read_until_ok(reader: &mut (impl tokio::io::AsyncBufReadExt + Unpin)) -> L1Result<bool> {
        let mut line = String::new();
        loop {
            line.clear();
            let n = reader.read_line(&mut line).await
    ...
    pub async fn health_check(&self) -> bool {
        ...
        let socks_ok = tokio::time::timeout(Duration::from_secs(5), async {
            let mut stream = TcpStream::connect(&self.config.socks_addr).await.ok()?;
            stream.write_all(&[5, 1, 0]).await.ok()?;
            let mut buf = [0u8; 2];
            let _ = stream.read_exact(&mut buf).await.ok()?;
            Some(buf[0] == 5 && (buf[1] == 0 || buf[1] == 2))
        }).await.ok().and_then(|x| x).unwrap_or(false);
        ...
        if socks_ok {
            ...
            self.healthy.store(tcp_ok, Ordering::Relaxed);
            tcp_ok
        }
    }
    ...
    pub async fn health_check_loop(self: Arc<Self>) {
        loop {
            sleep(Duration::from_secs(TOR_HEALTH_CHECK_INTERVAL_SECS)).await;
            ...
            // RotationCoordinator 协调的电路轮转
            {
                ...
                if let Some(ref coord) = *coord {
                    ...
                    if self.healthy.load(Ordering::Relaxed) && self.config.circuit_rotate_interval > 0 {
                        static LAST_ROTATION: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
                        let last = LAST_ROTATION.load(Ordering::Relaxed);
                        ...
                        if now - last >= self.config.circuit_rotate_interval {
                            let _ = self.new_circuit().await;
                            LAST_ROTATION.store(now, Ordering::Relaxed);
                        }
                    }
                }
            }
...
mod tests {
    ...
    fn test_tor_config_defaults() {
        ...
        assert_eq!(cfg.socks_addr, "127.0.0.1:9050");
    ...
    async fn test_health_check_reflects_tor_status() {
        ...
        if status {
            println!("[test] Tor is running on {}", client.socks_addr());
        }
    ...
    fn test_auth_cookie_path_with_data_dir() {
        ...
        assert_eq!(cfg.auth_cookie_path(), PathBuf::from("/tmp/tor-test/control_auth_cookie"));
    }
