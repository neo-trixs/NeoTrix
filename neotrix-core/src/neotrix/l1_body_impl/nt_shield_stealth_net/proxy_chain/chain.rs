...
use super::types::{
    CONNECT_TIMEOUT_SECS, DEFAULT_ROTATION_INTERVAL_SECS, FAILOVER_LATENCY_THRESHOLD_MS,
    FAILOVER_THRESHOLD_SUCCESS_RATE, PROBE_INTERVAL_MS, QUICK_PROBE_TIMEOUT_MS,
    ProxyHealth, ProxyNode, ProxyProtocol, ProxySelectionRule,
};
...
impl DynamicProxyChain {
    ...
    pub async fn update_latency(&self, label: &str, latency_ms: f64) {
        self.latency_cache.write().await.insert(label.to_string(), latency_ms);
    }
            ...
            let idx = match rule {
                ...
                ProxySelectionRule::Weighted => {
                    ...
                    if total_weight <= 0.0 {
                        ...
                        pool.iter().position(|n| { roll -= n.weight; roll <= 0.0 })
    ...
    pub async fn start_rotation_loop(self: Arc<Self>) {
        ...
        loop {
            if let Some(coord) = self.coordinator.read().await.clone() {
                let secs = coord.seconds_until_rotation(RotationDomain::ProxyChain).await;
    ...
    pub async fn describe(&self) -> DynamicChainSummary {
        ...
        for i in 0..self.layers.len() {
            ...
            let label = if idx < layer.len() { layer[idx].label.clone() } else { "none".into() };
            layer_info.push(format!("layer[{}] {} ({}/{} avail)", i, label, idx + 1, pool_size));
        }
...
impl ProxyPool {
    ...
    pub async fn check_node(&self, node: &ProxyNode) -> ProxyHealth {
        self.check_node_inner(node, Duration::from_secs(CONNECT_TIMEOUT_SECS)).await
    }

    pub async fn quick_check_node(&self, node: &ProxyNode) -> ProxyHealth {
        self.check_node_inner(node, Duration::from_millis(QUICK_PROBE_TIMEOUT_MS)).await
    }
    ...
    pub async fn check_all_parallel(&self) -> Vec<ProxyHealth> {
        ...
        let tasks: Vec<_> = all_nodes.iter().map(|node| self.quick_check_node(node)).collect();
        join_all(tasks).await
    }

    pub async fn start_probe_loop(self: Arc<Self>, interval_ms: u64) {
        let actual_interval = if interval_ms == 0 { PROBE_INTERVAL_MS } else { interval_ms };
    ...
    pub async fn auto_failover(&self, results: &[ProxyHealth]) {
        ...
        for result in results {
            if parse_url_to_node(&result.node_label).is_some_and(|parsed| {
                ...
                if result.fail_count > result.success_count && result.success_count + result.fail_count > 2 {
                    failed_nodes += 1;
                }
            }
        }
        ...
        if success_rate < FAILOVER_THRESHOLD_SUCCESS_RATE || avg_latency > FAILOVER_LATENCY_THRESHOLD_MS {
            drop(chains);
