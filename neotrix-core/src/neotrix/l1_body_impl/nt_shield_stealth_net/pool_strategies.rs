...
fn sample_beta(alpha: f64, beta: f64) -> f64 {
    ...
    if ga + gb < 1e-10 { 0.5 } else { ga / (ga + gb) }
}
...
impl Default for StrategyLearner {
    fn default() -> Self {
        Self {
            ...
            alpha: 1.0,
            record_count: 0,
            persist_path: std::env::home_dir().unwrap_or_default().join(".neotrix").join("strategy_q.json"),
        }
    }
}
...
impl StrategyLearner {
    ...
    fn domain_key(host: &str) -> String {
        ...
        let n = parts.len();
        if n < 2 { return parts[0].to_string(); }
        let maybe_2part = format!("{}.{}", parts[n - 2], parts[n - 1]);
        let two_part_tlds = ["co.uk", "com.au", "co.jp", "or.jp", "ne.jp", "ac.uk",
            "gov.uk", "org.uk", "net.uk", "com.br", "org.br"];
    ...
    pub fn record_reward(&mut self, host: &str, strategy: &NodeSelectionStrategy, success: bool) {
        ...
        self.record_count += 1;
        if self.record_count.is_multiple_of(10) {
