...
impl RemoteServer {
    ...
    pub async fn start(self: Arc<Self>) -> L1Result<u16> {
        ...
        let listener = loop {
            match TcpListener::bind(("0.0.0.0", port)).await {
                ...
                Err(e) => return Err(L1Error::Network(format!("RemoteControl bind failed: {}", e))),
            }
        };
    ...
    /// Signal graceful shutdown to the event loop
    ...
    async fn handle_tcp_connection(self: Arc<Self>, mut stream: TcpStream, peer_addr: String) -> L1Result<()> {
        let conn_id = format!(
            "rc-{}-{}",
            peer_addr,
        ...
        let n = timeout(Duration::from_secs(10), stream.read(&mut buf)).await
            .map_err(|_| L1Error::Network("auth timeout".into()))?
