...
// TODO: inject via DI — pass wasmtime::Engine through WasmPluginWrapper constructor
static WASM_ENGINE: LazyLock<wasmtime::Engine> = LazyLock::new(wasmtime::Engine::default);
...
impl WasmPluginWrapper {
    ...
    fn call_export(&self, func_name: &str, arg: &str) -> Result<String, String> {
        ...
        memory.data_mut(&mut store)[ptr as usize..(ptr + input_len) as usize].copy_from_slice(input_bytes);
