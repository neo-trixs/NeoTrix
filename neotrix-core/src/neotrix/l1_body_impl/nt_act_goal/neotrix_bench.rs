...
use crate::neotrix::l1_body_impl::nt_l1_shared_types::ProjectSnapshot;
...
impl NeoTrixBench {
    ...
    pub fn score_snapshot(snapshot: &ProjectSnapshot) -> BenchScore {
        let compile_health = Self::compile_health(snapshot.compile_errors, snapshot.compile_warnings);
        ...
        BenchScore { compile_health, test_coverage, code_hygiene, composite }
    }
    ...
    pub fn trend_report(prev: &BenchResult, curr: &BenchResult) -> Vec<String> {
        ...
        lines.push(format!("综合: {:.1}% {}", curr.score.composite * 100.0, delta(prev.score.composite, curr.score.composite)));
        lines.push(format!("编译健康: {:.1}% {}", curr.score.compile_health * 100.0, delta(prev.score.compile_health, curr.score.compile_health)));
        lines.push(format!("测试覆盖: {:.1}% {}", curr.score.test_coverage * 100.0, delta(prev.score.test_coverage, curr.score.test_coverage)));
        lines.push(format!("代码卫生: {:.1}% {}", curr.score.code_hygiene * 100.0, delta(prev.score.code_hygiene, curr.score.code_hygiene)));
        lines
    }
}
