...
fn extract_url(args: &serde_json::Value) -> Option<String> {
    match args {
        serde_json::Value::String(s) => Some(s.clone()),
        serde_json::Value::Object(obj) => obj.get("url").and_then(|v| v.as_str()).map(|s| s.to_string()),
        _ => None,
    }
}
...
fn domain_from_url(raw: &str) -> Option<String> {
    ...
    url::Url::parse(&url_str).ok()?.host_str().map(|s| s.to_string())
}

fn domain_matches_allowlist(domain: &str, allowlist: &[String]) -> bool {
    allowlist.iter().any(|pattern| tool_match_pattern(pattern, domain))
}
...
fn collect_all_string_args(args: &serde_json::Value) -> Vec<String> {
    match args {
        ...
        serde_json::Value::Object(obj) => obj.values().filter_map(|v| v.as_str()).map(|s| s.to_string()).collect(),
...
const DANGEROUS_TOOLS: &[&str] = &[
    "rm", "dd", "mkfs", "format", "shutdown", "reboot", "poweroff", "halt", "init", "fdisk", "parted",
];

const DANGEROUS_CHAIN_KEYWORDS: &[&str] = &["rm", "dd", "wget", "curl", "shutdown", "reboot", ":(){ :|:& };:"];
                ...
                if let Some(ref s) = cmd {
                    ...
                    if DANGEROUS_TOOLS.contains(&first) {
                        return CheckVerdict::Fail(format!(
                            "[SEC-001] Dangerous command '{}' is blocked", first
                        ));
                    }
                }
                CheckVerdict::Pass
        ...
        self.register(SecurityCheck {
            ...
            check_fn: Box::new(|ctx: &ToolCallContext| {
                ...
                if let Some(ref s) = cmd {
                    ...
                    if lower.contains("mkfs") && (lower.contains("/dev/sd") || lower.contains("/dev/nvme")) {
                        return CheckVerdict::Fail(
        ...
        self.register(SecurityCheck {
            ...
            check_fn: Box::new(|ctx: &ToolCallContext| {
                ...
                if let Some(ref s) = cmd {
                    ...
                    // Pipe to shell
                    let lower = s.to_lowercase();
                    if lower.contains("| sh") || lower.contains("| bash") || lower.contains("| /bin/sh") || lower.contains("| /bin/bash") {
                        return CheckVerdict::Fail(
...
mod tests {
    ...
    fn test_sec_001_denies_dangerous_command() {
        ...
        let results = reg.evaluate("bash", &json!("dd if=/dev/zero of=/tmp/x"), &ToolSource::User);
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-001" && matches!(v, CheckVerdict::Fail(_))
        }));
    }
    ...
    fn test_sec_002_denies_destructive_pattern() {
        ...
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-002" && matches!(v, CheckVerdict::Fail(_))
        }));
    }
    ...
    fn test_sec_005_warns_on_excessive_calls() {
        ...
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-005" && matches!(v, CheckVerdict::Warn(_))
        }));
    }
...
"#;
    ...
    fn test_sec_006_path_traversal() {
        ...
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-006" && matches!(v, CheckVerdict::Fail(_))
        }));
    }
    ...
    fn test_sec_008_command_injection() {
        ...
        let results = reg.evaluate(
            "bash",
            &json!("echo $(cat /etc/passwd)"),
            &ToolSource::User,
        );
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-008" && matches!(v, CheckVerdict::Fail(_))
        }));
    }
    ...
    fn test_sec_010_dangerous_curl() {
        ...
        assert!(results.iter().any(|(id, v)| {
            id == "SEC-010" && matches!(v, CheckVerdict::Fail(_))
        }));
    }
    ...
    fn test_inspect_returns_require_approval_for_warn() {
        ...
        let results = reg.evaluate("bash", &json!("echo x"), &ToolSource::User);
        let warned = results.iter().any(|(_, v)| matches!(v, CheckVerdict::Warn(_)));
        assert!(warned, "expected at least one Warn verdict after 55 calls");
    }
