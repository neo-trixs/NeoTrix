...
mod tests {
    ...
    fn test_new_manager_has_default_policies() {
        ...
        assert!(summary.iter().any(|e| e.action == PermissionAction::FileRead && e.rule == PolicyRule::Allow));
    ...
    fn test_check_denied_action() {
        ...
        assert_eq!(m.check(&req), PermissionDecision::Denied("Blocked by policy".into()));
    }
    ...
    fn test_set_policy_and_verify() {
        ...
        let summary = m.policy_summary();
        let entry = summary.iter().find(|e| e.action == PermissionAction::ModelCall).expect("ModelCall policy should exist after set_policy");
    ...
    fn test_policy_entry_roundtrip() {
        ...
        let deserialized: PolicyEntry = serde_json::from_str(&json).expect("deserialize PolicyEntry");
