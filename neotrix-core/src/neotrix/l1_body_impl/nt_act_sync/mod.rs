...
impl FileSync {
    pub fn new(discovery_port: u16, sync_port: u16, local_root: String) -> Result<Self, String> {
        ...
        let server = match SyncServer::bind(sync_port, {
            ...
            move |_path| {
                let p = Path::new(&root);
                if p.exists() {
                    scan_directory(p, &[], &[".git".into(), "target".into(), "node_modules".into()]).ok()
                } else {
                    None
                }
            }
    ...
    pub fn discover_peers(&mut self, duration_ms: u64) -> Result<Vec<SyncPeer>, String> {
        let agents = self.discovery.discover(duration_ms).map_err(|e| e.to_string())?;
    ...
    pub fn compute_diff(&self, peer_id: &str) -> Result<(FileIndex, FileIndex, SyncDiff), String> {
        let pair = self.pairs.iter().find(|p| p.peer_id == peer_id)
            .ok_or_else(|| format!("pair not found: {}", peer_id))?;

        let local = scan_directory(
            Path::new(&self.local_root),
            &pair.directories.first().map(|d| d.include_patterns.clone()).unwrap_or_default(),
            &pair.directories.first().map(|d| d.exclude_patterns.clone()).unwrap_or_default(),
        )?;

        let remote = SyncClient::request_index(
            &pair.peer_host,
            pair.peer_port,
            &self.local_root,
        )?;
    ...
    pub fn execute_sync(&mut self, peer_id: &str) -> Result<u64, String> {
        ...
        for entry in &diff.to_receive {
            SyncClient::pull_file(&pair.peer_host, pair.peer_port, Path::new(&self.local_root), entry)?;
            total += entry.size;
        }
