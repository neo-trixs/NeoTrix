...
impl TweetStream {
    pub fn new() -> Self {
        Self {
            ...
            seen_texts: Vec::with_capacity(4096),
            absorbed_count: 0,
            total_seen: 0,
        }
    }
    ...
    pub fn is_novel(&self, tweet: &RawTweet) -> bool {
        ...
        let prefix = simple_hash(&tweet.text.chars().take(20).collect::<String>());
        if self.seen_prefixes.contains(&prefix) {
            return false;
        }
        for existing in &self.seen_texts {
            if tweet.text.starts_with(existing) || existing.starts_with(&tweet.text) {
                return false;
            }
        }
        true
    ...
    pub fn mark_seen(&mut self, tweet: &RawTweet) {
        ...
        self.seen_texts.push(tweet.text.clone());
        self.total_seen += 1;
    }
    ...
    pub fn signal_purity(tweet: &RawTweet) -> f64 {
        ...
        if has_url { purity -= 0.1; }                 // 纯链接稀释
        if text.len() < 30 { purity -= 0.2; }
    ...
    pub fn information_gain(tweet: &RawTweet, known_concepts: &[String]) -> f64 {
        ...
        let interaction_boost = ((tweet.likes as f64 * 0.01) + (tweet.retweets as f64 * 0.03)).min(0.3);

        let known_overlap = if known_concepts.is_empty() {
            0.0
        } else {
            let text_lower = text.to_lowercase();
            let matches = known_concepts.iter()
    ...
    /// 计算综合负熵分数
    pub fn score_tweet(&self, tweet: &RawTweet, known_concepts: &[String], curiosity_bonus: f64) -> NegentropyScore {
        let info_gain = Self::information_gain(tweet, known_concepts);
        let purity = Self::signal_purity(tweet);
        let novelty = if self.total_seen == 0 { 1.0 } else { 1.0 - (self.total_seen as f64 / 1000.0).min(0.5) };
    ...
    pub fn process_timeline(
        ...
        // 按负熵降序排列
        scored.sort_by(|a, b| b.negentropy.partial_cmp(&a.negentropy).unwrap_or(std::cmp::Ordering::Equal));
...
mod tests {
    ...
    fn test_dedup_exact() {
        ...
        let t = make_tweet("1", "Hello world this is a test tweet with enough words", 10, 2);
    ...
    fn test_dedup_prefix() {
        let mut stream = TweetStream::new();
        let t1 = make_tweet("1", "Hello world this is a test tweet with enough words for dedup", 10, 2);
        let t2 = make_tweet("2", "Hello world this is a test tweet with enough words but different", 5, 1);
        stream.mark_seen(&t1);
    ...
    fn test_information_gain_known() {
        let t = make_tweet("1", "Rust programming is great for systems programming", 50, 10);
    ...
    fn test_extract_keywords() {
        let kw = TweetStream::extract_keywords("Rust programming language is great for systems programming and network services");
        assert!(kw.contains(&"programming".to_string()));
    }

    #[test]
    fn test_score_to_ingestion_text() {
        let t = make_tweet("1", "Test tweet with enough words for ingestion conversion pipeline testing purposes", 10, 2);
        let stream = TweetStream::new();
