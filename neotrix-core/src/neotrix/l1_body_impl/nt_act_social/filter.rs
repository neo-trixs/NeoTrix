...
impl SelfControlledFilter {
    ...
    pub fn record_keyword_feedback(&self, keyword: &str, positive: bool) {
        let mut scores = self.keyword_scores.lock().unwrap_or_else(|e| e.into_inner());
    ...
    pub fn top_keywords(&self, n: usize) -> Vec<(String, f64)> {
        let scores = self.keyword_scores.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn calc_relevance(&self, moment: &VideoInfo, context: &FilterContext) -> f64 {
        ...
        let kw_scores = self.keyword_scores.lock().unwrap_or_else(|e| e.into_inner());
        let mut total = 0.0;
        let mut matched = 0;
