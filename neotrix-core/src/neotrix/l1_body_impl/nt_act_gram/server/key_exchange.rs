...
pub fn parse_req_dh_params(data: &[u8]) -> Result<ReqDhParams, String> {
    ...
    if cid != REQ_DH_PARAMS {
        return Err(format!("req_DH: expected 0x{REQ_DH_PARAMS:08x}, got 0x{cid:08x}"));
    }
...
pub fn build_server_dh_params_ok(
    nonce: &[u8; 16],
    server_nonce: &[u8; 32],
    g_a: &[u8],
) -> Vec<u8> {
...
pub fn parse_set_client_dh_params(data: &[u8]) -> Result<SetClientDhParams, String> {
    ...
    if cid != SET_CLIENT_DH_PARAMS {
        return Err(format!("set_client_DH: expected 0x{SET_CLIENT_DH_PARAMS:08x}, got 0x{cid:08x}"));
    }
    let nonce = read_nonce(&mut r)?;
    let server_nonce = read_server_nonce(&mut r)?;
...
pub fn extract_g_b_from_client_inner(decrypted: &[u8]) -> Result<Vec<u8>, String> {
    ...
    if cid != 0x6643b654 {
        return Err(format!("expected client_DH_inner_data 0x6643b654, got 0x{cid:08x}"));
    }
...
mod tests {
    ...
    fn test_build_res_pq_contains_constructor() {
        ...
        assert_eq!(r.read_uint32().expect("value should be ok in test"), 0x05162463);
    }
    ...
    fn test_server_dh_params_ok_roundtrip() {
        ...
        assert_eq!(r.read_uint32().expect("value should be ok in test"), 0xd0e8075c);
    }
    ...
    fn test_dh_gen_ok_has_correct_constructor() {
        ...
        let mut r = TlReader::new(result);
        assert_eq!(r.read_uint32().expect("value should be ok in test"), 0x3bcbf734);
    }
