...
use crate::neotrix::nt_act_crypto::{CryptoAgent, OpportunityType, ChainType};
use super::monetize::AiToEarnBridge;
use super::pipeline::EarnPipeline;
use super::publisher::{PublisherRegistry, PublishResult};
use super::tracker::{EarnStats, EarningsRecord, EarnTracker};
use super::video::VideoPipeline;

use super::wealth_model::{WealthModel, CapitalSource};
...
impl EarnAgent {
    pub fn new() -> Self {
        Self {
            ...
            work_dir: "/tmp/neotrix_earn".to_string(),
    ...
    pub fn publishers(&self) -> &PublisherRegistry { &self.publishers }
    pub fn publishers_mut(&mut self) -> &mut PublisherRegistry { &mut self.publishers }
    ...
    pub fn open_arbitrage_position(
        &mut self, instrument: &str, notional: f64, leverage: f64,
    ...
    pub fn run_cycle(&mut self, best_platform: Option<&str>) -> CycleResult {
        ...
        let platform: Option<&str> = best_platform.or_else(|| {
            if bp == "none" { None } else { Some(bp.as_str()) }
        });
                    ...
                    Ok(result) => {
                        for pr in result.platform_results {
                            all_results.push(PublishResult {
                                ...
                                success: pr.success, post_url: pr.post_url, error: pr.error,
                            });
                        }
                    }
                    Err(e) => {
                        all_results.push(PublishResult {
                            platform: "aitoearn".into(), success: false,
        ...
        let (earnings_delta, platform_earnings) = if let Some(ref bridge) = self.aitoearn_bridge {
            if bridge.is_configured() {
                bridge.fetch_real_earnings_reward()
        ...
        for (platform, amount) in &platform_earnings {
            self.tracker.record_earning(EarningsRecord {
                ...
                amount: *amount, currency: "USD".to_string(),
                content_title: plan.title.clone(),
            });
        }
        ...
        if earnings_delta > 0.0 {
            ...
            self.wealth_model.compound_day(0.0, capital_return.max(0.01).min(1.0));
        }
    ...
    fn generate_content(engine: &super::LlmEngine, plan: &ContentPlan) -> Result<String, String> {
        let platform = plan.platforms.first().map(|s| s.as_str()).unwrap_or("generic");
        let brand = "NeoTrix";
        let tagline = "The Self-Improving Reasoning Agent";

        let prompt = format!(
            "You are a content creator for {brand} ({tagline}).\n\
             ...
             - Include concrete architecture details (SEAL loop, E8 engine, HyperCube, GWT)\n\
             - {multi} lines or less for this format\n\
             - Output ONLY the content body, no metadata\n\nContent body:",
            brand = brand, tagline = tagline,
            content_type = plan.content_type, platform = platform,
            title = plan.title,
    ...
    pub fn strategic_report(&self) -> Vec<String> {
        ...
        let best_strats: Vec<String> = self.wealth_model.suggest_optimal_strategy()
            .iter().map(|m| format!("  {:?}", m)).collect();
        ...
        if self.financial_stack.derivative_book.open_positions.is_empty() {
        ...
        report.push(format!("  知识资本化潜力: ${:.0}/年", total_knowledge_capital));
        report.push(format!("  金融抽象倍率: {:.1}x", abstraction_mult));
        report.push(format!("  资产: ${:.2} (自有 ${:.2}, 杠杆 ${:.2})",
        ...
        report.push(format!("  资本/劳动收入比: {:.2}",
            self.wealth_model.compounding_tracker.capital_to_labor_ratio));
        report.push(format!("  总收益: ${:.2}", self.tracker.stats().total_earnings));
        report.push(format!("  信息源组合质量: {:.1}%",
    ...
    pub fn load_state(&mut self, path: &str) -> Result<(), String> {
        ...
        let stats: EarnStats = serde_json::from_str(&json)
            .map_err(|e| format!("Deserialize failed: {}", e))?;
        self.tracker = EarnTracker::from_stats(stats);
        Ok(())
    }

    pub fn earnings_stats(&self) -> &EarnStats { self.tracker.stats() }
    pub fn state(&self) -> &AgentState { &self.state }
    pub fn total_cycles(&self) -> u64 { self.total_cycles }
    pub fn available_platforms(&self) -> Vec<&str> { self.publishers.list() }
    pub fn tracker(&self) -> &EarnTracker { &self.tracker }
    ...
    pub fn init_nt_act_crypto(&mut self) -> Arc<Mutex<CryptoAgent>> {
        ...
        self.nt_act_crypto.clone().unwrap_or_else(|| {
            let c = Arc::new(Mutex::new(CryptoAgent::new()));
            self.nt_act_crypto = Some(c.clone());
            c
        })
    }
    ...
    pub fn crypto_earnings_total(&self) -> f64 {
        self.nt_act_crypto
            .as_ref()
            .map(|c| c.lock().unwrap_or_else(|e| e.into_inner()).collector.total_earnings())
            .unwrap_or(0.0)
    }
    ...
    pub fn record_crypto_earning(
        ...
        if let Some(ref crypto_arc) = self.nt_act_crypto {
            let mut crypto = crypto_arc.lock().unwrap_or_else(|e| e.into_inner());
            crypto.collector.record_simple(chain.clone(), opp_type, token, amount, value_usd, label);
...
mod tests {
    ...
    fn echo_registry() -> PublisherRegistry {
        ...
        reg.register(Box::new(super::super::publisher::CliPublisher::new("twitter", "echo 'mock publish'")));
        reg.register(Box::new(super::super::publisher::CliPublisher::new("github", "echo 'mock publish'")));
        reg
    }
    ...
    fn test_open_arbitrage_position() {
        let mut agent = EarnAgent::new();
        agent.open_arbitrage_position("BTC/USD", 10000.0, 10.0,
            crate::neotrix::nt_act_earn::financial_abstraction::PositionDirection::Long);
        assert_eq!(agent.financial_stack.derivative_book.total_notional, 10000.0);
    }
