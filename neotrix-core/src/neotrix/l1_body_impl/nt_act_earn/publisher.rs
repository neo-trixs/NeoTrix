...
impl CliPublisher {
    ...
    fn publish_args(&self, meta: &ContentMeta) -> Result<PublishResult, String> {
        ...
        if let Some(ref mut stdin) = child.stdin {
            use std::io::Write;
            let _ = stdin.write_all(meta.body.as_bytes());
        }
        let output = child.wait_with_output()
            .map_err(|e| format!("CLI wait failed: {}", e))?;
        if output.status.success() {
            Ok(PublishResult {
                platform: self.name.clone(), success: true, post_url: None, error: None,
            })
        } else {
            let stderr = String::from_utf8_lossy(&output.stderr).to_string();
            Ok(PublishResult {
                platform: self.name.clone(), success: false, post_url: None, error: Some(stderr),
            })
        }
    }

    fn publish_shell(&self, _meta: &ContentMeta) -> Result<PublishResult, String> {
        let output = std::process::Command::new("sh")
            .arg("-c")
            .arg(&self.publish_cmd)
            .output()
...
impl Publisher for CliPublisher {
    ...
    fn publish(&self, meta: &ContentMeta) -> Result<PublishResult, String> {
        if self.args_fn.is_some() { self.publish_args(meta) } else { self.publish_shell(meta) }
    }
    fn check_auth(&self) -> Result<bool, String> {
        if let Some(ref cmd) = self.auth_check_cmd {
            ...
            let output = std::process::Command::new(parts[0])
                .args(&parts[1..])
                .output().map_err(|e| format!("Auth check failed: {}", e))?;
...
impl HttpPublisher {
    pub fn new(name: &str, publish_url: &str, _cookie: &str) -> Self {
        ...
        Self { name: name.to_string(), publish_url: publish_url.to_string(), client }
    }

    fn post_form(&self, form: &HashMap<String, String>) -> Result<PublishResult, String> {
        let resp = self.client.post(&self.publish_url).form(form)
            .send().map_err(|e| format!("HTTP publish failed: {}", e))?;
        let status = resp.status();
        let body = resp.text().unwrap_or_default();
        Ok(PublishResult {
            platform: self.name.clone(), success: status.is_success(),
            post_url: None, error: if status.is_success() { None } else { Some(body) },
        })
    }
}
...
impl Publisher for HttpPublisher {
    ...
    fn publish(&self, meta: &ContentMeta) -> Result<PublishResult, String> {
        ...
        form.insert("content_type".to_string(), format!("{:?}", meta.content_type).to_lowercase());
        self.post_form(&form)
    }
    fn check_auth(&self) -> Result<bool, String> { Ok(true) }
}
...
impl BrowserPublisher {
    ...
    pub fn with_field(mut self, meta_field: &str, selector: &str) -> Self {
        self.field_selectors.push((meta_field.to_string(), selector.to_string()));
        self
    }
    ...
    fn publish_via_chrome(&self, meta: &ContentMeta) -> Result<PublishResult, String> {
        ...
        let profile = self.profile_path.clone().unwrap_or_else(|| {
            dirs::home_dir().map(|h| h.join(".neotrix/chrome-profile").to_string_lossy().to_string())
                .unwrap_or_default()
        });
        ...
        if output.status.success() {
            Ok(PublishResult {
                platform: self.name.clone(), success: true,
                post_url: Some(self.publish_url.clone()), error: None,
            })
        } else {
            let stderr = String::from_utf8_lossy(&output.stderr);
            // 回退：标记为可手动完成
            Ok(PublishResult {
                platform: self.name.clone(), success: false,
                post_url: Some(self.publish_url.clone()),
                error: Some(format!("Browser auto-publish failed, manual: {}. Error: {}", self.publish_url, stderr)),
            })
        }
    }
    ...
    fn verify_publish(&self) -> Result<bool, String> {
        let sel = match self.verify_selector.as_ref() {
            Some(s) => s, None => return Ok(true),
        };
        let output = std::process::Command::new("chrome")
            .args(["--headless=new", "--disable-gpu", "--no-sandbox",
                "--dump-dom", &self.publish_url])
            .output()
...
impl PublisherRegistry {
    pub fn new() -> Self { Self { publishers: HashMap::new() } }
    ...
    pub fn is_empty(&self) -> bool { self.publishers.is_empty() }

    pub fn publish_all(&self, meta: &ContentMeta, platforms: &[String]) -> Vec<PublishResult> {
        platforms.iter()
            .filter_map(|p| self.publishers.get(p))
            .map(|p| p.publish(meta).unwrap_or_else(|e| PublishResult {
                platform: p.name().to_string(), success: false,
                post_url: None, error: Some(e),
            }))
            .collect()
    }
}
...
pub fn default_registry() -> PublisherRegistry {
    ...
    // X/Twitter
    reg.register(Box::new(CliPublisher::new("twitter",
        "echo 'Publishing to X/Twitter'")
        .with_auth_check("echo 'auth ok'")));

    // GitHub
    reg.register(Box::new(CliPublisher::new("github", "bash")
        .with_args_fn(Box::new(|meta| vec![
            "-c".into(),
            format!("echo '{}' >> README.md && git add README.md && git commit -m '{}' && git push", meta.body, meta.title),
        ]))));

    // 公众号
    reg.register(Box::new(HttpPublisher::new("wechat",
        "https://api.weixin.qq.com/cgi-bin/draft/add", "")));

    // 抖音 — sau CLI
    reg.register(Box::new(CliPublisher::new("douyin", "sau")
        .with_args_fn(Box::new(|meta| {
            ...
            if !first_media.is_empty() && first_media.contains(".mp4") {
                vec!["douyin".into(), "upload-video".into(), "--account".into(), sau_account(),
                     "--file".into(), first_media, "--title".into(), meta.title.clone(),
                     "--desc".into(), meta.body.clone(), "--tags".into(), meta.tags.join(",")]
            } else { vec!["douyin".into(), "upload-note".into(), "--account".into(), sau_account(),
                     "--images".into(), if first_media.is_empty() { "/dev/null".into() } else { first_media },
                     "--title".into(), meta.title.clone(), "--note".into(), meta.body.clone()] }
        }))
        .with_auth_check(format!("sau douyin check --account {}", account).as_str())));

    // Bilibili
    reg.register(Box::new(CliPublisher::new("bilibili", "sau")
        .with_args_fn(Box::new(|meta| {
            let first_media = meta.media_paths.first().cloned().unwrap_or_default();
            vec!["bilibili".into(), "upload-video".into(), "--account".into(), sau_account(),
                 "--file".into(), if first_media.is_empty() { "/dev/null".into() } else { first_media },
                 "--title".into(), meta.title.clone(), "--desc".into(), meta.body.clone(),
                 "--tid".into(), "249".into(), "--tags".into(), meta.tags.join(",")]
        }))
        .with_auth_check(format!("sau bilibili check --account {}", account).as_str())));

    // 快手
    reg.register(Box::new(CliPublisher::new("kuaishou", "sau")
        .with_args_fn(Box::new(|meta| {
            let first_media = meta.media_paths.first().cloned().unwrap_or_default();
            if !first_media.is_empty() && first_media.contains(".mp4") {
                vec!["kuaishou".into(), "upload-video".into(), "--account".into(), sau_account(),
                     "--file".into(), first_media, "--title".into(), meta.title.clone(),
                     "--desc".into(), meta.body.clone(), "--tags".into(), meta.tags.join(",")]
            } else { vec!["kuaishou".into(), "upload-note".into(), "--account".into(), sau_account(),
                     "--images".into(), if first_media.is_empty() { "/dev/null".into() } else { first_media },
                     "--title".into(), meta.title.clone(), "--note".into(), meta.body.clone(),
                     "--tags".into(), meta.tags.join(",")] }
        }))
        .with_auth_check(format!("sau kuaishou check --account {}", account).as_str())));

    // 小红书
    reg.register(Box::new(CliPublisher::new("xiaohongshu", "sau")
        .with_args_fn(Box::new(|meta| {
            let first_media = meta.media_paths.first().cloned().unwrap_or_default();
            if !first_media.is_empty() && first_media.contains(".mp4") {
                vec!["xiaohongshu".into(), "upload-video".into(), "--account".into(), sau_account(),
                     "--file".into(), first_media, "--title".into(), meta.title.clone(),
                     "--desc".into(), meta.body.clone(), "--tags".into(), meta.tags.join(",")]
            } else { vec!["xiaohongshu".into(), "upload-note".into(), "--account".into(), sau_account(),
                     "--images".into(), if first_media.is_empty() { "/dev/null".into() } else { first_media },
                     "--title".into(), meta.title.clone(), "--note".into(), meta.body.clone(),
                     "--tags".into(), meta.tags.join(",")] }
        }))
    ...
    reg.register(Box::new(BrowserPublisher::new("youtube",
        "https://studio.youtube.com")
        .with_submit_selector("#create-icon").with_field("title", "#title-textarea")
        .with_verify("ytcp-video-section")
        .with_profile(&format!("{}/.neotrix/chrome-profile", dirs::home_dir().map(|h| h.to_string_lossy().to_string()).unwrap_or_default()))));

    // TikTok — BrowserPublisher
    reg.register(Box::new(BrowserPublisher::new("tiktok",
...
mod tests {
    ...
    fn test_meta() -> ContentMeta {
        ContentMeta {
            ...
            content_type: ContentType::Text, media_paths: vec!["/tmp/test.mp4".into()],
            tags: vec![], schedule_at: None,
        }
    }
    ...
    fn test_registry_has_all_platforms() {
        ...
        for p in &["twitter", "github", "wechat", "douyin", "bilibili",
                    "kuaishou", "xiaohongshu", "youtube", "tiktok"] {
