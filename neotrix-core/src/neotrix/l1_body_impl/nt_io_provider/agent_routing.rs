...
impl FailoverStrategy {
    ...
    pub fn find_failover<'a>(
        ...
        candidates.sort_by(|a, b| {
            ...
            // free-tier 优先 (use stored free_tier_providers set)
            let kind_a = ProviderKind::from_type(&a.provider_type);
            let kind_b = ProviderKind::from_type(&b.provider_type);
            let a_free = if self.free_tier_providers.contains(&kind_a) { 0 } else { 1 };
            let b_free = if self.free_tier_providers.contains(&kind_b) { 0 } else { 1 };
            if a_free != b_free {
