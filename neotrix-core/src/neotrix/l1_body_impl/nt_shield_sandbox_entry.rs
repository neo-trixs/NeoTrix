...
impl Default for Sandbox {
    fn default() -> Self {
        Self {
            ...
            work_dir: "/tmp/neotrix-sandbox".to_string(),
        }
    }
}
...
impl Sandbox {
    ...
    #[cfg(feature = "sandbox")]
    fn run_wasm_module(&self, input: &str) -> L1Result<String> {
        use wasmtime::{Engine, Module, Store, Instance};
        let engine = Engine::default();
        let module_bytes = self.load_wasm_module("sandbox")?;
        let module = Module::new(&engine, &module_bytes)
            .map_err(|e| L1Error::Wasm(format!("WASM compile: {}", e)))?;
        if module.exports().any(|e| e.name() == "main") {
            let mut store: Store<()> = Store::new(&engine, ());
            let instance = Instance::new(&mut store, &module, &[])
                .map_err(|e| L1Error::Wasm(format!("WASM instantiate: {}", e)))?;
            let func = instance.get_typed_func::<(), ()>(&mut store, "main")
                .map_err(|e| L1Error::Wasm(format!("WASM func: {}", e)))?;
            func.call(&mut store, ()).map_err(|e| L1Error::Wasm(format!("WASM exec: {}", e)))?;
        }
        Ok(format!("wasm:{}", input))
    }

    #[cfg(feature = "sandbox")]
    fn load_wasm_module(&self, name: &str) -> L1Result<Vec<u8>> {
        let path = std::path::PathBuf::from(&self.work_dir).join(format!("{}.wasm", name));
        std::fs::read(&path).map_err(|e| L1Error::Wasm(format!("WASM load {}: {}", name, e)))
    }

    /// Pull docker image (async)
    pub async fn pull_image(&self) -> L1Result<String> {
        if self.mode != SandboxMode::Docker {
            return Err(L1Error::from("Not in Docker mode"));
        }
...
impl SandboxPool {
    ...
    pub fn len(&self) -> usize { self.sandboxes.len() }
    pub fn is_empty(&self) -> bool { self.sandboxes.is_empty() }
}
