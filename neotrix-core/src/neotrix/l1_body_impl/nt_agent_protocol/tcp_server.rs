...
impl AgentServer {
    ...
    pub async fn start(self: Arc<Self>) -> L1Result<u16> {
        let mut port = self.port;
        let listener = loop {
            match TcpListener::bind(("0.0.0.0", port)).await {
                Ok(l) => break l,
                Err(_) if port < self.port + 5 => { port += 1; }
                Err(e) => return Err(L1Error::Network(format!("TCP bind failed: {}", e))),
            }
        };
        ...
        tokio::spawn(async move {
            ...
            loop {
                ticker.tick().await;
                let mut sessions = sessions_heartbeat.lock().unwrap_or_else(|e| e.into_inner());
                let now = Instant::now();
                sessions.retain(|_, s| {
...
async fn handle_connection(mut stream: TcpStream, sessions: Arc<Mutex<HashMap<String, AgentSession>>>) -> L1Result<()> {
    let mut buf = vec![0u8; 4096];
    loop {
        let n = stream.read(&mut buf).await
            .map_err(|e| L1Error::Network(format!("read error: {}", e)))?;
        if n == 0 { break; }
        ...
        if trimmed == "HEARTBEAT" {
            stream.write_all(b"HEARTBEAT_ACK\n").await
                .map_err(|e| L1Error::Network(format!("write error: {}", e)))?;
            ...
            sessions.lock().unwrap_or_else(|e| e.into_inner()).insert(session.agent_id.clone(), session);
