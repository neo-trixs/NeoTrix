...
impl SecurityManager {
    ...
    pub fn check_transaction(&self, to: &str, value_eth: f64, data: &[u8]) -> Vec<ScamWarning> {
    ...
    pub fn revoke_approval(&mut self, chain: &ChainType, token: &str, spender: &str) {
        self.approvals
            .retain(|a| !(a.chain == *chain && a.token == token && a.spender == spender));
    }
...
mod tests {
    ...
    fn test_check_large_transfer() {
        let sm = SecurityManager::new();
        let warnings =
            sm.check_transaction("0x1234567890abcdef1234567890abcdef12345678", 100.0, &[]);
    ...
    fn test_simulate_tx() {
        ...
        let sm = SecurityManager::new();
        let sim = sm.simulate_tx(&client, "0xfrom", "0xto", &[0u8; 100], 1000000);
