...
impl Portfolio {
    ...
    pub fn remove_position(&mut self, chain: &ChainType, asset: &str) {
        self.positions.retain(|p| !(p.chain == *chain && p.asset == asset));
    }

    pub fn update_value(&mut self, chain: &ChainType, asset: &str, value_usd: f64) {
        if let Some(pos) = self.positions.iter_mut().find(|p| p.chain == *chain && p.asset == asset) {
    ...
    pub fn summary(&self) -> PortfolioSummary {
        ...
        by_chain_vec.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

        let mut by_asset_vec: Vec<(String, f64)> = by_asset.into_iter().collect();
        by_asset_vec.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

        let n = self.positions.len();
        let div_score = if n <= 1 {
            0.0
        } else {
            let max_single = self.positions.iter().map(|p| p.value_usd).fold(0.0, f64::max);
            if total_value > 0.0 {
...
impl ImpermanentLossCalculator {
    pub fn calculate(
        ...
        ImpermanentLoss {
            pair: format!("{:.1}% / {:.1}%", price_change_a * 100.0, price_change_b * 100.0),
    ...
    pub fn rebalance_suggestion(
        ...
        let target_map: HashMap<&str, f64> = target_allocations.iter()
            .map(|(a, p)| (a.as_str(), *p))
            .collect();
...
mod tests {
    ...
    fn test_diversification_score() {
        ...
        p.add_position(Position {
            chain: ChainType::Ethereum, asset: "ETH".into(),
            amount: 1.0, value_usd: 10000.0, cost_basis_usd: 5000.0,
        });
        let s = p.summary();
        assert!((s.diversification_score - 0.0).abs() < 0.01);

        p.add_position(Position {
            chain: ChainType::Bsc, asset: "BNB".into(),
            amount: 10.0, value_usd: 5000.0, cost_basis_usd: 4000.0,
        });
        let s = p.summary();
    ...
    fn test_rebalance_suggestion() {
        ...
        let suggestions = ImpermanentLossCalculator::rebalance_suggestion(&current, &target, 10000.0);
