...
pub use airdrop::{AirdropCheckResult, AirdropClaimer, AirdropInfo, AirdropRegistry};
pub use bridge::{BridgeAnalyzer, BridgeProtocol, BridgeRegistry, BridgeRoute, BridgeTx};
pub use chain::{ChainConfig, ChainRegistry, ChainType};
pub use collector::{CryptoCollector, CryptoEarnings};
pub use dex::{DexConfig, DexPool, DexProtocol, DexRegistry, DexSwapper, SwapQuote};
pub use evm::{EvmClient, MultiEvmClient};
pub use gas::{GasPriceInfo, GasStats, GasTracker, Speed};
pub use monitor::{
    ChainMonitor, LiquidationEvent, MempoolTx, NewPoolEvent, WhaleCategory, WhaleTx,
};
pub use opportunity::{
    InformationSource, Opportunity, OpportunityScanner, OpportunitySourceType, OpportunityType,
    SourceRegistry,
};
pub use portfolio::{
    ImpermanentLoss, ImpermanentLossCalculator, Portfolio, PortfolioSummary, Position,
};
pub use security::{
    ApprovalEntry, ScamCategory, ScamSeverity, ScamWarning, SecurityManager, TxSimulation,
};
pub use self_evolve::{AdaptiveConfig, BacktestResult, SelfEvolver};
pub use token::{ApprovalInfo, Erc20Abi, TokenBalance, TokenInfo, TokenRegistry};
pub use tx::{SignedTx, Tx1559, TxBuilder, TxLegacy, TxReceipt, TxType};
pub use wallet::{CryptoWallet, WalletManager};
pub use wallet_store::{WalletFile, WalletInfo, WalletStore};
pub use yields::{YieldOpportunity, YieldScanner, YieldType};
...
impl CryptoAgent {
    ...
    pub fn setup_default_wallet(&mut self, label: &str) -> &CryptoWallet {
        ...
        self.wallet_manager
            .active_wallet()
            .expect("add_wallet always sets active=Some(index) and wallet is pushed")
    }
    ...
    /// 执行一次完整的扫描 + 分析 + 学习迭代
    pub fn run_iteration(&mut self) -> CryptoIterationResult {
        ...
        if let Some(wallet) = self.wallet_manager.active_wallet() {
            for airdrop_result in self
                .airdrop_registry
                .eligible_airdrops(&wallet.address, &[])
            {
                if airdrop_result.is_eligible {
                    all.push(Opportunity {
                        ...
                        description: format!(
                            "Est. ${} from {}",
                            airdrop_result.estimated_amount, airdrop_result.airdrop.protocol
                        ),
        ...
        let ranked = self
            .scanner
            .rank_opportunities(&all, self.config.min_opportunity_value_usd);
        ...
        if let Some(ref b) = best {
            ...
            self.self_evolver
                .backtest(&b.opportunity_type.name(), &fake_stats);
        }
        ...
        if self.config.auto_execute {
            if let Some(best_opp) = &best {
                if best_opp.estimated_value_usd > self.config.min_opportunity_value_usd * 10.0 {
                    log::info!(
                        "[CryptoAgent] auto-executing best opportunity: {} on {}",
                        best_opp.title,
                        best_opp.chain
                    );
    ...
    pub fn learn_and_adapt(&mut self) -> Vec<String> {
        ...
        if risk_score > 3.0 {
            insights.push(format!(
                "⚠️ 授权风险评分: {:.1}/10 — 建议检查授权",
                risk_score
            ));
        }

        if let Some(cheapest) = self
            .gas_tracker
            .cheapest_chain(&self.config.preferred_chains)
        {
    ...
    pub fn scan_opportunities(&mut self) -> Vec<Opportunity> {
        ...
        if let Some(wallet) = self.wallet_manager.active_wallet() {
            let chain_opps = self
                .scanner
                .on_chain_scan(&wallet.address, &self.evm_clients);
    ...
    pub fn execute_best_opportunity(&mut self) -> Result<(String, String, String, f64), String> {
        ...
        let best = self
            .scanner
            .best_opportunity(&opportunities, self.config.min_opportunity_value_usd)
        ...
        if gas_price > self.config.max_gas_price_gwei {
            return Err(format!(
                "gas too high on {}: {:.1} > {:.1}",
                chain, gas_price, self.config.max_gas_price_gwei,
            ));
        }
        ...
        self.scanner.learn_from_execution(
            true,
            &best.opportunity_type,
            best.estimated_value_usd,
            best.execution_gas_cost,
        );
        self.self_evolver.backtest(
            &best.opportunity_type.name(),
            &crate::neotrix::nt_act_crypto::opportunity::StrategyStats {
    ...
    pub fn generate_strategic_insights(&self) -> Vec<String> {
        ...
        insights.push(format!(
            "已连接链数量: {}",
            self.chain_registry.connected_count()
        ));
        insights.push(format!("可操作链数量: {}", self.operational_chains().len()));
        insights.push(format!(
            "已发现机会: {}",
            self.scanner.total_opportunities_found()
        ));
        insights.push(format!(
            "已完成收益: ${:.2}",
            self.collector.total_earnings()
        ));
        insights.push(format!(
            "已领取总额: ${:.2}",
            self.scanner.total_value_claimed()
        ));
...
/// 一次迭代的执行结果
#[derive(Clone, Debug)]
pub struct CryptoIterationResult {
    pub iteration: u64,
...
impl CryptoIterationResult {
    pub fn summary(&self) -> Vec<String> {
        vec![
            format!("═══ Iteration #{} ═══", self.iteration),
            format!(
                "扫描发现 {} 个机会 ({} 个值得处理)",
                self.total_opportunities, self.ranked_opportunities
            ),
...
mod tests {
    ...
    fn test_learn_and_adapt() {
        ...
        agent
            .scanner
            .learn_from_execution(true, &OpportunityType::FaucetClaim, 10.0, 0.5);
    ...
    fn test_import_wallet() {
        ...
        assert!(agent.wallet_manager.active_wallet().is_some());
        assert_eq!(
            agent.wallet_manager.active_wallet().unwrap().label,
            "imported"
        );
