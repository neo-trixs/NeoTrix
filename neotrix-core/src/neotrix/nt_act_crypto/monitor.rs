...
impl ChainMonitor {
    ...
    pub fn scan_new_pools(&mut self, _clients: &MultiEvmClient) -> Vec<NewPoolEvent> {
        vec![]
    }

    pub fn scan_whales(&mut self, _clients: &MultiEvmClient, _min_usd: f64) -> Vec<WhaleTx> {
        vec![]
    }

    pub fn scan_liquidations(&mut self, _clients: &MultiEvmClient) -> Vec<LiquidationEvent> {
        vec![]
    }
