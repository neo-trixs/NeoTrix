...
impl YieldScanner {
    ...
    pub fn by_chain(&self, chain: &ChainType) -> Vec<&YieldOpportunity> {
        self.opportunities.iter().filter(|o| o.chain == *chain).collect()
    }

    pub fn by_type(&self, yield_type: &YieldType) -> Vec<&YieldOpportunity> {
        self.opportunities.iter().filter(|o| o.yield_type == *yield_type).collect()
    }

    pub fn best_apy(&self, min_tvl: f64, max_risk: f64) -> Vec<&YieldOpportunity> {
        let mut filtered: Vec<&YieldOpportunity> = self.opportunities.iter()
            .filter(|o| o.tvl_usd >= min_tvl && o.risk_score <= max_risk)
            .collect();
        filtered.sort_by(|a, b| b.apy.partial_cmp(&a.apy).unwrap());
        filtered
    }

    pub fn risk_adjusted_rank(&self) -> Vec<(&YieldOpportunity, f64)> {
        let mut ranked: Vec<(&YieldOpportunity, f64)> = self.opportunities.iter()
            .map(|o| {
    ...
    pub fn by_chain_and_type(&self, chain: &ChainType, yield_type: &YieldType) -> Vec<&YieldOpportunity> {
