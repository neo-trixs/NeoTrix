...
impl BridgeRegistry {
    ...
    pub fn find_routes(
        ...
        self.routes.iter().filter(|r| {
            r.from_chain == *from
                && r.to_chain == *to
                && r.supports_token.iter().any(|t| t.to_uppercase() == token_upper)
        }).collect()
    }

    pub fn find_all_from(&self, from: &ChainType) -> Vec<&BridgeRoute> {
        self.routes.iter().filter(|r| r.from_chain == *from).collect()
    }

    pub fn cheapest_route(&self, from: &ChainType, to: &ChainType, token: &str) -> Option<&BridgeRoute> {
        let mut routes = self.find_routes(from, to, token);
        routes.sort_by(|a, b| a.estimated_fee_usd.partial_cmp(&b.estimated_fee_usd).unwrap());
...
mod tests {
    ...
    fn test_find_routes() {
        ...
        assert!(routes.iter().any(|r| r.protocol == BridgeProtocol::Stargate));
    ...
    fn test_estimate_bridge_cost() {
        let cost = BridgeAnalyzer::estimate_bridge_cost(
            &ChainType::Ethereum, 50.0, 100_000, 0.5, 3000.0,
        );
