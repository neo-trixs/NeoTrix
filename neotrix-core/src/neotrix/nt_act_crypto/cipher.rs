...
fn ensure_key_dir() -> Result<PathBuf, String> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("create key dir failed: {}", e))?;
        #[cfg(unix)]
        {
...
fn load_or_create_master_key() -> Result<[u8; 32], String> {
    ...
    if path.exists() {
        let data = std::fs::read(&path).map_err(|e| format!("read master key failed: {}", e))?;
        if data.len() != 32 {
        ...
        std::fs::write(&tmp_path, &key).map_err(|e| format!("write master key failed: {}", e))?;
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        std::fs::rename(&tmp_path, &path)
            .map_err(|e| format!("rename master key failed: {}", e))?;
        Ok(key)
    }
}

fn key_to_cipher(key: &[u8; 32]) -> Aes256Gcm {
    Aes256Gcm::new_from_slice(key).expect("master key is [u8; 32], exact AES-256 key size")
}
...
pub fn decrypt_bytes(encrypted_hex: &str) -> Result<Vec<u8>, String> {
    let combined = hex::decode(encrypted_hex).map_err(|e| format!("decode hex failed: {}", e))?;
    if combined.len() < 12 {
        return Err("ciphertext too short".into());
    }
    let nonce = &combined[..12];
