...
impl CryptoCollector {
    ...
    pub fn earnings_by_chain(&self) -> Vec<(ChainType, f64)> {
        let mut map: std::collections::HashMap<ChainType, f64> = std::collections::HashMap::new();
    ...
    pub fn best_opportunity_type(&self) -> Option<OpportunityType> {
        self.earnings_by_type().first().map(|(t, _)| t.clone())
    }

    pub fn best_chain(&self) -> Option<ChainType> {
        self.earnings_by_chain().first().map(|(c, _)| c.clone())
    }
