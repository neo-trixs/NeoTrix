...
impl WalletManager {
    pub fn new() -> Self {
        Self { wallets: Vec::new(), active: None }
    }
    ...
    pub fn set_active(&mut self, index: usize) -> Result<(), String> {
        if index < self.wallets.len() {
            ...
            Err(format!("wallet index {} out of bounds (max {})", index, self.wallets.len().saturating_sub(1)))
        }
    }
    ...
    pub fn remove_wallet(&mut self, index: usize) -> Option<CryptoWallet> {
        if index < self.wallets.len() {
            ...
            if self.active == Some(index) {
                self.active = if self.wallets.is_empty() { None } else { Some(0) };
            }
            Some(w)
        } else {
            None
