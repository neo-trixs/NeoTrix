...
impl Erc20Abi {
    pub fn balance_of(owner: &str) -> Vec<u8> {
        ...
        data.extend_from_slice(
            &hex::decode("70a08231").expect("compile-time hex literal for balanceOf selector"),
        );
    ...
    pub fn allowance(owner: &str, spender: &str) -> Vec<u8> {
        ...
        let mut data = Vec::with_capacity(68);
        data.extend_from_slice(
            &hex::decode("dd62ed3e").expect("compile-time hex literal for allowance selector"),
        );
        data.extend_from_slice(&owner_bytes);
        data.extend_from_slice(&spender_bytes);
        data
...
mod tests {
    ...
    fn test_transfer_encoding() {
        let data = Erc20Abi::transfer("0x1234567890abcdef1234567890abcdef12345678", 1000000);
    ...
    fn test_approve_encoding() {
        let data = Erc20Abi::approve("0x1234567890abcdef1234567890abcdef12345678", u128::MAX);
        assert_eq!(data.len(), 68);
