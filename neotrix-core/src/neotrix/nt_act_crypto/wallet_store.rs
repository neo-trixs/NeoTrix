...
impl WalletStore {
    ...
    fn wallet_dir() -> PathBuf {
        dirs::home_dir()
            ...
            .unwrap_or_else(|| PathBuf::from("/tmp/neotrix_wallets"))
    }
    ...
    pub fn load_wallet(&self, label: &str) -> Result<CryptoWallet, String> {
        ...
        let pk = if file.encrypted_key.starts_with("0x") && file.encrypted_key.len() > 42 {
            cipher::decrypt_from_hex(&file.encrypted_key)
                ...
                if let Ok(json) = std::fs::read_to_string(&path) {
                    if let Ok(file) = serde_json::from_str::<WalletFile>(&json) {
                        wallets.push(WalletInfo {
                            ...
                            encrypted: file.encrypted_key.starts_with("0x") && file.encrypted_key.len() > 42,
...
mod tests {
    ...
    fn test_load_all() {
        let (store, _tmp) = test_store();
        store.save_wallet(&CryptoWallet::generate_evm("w1")).unwrap();
        store.save_wallet(&CryptoWallet::generate_evm("w2")).unwrap();
