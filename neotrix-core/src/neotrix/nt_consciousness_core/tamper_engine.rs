...
pub struct TamperEngine {
    /// 规则列表
    pub rules: Vec<TamperRule>,
    /// 统计
    pub stats: TamperStats,
}
...
impl TamperEngine {
    ...
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
            stats: TamperStats::default(),
        }
    }

    /// 添加规则
    pub fn add_rule(&mut self, rule: TamperRule) {
    ...
    /// 批量处理
    pub fn process_batch(&mut self, inputs: &[String]) -> Vec<(String, bool)> {
        inputs.iter().map(|input| self.process(input)).collect()
    }

    /// 获取统计
    pub fn stats(&self) -> TamperEngineStats {
        TamperEngineStats {
...
/// 篡改引擎统计
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TamperEngineStats {
    pub total_rules: usize,
    pub total_triggers: u32,
    pub top_rules: Vec<(String, u32)>,
}

impl std::fmt::Display for TamperEngineStats {
