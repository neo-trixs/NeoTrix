    ...
    pub fn auto_register_all() -> Vec<McpServerEntry> {
        ...
        discovered
            ...
            .map(|entry| match Self::try_register(&entry) {
                ...
                Err(e) => McpServerEntry {
                    status: McpServerStatus::Failed(e),
                    ..entry
                },
            })
            .collect()
        ...
        if entry.status == McpServerStatus::Verified {
            registry.register(McpServer {
                ...
                tools: vec![McpToolDef {
                    ...
                    description: format!("MCP server: {} — dispatches all tools", entry.name),
...
#[cfg(test)]
mod tests {
    // NOTE: Tests temporarily override PATH via std::env::set_var.
    // All tests are #[test] (sync, separate OS threads) — no concurrent races.
    use super::*;
    use std::os::unix::fs::PermissionsExt;
...
"#;
        ...
        let mut perm = std::fs::metadata(&path)
            .expect("metadata of freshly written mock server should exist")
            .permissions();
        perm.set_mode(0o755);
        std::fs::set_permissions(&path, perm)
            .expect("set_permissions on mock MCP server should succeed");
        path
    }
    ...
    fn create_non_mcp_binary(dir: &std::path::Path, name: &str) -> PathBuf {
        ...
        let mut perm = std::fs::metadata(&path)
            .expect("metadata of freshly written non-MCP binary should exist")
            .permissions();
        perm.set_mode(0o755);
        std::fs::set_permissions(&path, perm)
            .expect("set_permissions on non-MCP binary should succeed");
        path
    }
    ...
    fn test_try_register_verifies_mcp_protocol() {
        ...
        assert_eq!(
            result
                .expect("try_register on mock MCP server should return Ok")
                .status,
    ...
    fn test_try_register_fails_on_no_response() {
        ...
        let mut perm = std::fs::metadata(&path)
            .expect("metadata of silent MCP server should exist")
            .permissions();
        perm.set_mode(0o755);
        std::fs::set_permissions(&path, perm)
            .expect("set_permissions on silent MCP server should succeed");
    ...
    fn test_auto_register_all_returns_entries() {
        ...
        let verified: Vec<_> = entries
            .iter()
            .filter(|e| e.status == McpServerStatus::Verified)
            .collect();
    ...
    fn test_version_extraction() {
        ...
        let mut perm = std::fs::metadata(&path)
            .expect("metadata of versioned mock server should exist")
            .permissions();
        perm.set_mode(0o755);
        std::fs::set_permissions(&path, perm)
            .expect("set_permissions on versioned mock server should succeed");
    ...
    fn test_version_extraction_fails_gracefully() {
        ...
        let mut perm = std::fs::metadata(&path)
            .expect("metadata of no-version mock server should exist")
            .permissions();
        perm.set_mode(0o755);
        std::fs::set_permissions(&path, perm)
            .expect("set_permissions on no-version mock server should succeed");
