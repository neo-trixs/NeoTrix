...
impl SemiFormalTemplate {
    ...
    pub fn add_premise(&mut self, category: PremiseCategory, statement: &str, confidence: f64) {
        self.premises.push(SemiFormalPremise {
            category,
    ...
    pub fn add_trace(&mut self, step: &str, file: &str, start: usize, end: usize, behaviour: &str) {
        self.traces.push(ExecutionTrace {
            ...
            observed_behaviour: behaviour.to_string(),
        });
    }
    ...
    pub fn add_conclusion(
        ...
        self.conclusions.push(FormalConclusion {
            statement: statement.to_string(),
...
fn analyze_file_context(file_path: &str) -> Vec<(String, String)> {
    ...
    if let Ok(content) = std::fs::read_to_string(file_path) {
        ...
        context.push((
            ...
            if has_unsafe { "yes" } else { "no" }.to_string(),
        ));
...
impl AgenticCodeReasoner {
    ...
    pub fn analyze_request(&self, request: &str, file_path: &str) -> ReasoningStep {
        ...
        let description = if file_context.is_empty() {
            format!(
                "Analyze request: {} (file not found: {})",
                request, file_path
            )
        } else {
            let ctx_summary: Vec<String> = file_context
                .iter()
                .map(|(k, v)| format!("{}={}", k, v))
                .collect();
            format!(
    ...
    pub fn plan_implementation(
        ...
        if let ReasoningStep::AnalyzeRequest {
            file_context: ctx, ..
        } = analysis
        {
            for (k, v) in ctx {
        ...
        if lower.contains("struct") || lower.contains("type") {
            ...
            template.add_conclusion(
                "Types must be defined before functions that use them",
                vec!["Structural premise: file exists".into()],
                vec![],
                Verdict::Pass,
            );
        }
        if lower.contains("fn") || lower.contains("function") || lower.contains("method") {
            plan_steps
                .push("Implement functions with type signatures matching existing patterns".into());
        }
        ...
        if lower.contains("error") || lower.contains("result") || lower.contains("safe") {
            ...
            template.add_premise(
                PremiseCategory::Requirement,
        ...
        template.overall_assessment = format!(
            "Plan has {} steps covering {} premises",
            plan_steps.len(),
            template.premises.len()
        );

        ReasoningStep::PlanImplementation {
            steps: plan_steps,
            template,
        }
    }
            ...
            ReasoningStep::PlanImplementation { steps, .. } => {
                ...
                for step in steps {
                    ...
                    if lower.contains("error handling") || lower.contains("unwrap") {
                        generated.push_str(
                            "// Error handling will replace unwrap calls with ? or expect\n",
                        );
    ...
    pub fn review_code(&self, file_path: &str, request: &str) -> ReasoningStep {
        ...
        template.add_premise(
            PremiseCategory::Structural,
            &format!("File has {} lines", lines.len()),
            1.0,
        );
        template.add_premise(
            PremiseCategory::Structural,
            &format!("{} public functions", pub_fn_count),
            0.95,
        );
        template.add_premise(
            PremiseCategory::CallGraph,
            &format!("{} unsafe blocks", unsafe_count),
            0.9,
        );
        ...
        if unsafe_count > 0 && !content.contains("// SAFETY:") {
            ...
            template.add_conclusion(
                ...
                vec![],
                vec!["No SAFETY comment found".into()],
                Verdict::Fail,
            );
        } else {
            template.add_conclusion(
                "Unsafe blocks are documented",
                vec!["SAFETY comment exists".into()],
                vec![],
                Verdict::Pass,
            );
        }
        ...
        if unwrap_count > 0 {
            issues.push(format!(
                "{} .unwrap() calls should use error handling",
                unwrap_count
            ));
            template.add_conclusion(
                "Production code should avoid unwrap",
                vec![],
                vec![format!("{} unwrap calls found", unwrap_count)],
                Verdict::NeedsReview,
            );
        }
        ...
        if !test_exists {
            if request.to_lowercase().contains("test") {
                ...
                template.add_conclusion(
                    "Tests must be added for this change",
                    vec!["Requirement premise".into()],
                    vec!["No #[cfg(test)] found".into()],
                    Verdict::Fail,
                );
            }
        }
        ...
        template.overall_assessment = format!(
            "Review found {} issues: {} premises, {} conclusions",
            issues.len(),
            template.premises.len(),
            template.conclusions.len()
        );

        ReasoningStep::ReviewCode {
            reasoning: template,
            issues,
        }
    }
        ...
        for fb in feedback {
            if fb.contains("no SAFETY") || fb.contains("SAFETY review") {
                if refined.contains("unsafe {") && !refined.contains("// SAFETY:") {
                    refined =
                        refined.replace("unsafe {", "// SAFETY: reviewed for soundness\nunsafe {");
            ...
            if fb.contains("unwrap") {
                if refined.contains(".unwrap(") {
                    ...
                    reasons
                        .push("Replaced unwrap with expect for better error messages".to_string());
                }
            }
    ...
    pub fn verify_with_cargo(&self, _file_path: &str) -> ReasoningStep {
        ...
        match run_cargo_check(project) {
            Ok((true, output)) => ReasoningStep::VerifyCorrectness {
                compile_ok: true,
                output: output.lines().last().unwrap_or("OK").to_string(),
            },
            Ok((false, output)) => ReasoningStep::VerifyCorrectness {
                compile_ok: false,
                output: output.lines().take(20).collect::<Vec<_>>().join("\n"),
            },
            Err(e) => ReasoningStep::VerifyCorrectness {
                compile_ok: false,
                output: e,
            },
        }
    ...
    pub fn run_reasoning_cycle(&mut self, request: &str, file_path: &str) -> Vec<ReasoningStep> {
        ...
        loop {
            ...
            let all_pass = match &review {
                ReasoningStep::ReviewCode { reasoning, .. } => reasoning
                    .conclusions
                    .iter()
                    .all(|c| c.verdict == Verdict::Pass),
                _ => false,
            };
        ...
        if let ReasoningStep::VerifyCorrectness { compile_ok, .. } = &steps
            .last()
            .expect("steps was just pushed with verify on line 507, guaranteed non-empty")
        {
    ...
    pub fn apply_changes(&mut self, code: &str, file_path: &str) -> Result<String, String> {
        let result = self
            .applier
            .safe_write(file_path, code, "agentic_reasoning");
        if result.success {
            Ok(format!(
                "Applied to {} (backup: {})",
                file_path,
    ...
    pub fn generate_steps(_request: &str, _context: &[String]) -> Vec<ReasoningStep> {
        ...
        template.add_premise(
            PremiseCategory::Requirement,
            "Generate steps for code change",
            0.8,
        );
        template.overall_assessment = "Generated reasoning steps".to_string();
        vec![
            ReasoningStep::AnalyzeRequest {
                description: format!("Request: {}", _request),
                file_context: vec![],
            },
            ReasoningStep::PlanImplementation {
                steps: vec!["Implement change".into()],
                template,
            },
            ReasoningStep::WriteCode {
                code: String::new(),
            ...
            ReasoningStep::ReviewCode {
                reasoning: SemiFormalTemplate::new(),
                issues: vec![],
            },
            ReasoningStep::RefineCode {
                changes: vec![],
                reason: String::new(),
            },
            ReasoningStep::VerifyCorrectness {
                compile_ok: false,
                output: String::new(),
            },
        ]
    ...
    pub fn execute_step(&mut self, step: ReasoningStep) -> Result<String, String> {
        ...
        self.history.push(
            self.current_step
                .clone()
                .expect("current_step was just set to Some(step) on line 544"),
        );
...
mod tests {
    ...
    #[test]
    fn test_analyze_request_file_exists() -> Result<(), String> {
        let (_path, path_str) =
            create_temp_file("pub fn foo() {}\nfn bar() {}\n", "test_analyze_req.rs");
        let r = AgenticCodeReasoner::new("/tmp");
        ...
        match step {
            ...
            _ => Err("Expected AnalyzeRequest".into()),
        }
    }

    #[test]
    fn test_analyze_request_file_not_found() -> Result<(), String> {
        let r = AgenticCodeReasoner::new("/tmp");
        let step = r.analyze_request("fix bug", "/nonexistent/path.rs");
        assert_eq!(step.label(), "AnalyzeRequest");
        match step {
            ReasoningStep::AnalyzeRequest { description, .. } => {
    ...
    #[test]
    fn test_plan_implementation() -> Result<(), String> {
        ...
        match plan {
            ReasoningStep::PlanImplementation { steps, template } => {
                assert!(!steps.is_empty());
                assert!(template
                    .premises
                    .iter()
                    .any(|p| p.statement.contains("file_path")));
                Ok(())
            }
            _ => Err("Expected PlanImplementation".into()),
        }
    }

    #[test]
    fn test_write_code_from_plan() -> Result<(), String> {
        let r = AgenticCodeReasoner::new("/tmp");
        let mut template = SemiFormalTemplate::new();
        ...
        match write {
            ReasoningStep::WriteCode { code, target_file } => {
                assert!(code.contains("#[cfg(test)]"));
                assert_eq!(target_file, "test.rs");
                Ok(())
            }
            _ => Err("Expected WriteCode".into()),
        }
    }

    #[test]
    fn test_review_code_detects_issues() -> Result<(), String> {
        let (_path, path_str) = create_temp_file(
            "unsafe { *p = 1; }\nfn foo() { x.unwrap(); }",
            "test_review_issues.rs",
        );
        let r = AgenticCodeReasoner::new("/tmp");
        let review = r.review_code(&path_str, "fix unsafe code");
        match review {
            ReasoningStep::ReviewCode { issues, reasoning } => {
                assert!(!issues.is_empty());
                assert!(!reasoning.premises.is_empty());
                assert!(!reasoning.conclusions.is_empty());
                Ok(())
            }
            _ => Err("Expected ReviewCode".into()),
        }
    }

    #[test]
    fn test_review_code_clean_passes() -> Result<(), String> {
        let (_path, path_str) =
            create_temp_file("pub fn safe() -> i32 { 42 }\n", "test_review_clean.rs");
        let r = AgenticCodeReasoner::new("/tmp");
        let review = r.review_code(&path_str, "simple code");
        match review {
            ReasoningStep::ReviewCode { issues, .. } => {
                assert!(issues.is_empty());
                Ok(())
            }
            _ => Err("Expected ReviewCode".into()),
    ...
    #[test]
    fn test_refine_code_adds_safety() -> Result<(), String> {
        ...
        match refine {
            ...
            _ => Err("Expected RefineCode".into()),
        }
    }

    #[test]
    fn test_refine_code_no_changes_needed() -> Result<(), String> {
        let r = AgenticCodeReasoner::new("/tmp");
        let code = "fn foo() { let x = 42; }";
        let feedback: Vec<String> = vec![];
        let refine = r.refine_code(code, &feedback);
        match refine {
            ReasoningStep::RefineCode { reason, .. } => {
    ...
    fn test_execute_step_tracks_history() {
        let mut r = AgenticCodeReasoner::new("/tmp");
        let step = ReasoningStep::AnalyzeRequest {
    ...
    fn test_full_reasoning_cycle() {
        let (_path, path_str) =
            create_temp_file("fn existing() -> i32 { 42 }", "test_full_cycle.rs");
        let mut r = AgenticCodeReasoner::new("/tmp");
    ...
    fn test_file_context_analysis() {
        ...
        assert!(ctx
            .iter()
            .any(|(k, v)| k == "functions" && v.contains("foo") && v.contains("bar")));
