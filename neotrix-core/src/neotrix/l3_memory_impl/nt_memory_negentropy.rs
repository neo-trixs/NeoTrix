...
/// Trait for phi calculation (replaces L5 IITPhiCalculator dependency)
pub trait PhiCalculator {
    fn compute_phi(&mut self, state: &[f64]) -> PhiReport;
    fn record(&mut self, phi: f64);
}

#[derive(Debug, Clone)]
pub struct PhiReport {
    pub phi: f64,
    pub major_interaction: f64,
}

pub struct NegentropyCalculator {
    pub metric: NegentropyMetric,
    pub phi_calc: Box<dyn PhiCalculator>,
    pub prev_total: f64,
...
impl NegentropyCalculator {
    pub fn new(phi_calc: Box<dyn PhiCalculator>) -> Self {
        Self {
    ...
    pub fn sensor_phi(state: &[f64], calc: &mut dyn PhiCalculator) -> f64 {
    ...
    pub fn sensor_kb_order(kb: Option<&KnowledgeBase>) -> f64 {
        ...
        let ratio: f64 = edges / nodes.max(1.0);
        let optimal: f64 = 4.0;
        let mut ratio_score = 1.0 - (ratio - optimal).abs() / optimal.max(ratio);
        if ratio_score < 0.0 { ratio_score = 0.0; }
        if ratio_score > 1.0 { ratio_score = 1.0; }
