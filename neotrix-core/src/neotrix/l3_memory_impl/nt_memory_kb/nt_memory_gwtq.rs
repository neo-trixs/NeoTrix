...
fn specialist_to_search_query(st: &SpecialistType) -> Vec<&'static str> {
    match st {
        ...
        SpecialistType::EvidenceWeightedHypothesis => vec!["evidence weighting", "hypothesis evaluation", "belief revision", "bayesian update", "source credibility"],
        SpecialistType::Orchestrator => vec!["orchestration", "task routing", "sub-agent coordination", "worktree management"],
    }
}
...
impl super::KnowledgeBase {
    ...
    pub fn query_by_e8_state(&self, state: ReasoningHexagram, limit: usize) -> Result<Vec<SearchResult>, String> {
        ...
        for q in queries {
            let r = search::hybrid_search(&conn, q, limit, bm25.as_ref())
    ...
    pub fn query_broadcast_context(&self, content: &str, limit: usize) -> Result<Vec<SearchResult>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let bm25 = self.bm25.read().ok().and_then(|b| b.clone());
        let results = search::hybrid_search(&conn, content, limit, bm25.as_ref())
            .map_err(|e| format!("Search error: {}", e))?;
        Ok(results)
    }
}
