    ...
    pub fn generate_entrance(
        ...
        match entrance.entrance_type {
            ...
            EntranceType::ScaleIn => {
                filters.push(format!("scale=iw*({}):ih*({})", progress, progress));
            }
        ...
        if let Some(token) = self.registry.resolve(token_name) {
            match &token.value {
                TokenValue::Color { r, g, b, .. } => {
                    ...
                    let ratio = if token_name.contains("sparse") {
                        0.06
                    } else {
                        0.12
                    };
                    let mold = if token_name.contains("sparse") {
                        200
                    } else {
                        80
                    };
    ...
    fn lookup_easing(&self, name: &str) -> String {
        if let Some(token) = self.registry.resolve(name) {
            if let TokenValue::Easing { x1, y1, x2, y2 } = &token.value {
                ...
                return format!(
                    "1.02+0.04*((1-{}))*on/360",
                    sample_bezier_approx(*x1, *y1, *x2, *y2)
                );
            }
        }
