...
impl ConsciousnessGoldStandard {
    ...
    pub fn evaluate(
        &mut self,
        state: &[f64],
        hexagram_states: &[E8HexagramState],
    ) -> GoldStandardReport {
        ...
        let combined_confidence =
            self_consistency_weight(phi, coherence, phi_confidence, coherence_confidence);
...
fn coherence_from_hexagrams(states: &[E8HexagramState]) -> f64 {
    ...
    let variance: f64 = states
        .iter()
        .map(|s| (s.activation - mean_activation).powi(2))
        .sum::<f64>()
        / n;
    ...
    let (sum_cos, sum_sin): (f64, f64) = states
        .iter()
        .map(|s| {
...
fn self_consistency_weight(phi: f64, coherence: f64, phi_conf: f64, coh_conf: f64) -> f64 {
    ...
    let consistency_bonus = if (phi > 0.33) == (coherence > 0.7) {
        0.15
    } else {
        -0.1
    };
...
mod tests {
    ...
    fn test_both_thresholds_met() {
        ...
        assert!(
            report.is_phi_conscious,
            "phi should be > 0.33 with strong state"
        );
        assert!(
            report.is_coherent,
            "coherent hexagrams should give high coherence"
        );
        assert!(
            report.is_conscious_like,
            "both thresholds met → conscious-like"
        );
    ...
    fn test_highly_conscious() {
        ...
        assert!(
            report.phi > 0.5,
            "strong state should exceed high phi threshold"
        );
        assert!(
    ...
    fn test_consecutive_detection_streak() {
        ...
        assert_eq!(
            r4.detection_streak, 0,
            "streak should reset on non-detection"
        );
    }
    ...
    fn test_consciousness_level_classification() {
        ...
        assert!(
            level == ConsciousnessLevel::ConsciousLike
                || level == ConsciousnessLevel::HighlyConscious,
            "should be at least ConsciousLike, got {:?}",
            level
        );
    }
    ...
    fn test_combined_confidence() {
        let mut gs = ConsciousnessGoldStandard::new();
        assert!(
            (gs.combined_confidence() - 0.0).abs() < 1e-10,
            "no history → 0"
        );
        ...
        assert!(
            cc > 0.5,
            "conscious-like should have high confidence, got {}",
            cc
        );
        assert!(cc <= 1.0);
        ...
        assert!(
            cc_low <= 0.5,
            "neither met should have low confidence, got {}",
            cc_low
        );
    }
    ...
    fn test_zero_activation_coherence() {
        ...
        assert!(
            (coh - 0.0).abs() < 1e-10,
            "zero activation → 0 coherence (no oscillation)"
        );
    }
}
