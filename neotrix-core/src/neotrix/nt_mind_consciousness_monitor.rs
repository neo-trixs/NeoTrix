...
const MAX_STRATEGY_HISTORY: usize = 100;
const MAX_TREND_LENGTH: usize = 10_000;
...
impl AwarenessTrends {
    pub fn record_strategy_effectiveness(&mut self, strategy: &str, value: f64) {
        let history = self
            .strategy_effectiveness
            .entry(strategy.to_string())
            .or_default();
        history.push(value);
        if history.len() > MAX_STRATEGY_HISTORY {
            history.remove(0);
        }
    }
}
...
impl ConsciousnessMonitor {
    ...
    pub fn observe(&mut self) {
        ...
        self.current.is_conscious_bound =
            coherence > CONSCIOUS_BOUND_THRESHOLD && phi.phi > CONSCIOUS_BOUND_THRESHOLD;
        ...
        if self.trends.phi_trend.len() > MAX_TREND_LENGTH {
            self.trends.phi_trend.drain(0..MAX_TREND_LENGTH / 2);
        }
        self.trends.coherence_trend.push(coherence);
        if self.trends.coherence_trend.len() > MAX_TREND_LENGTH {
            self.trends.coherence_trend.drain(0..MAX_TREND_LENGTH / 2);
        }
        self.trends.health_trend.push(health);
        if self.trends.health_trend.len() > MAX_TREND_LENGTH {
            self.trends.health_trend.drain(0..MAX_TREND_LENGTH / 2);
        }
    }
    ...
    pub fn observe_conversation_turn(&mut self, topic: &str, response_len: usize, complexity: f64) {
        ...
        let self_assessed_quality =
            0.5 * topic_coherence + 0.3 * user_engagement + 0.2 * (1.0 - topic_drift);
    ...
    pub fn introspect(&self) -> String {
        ...
        if !self.current.active_blind_spots.is_empty() {
            ...
            parts.push(format!(
                "Blind spots ({}): {}",
                self.current.active_blind_spots.len(),
                spots.join("; ")
            ));
        }
    ...
    pub fn get_report(&self) -> AwarenessReport {
        ...
        let blind_spots: Vec<String> = self
            .current
            .active_blind_spots
            .iter()
    ...
    fn estimate_conversation_stage(
        &self,
        turn_count: usize,
        topic_drift: f64,
    ) -> ConversationStage {
...
mod tests {
    ...
    fn test_conversation_turn_tracking() {
        ...
        assert_eq!(
            cm.current.conversation_awareness.stage,
    ...
    fn test_awareness_snapshot_history() {
        ...
        for i in 1..5 {
            assert!(
                cm.awareness_history[i].conversation_turn
                    >= cm.awareness_history[i - 1].conversation_turn
            );
        }
    }
    ...
    fn test_blind_spot_aggregation() {
        let mut cm = ConsciousnessMonitor::new();
        let spots = vec![
            BlindSpotSummary::new(
                "strategy_fixation",
                2,
                "Over-reliance on Direct strategy",
                "Boost alternatives",
            ),
            BlindSpotSummary::new(
                "context_overload",
