    ...
    fn merge_pass(&self, latent: &[f64], tiles: &[AdaptivePartition]) -> Vec<AdaptivePartition> {
        ...
        while i < tiles.len() {
            ...
            while i + 1 < tiles.len() {
                ...
                {
                    let combined_activity = Self::activity_score(&latent[merged.start..next.end]);
...
mod tests {
    ...
    fn test_repartition_trigger_high_error() {
        let state = sample_state();
        let p = ObjectPartition::adaptive(&state, 3);
        assert!(AdaptivePartitioner::repartition_strategy(
            &state, &p, 0.9, 0.5
        ));
    }
    ...
    fn test_adaptive_activity_score_uniform() {
        ...
        assert!(
            (score - 0.0).abs() < 1e-12,
            "Uniform tile should have 0 activity"
        );
    }

    #[test]
    fn test_adaptive_activity_score_high() {
        let tile: Vec<f64> = (0..10)
            .map(|i| if i % 2 == 0 { 10.0 } else { -10.0 })
            .collect();
        let score = AdaptiveObjectPartition::activity_score(&tile);
        assert!(
            score > 0.1,
            "Alternating values should have high activity: {}",
            score
        );
    }
    ...
    fn test_adaptive_activity_score_empty() {
        ...
        assert!(
            (score - 0.0).abs() < 1e-12,
            "Empty tile should have 0 activity"
        );
    }
    ...
    fn test_adaptive_activity_score_single() {
        ...
        assert!(
            (score - 0.0).abs() < 1e-12,
            "Single element should have 0 activity"
        );
    }
    ...
    fn test_adaptive_partition_uniform_latent_merges_to_coarse() {
        ...
        assert!(
            partitions.len() >= 2,
            "Expected ≥2 tiles from uniform 128-dim: got {}",
            partitions.len()
        );
        ...
        for p in partitions {
            assert!(
                p.size >= 32,
                "Uniform latent should produce coarse tiles, got size {}",
                p.size
            );
            assert!(
                (p.activity - 0.0).abs() < 1e-12,
                "Uniform tile should have 0 activity"
            );
        }
        ...
        assert!(
            summary.contains("coarse"),
            "Uniform partition summary should mention coarse tiles: {}",
            summary
        );
    }
    ...
    fn test_adaptive_partition_high_activity_splits_to_fine() {
        ...
        for p in partitions {
            assert!(
                p.size <= 8,
                "High-activity tile should be fine, got size {}",
                p.size
            );
            assert!(
                p.activity > 0.05,
                "High-activity tile should have high activity: {}",
                p.activity
            );
        }
        let summary = aop.partition_summary();
        assert!(
            summary.contains("fine"),
            "High-activity partition should mention fine tiles: {}",
            summary
        );
    }
    ...
    fn test_adaptive_partition_mixed_activity() {
        ...
        assert!(
            low_activity > 0,
            "Mixed latent should have low-activity tiles, found {}",
            low_activity
        );
        assert!(
            high_activity > 0,
            "Mixed latent should have high-activity tiles, found {}",
            high_activity
        );
    }
    ...
    fn test_adaptive_partition_too_small_for_min_tile() {
        ...
        assert_eq!(
            partitions.len(),
            1,
            "Latent < min_tile_size should produce 1 partition"
        );
    ...
    fn test_adaptive_partition_empty_latent() {
        ...
        assert!(
            partitions.is_empty(),
            "Empty latent should produce empty partitions"
        );
    ...
    fn test_adaptive_partition_summary_format() {
        ...
        assert!(
            summary.contains("tiles:"),
            "Summary should contain 'tiles:'"
        );
        assert!(summary.contains("fine"), "Summary should contain 'fine'");
        assert!(
            summary.contains("medium"),
            "Summary should contain 'medium'"
        );
        assert!(
            summary.contains("coarse"),
            "Summary should contain 'coarse'"
        );
        ...
        assert_eq!(
            total,
            aop.num_partitions(),
            "Summary total should match num_partitions"
        );
    }
    ...
    fn test_object_partition_adaptive_v2_convenience() {
        ...
        assert!(
            aop.num_partitions() > 0,
            "adaptive_v2 should produce partitions"
        );
        let summary = aop.partition_summary();
        assert!(
            summary.contains("fine"),
            "High-activity using adaptive_v2 should produce fine tiles"
        );
    }
