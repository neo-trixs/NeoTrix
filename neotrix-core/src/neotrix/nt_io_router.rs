    ...
    pub fn route(&mut self, prompt: &str, context: &TaskContext) -> CognitiveDecision {
        ...
        let (system, latency_budget_ms, reasoning, confidence) = if !self.enabled {
            (
                ...
                5000,
                "cognitive routing disabled, fallback S2".to_string(),
                0.5,
            )
        } else if complexity_score <= self.system1_threshold
            && uncertainty < self.uncertainty_threshold
        {
            (
                CognitiveSystem::System1,
                100,
                "complexity low and uncertainty below threshold → fast S1 intuition".to_string(),
                1.0 - uncertainty,
            )
            ...
            (
                CognitiveSystem::System2,
                5000,
                "complexity within S2 range → full symbolic reasoning".to_string(),
                0.8 - uncertainty * 0.3,
            )
        } else {
            (
                ...
                u64::MAX,
                "high complexity or critical → S3 self-reflection".to_string(),
                0.5 - uncertainty * 0.2,
            )
        };
        ...
        *self
            .stats
            .routes_by_system
            .entry(system.label().to_string())
            .or_insert(0) += 1;
        match system {
            CognitiveSystem::System1 => {
                self.stats.system1_latency_avg_ms =
                    self.stats.system1_latency_avg_ms * 0.9 + elapsed * 0.1;
            }
            CognitiveSystem::System2 => {
                self.stats.system2_latency_avg_ms =
                    self.stats.system2_latency_avg_ms * 0.9 + elapsed * 0.1;
            }
            CognitiveSystem::System3 => {
                self.stats.system3_latency_avg_ms =
                    self.stats.system3_latency_avg_ms * 0.9 + elapsed * 0.1;
            }
        }
...
use crate::core::nt_core_consciousness::{
    AppraisalEngine, AttentionSchemaEngine, BeliefRevisionEngine, CognitiveFlexibility,
    ExecutiveController, IntrinsicMotivationEngine,
};

/// Aggregate state report from one consciousness pipeline step
#[derive(Debug, Clone)]
pub struct PipelineReport {
    pub cognitive_load: f64,
    pub dissonance_count: usize,
    pub attention_shift_rate: f64,
...
impl PipelineReport {
    ...
    /// Effective uncertainty from pipeline's internal state (0..1).
    /// High dissonance + high cognitive load + high emotion → high uncertainty.
    pub fn effective_uncertainty(&self) -> f64 {
        let load_factor = self.cognitive_load * 0.3;
        let dissonance_factor = (self.dissonance_count as f64).min(5.0) / 5.0 * 0.3;
        let emotion_factor = self.emotion_intensity * 0.2;
        let curiosity_factor = self.curiosity_score * 0.2;
        (load_factor + dissonance_factor + emotion_factor + curiosity_factor).min(1.0)
    }

    /// Recommend a cognitive system adjustment based on internal state.
    /// High cognitive load → prefer S1; high dissonance → S3; else S2.
    pub fn recommend_system_shift(&self) -> Option<&'static str> {
        if self.cognitive_load > 0.8 {
            Some("overloaded → prefer System1")
        } else if self.dissonance_count > 3 {
            Some("high dissonance → escalate to System3")
        } else if self.emotion_intensity > 0.7 {
            Some("strong emotion → verify with System2")
        } else {
            None
        }
    }
...
/// ConsciousnessPipeline — wires 6 cognitive modules into one update cycle.
#[derive(Debug)]
pub struct ConsciousnessPipeline {
    pub attention: AttentionSchemaEngine,
    pub executive: ExecutiveController,
    pub flexibility: CognitiveFlexibility,
    pub belief: BeliefRevisionEngine,
    pub motivation: IntrinsicMotivationEngine,
    pub appraisal: AppraisalEngine,
    pub cycle: u64,
    pub last_report: PipelineReport,
    started_at: Instant,
}
...
impl ConsciousnessPipeline {
    pub fn new() -> Self {
        Self {
            attention: AttentionSchemaEngine::new(100),
            executive: ExecutiveController::new(),
            flexibility: CognitiveFlexibility::new(),
            belief: BeliefRevisionEngine::new(0.5),
            motivation: IntrinsicMotivationEngine::new(),
            appraisal: AppraisalEngine::new(),
            cycle: 0,
            last_report: PipelineReport::empty(),
            started_at: Instant::now(),
        }
    }

    /// Run one full consciousness update cycle.
    /// Order: motivation tick → flexibility check → dissonance scan → aggregate.
    pub fn step(&mut self) -> &PipelineReport {
        self.cycle += 1;

        // 1. Motivation time-step decay
        self.motivation.tick();

        // 2. Cognitive flexibility — evaluate if switch is needed
        let _switch = self.flexibility.evaluate_switch(0.0, false);

        // 3. Belief — scan for dissonance
        let dissonance = self.belief.detect_dissonance();

        // 4. Aggregate state
        let elapsed = self.started_at.elapsed().as_secs_f64();
        let shift_rate = if elapsed > 0.0 {
            self.attention.attention_trace.len() as f64 / elapsed
        } else {
            0.0
        };

        let emotion_intensity = self
            .appraisal
            .last_assessment
            .as_ref()
            .map(|a| a.emotion_focused_score)
            .unwrap_or(0.0);

        let curiosity = self.motivation.curiosity_score(0.0);
        let drive = self.motivation.current_drive("idle", 0.0);

        self.last_report = PipelineReport {
            cognitive_load: self.executive.cognitive_load(),
            dissonance_count: dissonance.len(),
            attention_shift_rate: shift_rate,
            motivation_drive: drive,
...
/// Global consciousness pipeline singleton.
pub static CONSCIOUSNESS_PIPELINE: LazyLock<Mutex<ConsciousnessPipeline>> =
    LazyLock::new(|| Mutex::new(ConsciousnessPipeline::new()));

/// Extension: CognitiveRouter with pipeline-aware routing
impl CognitiveRouter {
    /// Route with optional consciousness pipeline state for improved uncertainty
    /// and system selection.
    pub fn route_with_pipeline(
        &mut self,
        prompt: &str,
        context: &TaskContext,
        pipeline: Option<&PipelineReport>,
    ) -> CognitiveDecision {
        let base = self.route(prompt, context);
        let Some(pipe) = pipeline else { return base };

        // Adjust uncertainty from pipeline state
        let pipe_uncertainty = pipe.effective_uncertainty();
        let combined_uncertainty = (base.uncertainty + pipe_uncertainty * 0.3).min(1.0);

        // Adjust system / confidence based on pipeline recommendation
        let (system, latency_budget_ms, reasoning, confidence) = if !self.enabled {
            (
                base.system,
        ...
        } else if let Some(shift) = pipe.recommend_system_shift() {
            match shift {
                s if s.contains("System1") => (
                    CognitiveSystem::System1,
                    100,
                    format!("{}. pipeline: overloaded", base.reasoning),
                    (base.confidence * 0.8).max(0.3),
                ),
                s if s.contains("System3") => (
                    CognitiveSystem::System3,
                    u64::MAX,
                    format!("{}. pipeline: dissonance escalation", base.reasoning),
                    (base.confidence * 0.6).max(0.2),
                ),
                _ => (
                    CognitiveSystem::System2,
                    5000,
                    format!("{}. pipeline: emotional verification", base.reasoning),
                    (base.confidence * 0.9).max(0.3),
                ),
            }
        } else {
            (
        ...
        CognitiveDecision {
            ...
            requires_verification: base.requires_verification
                || pipe.recommend_system_shift().is_some(),
            reasoning,
        }
    }
}
...
mod tests {
    ...
    fn test_system1_simple_task() {
        ...
        assert_eq!(
            decision.system,
            CognitiveSystem::System1,
            "trivial task should route to S1"
        );
        assert_eq!(
            decision.latency_budget_ms, 100,
            "S1 latency budget should be 100ms"
        );
    ...
    fn test_system2_complex_task() {
        ...
        assert_eq!(
            decision.system,
            CognitiveSystem::System2,
            "complex refactor should route to S2"
        );
        assert_eq!(
            decision.latency_budget_ms, 5000,
            "S2 latency budget should be 5000ms"
        );
    }
    ...
    fn test_system3_uncertain_task() {
        ...
        assert_eq!(
            decision.system,
            CognitiveSystem::System3,
            "highly uncertain task should route to S3"
        );
        assert_eq!(
            decision.latency_budget_ms,
            u64::MAX,
            "S3 latency budget should be unlimited"
        );
        assert!(
            decision.uncertainty > 0.5,
            "uncertainty should be high for ambiguous prompt"
        );
    }
    ...
    fn test_uncertainty_estimation() {
        ...
        let ambiguous = router
            .estimate_uncertainty("maybe we could possibly do something but also perhaps not sure");
        assert!(
            ambiguous > clear,
            "ambiguous prompt should have higher uncertainty than clear prompt"
        );
        assert!(clear >= 0.0 && clear <= 1.0, "uncertainty should be 0-1");
        assert!(
            ambiguous >= 0.0 && ambiguous <= 1.0,
            "uncertainty should be 0-1"
        );
    }
    ...
    fn test_requires_verification() {
        ...
        assert!(
            ...
            "security prompts should require verification"
        );
    }
    ...
    fn test_pipeline_new() {
        let pipe = ConsciousnessPipeline::new();
        assert_eq!(pipe.cycle, 0);
        assert!((pipe.last_report.cognitive_load - 0.0).abs() < f64::EPSILON);
        assert!(pipe.last_report.effective_uncertainty() >= 0.0);
    }

    #[test]
    fn test_pipeline_step_increments_cycle() {
        let mut pipe = ConsciousnessPipeline::new();
        pipe.step();
        assert_eq!(pipe.cycle, 1);
        assert_eq!(pipe.last_report.cycle, 1);
    }

    #[test]
    fn test_pipeline_step_produces_report() {
        let mut pipe = ConsciousnessPipeline::new();
        let report = pipe.step();
        assert!(report.cognitive_load >= 0.0);
        assert!(report.motivation_drive >= 0.0);
        assert!(report.curiosity_score >= 0.0);
        assert!(report.effective_uncertainty() >= 0.0 && report.effective_uncertainty() <= 1.0);
    }

    #[test]
    fn test_pipeline_effective_uncertainty_bounds() {
        let report = PipelineReport {
            cognitive_load: 1.0,
            dissonance_count: 5,
            attention_shift_rate: 10.0,
        ...
        let u = report.effective_uncertainty();
        assert!(
            u >= 0.0 && u <= 1.0,
            "effective_uncertainty {u} should be in [0,1]"
        );
        assert!(
            u > 0.5,
            "high load + dissonance + emotion should push uncertainty above 0.5"
        );
    }

    #[test]
    fn test_pipeline_recommend_overloaded() {
        let report = PipelineReport {
            cognitive_load: 0.9,
            dissonance_count: 0,
            attention_shift_rate: 0.0,
        ...
        let rec = report.recommend_system_shift();
        assert!(
            rec.is_some(),
            "high cognitive load should produce recommendation"
        );
        assert!(rec.unwrap().contains("System1"));
    }

    #[test]
    fn test_pipeline_recommend_dissonance() {
        let report = PipelineReport {
            cognitive_load: 0.3,
            dissonance_count: 4,
            attention_shift_rate: 0.0,
        ...
        assert!(
            rec.is_some(),
            "high dissonance should produce recommendation"
        );
        assert!(rec.unwrap().contains("System3"));
    }

    #[test]
    fn test_pipeline_recommend_emotional() {
        let report = PipelineReport {
            cognitive_load: 0.3,
            dissonance_count: 0,
            attention_shift_rate: 0.0,
        ...
        let rec = report.recommend_system_shift();
        assert!(rec.is_some(), "high emotion should produce recommendation");
        assert!(rec.unwrap().contains("System2"));
    }

    #[test]
    fn test_pipeline_no_recommend_when_balanced() {
        let report = PipelineReport {
            cognitive_load: 0.3,
            dissonance_count: 0,
            attention_shift_rate: 0.0,
        ...
        assert!(
            report.recommend_system_shift().is_none(),
            "balanced state should not recommend shift"
        );
    }

    #[test]
    fn test_route_with_pipeline_no_pipeline() {
        let mut router = CognitiveRouter::new();
        let ctx = TaskContext::new("hello");
        // Without pipeline, should behave identically to route()
        let base = router.route("hello", &ctx);
        let piped = router.route_with_pipeline("hello", &ctx, None);
        assert_eq!(base.system, piped.system);
        assert_eq!(base.confidence, piped.confidence);
    }

    #[test]
    fn test_route_with_pipeline_overloaded() {
        let mut router = CognitiveRouter::new();
        let ctx = TaskContext::new("hello");
        let report = PipelineReport {
            cognitive_load: 0.95,
        ...
        let decision = router.route_with_pipeline("hello", &ctx, Some(&report));
        assert_eq!(
            decision.system,
            CognitiveSystem::System1,
            "overloaded pipeline should force S1, got {:?}",
            decision.system
        );
        assert!(
            decision.requires_verification,
            "pipeline override should set requires_verification"
        );
    }

    #[test]
    fn test_route_with_pipeline_high_dissonance() {
        let mut router = CognitiveRouter::new();
        let ctx = TaskContext::new("hello");
        let report = PipelineReport {
            cognitive_load: 0.3,
        ...
        assert_eq!(
            decision.system,
            CognitiveSystem::System3,
            "high dissonance pipeline should force S3, got {:?}",
            decision.system
        );
    }

    #[test]
    fn test_global_pipeline_singleton() {
        let mut pipe = CONSCIOUSNESS_PIPELINE.lock().unwrap();
        assert_eq!(pipe.cycle, 0);
        pipe.step();
        assert_eq!(pipe.cycle, 1);
    }
    ...
    fn test_pipeline_default() {
        let pipe = ConsciousnessPipeline::default();
        assert_eq!(pipe.cycle, 0);
    }
}
