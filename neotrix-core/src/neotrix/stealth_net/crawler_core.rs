...
pub(crate) fn detect_category(title: &str, body: &str, url: &str) -> ContentCategory {
    ...
    } else if combined.contains("forum") || combined.contains("board") || combined.contains("thread")
        || combined.contains("discuss") || combined.contains("comment") {
        ContentCategory::Forum
    } else if combined.contains("news") || combined.contains("article") || combined.contains("blog")
        || combined.contains("press") {
        ContentCategory::News
    } else if combined.contains("wiki") || combined.contains("wikipedia") {
        ContentCategory::Wiki
    } else if combined.contains("search") || combined.contains("crawl") || combined.contains("index") {
        ContentCategory::Search
    } else if url.ends_with(".pdf") || url.ends_with(".zip") || url.ends_with(".tar.gz")
...
impl OnionIndex {
    pub(crate) fn new() -> Self {
        Self { entries: Vec::new(), keyword_map: HashMap::new(), url_map: HashMap::new() }
    }
    ...
    pub(crate) fn search(&self, query: &str, max: usize) -> Vec<SearchResult> {
        ...
        let mut scores: Vec<(f64, usize)> = self.entries.iter().enumerate()
            .map(|(i, e)| {
                let mut score = 0.0;
                let title_lower = e.title.to_lowercase();
                let snippet_lower = e.snippet.to_lowercase();
                for term in &terms {
                    if title_lower.contains(term) { score += 10.0; }
                    if e.keywords.iter().any(|k| k.contains(term)) { score += 5.0; }
                    if snippet_lower.contains(term) { score += 2.0; }
                }
                (score, i)
            })
            .collect();
    ...
    pub(crate) fn load(&mut self, path: &PathBuf) {
        ...
        let entries: Vec<OnionIndexEntry> = data
            ...
            .unwrap_or_default();
        for e in entries {
            if self.url_map.contains_key(&e.url) { continue; }
