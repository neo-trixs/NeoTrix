...
use crate::neotrix::http_factory::H2SettingsProfile;
...
impl Browser {
    ...
    pub fn compatible_platforms(&self) -> &'static [Platform] {
        match self {
            Browser::Chrome => &[Platform::Windows, Platform::MacOS, Platform::Linux, Platform::ChromeOS, Platform::Android],
            Browser::Firefox => &[Platform::Windows, Platform::MacOS, Platform::Linux, Platform::Android],
            Browser::Safari => &[Platform::MacOS, Platform::IOS],
            Browser::Edge => &[Platform::Windows, Platform::MacOS, Platform::Linux, Platform::Android],
        }
    }
...
impl Platform {
    pub fn all() -> &'static [Platform] {
        &[Platform::Windows, Platform::MacOS, Platform::Linux, Platform::ChromeOS, Platform::Android, Platform::IOS]
    }
...
impl BrowserFingerprintProfile {
    ...
    fn chrome_macos() -> Self {
        Self {
            ...
            fonts: vec!["Academy Engraved LET".into(), "American Typewriter".into(), "Apple Color Emoji".into(),
                        "Apple SD Gothic Neo".into(), "Arial".into(), "Avenir".into(), "Baskerville".into(),
                        "Big Caslon".into(), "Bodoni 72".into(), "Bradley Hand".into(),
                        "Chalkboard SE".into(), "Cochin".into(), "Copperplate".into(), "Courier New".into(),
                        "Didot".into(), "Futura".into(), "Geneva".into(), "Georgia".into(),
                        "Gill Sans".into(), "Helvetica".into(), "Menlo".into(), "Monaco".into(),
                        "Optima".into(), "Palatino".into(), "Papyrus".into(), "San Francisco".into(),
                        "Savoye LET".into(), "Snell Roundhand".into(), "Times New Roman".into(), "Trebuchet MS".into(),
                        "Verdana".into()],
            audio_context_noise: 0.0002,
            screen_width: 2560, screen_height: 1600, color_depth: 30,
            hardware_concurrency: 12, device_memory_gb: 16,
        }
    }
    ...
    fn safari_macos() -> Self {
        ...
        p.screen_width = 1728; p.screen_height = 1117; p.color_depth = 30;
    ...
    fn firefox_linux() -> Self {
        Self {
            ...
            fonts: vec!["Bitstream Charter".into(), "Cantarell".into(), "Courier 10 Pitch".into(),
                        "DejaVu Sans".into(), "DejaVu Sans Mono".into(), "DejaVu Serif".into(),
                        "Droid Sans".into(), "Droid Sans Mono".into(), "Droid Serif".into(),
                        "Fira Sans".into(), "FreeMono".into(), "FreeSans".into(), "FreeSerif".into(),
                        "Liberation Mono".into(), "Liberation Sans".into(), "Liberation Serif".into(),
                        "Noto Sans".into(), "Noto Sans Mono".into(), "Noto Serif".into(),
                        "Open Sans".into(), "Roboto".into(), "Source Code Pro".into(), "Ubuntu".into()],
            audio_context_noise: 0.00005,
            screen_width: 1920, screen_height: 1080, color_depth: 24,
            hardware_concurrency: 4, device_memory_gb: 4,
        }
    }
    ...
    fn chrome_android() -> Self {
        Self {
            ...
            fonts: vec!["Droid Sans Mono".into(), "Noto Sans".into(), "Roboto".into(),
                        "Google Sans".into(), "Noto Color Emoji".into()],
            audio_context_noise: 0.0003,
            screen_width: 1080, screen_height: 2400, color_depth: 24,
            hardware_concurrency: 8, device_memory_gb: 8,
        }
    }
...
"#, self.canvas_noise_level, self.webgl_vendor, self.webgl_renderer,
    ...
    pub fn to_headers(&self) -> Vec<(&'static str, String)> {
        vec![
            ("X-Fingerprint-Canvas", format!("{:.4}", self.canvas_noise_level)),
            ("X-Fingerprint-WebGL-Vendor", self.webgl_vendor.clone()),
            ("X-Fingerprint-WebGL-Renderer", self.webgl_renderer.clone()),
            ("X-Fingerprint-Fonts", self.fonts.join(", ").chars().take(200).collect()),
            ("X-Fingerprint-Screen", format!("{}x{}x{}", self.screen_width, self.screen_height, self.color_depth)),
            ("X-Fingerprint-Hardware", format!("{} cores, {}GB RAM", self.hardware_concurrency, self.device_memory_gb)),
        ]
    }
}
...
impl SystemFingerprintGenerator {
    ...
    pub fn generate(&self, config: &SystemFingerprintConfig) -> SystemFingerprint {
        ...
        let platform = config.platform.unwrap_or_else(|| {
            ...
            if roll < 0.50 { Platform::Windows }
            else if roll < 0.70 { Platform::MacOS }
            else if roll < 0.85 { Platform::Linux }
            else if roll < 0.93 { Platform::ChromeOS }
            else if roll < 0.97 { Platform::Android }
            else { Platform::IOS }
        });
        ...
        let nt_world_browse = config.nt_world_browse.unwrap_or_else(|| {
            ...
            for &b in &[Browser::Chrome, Browser::Firefox, Browser::Safari, Browser::Edge] {
                if compat.contains(&b) {
                    if roll < 0.55 && b == Browser::Chrome { return b; }
                    if roll < 0.75 && b == Browser::Firefox { return b; }
                    if roll < 0.90 && b == Browser::Safari { return b; }
                    if roll < 1.00 && b == Browser::Edge { return b; }
                }
            }
            compat[0]
        });
        ...
        let (sec_ch_ua, sec_ch_ua_mobile, sec_ch_ua_platform) = match nt_world_browse {
            ...
            Browser::Firefox | Browser::Safari => {
                (String::new(), "?0".into(), platform.sec_ch_ua_platform().to_string())
            }
        };
    ...
    pub fn validate_consistency(fp: &SystemFingerprint) -> Vec<String> {
        ...
        if fp.hardware_concurrency == 0 || fp.hardware_concurrency > 128 {
            issues.push(format!("Unrealistic hardware_concurrency: {}", fp.hardware_concurrency));
        }
