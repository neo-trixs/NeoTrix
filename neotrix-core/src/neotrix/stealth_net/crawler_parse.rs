...
pub(crate) fn extract_body_text(html: &str, max_len: usize) -> String {
    ...
    for ch in html.chars() {
        ...
        if ch == '>' {
            ...
            if tn == "script" { in_script = true; }
            if tn == "/script" { in_script = false; }
            if tn == "style" { in_style = true; }
            if tn == "/style" { in_style = false; }
            continue;
        }
        if in_tag { tag_name.push(ch); continue; }
...
#[allow(dead_code)]
pub(crate) fn extract_snippet(html: &str, max_len: usize) -> String {
    extract_body_text(html, max_len)
}
...
pub(crate) fn extract_keywords(body: &str, title: &str) -> Vec<String> {
    ...
    let stop_words: HashSet<&str> = [
        ...
        "further", "then", "once", "here", "there", "http", "https", "www",
        "com", "org", "net", "html", "onion",
    ].into_iter().collect();
    ...
    let mut words: Vec<(usize, String)> = freq.into_iter()
        .map(|(k, v)| (v, k))
        .collect();
