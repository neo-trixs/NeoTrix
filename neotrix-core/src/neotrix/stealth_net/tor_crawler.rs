...
impl TorCrawler {
    ...
    async fn socks_client(&self) -> Result<reqwest::Client, String> {
        ...
        reqwest::Client::builder()
            ...
            .user_agent("Mozilla/5.0 (compatible; NeoTrixTorCrawler/2.0)")
            .build()
    ...
    pub async fn enqueue(&self, url: &str, depth: u32, priority: CrawlPriority) {
        ...
        if visited.contains(url) || visited.len() >= MAX_VISITED { return; }
        visited.insert(url.to_string());
        drop(visited);

        let mut queue = self.queue.write().await;
        if queue.len() >= MAX_QUEUE_SIZE { return; }
    ...
    pub async fn search_engine(&self, engine_url: &str, query: &str, client: &reqwest::Client) -> Vec<String> {
        ...
        let search_url = if engine_url.contains("ahmia") || engine_url.contains("juhanurmi") {
            format!("{}/search/?q={}", engine_url.trim_end_matches('/'), urlencoding(query))
        } else if engine_url.contains("torch") {
            format!("{}/search?query={}", engine_url.trim_end_matches('/'), urlencoding(query))
        } else if engine_url.contains("duckduckgo") {
            format!("{}/html/?q={}", engine_url.trim_end_matches('/'), urlencoding(query))
    ...
    pub async fn search(&self, query: &str) -> Vec<String> {
        ...
        for engine in SEARCH_ENGINES {
            let results = self.search_engine(engine, query, &client).await;
            for url in results {
        ...
        // Enqueue discovered URLs as priority SearchResult
        for url in &all_found {
            self.enqueue(url, 0, CrawlPriority::SearchResult).await;
        }
    ...
    async fn crawl_page(&self, client: &reqwest::Client, job: &CrawlJob) -> Result<CrawledPage, String> {
        let resp = client.get(&job.url)
            .send()
            .await
    ...
    fn store_page(&self, page: &CrawledPage) -> Result<(), String> {
        ...
        let path = self.store_path.join("pages").join(format!("{}.json", url_hash));
    ...
    fn index_page(&self, page: &CrawledPage) {
        let Ok(mut idx) = self.index.try_write() else { return };
        idx.insert(page);
    }
    ...
    pub async fn run(self: Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            let discovery_queries = [
                "onion directory", "hidden wiki", "tor links",
                        ...
                        match nt_world_crawl.crawl_page(&client, &job).await {
                            Ok(page) => {
                                ...
                                let next_priority = match job.priority {
                                    CrawlPriority::SearchResult => CrawlPriority::Discovered,
                                    CrawlPriority::Seed => CrawlPriority::Discovered,
                                    CrawlPriority::Discovered if job.depth >= 3 => CrawlPriority::Deep,
...
mod tests {
    ...
    #[test]
    fn test_onion_links() {
        let html = r#"<a href="http://abc123.onion/">link</a><a href="http://def456.onion/">link2</a>"#;
    ...
    fn test_extract_keywords() {
        ...
        assert!(keywords.contains(&"routing".to_string()) || keywords.contains(&"routing".to_string()));
    ...
    fn test_detect_category_marketplace() {
        let cat = detect_category("Buy cheap stuff", "market prices escrow", "http://market.onion/");
        assert_eq!(cat, ContentCategory::Marketplace);
    }

    #[test]
    fn test_detect_category_forum() {
        let cat = detect_category("Discussion board", "forum thread comment", "http://forum.onion/");
    ...
    fn test_priority_ordering() {
        assert!(CrawlPriority::SearchResult < CrawlPriority::Seed);
