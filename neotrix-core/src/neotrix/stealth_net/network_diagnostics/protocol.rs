...
use super::types::{
    ApiEndpoint, ApiHealth, ConnectionFailureRootCause, EndpointDiagnostic,
    NetworkEnvironment, VpnType,
};
...
impl EwmaDetector {
    pub fn new(alpha: f64, warmup: u64) -> Self {
        Self { alpha, mean: 0.0, variance: 0.0, sample_count: 0, warmup }
    }
    ...
    pub fn observe(&mut self, value: f64) -> f64 {
        ...
        let z = diff / std;
        if self.sample_count < self.warmup { 0.0 } else { z }
    }
...
impl CusumDetector {
    pub fn new(target: f64, k: f64, h: f64, warmup: u64) -> Self {
        Self { target, k, h, sum_plus: 0.0, sum_minus: 0.0, sample_count: 0, warmup }
    }

    /// Returns (is_above_alarm, is_below_alarm)
    pub fn observe(&mut self, value: f64) -> (bool, bool) {
        self.sample_count += 1;
        if self.sample_count < self.warmup { return (false, false); }
        ...
        if above { self.sum_plus = 0.0; }
        if below { self.sum_minus = 0.0; }
        (above, below)
    }
...
impl HoltWinters {
    ...
    pub fn new(alpha: f64, beta: f64, gamma: f64, period: usize, warmup_periods: usize) -> Self {
        Self {
            ...
            t: 0, residual_std: 0.0, warmup_periods,
        }
    }
    ...
    pub fn step(&mut self, y: f64) -> (f64, f64) {
        ...
        if self.t < self.period * self.warmup_periods {
            ...
            if self.t == 0 {
                ...
                self.level = self.level * (self.t as f64 / (self.t + 1) as f64)
                    + y / (self.t + 1) as f64;
                ...
                let new_seasonal = self.gamma * (y / new_level.max(1e-9))
                    + (1.0 - self.gamma) * last_seasonal;
        ...
        let z = if self.residual_std > 1e-9 { residual / (self.residual_std + 1e-9) } else { 0.0 };
        (forecast, z)
    }
...
fn default_interface() -> Option<String> {
    std::process::Command::new("route")
        ...
        .and_then(|o| {
            let out = String::from_utf8_lossy(&o.stdout);
            out.lines().find(|l| l.contains("interface:"))
...
fn detect_vpn_type(tuns: &[String], fake_ip: &Option<String>) -> Option<VpnType> {
    if fake_ip.is_some() {
        if let Ok(output) = std::process::Command::new("ps").args(["aux"]).output() {
            ...
            if out.contains("shadowrocket") { return Some(VpnType::Shadowrocket); }
            if out.contains("surge") { return Some(VpnType::Surge); }
            if out.contains("clash") { return Some(VpnType::Clash); }
        }
        if !tuns.is_empty() { return Some(VpnType::UnknownTun); }
    }
    None
}
...
pub async fn check_endpoint(endpoint: &ApiEndpoint) -> EndpointDiagnostic {
    ...
    // TCP connect timing
    let addr = match timeout(Duration::from_secs(5), tokio::net::lookup_host(format!("{}:443", host_port))).await {
        Ok(Ok(mut addrs)) => addrs.next(),
        _ => {
            return EndpointDiagnostic {
                endpoint: endpoint.name,
                health: ApiHealth::DnsFailure { reason: "DNS resolution failed".into() },
                timing: None,
    ...
    let addr = match addr {
        ...
        None => {
            return EndpointDiagnostic {
                endpoint: endpoint.name,
                health: ApiHealth::DnsFailure { reason: "No address resolved".into() },
                timing: None,
                root_cause: Some(ConnectionFailureRootCause::DnsResolutionFailed),
            };
        }
    ...
    match timeout(Duration::from_secs(10), TcpStream::connect(addr)).await {
        ...
        Err(_) => {
            return EndpointDiagnostic {
                endpoint: endpoint.name,
                health: ApiHealth::Unreachable { reason: "TCP connection timeout".into() },
                timing: None,
        ...
        .build()
    {
        ...
        Err(e) => {
            return EndpointDiagnostic {
                endpoint: endpoint.name,
                health: ApiHealth::Unreachable { reason: format!("HTTP client error: {}", e) },
                timing: None,
    ...
    let response = match client.head(endpoint.url).send().await {
        ...
        Err(e) => {
            ...
            if err_str.contains("503") || err_str.contains("status code 503") {
                return EndpointDiagnostic {
                    endpoint: endpoint.name,
                    health: ApiHealth::HttpError { status_code: 503, body_snippet: err_str },
                    timing: None,
    ...
    let health = if status == 503 {
        ApiHealth::HttpError { status_code: 503, body_snippet: String::new() }
    } else if ttfb > Duration::from_secs(10) {
        ApiHealth::Degraded { latency: total, ttfb }
    } else if status == endpoint.expected_status || status == 401 || status == 422 {
        if ttfb > Duration::from_secs(3) {
            ApiHealth::Degraded { latency: total, ttfb }
        } else {
            ApiHealth::Healthy { latency: total, ttfb }
        }
    } else {
        ApiHealth::HttpError { status_code: status, body_snippet: String::new() }
    };
...
pub async fn scan_environment() -> NetworkEnvironment {
    ...
    let (country, org) = if let Ok(resp) = reqwest::get("https://ipinfo.io/json").await {
        if let Ok(body) = resp.text().await {
            let c = body.split("\"country\":\"")
                .nth(1).and_then(|s| s.split('"').next()).map(|s| s.to_string());
            let o = body.split("\"org\":\"")
                .nth(1).and_then(|s| s.split('"').next()).map(|s| s.to_string());
            (c, o)
        } else { (None, None) }
    } else { (None, None) };
...
mod tests {
    ...
    fn test_ewma_anomaly_detection() {
        ...
        for _ in 0..20 { e.observe(0.5); } // warmup with 0.5s
