...
mod tests {
    ...
    fn test_deterministic_classifier_fake_ip() {
        ...
        let health = ApiHealth::DnsFailure { reason: "DNS failure".into() };
    ...
    fn test_deterministic_classifier_503() {
        ...
        let result = RootCauseClassifier::classify_deterministic(&health, &env);
        assert!(result.is_some());
        assert_eq!(result.unwrap().cause, ConnectionFailureRootCause::HttpServiceUnavailable);
    }
    ...
    fn test_remediation_engine() {
        let action =
            RemediationEngine::recommend(&ConnectionFailureRootCause::DnsResolutionFailed);
        ...
        assert!(action2.is_none(), "server-side issues should not get network remediation");
    }

    #[test]
    fn test_should_auto_remediate() {
        assert!(RemediationEngine::should_auto_remediate(&ConnectionFailureRootCause::FakeIpDns));
        assert!(
            !RemediationEngine::should_auto_remediate(&ConnectionFailureRootCause::HttpServiceUnavailable)
        );
        assert!(!RemediationEngine::should_auto_remediate(
            &ConnectionFailureRootCause::ProviderTooSlow { ttfb: Duration::from_secs(20) }
        ));
    }
}
