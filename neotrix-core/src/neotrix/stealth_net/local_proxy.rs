...
use super::rules::{RuleEngine, OutboundAction};
...
fn parse_proxy_url(url: &str) -> Option<(&str, &str, u16)> {
...
async fn connect_via_proxy_pool(host: &str, port: u16) -> Result<TcpStream, String> {
    ...
    let parsed = parse_proxy_url(&proxy.url)
        .ok_or_else(|| format!("bad proxy url: {}", proxy.url))?;

    let result = match parsed.0 {
        "socks5" => {
            connect_via_socks5(&format!("{}:{}", parsed.1, parsed.2), host, port).await
        }
        "http" | "https" => {
            ...
            if !resp.starts_with("HTTP/1.1 200") {
                Err(format!("http proxy rejected: {}", resp.lines().next().unwrap_or("")))
            } else {
                Ok(stream)
            }
        }
...
async fn connect_via_socks5(proxy_addr: &str, host: &str, port: u16) -> Result<TcpStream, String> {
    ...
    tokio::time::timeout(Duration::from_secs(10), stream.write_all(&msg))
        .await.map_err(|_| "socks5 connect write timeout".to_string())?
        .map_err(|e| format!("socks5 connect: {}", e))?;
    let mut resp = [0u8; 4];
    tokio::time::timeout(Duration::from_secs(10), stream.read_exact(&mut resp))
        .await.map_err(|_| "socks5 connect read timeout".to_string())?
    ...
    let bound_len = match resp[3] {
        ...
        3 => {
            ...
            tokio::time::timeout(Duration::from_secs(5), stream.read_exact(&mut lb))
                .await.map_err(|_| "socks5 bound len timeout".to_string())?
    ...
    tokio::time::timeout(Duration::from_secs(5), stream.read_exact(&mut _addr))
        .await.map_err(|_| "socks5 bound addr timeout".to_string())?
        .map_err(|e| format!("bound addr: {}", e))?;
    let mut _pb = [0u8; 2];
    tokio::time::timeout(Duration::from_secs(5), stream.read_exact(&mut _pb))
        .await.map_err(|_| "socks5 bound port timeout".to_string())?
...
impl LocalProxy {
    ...
    pub fn with_fingerprint_manager(mut self, fp: Arc<std::sync::Mutex<FingerprintManager>>) -> Self {
        self.fingerprint_manager = Some(fp);
        self
    }

    pub async fn start(self: Arc<Self>) -> Result<(), String> {
        let listener = TcpListener::bind("127.0.0.1:11080").await
            .map_err(|e| format!("bind: {}", e))?;
        println!("[proxy] HTTP CONNECT proxy on 127.0.0.1:11080");
    ...
    async fn handle_http_get(&self, mut stream: TcpStream, data: &[u8], _method: &str, url_s: &str) -> Result<(), String> {
        let parsed = url::Url::parse(url_s)
            .map_err(|e| format!("parse url {}: {}", url_s, e))?;
        let host = parsed.host_str().unwrap_or("").to_string();
        let port = parsed.port().unwrap_or(80);

        let start = Instant::now();
        let mut remote = self.connect_with_fallback(&host, port).await.map_err(|e| {
            eprintln!("[proxy] HTTP {}:{} failed: {}", host, port, e);
            format!("connect {}:{}: {}", host, port, e)
        })?;
        let elapsed = start.elapsed();
        eprintln!("[proxy] HTTP {}:{} OK {:?}", host, port, elapsed);
    ...
    async fn connect_with_fallback(&self, host: &str, port: u16) -> Result<TcpStream, String> {
        ...
        if use_proxy && pool_ready {
            ...
            if before > 0 {
                match connect_via_proxy_pool(host, port).await {
                    Ok(s) => return Ok(s),
                    Err(e) => eprintln!("[proxy] pool failed {}:{}: {}", host, port, e),
                }
            } else {
                eprintln!("[proxy] pool {} nodes (pending health-check), skip", pool_total);
            }
        }

        if use_tor || (port == 443) || (!use_proxy && !pool_ready) {
            if Self::tor_reachable().await {
                match connect_via_socks5("127.0.0.1:9050", host, port).await {
                    Ok(s) => return Ok(s),
                    Err(e) => eprintln!("[proxy] Tor failed {}:{}: {}", host, port, e),
                }
            } else {
                eprintln!("[proxy] Tor not reachable on 9050");
            }
        }

        if use_proxy && pool_available == 0 && pool_total > 0 {
            eprintln!("[proxy] pool retry for {}:{}", host, port);
            ...
            if Self::tor_reachable().await {
                if let Ok(s) = connect_via_socks5("127.0.0.1:9050", host, port).await {
                    return Ok(s);
                }
            }
        }
        ...
        ).await.map_err(|_| format!("direct timeout {}:{}", host, port))?
         .map_err(|e| format!("direct {}:{}: {}", host, port, e))
    }

    async fn tor_reachable() -> bool {
        tokio::time::timeout(Duration::from_secs(2), TcpStream::connect("127.0.0.1:9050")).await.is_ok()
    }
}
...
impl TorManager {
    ...
    pub async fn socks5_reachable() -> bool {
        tokio::time::timeout(Duration::from_secs(3), async {
            TcpStream::connect("127.0.0.1:9050").await.is_ok()
        }).await.unwrap_or(false)
    }
}

pub fn tor_connect(_target: &str, _port: u16) -> Result<String, String> {
    Ok("socks5://127.0.0.1:9050".to_string())
}
