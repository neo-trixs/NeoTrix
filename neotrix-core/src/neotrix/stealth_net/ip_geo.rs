...
impl IpGeoLocator {
    pub fn new() -> Self {
        Self {
            cache: Arc::new(Mutex::new(LruCache::new(NonZeroUsize::new(1024).expect("result")))),
    ...
    pub async fn lookup(&self, ip: &str) -> Option<GeoResult> {
        ...
        let url = format!("http://ip-api.com/json/{}?fields=query,countryCode,timezone", ip);
        ...
        let result = GeoResult {
            ip: json.get("query").and_then(|v| v.as_str()).unwrap_or(ip).to_string(),
            country_code: json.get("countryCode").and_then(|v| v.as_str()).unwrap_or("??").to_string(),
    ...
    fn extract_vmess_host(b64: &str) -> Option<String> {
        ...
        json.get("add").and_then(|v| v.as_str()).map(|s| s.to_string())
    }
    ...
    pub async fn lookup_batch(&self, ips: &[String]) -> Vec<(String, GeoResult)> {
        ...
        for chunk in uncached.chunks(100) {
            ...
            if let Ok(resp) = self.client.post("http://ip-api.com/batch")
                ...
                .await
            {
                if let Ok(text) = resp.text().await {
                    if let Ok(arr) = serde_json::from_str::<Vec<serde_json::Value>>(&text) {
                        let mut cache = self.cache.lock().await;
                        for entry in arr {
                            let ip = entry.get("query").and_then(|v| v.as_str()).unwrap_or("").to_string();
                            let status = entry.get("status").and_then(|v| v.as_str()).unwrap_or("");
                            if status == "fail" || ip.is_empty() { continue; }
                            let geo = GeoResult {
                                ip: ip.clone(),
                                country_code: entry.get("countryCode").and_then(|v| v.as_str()).unwrap_or("??").to_string(),
                                timezone: entry.get("timezone").and_then(|v| v.as_str()).unwrap_or("UTC").to_string(),
                            };
