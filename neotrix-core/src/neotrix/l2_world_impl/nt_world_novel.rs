...
//!
//! 从 `bin/nt_world_novel_ingest.rs` 提炼: Royal Road 排行榜爬取 + 起点中文网
//! 书目注入(port of `scripts/novel-world-absorb.py` 分类+注入层) +
...
// ============================================================================
// Royal Road 排行榜爬取 (network)
// ============================================================================

/// Royal Road 排行榜列表
pub const ROYALROAD_RANKINGS: &[(&str, &str)] = &[
...
pub const MAX_PER_LIST: usize = 30;
...
/// 爬取 Royal Road 各排行榜并吸收 Book 节点 + 世界构建分析。返回吸收小说数。
pub fn crawl_royalroad_rankings(conn: &Connection, client: &reqwest::blocking::Client) -> usize {
    let ts = now();
    let mut total = 0usize;
    for (list_name, url) in ROYALROAD_RANKINGS {
        log::info!("[novel] crawl Royal Road: {}", list_name);
        let fetched = fetch_and_parse_list(conn, client, list_name, url, ts);
        log::info!("[novel]   absorbed {} novels from {}", fetched, list_name);
        total += fetched;
...
fn fetch_and_parse_list(
    ...
    let html = match client.get(url).send() {
        ...
        Ok(r) => { eprintln!("[novel] HTTP {} for {}", r.status(), url); return 0; }
        Err(e) => { eprintln!("[novel] Failed: {}", e); return 0; }
    };
    ...
    for entry in html.split("fiction-list-item row\">").skip(1) {
        ...
        let href = extract_between(entry, "<a href=\"", "\"").unwrap_or_default();
        ...
        let rating = extract_between(entry, "title=\"", "\"")
            .or_else(|| extract_between(entry, "aria-label=\"Rating: ", " out of 5\""))
            .unwrap_or_default()
            .trim()
            .to_string();
...
",
    ...
    _ = conn.execute(
        ...
        params![analysis_id, novel_id, format!("World-building analysis for {}", title), ts],
    ).unwrap_or(0);
}

/// 创建/更新 Royal Road Book 节点并挂到 world-building-root。
pub fn create_or_update_node(
    ...
    conn.execute(
        ...
        params![node_id, title, summary, clean_content, url, ts],
    )?;
    ...
    if !author.is_empty() {
        ...
        conn.execute(
            ...
            params![node_id, author_id, format!("{} written by {}", title, author), ts],
        )?;
    }
    ...
    conn.execute(
        ...
        params![node_id, wb_id, format!("{} ranked on {}", title, list_name), ts],
    )?;

    Ok(())
}

/// 汇总世界构建模式到 Summary 节点, 返回世界观类型数。
pub fn summarize_world_building_patterns(conn: &Connection) -> usize {
    let ts = now();
