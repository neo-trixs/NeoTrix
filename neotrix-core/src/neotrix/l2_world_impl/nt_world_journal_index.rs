use crate::core::nt_core_hcube::vsa_quantized::{QuantizedVSA, VSA_DIM, pack_binary, similarity_packed};
...
impl JournalIndex {
    pub fn open() -> Result<Self, String> {
        ...
        let conn = rusqlite::Connection::open(&path).map_err(|e| format!("cannot open journal index: {}", e))?;
        conn.execute_batch(
    ...
    fn fingerprint(text: &str) -> Vec<u8> {
        let seed: u64 = text.bytes().fold(0u64, |acc, b| acc.wrapping_mul(31).wrapping_add(b as u64));
    ...
    pub fn add_entry(&self, id: &str, goal_text: &str, timestamp: &str, evidence_count: usize, success: bool) -> Result<(), String> {
        ...
        stmt.execute(rusqlite::params![id, goal_text, timestamp, fp, bfp, evidence_count as i64, success as i64])
    ...
    pub fn search_with_mode(&self, query: &str, k: usize, mode: SearchMode) -> Result<Vec<(JournalEntry, f64)>, String> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            Ok((id, goal_text, timestamp, fp, bfp, evidence_count as usize, success != 0))
        }).map_err(|e| format!("query: {}", e))?;

        for row in rows {
            let (id, goal_text, timestamp, fp, bfp, evidence_count, success) = row.map_err(|e| format!("row: {}", e))?;
            let sim = match mode {
                SearchMode::Fast => {
                ...
                SearchMode::Accurate => {
                    QuantizedVSA::similarity(&query_fp, &fp)
                }
            };
            if sim > 0.3 {
                candidates.push((JournalEntry { id, goal_text, timestamp, evidence_count, success }, fp, sim));
            }
        }
    ...
    pub fn count(&self) -> Result<usize, String> {
        let mut stmt = self.conn.prepare("SELECT COUNT(*) FROM journal_entries")
    ...
    pub fn get(&self, id: &str) -> Result<Option<JournalEntry>, String> {
        ...
        let mut rows = stmt.query_map(rusqlite::params![id], |row| {
            ...
            Ok(JournalEntry { id, goal_text, timestamp, evidence_count: evidence_count as usize, success: success != 0 })
        }).map_err(|e| format!("query: {}", e))?;
        match rows.next() {
...
impl Default for JournalIndex {
    /// Creates an in-memory journal index for testing/transient use.
    /// For persistent storage, use `JournalIndex::open()`.
    fn default() -> Self {
        // open_in_memory can only fail under extreme OS resource exhaustion.
        // Try a temp-file fallback before giving up.
        let conn = match rusqlite::Connection::open_in_memory() {
            Ok(c) => c,
            Err(e) => {
                log::error!("[journal_index] in-memory SQLite failed: {}. Trying temp file.", e);
                let tmp = std::env::temp_dir().join("neotrix_journal_index.db");
                match rusqlite::Connection::open(&tmp) {
                    Ok(c) => c,
                    Err(e2) => {
                        log::error!("[journal_index] temp file SQLite also failed: {}.", e2);
                        // Last resort: retry in-memory (fails only under OS exhaustion)
                        rusqlite::Connection::open_in_memory()
                            .unwrap_or_else(|e| {
                                log::error!("[journal_index] unrecoverable: all SQLite fallbacks failed: {}", e);
                                std::process::abort()
                            })
                    }
                }
...
mod tests {
    ...
    fn test_add_and_count() {
        ...
        idx.add_entry("test-1", "实现用户登录功能", "2026-06-09T12:00:00", 3, true).unwrap();
        idx.add_entry("test-2", "修复内存泄漏", "2026-06-09T13:00:00", 2, false).unwrap();
    ...
    fn test_search_returns_relevant() {
        let idx = JournalIndex::default();
        idx.add_entry("t1", "实现用户登录功能，包括JWT认证", "2026-06-09", 3, true).unwrap();
        idx.add_entry("t2", "修复内存泄漏", "2026-06-09", 2, false).unwrap();
        idx.add_entry("t3", "设计数据库表结构", "2026-06-08", 5, true).unwrap();
    ...
    fn test_search_exact_match() {
        let idx = JournalIndex::default();
        idx.add_entry("t1", "Rust异步编程教程", "2026-06-09", 4, true).unwrap();
    ...
    fn test_get_existing_entry() {
        let idx = JournalIndex::default();
        idx.add_entry("test-get", "重构模块", "2026-06-09", 1, true).unwrap();
    ...
    fn test_search_fast_mode() {
        let idx = JournalIndex::default();
        idx.add_entry("f1", "神经网络训练", "2026-06-09", 2, true).unwrap();
        idx.add_entry("f2", "数据库查询优化", "2026-06-09", 3, true).unwrap();
        let results = idx.search_with_mode("神经网络", 5, SearchMode::Fast).unwrap();
        assert!(!results.is_empty());
    ...
    fn test_search_accurate_mode() {
        let idx = JournalIndex::default();
        idx.add_entry("a1", "用户登录功能", "2026-06-09", 2, true).unwrap();
        let results = idx.search_with_mode("用户登录功能", 5, SearchMode::Accurate).unwrap();
        assert!(!results.is_empty());
