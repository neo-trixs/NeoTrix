...
impl CheckpointSpider {
    ...
    pub fn crawl_with_checkpoint<F>(
        &mut self,
        process_page: F,
        fetch_fn: Option<&dyn Fn(&CrawlRequest) -> Result<CrawlResponse, String>>,
    ) -> Result<SpiderReport, String>
    where
        F: FnMut(&CrawlResponse) -> Result<(), String>,
    {
        let mut process_page = process_page;
