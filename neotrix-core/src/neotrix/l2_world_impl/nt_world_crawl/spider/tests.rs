...
fn test_crawl_with_checkpoint_completes() {
    ...
    let report = spider.crawl_with_checkpoint(|resp| {
        ...
        processed += 1;
        Ok(())
    }, None).expect("crawl with checkpoint should succeed");
...
fn test_crawl_with_checkpoint_saves_on_interrupt() {
    ...
    }, None).expect("crawl with checkpoint should succeed on interrupt");
...
fn test_spider_report_display() {
    ...
    let report = spider.crawl_with_checkpoint(|_| Ok(()), None).expect("crawl with checkpoint should succeed");
