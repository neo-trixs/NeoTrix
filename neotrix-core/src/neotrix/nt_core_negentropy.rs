...
impl NegentropyCalculator {
    ...
    pub fn sensor_prediction_acc(jepa: Option<&JepaWorldModel>, jepa_error: Option<f64>) -> f64 {
        ...
        let stability = if jepa.check_rollout_stability() {
            0.8
        } else {
            0.3
        };
    ...
    pub fn sensor_temporal_coherence(stream: &ConsciousnessStream) -> f64 {
        ...
        let variance =
            recency.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / recency.len() as f64;
    ...
    pub fn compute_full(
        ...
        self.compute_full_with_jepa_error(
            state,
            vsa_pool,
            kb,
            jepa,
    ...
    pub fn compute_kb_only(
        ...
        let import_rate = if round_time > 0.0 {
            round_recs as f64 / round_time
        } else {
            0.0
        };
