...
use crate::neotrix::nt_expert_routing::{Context, WorldModel};
...
impl WorldModelV2 {
    ...
    pub fn predict_all(&self, context_features: &[f64]) -> (f64, f64, f64) {
        ...
        let _expert_scores = self
            .legacy
            .predict_expert_performance(&vec![0.0; 64], &ctx, &[0]);
    ...
    pub fn run_prediction_cycle(
    ...
    ) -> (FreeEnergyReport, PhiReport, bool) {
        ...
        let fe_report =
            self.nt_world_infer
                .compute_free_energy(jepa_energy, e8_entropy, e8_energy_gradient);
    ...
    pub fn free_energy_report(&mut self) -> FreeEnergyReport {
        self.nt_world_infer
            .compute_free_energy(self.last_jepa_energy, 0.0, 0.0)
    }
