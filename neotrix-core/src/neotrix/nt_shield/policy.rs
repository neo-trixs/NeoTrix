...
impl ActionPolicy {
    pub fn new() -> Self {
        ...
        rules.insert(
            "modify_dependency".to_string(),
            PolicyDecision::RequireConfirmation,
        );
        rules.insert(
            "access_nt_world_browse_auto".to_string(),
            PolicyDecision::RequireConfirmation,
        );
        rules.insert(
            "access_tor_network".to_string(),
            PolicyDecision::RequireConfirmation,
        );
    ...
    pub fn set_profile(&mut self, profile: &str) {
        ...
        match profile {
            "strict-nt_shield" => {
                self.rules
                    .insert("read_file".to_string(), PolicyDecision::Allow);
                self.rules
                    .insert("write_file".to_string(), PolicyDecision::Deny);
                self.rules.insert(
                    "execute_command".to_string(),
                    PolicyDecision::RequireConfirmation,
                );
                ...
                self.rules
                    .insert("access_tor_network".to_string(), PolicyDecision::Deny);
            }
            "general" => {
                self.rules
                    .insert("write_file".to_string(), PolicyDecision::Allow);
                self.rules
                    .insert("execute_command".to_string(), PolicyDecision::Allow);
                self.rules
                    .insert("network_request".to_string(), PolicyDecision::Allow);
            }
            "network-isolated" => {
                // 仅允许 LLM API 域名 + 显式白名单
                self.rules
                    .insert("network_request".to_string(), PolicyDecision::Deny);
                self.rules.insert(
                    "write_file".to_string(),
                    PolicyDecision::RequireConfirmation,
                );
    ...
    pub fn is_domain_allowed(&self, domain: &str) -> bool {
        self.network_allowlist.contains(domain)
            || self
                .network_allowlist
                .iter()
                .any(|d| domain.ends_with(&format!(".{}", d)) || domain == d)
    }
...
mod tests {
    ...
    fn test_network_default_deny() {
        let p = ActionPolicy::new();
        assert!(
            !p.evaluate("network_request"),
            "network_request should default to Deny"
        );
    }
    ...
    fn test_llm_domain_auto_allowed() {
        ...
        assert_eq!(
            p.evaluate_network("api.anthropic.com"),
            PolicyDecision::Allow
        );
        assert_eq!(
            p.evaluate_network("generativelanguage.googleapis.com"),
            PolicyDecision::Allow
        );
    }
    ...
    fn test_custom_domain_allowlist() {
        ...
        assert_eq!(
            p.evaluate_network("my.internal.api.com"),
            PolicyDecision::Allow
        );
    }
