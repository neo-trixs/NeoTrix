...
fn load_master_key() -> NeoTrixResult<[u8; 32]> {
    match std::env::var(MASTER_KEY_ENV) {
        ...
        Err(_) => {
            ...
            log::warn!(
                "[neotrix-keyvault] {} not set; generated random key: {}",
                MASTER_KEY_ENV,
                hex_key
            );
            Ok(key)
...
fn decrypt_value(cipher: &Aes256Gcm, encoded: &str) -> NeoTrixResult<String> {
    let data = base64::engine::general_purpose::STANDARD
        .decode(encoded)
        .map_err(|e| NeoTrixError::General {
            msg: format!("Base64 decode failed: {}", e),
            backtrace: None,
        })?;
...
impl LocalStore {
    fn new() -> NeoTrixResult<Self> {
        ...
        let entries = if path.exists() {
            ...
            serde_json::from_str(&data).map_err(|e| NeoTrixError::Serde(e.to_string()))?
        } else {
            HashMap::new()
        };
        Ok(Self {
    ...
    fn set(&self, key: &str, value: &str) -> NeoTrixResult<()> {
        ...
        if entries.len() > MAX_VAULT_ENTRIES {
            let excess = entries.len() - MAX_VAULT_ENTRIES;
            let keys: Vec<String> = entries.keys().take(excess).cloned().collect();
            for k in keys {
                entries.remove(&k);
            }
        }
    ...
    fn save(&self) -> NeoTrixResult<()> {
        if !*self.dirty.lock().unwrap_or_else(|e| e.into_inner()) {
            return Ok(());
        }
        let entries = self.entries.lock().unwrap_or_else(|e| e.into_inner());
        let json =
            serde_json::to_string(&*entries).map_err(|e| NeoTrixError::Serde(e.to_string()))?;
...
impl KeyringStore {
    ...
    fn set(&self, key: &str, value: &str) -> NeoTrixResult<()> {
        let entry =
            keyring::Entry::new(&self.service_name, key).map_err(|e| NeoTrixError::General {
                msg: format!("Keyring entry error: {}", e),
                backtrace: None,
            })?;
        entry
            .set_password(value)
            .map_err(|e| NeoTrixError::General {
                msg: format!("Keyring set failed: {}", e),
                backtrace: None,
            })?;
        Ok(())
    }
    ...
    fn get(&self, key: &str) -> NeoTrixResult<Option<String>> {
        ...
        match entry.get_password() {
            ...
            Err(e) => Err(NeoTrixError::General {
                msg: format!("Keyring get failed: {}", e),
                backtrace: None,
            }),
        }
    }
    ...
    fn delete(&self, key: &str) -> NeoTrixResult<()> {
        ...
        entry
            .delete_credential()
            .map_err(|e| NeoTrixError::General {
                msg: format!("Keyring delete failed: {}", e),
                backtrace: None,
            })?;
        Ok(())
    }
...
impl KeyVault {
    pub fn new(_service_name: &str) -> NeoTrixResult<Self> {
        ...
        Ok(Self {
            local_store,
            #[cfg(feature = "keyring")]
            keyring: KeyringStore::new(_service_name),
        })
    }
...
mod tests {
    ...
    fn test_local_store_set_get() {
        ...
        assert_eq!(
            store.get("api_key").expect("store.get"),
        ...
        assert_eq!(
            store2.get("api_key").expect("store.get"),
            Some("sk-test123".to_string())
        );
    }
