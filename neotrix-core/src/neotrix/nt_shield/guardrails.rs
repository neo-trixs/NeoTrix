        ...
        if call_count > self.config.max_tool_calls {
            GuardrailResult {
                ...
                violations: vec![GuardrailViolation {
                    ...
                    detail: format!(
                        "Tool call count {} exceeds max {}",
                        call_count, self.config.max_tool_calls
                    ),
...
mod tests {
    ...
    fn test_input_guardrail_blocks_long_input() {
        ...
        let result = guard.validate(
            "test",
            "this is a very long input that exceeds the limit",
            None,
        );
    ...
    fn test_guardrail_system() {
        ...
        let result = system.check_tool_call(
            "web_scrape",
            "https://example.com",
            Some(&ToolPermission::Network),
        );
    ...
    fn test_output_guardrail_warns_on_long() {
        ...
        assert!(result
            .violations
            .iter()
            .any(|v| v.rule == "max_output_length"));
    }
}
