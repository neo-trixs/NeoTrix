...
const MAX_SESSION_MEMORY: usize = 10_000;
...
impl SecurityGuard {
    ...
    pub fn check(&self, action: &str, target: &str) -> Result<bool, GuardRequest> {
        ...
        let request = GuardRequest::new(
            action,
            target,
            &format!("{} on {} needs approval", action, target),
        );
        if let Ok(mut pending) = self.pending.lock() {
            if pending.len() >= MAX_PENDING {
                Self::prune_pending(&mut pending, (MAX_PENDING * 20 / 100).max(1));
            }
        ...
        if let Ok(mut pending) = self.pending.lock() {
            if let Some(pos) = pending.iter().position(|r| r.id == id) {
                ...
                if let Ok(mut memory) = self.session_memory.lock() {
                    if memory.len() >= MAX_SESSION_MEMORY {
                        if let Some(oldest) = memory.keys().next().cloned() {
                ...
                let label = match &decision {
                    GuardDecision::Allowed
                    | GuardDecision::AllowedOnce
                    | GuardDecision::AllowedSession => "ALLOW",
                    _ => "DENY",
                };
                self.audit
                    .append(label, &req.action, &req.target, "user_resolved");
                return true;
            }
        }
        false
    ...
    /// 清除最旧的 N 个待确认请求
    fn prune_pending(pending: &mut Vec<GuardRequest>, count: usize) {
        let drain_end = count.min(pending.len());
    ...
    pub fn is_blocked(&self, action: &str, target: &str) -> bool {
        match action {
            ...
            "command_exec" => {
                ...
                self.blocked_commands.iter().any(|bc| lower.contains(bc))
            }
            "network_access" => {
...
impl AuditLog {
    ...
    pub fn append(&self, decision: &str, action: &str, target: &str, reason: &str) {
        ...
        if let Ok(mut entries) = self.entries.lock() {
            ...
            if entries.len() > MAX_AUDIT_ENTRIES {
                entries.drain(0..MAX_AUDIT_ENTRIES / 5);
            }
        }
    }
    ...
    pub fn recent(&self, n: usize) -> Vec<AuditEntry> {
        match self.entries.lock() {
            Ok(e) => {
                let len = e.len();
                e.iter().skip(len.saturating_sub(n)).cloned().collect()
            }
            Err(e) => {
