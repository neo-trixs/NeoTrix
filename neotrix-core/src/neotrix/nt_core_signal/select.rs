...
impl SelectableOperator {
    ...
    pub fn step(
        &self,
        state: &mut crate::core::nt_core_ssm::SelectiveState,
        input: &Vector,
    ) -> Vector {
    ...
    pub fn step_mimo(
        ...
        for s in 0..self.num_streams {
            ...
            for i in 0..self.hidden_dim.min(state_real.len().saturating_sub(offset)) {
                ...
                if idx < state_real.len() {
                    state_real[idx] = new_hr;
                }
                if idx < state_imag.len() {
                    state_imag[idx] = new_hi;
                }
            }
...
mod tests {
    ...
    fn test_mimo_step_returns_correct_number_of_outputs() {
        ...
        let inputs = vec![vec![0.5; 16], vec![-0.3; 16]];
    ...
    fn test_mimo_state_evolves_after_step() {
        ...
        assert!(
            has_real || has_imag,
            "state should have non-zero values after step"
        );
    }
    ...
    fn test_selectable_operator_backward_compat() {
        ...
        let mut state = crate::core::nt_core_ssm::SelectiveState {
            data: vec![0.0; 8],
            hidden: vec![0.0; 16],
