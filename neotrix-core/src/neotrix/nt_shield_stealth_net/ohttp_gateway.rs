...
impl OhttpRelayClient {
    ...
    pub async fn fetch_key_config(&self) -> Result<OhttpKeyConfig, String> {
        let url = self.config
            .key_config_url
            .as_deref()
            .unwrap_or(&format!("{}/ohttp-keys", self.config.gateway_url));
    ...
    pub async fn relay_request(
        ...
        let key_config = self.gateway_key
            .read()
            .await
            .clone()
            .ok_or_else(|| "gateway key not loaded — call fetch_key_config first".to_string())?;
    ...
    fn seal_request(
        ...
        let shared = SharedSecret::from_sec1_bytes(&key_config.public_key)
            .map_err(|e| format!("shared secret: {}", e))
            .and_then(|ss| {
                self.local_secret
                    .diffie_hellman(&gateway_pk)
                    .raw_secret_bytes()
                    .as_slice()
                    .try_into()
    ...
    fn open_response(
        ...
        let gateway_pk = self.local_secret.public_key();
        let shared = SharedSecret::new_from_sec1_bytes(&gateway_pk.to_encoded_point(false).as_bytes().to_vec())
            .ok()
            .and_then(|_| {
                self.local_secret
                    .diffie_hellman(&PublicKey::from_sec1_bytes(&encrypted[1..1 + enc_len]).ok()?)
                    .raw_secret_bytes()
                    .as_slice()
                    .try_into()
                    .ok()
            })
            .ok_or_else(|| "response decryption: shared secret failed".to_string())?;
...
fn hkdf_sha256(ikm: &[u8], salt: &[u8], info: &[u8], len: usize) -> Vec<u8> {
    ...
    let mut mac = Hmac::<Sha256>::new_from_slice(salt).expect("hkdf salt");
    mac.update(ikm);
    ...
    while okm.len() < len {
        let mut mac = Hmac::<Sha256>::new_from_slice(&prk).expect("hkdf prk");
        mac.update(&t);
        mac.update(info);
        mac.update(&[counter]);
...
mod tests {
    ...
    fn test_seal_request_requires_key() {
        ...
        assert!(result.is_ok(), "seal_request should succeed: {:?}", result.err());
