...
impl TorClient {
    ...
    pub fn into_reqwest_proxy(&self) -> NeoTrixResult<reqwest::Proxy> {
        ...
        reqwest::Proxy::all(&url).map_err(|e| NeoTrixError::Network(format!("Failed to create SOCKS5 proxy: {}", e)))
    }
    ...
    pub async fn start(&self) -> NeoTrixResult<()> {
        ...
        let child = cmd.spawn()
            .map_err(|e| NeoTrixError::General { msg: format!("Failed to start Tor: {} — is 'tor' installed?", e), backtrace: None })?;
        *proc = Some(child);
    ...
    pub async fn stop(&self) -> NeoTrixResult<()> {
        ...
        if let Some(mut child) = proc.take() {
            child.kill().map_err(NeoTrixError::Io)?;
    ...
    async fn send_control_command(&self, cmd: &str) -> NeoTrixResult<()> {
        ...
        // Step 4: 读命令响应
        if !Self::read_until_ok(&mut reader).await? {
            return Err(NeoTrixError::General { msg: format!("Command failed: {}", cmd.trim()), backtrace: None });
        }
        Ok(())
    }
    ...
    async fn read_auth_response(
        ...
        loop {
            ...
            if trimmed.starts_with("5") && trimmed.len() >= 4 {
                return Err(NeoTrixError::Network(format!("Tor auth error: {}", trimmed)));
            }
    ...
    fn handle_safecookie(b64_data: &str, config: &TorConfig) -> NeoTrixResult<Option<String>> {
        ...
        if cookie.len() != 32 {
            return Err(NeoTrixError::Config(format!("Invalid auth cookie size: {} (expected 32)", cookie.len())));
        }
        ...
        if server_nonce.len() != 32 {
            return Err(NeoTrixError::Network(format!("Invalid server nonce size: {}", server_nonce.len())));
        }
    ...
    async fn read_until_ok(reader: &mut (impl tokio::io::AsyncBufReadExt + Unpin)) -> NeoTrixResult<bool> {
        let mut line = String::new();
        loop {
            line.clear();
            let n = reader.read_line(&mut line).await
    ...
    pub async fn health_check(&self) -> bool {
        ...
        let socks_ok = tokio::time::timeout(Duration::from_secs(5), async {
            ...
            let _ = stream.write_all(&[5, 1, 0]).await.ok()?;
            let mut buf = [0u8; 2];
