...
mod tests {
    ...
    fn test_cyclic_round_robin() {
        ...
        let first = rotator.next().expect("rotator should return proxy").url.clone();
        let second = rotator.next().expect("rotator should return proxy").url.clone();
        let third = rotator.next().expect("rotator should return proxy").url.clone();
        let fourth = rotator.next().expect("rotator should return proxy").url.clone();
    ...
    fn test_random_returns_valid_proxy() {
        ...
        for _ in 0..20 {
            ...
            assert!(["http://proxy-a:8080", "http://proxy-b:8080", "http://proxy-c:8080"]
    ...
    fn test_cyclic_returns_all_proxies() {
        ...
        for _ in 0..6 {
            let url = rotator.next().expect("rotator should return proxy").url.clone();
            seen.insert(url);
        }
