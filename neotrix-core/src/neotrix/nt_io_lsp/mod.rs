...
mod tests {
    ...
    #[test]
    fn test_completion_item_construction() {
        let item = CompletionItem {
            label: "println".into(),
            kind: Some(2),
            detail: Some("macro".into()),
            documentation: Some("Prints to stdout".into()),
        };
    ...
    #[test]
    fn test_hover_info_construction() {
        let hover = HoverInfo {
            contents: "test docs".into(),
            range: Some(Range {
                start: Position {
                    line: 0,
                    character: 0,
        ...
        assert_eq!(hover.contents, "test docs");
    }

    #[test]
    fn test_path_to_uri_absolute() {
        let path = Path::new("/home/user/file.rs");
        let uri = path_to_uri(path, "file:///home/user");
    ...
    fn test_path_to_uri_relative() {
        ...
        let uri = path_to_uri(path, "file:///project");
        assert_eq!(uri, "file:///project/src/main.rs");
    }

    #[test]
    fn test_extract_position() {
        let val = serde_json::json!({"line": 42, "character": 7});
        let pos = extract_position(&val);
    ...
    fn test_extract_hover_contents_string() {
        let val = serde_json::json!("simple hover text");
        assert_eq!(extract_hover_contents(&val), "simple hover text");
    }

    #[test]
    fn test_extract_hover_contents_markdown() {
        let val = serde_json::json!({
            "kind": "markdown",
            "value": "## Title\nbody text"
        });
        assert_eq!(extract_hover_contents(&val), "## Title\nbody text");
    }

    #[test]
    fn test_extract_diagnostics_empty() {
        let val = serde_json::json!({"items": []});
        let diags = extract_diagnostics(&val);
        assert!(diags.is_empty());
    }

    #[test]
    fn test_extract_diagnostics_with_items() {
        let val = serde_json::json!({
            "items": [{
                "range": {
        ...
        assert_eq!(diags[0].message, "test error message");
        assert_eq!(diags[0].severity, DiagnosticSeverity::Error);
        assert_eq!(diags[0].source.as_deref(), Some("rustc"));
    }

    #[test]
    fn test_extract_diagnostics_array() {
        let val = serde_json::json!([{
            "range": {
                "start": {"line": 0, "character": 0},
                "end": {"line": 1, "character": 1}
    ...
    #[test]
    fn test_extract_completion_item() {
        let val = serde_json::json!({
            "label": "foo",
            "kind": 3,
            "detail": "a function"
        });
        let item = extract_completion_item(val);
        assert_eq!(item.label, "foo");
        assert_eq!(item.kind, Some(3));
        assert_eq!(item.detail.as_deref(), Some("a function"));
    }

    #[test]
    fn test_lsp_error_display() {
        assert_eq!(
            format!("{}", LspError::NotInitialized),
            "LSP client not initialized"
        );
        assert_eq!(
            format!("{}", LspError::AlreadyShutdown),
            "LSP client already shutdown"
        );
        let conn_err = LspError::ConnectionFailed("refused".into());
        assert!(format!("{}", conn_err).contains("refused"));
        let req_err = LspError::RequestFailed(-32601, "method not found".into());
        assert!(format!("{}", req_err).contains("method not found"));
    }

    #[test]
    fn test_extract_location_array() {
        let val = serde_json::json!([
        ...
        let locs = extract_location_array(&val);
    ...
    #[test]
    fn test_extract_location_array_null() {
    ...
    #[test]
    fn test_lsp_error_std_error_trait() {
        let err = LspError::IoError("disk full".into());
        let boxed: Box<dyn std::error::Error> = Box::new(err);
        assert!(format!("{}", boxed).contains("disk full"));
    }
}
