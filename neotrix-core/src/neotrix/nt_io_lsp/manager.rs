...
impl LspServerManager {
    ...
    pub async fn spawn_for_lang(&mut self, lang: &str) -> Result<&mut LspClient, LspError> {
        if self.active_servers.contains_key(lang) {
            return Ok(self.active_servers.get_mut(lang).unwrap());
        }
        let config = self.servers.get(lang).ok_or_else(|| {
            LspError::ConnectionFailed(format!("no server config for language: {}", lang))
        })?;
