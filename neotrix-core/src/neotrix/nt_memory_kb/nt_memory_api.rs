...
use super::{KnowledgeBase, KnowledgeSearchResult, SearchMatchType};
...
impl KbApiState {
    ...
    pub fn try_open_default() -> Option<Self> {
        let kb = KnowledgeBase::open(None)
            .map_err(|e| {
                log::warn!("nt_memory_api: KnowledgeBase::open(None) failed: {}", e);
            })
            .ok()?;
        Some(Self {
...
pub async fn advanced_query_handler(
    ...
    let results: Vec<KnowledgeSearchResult> = if let Some(text) = &body.text {
    ...
    let filtered: Vec<KnowledgeSearchResult> = results
        .into_iter()
        .filter(|r| {
    ...
    let paginated: Vec<KnowledgeSearchResult> =
