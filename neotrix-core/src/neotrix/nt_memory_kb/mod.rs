...
pub use nt_memory_integration::PersistKnowledgeRequest;
...
impl KnowledgeBase {
    pub fn open(path: Option<PathBuf>) -> Result<Self, String> {
        let db_path = path.unwrap_or_else(|| {
            ...
            if let Err(e) = std::fs::create_dir_all(&p) {
                log::warn!(
                    "[knowledge-base] failed to create dir {}: {}",
                    p.display(),
                    e
                );
            }
    ...
    pub fn delete_node(&self, id: &str) -> Result<bool, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r =
            nt_memory_store::delete_node(&conn, id).map_err(|e| format!("Delete error: {}", e))?;
        if r {
            self.mark_bm25_dirty();
        }
        Ok(r)
    ...
    pub fn search(&self, query: &str, limit: usize) -> Result<Vec<KnowledgeSearchResult>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        ...
        if results.len() < limit {
            ...
            let bm25_results = {
                ...
                cache
                    .as_ref()
                    .map(|bm25| bm25.search(query, limit))
                    .unwrap_or_default()
            };
    ...
    pub fn shortest_path(
        ...
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        nt_memory_graph::shortest_path(&conn, from_id, to_id, max_depth)
    ...
    pub fn enqueue_seed_urls(&self, urls: &[(&str, i64, &str)]) -> Result<usize, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r = nt_memory_crawl::enqueue_seed_urls(&conn, urls)
            .map_err(|e| format!("Enqueue error: {}", e))?;
        self.mark_bm25_dirty();
        Ok(r)
    }

    pub fn discover_wiki_category(
        &self,
        category: &str,
        max_pages: usize,
        max_depth: u32,
        ...
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r = nt_memory_crawl::discover_wiki_category_members(
            &conn, category, max_pages, max_depth, enqueue,
        )?;
        self.mark_bm25_dirty();
        Ok(r)
    }

    pub fn enqueue_search_results(
        &self,
        query: &str,
        max_results: usize,
        priority: i64,
        ...
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r = nt_memory_crawl::enqueue_search_results_from_engine(
            &conn,
            query,
            max_results,
            priority,
    ...
    pub fn run_crawl_cycle(
        &self,
        max_items: usize,
    ) -> Result<nt_memory_crawl::CrawlCycleReport, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
    ...
    pub fn validate_urls(&self, num_workers: usize) -> Result<(usize, usize), String> {
        ...
        tokio::runtime::Handle::current().block_on(nt_memory_crawl::validate_urls_parallel(
            &conn,
            &db_path,
            num_workers,
        ))
    ...
    pub fn run_crawl_cycle_parallel(
        ...
        let db_path = self.db_path.to_string_lossy().to_string();
        let r =
            tokio::runtime::Handle::current().block_on(nt_memory_crawl::run_crawl_cycle_parallel(
                &conn,
                &db_path,
                max_items,
                num_workers,
    ...
    pub fn seed_foundational(&self) -> Result<usize, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r = nt_memory_seed::seed_foundational_knowledge(&conn)
    ...
    pub fn persist_mined(
        &self,
        req: &nt_memory_integration::PersistKnowledgeRequest,
    ) -> Result<String, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let r = nt_memory_integration::persist_mined_knowledge(&conn, req);
        self.mark_bm25_dirty();
        r
    }
    ...
    pub fn import_knowledge_engine(
        &self,
        entries: &[super::nt_mind::knowledge_engine::KnowledgeEntry],
        relations: &[super::nt_mind::knowledge_engine::KnowledgeRelation],
    ...
    fn rebuild_bm25(&self) -> Result<(), String> {
        ...
        let nodes = nt_memory_store::get_all_nodes(&conn)
            .map_err(|e| format!("Rebuild BM25 error: {}", e))?;
        let docs: Vec<Bm25Document> = nodes
            .iter()
            .map(|n| {
                let text = format!(
                    "{} {} {}",
    ...
    pub fn ensure_embeddings(&self) -> Result<usize, String> {
        ...
        let config = config
            .ok_or_else(|| "Embedding config not set. Call with_embedding() first.".to_string())?;
        ...
        let pending: Vec<(String, String)> = {
            ...
            missing
                ...
                .filter_map(|id| {
                    ...
                    Some((
                        id.clone(),
                        nt_memory_embed::build_node_text(
                            &node.title,
    ...
    pub fn semantic_search(
        ...
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let all = nt_memory_embed::load_all_embeddings(&conn).map_err(|e| format!("DB: {}", e))?;

        let mut scored: Vec<(f64, String)> = all
            .iter()
            .map(|(id, vec)| {
                (
                    nt_memory_embed::cosine_similarity(&query_vec, vec),
                    id.clone(),
                )
            })
            .collect();
    ...
    pub fn hybrid_rerank_search(
        ...
        let all_embeddings =
            nt_memory_embed::load_all_embeddings(&conn).map_err(|e| format!("DB: {}", e))?;
        drop(conn);
        ...
        let emb_map: std::collections::HashMap<&str, &[f32]> = all_embeddings
            .iter()
            .map(|(id, v)| (id.as_str(), v.as_slice()))
            .collect();

        // 5. Rerank FTS results by weighted score: 0.3 × FTS + 0.7 × cosine
        let mut hybrid: Vec<(f64, KnowledgeSearchResult)> = fts_results
            .into_iter()
            .map(|r| {
                let cos = emb_map
                    .get(r.node.id.as_str())
        ...
        let mut results: Vec<KnowledgeSearchResult> = hybrid.into_iter().map(|(_, r)| r).collect();
    ...
    pub fn hybrid_rerank_search_batch(
        ...
        queries: &[&str],
        limit: usize,
    ) -> Result<Vec<Vec<KnowledgeSearchResult>>, String> {
        if queries.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for (i, query_vec) in query_vecs.iter().enumerate() {
            let q = queries[i];
            let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
            let fts_results = nt_memory_search::search_fts(&conn, q, limit * 3).unwrap_or_default();
            drop(conn);
            ...
            let mut results: Vec<KnowledgeSearchResult> =
                hybrid.into_iter().map(|(_, r)| r).collect();
            for r in &mut results {
                r.matched_on.push(SearchMatchType::VectorSimilarity);
            }
    ...
    pub fn store_evolution_record(
        &self,
        record: &nt_memory_types::EvolutionRecord,
    ) -> Result<String, String> {
        let json = serde_json::to_string(record).map_err(|e| format!("Serialize: {}", e))?;
        let summary = format!(
            "[{:?}] {} — gain: {:.2}",
            record.pattern_type, record.description, record.effectiveness_gain
        );
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let id = nt_memory_store::insert_or_get_node(
            &conn,
            &format!("EVO_PATTERN:{}", record.id),
            NodeType::Insight,
            Some(&summary),
            None,
            Some("evolution_pattern"),
        )
        .map_err(|e| format!("Insert node: {}", e))?;
        conn.execute(
            "UPDATE nodes SET metadata = ?1 WHERE id = ?2",
    ...
    pub fn get_evolution_history(
        &self,
        limit: usize,
    ) -> Result<Vec<nt_memory_types::ConversationRecord>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        ...
        for row in results {
            if let Ok(json_str) = row {
                if let Ok(rec) =
                    serde_json::from_str::<nt_memory_types::ConversationRecord>(&json_str)
                {
                    records.push(rec);
                }
            }
    ...
    pub fn get_evolution_patterns(
        &self,
        limit: usize,
    ) -> Result<Vec<nt_memory_types::EvolutionRecord>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let mut stmt = conn.prepare(
    ...
    pub fn distill_session(
        ...
        let assistant_msgs: Vec<&str> = messages
            ...
            .map(|(_, c)| c.as_str())
            .collect();
        let user_intent = user_msgs
            .first()
            .unwrap_or(&"")
            .chars()
            .take(200)
            .collect::<String>();
        let last_response = assistant_msgs
            .last()
            .map(|s| s.to_string())
            .unwrap_or_default();
        ...
        let analysis = format!(
            "Session '{}' with {} user messages, {} assistant messages",
            name,
            user_msgs.len(),
            assistant_msgs.len()
        );
