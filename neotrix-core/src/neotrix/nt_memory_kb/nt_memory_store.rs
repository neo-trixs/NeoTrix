...
pub fn update_node_metadata(
    ...
    conn.execute(
        ...
        params![metadata.to_string(), unix_now(), id],
    )?;
    Ok(())
}
...
pub fn claim_crawl_urls_batch(
    conn: &Connection,
    max_items: usize,
) -> rusqlite::Result<Vec<CrawlQueueItem>> {
    ...
    if !ids.is_empty() {
        ...
        conn.execute(
            &format!(
                "UPDATE crawl_queue SET status='processing', last_attempt={} WHERE id IN ({})",
                unix_now(),
                placeholders.join(",")
            ),
