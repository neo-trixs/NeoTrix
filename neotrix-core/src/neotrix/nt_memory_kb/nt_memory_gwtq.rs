...
use super::nt_memory_types::{KnowledgeSearchResult, NodeType};

/// 将 E8 推理状态映射到 KnowledgeBase 查询关键词
fn e8_state_to_search_query(state: ReasoningHexagram) -> Vec<&'static str> {
    let mut tags = Vec::new();
    if state.abstraction() == 0 {
        tags.push("concrete reasoning");
    } else {
        tags.push("abstract reasoning");
    }
    if state.scope() == 0 {
        tags.push("focused scope");
    } else {
        tags.push("broad scope");
    }
    if state.method() == 0 {
        tags.push("analytical method");
    } else {
        tags.push("generative method");
    }
    if state.depth() == 0 {
        tags.push("deep analysis");
    } else {
        tags.push("fast reasoning");
    }
    if state.reasoning_mode() == 0 {
        tags.push("solo reasoning");
    } else {
        tags.push("collaborative reasoning");
    }
    if state.stance() == 0 {
        tags.push("certain stance");
    } else {
        tags.push("exploratory stance");
    }
    tags
}
...
fn specialist_to_search_query(st: &SpecialistType) -> Vec<&'static str> {
    match st {
        ...
        SpecialistType::CodeAnalyzer => {
            vec!["code analysis", "static analysis", "program analysis"]
        }
        ...
        SpecialistType::CreativityGenerator => {
            vec!["creativity", "creative generation", "divergent thinking"]
        }
        SpecialistType::ReflectionEngine => {
            vec!["self reflection", "introspection", "meta cognition"]
        }
        SpecialistType::MetaCognitionAnalyst => {
            vec!["meta cognition", "self awareness", "cognitive monitoring"]
        }
...
impl super::KnowledgeBase {
    ...
    pub fn record_consciousness_snapshot(
        &self,
        phi: f64,
        coherence: f64,
        is_conscious: bool,
        ...
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let summary = format!(
            "phi={}, coherence={}, is_conscious={}, level={}, details={}",
            phi, coherence, is_conscious, level, details
        );
    ...
    pub fn query_broadcast_context(
        &self,
        content: &str,
        limit: usize,
    ) -> Result<Vec<KnowledgeSearchResult>, String> {
