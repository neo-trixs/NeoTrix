...
impl RateLimiter {
    pub fn new() -> Self {
        let limits = HashMap::from([
            (
                "youtube".into(),
                PlatformRateLimit { max_calls: 100, window_secs: 60, min_interval_ms: 200 },
            ),
            (
                "tiktok".into(),
                PlatformRateLimit { max_calls: 50, window_secs: 60, min_interval_ms: 500 },
            ),
            (
                "instagram".into(),
                PlatformRateLimit { max_calls: 30, window_secs: 60, min_interval_ms: 1000 },
            ),
            (
                "twitter".into(),
                PlatformRateLimit { max_calls: 45, window_secs: 900, min_interval_ms: 500 },
            ),
            (
                "reddit".into(),
                PlatformRateLimit { max_calls: 60, window_secs: 60, min_interval_ms: 200 },
            ),
            (
                "bilibili".into(),
                PlatformRateLimit { max_calls: 30, window_secs: 60, min_interval_ms: 500 },
            ),
        ]);
    ...
    pub fn wait_if_needed(&self, platform: &str) -> Duration {
        ...
        let last_call = self.last_call.lock().expect("rate_limit last_call lock poisoned");
        ...
        let mut windows = self.windows.lock().expect("rate_limit windows lock poisoned");
    ...
    pub fn record_call(&self, platform: &str) {
        let mut last_call = self.last_call.lock().expect("rate_limit last_call lock poisoned");
...
mod tests {
    ...
    #[test]
    fn test_rate_limiter_record_call_then_wait() {
        let rl = RateLimiter::new();
