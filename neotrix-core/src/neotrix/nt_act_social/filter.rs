...
const MAX_KEYWORD_SCORES: usize = 1_000;
...
impl SelfControlledFilter {
    ...
    pub fn apply_diversity_boost(&self, scored: Vec<ScoredMoment>) -> Vec<ScoredMoment> {
        ...
        for s in scored {
            let source_key = format!(
                "{:?}:{}",
                s.moment.platform,
                s.moment.author.as_deref().unwrap_or("unknown")
            );
    ...
    pub fn record_feedback(&self, moment_id: &str, liked: bool) {
        let mut feedback = self.user_feedback.lock().unwrap_or_else(|e| e.into_inner());
    ...
    pub fn record_keyword_feedback(&self, keyword: &str, positive: bool) {
        ...
        if scores.len() >= MAX_KEYWORD_SCORES {
            let min_key = scores.iter().min_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
                .map(|(k, _)| k.clone());
    ...
    fn calc_relevance(&self, moment: &VideoInfo, context: &FilterContext) -> f64 {
        ...
        let text = format!(
            ...
            moment.title,
            moment.description,
            moment.author.as_deref().unwrap_or("")
        );
    ...
    fn calc_diversity(&self, moment: &VideoInfo) -> f64 {
        let seen = self.seen_ids.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn tune_weights(&self) {
        let feedback = self.user_feedback.lock().unwrap_or_else(|e| e.into_inner());
        if feedback.len() < 5 {
            return;
        }
...
mod tests {
    ...
    fn test_calc_relevance_with_match() {
        let filter = SelfControlledFilter::new(FilterConfig::default());
        let ctx = FilterContext {
            interest_keywords: vec!["test".into()],
            ..Default::default()
        };
    ...
    fn test_calc_quality_with_thumbnail_and_duration() {
        ...
        let video = VideoInfo {
            ...
            published_at: None,
            thumbnail_url: Some("https://x.com/thumb.jpg".into()),
            url: "https://x.com/v".into(),
        };
