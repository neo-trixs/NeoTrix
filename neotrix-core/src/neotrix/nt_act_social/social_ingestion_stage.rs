...
use crate::neotrix::nt_mind_ingestion::{SourceType, IngestionConfig};
...
impl SocialIngestionStage {
    ...
    fn curiosity_signal(brain: &SelfIteratingBrain) -> f64 {
        ...
        let gap = brain._goal_contract.as_ref()
            .map(|g| {
        ...
        (entropy * 0.3 + curiosity * 0.4 + (1.0 - gap) * 0.2 + stagnation * 0.1)
            .clamp(0.0, 1.0)
    }
    ...
    fn inject_to_scratchpad(
        ...
        log::info!("[social-ingest] negentropy={:.3} | {}", negentropy, text.chars().take(80).collect::<String>());
    }
}
...
impl BrainStage for SocialIngestionStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        log::info!("[social-ingestion] curiosity={:.3} — activating scrape", curiosity);

        // 3. 更新已知概念
        self.refresh_known_concepts(brain);

        // 4. 爬取 X.com 时间线
        let mut guard = self.scraper.lock().map_err(|e| {
            NeoTrixError::Brain(format!("scraper lock: {}", e))
        })?;
        ...
        // 6. 负熵处理管线
        let known = self.known_concepts.lock()
            .map(|kc| kc.clone())
            .unwrap_or_default();

        let mut stream = self.stream.lock().map_err(|e| {
            NeoTrixError::Brain(format!("stream lock: {}", e))
        })?;

        let absorbed = stream.process_timeline(
            &timeline.tweets,
            &known,
            curiosity,
            5,
        ...
        for score in &absorbed {
            ...
            let keywords = super::tweet_stream::TweetStream::extract_keywords(&score.raw_tweet.text);
            if let Ok(mut kc) = self.known_concepts.lock() {
                for kw in keywords {
                    if !kc.contains(&kw) {
                        kc.push(kw);
        ...
        log::info!(
            ...
            absorbed.len(), total_gain, reward_bonus,
        );
...
mod tests {
    ...
    #[test]
    fn test_curiosity_signal_zero() {
        // can only test formula without brain
        assert!(true);
    }
    ...
    fn test_inject_to_scratchpad_sets_pad() {
        ...
        if let Some(ref pad) = brain._ingestion_scratchpad {
            ...
            assert_eq!(pad.source_type, SourceType::Social);
        }
    }
}
