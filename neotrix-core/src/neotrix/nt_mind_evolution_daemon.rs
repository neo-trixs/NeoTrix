...
use super::nt_core_iit_phi::IITPhiCalculator;
...
impl PersistentIssueTracker {
    pub fn new(_storage_pathh: &str) -> Self {
        Self {
            issues: Vec::new(),
            counter: 0,
        }
    ...
    pub fn mark_stale(&mut self, max_cycles: u64, current_cycle: u64) {
        for ti in self.issues.iter_mut() {
            if current_cycle.saturating_sub(ti.last_seen_at) > max_cycles {
                if matches!(
                    ti.lifecycle,
                    IssueLifecycle::Fixed(_) | IssueLifecycle::Failed(_)
                ) {
...
const MAX_PHI_HISTORY: usize = 2000;
const MAX_COHERENCE_HISTORY: usize = 2000;
...
impl EvolutionDaemon {
        ...
        for id in unattempted {
            if let Some(ti) = self.tracker.issues.iter().find(|t| t.id == id) {
                ...
                let result = match ti.issue_type {
                    IssueType::MissingTests if !file.is_empty() => AutoFixer::add_test_stub(file),
                    ...
                    IssueType::TodoLeftovers => AutoFixer::cleanup_todos("src/lib.rs")
                ...
                match result {
                    Ok(msg) => {
                        ...
                        if self.config.verbose {
                            log::info!("[daemon] 🔧 fixed {}: {}", id, msg);
                        }
                    }
                    Err(e) => {
                        self.tracker.record_failure(&id, &e);
                        if self.config.verbose {
                            log::warn!("[daemon] ⚠ fix failed {}: {}", id, e);
                        }
                    }
                }
            }
    ...
    pub fn compute_jepa_energy(&self) -> f64 {
        let unresolved = self
            .tracker
            .get_unattempted(self.config.max_fix_attempts)
            .len() as f64;
        ...
        for (idx, _fe) in &ordered {
            if let Some(goal) = goals.get(*idx) {
                ...
                let fix_result = match goal.category {
                    ...
                    GoalCategory::Knowledge => continue,
                };
                ...
                if fix_result.is_ok() {
                    ...
                    if self.phi_reward_history.len() > MAX_PHI_HISTORY * 2 {
                        self.phi_reward_history
                            .drain(0..self.phi_reward_history.len() - MAX_PHI_HISTORY);
                    }
                    let causal_coherence = self.compute_causal_coherence();
                    self.causal_coherence_history.push(causal_coherence);
                    if self.causal_coherence_history.len() > MAX_COHERENCE_HISTORY * 2 {
                        self.causal_coherence_history
                            .drain(0..self.causal_coherence_history.len() - MAX_COHERENCE_HISTORY);
                    }
...
mod tests {
    ...
    fn test_register_and_mark_fixed() {
        ...
        let id = d
            .tracker
            .register_issue(Some("foo.rs".into()), IssueType::MissingTests);
    ...
    fn test_register_and_record_failure() {
        ...
        let id = d
            .tracker
            .register_issue(Some("foo.rs".into()), IssueType::CompileWarning);
    ...
    fn test_max_failures_mark_failed() {
        ...
        let id = d
            .tracker
            .register_issue(Some("foo.rs".into()), IssueType::Other);
        for _ in 0..3 {
    ...
    fn test_get_unattempted_filters_marked() {
        ...
        let id1 = d
            .tracker
            .register_issue(Some("a.rs".into()), IssueType::MissingTests);
        let id2 = d
            .tracker
            .register_issue(Some("b.rs".into()), IssueType::LargeFile);
    ...
    fn test_stale_marking() {
        ...
        let id = d
            .tracker
            .register_issue(Some("x.rs".into()), IssueType::Other);
        d.tracker.mark_fixed(&id, 1);
        d.cycle_count = 20;
    ...
    fn test_causal_coherence_increases_with_fixes() {
        ...
        d.tracker
            .register_issue(Some("test.rs".into()), IssueType::MissingTests);
        // Mark it as fixed
