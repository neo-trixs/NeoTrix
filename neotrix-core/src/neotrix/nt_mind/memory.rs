...
pub use crate::core::nt_core_bank::{
    MemoryDetailedStats, MemoryIterationResult, MemoryLifecycle, MemoryTier, ReasoningBank,
    ReasoningBankStats, ReasoningMemory, TemporalContext,
};
...
impl ReasoningBank {
    /// Enable Kronecker-structured O(N log N) cleanup for approximate
    /// nearest-neighbor recall. Dimension must match the embedding dimension
    /// (typically 64).
    pub fn with_kronecker(mut self, dim: usize) -> Self {
        self.set_kronecker(KroneckerCleanup::new(dim));
        self
    }
    /// 初始化外部仓库分析知识库（2026-05-14 11-repo 深度分析注入）
        ...
        let repo_knowledge = vec![
            (
                "Hyperframes: HTML-native video rendering framework. Write HTML+CSS+GSAP compositions, \
                 preview in nt_world_browse, render to MP4 via Puppeteer+FFmpeg. Deterministic capture, \
            ...
            (
                "Yao Webnt_shield Skill: AI agent skill for website nt_shield audit with 275 built-in \
            ...
            (
                "Botasaurus: All-in-one Python web scraping framework. Three decorators: \
                 @nt_world_browse (anti-detection Chrome driver), @request (humane HTTP), @task (generic). \
    ...
    /// 初始化中国古代宇宙论/数理知识（2026-05-24 深度研究注入）
...
//                 从'阻滞'到'全流通'——解释重子生成和CP破坏通过双曲偏差(Hyperbolic Bias)。\
    ...
    pub fn initialize_with_awesome_design_knowledge(&mut self) {
        ...
        log::info!(
            "[knowledge] AwesomeDesignSkills: {} seed memories injected",
            count
        );
    }
}
