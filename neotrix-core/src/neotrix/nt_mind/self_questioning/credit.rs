...
impl AdcaCreditAssigner {
    pub fn assign(trajectory: &ExplorationTrajectory, outcome_score: f64) -> CreditAssignment {
        ...
        let positive_steps: Vec<usize> = step_credits
            .iter()
            .filter(|s| s.advantage > 0.0)
            .map(|s| s.step_index)
            .collect();
        let negative_steps: Vec<usize> = step_credits
            .iter()
            .filter(|s| s.advantage < 0.0)
            .map(|s| s.step_index)
            .collect();
        let trajectory_id = format!(
            "traj-b{}-d{}",
            trajectory.breadth_phase_steps, trajectory.depth_phase_steps,
        );
        CreditAssignment {
            trajectory_id,
            step_credits,
    ...
    fn llm_attribution(trajectory: &ExplorationTrajectory) -> Vec<StepCredit> {
        ...
        for i in 0..total_steps {
            ...
            let key_tokens: Vec<String> =
                action.split_whitespace().map(|t| t.to_string()).collect();
...
mod tests {
    use super::*;
    use crate::neotrix::nt_mind::self_questioning::types::{
        EntityDesc, EnvironmentProfile, OperationDesc,
    };
    ...
    fn test_no_state_change_penalty() {
        let traj = make_trajectory(vec!["s0", "s0", "s0"], vec!["step1", "step2", "step3"]);
        let assignment = AdcaCreditAssigner::assign(&traj, 0.5);
    ...
    fn test_primacy_higher_attribution() {
        let traj = make_trajectory(vec!["s0", "s1", "s2"], vec!["first", "second", "third"]);
