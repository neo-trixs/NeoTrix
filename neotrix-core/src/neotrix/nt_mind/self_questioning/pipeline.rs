...
impl SelfQuestioningPipeline {
        ...
        match domain {
            "code-analysis" => EnvironmentProfile {
                ...
                entities: vec![
                    EntityDesc {
                        name: "Function".into(),
                        attributes: vec!["signature".into(), "body".into(), "complexity".into(), "dependencies".into()],
                        parent: None,
                    },
                    EntityDesc {
                        name: "Variable".into(),
                        attributes: vec!["name".into(), "type".into(), "scope".into(), "mutability".into()],
                        parent: None,
                    },
                    EntityDesc {
                        name: "Module".into(),
            ...
            "web-design" => EnvironmentProfile {
                ...
                entities: vec![
                    EntityDesc {
                        name: "Component".into(),
                        attributes: vec!["props".into(), "state".into(), "children".into(), "style".into()],
                        parent: None,
                    },
                    EntityDesc {
                        name: "Page".into(),
    ...
    pub fn explore(
        ...
        for i in 0..b_steps {
            ...
            states.push(format!("state_b_{}_entity_{}_attr_{}", i, entity.name, attr));
            actions.push(format!("examine({}, {})", entity.name, attr));
            observations.push(format!("Found attribute {} of {} with value pattern", attr, entity.name));
        }
    ...
    pub fn synthesize_tasks(&self, traj: &ExplorationTrajectory) -> Vec<GeneratedTask> {
        ...
        for (i, entity) in profile.entities.iter().enumerate() {
            ...
            tasks.push(GeneratedTask {
                ...
                source_trajectory_id: format!("traj-{}", traj.breadth_phase_steps + traj.depth_phase_steps),
            });
        }
    ...
    pub fn curate(
        ...
        for task in tasks {
            ...
            let proxy_reward = if passed_dedup && passed_feasibility && judge_score >= self.config.min_judge_score {
                judge_score * 0.9 + 0.1
            } else {
                judge_score * 0.3
            };
    ...
    pub fn explore_with_guidance(
        ...
        for i in 0..b_steps {
            ...
            if !patterns.is_empty() && (i as f64 / b_steps as f64) < bias_factor {
                ...
                action = format!("guided_{}({}, {})", pattern.trim_start_matches("pattern:"), entity.name, attr);
            } else {
        ...
        for j in 0..d_steps {
            ...
            if let Some(hit) = pattern_hit {
                action = format!("guided_execute({}, {}) <- bias_from({})", op.name, param, hit);
            } else {
