...
mod tests {
    ...
    use super::super::{
        EvolutionArchive, SelectionConfig, ParentSelection, ArchiveEntry, FileDiff,
        MetaAgent, MetaAgentConfig, SelfModificationProposal, ModificationTarget,
        SafetyCheckResult, StagedEvaluation, HyperAgent,
    };
    ...
    fn test_archive_select_latest() {
        ...
        let latest = archive.select_by_latest().expect("should select latest entry");
    ...
    fn test_archive_select_random() {
        ...
        assert!(r.is_some());
        assert!(["e1", "e2"].contains(&r.expect("random selection should return an entry").id.as_str()));
    }
    ...
    fn test_archive_prune_low_score() {
        let config = SelectionConfig { archive_capacity: 10, ..Default::default() };
    ...
    fn test_meta_agent_filter_diff_protected() {
        ...
        assert!(result.expect_err("filter_diff should error on blocked path").contains("blocked"));
    ...
    fn test_meta_agent_safety_check() {
        let config = MetaAgentConfig { self_referential: false, ..Default::default() };
        let agent = MetaAgent::new(config);
    ...
    fn test_meta_agent_should_continue_budget() {
        let config = MetaAgentConfig { budget: 5, ..Default::default() };
        let agent = MetaAgent::new(config);
        assert!(agent.should_continue());

        let mut agent = MetaAgent::new(MetaAgentConfig { budget: 3, ..Default::default() });
        agent.iteration = 3;
        assert!(!agent.should_continue());
    ...
    fn test_archive_lineage_tracking() {
        ...
        let parent_ref = archive.select_by_best().expect("should select best entry for lineage");
        ...
        let grandchild_lineage = {
            let child_ref = archive.entries.iter().find(|e| e.id == "c1").expect("c1 entry should exist in archive");
        ...
        let gc = archive.entries.iter().find(|e| e.id == "gc1").expect("gc1 entry should exist in archive");
    ...
    fn test_meta_agent_run_generation_basic() {
        let config = MetaAgentConfig { budget: 3, ..Default::default() };
    ...
    fn test_forward_zero_iterations() {
        let agent = MetaAgent::new(MetaAgentConfig::default());
        let proposals = agent.forward("repo", "eval", 0);
    ...
    fn test_prune_to_capacity() {
        let config = SelectionConfig { archive_capacity: 2, ..Default::default() };
        let mut archive = EvolutionArchive::new(config);
        archive.add(make_entry("e1", 0.9, 0, 100));
        archive.add(make_entry("e2", 0.8, 0, 200));
