...
impl SideGit {
    pub fn new() -> Self {
        ...
        Self {
            snapshots_dir: PathBuf::from(home).join(".neotrix").join("snapshots"),
            initialized: false,
            snapshot_count: 0,
        }
    }
    ...
    pub fn init(&mut self) -> Result<(), String> {
        ...
        if !dir.join(".git").exists() {
            ...
            if !output.status.success() {
                return Err(format!("git init error: {}", String::from_utf8_lossy(&output.stderr)));
            }
    ...
    pub fn snapshot_file(&mut self, file_path: &Path, workspace_root: &Path) -> Result<String, String> {
        if !self.initialized {
            self.init()?;
        }

        let rel_path = file_path.strip_prefix(workspace_root)
            .unwrap_or(file_path);
        ...
        let msg = format!("snapshot {}: {}", self.snapshot_count + 1, rel_path.display());
    ...
    pub fn snapshot_files(&mut self, file_paths: &[PathBuf], workspace_root: &Path) -> Result<u64, String> {
        let mut count = 0;
    ...
    pub fn snapshots_dir(&self) -> &Path { &self.snapshots_dir }
}
...
mod tests {
    ...
    fn test_new_defaults() {
        ...
        assert!(sg.snapshots_dir().to_string_lossy().contains(".neotrix/snapshots"));
    }
