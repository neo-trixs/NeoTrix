...
use super::cortex_memory::{CortexMemory, DimensionTag, MemoryTrace, Modality};
use super::goal_loop::GoalConfig;
use super::goal_loop::GoalLoop;
use super::predictive_cortex::{HorizonForecast, PredictiveCortex};
...
use crate::neotrix::nt_expert_routing::{TaskType, WorldModelV2};
...
impl PanoramaPipeline {
    ...
    pub fn lookahead_scan(&mut self, brain: &SelfIteratingBrain, horizon: usize) -> LookaheadTag {
        let features: Vec<f64> = brain
            .brain
            .capability
            .arr
            .iter()
        ...
        let prob = self
            .predictive_cortex
            .lookahead_anomaly_prob(&features, horizon);
    ...
    pub fn run_cycle(
        ...
        if let Some(ref last_forecast) = self.last_forecast.clone() {
            self.predictive_cortex
                .record_outcome(last_forecast, &latent);
        ...
        let new_forecast = self
            .predictive_cortex
            .predict_horizon(&features.iter().take(32).copied().collect::<Vec<_>>(), 3);
        ...
        let mem = ReasoningMemory::new(
            ...
            &[
                MicroEdit::AdjustDimension(
                    "prediction_energy".into(),
                    fe_report.prediction_energy.min(1.0),
                ),
    ...
    fn store_prediction(&mut self, latent: &[f64], fe: &FreeEnergyReport, phi: &PhiReport) {
        ...
        self.hypercube.insert(
            ...
            &format!(
                "pred_{}_fe={:.3}_phi={:.3}",
                self.cycle, fe.variational_fe, phi.phi
            ),
        );
        ...
        let trace = MemoryTrace::new(
            ...
            "nt_world_model",
            &format!(
                "JEPA latent pred energy={:.4} phi={:.4}",
                fe.prediction_energy, phi.phi
            ),
