...
impl KnowledgeFreshness {
    ...
    pub fn should_rescan(&self) -> bool {
        matches!(
            self,
            KnowledgeFreshness::Stale | KnowledgeFreshness::Expired
        )
    }
}
...
impl KnowledgeAging {
    ...
    pub fn run_aging_cycle(&mut self) -> AgingReport {
        ...
        AgingReport {
            ...
            stale_count,
            expired_count,
            avg_age_days: if self.entries.is_empty() {
                0.0
            } else {
                total_age_days / (self.entries.len() + stale_count + expired_count) as f64
            },
