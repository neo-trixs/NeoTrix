use crate::neotrix::nt_mind::cortex_types::{
    ActionPlan, HorizonForecast, PredictedStep, PredictiveCortex, ANOMALY_ENERGY_THRESHOLD,
    CONFIDENCE_LOW_THRESHOLD, FE_DIVERGENCE_THRESHOLD,
};

impl PredictiveCortex {
    pub fn predict_horizon(&mut self, current_latent: &[f64], horizon: usize) -> HorizonForecast {
    ...
    pub fn lookahead_anomaly_prob(&mut self, current_latent: &[f64], horizon: usize) -> f64 {
        let forecast = self.predict_horizon(current_latent, horizon);
        if forecast.anomaly_predicted {
            let min_conf = forecast
                .trajectory
                .iter()
                .map(|s| s.confidence)
