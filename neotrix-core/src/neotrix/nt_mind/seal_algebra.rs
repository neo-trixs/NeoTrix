...
impl SEALAlgebra {
    ...
    pub fn validate_micro_edit(&self, edit: &MicroEdit) -> bool {
        if let MicroEdit::AdjustDimension(_, amount) = edit {
            ...
            pairs
                .iter()
                .all(|(_, amount)| amount.abs() <= self.max_adjustment)
        } else {
...
mod tests {
    ...
    fn test_validate_convergence() {
        ...
        // 大幅度，可能不收敛
        let invalid_edits = vec![MicroEdit::AdjustDimension("typography".to_string(), 1.5)];
        assert!(
            !algebra.validate_convergence(&invalid_edits),
            "大幅度应该不收敛"
        );
    }
