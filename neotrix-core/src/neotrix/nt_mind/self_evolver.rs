...
impl SelfEvolver {
    ...
    pub fn evolve_from_url(&mut self, url: &str) -> NeoTrixResult<f64> {
        log::info!("🌐 evolve_from_url: {}", url);
        // 1. 信息获取
        let local_path = self.fetch_information(url)?;
        log::info!("📁 Local path: {}", local_path.display());

        // 2. 深度分析
        let analysis = self.analyze_local_path(&local_path)?;
        log::info!(
            "📊 Analysis: type={}, confidence={}, edits_count={}",
            analysis.item_type,
            analysis.confidence,
            analysis.suggested_edits.len()
        );

        // 3. 生成 MicroEdit 序列
        let micro_edits = self.generate_micro_edits(&analysis)?;
        log::info!("🔧 Micro edits count: {}", micro_edits.len());
        for (i, edit) in micro_edits.iter().enumerate() {
            log::info!("  Edit {}: {:?}", i, edit);
        }

        // 4. 应用到 ReasoningBrain
        log::info!(
            "🧠 Before apply: quality_gates={:.3}",
            self.brain.capability.arr[IDX_QUALITY_GATES]
        );
        for edit in &micro_edits {
            self.apply_micro_edit(edit);
        }
        log::info!(
            "🧠 After apply: quality_gates={:.3}",
            self.brain.capability.arr[IDX_QUALITY_GATES]
        );

        // 5. 保存到 brain.json
        let reward = self.calculate_reward(&analysis);
        log::info!("💾 Saving brain.json...");
        self.brain.save()?;
        log::info!("✅ Saved brain.json, reward={:.3}", reward);

        // 6. 存储到 ReasoningBank
        let memory = ReasoningMemory {
            id: uuid::Uuid::new_v4().to_string(),
            task_description: format!("Evolve from: {}", analysis.source_url),
            task_type: crate::neotrix::nt_expert_routing::TaskType::CodeAnalysis,
    ...
    fn fetch_information(&self, url: &str) -> NeoTrixResult<PathBuf> {
        let parsed =
            Url::parse(url).map_err(|e| NeoTrixError::Network(format!("无效URL: {}", e)))?;
        ...
        if parsed.domain() == Some("github.com") {
            ...
            let tmp = target_file.with_extension("tmp");
            std::fs::write(&tmp, content)?;
            std::fs::rename(&tmp, &target_file)?;
            Ok(target_file)
        }
    }
    ...
    fn fetch_http(url: &str) -> NeoTrixResult<String> {
        ...
        resp.text()
            .map_err(|e| NeoTrixError::Network(format!("读取响应失败: {}", e)))
    }
    ...
    fn analyze_local_path(&self, path: &PathBuf) -> NeoTrixResult<AnalysisResult> {
        ...
        if path.is_dir() {
            ...
            let stream_count = [has_code, has_docs, has_xref]
                .iter()
                .filter(|&&b| b)
                .count();
    ...
    fn analyze_code_stream(
        &self,
        repo_path: &Path,
        analysis: &mut ThreeStreamAnalysis,
    ) -> NeoTrixResult<()> {
        // 1. 检测包管理器文件
        ...
        if cargo_path.exists() {
            if let Ok(content) = std::fs::read_to_string(&cargo_path) {
                analysis
                    .code_insights
                    .push("Rust project (Cargo.toml)".to_string());
                if content.contains("tokio") {
                    analysis.code_edits.push(MicroEdit::AdjustDimension(
                        "inference_depth".to_string(),
                        0.08,
                    ));
                    analysis
                        .code_insights
                        .push("Async runtime: tokio".to_string());
                }
                if content.contains("wgpu") || content.contains("vulkano") {
                    analysis
                        .code_edits
                        .push(MicroEdit::AdjustDimension("experimental".to_string(), 0.07));
                }
            }
        }
        ...
        if pkg_path.exists() {
            if let Ok(content) = std::fs::read_to_string(&pkg_path) {
                ...
                if content.contains("tailwindcss") {
                    analysis.code_edits.push(MicroEdit::AdjustDimension(
                        "tailwind_proficiency".to_string(),
                        0.10,
                    ));
                }
                if content.contains("react-aria") || content.contains("radix-ui") {
                    analysis.code_edits.push(MicroEdit::AdjustDimension(
                        "react_aria_usage".to_string(),
                        0.10,
                    ));
                }
            }
        ...
        if src_path.is_dir() {
            ...
            if let Ok(entries) = std::fs::read_dir(&src_path) {
                for entry in entries.flatten() {
                    if entry
                        .path()
                        .extension()
                        .is_some_and(|e| e == "rs" || e == "ts" || e == "tsx" || e == "py")
                    {
                        module_count += 1;
                    }
                }
            ...
            analysis
                .code_insights
                .push(format!("Source modules: {}", module_count));
            if module_count > 20 {
                analysis.code_edits.push(MicroEdit::AdjustDimension(
            ...
            if module_count == 0 {
                analysis
                    .code_insights
                    .push("No source files found in src/".to_string());
            }
        }
    ...
    fn analyze_docs_stream(content: &str) -> (Vec<MicroEdit>, Vec<String>) {
        ...
        if c.contains("mcp") || c.contains("copilot") {
            edits.push(MicroEdit::AdjustDimension(
                "semantic_layer".to_string(),
                0.12,
            ));
            edits.push(MicroEdit::AdjustDimension(
                "ai_native_states".to_string(),
                0.10,
            ));
        ...
        // Testing
        if c.contains("test")
            && (c.contains("tdd") || c.contains("coverage") || c.contains("pytest"))
        {
    ...
    fn analyze_insights_stream(
        &self,
        current: &ThreeStreamAnalysis,
        out: &mut ThreeStreamAnalysis,
    ) {
        // 1. 文档描述了但代码没有的功能
        if current.docs_insights.is_empty() && !current.code_insights.is_empty() {
            out.cross_references
                .push("Doc gap: 项目有代码但缺少文档".to_string());
        }

        // 2. 代码中没有的文档声称功能
        if !current.docs_insights.is_empty() && current.code_insights.is_empty() {
            out.cross_references
                .push("Code gap: 文档描述的功能在代码中未找到".to_string());
        }
        ...
        if current
            .code_insights
            .iter()
            .any(|i| i.contains("Source modules"))
        {
            out.cross_references
                .push("Code analysis complete: module structure detected".to_string());
        }
        ...
        if [has_rust, has_python, has_node]
            .iter()
            .filter(|&&b| b)
            .count()
            > 1
        {
            out.gap_analysis
                .push("Multi-language: 跨语言项目需要统一API规范".to_string());
        }
    }
            ...
            MicroEdit::AdjustDimension(dim, delta) => {
                if let Some(idx) = CapabilityVector::index_from_name(dim) {
                    ...
                    *val = (*val + delta).clamp(0.0, 1.0);
                    log::info!(
                        "🔧 AdjustDimension: dim={}, idx={}, old={:.3}, delta={:.3}, new={:.3}",
                        dim,
                        idx,
                        old_val,
                        delta,
                    ...
                    log::warn!("❌ AdjustDimension: unknown dim={}", dim);
                }
            }
            MicroEdit::NormalizeVector => {
                log::info!(
                    "🔧 NormalizeVector: before quality_gates={:.3}",
                    self.brain.capability.arr[IDX_QUALITY_GATES]
                );
                self.brain.capability.normalize();
                log::info!(
                    "🔧 NormalizeVector: after quality_gates={:.3}",
                    self.brain.capability.arr[IDX_QUALITY_GATES]
                );
            }
            _ => {}
    ...
    fn test_evolve_all_repos() {
        ...
        for url in urls {
            match evolver.evolve_from_url(url) {
                Ok(reward) => log::info!("✅ Evolved from {}: reward={:.3}", url, reward),
                Err(e) => log::error!("❌ Failed to evolve from {}: {}", url, e),
            }
        }
    }
