...
impl SpatialModel {
        ...
        for zone in &features.presence {
            *self.heatmap.entry(zone.zone_id.clone()).or_insert(0.0) += zone.probability * 0.1;
            if zone.probability > 0.6 {
                let occupant = self
                    .occupants
                    .entry(format!("occ_{}", zone.zone_id))
                    .or_insert(OccupantTrace {
                        current_zone: None,
                        first_seen: ts,
                        last_seen: ts,
                        total_visits: 0,
    ...
    pub fn occupant_count(&self) -> usize {
        ...
        self.occupants
            .values()
            .filter(|o| o.last_seen > cutoff)
            .count()
    }

    pub fn zone_heatmap(&self) -> &HashMap<String, f64> {
        &self.heatmap
    }
...
mod tests {
    ...
    fn test_wifi_sensing_engine_creation() {
        let engine = WifiSensingEngine::new(
            SensingSource::Simulation {
                interval_secs: 1,
                simulate_people: 1,
            },
    ...
    fn test_simulate_tick() {
        let mut engine = WifiSensingEngine::new(
            SensingSource::Simulation {
                interval_secs: 1,
                simulate_people: 1,
            },
        ...
        let vs = features
            .vital_signs
            .expect("vital_signs should be ok in test");
    ...
    fn test_spatial_model_occupant_tracking() {
        ...
        let features = SpatialFeatures {
            ...
            presence: vec![ZonePresence {
                zone_id: "test_zone".into(),
                probability: 0.9,
                person_count: 1,
                confidence: 0.85,
            }],
