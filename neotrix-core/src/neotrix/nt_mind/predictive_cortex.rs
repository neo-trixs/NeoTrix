...
impl PredictiveCortex {
    ...
    pub fn record_outcome(&mut self, forecast: &HorizonForecast, actual_latent: &[f64]) -> OutcomeRecord {
        self.cycle += 1;

        let predicted = forecast.trajectory.first()
...
mod tests {
    ...
    fn make_test_latent(dim: usize) -> Vec<f64> {
        (0..dim).map(|i| (i as f64 / dim as f64) * 2.0 - 1.0).collect()
    }
    ...
    fn test_plan_best_action() {
        ...
        let actions: Vec<Vec<f64>> = (0..3).map(|i| {
            (0..32).map(|j| ((i + j) as f64 / 32.0) * 2.0 - 1.0).collect()
        }).collect();
    ...
    fn test_counterfactual_vs_baseline_differs() {
        ...
        let baseline_last_fe = baseline.trajectory.last().map(|s| s.free_energy).unwrap_or(0.0);
    ...
    fn test_detect_degradation_triggers_after_bad_predictions() {
        ...
        assert!(signal.is_some(), "degraded cortex should produce repair signal");
    ...
    fn test_repair_signal_has_reason() {
        ...
        assert!(signal.diagnosis.contains("PredictiveCortex"),
            "diagnosis should reference PredictiveCortex: {}", signal.diagnosis);
        assert!(signal.diagnosis.contains("quality"),
            "diagnosis should mention quality: {}", signal.diagnosis);
    }
