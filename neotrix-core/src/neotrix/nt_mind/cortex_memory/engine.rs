...
impl CortexMemory {
    ...
    /// Max entries in trace_map before pruning oldest 20%
    const MAX_TRACE_MAP: usize = 100_000;
    ...
    pub fn store(&mut self, mut trace: MemoryTrace) -> String {
        ...
        if trace.embedding.is_none() {
            let text_for_embed =
                format!("{} {} {}", trace.title, trace.summary, trace.tags.join(" "));
            trace.embedding = Some(self.embedder.embed(&text_for_embed));
        }

        // Prune indexes when trace_map exceeds capacity
        if self.trace_map.len() >= Self::MAX_TRACE_MAP {
            let remove_count = Self::MAX_TRACE_MAP / 5;
            let ids_to_remove: Vec<String> =
                self.trace_map.keys().take(remove_count).cloned().collect();
            for rid in &ids_to_remove {
                self.trace_map.remove(rid);
                self.layer_index.values_mut().for_each(|s| {
                    s.remove(rid);
                });
                self.dimension_index.values_mut().for_each(|s| {
                    s.remove(rid);
                });
                self.modality_index.values_mut().for_each(|s| {
                    s.remove(rid);
                });
                self.source_index.values_mut().for_each(|s| {
                    s.remove(rid);
                });
                self.timeline_index.values_mut().for_each(|s| {
                    s.remove(rid);
                });
            }
        }
        ...
        self.modality_index
            .entry(trace.modality.name().to_string())
            .or_default()
            .insert(id.clone());

        self.source_index
            .entry(trace.source_type.clone())
            .or_default()
            .insert(id.clone());
    ...
    pub fn consolidate_cms(&mut self, iteration: u64, config: &CmsConfig) -> CmsResult {
        ...
        if iteration.is_multiple_of(config.topic_frequency as u64) {
            result.topic_to_event =
                self.consolidate_layer(MemoryLayer::Topic, config.event_threshold);
        }
        if iteration.is_multiple_of(config.event_frequency as u64) {
            result.event_to_fact =
                self.consolidate_layer(MemoryLayer::Event, config.fact_threshold);
        }
        result.topic_layer_size = self
            .layer_index
            .get(&MemoryLayer::Topic)
            .map(|s| s.len())
            .unwrap_or(0);
        result.event_layer_size = self
            .layer_index
            .get(&MemoryLayer::Event)
            .map(|s| s.len())
            .unwrap_or(0);
        result.fact_layer_size = self
            .layer_index
            .get(&MemoryLayer::Fact)
            .map(|s| s.len())
            .unwrap_or(0);
        result
    }
    ...
    pub fn query_by_dimension(&self, dim: DimensionTag, limit: usize) -> Vec<&MemoryTrace> {
        self.dimension_index
            .get(&dim)
            .map(|ids| {
                let mut traces: Vec<&MemoryTrace> =
                    ids.iter().filter_map(|id| self.trace_map.get(id)).collect();
                traces.sort_by(|a, b| {
                    b.importance
    ...
    pub fn query_by_dimensions(&self, dims: &[DimensionTag], limit: usize) -> Vec<&MemoryTrace> {
        ...
        let mut traces: Vec<&MemoryTrace> = all_ids
            .iter()
    ...
    pub fn recall(&mut self, query: &str, limit: usize) -> Vec<(&MemoryTrace, f64)> {
        ...
        let mut scored: Vec<(&MemoryTrace, f64)> = self
            .trace_map
            .values()
            .filter_map(|trace| {
    ...
    pub fn get_associations(&self, trace_id: &str, limit: usize) -> Vec<&MemoryTrace> {
        self.trace_map
            .get(trace_id)
            .map(|trace| {
                let mut assoc: Vec<&MemoryTrace> = trace
                    .associations
                    .iter()
    ...
    pub fn dimension_chain(&self, category: &str, limit: usize) -> Vec<&MemoryTrace> {
        let dims: Vec<DimensionTag> = DimensionTag::all()
            .into_iter()
    ...
    pub fn stats(&self) -> CortexStats {
        let per_dim: HashMap<String, usize> = self
            .dimension_index
            .iter()
            .map(|(k, v)| (format!("{:?}", k), v.len()))
            .collect();
        let per_modality: HashMap<String, usize> = self
            .modality_index
            .iter()
    ...
    pub fn report(&self) -> String {
        ...
        r.push_str(&format!(
            "短期缓冲: {} / {}\n",
            self.nt_world_sense_buffer.len(),
            self.nt_world_sense_capacity
        ));
        r.push_str(&format!(
            "长期记忆: {} / {}\n",
            self.long_term.len(),
        ...
        for cat in &categories {
            ...
            if !traces.is_empty() {
                ...
                for t in traces {
                    let dims: Vec<String> =
                        t.dimensions.iter().map(|d| format!("{:?}", d)).collect();
                    r.push_str(&format!(
                        "  • {} [{:?}] ← {}\n    {}\n",
                        t.title,
                        t.modality,
    ...
    pub fn export_json(&self) -> serde_json::Value {
        let traces: Vec<&MemoryTrace> = self
            .long_term
            .iter()
            .chain(self.nt_world_sense_buffer.iter())
            .collect();
    ...
    pub fn create_hyperedge(&mut self, hyperedge_id: &str, trace_ids: &[&str]) -> bool {
        ...
        self.hyperedge_index
            .insert(hyperedge_id.to_string(), existing);
        true
    }
    ...
    pub fn retrieve_coarse_to_fine(
        &mut self,
        query: &str,
        k: usize,
    ) -> (Vec<MemoryTrace>, Vec<MemoryTrace>, Vec<MemoryTrace>) {
        ...
        let topic: Vec<MemoryTrace> = topic_scores
            .into_iter()
            .take(k)
            .map(|(_, t)| t.clone())
            .collect();
        let event: Vec<MemoryTrace> = event_scores
            .into_iter()
            .take(k)
            .map(|(_, t)| t.clone())
            .collect();
        let fact: Vec<MemoryTrace> = fact_scores
            .into_iter()
            .take(k)
            .map(|(_, t)| t.clone())
            .collect();
...
pub fn inject_from_web_miner(
    ...
    let mut tags: Vec<String> = vec![source_type.to_string(), format!("{}_sources", source_type)];
    for (dim, _) in edits {
        tags.push(dim.clone());
    }

    let trace = MemoryTrace::new(title, source_url, summary, Modality::Text, dims)
        .with_tags(tags)
