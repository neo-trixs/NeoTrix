...
mod tests {
    ...
    fn test_default_configs_contain_known_servers() {
        ...
        assert!(names.contains(&"typescript-language-server"), "missing typescript-language-server");
    ...
    fn test_config_rust_has_cargo_toml() {
        let mgr = LspManager::new();
        let rc = mgr.configs.iter().find(|c| c.name == "rust-analyzer").unwrap();
    ...
    fn test_send_request_on_missing_server() {
        let mut mgr = LspManager::new();
        assert!(mgr.send_request("missing", "textDocument/hover", serde_json::json!({})).is_none());
    }
}
