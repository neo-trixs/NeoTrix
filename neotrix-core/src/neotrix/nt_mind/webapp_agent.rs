...
impl ExtractedContent {
    ...
    pub fn summary(&self, max_chars: usize) -> String {
        ...
        if !self.headings.is_empty() {
            parts.push(format!(
                "📑 {}",
                self.headings[..self.headings.len().min(5)].join(" > ")
            ));
        }
...
impl WebAppRegistry {
    ...
    pub fn ingest_from_browser(
        ...
        let knowledge = CollectedKnowledge {
            ...
            source_type: if WebAppAgent::detect(url, title).is_some() {
                "webapp".into()
            } else {
                "webpage".into()
            },
...
mod tests {
    ...
    fn test_detect_unknown() {
        assert_eq!(WebAppAgent::detect("https://example.com", "Example"), None);
    }
    ...
    fn test_extract_metadata() {
        ...
        let extracted = ExtractedContent::from_html(html, "");
        assert_eq!(
            extracted.metadata.get("description").map(|s| s.as_str()),
            Some("Test desc")
        );
    }
    ...
    fn test_remove_agent() {
        ...
        let ag_id = reg
            .detect_or_create("https://web.whatsapp.com", "WhatsApp")
            .unwrap()
            .id
            .clone();
