...
impl TierPromptBuilder {
    ...
    pub fn build_context_prompt(tier: ContextTier, query: &str) -> String {
        ...
        let instruction = match tier {
            ContextTier::Small => "Brief search. Return max 3 key findings. Be concise.",
            ContextTier::Medium => "Balanced search. Return top findings organized by relevance.",
            ContextTier::Large => {
        ...
        format!("{}\n{}\n\nContext query: {}", header, instruction, query)
    }

    pub fn build_impact_prompt(tier: ContextTier, target: &str) -> String {
        let header = Self::tier_header(tier);
        let instruction = match tier {
            ContextTier::Small => "Quick impact scan. Show only direct dependencies (depth 1-2).",
            ContextTier::Medium => "Standard impact analysis. Include depth 3.",
            ContextTier::Large => "Deep impact. Full transitive dependencies.",
            ContextTier::Massive => {
                "Exhaustive. All depths, all edge types, architectural implications."
            }
        };
        format!("{}\n{}\n\nImpact target: {}", header, instruction, target)
    }
    ...
    pub fn build_system_prompt(tier: ContextTier, tool_name: &str, query: &str) -> String {
        ...
        let (task_label, prompt_body) = match tool_name {
            "context" | "search" => ("Context Gathering", Self::build_context_prompt(tier, query)),
            "impact" | "dependencies" => {
                ("Impact Analysis", Self::build_impact_prompt(tier, query))
            }
            "architecture" => (
                "Architecture Analysis",
                Self::build_architecture_prompt(tier),
            ),
            "quality" | "lint" | "review" => {
            ...
            other => {
                let fallback =
                    format!("Generic analysis using tool '{}'.\nQuery: {}", other, query);
                ("Analysis", fallback)
            }
        };
        format!(
...
mod tests {
    ...
    fn test_context_prompt_includes_query() {
        let prompt =
            TierPromptBuilder::build_context_prompt(ContextTier::Medium, "find the error handler");
    ...
    fn test_system_prompt_unknown_tool_falls_back() {
        let prompt = TierPromptBuilder::build_system_prompt(
            ContextTier::Small,
            "custom_tool",
            "do something",
        );
