...
use super::clawbench::{ClassificationResult, TrajectoryClassifier, TrajectoryDynamics};
...
impl ClawBenchReport {
    pub fn display(&self) {
        log::info!("╭─ ClawBench Trajectory Classification ──────────────╮");
        log::info!(
            "│ Accuracy:  {:.3}  Precision: {:.3}              │",
            self.accuracy,
            self.precision
        );
        log::info!(
        ...
        log::info!("│                      SA    LC    ND                  │");
        log::info!(
            "│ StrangeAttractor   {:>4}  {:>4}  {:>4}              │",
            self.confusion_matrix[0][0],
            self.confusion_matrix[0][1],
            self.confusion_matrix[0][2]
        );
        log::info!(
            "│ LimitCycle         {:>4}  {:>4}  {:>4}              │",
            self.confusion_matrix[1][0],
            self.confusion_matrix[1][1],
            self.confusion_matrix[1][2]
        );
        log::info!(
            "│ NormalDiffusion    {:>4}  {:>4}  {:>4}              │",
            self.confusion_matrix[2][0],
            self.confusion_matrix[2][1],
            self.confusion_matrix[2][2]
        );
        log::info!("├──────────────────────────────────────────────────────┤");
        log::info!("│ Per-Class Stats:                                     │");
...
const ACTIONS_A: &[&str] = &[
    "read", "write", "search", "delete", "verify", "plan", "refactor", "test", "deploy", "monitor",
];
...
impl ClawBenchBenchmark {
    pub fn run() -> ClawBenchReport {
        ...
        for (class_idx, dyn_ty) in [
            TrajectoryDynamics::StrangeAttractor,
            TrajectoryDynamics::LimitCycle,
            TrajectoryDynamics::NormalDiffusion,
        ]
        .iter()
        .enumerate()
        {
...
mod tests {
    ...
    fn test_gen_strange_attractor_high_variance() {
        ...
        assert!(
            result.reward_variance > 0.2,
            "variance {} should be > 0.2 for strange attractor",
            result.reward_variance
        );
        assert!(
            result.action_switches > 0.4,
            "switch ratio {} should be > 0.4",
            result.action_switches
        );
    }
    ...
    fn test_gen_limit_cycle_alternating() {
        ...
        assert_eq!(
            result.dynamics,
            TrajectoryDynamics::LimitCycle,
            "limit cycle generation should be classified as LimitCycle, got {:?}",
            result.dynamics
        );
    }
    ...
    fn test_gen_normal_diffusion_low_variance() {
        ...
        let result = classifier.classify(&rewards, &actions);
        assert!(
            result.reward_variance < 0.1,
            "variance {} should be < 0.1 for normal diffusion",
            result.reward_variance
        );
    }
    ...
    fn test_confusion_matrix_total_matches_samples() {
        let report = ClawBenchBenchmark::run();
        let total: usize = report.confusion_matrix.iter().flat_map(|r| r.iter()).sum();
    ...
    fn test_per_class_has_all_three() {
        ...
        assert!(report
            .per_class
            .contains_key(&TrajectoryDynamics::StrangeAttractor));
        assert!(report
            .per_class
            .contains_key(&TrajectoryDynamics::LimitCycle));
        assert!(report
            .per_class
            .contains_key(&TrajectoryDynamics::NormalDiffusion));
    }
