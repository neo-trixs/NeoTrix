...
use crate::neotrix::nt_memory_kb::{KnowledgeBase, PersistKnowledgeRequest};
...
impl WebKnowledgeMiner {
    ...
    pub fn mine_url(&mut self, url_str: &str) -> NeoTrixResult<WebMinedKnowledge> {
        ...
        const MAX_PROCESSED: usize = 100000;
        if self.processed.len() > MAX_PROCESSED {
            let keys: Vec<String> = self.processed.keys().take(1000).cloned().collect();
            for k in keys {
    ...
    pub fn mine_all_persist(
        ...
        if let Some(ref kb) = self.nt_memory_kb {
            for mined in &self.mined_history {
                ...
                let _ = kb.persist_mined(&PersistKnowledgeRequest {
                    title: mined.title.clone(),
                    summary: mined.summary.clone(),
                    url: mined.source_url.clone(),
                    source_type: source_name.to_string(),
                    confidence: mined.confidence,
                    edits: mined.edits.clone(),
                    insights: mined.extracted_insights.clone(),
                });
            }
        }
        result
        ...
        for (_i, url) in urls.iter().enumerate() {
            match self.mine_url(url) {
                Ok(knowledge) => {
                    ...
                    const MAX_MINED_HISTORY: usize = 10000;
                    if self.mined_history.len() > MAX_MINED_HISTORY {
                        let excess = self.mined_history.len() - MAX_MINED_HISTORY;
                        self.mined_history.drain(0..excess);
                    }
                    let edits_count = knowledge.edits.len();
                    total_edits += edits_count;

                    // 注册知识来源
                    brain.register_knowledge_source(
                        &knowledge.source_name,
                        Self::edits_to_vector(&knowledge.edits),
                    );
                    ...
                    let micro_edits: Vec<MicroEdit> = knowledge
                        .edits
                        .iter()
    ...
    fn mine_arxiv(&self, url_str: &str) -> NeoTrixResult<WebMinedKnowledge> {
        let parsed =
            Url::parse(url_str).map_err(|_| NeoTrixError::Network("无效 arXiv URL".to_string()))?;
        let paper_id = parsed
            .path()
            .trim_end_matches('/')
            .split('/')
        ...
        // 简易 XML 解析
        let title =
            extract_xml_tag(&xml_text, "title").unwrap_or_else(|| format!("arXiv:{}", paper_id));
    ...
    fn mine_github(&self, url_str: &str) -> NeoTrixResult<WebMinedKnowledge> {
        ...
        let title = parsed_json["full_name"]
            .as_str()
            .unwrap_or(&repo_full)
            .to_string();
        let desc = parsed_json["description"]
            .as_str()
            .unwrap_or("")
            .to_string();
        let lang = parsed_json["language"]
            .as_str()
            .unwrap_or("unknown")
            .to_string();
        let topics: Vec<String> = parsed_json["topics"]
            .as_array()
            .map(|arr| {
                arr.iter()
        ...
        if desc_lower.contains("ai")
            || desc_lower.contains("machine learning")
            || desc_lower.contains("deep learning")
        {
    ...
    fn mine_generic(&self, url_str: &str) -> NeoTrixResult<WebMinedKnowledge> {
        ...
        Ok(WebMinedKnowledge {
            ...
            source_name: format!(
                "web_{}",
                title.chars().take(20).collect::<String>().replace(' ', "_")
            ),
    ...
    fn analyze_wikipedia_content(title: &str, extract: &str) -> (Vec<(String, f64)>, Vec<String>) {
        ...
        // 文明/历史
        if lower.contains("civilization")
        ...
        // 时间线/进化
        if lower.contains("evolution")
            || lower.contains("timeline")
            || lower.contains("chronology")
            || lower.contains("million years")
            || lower.contains("billion years")
        {
            edits.push(("inference_depth".to_string(), 0.07));
        ...
        if title_lower.contains("history")
            || title_lower.contains("time")
            || title_lower.contains("evolution")
        {
            edits.push(("synthesis".to_string(), 0.05));
        }
        if title_lower.contains("dimension")
            || title_lower.contains("universe")
            || title_lower.contains("physics")
        {
    ...
    pub fn stats(&self) -> WebMinerStats {
        let by_type: HashMap<WebSourceType, usize> =
            self.mined_history
                .iter()
...
mod tests {
    ...
    fn test_web_source_type_detect() {
        assert_eq!(
            WebSourceType::detect("https://en.wikipedia.org/wiki/History"),
            WebSourceType::Wikipedia
        );
        assert_eq!(
            WebSourceType::detect("https://arxiv.org/abs/2506.10943"),
            WebSourceType::ArXiv
        );
        assert_eq!(
            WebSourceType::detect("https://github.com/rust-lang/rust"),
            WebSourceType::GitHub
        );
        assert_eq!(
            WebSourceType::detect("https://example.com/page"),
            WebSourceType::GenericUrl
        );
        assert_eq!(
            WebSourceType::detect("https://knowledge-base.com"),
            WebSourceType::KnowledgeBase
        );
    }
