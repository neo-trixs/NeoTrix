        ...
        if format_upper == "YAML" || format_upper == "YML" {
            if let Ok(toml_section_re) = Regex::new(r"(?m)^\[(\w[\w_.-]*)\]$") {
                ...
                if !has_toml_sections {
                    ...
                    summary.push_str(&format!("# {} Configuration\n\n", format_upper));
                    let yaml_kv =
                        Regex::new(r"(?m)^\s*([a-zA-Z_][\w_-]*)\s*:\s*(.+)$").expect("hardcoded yaml kv regex");
            ...
            let toml_section_re = Regex::new(r"(?m)^\[(\w[\w_.-]*)\]$").expect("hardcoded toml section regex");
            let toml_kv_re = Regex::new(r"(?m)^(\w[\w_-]*)\s*=\s*(.+)$").expect("hardcoded toml kv regex");
