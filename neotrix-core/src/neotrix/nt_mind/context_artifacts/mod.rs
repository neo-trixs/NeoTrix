...
pub use store::{ArtifactBuilder, ArtifactStore};
pub use types::{Artifact, ArtifactSourceConfig, ArtifactType, ArtifactsConfig};
...
"#;
    ...
    fn test_indexer_with_valid_config() {
        ...
        std::fs::write(
            &config_path,
            serde_json::to_string_pretty(&config).expect("value should be ok in test"),
        )
        .expect("write config");
