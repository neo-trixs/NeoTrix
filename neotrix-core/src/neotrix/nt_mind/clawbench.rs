...
impl TrajectoryClassifier {
    ...
    fn detect_limit_cycle(&self, rewards: &[f64]) -> bool {
        ...
        let directions: Vec<i32> = rewards
            .windows(2)
            .map(|w| {
                if w[1] > w[0] {
                    1
...
mod tests {
    ...
    fn test_normal_diffusion() {
        ...
        let actions = make_actions(&[
            "read", "read", "search", "read", "write", "write", "write", "verify", "verify", "done",
        ]);
    ...
    fn test_strange_attractor() {
        ...
        let actions = make_actions(&[
            "write", "delete", "search", "write", "delete", "search", "write", "delete", "search",
            "write",
        ]);
        let result = classifier.classify(&rewards, &actions);
    ...
    fn test_limit_cycle() {
        ...
        let actions = make_actions(&[
            "search", "search", "write", "write", "search", "search", "write", "write", "search",
        ]);
        let result = classifier.classify(&rewards, &actions);
    ...
    fn test_trajectory_analysis_report() {
        ...
        assert_eq!(
            *report
                .dynamics_counts
                .get(&TrajectoryDynamics::NormalDiffusion)
        ...
        assert_eq!(
            *report
                .dynamics_counts
                .get(&TrajectoryDynamics::StrangeAttractor)
                .expect("value should be ok in test"),
            1
        );
