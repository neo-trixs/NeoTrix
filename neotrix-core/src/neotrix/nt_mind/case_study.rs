    ...
    fn test_full_case_study() {
        ...
        let cs = writer.write(
            ...
            vec![
                (
                    "Quantization",
                    "Converted FP32 to INT8",
                    vec![("Model size reduction".to_string(), 0.75)],
                ),
                (
                    "Batching",
                    "Implemented dynamic batching",
                    vec![("Throughput increase".to_string(), 3.5)],
                ),
            ],
            vec![
                (
                    "Latency",
                    "Reduced to 920ms",
                    vec![("Improvement".to_string(), 0.6)],
                ),
                (
                    "Cost",
                    "40% reduction in compute costs",
                    vec![("Savings".to_string(), 0.4)],
                ),
            ],
