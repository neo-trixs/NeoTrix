...
pub fn save_router_config(config: &RouterConfig, path: Option<PathBuf>) -> Result<(), String> {
    ...
    std::fs::write(&tmp, &toml_str).map_err(|e| format!("写入失败: {}", e))?;
...
pub struct RouterConfig {
    ...
    pub fallback_chain: Vec<RouterTier>,
...
fn has_math_indicators(s: &str) -> bool {
    s.contains('+') && (s.contains('=') || s.contains('x') || s.contains('y'))
        ...
        || s.contains("矩阵")
        || s.contains("vector")
        || s.contains("algorithm complexity")
        || s.contains("复杂度")
}
...
fn calc_code_ratio(s: &str) -> f64 {
    ...
    if s.is_empty() {
        ...
        code_chars as f64 / s.len() as f64
    }
}
...
impl ModelRouter {
    ...
    fn learn_and_adjust(&mut self) {
        ...
        for tier in [
            ...
            // 如果成功率太低，下次同类请求上升一级
            if rate < self.adaptive_params.downgrade_threshold && tier < RouterTier::T4 {
                let next_tier = RouterTier::from_int(tier as u8 + 1);
                log::info!(
                    "[router] auto upgrade {:?} -> {:?} (success rate {:.1}%)",
                    tier,
                    next_tier,
                    rate * 100.0
                );
            ...
            // 如果成功率很高，可考虑降级省钱
            if rate > self.adaptive_params.upgrade_threshold && tier > RouterTier::T0 {
                let prev_tier = RouterTier::from_int(tier as u8 - 1);
                log::info!(
                    "[router] auto downgrade {:?} -> {:?} (success rate {:.1}%)",
                    tier,
                    prev_tier,
                    rate * 100.0
                );
...
impl ModelRouter {
    ...
    /// 失败时回退到更低 tier
    pub fn fallback(&self, current_tier: RouterTier) -> Option<TierModelMapping> {
        let idx = self
            .config
            .fallback_chain
            .iter()
...
mod tests {
    ...
    fn test_greeting_routes_to_t0() {
        ...
        assert_eq!(d.tier, RouterTier::T0);
    }
    ...
    fn test_complex_reasoning_routes_to_t4() {
        ...
        assert_eq!(d.tier, RouterTier::T2);
    }
    ...
    fn test_code_review_routes_to_t3() {
        ...
        assert!(d.tier >= RouterTier::T2);
    }

    #[test]
    fn test_fallback_chain() {
        let router = ModelRouter::new();
        let fb = router.fallback(RouterTier::T4);
        assert!(fb.is_some());
        assert!(matches!(
            fb.expect("fb should be ok in test").tier,
            RouterTier::T3
        ));
    }
