...
impl SmartContentExtractor {
    ...
    fn readability_extract(html: &str) -> Option<(Vec<ContentBlock>, String, ExtractionTier)> {
        ...
        let sections: Vec<&str> = body_html
            .split_inclusive("</div>")
    ...
    fn extract_metadata(html: &str) -> HashMap<String, String> {
        ...
        let re = Regex::new(r#"<meta\s[^>]*?(?:property|name)=["']([^"']+)["'][^>]*?content=["']([^"']*)["'][^>]*>"#).expect("Regex::new failed for meta tag pattern - check regex syntax");
        ...
        let re2 = Regex::new(r#"<meta\s[^>]*?content=["']([^"']*)["'][^>]*?(?:property|name)=["']([^"']+)["'][^>]*>"#).expect("Regex::new failed for meta alt pattern - check regex syntax");
    ...
    fn detect_list(html: &str) -> bool {
        let ul_re = Regex::new(r"<ul\b[^>]*>[\s\S]*?</ul>").expect("Regex::new failed for <ul> pattern - check regex syntax");
        let ol_re = Regex::new(r"<ol\b[^>]*>[\s\S]*?</ol>").expect("Regex::new failed for <ol> pattern - check regex syntax");
        ul_re.find_iter(html).count() + ol_re.find_iter(html).count() >= 2
    }

    fn detect_table(html: &str) -> bool {
        let table_re = Regex::new(r"<table\b[^>]*>[\s\S]*?</table>").expect("Regex::new failed for <table> pattern - check regex syntax");
        table_re.find_iter(html).count() >= 1
    }

    fn count_links(html: &str) -> usize {
        let link_re = Regex::new(r#"<a\b[^>]*href=["']https?://[^"']+["']"#).expect("Regex::new failed for <a href> pattern - check regex syntax");
...
fn lazy_html_content<'a>(
    html: &'a str,
    tags: &[&str],
) -> Option<(Vec<ContentBlock>, String, ExtractionTier)> {
    for tag in tags {
...
fn aria_content(html: &str) -> Option<(Vec<ContentBlock>, String, ExtractionTier)> {
    let re =
        Regex::new(r#"(<div\b[^>]*?role=["'](?:main|article|region)["'][^>]*?>[\s\S]*?</div>)"#)
            .ok()?;
    let mut blocks = Vec::new();
    for cap in re.captures_iter(html) {
...
fn extract_tag_content<'a>(html: &'a str, tag: &str) -> Option<String> {
    ...
    html.find(&open).and_then(|start| {
        ...
        html[cs..]
            .find(&close)
            .map(|end| html[cs..cs + end].trim().to_string())
    })
}
...
mod tests {
    ...
    fn test_strip_html_removes_script_style() {
        let html =
            "<html><script>alert('x')</script><style>.cls{}</style><p>Real content</p></html>";
    ...
    fn test_extraction_quality_score() {
        let high_quality = SmartContentExtractor::score_quality(
            "Long text content with enough substance to pass the quality threshold. "
                .repeat(50)
                .as_str(),
            &format!(
