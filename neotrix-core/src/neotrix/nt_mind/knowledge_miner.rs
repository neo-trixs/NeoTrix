...
impl KnowledgeMiner {
            ...
            match self.mine_single(&url) {
                Ok(knowledge) => {
                    ...
                    brain.register_knowledge_source(
                        ...
                        knowledge.capability_vector.clone(),
                    );
                    ...
                    let memory = ReasoningMemory {
                        ...
                        task_description: format!(
                            "KnowledgeMined: {} ({})",
                            knowledge.source_name, knowledge.domain
                        ),
                        ...
                        lifecycle: crate::neotrix::nt_mind::memory::MemoryLifecycle::new(
                    ...
                    self.mined_sources
                        .insert(url.clone(), knowledge.source_name.clone());
                    results.push(knowledge);
                }
                Err(e) => {
                    log::error!("[KnowledgeMiner] Failed to mine {}: {}", url, e);
                }
            }
        }
    ...
    fn mine_single(&self, url: &str) -> NeoTrixResult<MinedKnowledge> {
        let parsed =
            Url::parse(url).map_err(|e| NeoTrixError::Network(format!("无效 URL: {}", e)))?;
    ...
    fn analyze_readme(content: &str) -> (Vec<MicroEdit>, Vec<String>) {
        ...
        if lower.contains("design system")
            || lower.contains("design tokens")
            || lower.contains("component library")
        {
            edits.push(MicroEdit::AdjustDimension(
                "compound_composition".to_string(),
                0.08,
    ...
    pub fn clean_processed(&mut self) {
        self.discovery_queue
            .retain(|url| !self.processed.contains(url));
    }
}
...
mod tests {
    use super::*;
    use crate::core::nt_core_util::TestDir;

    fn miner_with_tmpdir() -> (KnowledgeMiner, TestDir) {
        let td = TestDir::new();
        let miner = KnowledgeMiner::new(td.path().to_path_buf());
        (miner, td)
    }

    #[test]
    fn test_knowledge_miner_new() {
        let (miner, _td) = miner_with_tmpdir();
    ...
    #[test]
    fn test_enqueue() {
        let (mut miner, _td) = miner_with_tmpdir();
    ...
    fn test_domain_to_task_type() {
        assert_eq!(
            KnowledgeMiner::domain_to_task_type("frontend"),
            TaskType::UIDesign
        );
        assert_eq!(
            KnowledgeMiner::domain_to_task_type("backend"),
            TaskType::CodeGeneration
        );
        assert_eq!(
            KnowledgeMiner::domain_to_task_type("ai-ml"),
            TaskType::CodeAnalysis
        );
        assert_eq!(
            KnowledgeMiner::domain_to_task_type("general"),
            TaskType::General
        );
    }
    ...
    fn test_generate_report_nonempty() {
        let (miner, _td) = miner_with_tmpdir();
        let results = vec![MinedKnowledge {
    ...
    fn test_stats() {
        ...
        miner
            .mined_sources
            .insert("u1".to_string(), "s1".to_string());
