...
impl ActionSequenceMiner {
    ...
    pub fn mine(&self, traces: &[ToolTrace]) -> Vec<SkillProposal> {
        ...
        clusters
            ...
            .map(|steps| {
                let count =
                    sequences.iter().filter(|s| *s == &steps).count() as f64;
...
impl PreferenceTracker {
    ...
    pub fn record_signal(&mut self, signal_type: &str, value: f64) {
        ...
        self.signal_history.push((signal_type.to_string(), value, ts));
    }
            ...
            match signal_type.as_str() {
                ...
                _ => {
                    if signal_type.starts_with("task_type:") {
                        ...
                        if *value > 0.3 && !self.profile.preferred_task_types.contains(&task.to_string()) {
...
impl PolicyUpdater {
        ...
        match trend {
            Trend::Improving => {
                ...
                self.regularization_weight =
                    (self.regularization_weight * 0.95).max(0.01);
            }
            Trend::Declining => {
                self.learning_rate = (self.learning_rate * 0.8).max(0.001);
                self.regularization_weight =
                    (self.regularization_weight * 1.05).min(1.0);
            }
            Trend::Stable => {}
        }
    ...
    fn detect_trend(rewards: &[f64]) -> Trend {
        ...
        let sum_xy: f64 = rewards
            .iter()
            .enumerate()
            .map(|(i, y)| i as f64 * y)
            .sum();
        let sum_xx: f64 = (0..rewards.len())
...
impl FullDimensionEvolver {
    ...
    pub fn evolve_brain(
        ...
        if combined.is_empty() {
            return (self.policy_updater.learning_rate, self.policy_updater.regularization_weight);
        }
    ...
    pub fn evolve(
        ...
        let internal_reward =
            self.reward_collector.ema_by_task_type("general");
...
mod tests {
    ...
    fn test_preference_tracker_update() {
        ...
        assert!(
            (tracker.profile.context_depth_preference - 0.5).abs() < 0.01
        );
        assert!(
            (tracker.profile.verbosity_preference - 0.5).abs() < 0.01
        );
