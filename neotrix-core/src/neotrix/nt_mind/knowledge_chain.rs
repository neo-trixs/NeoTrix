    ...
    fn absorb_knowledge(
        ...
        for knowledge in sources {
            ...
            self.chain_history.push(ChainRecord {
                ...
                success: reward > 0.01,
                details: format!(
                    "domain={}, confidence={:.2}, reward={:.3}",
                    knowledge.domain, knowledge.confidence, reward
                ),
                timestamp: chrono::Utc::now().timestamp(),
                reward,
            });
            const MAX_CHAIN_HISTORY: usize = 10000;
            if self.chain_history.len() > MAX_CHAIN_HISTORY {
                let excess = self.chain_history.len() - MAX_CHAIN_HISTORY;
                self.chain_history.drain(0..excess);
            }
            ...
            let memory = ReasoningMemory::new(
                &format!(
                    "KnowledgeChain: {} from {}",
                    knowledge.source_name, knowledge.source_url
                ),
                task_type,
...
mod tests {
    ...
    fn test_knowledge_chain_new() {
        ...
        let chain = KnowledgeChain::new(dir.path().to_path_buf());
    ...
    fn test_init_default_discovery() {
        let dir = tempfile::tempdir().unwrap();
        let mut chain = KnowledgeChain::new(dir.path().to_path_buf());
    ...
    fn test_knowledge_chain_phase_ordering() {
        ...
        let phases = vec![
            Discovery, Mining, Validation, Absorption, Storage, Reporting,
        ];
