...
impl GapAnalyzer {
    ...
    pub fn analyze(
        ...
        seeds.sort_by(|a, b| b.priority.partial_cmp(&a.priority).unwrap_or(std::cmp::Ordering::Equal));
        seeds
    }
...
mod tests {
    ...
    fn test_analyze_capability_only_no_router() {
        let mut brain = ReasoningBrain::new();
        brain.capability = CapabilityVector::from_array(&vec![0.5; 23]).expect("value should be ok in test");
        // 使 inference_depth 弱
