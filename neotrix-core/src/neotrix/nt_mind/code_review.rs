...
impl IssueCategory {
    pub fn label(&self) -> &'static str {
        match self {
            ...
            IssueCategory::UnsafeCode => "Unsafe Code",
        ...
        for (i, line) in code.lines().enumerate() {
                ...
                && !line.trim().starts_with("#[")
            {
                issues.push(ReviewIssue {
                    ...
                    message: "[OWASP A08:2025] 使用 .unwrap() 可能导致 panic（异常未处理）"
                        .to_string(),
                    line: Some((i + 1) as u32),
                    suggestion: Some(
                        "替换为 .expect(\"msg\") 或 ? 操作符 + 统一 NeoTrixError".to_string(),
                    ),
                });
            }
        }
        ...
        for (i, line) in code.lines().enumerate() {
            if line.contains("unsafe {") && !line.trim().starts_with("//") {
                issues.push(ReviewIssue {
                    ...
                    line: Some((i + 1) as u32),
                    suggestion: Some(
                        "添加 Safety: 注释说明为什么 unsafe 是安全的; 考虑使用安全抽象替代"
                            .to_string(),
                    ),
                });
            }
        ...
        for (i, line) in code.lines().enumerate() {
                ...
                || trimmed.contains("\"/etc/")
            {
                issues.push(ReviewIssue {
                    ...
                    line: Some((i + 1) as u32),
                    suggestion: Some(
                        "使用 std::env::temp_dir() 或可配置项替代; 环境变量: $HOME, $TMPDIR"
                            .to_string(),
                    ),
                });
            }
            ...
            for (pattern, label) in &secret_patterns {
                if trimmed.contains(pattern) && (trimmed.contains('=') || trimmed.contains(": \""))
                {
                    issues.push(ReviewIssue {
                        ...
                        category: IssueCategory::Security,
                        message: format!(
                            "[OWASP A04:2025] 检测到可能硬编码的 {} (line {})",
                            label,
                            i + 1
                        ),
                        line: Some((i + 1) as u32),
                        suggestion: Some(
                            "使用环境变量或加密 vault 替代硬编码; 审核 git history 是否已泄露"
                                .to_string(),
                        ),
                    });
                    break;
...
impl CodeReviewLoop {
    ...
    pub fn iterate(
        ...
        let converged =
            report.score >= self.quality_target || self.iteration as usize >= self.max_iterations;
    ...
    pub fn compute_seal_reward(&self) -> f64 {
        ...
        let prev = self
            ...
            .expect("history has at least 2 entries");
    ...
    pub fn delta_improvement(&self) -> f64 {
        ...
        self.history
            .last()
            .expect("history.len() >= 2 per guard")
            .score
...
mod tests {
    ...
    fn test_detect_unsafe() {
        ...
        assert!(report
            .issues
            .iter()
            .any(|i| i.category == IssueCategory::UnsafeCode));
    }
    ...
    fn test_owasp_a05_detect_command_injection() {
        ...
        assert!(report
            .issues
            .iter()
            .any(|i| i.message.contains("shell 注入")));
    }
