...
use crate::core::nt_core_epoch::{
    all_frameworks, evaluate_in_epoch, CognitiveFramework, EarthEpoch, FrameworkRoute,
};
use crate::core::{CapabilityVector, TaskType};
...
impl PanoramicBrain {
    ...
    pub fn evaluate_with_rubric(&self, task: &str, _task_type: Option<TaskType>) -> SkillLens {
        ...
        SkillLens {
            instruction_compliance: count_matches(&[
                "instruction",
                "follow",
                "comply",
                "obey",
    ...
    pub fn route_task(
        ...
        let mut weights: Vec<(EarthEpoch, f64)> = EarthEpoch::all()
            .iter()
            .map(|epoch| {
                let fw = self
                    .frameworks
        ...
        let primary = weights
            .first()
            .map(|(e, _)| *e)
            .unwrap_or(EarthEpoch::E7Network);
    ...
    pub fn absorb_reward(&mut self, task: &str, task_type: TaskType, reward: f64) {
        ...
        self.activation_log
            .push((primary, task.to_string(), reward));
        const MAX_ACTIVATION_LOG: usize = 10000;
        if self.activation_log.len() > MAX_ACTIVATION_LOG {
            self.activation_log.drain(0..MAX_ACTIVATION_LOG / 5);
        }
    ...
    fn sync_to_legacy(&mut self) {
        if let Some(fw) = self.frameworks.get(&self.active_epoch) {
            ...
            self.legacy_capability.set_inference_depth(
                (self.legacy_capability.inference_depth()
                    + avg_state * self.active_epoch as u8 as f64 / 8.0)
                    / 2.0,
            );
            ...
            self.legacy_capability.set_synthesis(
                (self.legacy_capability.synthesis() + avg_state * fw.average_reward().max(0.3))
                    / 2.0,
            );
    ...
    pub fn epoch_report(&self) -> Vec<(EarthEpoch, f64, f64, usize)> {
        let mut report: Vec<(EarthEpoch, f64, f64, usize)> = EarthEpoch::all()
            .iter()
            .map(|epoch| {
                let fw = self
                    .frameworks
    ...
    pub fn transfer_knowledge(&mut self, from: EarthEpoch, to: EarthEpoch, rate: f64) {
        let source_state = self.frameworks.get(&from).map(|fw| fw.state.clone());
...
mod tests {
    ...
    fn test_panoramic_brain_initializes_all_epochs() {
        ...
        assert_eq!(
            brain.frameworks.len(),
            8,
            "All 8 epochs should be initialized"
        );
        for epoch in EarthEpoch::all() {
            assert!(
                brain.frameworks.contains_key(&epoch),
    ...
    fn test_routing_returns_weights_for_all_epochs() {
        let brain = PanoramicBrain::new();
        let route = brain.route_task(
            "analyze experimental data",
            Some(TaskType::CodeAnalysis),
            false,
        );
        assert_eq!(
            route.weights.len(),
            8,
            "Should have weights for all 8 epochs"
        );
    ...
    fn test_evaluate_task_returns_valid_scores() {
        ...
        assert!(
            ...
            "Primary score {} should be in [0,1]",
            primary
        );
    ...
    fn test_absorb_reward_updates_epoch_state() {
        ...
        if let Some(fw) = brain.framework(route.primary) {
            assert!(
                fw.activation_count > 0,
                "Primary epoch {:?} should have activation count > 0",
                route.primary
            );
        }
    }
    ...
    fn test_strengthen_epoch() {
        ...
        assert!(
            after > before,
            "Epoch router bias should increase after strengthening"
        );
    }
    ...
    fn test_knowledge_transfer_between_epochs() {
        ...
        let _e7_state_before = brain
            .framework(EarthEpoch::E7Network)
            .expect("value should be ok in test")
            .state
            .clone();
        let e6_state_before = brain
            .framework(EarthEpoch::E6Planetary)
            .expect("value should be ok in test")
            .state
            .clone();
        ...
        let e6_state_after = brain
            .framework(EarthEpoch::E6Planetary)
            .expect("value should be ok in test")
            .state
            .clone();
        // E6 should have moved toward E7 in at least some dimensions
        let movement: f64 = e6_state_after
            .iter()
            .zip(e6_state_before.iter())
            .map(|(a, b)| (a - b).abs())
            .sum();
        assert!(
            movement > 0.0,
            "Knowledge transfer should change E6 state towards E7"
        );
    }
    ...
    fn test_routing_prefers_matching_epoch() {
        ...
        let route = brain.route_task(
            "rigorous experimental measurement with statistical verification",
            Some(TaskType::CodeAnalysis),
            false,
        );
        let e4_weight = route
    ...
    fn test_evaluate_with_rubric_keyword_matching() {
        ...
        assert!(
            lens.creativity > 0.0,
            "Creativity should be >0 for creative task"
        );
        assert!(
            lens.planning > 0.0,
            "Planning should be >0 for planning task"
        );
        ...
        assert!(
            lens2.instruction_compliance > 0.0,
            "Compliance should be >0"
        );

        // Generic task with no keywords
        let lens3 = brain.evaluate_with_rubric("xyz unknown task", None);
        assert!(
            (lens3.average() - 0.0).abs() < 1e-10,
            "Generic task should score near 0"
        );
    }
    ...
    fn test_route_task_with_rubric_modifies_weights() {
        ...
        assert!(
            (total_with - total_no).abs() > 0.001 || total_with == total_no,
    ...
    fn test_evaluate_task_with_rubric_boosts_score() {
        ...
        assert!(
            score_no >= 0.0 && score_no <= 1.0,
            "Base score {} should be in [0,1]",
            score_no
        );
        assert!(
            score_with >= 0.0 && score_with <= 1.0,
            "Rubric score {} should be in [0,1]",
            score_with
        );
    }
