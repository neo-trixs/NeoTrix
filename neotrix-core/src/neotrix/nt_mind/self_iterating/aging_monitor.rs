...
impl AgingMonitor {
    ...
    pub fn record_snapshot(&mut self, iteration: u64, capability: &crate::core::CapabilityVector) {
        ...
        self.capability_history
            .push_back((iteration, capability.to_full_vector()));
    }
    ...
    pub fn detect_interference(&self, brain: &SelfIteratingBrain) -> f64 {
        ...
        let similarity_sum: f64 = mem_vec
            ...
            .sum();
        let avg_diff: f64 = if memory_count > 1.0 {
            similarity_sum / memory_count
        } else {
            0.0
        };
...
impl Default for AgingDiagnosisStage {
    fn default() -> Self {
        Self
    }
}
impl AgingDiagnosisStage {
    pub fn new() -> Self {
        Self
    }
}
impl BrainStage for AgingDiagnosisStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        {
            let monitor = &mut brain.seal_rl.aging_monitor;
            ...
            monitor.interference_score = if memory_count >= 5 && !memory_pairs.is_empty() {
                ...
                (1.0 - similarity_sum / memory_count as f64 * 2.0)
                    .max(0.0)
                    .min(1.0)
            } else {
                0.0
            ...
            monitor.maintenance_score = if total_absorbs >= 5 {
                ...
                if n >= 3 {
                    match (
                        monitor.capability_history.front(),
                        monitor.capability_history.back(),
                    ) {
        ...
        let report = brain.seal_rl.aging_monitor.diagnostic_report();
        log::info!("[aging] {}", report);

        if let Some(alert) = brain.seal_rl.aging_monitor.has_alert() {
