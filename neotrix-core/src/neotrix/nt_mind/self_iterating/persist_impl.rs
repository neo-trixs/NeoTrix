...
use super::brain_impl::{BrainMetadata, DefaultSealStrategy, ReasoningBrain};
...
impl ReasoningBrain {
    ...
    pub fn save_to_dir(&self, base_dir: Option<&std::path::Path>) -> NeoTrixResult<()> {
        ...
        std::fs::write(&brain_tmp, &brain_data)?;
        let _ = std::fs::set_permissions(&brain_tmp, std::fs::Permissions::from_mode(0o600));
        std::fs::rename(&brain_tmp, &brain_path)?;
        ...
        let meta_tmp = metadata_path.with_extension("tmp");
        std::fs::write(&meta_tmp, &metadata_json)?;
        std::fs::rename(&meta_tmp, &metadata_path)?;
    ...
    pub fn load_from_dir(base_dir: Option<&std::path::Path>) -> NeoTrixResult<Self> {
        ...
        let metadata_json = match std::fs::read_to_string(&metadata_path) {
            ...
            Err(e) => return Err(NeoTrixError::Io(std::sync::Arc::new(e))),
        };
    ...
    fn brain_path() -> PathBuf {
        nt_core_util::home_dir().join(".neotrix").join("brain.json")
    }
