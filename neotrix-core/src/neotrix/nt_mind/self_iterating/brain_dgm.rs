...
impl EditCritic {
    ...
    pub fn score_edits(&self, edits: &[MicroEdit], _task: &str) -> f64 {
        ...
        if edits
            .iter()
            .any(|e| matches!(e, MicroEdit::NormalizeVector))
        {
            score += 0.1;
        }
        ...
        if edits
            .iter()
            .any(|e| matches!(e, MicroEdit::UpdateLearningRate(_)))
        {
            score += 0.1;
        }
    ...
    pub fn is_valid(&self, edits: &[MicroEdit]) -> bool {
        !edits.is_empty()
            && edits
                .iter()
                .any(|e| matches!(e, MicroEdit::AdjustDimension(_, _)))
    }
}
...
impl DgmSelfEditStrategy {
    ...
    pub fn generate_via_diffusion(&self, context: &EditContext) -> Vec<MicroEdit> {
        ...
        for step in 0..self.num_diffusion_steps {
            let noise_level = self.noise_schedule.get(step).copied().unwrap_or(0.01);
...
impl DgmEditOrchestrator {
    ...
    pub fn generate_best_edit(&self, brain: &ReasoningBrain, task: &str) -> Vec<MicroEdit> {
        ...
        for _ in 0..self.num_candidates {
            ...
            for step in 0..self.dgm.num_diffusion_steps {
                let noise_level = self.dgm.noise_schedule.get(step).copied().unwrap_or(0.01);
    ...
    pub fn orchestrate_diffusion(&self, brain: &ReasoningBrain, task: &str) -> Vec<Vec<MicroEdit>> {
...
mod tests {
    ...
    fn test_zero_steps_is_identity() {
        ...
        for (r, d) in result.iter().zip(direct.iter()) {
            match (r, d) {
                (
                    MicroEdit::AdjustDimension(r_dim, r_val),
                    MicroEdit::AdjustDimension(d_dim, d_val),
                ) => {
    ...
    fn test_edit_to_micro() {
        ...
        assert!(edits
            .iter()
            .any(|e| matches!(e, MicroEdit::UpdateLearningRate(0.05))));
    }
