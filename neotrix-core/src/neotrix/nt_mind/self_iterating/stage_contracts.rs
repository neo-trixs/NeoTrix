...
impl StageContract for ReadOnlyContract {
    ...
    fn pre_check(&self, _brain: &SelfIteratingBrain) -> Vec<ContractViolation> {
        vec![]
    }
    fn post_check(
        &self,
        ...
        if brain.iteration != before.iteration {
            violations.push(ContractViolation {
                ...
                message: format!(
                    "read_only contract violated: iteration changed from {} to {}",
                    before.iteration, brain.iteration
                ),
            });
        }
        if brain.task_scratch.reward != before.reward {
            violations.push(ContractViolation {
                stage_name: String::new(),
                severity: ContractSeverity::Warning,
                message: format!(
                    "read_only contract violated: reward changed from {} to {}",
                    before.reward, brain.task_scratch.reward
                ),
            });
        }
        violations
    }
    fn invariant_check(&self, _brain: &SelfIteratingBrain) -> Vec<ContractViolation> {
        vec![]
    }
}
...
impl StageContract for CapabilityStableContract {
    ...
    fn post_check(
        ...
        let mut violations = Vec::new();
        if let (Some(before_score), Some(after_score)) = (
            before.champion_score,
            brain.champion.as_ref().map(|c| c.score),
        ) {
            if before_score > 0.0 {
                ...
                if degradation > self.max_degradation {
                    violations.push(ContractViolation {
                        ...
                        severity: ContractSeverity::Warning,
                        message: format!(
                            "capability degraded by {:.1}% (max allowed {:.1}%)",
                            degradation * 100.0,
                            self.max_degradation * 100.0
                        ),
                    });
                }
            }
    ...
    fn post_check(
        ...
        if calls_used > self.max_tool_calls_per_run {
            vec![ContractViolation {
                ...
                severity: ContractSeverity::Warning,
                message: format!(
                    "used {} tool calls (budget {})",
                    calls_used, self.max_tool_calls_per_run
                ),
            }]
        } else {
            vec![]
...
impl StageContract for MonotonicIterationContract {
    ...
    fn pre_check(&self, brain: &SelfIteratingBrain) -> Vec<ContractViolation> {
        ...
        let mut violations = Vec::new();
        if let Some(last) = brain.seal_rl.stage_results.last() {
    ...
    fn invariant_check(&self, brain: &SelfIteratingBrain) -> Vec<ContractViolation> {
        if brain.iteration > 0 && brain.task_scratch.reward.is_nan() {
            vec![ContractViolation {
                stage_name: String::new(),
                severity: ContractSeverity::Error,
...
impl StageContract for NoSelfEditContract {
    ...
    fn post_check(
        ...
        let edits_after = brain.task_scratch.micro_edits.len();
        if edits_after > edits_before {
            vec![ContractViolation {
                stage_name: String::new(),
                severity: ContractSeverity::Error,
                message: format!(
                    "stage generated {} micro-edits despite no_self_edit contract",
                    edits_after.saturating_sub(edits_before)
                ),
            }]
        } else {
            vec![]
...
impl ContractRegistry {
    ...
    pub fn has_any_violations(&self, violations: &[ContractViolation]) -> bool {
        violations
            .iter()
            .any(|v| matches!(self.policy.action_for(v.severity), ActionOnViolation::Block))
    }
}
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        for v in &violations {
            match policy.action_for(v.severity) {
                ...
                ActionOnViolation::Log => {
                    log::warn!(
                        "[contract] pre-check FAIL: stage={} contract={} msg={:?}",
                        self.name(),
                        contract_name_for(&self.contracts),
                        v.message
                    );
                }
                ActionOnViolation::Block => {
                    log::error!(
                        "[contract] pre-check BLOCK: stage={} contract={} msg={:?}",
                        self.name(),
                        contract_name_for(&self.contracts),
                        v.message
                    );
        ...
        for v in &post_violations {
            match policy.action_for(v.severity) {
                ...
                ActionOnViolation::Log => {
                    log::warn!(
                        "[contract] post-check FAIL: stage={} msg={:?}",
                        self.name(),
                        v.message
                    );
                }
                ActionOnViolation::Block => {
                    log::error!(
                        "[contract] post-check BLOCK: stage={} msg={:?}",
                        self.name(),
                        v.message
                    );
                    return Err(NeoTrixError::Brain(format!(
                        "Contract violation (post) in '{}': {}",
                        self.name(),
                        v.message
                    )));
                }
...
pub mod presets {
    ...
    pub fn read_only() -> Vec<Box<dyn StageContract>> {
        vec![
            ...
            Box::new(ResourceBudgetContract {
...
pub fn with_contract_preset(
    stage: Box<dyn BrainStage>,
    preset: Vec<Box<dyn StageContract>>,
) -> ContractAwareStage {
...
#[cfg(test)]
#[allow(dead_code)]
/// Test helpers (dummy_brain, etc.) are used across multiple tests;
/// blanket suppression avoids false positives when only a subset runs.
mod tests {
    use super::*;
    ...
    fn test_read_only_contract_passes() {
        ...
        assert!(
            violations.is_empty(),
            "read-only contract should pass when state unchanged"
        );
    }
    ...
    fn test_read_only_contract_catches_iteration_change() {
        ...
        assert!(
            !violations.is_empty(),
            "read-only contract should catch iteration change"
        );
    }
    ...
    fn test_resource_budget_contract() {
        ...
        let contract = ResourceBudgetContract {
    ...
    fn test_enforcement_policy_default() {
        ...
        assert_eq!(
            policy.action_for(ContractSeverity::Suggestion),
            ActionOnViolation::Log
        );
        assert_eq!(
            policy.action_for(ContractSeverity::Warning),
            ActionOnViolation::Log
        );
        assert_eq!(
            policy.action_for(ContractSeverity::Error),
            ActionOnViolation::Block
        );
    }
    ...
    fn test_no_self_edit_contract_detects_edits() {
        ...
        impl BrainStage for EditorStage {
            ...
            fn process(
                ...
                brain.task_scratch.micro_edits.push(
    ...
    fn test_capability_change_without_champion_no_violation() {
        ...
        impl BrainStage for NoChampionStage {
            ...
            fn process(
                ...
                brain.task_scratch.reward = 100.0;
                Ok(StageDecision::Continue)
            }
        }
