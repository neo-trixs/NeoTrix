...
pub use super::brain_core::{DefaultAbsorbValidator, ReasoningBrain};
pub use super::brain_ewc::{EvaluationRecord, FisherMatrix, RLAlgorithm, WeightUpdateRecord};
pub use super::brain_seal::{
    AggressiveSealStrategy, CapabilityDelta, ConservativeSealStrategy, DefaultSealStrategy,
    SealEditStrategy,
};
...
mod tests {
    ...
    fn test_strategy_conservative() {
        ...
        let edits = brain
            .strategy
            .generate_edit(&brain, "code review nt_shield");
        assert!(!edits.is_empty());
        for e in &edits {
            assert!(
                e.delta <= 0.05 + 1e-12,
                "conservative delta {} > 0.05",
                e.delta
            );
    ...
    fn test_fisher_ewc_scales_down() {
        ...
        brain.absorb(KnowledgeSource::HeroUI);
        let _first_penalty = brain
            .fisher
            .as_ref()
            .unwrap()
            .ewc_penalty(&[0.0; 23], brain.capability.arr());

        let _cap_before = brain.capability.clone();
        brain.absorb(KnowledgeSource::AgenticDS);
        let has_importance = brain
            .fisher
            .as_ref()
            .unwrap()
            .values
        ...
        assert!(
            has_importance,
            "Fisher should have non-zero importance after first absorb"
        );
    }
}
