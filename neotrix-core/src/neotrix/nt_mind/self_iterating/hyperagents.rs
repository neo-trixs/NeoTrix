...
mod tests {
    ...
    fn make_record_with_latent(generation: u64, score: Option<f64>, latent: Vec<f64>) -> HyperAgentRecord {
    ...
    fn test_parent_selection_best() {
        ...
        assert!((best - 0.9).abs() < 1e-6, "Best strategy should pick highest score");
    }
    ...
    fn test_parent_selection_latest() {
        ...
        let parent = archive.select_parent().expect("should select a parent");
        assert_eq!(parent.generation, 2, "Latest strategy should pick last added");
    }
    ...
    fn test_filter_protected_paths() {
        ...
        assert!(!filtered.iter().any(|d| d.file_path.to_string_lossy().contains("secret/")));
    }
    ...
    fn test_cosine_distance_opposite() {
        ...
        assert!((d - 1.0).abs() < 1e-6, "opposite vectors should have distance 1");
    }
    ...
    fn test_cosine_distance_orthogonal() {
        ...
        let b = vec![0.0, 1.0];
        let d = cosine_distance(&a, &b);
        assert!((d - 0.5).abs() < 1e-6, "orthogonal vectors should have distance 0.5");
    }
    ...
    fn test_compute_novelty_with_similar_record() {
        ...
        assert!(novelty < 0.1, "similar latent should have low novelty: {}", novelty);
    }
    ...
    fn test_diversity_weighted_selection() {
        ...
        let parent = archive.select_parent().expect("should select a parent");
        let best = parent.score.expect("score should be present");
        assert!((best - 0.9).abs() < 1e-6, "DiversityWeighted should still prefer high score");
    }
    ...
    fn test_evaluate_proposal_lower_fe_is_better() {
        ...
        assert!(low_fe_score > high_fe_score, "lower FE should give higher score");
    }
    ...
    fn test_evaluate_proposal_target_weights() {
        ...
        let base = agent.evaluate_proposal(
            &SelfModificationProposal { target: ModificationTarget::CapabilityExtension, diffs: vec![], expected_impact: "".into(), safety_check: SafetyCheckResult::Passed },
            fe,
        );
        let meta = agent.evaluate_proposal(
            &SelfModificationProposal { target: ModificationTarget::MetaAgent, diffs: vec![], expected_impact: "".into(), safety_check: SafetyCheckResult::Passed },
            fe,
        );
        let improve = agent.evaluate_proposal(
            &SelfModificationProposal { target: ModificationTarget::ImprovementMechanism, diffs: vec![], expected_impact: "".into(), safety_check: SafetyCheckResult::Passed },
            fe,
        );
        assert!(meta > base, "MetaAgent should score higher than CapabilityExtension");
        assert!(improve > meta, "ImprovementMechanism should score highest (exploration bonus)");
    }
