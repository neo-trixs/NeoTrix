...
use super::super::core::{
    AbsorptionRecord, BrainMutView, CapabilityVector, KnowledgeSource, PerformanceEvaluator,
};
use super::super::memory::ReasoningBank;
use super::brain_ewc::{FisherMatrix, WeightUpdateRecord};
use super::brain_seal::{DefaultSealStrategy, SealEditStrategy};
...
impl ReasoningBrain {
    ...
    pub fn absorb_from_custom(&mut self, name: &str) -> bool {
        if let Some(vector) = self.custom_sources.get(name) {
            self.capability
                .update_from_other(vector, self.learning_rate);
    ...
    pub fn sia_should_switch_to_weight(&self, recent_rewards: &[f64]) -> bool {
        ...
        let avg_improvement: f64 =
            last3.windows(2).map(|w| w[1] - w[0]).sum::<f64>() / (last3.len() - 1) as f64;
        avg_improvement.abs() < 0.01
    }

    pub fn update_ssm(&mut self) {}
...
impl BrainMutView for ReasoningBrain {
