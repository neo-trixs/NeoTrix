...
impl BrainStage for DGMMetaEvolveStage {
    ...
    fn process(&self, _brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        match &proposal.safety_check {
            ...
            SafetyCheckResult::NeedsHumanReview { concern } => Ok(StageDecision::Rollback(
                format!("DGMMetaEvolve needs human review: {}", concern),
            )),
        }
    }
}
...
impl BrainStage for MetaEvolveStage {
    ...
    fn process(&self, _brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        match &proposal.safety_check {
            ...
            SafetyCheckResult::NeedsHumanReview { concern } => Ok(StageDecision::Rollback(
                format!("MetaEvolve needs human review: {}", concern),
            )),
        }
    }
}
