...
impl CheckpointManager {
    ...
    pub fn push_from_brain(
        ...
        self.push(
            iteration, snapshot, permission, autonomy, reward, stage_name,
        );
    }
...
impl BrainStage for RewindStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        if brain.task_scratch.reward < -0.3 {
            let best = {
            ...
            if let Some(cp) = best {
                ...
                brain.task_scratch.reward = cp.reward;
                log::info!(
...
mod tests {
    ...
    fn make_snapshot(brain: &SelfIteratingBrain) -> BrainSnapshot {
        BrainSnapshot::new(&brain.brain, &brain.task_scratch.current_task_type)
    }
    ...
    fn test_rewind_stage_low_reward() {
        ...
        brain.task_scratch.reward = -0.5;
        let stage = RewindStage::new();
        let result = stage.process(&mut brain);
        assert!(result.is_ok());
        assert!(
            brain.task_scratch.reward >= 0.0,
