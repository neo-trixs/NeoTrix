use crate::core::skill::{Skill, SkillRegistry};
...
mod tests {
    ...
    fn test_crystallize_high_reward() {
        ...
        let id =
            crystallizer.crystallize("Implement a binary search tree", "code_generation", 0.85);
        assert!(id.is_some());
    ...
    fn test_crystallize_low_reward_skip() {
        let mut crystallizer = SkillCrystallizer::new();
        let id = crystallizer.crystallize("Failed attempt", "code_generation", 0.3);
        assert!(id.is_none());
    ...
    fn test_skill_reuse_tracking() {
        let mut crystallizer = SkillCrystallizer::new();
        let id = crystallizer
            .crystallize("Test task", "debugging", 0.9)
            .expect("value should be ok in test");
        let skill = crystallizer
            .registry
            .get_mut(&id)
            .expect("value should be ok in test");
    ...
    fn test_threshold_configurable() {
        ...
        assert!(crystallizer
            .crystallize("Medium task", "refactoring", 0.6)
            .is_some());
        assert!(crystallizer
            .crystallize("Low task", "refactoring", 0.4)
            .is_none());
    }
