...
impl Default for BoundedEditStage {
    fn default() -> Self {
        Self
    }
}
impl BoundedEditStage {
    pub fn new() -> Self {
        Self
    }
}
impl BrainStage for BoundedEditStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let budget = brain.seal_rl.lr_scheduler.current_budget();
        let edits = brain._take_micro_edits();
        if edits.len() > budget {
            log::info!(
                "[bounded-edit] trimming {} edits to budget {}",
                edits.len(),
                budget
            );
        ...
        brain.seal_rl.lr_scheduler.step();
    ...
    fn verify_step(
        ...
        Some(super::vsi_verifier::verify_budget(edit_count, budget))
    }
}

pub struct ValidationGateStage;
impl Default for ValidationGateStage {
    fn default() -> Self {
        Self
    }
}
impl ValidationGateStage {
    pub fn new() -> Self {
        Self
    }
}
impl BrainStage for ValidationGateStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        ...
        let accepted = brain
            .seal_rl
            .validation_gate
            .accept(iteration, candidate_score);
        ...
        if !accepted {
            ...
            let rejected = RejectedEdit {
                ...
                reason: format!(
                    "validation gate: score {:.4} <= best {:.4}",
                    candidate_score, brain.seal_rl.validation_gate.best_score
                ),
                iteration,
            };
            brain.seal_rl.rejected_buffer.push(rejected);
            log::info!(
                "[validation-gate] rejected candidate at iter {}: score={:.4} <= best={:.4}",
                iteration,
                candidate_score,
                brain.seal_rl.validation_gate.best_score
            );
            brain._snapshot_restore();
        } else {
            log::info!(
                "[validation-gate] accepted candidate at iter {}: score={:.4} (best={:.4})",
                iteration,
                candidate_score,
                brain.seal_rl.validation_gate.best_score
            );
...
impl Default for RejectedBufferFeedbackStage {
    fn default() -> Self {
        Self
    }
}
impl RejectedBufferFeedbackStage {
    pub fn new() -> Self {
        Self
    }
}
impl BrainStage for RejectedBufferFeedbackStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let patterns = brain.seal_rl.rejected_buffer.recent_failure_patterns(5);
...
impl Default for EpochSlowUpdateStage {
    fn default() -> Self {
        Self
    }
}
impl EpochSlowUpdateStage {
    pub fn new() -> Self {
        Self
    }
}
impl BrainStage for EpochSlowUpdateStage {
    ...
    fn process(&self, brain: &mut SelfIteratingBrain) -> Result<StageDecision, NeoTrixError> {
        let trend = brain.seal_rl.validation_gate.score_trend(10);
        log::info!("[epoch-slow] score trend (last 10): {:.4}", trend);
        if trend < -0.05 {
            log::warn!(
                "[epoch-slow] negative trend detected ({:.4}), consider revision",
                trend
            );
        }
        Ok(StageDecision::Continue)
