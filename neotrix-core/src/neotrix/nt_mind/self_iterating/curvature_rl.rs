...
impl CurvatureTracker {
    ...
    pub fn record_negentropy(&mut self, n_total: f64) {
        ...
        self.negentropy_regime_history.push(self.last_negentropy_regime);
    ...
    fn update_negentropy_derivatives(&mut self) {
        ...
        let first: Vec<f64> = (1..n)
            ...
            .collect();
        let second: Vec<f64> = (1..first.len())
            .map(|i| first[i] - first[i - 1])
            .collect();
    ...
    pub fn oscillation_frequency(&self) -> f64 {
        ...
        let osc_count = recent.iter().filter(|&&r| *r == CurvatureRegime::Oscillating).count();
    ...
    pub fn negentropy_oscillation_frequency(&self) -> f64 {
        ...
        let recent: Vec<_> = self.negentropy_regime_history.iter().rev().take(20).collect();
        let osc_count = recent.iter().filter(|&&r| *r == CurvatureRegime::Oscillating).count();
        osc_count as f64 / recent.len() as f64
    }
...
mod tests {
    ...
    fn test_policy_adapt_lr_stable() {
        ...
        assert!((lr - 0.01).abs() < 0.001, "stable regime should keep base lr: got {}", lr);
    }
