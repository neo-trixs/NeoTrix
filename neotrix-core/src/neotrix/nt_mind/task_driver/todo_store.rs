...
impl TodoStore {
    pub fn load(path: &str) -> Result<Self, String> {
        let content =
            std::fs::read_to_string(path).map_err(|e| format!("读取 TODO.yml 失败: {}", e))?;
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        let tmp = std::path::Path::new(&self.path).with_extension("tmp");
        std::fs::write(&tmp, &content).map_err(|e| format!("写入 TODO.yml 失败: {}", e))?;
        std::fs::rename(&tmp, &self.path).map_err(|e| format!("重命名 TODO.yml 失败: {}", e))
    }
...
impl DependencyGraph {
    pub fn ready_pending(&self, done_ids: &HashSet<String>) -> Vec<&TodoItem> {
        let mut ready: Vec<&TodoItem> = self
            .items
            .iter()
            .filter(|i| i.status == ItemStatus::Pending)
...
fn count_done_external(items: &[TodoItem]) -> u64 {
    items
        .iter()
        .filter(|i| i.id.starts_with("S19-") && i.status == ItemStatus::Done)
        .count() as u64
}
