...
impl CargoVerifier {
    ...
    pub fn check_all_features(&self) -> VerifyResult {
        run_cargo(
            &self.project_dir,
            &["check", "--all-features", "--lib", "-p", "neotrix"],
        )
    }
    ...
    pub fn verify_all(&self) -> Vec<(&'static str, VerifyResult)> {
        ...
        if results.last().expect("verify_all: results should NOT be empty—push succeeded above").1.passed {
...
fn run_cargo(dir: &str, args: &[&str]) -> VerifyResult {
    if cfg!(test) {
        return VerifyResult {
            ...
            errors: vec![],
            warnings: vec![],
            details: "skipped during test".into(),
        };
    }
