...
impl ReactDoctorEngine {
    pub fn calculate_score(diagnostics: &[ReactDiagnostic]) -> ReactHealthReport {
        ...
        let score_f = 100.0 - unique_error_rules as f64 * 1.5 - unique_warning_rules as f64 * 0.75;
        ...
        for cat in ReactRuleCategory::all() {
            let count = diagnostics
                .iter()
                .filter(|d| std::mem::discriminant(&d.category) == std::mem::discriminant(&cat))
                .count();
            if count > 0 {
    ...
    pub fn explain_diagnostic(
        ...
        if line_idx > 0 {
            ...
            if prev.contains("react-doctor-disable-next-line") {
                let listed = prev
                    .split("react-doctor-disable-next-line")
                    .nth(1)
                    .unwrap_or("");
            ...
            if prev.contains("react-doctor-disable-line")
                && current_line.contains("react-doctor-disable-line")
            {
                return SuppressionAnalysis::suppressed();
            }
        }
...
mod tests {
    ...
    fn test_calculate_score_needs_work() {
        ...
        assert!(
            report.score >= 50 && report.score <= 74,
            "score {} should be 50-74",
            report.score
        );
    ...
    fn test_explain_not_suppressed() {
        ...
        diag.line = 1;
        let lines = vec!["useEffect(() => { fetch('/api') }, [])".to_string()];
        let analysis = ReactDoctorEngine::explain_diagnostic(&diag, &lines);
        assert!(!analysis.is_suppressed);
