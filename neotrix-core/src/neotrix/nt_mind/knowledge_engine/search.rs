            ...
            if let Some(end) = xml[abs_start..].find("</entry>") {
                ...
                if !title.is_empty() {
                    let entry = KnowledgeEntry::new(
                        ...
                        SourceType::ArXiv,
                        &arxiv_id,
                    ).with_tags(categories).with_importance(0.7);
                    let _ = authors;
                    entries.push(entry);
                }
                pos = abs_start + end + 8;
    ...
    fn parse_s2_response(&self, json: &str) -> Vec<KnowledgeEntry> {
        ...
        if let Some(papers) = v["data"].as_array() {
            for paper in papers {
                ...
                if !title.is_empty() {
                    let entry = KnowledgeEntry::new(&title, &abstract_, SourceType::SemanticScholar, &paper_id)
    ...
    fn parse_wiki_search(&self, json: &str) -> Vec<KnowledgeEntry> {
        ...
        if let Some(results) = v["query"]["search"].as_array() {
            for result in results {
                ...
                if !title.is_empty() {
                    let entry = KnowledgeEntry::new(&title, &strip_html(&snippet), SourceType::Wikipedia, &url)
                        .with_importance(0.6);
                    entries.push(entry);
                }
            }
...
impl KnowledgeEngine {
    pub fn search(&mut self, query: &str, limit: usize) -> Vec<(&KnowledgeEntry, f64)> {
        ...
        let mut scored: Vec<(&KnowledgeEntry, f64)> = self.entries.values()
            .map(|e| {
    ...
    pub fn search_by_tag(&self, tag: &str, limit: usize) -> Vec<&KnowledgeEntry> {
        self.tag_index.get(tag)
            .map(|ids| {
                let mut entries: Vec<&KnowledgeEntry> = ids.iter()
    ...
    pub fn search_by_source(&self, source: &SourceType, limit: usize) -> Vec<&KnowledgeEntry> {
    ...
    pub fn get_related(&self, entry_id: &str, limit: usize) -> Vec<&KnowledgeEntry> {
        ...
        for rel in &self.relations {
            let target = if rel.from_id == entry_id { Some(&rel.to_id) }
                        else if rel.to_id == entry_id { Some(&rel.from_id) }
                        else { None };
