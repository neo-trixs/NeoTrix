...
use crate::core::nt_core_self::skill_crystal::{CrystalRegistry, SkillCrystal};
...
impl AutoCrystallizer {
    ...
    pub fn summary(&self) -> String {
        format!(
            ...
            self.total_crystallized, self.auto_crystallize, self.min_reward_threshold,
        )
    }
}
...
mod tests {
    ...
    fn test_crystallize_from_trace() {
        ...
        let mut bank = ReasoningBank::new(100);
        let insights = vec![
            "pattern: use agent isolation".to_string(),
            "pattern: module boundaries".to_string(),
        ];
    ...
    fn test_crystallizer_disabled() {
        ...
        let mut bank = ReasoningBank::new(100);
        let result =
            c.crystallize_from_absorption(&mut brain, &mut bank, "url", "name", "domain", &[], 0.9);
        assert!(result.is_none());
    }
}
