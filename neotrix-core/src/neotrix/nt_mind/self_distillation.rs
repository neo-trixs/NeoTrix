...
impl FixedTeacher {
    ...
    pub fn freeze(&mut self, outputs: Vec<Vec<f64>>) {
...
mod tests {
    ...
    fn test_token_kl_clipping() {
        ...
        assert!(
            loss <= 0.02,
            "Clipped loss should not exceed token_kl_clip, got {}",
            loss
        );
    }
    ...
    fn test_feed_and_compute_loss() {
        ...
        assert!(
            trainer.student_logits.is_empty(),
            "Logits should be cleared after compute_loss"
        );
        assert!(
            trainer.teacher_logits.is_empty(),
            "Teacher logits should be cleared after compute_loss"
        );
    }
