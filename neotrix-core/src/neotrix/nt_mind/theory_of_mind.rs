...
impl GapCalibrator {
    ...
    pub fn calibrate(&self, query: &str, intent: InferredIntent) -> IntentFrame {
        ...
        let patterns: Vec<(ImplicitNeed, Vec<&str>)> = vec![
            (
                ImplicitNeed::AssessFeasibility,
                vec!["can i", "is it possible", "could i", "is there a way"],
            ),
            (
                ImplicitNeed::SeekApproval,
                vec!["should i", "worth", "recommend", "good idea"],
            ),
            (
                ImplicitNeed::SurfaceRisks,
                vec!["what if", "risk", "danger", "downside", "worst case"],
            ),
            (
                ImplicitNeed::LearnRationale,
                vec!["why", "reason", "rationale", "motivation", "purpose"],
            ),
            (
                ImplicitNeed::CompareAlternatives,
                vec![" vs ", " or ", "alternative", "compared to", "better"],
            ),
            (
                ImplicitNeed::VerifyInformation,
                vec!["confirm", "verify", "double-check", "check if", "validate"],
            ),
            (
                ImplicitNeed::CheckConstraints,
                vec![
                    "constraint",
                    "limit",
                    "require",
            ...
            (
                ImplicitNeed::UnderstandImpact,
                vec!["impact", "affect", "consequence", "effect on", "change"],
            ),
            (
                ImplicitNeed::ClarifyGoal,
                vec![
                    "what do i need",
                    "how to",
                    "where to start",
...
impl TheoryOfMind {
    ...
    pub fn infer(&mut self, query: &str) -> IntentFrame {
        ...
        self.session_intents
            .push((query.to_string(), frame.clone()));
    ...
    pub fn literal_intent(&self, task: &str) -> InferredIntent {
            ...
            || task_lower.contains("make")
        {
            ...
            || task_lower.contains("not working")
        {
    ...
    pub fn record_confusion(&mut self, task: &str) {
        let signal = if task.contains("again")
            || task.contains("still")
            || task.contains("not right")
            || task.contains("wrong")
        {
            0.8
        } else {
...
mod tests {
    ...
    fn test_literal_intent_learn() {
        ...
        assert_eq!(
            tom.literal_intent("learn about quantum computing"),
            InferredIntent::Learn
        );
    }
    ...
    fn test_gap_calibration_assess_feasibility() {
        ...
        assert!(frame
            .implicit_needs
            .contains(&ImplicitNeed::AssessFeasibility));
    ...
    fn test_gap_calibration_verify() {
        ...
        assert!(frame
            .implicit_needs
            .contains(&ImplicitNeed::VerifyInformation));
    }
