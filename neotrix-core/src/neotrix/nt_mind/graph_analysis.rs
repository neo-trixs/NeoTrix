...
use crate::neotrix::nt_mind::graph_types::{
    CodeGraphStats, EdgeKind, EnrichedSearchResult, GraphNode, ImpactHop, ImpactResult,
};
...
impl CodeGraph {
    ...
    pub fn search_enriched(&self, query: &str, max_results: usize) -> Vec<EnrichedSearchResult> {
        ...
        let mut scored: Vec<(i32, &GraphNode)> = self
            .nodes
            .values()
            .filter(|n| n.name.to_lowercase().contains(&query_lower))
            .map(|n| {
                let mut score: i32 = if n.name.eq_ignore_ascii_case(&query_lower) {
                    10
                ...
                if n.kind.as_str() == query_lower {
                    score += 3;
                }
                if let Some(ref fp) = n.file_path {
                    if fp.to_string_lossy().to_lowercase().contains(&query_lower) {
                        score += 1;
                    }
                }
                (score, n)
