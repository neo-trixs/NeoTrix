...
impl BuildContext {
    pub fn analyze(root: &Path) -> Result<Self, String> {
        ...
        while let Some(dir) = queue.pop() {
            ...
            if !tom.exists() || !seen_paths.insert(tom.canonicalize().unwrap_or_else(|_| tom.clone())) {
                continue;
            }
            let content =
                fs::read_to_string(&tom).map_err(|e| format!("cannot read {}: {}", tom.display(), e))?;
            let value: toml::Value =
                toml::from_str(&content).map_err(|e| format!("cannot parse {}: {}", tom.display(), e))?;
    ...
    pub fn detect_cycles(&self) -> Vec<Vec<String>> {
            ...
            if let Some(neighbors) = adj.get(node) {
                for &next in neighbors {
                    if !visited.contains(next) {
                        ...
                        let pos = rec_stack.iter().position(|&x| x == next).expect("node in rec_stack confirmed by rec_set");
        ...
        for node in adj.keys() {
            if !visited.contains(node) {
                dfs(node, &adj, &mut visited, &mut rec_stack, &mut rec_set, &mut cycles);
            }
        }
