...
impl AttentionHypercubeBridge {
    ...
    pub fn recall_from_attention(
        ...
        for head in active {
            let Some(dim) = self.domain_dimension(&head.domain) else {
                continue;
            };
        ...
        results.sort_by(|a, b| {
            b.domain_activation
                .partial_cmp(&a.domain_activation)
...
mod tests {
    ...
    fn test_domain_dimension_mapping() {
        ...
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::Code),
            Some(DimensionAxis::CodeUnderstanding)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::Planning),
            Some(DimensionAxis::SystemDesign)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::Creativity),
            Some(DimensionAxis::Creativity)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::PatternMatch),
            Some(DimensionAxis::Abstraction)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::Semantic),
            Some(DimensionAxis::KnowledgeRetrieval)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::Temporal),
            Some(DimensionAxis::Time)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::SelfReflection),
            Some(DimensionAxis::Safety)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::ToolUse),
            Some(DimensionAxis::Performance)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::GoalAlignment),
            Some(DimensionAxis::Agency)
        );
        assert_eq!(
            bridge.domain_dimension(&AttentionDomain::RiskAssessment),
            Some(DimensionAxis::Certainty)
        );
    }
