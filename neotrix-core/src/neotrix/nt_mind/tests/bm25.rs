...
mod tests {
    ...
    fn test_bank_bm25_type_filtered() {
        ...
        let mem2 =
            ReasoningMemory::new("design system color palette", TaskType::UIDesign, &[], 0.85);
        bank.store(mem1);
        bank.store(mem2);
    ...
    fn test_multi_modal_search() {
        ...
        let mut m1 = ReasoningMemory::new(
            "async tokio runtime concurrency",
            TaskType::CodeGeneration,
            &micro_edits,
            0.9,
        );
        ...
        let mut m2 = ReasoningMemory::new(
        ...
        let mut m3 = ReasoningMemory::new(
        ...
        assert!(
            results[0].task_description.contains("async"),
            "top result should match async query"
        );
    }
    ...
    fn test_graph_search_by_entities() {
        let mut bank = ReasoningBank::new(10);
        let micro_edits = vec![MicroEdit::AdjustDimension("typography".to_string(), 0.1)];
        bank.store(ReasoningMemory::new(
            "async tokio runtime concurrency",
            TaskType::CodeGeneration,
            &micro_edits,
            0.9,
        ...
        assert!(
            !results.is_empty(),
            "graph search should find memory-related results"
        );
    }
}
