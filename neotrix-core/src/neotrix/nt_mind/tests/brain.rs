#[cfg(test)]
mod tests {
    use crate::neotrix::nt_mind::{CapabilityVector, KnowledgeSource, ReasoningBrain};
    ...
    fn test_evaluate_capability() {
        ...
        brain.absorb_batch(&[KnowledgeSource::HeroUI, KnowledgeSource::BaseUI]);
    ...
    fn test_brain_persistence() {
        ...
        assert!(
            (loaded_brain.capability.typography() - brain.capability.typography()).abs() < 0.001
        );
    ...
    fn test_multi_source_absorb() {
        ...
        assert!(
            brain.capability.typography() > initial_typography
