...
impl GoalLoop {
    ...
    pub fn save(&self) -> NeoTrixResult<()> {
        if let Some(parent) = self.persistence_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| NeoTrixError::General { msg: e.to_string(), backtrace: None })?;
        }
        ...
        let json = serde_json::to_string_pretty(&data)
            .map_err(|e| NeoTrixError::General { msg: e.to_string(), backtrace: None })?;
