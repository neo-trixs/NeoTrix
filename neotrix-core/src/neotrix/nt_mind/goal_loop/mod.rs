...
pub use types::{
    CircuitBreaker, CircuitState, GoalConfig, GoalIterationRecord, GoalLoopState, GoalPriority,
    GoalScheduleStrategy, PlanLevel, PlanTemplate, RateLimiter,
};
...
mod tests {
    ...
    fn test_goal_budget_exhaustion() {
        ...
        assert_eq!(
            ...
            Some(GoalLoopState::BudgetLimited)
        );
    }
    ...
    fn test_state_labels() {
        assert_eq!(GoalLoopState::Pursuing.label(), "pursuing");
        assert_eq!(GoalLoopState::Achieved.label(), "achieved");
        assert_eq!(GoalLoopState::Paused.label(), "paused");
        assert!(GoalLoopState::Achieved.is_terminal());
        assert!(!GoalLoopState::Pursuing.is_terminal());
    }
