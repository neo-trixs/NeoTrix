...
impl EvalMonitor {
    ...
    fn init_known_benchmarks(&mut self) {
        self.landscape.known_benchmarks = vec![
            EvalBenchmark {
                ...
                description: "动力系统诊断 + pass^k + 信噪比加权。轨迹级评估: 陷阱/极限环/扩散"
                    .into(),
                relevance: 0.90,
                last_checked: None,
                absorbed: false,
    ...
    pub fn pipeline_analyze_trajectory(
        &mut self,
        rewards: &[f64],
        actions: &[String],
    ...
    pub fn insert_atlas_state(&mut self, state: AgentAtlasState) {
        ...
        self.atlas_action_log
            .push((state, chrono::Utc::now().to_rfc3339()));
    }
...
mod tests {
    ...
    fn test_atlas_state_distribution() {
        ...
        assert_eq!(
            *dist
                .get(&AgentAtlasState::Act)
                .expect("value should be ok in test"),
            2
        );
        assert_eq!(
            *dist
                .get(&AgentAtlasState::Ask)
                .expect("value should be ok in test"),
            1
        );
        assert_eq!(
            *dist
                .get(&AgentAtlasState::Refuse)
                .expect("value should be ok in test"),
            1
        );
