...
impl ReasoningBenchReport {
    pub fn display(&self) {
        log::info!("╭─ Reasoning Engine Quality Benchmark ───────────────╮");
        log::info!(
            "│ Overall Score: {:.3}                                 │",
            self.score
        );
        ...
        for dim in &self.dimensions {
            ...
            log::info!("│ {:<30} {:>6.1} {} │", dim.name, dim.score * 100.0, bar);
        }
        log::info!("╰──────────────────────────────────────────────────────╯");
    }
}
...
impl ReasoningBenchmark {
    pub fn run() -> ReasoningBenchReport {
        ...
        let weighted = dimensions.iter().map(|d| d.score * d.weight).sum::<f64>() / total_weight;
    ...
    fn test_knowledge_retrieval() -> f64 {
        ...
        let traces = vec![
            ...
            MemoryTrace::new(
                ...
                Modality::Text,
                vec![
                    DimensionTag::KnowledgeCulture,
                    DimensionTag::KnowledgePhilosophy,
                ],
            ),
        ];
        ...
        let culture_recalled = culture_results
            .iter()
            .any(|e| e.source.contains("wikipedia"));
        ...
        let density_uniform = if total_cells >= 3 {
            1.0
        } else {
            total_cells as f64 / 3.0
        };
...
mod tests {
    ...
    fn test_attention_router_activates_specialists() {
        ...
        assert!(
            score > 0.0,
            "attention router should activate specialists: {:.3}",
            score
        );
        assert!(score <= 1.0);
    }
    ...
    fn test_knowledge_retrieval_inserts_and_recalls() {
        ...
        assert!(
            score > 0.0,
            "knowledge retrieval should recall entries: {:.3}",
            score
        );
        assert!(score <= 1.0);
    }
    ...
    fn test_score_bar_format() {
        ...
        assert!(
            bar.len() >= 4,
            "bar '{}' should have brackets and content",
            bar
        );
