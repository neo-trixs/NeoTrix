...
impl ActionSequenceMiner {
    pub fn new(min_sequence_length: usize, min_frequency: usize) -> Self {
        Self { min_sequence_length, min_frequency }
    }
    ...
    pub fn mine(&self, traces: &[ToolTrace]) -> Vec<SkillProposal> {
        ...
        for (cluster_idx, cluster) in clustered.iter().enumerate() {
            ...
            proposals.push(SkillProposal {
                ...
                confidence,
                steps,
                source_sequences: cluster.iter().map(|seq| seq.iter().map(|s| (*s).to_string()).collect()).collect(),
            });
        }
...
impl Default for SkillDiagnoser {
    fn default() -> Self {
        Self { llm_diagnosis_enabled: true }
    }
}

impl SkillDiagnoser {
    pub fn new(llm_diagnosis_enabled: bool) -> Self {
        Self { llm_diagnosis_enabled }
    }
    ...
    pub fn diagnose(
        ...
        if let Some(fb) = user_feedback {
            if fb < 0.3 {
                return SkillDiagnosis {
                    ...
                    severity: 1.0 - fb,
                    suggested_fix: "Review and rewrite skill steps based on user rejection".to_string(),
                };
            }
        }
        ...
        if skill.effectiveness < 0.4 {
            return SkillDiagnosis {
                ...
                suggested_fix: "Increase confidence through repeated successful application".to_string(),
            };
        }
    ...
    pub fn repair(&self, skill: &SkillCrystal, diagnosis: &SkillDiagnosis) -> Vec<MicroEdit> {
        ...
        match diagnosis.issue_type {
            ...
            SkillIssue::OutdatedPattern => {
                vec![
                    MicroEdit::ModifiedDimension("skill_pattern".to_string(), skill.effectiveness, 0.3),
...
impl SkillEvolver {
    ...
    pub fn mine_and_propose(&mut self, raw_traces: &[(String, u64, bool)], timestamp: u64) -> Vec<SkillProposal> {
        let traces: Vec<ToolTrace> = raw_traces.iter()
    ...
    pub fn diagnose_skill(&self, skill: &SkillCrystal, user_feedback: Option<f64>, llm_eval: Option<&str>) -> SkillDiagnosis {
        self.diagnoser.diagnose(skill, user_feedback, llm_eval)
    }

    /// Repair a skill and track the operation.
    pub fn repair_skill(&mut self, skill: &SkillCrystal, diagnosis: &SkillDiagnosis) -> Vec<MicroEdit> {
...
mod tests {
    ...
    fn test_action_sequence_miner_pattern() {
        ...
        let traces: Vec<ToolTrace> = vec![
            ToolTrace { tool_name: "read".into(), args: vec![], success: true, duration_ms: 10, timestamp: 1 },
            ToolTrace { tool_name: "edit".into(), args: vec![], success: true, duration_ms: 20, timestamp: 2 },
            ToolTrace { tool_name: "read".into(), args: vec![], success: true, duration_ms: 10, timestamp: 3 },
            ToolTrace { tool_name: "edit".into(), args: vec![], success: true, duration_ms: 20, timestamp: 4 },
        ];
    ...
    fn test_skill_diagnoser_user_feedback() {
        ...
        let crystal = SkillCrystal::new(0, "test_skill", "pattern", StrategyKind::Direct, AttentionDomain::Code, 1);
    ...
    fn test_skill_diagnoser_low_confidence() {
        let diagnoser = SkillDiagnoser::new(true);
        let mut crystal = SkillCrystal::new(0, "weak", "pattern", StrategyKind::Direct, AttentionDomain::Code, 1);
    ...
    fn test_skill_diagnoser_llm_eval() {
        let diagnoser = SkillDiagnoser::new(true);
        let crystal = SkillCrystal::new(0, "s", "p", StrategyKind::Direct, AttentionDomain::Code, 1);
    ...
    fn test_skill_diagnoser_no_issue() {
        let diagnoser = SkillDiagnoser::new(true);
        let mut crystal = SkillCrystal::new(0, "good", "pattern", StrategyKind::Direct, AttentionDomain::Code, 1);
    ...
    fn test_skill_repairer_low_confidence() {
        ...
        let crystal = SkillCrystal::new(0, "test", "p", StrategyKind::Direct, AttentionDomain::Code, 1);
    ...
    fn test_skill_repairer_no_repair_needed() {
        let repairer = SkillRepairer::new();
        let crystal = SkillCrystal::new(0, "ok", "p", StrategyKind::Direct, AttentionDomain::Code, 1);
        let diagnosis = SkillDiagnosis {
            needs_repair: false,
            issue_type: SkillIssue::LowConfidence,
            severity: 0.0,
    ...
    fn test_skill_evolver_full_cycle() {
        ...
        let mut crystal = SkillCrystal::new(0, "main", "pattern", StrategyKind::Direct, AttentionDomain::Code, 1);
        crystal.effectiveness = 0.3;

        let raw: Vec<(String, u64, bool)> = vec![
            ("read".into(), 10, true),
            ("edit".into(), 20, true),
        ];
