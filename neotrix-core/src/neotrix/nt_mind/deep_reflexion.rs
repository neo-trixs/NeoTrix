...
impl InnerGuard {
    ...
    pub fn with_thresholds(trajectory: usize, error: usize, window: usize) -> Self {
        Self {
    ...
    pub fn detect_anomalies(&mut self, trajectory: &[TraceStep]) -> Vec<AnomalyType> {
        ...
        if self.trajectory_history.len() >= self.trajectory_repetition_threshold {
            let recent = &self.trajectory_history
                [self.trajectory_history.len() - self.trajectory_repetition_threshold..];
        ...
        if self.pseudo_termination_log.len() >= self.loop_pseudo_termination_window {
            let window = &self.pseudo_termination_log
                [self.pseudo_termination_log.len() - self.loop_pseudo_termination_window..];
    ...
    pub fn intervene(trajectory: &mut Vec<TraceStep>, anomaly: &AnomalyType) -> String {
        match anomaly {
            AnomalyType::TrajectoryRepetition => {
                let intervention =
                    "Trajectory repetition detected. Switching to explore alternative approach."
                        .to_string();
            ...
            AnomalyType::ErrorRepetition(action) => {
                let intervention = format!(
                    "Error repetition on action '{}'. Applying backoff and alternative strategy.",
                    action
                );
            ...
            AnomalyType::LoopPseudoTermination => {
                let intervention =
                    "Pseudo-termination detected. Forcing hard stop and escalation.".to_string();
                intervention
            }
        }
    }
...
impl OuterReflector {
    ...
    pub fn extract_heuristic(rca: &RootCauseAnalysis) -> StrategyHeuristic {
        ...
        StrategyHeuristic {
            name: name.clone(),
            description: format!(
                "Heuristic to prevent {} failures: {}",
                rca.failure_type.as_str(),
                rca.root_cause
            ),
    ...
    pub fn store_as_memory(
        ...
        let mem = ReasoningMemory::new(
            &desc,
            crate::core::nt_core_knowledge::TaskType::Reflection,
            &[],
    ...
    fn infer_root_cause(trajectory: &[TraceStep], outcome: &str, ftype: &FailureType) -> String {
        match ftype {
            FailureType::Planning => {
                ...
                match first_failure {
                    Some(step) => format!(
                        "Initial approach '{}' failed, leading to cascading breakdown",
                        step.action
                    ),
                    None => format!(
                        "Planning error: outcome '{}' not achieved despite all steps nominal",
                        outcome
                    ),
                }
            }
            FailureType::Execution => {
                let failures: Vec<&str> = trajectory
                    .iter()
                    .filter(|s| !s.success)
            ...
            FailureType::KnowledgeGap => {
                "Missing domain knowledge for task requirements".to_string()
            }
            FailureType::SafetyBlocked => "Action blocked by safety constraints".to_string(),
            FailureType::BudgetExhausted => {
                "Resource budget exhausted before completion".to_string()
            }
        }
    }
    ...
    fn suggest_prevention(ftype: &FailureType, root_cause: &str) -> String {
        match ftype {
            FailureType::Planning => format!(
                "Prevention: decompose plan into smaller verifiable steps. Issue: {}",
                root_cause
            ),
            FailureType::Execution => format!(
                "Prevention: add preconditions check before each step. Issue: {}",
                root_cause
            ),
            FailureType::KnowledgeGap => format!(
                "Prevention: query knowledge base before planning. Issue: {}",
                root_cause
            ),
            FailureType::SafetyBlocked => format!(
                "Prevention: pre-screen actions against safety policy. Issue: {}",
                root_cause
            ),
            FailureType::BudgetExhausted => format!(
                "Prevention: set conservative budget with checkpoint. Issue: {}",
                root_cause
            ),
        }
    }
    ...
    fn compute_severity(trajectory: &[TraceStep], outcome: &str) -> f64 {
        let failure_rate = trajectory.iter().filter(|s| !s.success).count() as f64
            / trajectory.len().max(1) as f64;
        let outcome_severity = if outcome.contains("complete") || outcome.contains("success") {
            0.2
        } else {
            0.8
        };
...
impl DualPathAligner {
    pub fn align_explicit(memory: &mut ReasoningMemory, feedback: &str) {
        let positive =
            feedback.contains("good") || feedback.contains("correct") || feedback.contains("yes");
        let negative =
            feedback.contains("bad") || feedback.contains("wrong") || feedback.contains("no");
        if positive {
    ...
    pub fn align_implicit(memory: &mut ReasoningMemory, context: &[TraceStep]) {
        let trace_success_rate =
            context.iter().filter(|s| s.success).count() as f64 / context.len().max(1) as f64;
...
mod tests {
    ...
    fn test_dual_path_align_explicit_positive() {
        let mut mem = ReasoningMemory::new(
            "test",
    ...
    fn test_dual_path_align_combined() {
        ...
        let ctx = vec![
            make_step("step1", "ok", true),
            make_step("step2", "ok", true),
        ];
