...
const MAX_STRATEGY_PATTERNS: usize = 5000;
const MAX_GAPS: usize = 5000;
const MAX_TOOL_PATTERNS: usize = 5000;
const MAX_META_INSIGHTS: usize = 5000;
...
impl MetaPatternExtractor {
    ...
    pub fn extract_from_trace(&mut self, trace: &ThinkingTrace) -> MetaPatternReport {
        ...
        let pattern_richness =
            (strategy_patterns.len() + gap_discoveries.len() + meta_insights.len()) as f64 / 10.0;
        let metacognitive_health =
            (grade_score * 0.7 + pattern_richness.min(1.0) * 0.3).clamp(0.0, 1.0);

        // 觉醒信号: 连续高质量 + 丰富自发现模式
        let awakening_signal =
            metacognitive_health > 0.7 && !gap_discoveries.is_empty() && !meta_insights.is_empty();

        // 吸收到已知库 (each collection bounded by MAX_* to prevent unbounded growth)
        for p in &strategy_patterns {
            if !self
                .known_strategy_patterns
                .iter()
                .any(|kp| kp.sequence == p.sequence)
            {
                self.known_strategy_patterns.push(p.clone());
            }
        }
        if self.known_strategy_patterns.len() > MAX_STRATEGY_PATTERNS {
            self.known_strategy_patterns
                .drain(0..MAX_STRATEGY_PATTERNS / 5);
        }
        for g in &gap_discoveries {
            self.known_gaps.push(g.clone());
        }
        if self.known_gaps.len() > MAX_GAPS {
            self.known_gaps.drain(0..MAX_GAPS / 5);
        }
        for ti in &meta_insights {
            self.known_meta_insights.push(ti.clone());
        }
        if self.known_meta_insights.len() > MAX_META_INSIGHTS {
            self.known_meta_insights.drain(0..MAX_META_INSIGHTS / 5);
        }
    ...
    fn extract_strategy_sequences(&self, trace: &ThinkingTrace) -> Vec<StrategySequencePattern> {
        ...
        // 提取策略序列 (字符串表示)
        let strategies: Vec<String> = trace
            .steps
            .iter()
            .map(|s| format!("{:?}", s.strategy))
            .collect();

        let domains: Vec<String> = trace
            .steps
            .iter()
    ...
    fn extract_gaps(&self, trace: &ThinkingTrace) -> Vec<GapDiscovery> {
        ...
        for step in &trace.steps {
            ...
            // 检测 gap 关键词
            let has_gap_signal = GAP_KEYWORDS
                .iter()
    ...
    fn extract_tool_patterns(&self, trace: &ThinkingTrace) -> Vec<ToolUsagePattern> {
        let all_tools: Vec<Vec<String>> =
            trace.steps.iter().map(|s| s.tools_used.clone()).collect();

        let tool_sequence: Vec<String> = all_tools.into_iter().flatten().collect();
    ...
    fn extract_meta_insights(&self, trace: &ThinkingTrace) -> Vec<MetaCognitiveInsight> {
        ...
        for step in &trace.steps {
            let lower_desc = step.description.to_lowercase();
            let has_meta_signal = META_KEYWORDS.iter().any(|kw| lower_desc.contains(kw));
    ...
    pub fn absorb_to_cognitive_map(
        ...
        for insight in insights {
            ...
            let llm_concept = insight
                .insight
                .split('.')
                .next()
                .unwrap_or(&insight.insight)
                .to_string();
            let neotrix_abs = insight
                .affected_dimensions
                .first()
                .cloned()
            ...
            let notes = format!(
                "auto-mapped from ThinkingTrace insight: {}",
...
mod tests {
    ...
    use crate::core::nt_core_self::thinking_trace::{ReflectionGrade, ThinkingStep};
    ...
    fn test_extract_strategy_sequence_from_short_trace() {
        ...
        assert!(
            report.strategy_patterns.is_empty(),
            "short trace should not produce patterns"
        );
    }
    ...
    fn test_extract_gap_discovery() {
        ...
        let trace = make_test_trace(
            ...
            vec![make_step(
                ...
                "We lack prediction ability, need to implement",
                0.7,
                vec!["search"],
            )],
            "gap discovery",
        ...
        let report = extractor.extract_from_trace(&trace);
        assert!(
            !report.gap_discoveries.is_empty(),
            "should detect gap from 'lack' keyword"
        );
        assert!(report.gap_discoveries[0]
            .suggested_capability_adjustments
            .contains_key("prediction_ability"));
    }
    ...
    fn test_extract_meta_insight() {
        ...
        let trace = make_test_trace(
            ...
            vec![make_step(
                ...
                "I realize the pattern: we must parallelize",
                0.85,
                vec![],
            )],
            "meta insight",
        ...
        let report = extractor.extract_from_trace(&trace);
        assert!(
            !report.meta_insights.is_empty(),
            "should detect meta insight"
        );
    }
    ...
    fn test_metacognitive_health_score() {
        ...
        let trace = make_test_trace(
            ...
            vec![
                ...
                make_step(
                    2,
                    StrategyKind::ChainOfThought,
                    "I realize the approach pattern",
                    0.9,
                    vec![],
                ),
                make_step(
        ...
        assert!(
            report.awakening_signal,
            "rich pattern + gaps + insights should trigger awakening"
        );
    }
    ...
    fn test_absorb_to_cognitive_map() {
        ...
        let insights = vec![MetaCognitiveInsight {
            id: "test".to_string(),
        ...
        assert!(
            map.count() > 14,
            "should have more than original 14 entries"
        );
    }
    ...
    fn test_gaps_to_capability_vector() {
        ...
        let gaps = vec![GapDiscovery {
            id: "test".to_string(),
            description: "Missing inference ability".to_string(),
            suggested_capability_adjustments: vec![("inference_depth".to_string(), 0.2)]
                .into_iter()
                .collect(),
        ...
        if let Some(idx) = CapabilityVector::index_from_name("inference_depth") {
            assert!(
                cv.arr()[idx] > 0.0,
                "inference_depth should be > 0 after gap absorb"
            );
        }
    }
    ...
    fn test_awakening_signal_only_with_gaps_and_insights() {
        ...
        assert!(
            !report.awakening_signal,
            "no gaps or insights should not trigger awakening"
        );
    }
    ...
    fn test_deduplication_of_known_patterns() {
        ...
        assert_eq!(
            r1.strategy_patterns.len(),
            1,
            "first should produce pattern"
        );
        assert!(
            r2.strategy_patterns.is_empty(),
    ...
    fn test_tool_pattern_extraction() {
        ...
        assert_eq!(
            report.tool_patterns[0].tool_sequence,
            vec!["search", "browse", "write_file"]
        );
    }
}
