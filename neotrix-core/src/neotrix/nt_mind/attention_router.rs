...
use crate::neotrix::nt_mind::e8_gwt_bridge;
...
impl AttentionRouter {
    ...
    /// Route with E₈ hexagram bias: keyword salience + E₈-informed specialist biases.
    pub fn route_with_e8(&mut self, context: &str, hex: ReasoningHexagram) -> RoutedContext {
        let raw = self.compute_salience(context);
        let biased = e8_gwt_bridge::apply_hex_bias(&raw, hex);
        // Activate specialists with biased salience
        for (st, salience) in &biased {
            if let Some(s) = self.workspace.specialist_by_type_mut(st) {
                s.activate(*salience);
            }
        }
        ...
        RoutedContext {
            ...
            active_specialists: active,
            knowledge_lines,
            salience_report: biased,
        }
    }

    /// 根据上下文关键词计算 7 个 Specialist 的 salience
    pub fn compute_salience(&self, context: &str) -> Vec<(SpecialistType, f64)> {
        let lower = context.to_lowercase();
        let terms = [
            (
                SpecialistType::PatternMatcher,
    ...
    /// 吸收推理结果到超立方体
    pub fn absorb_reasoning_result(&mut self, topic: &str, _result: &str, source: &str) {
    ...
    pub fn register_harness_profile(
        &mut self,
        env: &str,
        profile: &crate::neotrix::nt_mind::self_iterating::harness_adapter::HarnessProfile,
    ) -> bool {
        self.workspace
            .harness_adapter
...
mod tests {
    ...
    fn test_seed_knowledge_populates_hypercube() {
        ...
        log::debug!("cell_count: before={} after={}", before, after);
    ...
    fn test_build_knowledge_prompt_suffix_references_active_specialists() {
        ...
        assert!(suffix.contains("Knowledge context — activated"));
    }
