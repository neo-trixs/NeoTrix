...
use crate::core::nt_core_self::{StrategyKind, AttentionDomain, ReflectionGrade};
...
impl CognitiveEye {
    ...
    pub fn observe(
        ...
        if attention_domains_active.len() < 2 && self.history.len() >= 3
            && self.history.iter().rev().take(3).all(|s| s.attention_domains_active.len() < 2) {
                spots.push(CognitiveBlindSpot {
                    kind: BlindSpotKind::AttentionStarvation,
                    description: "Consistently <2 active attention heads — reasoning is too narrow".into(),
                    severity: 2,
                    repair: "Stimulate underused domains: SelfReflection, RiskAssessment, Creativity".into(),
        ...
        if context_usage_pct > 0.85 {
            spots.push(CognitiveBlindSpot {
                ...
                description: format!("Context window {:.0}% full — risk of recency bias", context_usage_pct * 100.0),
                severity: 2,
        ...
        if recent_trace_grades.len() >= 3 {
            let recent = recent_trace_grades.iter().rev().take(3).map(|g| g.score()).collect::<Vec<_>>();
            if recent[0] < recent[1] && recent[1] < recent[2] {
                spots.push(CognitiveBlindSpot {
                    kind: BlindSpotKind::GradeDegradation,
                    description: "Last 3 trace grades declining — reasoning quality dropping".into(),
                    severity: 3,
                    ...
                    strategy_boosts: vec![(StrategyKind::Reflection, 0.15), (StrategyKind::IterativeRefinement, 0.1)],
                });
            }
        }
        ...
        let neglected: Vec<&AttentionDomain> = all_domains.iter()
            .filter(|d| !attention_domains_active.contains(d))
            .collect();
        if neglected.len() >= 5 && self.history.len() >= 5 {
            let neglected_labels: Vec<String> = neglected.iter().map(|d| d.label().to_string()).collect();
            spots.push(CognitiveBlindSpot {
                kind: BlindSpotKind::DomainNeglect,
                description: format!("{} domains underused: {}", neglected.len(), neglected_labels.join(", ")),
                severity: 1,
        ...
        if !has_reflection && self.history.len() >= 4 {
            ...
            if last_4.iter().all(|s| !s.attention_domains_active.contains(&AttentionDomain::SelfReflection)) {
                spots.push(CognitiveBlindSpot {
        ...
        if !recent_errors.is_empty() && self.history.len() >= 3 {
            let prev_errors: Vec<&str> = self.history.iter()
                .rev().skip(1).take(2)
            ...
            for error in &recent_errors {
                if prev_errors.contains(&error.as_str()) {
                    spots.push(CognitiveBlindSpot {
                        ...
                        capability_deltas: vec![("debugging".into(), 0.1), ("error_handling".into(), 0.08)],
    ...
    pub fn summary(&self) -> String {
        let by_kind: HashMap<BlindSpotKind, usize> = self.findings.iter()
            .fold(HashMap::new(), |mut acc, f| {
                *acc.entry(f.kind).or_insert(0) += 1;
                acc
            });
        let mut parts: Vec<String> = by_kind.into_iter()
            .map(|(k, c)| format!("{}:{}", k.label(), c))
            .collect();
        parts.sort();
        format!("CognitiveEye #{} | {} spots | [{}]",
            self.total_observations, self.findings.len(), parts.join(" "))
    }
...
mod tests {
    ...
    fn test_observer_creates_snapshot() {
        ...
        let spots = eye.observe(HashMap::new(), vec![AttentionDomain::Code], 0.5, vec![], vec![], &cap);
    ...
    fn test_strategy_fixation_detected() {
        ...
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::StrategyFixation)),
            "expected strategy fixation, got: {:?}", spots.iter().map(|s| s.kind).collect::<Vec<_>>());
    }
    ...
    fn test_attention_starvation_detected() {
        ...
        eye.observe(dist.clone(), vec![AttentionDomain::Code], 0.5, vec![], vec![], &cap);
        eye.observe(dist.clone(), vec![AttentionDomain::Code], 0.5, vec![], vec![], &cap);
        let spots = eye.observe(dist, vec![AttentionDomain::Code], 0.5, vec![], vec![], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::AttentionStarvation)));
    }
    ...
    fn test_context_overload_detected() {
        let mut eye = CognitiveEye::new();
        let cap = CapabilityVector::default();
        let spots = eye.observe(HashMap::new(), vec![AttentionDomain::Code], 0.9, vec![], vec![], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::ContextOverload)));
    }
    ...
    fn test_grade_degradation_detected() {
        let mut eye = CognitiveEye::new();
        let cap = CapabilityVector::default();
        let grades = vec![ReflectionGrade::Excellent, ReflectionGrade::Good, ReflectionGrade::Adequate];
        let spots = eye.observe(HashMap::new(), vec![AttentionDomain::Code], 0.5, grades, vec![], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::GradeDegradation)));
    }
    ...
    fn test_error_recurrence_detected() {
        ...
        let cap = CapabilityVector::default();
        let dist: HashMap<StrategyKind, usize> = HashMap::new();
        eye.observe(dist.clone(), vec![AttentionDomain::Code], 0.5, vec![], vec!["null pointer".to_string()], &cap);
        eye.observe(dist.clone(), vec![AttentionDomain::Code], 0.5, vec![], vec!["other error".to_string()], &cap);
        let spots = eye.observe(dist, vec![AttentionDomain::Code], 0.5, vec![], vec!["null pointer".to_string()], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::ErrorRecurrence)));
    }
    ...
    fn test_low_reflection_detected() {
        ...
        for _ in 0..4 {
            eye.observe(dist.clone(), vec![AttentionDomain::Code, AttentionDomain::Planning], 0.5, grades.clone(), vec![], &cap);
        }
        let spots = eye.observe(dist, vec![AttentionDomain::Code, AttentionDomain::Planning], 0.5, grades, vec![], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::LowReflection)));
    }
    ...
    fn test_domain_neglect_detected() {
        ...
        let dist: HashMap<StrategyKind, usize> = HashMap::new();
        for _ in 0..5 {
            eye.observe(dist.clone(), vec![AttentionDomain::Code, AttentionDomain::Planning], 0.5, vec![], vec![], &cap);
        }
        let spots = eye.observe(dist, vec![AttentionDomain::Code, AttentionDomain::Planning], 0.5, vec![], vec![], &cap);
        assert!(spots.iter().any(|s| matches!(s.kind, BlindSpotKind::DomainNeglect)));
    }
