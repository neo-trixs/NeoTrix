#![allow(dead_code)]
// SPLIT PLAN:
//   File: 2714 lines — split into 6 files:
//   1. `media_format.rs`   — MediaFormat enum + impl (lines 7–407)
//   2. `input_type.rs`     — InputType enum + impl (lines 410–1085)
//   3. `file_metadata.rs`  — FileMetadata + LlmApproachType + ResponseStructure (lines 1088–1251)
//   4. `distiller.rs`      — ReasoningDistiller struct + impl (lines 1254–1798)
//   5. `extract.rs`        — Free functions: extract_metadata, infer_input_type/category/domain (lines 1800–1929)
//   6. `tests.rs`          — #[cfg(test)] module (lines 1930–2714)
//   How: create each file, `pub use` from a new `mod.rs`, delete original.
...
impl MediaFormat {
    ...
    pub fn category(&self) -> &str {
        match self {
            ...
            Self::Html | Self::Css | Self::Scss | Self::Less | Self::Vue | Self::Svelte => "web",
            ...
            | Self::LdJson
            | Self::Hcl
            | Self::Terraform => "config",

            Self::Md | Self::Rst | Self::Adoc | Self::Tex | Self::Ltx | Self::Org => "markup",

            Self::Ttf | Self::Otf | Self::Woff | Self::Woff2 | Self::Eot => "font",

            Self::Pt | Self::Pth | Self::Pkl | Self::BinModel | Self::Onnx | Self::Safetensors => {
                "model"
            }
...
impl InputType {
    ...
    pub fn from_bytes(data: &[u8]) -> Self {
        ...
        if data.starts_with(b"\xff\xd8\xff") {
            return Self::Image(MediaFormat::Jpeg);
        }
        if data.starts_with(b"\x89PNG\r\n\x1a\n") {
            return Self::Image(MediaFormat::Png);
        }
        if data.starts_with(b"GIF87a") || data.starts_with(b"GIF89a") {
            return Self::Image(MediaFormat::Gif);
        }
        if data.starts_with(b"RIFF") && data.windows(4).any(|w| w == b"WEBP") {
            return Self::Image(MediaFormat::Webp);
        }
        if data.starts_with(b"BM") {
            return Self::Image(MediaFormat::Bmp);
        }
        if data.starts_with(b"\x00\x00\x01\x00") || data.starts_with(b"\x00\x00\x02\x00") {
            return Self::Image(MediaFormat::Ico);
        }
        if data.starts_with(b"MM\x00*") || data.starts_with(b"II*\x00") {
            return Self::Image(MediaFormat::Tiff);
        }
        if data.starts_with(b"<?xml") || data.starts_with(b"<svg") {
            return Self::Image(MediaFormat::Svg);
        }
        if data.starts_with(b"\x00\x00\x00\x1cftyp") || data.starts_with(b"\x00\x00\x00\x20ftyp") {
            return Self::Image(MediaFormat::Avif);
        }
        ...
        if data.starts_with(b"RIFF") && data.len() > 8 && &data[8..12] == b"AVI " {
            return Self::Video(MediaFormat::Avi);
        }
        if data.starts_with(b"\x00\x00\x00\x14ftypqt")
            || data.starts_with(b"\x00\x00\x00\x1cftypqt")
        {
            return Self::Video(MediaFormat::Mov);
        }
        if data.starts_with(b"\x1aE\xdf\xa3") {
            return Self::Video(MediaFormat::Mkv);
        }
        if data.starts_with(b"\x30\x26\xb2\x75\x8e\x66\xcf\x11") {
            return Self::Video(MediaFormat::Wmv);
        }
        if data.starts_with(b"FLV\x01") {
            return Self::Video(MediaFormat::Flv);
        }
        if data.starts_with(b"\x1a\x45\xdf\xa3") && data.windows(4).any(|w| w == b"webm") {
            return Self::Video(MediaFormat::Webm);
        }
        if data.starts_with(b"OggS") && data.len() > 28 && data[28] == 0x01 {
            return Self::Video(MediaFormat::Ogv);
        }
        ...
        {
            return Self::Audio(MediaFormat::Mp3);
        }
        if data.starts_with(b"RIFF") && data.len() > 8 && &data[8..12] == b"WAVE" {
            return Self::Audio(MediaFormat::Wav);
        }
        if data.starts_with(b"fLaC") {
            return Self::Audio(MediaFormat::Flac);
        }
        if data.starts_with(b"OggS") && data.len() > 28 && data[28] == 0x02 {
            return Self::Audio(MediaFormat::Ogg);
        }
        if data.starts_with(b"\xff\xf1") || data.starts_with(b"\xff\xf9") {
            return Self::Audio(MediaFormat::Aac);
        }
        if data.starts_with(b"\x30\x26\xb2\x75\x8e\x66\xcf\x11\xa6\xd9\x00\xaa\x00\x62\xce\x6c") {
            return Self::Audio(MediaFormat::Wma);
        }
        if data.starts_with(b"OggS") && data.len() > 28 && data[28] == 0x03 {
            return Self::Audio(MediaFormat::Opus);
        }
        if data.starts_with(b"FORM") && data.len() > 8 && &data[8..12] == b"AIFF" {
            return Self::Audio(MediaFormat::Aiff);
        }
        if data.starts_with(b"#!AMR") || data.starts_with(b"\x23\x21\x41\x4d\x52\x0a") {
            return Self::Audio(MediaFormat::Amr);
        }
        if data.starts_with(b"MThd") {
            return Self::Audio(MediaFormat::Mid);
        }

        // Documents
        if data.starts_with(b"%PDF") {
            return Self::Document(MediaFormat::Pdf);
        }
        if data.starts_with(b"PK\x03\x04") && data.len() > 30 {
            let name_bytes = &data[30..data.len().min(38)];
            let name = String::from_utf8_lossy(name_bytes);
            if name.starts_with("word/") {
                return Self::Document(MediaFormat::Docx);
            }
            if name.starts_with("xl/") {
                return Self::Document(MediaFormat::Xlsx);
            }
            if name.starts_with("ppt/") {
                return Self::Document(MediaFormat::Pptx);
            }
            if name == "mimetype"
                || data.len() > 60 && data[30..58].windows(4).any(|w| w == b"epub")
            {
                return Self::Document(MediaFormat::Epub);
            }
        }
        if data.starts_with(b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1") {
            let clsid = &data[24..40];
            if clsid
                .starts_with(b"\xcf\x11\x89\x9a\xc8\x02\x00\x00\x00\x00\x00\x00\xe0\xc9\x00\x00")
            {
                return Self::Document(MediaFormat::Ppt);
            }
            if clsid
                .starts_with(b"\xcf\x11\x0a\x6b\x08\x02\x00\x00\x00\x00\x00\x00\xe0\xc9\x00\x00")
            {
                return Self::Document(MediaFormat::Xls);
            }
            if clsid
                .starts_with(b"\xcf\x11\xea\x79\x08\x02\x00\x00\x00\x00\x00\x00\xe0\xc9\x00\x00")
            {
                return Self::Document(MediaFormat::Doc);
            }
        }
        if data.starts_with(b"{\\rtf") {
            return Self::Document(MediaFormat::Rtf);
        }
        if data.starts_with(b"PK\x03\x04") && data.len() > 58 {
            let name_bytes = &data[30..data.len().min(80)];
            let name = String::from_utf8_lossy(name_bytes);
            if name.starts_with("mimetype")
                && data.windows(20).any(|w| w == b"application/epub+zip")
            {
                return Self::Document(MediaFormat::Epub);
            }
            if name.contains(".opf") {
                return Self::Document(MediaFormat::Epub);
            }
        }
        ...
        if data.starts_with(b"PK\x03\x04") {
            return Self::Archive(MediaFormat::Zip);
        }
        if data.starts_with(b"\x1f\x8b\x08") {
            return Self::Archive(MediaFormat::Gz);
        }
        if data.starts_with(b"BZh") {
            return Self::Archive(MediaFormat::Bz2);
        }
        if data.starts_with(b"\xfd7zXZ\x00") {
            return Self::Archive(MediaFormat::Xz);
        }
        if data.starts_with(b"\x28\xb5\x2f\xfd") {
            return Self::Archive(MediaFormat::Zst);
        }
        if data.starts_with(b"Rar!\x1a\x07\x01\x00") || data.starts_with(b"Rar!\x1a\x07\x00") {
            return Self::Archive(MediaFormat::Rar);
        }
        if data.starts_with(b"7z\xbc\xaf\x27\x1c") {
            return Self::Archive(MediaFormat::SevenZ);
        }
        if data.starts_with(b"\x04\x22\x4d\x18") {
            return Self::Archive(MediaFormat::Lz4);
        }

        // Fonts
        if data.starts_with(b"\x00\x01\x00\x00\x00") {
            return Self::Font(MediaFormat::Ttf);
        }
        if data.starts_with(b"OTTO") {
            return Self::Font(MediaFormat::Otf);
        }
        if data.starts_with(b"wOFF") {
            return Self::Font(MediaFormat::Woff);
        }
        if data.starts_with(b"wOF2") {
            return Self::Font(MediaFormat::Woff2);
        }

        // Models
        if data.len() > 4 && data[0..4].iter().all(|&b| b.is_ascii_alphanumeric()) {
            let header = String::from_utf8_lossy(&data[..data.len().min(64)]);
            if header.contains("pytorch") || header.contains("torch") {
                return Self::Model(MediaFormat::Pt);
            }
            if header.contains("onnx") {
                return Self::Model(MediaFormat::Onnx);
            }
            if header.contains("safetensors") {
                return Self::Model(MediaFormat::Safetensors);
            }
            if header.contains("pickle") || header.contains("PKL") {
                return Self::Model(MediaFormat::Pkl);
            }
        }
        if data.starts_with(b"\x80\x02")
            || data.starts_with(b"\x80\x03")
            || data.starts_with(b"\x80\x04")
            || data.starts_with(b"\x80\x05")
        {
            return Self::Model(MediaFormat::Pkl);
        }
        ...
        if data.starts_with(b"\x00asm") {
            return Self::Code(MediaFormat::Wasm);
        }
    ...
    pub fn infer(task: &str, has_image: bool, metadata: Option<&FileMetadata>) -> Self {
        ...
        let code_extensions = &[
            ".rs", ".py", ".js", ".ts", ".tsx", ".jsx", ".go", ".java", ".c", ".cpp", ".cxx",
            ".cc", ".h", ".hpp", ".hxx", ".swift", ".kt", ".kts", ".scala", ".rb", ".php", ".php3",
            ".php4", ".php5", ".phtml", ".pl", ".pm", ".lua", ".r", ".R", ".sh", ".bash", ".zsh",
            ".fish", ".ps1", ".bat", ".cmd", ".ex", ".exs", ".dart", ".wasm", ".sol", ".move",
            ".cairo", ".zig", ".nim", ".cr", ".elm", ".erl", ".hrl", ".hs", ".lhs", ".clj",
            ".cljs", ".edn", ".ml", ".mli", ".fs", ".fsx", ".v", ".vhdl", ".vhd", ".s", ".asm",
            ".S", ".inc", ".m", ".mm",
        ];
        ...
        let data_extensions = &[
            ".csv", ".tsv", ".json", ".xml", ".xsd", ".dtd", ".sql", ".sqlite", ".db", ".parquet",
            ".avro", ".proto",
        ];
        let web_extensions = &[
            ".html", ".htm", ".css", ".scss", ".sass", ".less", ".vue", ".svelte",
        ];
        let markup_extensions = &[
            ".md",
            ".markdown",
            ".rst",
            ".adoc",
        ...
        let image_extensions = &[
            ...
            ".avif", ".heic", ".heif", ".raw", ".cr2", ".nef", ".arw",
        ];
        let video_extensions = &[
            ".mp4", ".avi", ".mov", ".mkv", ".wmv", ".flv", ".webm", ".ogv", ".m4v", ".3gp",
            ".mpeg", ".mpg",
        ];
        let audio_extensions = &[
            ".mp3", ".wav", ".flac", ".ogg", ".aac", ".wma", ".m4a", ".opus", ".aiff", ".amr",
            ".mid", ".midi",
        ];
        ...
        let archive_extensions = &[
            ".zip", ".tar", ".gz", ".tgz", ".bz2", ".tbz2", ".xz", ".txz", ".zst", ".rar", ".7z",
            ".lz4", ".z", ".lz", ".arj", ".cab",
        ];
        ...
        let has_file_ref = lower.contains("file:")
        ...
        let any_word_ends_with = |exts: &[&str]| -> bool {
            lower
                .split_whitespace()
                .any(|w| exts.iter().any(|e| w.ends_with(e)))
        };

        if has_file_ref
            || any_word_ends_with(image_extensions)
            || any_word_ends_with(video_extensions)
            || any_word_ends_with(audio_extensions)
            || any_word_ends_with(document_extensions)
            || any_word_ends_with(archive_extensions)
            || any_word_ends_with(code_extensions)
            || any_word_ends_with(config_extensions)
            || any_word_ends_with(data_extensions)
            || any_word_ends_with(web_extensions)
            || any_word_ends_with(markup_extensions)
            || any_word_ends_with(font_extensions)
            || any_word_ends_with(model_extensions)
        {
            let ext = lower
                .split_whitespace()
                .flat_map(|w| {
                    image_extensions
                        ...
                        .chain(data_extensions)
                        .chain(web_extensions)
                        .chain(markup_extensions)
                        .chain(font_extensions)
                        .chain(model_extensions)
...
fn to_media_format(ext: &str) -> MediaFormat {
    match ext {
        ...
        ".jsx" => MediaFormat::Jsx,
        ".go" => MediaFormat::Go,
        ".java" => MediaFormat::Java,
        ".c" => MediaFormat::C,
        ".cpp" | ".cxx" | ".cc" => MediaFormat::Cpp,
        ".h" => MediaFormat::H,
        ".hpp" | ".hxx" => MediaFormat::Hpp,
        ".swift" => MediaFormat::Swift,
        ".kt" | ".kts" => MediaFormat::Kotlin,
        ".scala" => MediaFormat::Scala,
        ".rb" => MediaFormat::Rb,
        ".php" | ".php3" | ".php4" | ".php5" | ".phtml" => MediaFormat::Php,
        ".pl" | ".pm" => MediaFormat::Pl,
        ".lua" => MediaFormat::Lua,
        ".r" | ".R" => MediaFormat::R,
        ".sh" | ".bash" | ".zsh" => MediaFormat::Sh,
        ".fish" => MediaFormat::Fish,
        ".ps1" => MediaFormat::Ps1,
        ".bat" | ".cmd" => MediaFormat::Bat,
        ".ex" => MediaFormat::Ex,
        ".exs" => MediaFormat::Exs,
        ".dart" => MediaFormat::Dart,
        ".wasm" => MediaFormat::Wasm,
        ".sol" => MediaFormat::Sol,
        ".move" => MediaFormat::Move,
        ".cairo" => MediaFormat::Cairo,
        ".html" | ".htm" => MediaFormat::Html,
        ".css" => MediaFormat::Css,
        ".scss" | ".sass" => MediaFormat::Scss,
        ".less" => MediaFormat::Less,
        ".vue" => MediaFormat::Vue,
        ".svelte" => MediaFormat::Svelte,
        ".csv" => MediaFormat::Csv,
        ".tsv" => MediaFormat::Tsv,
        ".json" => MediaFormat::Json,
        ".xml" => MediaFormat::Xml,
        ".sql" => MediaFormat::Sql,
        ".parquet" => MediaFormat::Parquet,
        ".avro" => MediaFormat::Avro,
        ".proto" => MediaFormat::Proto,
        ".env" | ".envrc" => MediaFormat::Env,
        ".ini" => MediaFormat::Ini,
        ".cfg" | ".conf" => MediaFormat::Cfg,
        ".toml" => MediaFormat::Toml,
        ...
        ".mobi" | ".azw3" => MediaFormat::Mobi,
        ".zip" => MediaFormat::Zip,
        ".tar" => MediaFormat::Tar,
        ".gz" | ".tgz" => MediaFormat::Gz,
        ".bz2" | ".tbz2" => MediaFormat::Bz2,
        ".xz" | ".txz" => MediaFormat::Xz,
        ".zst" => MediaFormat::Zst,
        ".rar" => MediaFormat::Rar,
        ...
        ".ttf" => MediaFormat::Ttf,
        ".otf" => MediaFormat::Otf,
        ".woff" => MediaFormat::Woff,
        ".woff2" => MediaFormat::Woff2,
        ".eot" => MediaFormat::Eot,
        ".pt" | ".pth" => MediaFormat::Pt,
        ".pkl" => MediaFormat::Pkl,
        ".onnx" => MediaFormat::Onnx,
        ".safetensors" => MediaFormat::Safetensors,
...
// SECTION: FileMetadata + LlmApproachType + ResponseStructure
...
impl ReasoningDistiller {
    ...
    pub fn recommend_mode_weighted(
        &self,
        _task: &str,
        task_embed: &[u32],
    ) -> Option<(u8, String, f64, Vec<String>)> {
        ...
        for p in &self.patterns {
            ...
            let task_sim = Self::embedding_similarity(
                task_embed,
                &Self::task_embedding(&p.id.replace("rd_", "")),
            );
        ...
        let best = mode_scores
            ...
            .map(|(mode, scores)| {
                ...
                let variance: f64 = if n > 1.0 {
                    scores
                        .iter()
                        .map(|(s, _)| (s / total_w * n - weighted_avg).powi(2))
                        .sum::<f64>()
                        / (n - 1.0)
                } else {
                    0.0
        ...
        let top_approaches: Vec<String> = self.mode_profile(best.0).into_iter().take(3).collect();
    ...
    pub fn recommend_mode_for_input(
        &self,
        task: &str,
        has_image: bool,
        metadata: Option<&FileMetadata>,
    ) -> Option<(u8, String, Vec<String>)> {
        let input_type = InputType::infer(task, has_image, metadata);
        let embed = Self::task_embedding(task);
        let patterns_by_input: Vec<&LlmReasoningPattern> = self
            .patterns
            .iter()
        ...
        for p in &patterns_by_input {
            ...
            let age_weight = Self::decay_weight(self.decay_rate, elapsed.max(1));
            let task_sim =
                Self::embedding_similarity(&embed, &Self::task_embedding(&p.id.replace("rd_", "")));
    ...
    pub fn distillation_insight(&self) -> Vec<String> {
        ...
        let top_mode = self.mode_success_rates.iter().max_by(|(_, a), (_, b)| {
        ...
        let total_approaches: usize = self
    ...
    pub fn type_insights(&self) -> Vec<String> {
        ...
        for p in &self.patterns {
            let entry = type_hits
                .entry(p.input_type.name().to_string())
                .or_insert((0, 0.0));
            entry.0 += 1;
    ...
    pub fn mode_profile(&self, mode: u8) -> Vec<String> {
        let mode_patterns: Vec<&LlmReasoningPattern> =
            self.patterns.iter().filter(|p| p.e8_mode == mode).collect();

        if mode_patterns.is_empty() {
            return vec![format!("推理模式{} 无观察数据", mode)];
        }
        ...
        if input_counts.len() > 1 {
            ...
            let input_str: Vec<String> = sorted_input
                .iter()
                .map(|(t, c)| format!("{}:{}", t, c))
                .collect();
    ...
    pub fn type_profile(&self, input_type: &InputType) -> Vec<String> {
        let type_patterns: Vec<&LlmReasoningPattern> = self
            .patterns
            .iter()
...
// SECTION: Free functions (extract_metadata, infer_input_type, etc.)

fn extract_metadata(data: &[u8], input: &InputType) -> FileMetadata {
    ...
    match input {
        InputType::Image(f) => match f {
            MediaFormat::Png => {
                if data.len() >= 24 && data.starts_with(b"\x89PNG\r\n\x1a\n") {
                    ...
                    if data.len() > 25 {
                        meta.has_alpha = data[25] == 6 || data[25] == 4;
                    }
                }
            }
            MediaFormat::Jpeg => {
                        ...
                        while pos + 8 < data.len() {
                            ...
                            if &data[pos..pos + 4] == b"data" {
                                if sample_rate > 0 && channels > 0 && bits_per_sample > 0 {
                                    let bytes_per_sec = sample_rate as u64
                                        * channels as u64
                                        * (bits_per_sample as u64 / 8);
                                    if bytes_per_sec > 0 {
                                        meta.duration_ms =
                                            Some((chunk_size as u64 * 1000) / bytes_per_sec);
                                    }
                                }
                                break;
                            }
        ...
        InputType::Document(f) => match f {
            MediaFormat::Pdf => {
                if let Ok(content) = std::str::from_utf8(data) {
                    let pages_count = content
                        .split("/Type")
                        .skip(1)
                        .filter(|s| s.trim_start().starts_with("/Pages"))
                        .filter_map(|s| {
                            let after_count = s.split("/Count").nth(1)?;
                            let count_str = after_count.trim_start().split_whitespace().next()?;
...
mod tests {
    ...
    fn test_input_type_format() {
        assert_eq!(
            InputType::Image(MediaFormat::Jpeg).format(),
    ...
    fn test_from_bytes_gif() {
        assert_eq!(
            InputType::from_bytes(b"GIF87a"),
            InputType::Image(MediaFormat::Gif)
        );
        assert_eq!(
            InputType::from_bytes(b"GIF89a"),
            InputType::Image(MediaFormat::Gif)
        );
    }
    ...
    fn test_from_bytes_pdf() {
        assert_eq!(
            InputType::from_bytes(b"%PDF-1.4"),
            InputType::Document(MediaFormat::Pdf)
        );
    }
    ...
    fn test_from_bytes_zip() {
        assert_eq!(
            InputType::from_bytes(b"PK\x03\x04"),
            InputType::Archive(MediaFormat::Zip)
        );
    }
    ...
    fn test_from_bytes_mp3() {
        assert_eq!(
            InputType::from_bytes(b"ID3"),
            InputType::Audio(MediaFormat::Mp3)
        );
    }
    ...
    fn test_from_bytes_wasm() {
        assert_eq!(
            InputType::from_bytes(b"\x00asm\x01\x00\x00\x00"),
            InputType::Code(MediaFormat::Wasm)
        );
    }
    ...
    fn test_from_bytes_tiff() {
        assert_eq!(
            InputType::from_bytes(b"MM\x00*"),
            InputType::Image(MediaFormat::Tiff)
        );
        assert_eq!(
            InputType::from_bytes(b"II*\x00"),
            InputType::Image(MediaFormat::Tiff)
        );
    }
    ...
    fn test_from_bytes_gzip() {
        assert_eq!(
            InputType::from_bytes(b"\x1f\x8b\x08"),
            InputType::Archive(MediaFormat::Gz)
        );
    }
    ...
    fn test_recommend_for_type_no_data() {
        ...
        assert!(distiller
            .recommend_for_type(&InputType::Image(MediaFormat::Png))
            .is_none());
    }
    ...
    fn test_distiller_insights() {
        ...
        for i in 0..10 {
            distiller.observe(
                &format!("task {}", i),
                "Some response text here. Multiple sentences. With analysis. And findings.",
                3,
                "Code",
                0.7 + (i as f64 * 0.02),
                false,
    ...
    fn test_mode_profile() {
        ...
        assert!(profile[0].contains("推理模式7"));
    }
    ...
    fn test_recommend_mode_for_input_fallback() {
        let distiller = ReasoningDistiller::new();
        assert!(distiller
            .recommend_mode_for_input("fix bug", false, None)
            .is_none());
    }
    ...
    fn test_infer_tiff_extensions() {
        assert_eq!(
            InputType::infer("image.tiff", false, None),
            InputType::Image(MediaFormat::Tiff)
        );
        assert_eq!(
            InputType::infer("image.tif", false, None),
            InputType::Image(MediaFormat::Tiff)
        );
    }
    ...
    fn test_infer_webp() {
        assert_eq!(
            InputType::infer("image.webp", false, None),
            InputType::Image(MediaFormat::Webp)
        );
    }
    ...
    fn test_infer_heic() {
        assert_eq!(
            InputType::infer("photo.heic", false, None),
            InputType::Image(MediaFormat::Heic)
        );
    }
    ...
    fn test_infer_mov() {
        assert_eq!(
            InputType::infer("video.mov", false, None),
            InputType::Video(MediaFormat::Mov)
        );
    }
    ...
    fn test_infer_mkv() {
        assert_eq!(
            InputType::infer("video.mkv", false, None),
            InputType::Video(MediaFormat::Mkv)
        );
    }
    ...
    fn test_infer_webm() {
        assert_eq!(
            InputType::infer("video.webm", false, None),
            InputType::Video(MediaFormat::Webm)
        );
    }
    ...
    fn test_infer_ogg_video() {
        assert_eq!(
            InputType::infer("video.ogv", false, None),
            InputType::Video(MediaFormat::Ogv)
        );
    }
    ...
    fn test_infer_flac() {
        assert_eq!(
            InputType::infer("song.flac", false, None),
            InputType::Audio(MediaFormat::Flac)
        );
    }
    ...
    fn test_infer_opus() {
        assert_eq!(
            InputType::infer("audio.opus", false, None),
            InputType::Audio(MediaFormat::Opus)
        );
    }
    ...
    fn test_infer_aiff() {
        assert_eq!(
            InputType::infer("sound.aiff", false, None),
            InputType::Audio(MediaFormat::Aiff)
        );
    }
    ...
    fn test_infer_pptx() {
        assert_eq!(
            InputType::infer("slides.pptx", false, None),
            InputType::Document(MediaFormat::Pptx)
        );
    }
    ...
    fn test_infer_xlsx() {
        assert_eq!(
            InputType::infer("data.xlsx", false, None),
            InputType::Document(MediaFormat::Xlsx)
        );
    }
    ...
    fn test_infer_epub() {
        assert_eq!(
            InputType::infer("book.epub", false, None),
            InputType::Document(MediaFormat::Epub)
        );
    }
    ...
    fn test_infer_rar() {
        assert_eq!(
            InputType::infer("archive.rar", false, None),
            InputType::Archive(MediaFormat::Rar)
        );
    }
    ...
    fn test_infer_sevenz() {
        assert_eq!(
            InputType::infer("archive.7z", false, None),
            InputType::Archive(MediaFormat::SevenZ)
        );
    }
    ...
    fn test_infer_svg() {
        assert_eq!(
            InputType::infer("icon.svg", false, None),
            InputType::Image(MediaFormat::Svg)
        );
    }
    ...
    fn test_infer_bmp() {
        assert_eq!(
            InputType::infer("image.bmp", false, None),
            InputType::Image(MediaFormat::Bmp)
        );
    }
    ...
    fn test_infer_type_from_task_sql() {
        assert_eq!(
            InputType::infer("query.sql", false, None),
            InputType::Data(MediaFormat::Sql)
        );
    }
    ...
    fn test_infer_type_from_task_html() {
        assert_eq!(
            InputType::infer("index.html", false, None),
            InputType::Web(MediaFormat::Html)
        );
    }
    ...
    fn test_infer_type_from_task_yaml() {
        assert_eq!(
            InputType::infer("config.yaml", false, None),
            InputType::Config(MediaFormat::Yaml)
        );
    }
    ...
    fn test_infer_type_from_task_wasm() {
        assert_eq!(
            InputType::infer("module.wasm", false, None),
            InputType::Code(MediaFormat::Wasm)
        );
    }
    ...
    fn test_infer_type_from_task_onnx() {
        assert_eq!(
            InputType::infer("model.onnx", false, None),
            InputType::Model(MediaFormat::Onnx)
        );
    }
    ...
    fn test_infer_type_from_task_ttf() {
        assert_eq!(
            InputType::infer("font.ttf", false, None),
            InputType::Font(MediaFormat::Ttf)
        );
    }
    ...
    fn test_infer_type_from_task_otf() {
        assert_eq!(
            InputType::infer("font.otf", false, None),
            InputType::Font(MediaFormat::Otf)
        );
    }
}
