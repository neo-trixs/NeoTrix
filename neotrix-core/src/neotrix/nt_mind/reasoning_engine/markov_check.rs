...
impl MarkovCheck {
    ...
    pub fn evaluate(&mut self, z_prev: &[f64], action: u8, z_next: &[f64], random_z: &[f64]) -> f64 {
    ...
    pub fn markov_score(&self) -> f64 {
        (self.inverse_accuracy * 0.6 + self.contrastive_score * 0.4).max(0.0).min(1.0)
    }
...
mod tests {
    ...
    fn test_similar_states_same_action_high_inverse_accuracy() {
        ...
        assert!(acc > 0.5, "similar states should have high inverse accuracy, got {}", acc);
    }
    ...
    fn test_contrastive_score_higher_for_temporally_close_states() {
        ...
        assert!(score > 0.5, "temporally close states should have high contrastive score, got {}", score);
    }
    ...
    fn test_contrastive_score_lower_for_random_pairs() {
        ...
        let z_random = vec![0.5, 0.3, 0.7];
        let score = mc.check_contrastive(&z_prev, &z_next, &z_random);
        assert!(score < 0.5, "dissimilar pairs should have low contrastive score, got {}", score);
    }
    ...
    fn test_markov_score_combines_metrics() {
        ...
        assert!(score > 0.0 && score <= 1.0, "markov score should be in [0,1], got {}", score);
    }
