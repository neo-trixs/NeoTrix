...
impl ContextManager {
    ...
    pub fn insert(&mut self, content: &str, priority: u8) {
        ...
        match target_zone {
            MemoryZone::Frozen => self.frozen.push(entry),
    ...
    pub fn freeze(&mut self, index: usize) {
        ...
        let mut entry = self.active.remove(index).expect("result");
...
mod tests {
    ...
    fn test_evict_active_moves_entries_to_compress() {
        ...
        for i in (0..10).rev() {
            cm.active.push_back(ContextEntry::new(&format!("entry {}", i), i * 10));
            cm.current_tokens += cm.active.back().expect("value should be ok in test").token_estimate;
        }
    ...
    fn test_maintain_compress_zone() {
        ...
        assert!(usage >= cm.compress_threshold, "usage={:.2} should be >= 0.60 for compress test", usage);
    ...
    fn test_maintain_critical_zone() {
        ...
        let usage = cm.current_tokens as f64 / cm.max_tokens as f64;
        assert!(usage >= cm.active_threshold, "usage={:.2} should be >= 0.80 for critical test", usage);
    ...
    fn test_boundary_at_exact_thresholds() {
        ...
        let report = cm.pressure_report();
        assert!(report.usage_pct >= 59.0, "usage={:.2} should be near 60%", report.usage_pct);
        ...
        assert!(report2.usage_pct >= 79.0, "usage={:.2} should be near 80%", report2.usage_pct);
