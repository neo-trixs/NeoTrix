...
impl ReasoningEngine {
    pub fn reason_conversation(&mut self, query: &str) -> NeoTrixResult<String> {
        ...
        let prompt = format!(
            "You are NeoTrix, a helpful reasoning assistant.\n\
    ...
    pub fn reason_task(&mut self, task: &str) -> NeoTrixResult<String> {
        ...
        let principles_hint: String = self
            .principles
            .iter()
            .map(|p| {
                format!(
                    "  Principle: {} (avg_reward={:.2})",
                    p.description, p.avg_reward
                )
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    pub fn reason_error(&mut self, error_info: &str) -> NeoTrixResult<String> {
        ...
        let anti_patterns_hint: String = self
            .anti_patterns
            .iter()
            .map(|a| {
                format!(
                    "  Avoid: {} (seen {} failures)",
                    a.description, a.failure_count
                )
            })
            .collect::<Vec<_>>()
            .join("\n");
        let prompt = format!(
            "You are NeoTrix error diagnostician.\n\
             Error:\n{error_info}\n\n\
             Past solutions:\n{fixes}\n\n\
             Known anti-patterns to avoid:\n{anti}\n\n\
             Provide: 1) Root cause 2) Fix 3) Prevention for future",
            anti = if anti_patterns_hint.is_empty() {
                "  (none)"
            } else {
                &anti_patterns_hint
            }
    ...
    pub fn reason_knowledge(&mut self, query: &str) -> NeoTrixResult<String> {
        ...
        let knowledge: String = memories
            .iter()
            .map(|m| {
                format!(
                    "  {} (type={:?}, reward={:.2})",
                    m.task_description, m.task_type, m.reward
                )
            })
            .collect::<Vec<_>>()
            .join("\n");
        let prompt = format!(
            "You are NeoTrix knowledge base.\n\
             Query: {query}\n\n\
             Relevant experience:\n{knowledge}\n\n\
             Answer from knowledge + your training.",
            knowledge = if knowledge.is_empty() {
                "  (no direct match)"
            } else {
                &knowledge
            }
    ...
    pub fn reason_deep_research(
        ...
        if intent.needs_docs {
            let docs_prompt = format!(
                "Find documentation, API references, and official resources about: {}",
                query
            );
