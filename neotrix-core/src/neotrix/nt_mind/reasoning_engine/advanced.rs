...
use crate::neotrix::nt_mind::reasoning_engine::{ReasoningEngine, MAX_TRACES};
use crate::neotrix::nt_mind::reasoning_types::{
    CascadeConfig, CascadeResult, PerspectiveLens, ReasoningMethod, ReasoningTrace, ReasoningType,
};

impl ReasoningEngine {
    pub fn reason_multi_perspective(
        &mut self,
        task: &str,
            ...
            match self.call_llm(&prompt) {
                Ok(response) => {
                    ...
                    let trace = ReasoningTrace {
                        ...
                        vsa_tag: Some(VsaTagged::self_thought("reasoning")),
                    };
    ...
    fn update_capability_from_trace(&mut self, score: f64) {
        ...
        self.brain.capability_mut().arr_mut()[inf_idx] = (cur + score * 0.02).min(1.0);
        self.brain.capability_mut().normalize();
    }

    pub fn reason_cascade(
        &mut self,
        task: &str,
        config: Option<CascadeConfig>,
    ) -> NeoTrixResult<CascadeResult> {
        let mode = self.select_mode(task);
        self.current_state = self.current_state.transition_to(mode);
        self.state_trajectory.push(self.current_state);
