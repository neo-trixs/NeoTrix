...
use crate::neotrix::nt_mind::distillation::{ExperienceDistiller, StrategicPrinciple, AntiPattern};
use crate::neotrix::nt_world_model::TaskType;

impl ExperienceDistiller {
    pub fn distill_traces(traces: &[ReasoningTrace]) -> Vec<StrategicPrinciple> {
        let mut grouped: std::collections::HashMap<ReasoningType, Vec<&ReasoningTrace>> = std::collections::HashMap::new();
        for t in traces {
            if t.success {
        ...
        for (rtype, group) in grouped {
            ...
            let avg_score: f64 = group.iter().map(|t| t.outcome_score).sum::<f64>() / group.len() as f64;
            let tt = match rtype {
            ...
            principles.push(StrategicPrinciple {
                ...
                description: format!("Success pattern for {:?}: {} successful traces, avg score {:.2}", rtype, group.len(), avg_score),
                task_type: tt,
    ...
    pub fn contrastive_reflect_traces(traces: &[ReasoningTrace]) -> Vec<AntiPattern> {
        ...
        for f in &failures {
            let similar_success = successes.iter()
                .filter(|s| s.reasoning_type == f.reasoning_type)
                .max_by(|a, b| a.outcome_score.partial_cmp(&b.outcome_score).expect("expected valid comparison"));
            if let Some(_s) = similar_success {
                ...
                anti.push(AntiPattern {
                    id: uuid::Uuid::new_v4().to_string(),
                    description: format!("Failure pattern for {:?}: check approach", f.reasoning_type),
                    task_type: tt,
