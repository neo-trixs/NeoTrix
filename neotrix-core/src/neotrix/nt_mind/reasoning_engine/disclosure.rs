...
impl DisclosureLevel {
    pub fn all() -> Vec<Self> {
        vec![
            Self { level: 0, label: "summary".into() },
            Self { level: 1, label: "details".into() },
            Self { level: 2, label: "full".into() },
            Self { level: 3, label: "complete".into() },
        ]
    }
}
...
pub fn render_progressive(trace: &ReasoningTrace, level: u8) -> String {
    ...
    match level {
        ...
        _ => {
            let mut out = format!("{}\nPrompt: {}\nResponse: {}", base, trace.prompt, trace.llm_response);
...
mod tests {
    use super::*;
    use crate::neotrix::nt_mind::reasoning_types::{ReasoningType, ReasoningMethod, PerspectiveLens};
