        ...
        if report.has_actionable_insight && !report.capability_deltas.is_empty() {
            for (name, delta) in &report.capability_deltas {
                if let Some(idx) = CapabilityVector::index_from_name(name) {
                    ...
                    self.brain.capability_mut().arr_mut()[idx] = (cur + delta).clamp(0.0, 1.0);
                }
            }
    ...
    pub fn self_iterate(&mut self) {
        ...
        for a in new_anti {
            if !self
                .anti_patterns
                .iter()
                .any(|existing| existing.description == a.description)
            {
                self.anti_patterns.push(a);
            }
        }

        apply_principles(self.brain.capability_mut(), &self.principles, 0.5);
        avoid_anti_patterns(self.brain.capability_mut(), &self.anti_patterns);

        let type_coverage = self
            .traces
            .iter()
        ...
        self.brain.capability_mut().arr_mut()[analysis_idx] =
            (current_analysis + type_coverage * 0.05).min(1.0);
