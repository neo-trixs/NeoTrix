...
mod tests {
    ...
    #[test]
    fn test_infer_type() {
        assert_eq!(ReasoningEngine::infer_reasoning_type("fix bug"), ReasoningType::ErrorDebugging);
        assert_eq!(ReasoningEngine::infer_reasoning_type("implement x"), ReasoningType::TaskSolving);
        assert_eq!(ReasoningEngine::infer_reasoning_type("what is X"), ReasoningType::KnowledgeQuery);
        assert_eq!(ReasoningEngine::infer_reasoning_type("hello"), ReasoningType::Conversation);
    }
    ...
    fn test_e8_state_save_load() {
        ...
        assert!(save_result.is_ok(), "E8 save failed: {:?}", save_result.err());
        ...
        assert!(load_result.is_ok(), "E8 load failed: {:?}", load_result.err());
