...
impl ThinkingBridge {
    ...
    pub fn observe_task(&mut self, task: &str) -> usize {
        self.silicon.observe(&format!("task:{}", task))
    }
    pub fn observe_tool_use(&mut self, tool: &str, result: &str) -> (usize, usize) {
    ...
    pub fn evaluate_cognitive_health(&mut self) -> &CognitiveHealthReport {
        ...
        let idx = self.evaluator.history.len() - 1;
        let stability = self.evaluator.history[idx].stability_score;
        self.monitor.record_plan("cognitive-eval", stability);
        if self.monitor.auto_tune_enabled {
            let _ = self.monitor.auto_tune(&self.silicon);
        }
        &self.evaluator.history[idx]
    }
    ...
    pub fn auto_crystallize(&mut self) -> String {
        ...
        let traces: Vec<ThinkingTrace> = self
            ...
            .collect();
        let extracted =
            SkillBridge::extract_from_recent(&mut self.skill_registry, &traces, iteration);
...
mod tests {
    ...
    fn test_observe_task() {
        let mut bridge = ThinkingBridge::new("/tmp");
        let id = bridge.observe_task("analyze architecture");
        assert!(id > 0 || id == 0);
    ...
    fn test_observe_tool_use() {
        let mut bridge = ThinkingBridge::new("/tmp");
        let (call, result) = bridge.observe_tool_use("grep", "found 3 matches");
        assert!(call < result);
    }
    #[test]
    ...
    fn test_skill_extract_from_recent() {
        ...
        let extracted = SkillBridge::extract_from_recent(&mut reg, &[good_trace, poor_trace], 1);
    ...
    fn test_skill_auto_maintain_empty() {
        let mut reg = CrystalRegistry::new();
        assert_eq!(
            SkillBridge::auto_maintain_registry(&mut reg),
