...
/// Model tier classification — HashCortX getAgentTier
...
impl FailoverStrategy {
    ...
    pub fn find_failover<'a>(
        ...
        let current_tier = AgentTier::parse(failed_model);
        ...
        let marker = format!(
            ...
            failover_count,
            failed_model,
            best.default_model.as_deref().unwrap_or(&best.name)
        );
...
impl ProviderProfile {
    ...
    pub fn effective_tier(&self) -> AgentTier {
        if let Some(ref model) = self.default_model {
            AgentTier::parse(model)
        } else {
            // Provider-level tier heuristic
            match ProviderKind::from_type(&self.provider_type) {
                ProviderKind::Groq => AgentTier::Capable,
                ProviderKind::Gemini => AgentTier::Frontier,
                ProviderKind::OpenAI => AgentTier::Frontier,
                ProviderKind::Anthropic => AgentTier::Frontier,
                ProviderKind::Cerebras => AgentTier::Capable,
                ProviderKind::SambaNova => AgentTier::Capable,
                ProviderKind::DeepSeek => AgentTier::Strong,
                ProviderKind::Mistral => AgentTier::Capable,
                ProviderKind::Moonshot => AgentTier::Capable,
                ProviderKind::OpenRouter => AgentTier::Capable,
                ProviderKind::Ollama => AgentTier::Moderate,
                ProviderKind::Custom => AgentTier::Moderate,
            }
        }
    }
}
...
mod tests {
    ...
    fn test_model_tier_parse_frontier() {
        assert_eq!(AgentTier::parse("claude-4-opus"), AgentTier::Frontier);
        assert_eq!(AgentTier::parse("gpt-5"), AgentTier::Frontier);
    }

    #[test]
    fn test_model_tier_parse_strong() {
        assert_eq!(AgentTier::parse("claude-sonnet-4"), AgentTier::Strong);
        assert_eq!(AgentTier::parse("gpt-4o"), AgentTier::Strong);
        assert_eq!(AgentTier::parse("deepseek-v3"), AgentTier::Strong);
    }

    #[test]
    fn test_model_tier_parse_capable() {
        assert_eq!(AgentTier::parse("claude-haiku"), AgentTier::Capable);
        assert_eq!(AgentTier::parse("gpt-4o-mini"), AgentTier::Capable);
        assert_eq!(AgentTier::parse("llama-70b"), AgentTier::Capable);
    }

    #[test]
    fn test_model_tier_parse_small() {
        assert_eq!(AgentTier::parse("phi-3"), AgentTier::Small);
        assert_eq!(AgentTier::parse("tiny-llama"), AgentTier::Small);
    }
    ...
    fn test_failover_deterministic_order() {
        ...
        let profiles = vec![
            ProviderProfile {
                name: "slow-llama".into(),
                provider_type: "ollama".into(),
                api_key: None,
                base_url: None,
                default_model: Some("llama3.1-8b".into()),
                is_active: false,
            },
            ProviderProfile {
                name: "fast-groq".into(),
    ...
    fn test_failover_excludes_failed() {
        ...
        let profiles = vec![
            ProviderProfile {
                ...
                api_key: None,
                base_url: None,
                default_model: Some("llama-70b".into()),
                is_active: false,
            },
            ProviderProfile {
                name: "gemini-1".into(),
                ...
                api_key: None,
                base_url: None,
                default_model: Some("gemini-2.0".into()),
                is_active: false,
            },
        ];
        ...
        assert!(result.is_some());
        assert_eq!(
            result.expect("failover should find gemini-1").0.name,
            "gemini-1"
        );
    }
    ...
    fn test_failover_marker_format() {
        ...
        assert!(result.is_some());
        let marker = result
            .expect("failover should find a candidate for marker")
            .1;
    ...
    fn test_provider_tier_heuristic() {
        ...
        assert_eq!(profile.effective_tier(), AgentTier::Moderate);
        ...
        assert_eq!(profile2.effective_tier(), AgentTier::Frontier);
    }

    #[test]
    fn test_model_tier_label() {
        assert_eq!(AgentTier::Frontier.label(), "frontier");
        assert_eq!(AgentTier::Small.label(), "small");
    }
    ...
    fn test_provider_kind_free_tier_order() {
        ...
        assert!(
            ProviderKind::Ollama.free_tier_order() > ProviderKind::OpenRouter.free_tier_order()
        );
    }
    ...
    fn test_failover_tier_preference() {
        ...
        assert_eq!(
            result.expect("failover should prefer same-tier").0.name,
            "same-tier"
        );
    }
}
