...
impl AutoFixer {
    /// 运行 cargo fix 自动修复编译警告 — 测试时跳过以避免 build lock 死锁
    pub fn cargo_fix() -> Result<String, String> {
        if cfg!(test) {
            return Ok("skipped during test".into());
        }
    ...
    pub fn enable_ignored_test(file_path: &str, line: usize) -> Result<String, String> {
        let content = std::fs::read_to_string(file_path).map_err(|e| format!("读取失败: {}", e))?;
        ...
        if lines[idx].trim() == "#[ignore]" {
            ...
            let fpath: &std::path::Path = file_path.as_ref();
            let tmp = fpath.with_extension("tmp");
            std::fs::write(&tmp, &new_content).map_err(|e| format!("写入失败: {}", e))?;
            std::fs::rename(&tmp, fpath).map_err(|e| format!("重命名失败: {}", e))?;
    ...
    pub fn split_file(file_path: &str) -> Result<String, String> {
        ...
        for (i, block) in created.iter().enumerate() {
            ...
            let tmp = item_file.with_extension("tmp");
            std::fs::write(&tmp, block).map_err(|e| format!("写入失败: {}", e))?;
            std::fs::rename(&tmp, &item_file).map_err(|e| format!("重命名失败: {}", e))?;
        ...
        let tmp = mod_path.with_extension("tmp");
        std::fs::write(&tmp, &mod_rs).map_err(|e| format!("写入 mod.rs 失败: {}", e))?;
        std::fs::rename(&tmp, &mod_path).map_err(|e| format!("重命名 mod.rs 失败: {}", e))?;

        // 删除原文件
        std::fs::remove_file(file_path).map_err(|e| format!("删除原文件失败: {}", e))?;
