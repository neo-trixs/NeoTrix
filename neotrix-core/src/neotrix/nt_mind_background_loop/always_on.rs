    ...
    pub fn parse(expr: &str) -> Result<Self, String> {
        ...
        if let Some(rest) = expr.strip_prefix("every ") {
            ...
            if let Some(num) = rest.strip_suffix('m') {
                if let Ok(m) = num.parse::<u64>() {
                    return Ok(Self::Every { interval_secs: m * 60 });
                }
            }
            if let Some(num) = rest.strip_suffix('h') {
                if let Ok(h) = num.parse::<u64>() {
                    return Ok(Self::Every { interval_secs: h * 3600 });
                }
            }
        ...
        if let Some(rest) = expr.strip_prefix("daily at ") {
            ...
            if parts.len() == 2 {
                ...
                let minute = parts[1].parse::<u8>().map_err(|_| format!("Invalid minute: {}", parts[1]))?;
                if hour > 23 || minute > 59 {
                    return Err(format!("Invalid time: {hour}:{minute}"));
                }
                return Ok(Self::Daily { hour, minute });
            }
            return Err(format!("Invalid daily expression: '{expr}'. Use 'daily at HH:MM'"));
        }
        if let Some(rest) = expr.strip_prefix("weekly on ") {
            if let Some((wd, tm)) = rest.trim().split_once(" at ") {
                let weekday = wd.trim().parse::<u8>().map_err(|_| format!("Invalid weekday: {wd}"))?;
                if weekday > 6 {
                    return Err(format!("Invalid weekday: {weekday}. Use 0=Mon to 6=Sun"));
                }
                let tp: Vec<&str> = tm.trim().split(':').collect();
                if tp.len() == 2 {
                    let hour = tp[0].parse::<u8>().map_err(|_| format!("Invalid hour: {}", tp[0]))?;
                    let minute = tp[1].parse::<u8>().map_err(|_| format!("Invalid minute: {}", tp[1]))?;
                    if hour > 23 || minute > 59 {
                        return Err(format!("Invalid time: {hour}:{minute}"));
                    }
                    return Ok(Self::Weekly { weekday, hour, minute });
                }
            }
            return Err(format!("Invalid weekly expression: '{expr}'. Use 'weekly on <0-6> at HH:MM'"));
        }
        ...
        match *self {
            ...
            Self::Daily { hour, minute } => {
                ...
                let target = chrono::DateTime::<chrono::Utc>::from_naive_utc_and_offset(
                    today.and_hms_opt(hour as u32, minute as u32, 0).expect("Invalid time"),
                    chrono::Utc,
                );
                if target > from {
                    target
                    ...
                    chrono::DateTime::<chrono::Utc>::from_naive_utc_and_offset(
                        tomorrow.and_hms_opt(hour as u32, minute as u32, 0).expect("Invalid time"),
                        chrono::Utc,
                    )
                }
            }
            Self::Weekly { weekday, hour, minute } => {
                ...
                loop {
                    ...
                    if candidate.weekday() == target_wd {
                        ...
                        let target = chrono::DateTime::<chrono::Utc>::from_naive_utc_and_offset(
                            date.and_hms_opt(hour as u32, minute as u32, 0).expect("Invalid time"),
                            chrono::Utc,
                        );
                        if target > from {
                            return target;
...
impl AlwaysOnEngine {
    ...
    pub fn list_tasks(&self, filter: Option<&str>) -> Vec<&AlwaysOnTask> {
        match filter {
            ...
            Some("active") => self.tasks.iter().filter(|t| t.run_count < t.max_runs).collect(),
            Some("completed") => {
                let completed_ids: std::collections::HashSet<String> = self.completed_task_ids.iter().cloned().collect();
                self.tasks.iter().filter(|t| completed_ids.contains(&t.id)).collect()
            }
    ...
    pub fn status(&self) -> EngineStatus {
        EngineStatus {
            ...
            active_tasks: self.tasks.iter().filter(|t| t.run_count < t.max_runs).count(),
    ...
    pub fn full_cycle(&mut self) -> Result<CycleReport, String> {
        ...
        let pending: Vec<usize> = self.tasks.iter().enumerate()
            ...
            .filter(|(_, t)| {
                ...
                if let Some(ref sched) = t.schedule {
                    ...
                    if t.interval_secs == 0 { return true; }
                    match t.last_run {
                        Some(last) => {
