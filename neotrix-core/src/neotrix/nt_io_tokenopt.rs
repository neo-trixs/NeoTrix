...
pub fn entropy_gate(text: &str, keep_ratio: f64) -> String {
    ...
    let mut scored: Vec<(usize, f64, &str)> = lines
        ...
        .map(|(i, l)| {
            ...
            let entropy = trimmed.chars().filter(|c| c.is_alphanumeric()).count() as f64
...
mod tests {
    ...
    fn test_content_type_detection() {
        ...
        assert_eq!(
            ContentType::detect("def hello(): pass"),
            ContentType::CodePython
        );
        assert_eq!(
        ...
        assert_eq!(
            ContentType::detect("ERROR: connection failed"),
            ContentType::Log
        );
        assert!(matches!(
            ContentType::detect("hello world"),
            ContentType::Generic
        ));
    }
