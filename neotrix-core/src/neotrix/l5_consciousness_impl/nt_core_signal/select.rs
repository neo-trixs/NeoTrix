...
// === Mamba-2 SSD Operator ===
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SsdOperator {
    pub base: SelectableOperator,
    pub ssd_state: super::core::SsdState,
}
...
impl SsdOperator {
    pub fn new(dim: usize, hidden_dim: usize) -> Self {
        ...
        Self {
            base: SelectableOperator::new(dim, n),
    ...
    pub fn step_ssd(&mut self, input: &Vector, attention_context: &Vector) -> Vector {
        ...
        let dim = input.len().min(self.ssd_state.hidden.len());
        for i in 0..dim {
            let gate = sigmoid(input[i]);
            let ssm_val = ssm_output.get(i).copied().unwrap_or(0.0);
            self.ssd_state.hidden[i] = gate * ssm_val + (1.0 - gate) * self.ssd_state.hidden[i];
        }

        let cache_dim = dim.min(self.ssd_state.cache.len());
        for i in 0..cache_dim {
            let ctx = attention_context.get(i).copied().unwrap_or(0.0);
            self.ssd_state.cache[i] = 0.9 * self.ssd_state.cache[i] + 0.1 * ctx;
        }

        let g = self.ssd_state.dual_gate;
        let mut output = vec![0.0; dim];
        for i in 0..dim {
            output[i] = (1.0 - g) * self.ssd_state.hidden[i] + g * self.ssd_state.cache[i];
        }

        self.ssd_state.timestamp = Utc::now().timestamp();
        output
    }

    pub fn step_base(&self, state: &mut super::core::SelectiveState, input: &Vector) -> Vector {
        self.base.step(state, input)
    }
...
mod tests {
    ...
    #[test]
    fn test_ssd_operator_new() {
        let op = SsdOperator::new(23, 128);
        assert_eq!(op.dim(), 23);
        assert_eq!(op.hidden_dim(), super::super::core::SSM_STATE_SIZE);
        assert_eq!(op.ssd_state.hidden.len(), super::super::core::SSM_STATE_SIZE);
    }

    #[test]
    fn test_ssd_operator_step_ssd_produces_output() {
        let mut op = SsdOperator::new(8, 64);
        let input = vec![0.5; 8];
        let ctx = vec![0.3; 8];
        let output = op.step_ssd(&input, &ctx);
        assert_eq!(output.len(), 8);
    }
    ...
    #[test]
    fn test_ssd_operator_multiple_steps_evolve() {
        let mut op = SsdOperator::new(8, 64);
        let input = vec![0.5; 8];
        let ctx = vec![0.3; 8];
