...
use super::{AntiPatternDetector, CriticSeverity, DesignViolation};
...
impl ColorAntiPatternDetector {
    fn check_gray_text_on_colored_bg(&self, content: &str) -> Vec<DesignViolation> {
        ...
        ).expect("hardcoded regex is valid: gray-text-on-colored-bg");
    ...
    fn check_pure_black_gray(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(
            r"(?i)color\s*:\s*(?:rgb\s*\(\s*0\s*,\s*0\s*,\s*0\s*\)|#000(?:000)?|black)\s*[;}]",
        )
        .expect("hardcoded regex is valid: pure-black-gray");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_purple_gradient(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)(linear-gradient|radial-gradient|conic-gradient)\s*\([^)]*(?:purple|#800080|#9b59b6|#8e44ad)[^)]*\)").expect("hardcoded regex is valid: purple-gradient");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_dark_glow(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re =
            Regex::new(r"(?i)text-shadow\s*:\s*[^;]*rgba?\s*\(.*?0\s*,\s*0\s*,\s*0\s*,\s*0\.\d\)")
                .expect("hardcoded regex is valid: dark-glow");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_cyan_teal_default(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)(?:color|background(?:-color)?)\s*:\s*(?:#00bcd4|#009688|#00acc1|#26c6da|cyan|teal)\s*[;}]").expect("hardcoded regex is valid: cyan-teal-default");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_saturated_card_bg(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)\.(?:card|panel|box|tile)\s*\{[^}]*background(?:-color)?\s*:\s*(?:#[0-9a-f]{3,6}|rgba?\s*\([^)]+\))\s*;").expect("hardcoded regex is valid: saturated-card-bg");
        for m in re.find_iter(content) {
            let bg = m.as_str();
            let hex_re =
                Regex::new(r"#([0-9a-f]{6})").expect("hardcoded regex is valid: hex-color-capture");
    ...
    fn check_low_contrast_text(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)color\s*:\s*(?:#(?:999|aaa|bbb|ccc|ddd)|rgba?\s*\(\s*(?:\d{1,2})\s*,\s*(?:\d{1,2})\s*,\s*(?:\d{1,2})\s*,\s*0\.\d[25]\s*\))").expect("hardcoded regex is valid: low-contrast-text");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_brand_color_overuse(&self, content: &str) -> Vec<DesignViolation> {
        ...
        for color in &brand_colors {
            ...
            let re = Regex::new(&format!("(?i){}", escaped))
                .expect("hardcoded regex is valid: brand-color-overuse");
    ...
    fn check_rainbow_hover(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(
            r"(?i):hover\s*\{[^}]*(?:linear-gradient|background-image|filter)\s*:[^}]*\}",
        )
        .expect("hardcoded regex is valid: rainbow-hover");
        for m in re.find_iter(content) {
...
impl TypographyAntiPatternDetector {
    fn check_inter_only_font(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r#"(?i)font-family\s*:\s*['"]?Inter['"]?\s*[;}]"#)
            .expect("hardcoded regex is valid: inter-only-font");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_font_stack_unreasonable(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r#"(?i)font-family\s*:\s*['"]([^'"]+)['"]\s*[;}]"#)
            .expect("hardcoded regex is valid: font-stack-unreasonable");
        for m in re.find_iter(content) {
            let family = m.as_str();
            if !family.contains(',')
                && !family.contains("sans-serif")
    ...
    fn check_giant_heading(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)font-size\s*:\s*(?:7[2-9]|[89]\d|\d{3,})\s*px")
            .expect("hardcoded regex is valid: giant-heading");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_thin_light_font(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)font-weight\s*:\s*(?:100|200|300)\s*[;}]")
            .expect("hardcoded regex is valid: thin-light-font");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_uppercase_body(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)(?:body|p|\.text)\s*\{[^}]*text-transform\s*:\s*uppercase")
            .expect("hardcoded regex is valid: uppercase-body");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_line_height_too_tight(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)line-height\s*:\s*(?:1(?:\.0)?|1\.1[0-9]?)\s*[;}]")
            .expect("hardcoded regex is valid: line-height-too-tight");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
...
impl LayoutAntiPatternDetector {
    fn check_card_inception(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let card_count = content.matches(".card").count()
            + content.matches("card-").count()
            + content.matches("card__").count()
            + content.matches("card_").count();
        let nested = Regex::new(r"(?i)\.card\s*\{[^}]*\.card")
            .expect("hardcoded regex is valid: card-inception");
    ...
    fn check_tight_padding(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)padding\s*:\s*[012]px")
            .expect("hardcoded regex is valid: tight-padding");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_touch_target_too_small(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)\.(?:btn|button|link|icon|close|nav-item|menu-item)\s*\{[^}]*(?:width|height)\s*:\s*(?:2[0-9]|1[0-9]|[0-9])\s*px").expect("hardcoded regex is valid: touch-target-too-small");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_missing_grid(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let has_grid = content.contains("display: grid")
            || content.contains("display:grid")
            || content.contains("display: flex")
            || content.contains("display:flex");
    ...
    fn check_absolute_position_overuse(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let count = Regex::new(r"(?i)position\s*:\s*absolute")
            .expect("hardcoded regex is valid: absolute-position")
            .find_iter(content)
            .count();
        if count > 3 {
            violations.push(DesignViolation {
    ...
    fn check_fixed_header_without_skip(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let has_fixed_header =
            content.contains("position: fixed") || content.contains("position:fixed");
    ...
    fn check_missing_hierarchy(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let has_h1 = Regex::new(r"(?is)<h1[>\s]")
            .expect("hardcoded regex is valid: heading-h1")
            .is_match(content);
        let has_h2 = Regex::new(r"(?is)<h2[>\s]")
            .expect("hardcoded regex is valid: heading-h2")
            .is_match(content);
        if has_h2 && !has_h1 {
            violations.push(DesignViolation {
    ...
    fn check_inconsistent_border_radius(&self, content: &str) -> Vec<DesignViolation> {
        ...
        let radii: Vec<f64> = Regex::new(r"(?i)border-radius\s*:\s*(\d+)px")
            .expect("hardcoded regex is valid: border-radius-capture")
            .captures_iter(content)
            .filter_map(|c| c[1].parse::<f64>().ok())
            .collect();
        if radii.len() >= 3 {
            let unique: HashSet<u64> = radii.iter().map(|r| *r as u64).collect();
    ...
    fn check_overflow_hidden_content(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(
            r"(?i)(?:\.content|\.main|\.container|\.article|\.text)\s*\{[^}]*overflow\s*:\s*hidden",
        )
        .expect("hardcoded regex is valid: overflow-hidden");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
...
impl MotionAntiPatternDetector {
    ...
    fn check_parallax_overuse(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)transform\s*:\s*[^}]*translate[Z3]?[^}]*\d+[^}]*\)")
            .expect("hardcoded regex is valid: parallax-overuse");
        let parallax_hints = [
            "parallax",
            "scroll-effect",
            "data-speed",
    ...
    fn check_auto_play_video(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?is)<video[^>]*autoplay[^>]*>")
            .expect("hardcoded regex is valid: auto-play-video");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_infinite_animation(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i)animation(?:-iteration-count)?\s*:\s*infinite")
            .expect("hardcoded regex is valid: infinite-animation");
        for m in re.find_iter(content) {
            let before = &content[..m.start()];
            let is_loading = before.contains("spinner")
                || before.contains("loading")
                || before.contains("skeleton")
                || before.contains("pulse");
            if !is_loading {
                violations.push(DesignViolation {
...
impl AccessibilityAntiPatternDetector {
    fn check_skipped_heading_level(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?is)<(h[1-6])[>\s]")
            .expect("hardcoded regex is valid: heading-level-capture");
        let levels: Vec<u32> = re
            .captures_iter(content)
    ...
    fn check_missing_alt_text(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r#"(?is)<img[^>]*src\s*=\s*['"][^'"]+['"][^>]*>"#)
            .expect("hardcoded regex is valid: missing-alt-text");
        for m in re.find_iter(content) {
            let tag = m.as_str();
            if !tag.contains("alt=") {
                violations.push(DesignViolation {
    ...
    fn check_button_without_aria(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?is)<button[^>]*>[\s]*(?:<[^>]*>[\s]*)*</button>")
            .expect("hardcoded regex is valid: button-without-aria");
        for m in re.find_iter(content) {
            let tag = m.as_str();
            let has_label = tag.contains("aria-label") || tag.contains("aria-labelledby");
            let inner_text = Regex::new(r"(?is)>([^<]+)<")
                .expect("hardcoded regex is valid: inner-text-capture");
    ...
    fn check_focus_visible_removed(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?i):focus\s*\{[^}]*outline\s*:\s*none")
            .expect("hardcoded regex is valid: focus-visible-removed");
        for m in re.find_iter(content) {
    ...
    fn check_color_only_indicator(&self, content: &str) -> Vec<DesignViolation> {
        ...
        for pat in &patterns {
            let re = Regex::new(pat).expect("hardcoded regex is valid: color-only-pattern");
            for m in re.find_iter(content) {
                violations.push(DesignViolation {
    ...
    fn check_missing_skip_link(&self, content: &str) -> Vec<DesignViolation> {
        ...
        let has_skip = content.contains("skip")
            && (content.contains("skip-link")
                || content.contains("skip_to_content")
                || content.contains("skip-to-content")
                || content.contains("skipnav")
                || content.contains("skipNav"));
        let has_nav = content.contains("<nav")
    ...
    fn check_small_tap_target(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r#"(?is)<(?:button|a)[^>]*style\s*=\s*['"][^'"]*(?:width|height)\s*:\s*(?:2[0-9]|1[0-9]|[0-9])\s*px"#).expect("hardcoded regex is valid: small-tap-target");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_auto_play_audio(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re = Regex::new(r"(?is)<audio[^>]*autoplay[^>]*>")
            .expect("hardcoded regex is valid: auto-play-audio");
        for m in re.find_iter(content) {
            violations.push(DesignViolation {
    ...
    fn check_missing_lang_attr(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let re =
            Regex::new(r"(?is)<html[^>]*>").expect("hardcoded regex is valid: html-tag-capture");
        for m in re.find_iter(content) {
    ...
    fn check_form_without_label(&self, content: &str) -> Vec<DesignViolation> {
        let mut violations = Vec::new();
        let input_re = Regex::new(r"(?is)<(?:input|textarea|select)[^>]*>")
            .expect("hardcoded regex is valid: form-input-capture");
        for m in input_re.find_iter(content) {
            ...
            let id_re = Regex::new(r#"id\s*=\s*['"]([^'"]+)['"]"#)
                .expect("hardcoded regex is valid: id-capture");
            let has_label_for = if let Some(caps) = id_re.captures(tag) {
                ...
                .expect("hardcoded regex is valid: label-for-capture");
                label_re.is_match(content)
            } else {
                false
            };
            let has_wrapping = Regex::new(r"(?is)<label[^>]*>[\s\S]*?<(?:input|textarea|select)")
                .expect("hardcoded regex is valid: wrapping-label")
                .is_match(content);
