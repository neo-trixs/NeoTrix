...
use super::nt_mind_evolution_loop::{
    Issue, IssueType, ProjectSnapshot, EXCESS_UNWRAP_THRESHOLD, LARGE_FILE_THRESHOLD,
    MISSING_TESTS_THRESHOLD, TODO_LEFTOVERS_THRESHOLD,
};
...
impl SelfDiagnose {
    ...
    fn detect_excess_unwrap(snap: &ProjectSnapshot, cycle: u64, issues: &mut Vec<Issue>) {
        if snap.unwrap_count > EXCESS_UNWRAP_THRESHOLD {
            issues.push(Issue {
                ...
                description: format!(
                    ".unwrap() 过多: {} 处 (阈值={})",
                    snap.unwrap_count, EXCESS_UNWRAP_THRESHOLD
                ),
    ...
    fn detect_todo_leftovers(snap: &ProjectSnapshot, cycle: u64, issues: &mut Vec<Issue>) {
        if snap.todo_count > TODO_LEFTOVERS_THRESHOLD {
            issues.push(Issue {
                ...
                description: format!(
                    "TODO 残留: {} 处 (阈值={})",
                    snap.todo_count, TODO_LEFTOVERS_THRESHOLD
                ),
    ...
    fn plan_action(issue: &Issue) -> ActionPlan {
        match issue.issue_type {
            ...
            IssueType::TodoLeftovers => ActionPlan::HumanDecision {
                issue_type: IssueType::TodoLeftovers,
                file: None,
                reason: format!(
                    "{} 个 TODO 需要人工判定哪些已完成",
                    TODO_LEFTOVERS_THRESHOLD
                ),
            },
        ...
        match plan {
            ...
            ActionPlan::RemoveTodo { file } => {
                ...
                if removed > 0 {
                    ...
                    std::fs::rename(&tmp, file).map_err(|e| format!("重命名失败: {}", e))?;
            ...
            ActionPlan::ReviewUnsafe { file } => Err(format!("{} 需要人工审查 unsafe 块", file)),
            ActionPlan::ReplaceUnwrap { file } => Err(format!("{} 需要人工审查 unwrap 调用", file)),
            ActionPlan::HumanDecision { reason, .. } => Err(format!("需要人工决策: {}", reason)),
...
mod tests {
    ...
    fn test_compile_error_gets_highest_urgency() {
        ...
        let compile = items
            .iter()
            .find(|d| matches!(d.underlying_issue.issue_type, IssueType::CompileWarning));
    ...
    fn test_action_plan_maps_correctly() {
        let snap = dummy_snapshot();
        let (items, _) = SelfDiagnose::run_diagnosis(&snap, 1);
        let missing_test = items
            .iter()
            .find(|d| matches!(d.underlying_issue.issue_type, IssueType::MissingTests));
        assert!(missing_test.is_some());
        assert!(matches!(
            missing_test.unwrap().action,
            ActionPlan::AddTestStub { .. }
        ));
    }
    ...
    fn test_remove_todo_action_on_temporary_file() {
        ...
        .unwrap();
        let result = ActionExecutor::execute(&ActionPlan::RemoveTodo {
    ...
    fn test_diagnose_respects_thresholds() {
        ...
        let todo = items
            .iter()
            .filter(|d| matches!(d.underlying_issue.issue_type, IssueType::TodoLeftovers))
            .count();
