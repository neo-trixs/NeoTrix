...
use crate::core::nt_core_sense::{Sensor, SensorSample, SensoryEvent, SensoryEventKind, AnalysisReport};
...
impl ScreenCapture {
    ...
    fn capture_screen_macos(&self) -> Option<SensorSample> {
        ...
        sample.metadata.insert("method".into(), "screencapture".into());
        sample.metadata.insert("format".into(), "png".into());
        sample.metadata.insert("file_size_bytes".into(), file_size.to_string());
...
impl Sensor for ScreenCapture {
    ...
    fn to_event(&self, sample: SensorSample) -> SensoryEvent {
        let desc = format!("Screen capture: {}px, {} bytes",
            sample.metadata.get("file_size_bytes").unwrap_or(&"?".into()),
            sample.size());
        SensoryEvent {
            id: rand::random::<u64>(),
