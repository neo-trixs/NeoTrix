...
impl SensoryIntegrationHub {
    ...
    pub fn current_perception_narrative(&self) -> String {
        let real = if self.use_real_sensors {
            "real"
        } else {
            "file"
        };
        let sc = if self.screen_cap.is_active() {
            "active"
        } else {
            "inactive"
        };
        let mc = if self.mic_cap.is_active() {
            "active"
        } else {
            "inactive"
        };
        let vs = if self.visual.is_active() {
            "active"
        } else {
            "inactive"
        };
        let aus = if self.auditory.is_active() {
            "active"
        } else {
            "inactive"
        };
