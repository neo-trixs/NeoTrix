    ...
    fn build_client(&self) -> reqwest::blocking::Client {
        ...
        headers.insert(
            ...
            "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
                .parse()
                .expect("Accept header construction failed - reqwest header parse"),
        );
        headers.insert(
            reqwest::header::ACCEPT_LANGUAGE,
            "en-US,en;q=0.9"
                .parse()
                .expect("Accept-Language header construction failed"),
        );
        let ua = self
            .config
            .user_agent
        ...
        if let Ok(ua_val) = ua.parse::<reqwest::header::HeaderValue>() {
...
impl AntiDetect {
    ...
    pub fn new_with_defaults() -> Self {
        Self {
            user_agents: UserAgentRotation::default().agents().to_vec(),
        }
    }
}
