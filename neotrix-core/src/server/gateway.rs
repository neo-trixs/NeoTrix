...
use crate::neotrix::nt_io_provider::{
    create_provider, sanitize_history, FinishReason, LlmProvider, LlmProviderType, LlmRequest,
    Message, ProviderConfig, Role,
};
use futures::FutureExt;

pub const GATEWAY_DEFAULT_ADDR: &str = "127.0.0.1:1242";
...
impl OpenAICompatibleGateway {
    pub fn new(config: GatewayConfig) -> Self {
        ...
        for pc in &config.providers {
            let key = match pc.provider_type {
                ...
                LlmProviderType::OpencodeFree => "opencode",
            };
    ...
    pub fn from_env() -> Self {
        let config = GatewayConfig {
            addr: std::env::var("NEOTRIX_GATEWAY_ADDR")
...
pub use super::openai_compat::ErrorDetail;
...
async fn models_handler(State(state): State<GatewayState>) -> Json<ModelList> {
    ...
    let mut data: Vec<ModelInfo> = gateway
        .providers
        .keys()
        .map(|key| {
        ...
        return Err((
            ...
            Json(ApiError {
                error: ErrorDetail {
                    ...
                    code: "streaming_not_supported".into(),
                },
            }),
        ));
    }
    ...
    let provider = gateway.get_provider(&req.model).ok_or_else(|| {
        (
            ...
            Json(ApiError {
                error: ErrorDetail {
                    message: format!(
                        "Model '{}' is not supported by any configured provider",
                        req.model
                    ),
                    error_type: "invalid_request_error".to_string(),
...
pub fn start_gateway(gateway: OpenAICompatibleGateway, addr: &str) {
    ...
    tokio::spawn(async move {
        if let Err(panic) = std::panic::AssertUnwindSafe(async {
            ...
            log::info!(
                "OpenAI-compatible gateway listening on http://{}",
                addr_owned
            );
        ...
        .await
        {
            log::error!("[gateway] server panic: {:?}", panic);
        }
    });
}
...
mod tests {
    ...
    fn test_model_routing() {
        let config = GatewayConfig {
            addr: GATEWAY_DEFAULT_ADDR.to_string(),
            providers: vec![
                ProviderConfig {
        ...
        assert_eq!(
            gateway.resolve_provider("claude-3-opus-20240229"),
            Some("anthropic")
        );
        assert_eq!(
            gateway.resolve_provider("claude-3-5-sonnet-20241022"),
            Some("anthropic")
        );
    ...
    #[test]
    fn test_request_parsing() -> Result<(), String> {
        let json = r#"{
            "model": "gpt-4o",
            "messages": [
    ...
    fn test_default_provider_fallback() {
        ...
        let gateway = OpenAICompatibleGateway::new(config);
        assert_eq!(
            gateway.resolve_provider("unknown-model-xyz"),
            Some("openai")
        );
        assert_eq!(
            gateway.resolve_provider("some-random-model"),
            Some("openai")
        );
    }
}
