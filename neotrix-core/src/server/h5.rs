...
use axum::{
    Router, routing::get, response::Html,
...
pub fn validate_token(headers: &HeaderMap, expected: &str) -> bool {
    if expected.is_empty() || expected == "dev" { return true; }
...
pub fn cors_headers() -> HeaderMap {
    ...
    h.insert("Access-Control-Allow-Headers", HeaderValue::from_static("Authorization,Content-Type"));
    h
}
