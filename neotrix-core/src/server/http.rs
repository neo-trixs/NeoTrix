...
async fn reason_kernel_handler(Json(req): Json<ReasonKernelRequest>) -> Json<ReasonKernelResponse> {
    ...
    let circuit_names: Vec<String> = kernel.stats().active.iter().map(|m| format!("{:?}", m)).collect();
    let response = format_kernel_output(&output.state_delta, &req.prompt, stage, energy, &circuit_names);
