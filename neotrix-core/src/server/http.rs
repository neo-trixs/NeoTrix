...
async fn reason_handler(
    ...
    if let Some(ref mut engine) = agent.reasoning_engine {
        match engine.reason(&req.prompt) {
            ...
            Err(e) => Json(ReasonResponse {
                success: false,
                output: format!("Error: {}", e),
                capability_before: before,
                capability_after: before,
            }),
        }
    } else {
        ...
        Json(ReasonResponse {
            ...
            capability_before: before,
            capability_after: agent.brain.capability.arr.iter().sum::<f64>(),
        })
    }
}
...
async fn handle_ws(socket: WebSocket, state: AppState) {
    ...
    while let Some(msg) = receiver.next().await {
        if let Ok(Message::Text(text)) = msg {
            ...
            if let Err(e) = sender.send(Message::Text(echo.into())).await {
                ::log::warn!("[http] ws send failed: {}", e);
            }
        }
    }
}
...
pub async fn start_server(agent: Arc<RwLock<SelfIteratingBrain>>, addr: &str) {
    ...
    log::info!("NeoTrix API + WS + H5 server listening on {}", addr);
    log::info!("  API:     http://{}/health", addr);
    log::info!("  WebChat: http://{}/chat", addr);
    if let Err(e) = axum::serve(listener, app).await {
        log::error!("Server error on {}: {}", addr, e);
    }
}
