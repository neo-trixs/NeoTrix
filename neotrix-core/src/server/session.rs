...
impl SessionManager {
    pub fn new() -> Self {
        Self { sessions: HashMap::new(), active: None }
    }
    pub fn create(&mut self, id: &str, name: &str) -> Session {
        let s = Session { id: id.to_string(), name: name.to_string(), created_at: chrono::Utc::now().timestamp() };
        self.sessions.insert(id.to_string(), s.clone());
        self.active = Some(id.to_string());
        s
    }
    pub fn list(&self) -> Vec<&Session> { self.sessions.values().collect() }
    pub fn active(&self) -> Option<&Session> { self.active.as_ref().and_then(|id| self.sessions.get(id)) }
    pub fn switch(&mut self, id: &str) -> bool { if self.sessions.contains_key(id) { self.active = Some(id.to_string()); true } else { false } }
    pub fn remove(&mut self, id: &str) -> bool { self.sessions.remove(id).is_some() }
}
...
mod share_tests {
    ...
    fn test_share_ttl_expired() {
        ...
        let path = mgr.shares_dir.join(format!("{}.json", &expired_share.token));
