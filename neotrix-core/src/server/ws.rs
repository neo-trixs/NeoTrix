...
pub struct WsBridge {
    ...
    #[allow(dead_code)]
    receiver: RwLock<Option<mpsc::UnboundedReceiver<WsMessage>>>,
    /// 连接状态
    #[allow(dead_code)]
...
impl WsBridge {
    ...
    pub async fn register_session(&self, chat_id: &str, session_id: &str) {
        ...
        sessions.insert(chat_id.to_string(), SessionMapping {
    ...
    pub async fn send_message(&self, msg: WsMessage) -> Result<u64, String> {
        ...
        self.sender.send(msg.clone()).map_err(|e| format!("Send failed: {}", e))?;
        Ok(seq)
    }

    /// 发送消息并等待确认（FIFO Promise 链防竞态）
    pub async fn send_message_await_ack(&self, msg: WsMessage, timeout: Duration) -> Result<u64, String> {
        let seq = self.next_sequence().await;
        let mut acks = self.pending_acks.write().await;
        acks.insert(seq, msg.clone());
        drop(acks);

        self.sender.send(msg).map_err(|e| format!("Send failed: {}", e))?;
...
impl ImAdapter for TelegramAdapter {
    ...
    async fn send_message(&self, chat_id: &str, content: &str) -> Result<(), String> {
        ...
        if resp.status().is_success() { Ok(()) } else { Err(format!("Telegram API error: {}", resp.status())) }
    }
...
impl ImAdapter for WhatsAppAdapter {
    ...
    async fn send_message(&self, chat_id: &str, content: &str) -> Result<(), String> {
        let url = format!("https://graph.facebook.com/v22.0/{}/messages", self.phone_number_id);
        let client = reqwest::Client::builder()
            .danger_accept_invalid_certs(true)
            .build()
        ...
        let resp = client.post(&url)
            ...
            .send().await.map_err(|e| e.to_string())?;
        if resp.status().is_success() { Ok(()) } else { Err(format!("WhatsApp API error: {}", resp.status())) }
    }

    async fn start(&self, _bridge: &WsBridge) -> Result<(), String> {
        Err("WhatsApp WebHook registration + incoming message handling not implemented yet".to_string())
    }
}

impl WhatsAppAdapter {
    /// Verify WhatsApp WebHook challenge (Meta verification)
    pub fn verify_webhook(mode: &str, token: &str, challenge: &str, verify_token: &str) -> Option<String> {
...
impl ImAdapter for FeishuAdapter {
    ...
    async fn send_message(&self, chat_id: &str, content: &str) -> Result<(), String> {
        ...
        let resp = client.post(&self.webhook_url)
            ...
            .json(&body)
            .send().await.map_err(|e| e.to_string())?;
        if resp.status().is_success() { Ok(()) } else { Err(format!("Feishu API error: {}", resp.status())) }
    }
...
impl ImAdapter for DiscordAdapter {
    ...
    async fn send_message(&self, chat_id: &str, content: &str) -> Result<(), String> {
        ...
        let resp = client.post(&url)
            ...
            .send().await.map_err(|e| e.to_string())?;
        if resp.status().is_success() { Ok(()) } else { Err(format!("Discord API error: {}", resp.status())) }
    }
...
mod tests {
    ...
    fn test_telegram_adapter_name() {
        let adapter = TelegramAdapter { bot_token: "test:token".to_string() };
        assert_eq!(adapter.name(), "telegram");
    }

    #[test]
    fn test_whatsapp_adapter_name() {
        let adapter = WhatsAppAdapter { phone_number_id: "123".to_string(), access_token: "token".to_string() };
    ...
    fn test_ws_message_types() {
        let msg = WsMessage {
            chat_id: "c".to_string(), session_id: "s".to_string(),
            content: "text".to_string(), msg_type: WsMessageType::Text,
            timestamp: 0,
        };
    ...
    async fn test_multiple_sessions() {
        ...
        assert_eq!(bridge.resolve_session("chat_a").await, Some("sess_a".to_string()));
        assert_eq!(bridge.resolve_session("chat_b").await, Some("sess_b".to_string()));
    ...
    async fn test_sequence_numbers() {
        let bridge = WsBridge::new(WsBridgeConfig::default());
        let msg = WsMessage {
            chat_id: "c".into(), session_id: "s".into(),
            content: "1".into(), msg_type: WsMessageType::Text, timestamp: 0,
        };
        let seq1 = bridge.send_message(msg.clone()).await.expect("await should be ok in test");
        let seq2 = bridge.send_message(msg).await.expect("await should be ok in test");
        assert_eq!(seq1, 1);
        assert_eq!(seq2, 2);
    }
    ...
    async fn test_ack_message() {
        ...
        let msg = WsMessage {
            chat_id: "c".into(), session_id: "s".into(),
            content: "test".into(), msg_type: WsMessageType::Text, timestamp: 0,
        };
        let seq = bridge.send_message(msg).await.expect("await should be ok in test");
    ...
    async fn test_send_message_await_ack_timeout() {
        ...
        let msg = WsMessage {
            chat_id: "c".into(), session_id: "s".into(),
            content: "timeout test".into(), msg_type: WsMessageType::Text, timestamp: 0,
        };
        let result = bridge.send_message_await_ack(msg, Duration::from_millis(10)).await;
    ...
    fn test_im_adapter_registry() {
        ...
        let tg = TelegramAdapter { bot_token: "test:token".to_string() };
    ...
    fn test_im_adapter_list() {
        let mut registry = ImAdapterRegistry::new();
        registry.register(Box::new(TelegramAdapter { bot_token: "t:1".to_string() }));
        registry.register(Box::new(WhatsAppAdapter { phone_number_id: "123".to_string(), access_token: "t".to_string() }));
    ...
    async fn test_broadcast() {
        let mut registry = ImAdapterRegistry::new();
        registry.register(Box::new(TelegramAdapter { bot_token: "fake:token".to_string() }));
    ...
    fn test_feishu_adapter_name() {
        let a = FeishuAdapter { webhook_url: "https://open.feishu.cn/open-apis/bot/v2/hook/test".to_string(), app_id: None, app_secret: None };
        assert_eq!(a.name(), "feishu");
    }

    #[test]
    fn test_discord_adapter_name() {
        let a = DiscordAdapter { webhook_url: "https://discord.com/api/webhooks/123/abc".to_string() };
        assert_eq!(a.name(), "discord");
    }

    #[test]
    fn test_slack_adapter_name() {
        let a = SlackAdapter { webhook_url: "https://hooks.slack.com/services/T00/B00/xxx".to_string() };
    ...
    fn test_im_registry_with_multiple_adapters() {
        let mut registry = ImAdapterRegistry::new();
        registry.register(Box::new(TelegramAdapter { bot_token: "t:1".to_string() }));
        registry.register(Box::new(WhatsAppAdapter { phone_number_id: "p".to_string(), access_token: "a".to_string() }));
        registry.register(Box::new(FeishuAdapter { webhook_url: "https://open.feishu.cn/bot/test".to_string(), app_id: None, app_secret: None }));
        registry.register(Box::new(DiscordAdapter { webhook_url: "https://discord.com/webhooks/1/2".to_string() }));
        registry.register(Box::new(SlackAdapter { webhook_url: "https://hooks.slack.com/test".to_string() }));
