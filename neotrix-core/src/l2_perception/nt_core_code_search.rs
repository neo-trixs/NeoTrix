//! 代码符号索引 / ripgrep 混合检索 / RRF 融合 / 冲击分析 — D3 下沉至 core。
//!
//! 纯逻辑 (仅 std + serde + regex, 无 NT-* 层依赖), 自 l2_world_impl 下沉;
//! l2 `nt_world_code_search` 保留 re-export 兼容层, `neotrix::nt_world_code_search::*`
//! 路径不变。含 A4 (Axon) 吸收的 RRF 混合检索与深度分组冲击分析。
