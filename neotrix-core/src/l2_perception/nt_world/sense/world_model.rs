//! WorldModel 单一事实源 — 统一定义所有 WorldModel 相关类型
//!
//! 本文件包含:
//! - `JepaWorldModel`: JEPA 世界模型 (预测/训练/异常检测)
//! - `WorldModel`: 专家混合世界模型 (编码/预测/规划/更新)
//! - `ContextEncoder`: 任务上下文编码器
//! - `WifiStatus`: WiFi 感知状态
...
use super::types::{
    ...
    MultiScalePrediction, TDExperience, WorldModelState,
};
use super::encoder::JepaEncoder;
use super::predictor::JepaPredictor;
use super::loss::{EnergyModel, VicRegLoss, NonContrastiveLoss, JepaLossType};
...
pub struct JepaWorldModel {
    ...
    pub loss_type: JepaLossType,
    pub non_contrastive_loss: NonContrastiveLoss,
}
...
impl JepaWorldModel {
    pub fn new(input_dim: usize) -> Self {
        ...
        Self {
            ...
            sigreg_lambda: 0.01,
            td_flows: None,
            loss_type: JepaLossType::default(),
            non_contrastive_loss: NonContrastiveLoss::default(),
        }
    }
    ...
    pub fn train_step(&mut self, x_features: &[f64], y_features: &[f64]) -> (f64, f64, f64, f64) {
        ...
        let (vicreg_total, inv_loss, _var_loss, _cov_loss) = if self.loss_type == JepaLossType::NonContrastive {
            let (nc_total, _nc_mse, _nc_reg) = self.non_contrastive_loss.compute(&z_pred, &z_target);
            (nc_total, 0.0, 0.0, 0.0)
        } else {
