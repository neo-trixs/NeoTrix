...
/// 默认扫描源列表 (8-10 个)
pub fn _default_sources() -> Vec<SweepSource> {
    vec![
...
impl _SweepReport {
    pub fn summarize(sources: &[SweepSource], results: &[_SweepResult]) -> Self {
        let scanned = sources.len();
        let degraded = results
            .iter()
            .filter(|r| matches!(r, _SweepResult::Degraded { .. }))
            .count();
        let total_entries = results
            .iter()
            .filter(|r| matches!(r, _SweepResult::Entry { .. }))
            .count();
...
impl _SweepRunner {
    ...
    pub fn run(&self, sources: &[SweepSource], api_keys: &[&str]) -> Vec<_SweepResult> {
        sources
            .iter()
            .flat_map(|src| {
                let has_key = !src.needs_key || api_keys.contains(&src.name);
                if !has_key {
                    return vec![_SweepResult::Degraded { source: src.name, reason: "no_key" }];
                }
                let n = (src.priority as usize) % 3 + 1;
                (0..n)
                    .map(|i| _SweepResult::Entry {
                        source: src.name,
...
impl _DeltaDetector {
    ...
    /// 新条目 (不在 previous 中) → _DeltaEntry
    pub fn detect(&self, previous: &[&str], current: &[&str]) -> Vec<_DeltaEntry> {
        current
            .iter()
            .filter(|entry| !previous.contains(entry))
            .map(|entry| _DeltaEntry {
    ...
    pub fn _detect_with_config(
        ...
        current
            ...
            .filter_map(|entry| {
                ...
                Some(_DeltaEntry { entry: entry.to_string(), severity })
            })
            .collect()
    }

    /// 跨源相关性: 提取各条目关键词 token, 出现在 >= correlation_window 个条目 → 强信号。
    pub fn correlate(&self, entries: &[_DeltaEntry], window: usize) -> Vec<_CorrelatedSignal> {
    ...
    /// 按严重级别聚合告警计数
    pub fn alert(&self, entries: &[_DeltaEntry]) -> _AlertSummary {
        _AlertSummary::_from_entries(entries)
    }
}

/// 告警汇总
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct _AlertSummary {
    pub critical: usize,
    pub high: usize,
    pub medium: usize,
    pub info: usize,
...
impl _AlertSummary {
    pub fn _from_entries(entries: &[_DeltaEntry]) -> Self {
...
mod tests {
    ...
    fn test_sweep_all_keyed_ok() {
        let runner = _SweepRunner::new();
        let keys: Vec<&str> = _default_sources()
            .iter()
            .filter(|s| s.needs_key)
            .map(|s| s.name)
            .collect();
        let results = runner.run(&_default_sources(), &keys);
        assert!(results.iter().all(|r| matches!(r, _SweepResult::Entry { .. })));
        let report = _SweepReport::summarize(&_default_sources(), &results);
    ...
    fn test_sweep_degraded_no_key() {
        let runner = _SweepRunner::new();
        let results = runner.run(&_default_sources(), &[]);
        let report = _SweepReport::summarize(&_default_sources(), &results);
        assert!(report.degraded > 0);
        let degraded: Vec<&str> = results
            .iter()
            .filter_map(|r| match r {
                _SweepResult::Degraded { source, reason } if *reason == "no_key" => Some(*source),
                _ => None,
            })
            .collect();
    ...
    fn test_sweep_report_counts() {
        ...
        let keyed: usize = _default_sources().iter().filter(|s| s.needs_key).count();
    ...
    fn test_delta_severity_grading() {
        assert_eq!(_DeltaDetector::grade("vuln:CVE-2026-1234"), Severity::Critical);
        assert_eq!(_DeltaDetector::grade("breach at acme"), Severity::High);
        assert_eq!(_DeltaDetector::grade("software update available"), Severity::Info);
        assert_eq!(_DeltaDetector::grade("random notice"), Severity::Medium);
    }
    ...
    fn test_alert_aggregation() {
        let detector = _DeltaDetector::new();
        let deltas = vec![
            _DeltaEntry { entry: "vuln:x".into(), severity: Severity::Critical },
            _DeltaEntry { entry: "breach:y".into(), severity: Severity::High },
            _DeltaEntry { entry: "update:z".into(), severity: Severity::Info },
            _DeltaEntry { entry: "notice".into(), severity: Severity::Medium },
            _DeltaEntry { entry: "vuln:w".into(), severity: Severity::Critical },
        ];
        let summary = detector.alert(&deltas);
    ...
    #[test]
    fn test_alert_empty() {
        let summary = _DeltaDetector::new().alert(&[]);
    ...
    fn test_normalize_digit_placeholder() {
        assert_eq!(_DeltaDetector::normalize("CVE-2026-0001"), "cve");
        assert_eq!(_DeltaDetector::normalize("cve-2026-0002"), "cve");
        assert_eq!(_DeltaDetector::normalize("Breach at Acme Corp"), "breach at acme corp");
    }

    #[test]
    fn test_detect_with_config_semantic_dedup() {
        let detector = _DeltaDetector::new();
        let config = _DeltaConfig::default();
        // CVE-2026-0001 与 CVE-2026-0002 归一化后同为 "cve" → 只报一条 (semantic dedup)
        let deltas = detector._detect_with_config(
            &["dns:item-0"],
    ...
    fn test_detect_with_config_min_severity_filter() {
        let detector = _DeltaDetector::new();
        let config = _DeltaConfig { min_severity: Severity::High, .._DeltaConfig::default() };
        let deltas = detector._detect_with_config(
            &[],
    ...
    fn test_correlate_cross_source_signal() {
        let detector = _DeltaDetector::new();
        let entries = vec![
            _DeltaEntry { entry: "satellite:fire near reactor".into(), severity: Severity::High },
            _DeltaEntry { entry: "news:reactor incident report".into(), severity: Severity::High },
            _DeltaEntry { entry: "dns:unrelated item".into(), severity: Severity::Info },
        ];
    ...
    fn test_detect_with_config_no_dedup_preserves_all() {
        let detector = _DeltaDetector::new();
        let config = _DeltaConfig { dedup_normalize: false, .._DeltaConfig::default() };
        let deltas = detector._detect_with_config(
            &["dns:item-0"],
            &["dns:item-0", "vuln:CVE-2026-0001", "vuln:CVE-2026-0002"],
            &config,
