...
impl fmt::Display for _DiscoveryAction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            _DiscoveryAction::Keep => write!(f, "keep"),
            _DiscoveryAction::Prune => write!(f, "prune"),
            _DiscoveryAction::Transform => write!(f, "transform"),
            _DiscoveryAction::Flag => write!(f, "flag"),
        }
    }
}
...
/// The Regex is compiled on demand via `_compiled_pattern()`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct _DiscoveryRule {
    /// The regex pattern string (serialized as JSON string)
    pub pattern: String,
    /// The action to take when the pattern matches
    pub action: _DiscoveryAction,
...
impl _DiscoveryRule {
    ...
    pub fn new(pattern: &str, action: _DiscoveryAction, description: &str) -> Result<Self, regex::Error> {
        Ok(Self {
    ...
    pub fn _compiled_pattern(&self) -> Regex {
...
impl DiscoveryExtractor {
    ...
    pub fn new() -> Self {
        ...
        rules.push(_DiscoveryRule::new(
            r"^.{0,30}$",
            _DiscoveryAction::Prune,
    ...
    pub fn _with_rules(rules: Vec<_DiscoveryRule>) -> Self {
        Self {
            rules,
    ...
    pub fn add_rule(&mut self, rule: _DiscoveryRule) {
    ...
    ///
    /// Returns a list of _DiscoveredContent results, one per matching rule.
    pub fn extract(&mut self, content: &str) -> Vec<_DiscoveredContent> {
        ...
        for (i, rule) in self.rules.iter().enumerate() {
            ...
            results.push(_DiscoveredContent {
    ...
    fn compiled_patterns(&self) -> Vec<Regex> {
        self.rules.iter().map(|r| r._compiled_pattern()).collect()
    }
    ...
    fn is_short_navigation(&self, content: &str) -> bool {
        ...
        self.rules.iter().any(|rule| {
            let pattern = rule._compiled_pattern();
            pattern.is_match(content) &&
            (rule.action == _DiscoveryAction::Prune ||
...
fn passes_filters_for_content(content: &str, rules: &[_DiscoveryRule]) -> bool {
    let content_len = content.chars().count();
    if content_len <= 30 {
        return false;
    }
    ...
    for rule in rules {
        if rule.action == _DiscoveryAction::Prune {
            let pattern = rule._compiled_pattern();
...
mod tests {
    ...
    fn test_min_length_filter_prunes_short_content() {
        ...
        let pruned: Vec<_> = results.iter().filter(|r| r.action == _DiscoveryAction::Prune && !r.passes_filters).collect();
    ...
    fn test_nav_element_pruning() {
        ...
        let pruned: Vec<_> = results.iter().filter(|r| {
            r.rule_description.contains("navigation") || r.action == _DiscoveryAction::Prune
        }).collect();
    ...
    fn test_footer_element_pruning() {
        ...
        let pruned: Vec<_> = results.iter().filter(|r| {
            r.rule_description.contains("footer") || r.action == _DiscoveryAction::Prune
        }).collect();
    ...
    fn test_navigation_link_pruning() {
        ...
        let pruned: Vec<_> = results.iter().filter(|r| {
            r.action == _DiscoveryAction::Prune && r.rule_description.contains("navigation")
        }).collect();
