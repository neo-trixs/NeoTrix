...
pub struct ScraperConfig {
    ...
    #[serde(default)]
    pub stealth_params: _StealthParams,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct _StealthParams {
...
impl Default for _StealthParams {
    fn default() -> Self {
        Self {
...
impl _StealthParams {
    /// 按请求序号取本轮 referer (轮换)。
    pub fn _referer_for(&self, request_seq: u32) -> Option<String> {
    ...
    pub fn _headers_for(&self, request_seq: u32) -> Option<HashMap<String, String>> {
    ...
    /// 当前请求应等待的间隔 (含抖动)。
    pub fn _interval_for(&self) -> std::time::Duration {
        if !self.enabled {
    ...
    pub fn _should_rotate_proxy(&self, request_seq: u32) -> bool {
...
impl Default for ScraperConfig {
    fn default() -> Self {
        Self {
            ...
            stealth_params: _StealthParams::default(),
        }
    }
}
...
impl BrowserScraper {
    ...
    fn do_fetch(&self, url: &str, referer: Option<&str>) -> ScrapeResult {
        ...
        let ua = self
            ...
            .user_agent
            .clone()
            .unwrap_or_else(|| AntiDetect::_new_with_defaults()._random_ua().to_string());
    ...
    pub fn _human_get(&self, url: &str) -> ScrapeResult {
        self.do_fetch(url, Some("https://www.google.com/"))
    }

    pub fn _cf_get(&self, url: &str) -> ScrapeResult {
...
impl RequestScraper {
    ...
    fn build_client(&self) -> reqwest::blocking::Client {
        ...
        if let Some(variant) = self.config.stealth_params._headers_for(self.request_seq) {
            for (k, v) in variant {
    ...
    fn fetch(&mut self, url: &str, referer: Option<&str>) -> ScrapeResult {
        ...
        if self.config.stealth_params.enabled {
            let delay = self.config.stealth_params._interval_for();
            if !delay.is_zero() {
                std::thread::sleep(delay);
            }
            if self.config.stealth_params._should_rotate_proxy(seq) {
                log::trace!("[scrape] stealth: rotating proxy at request {}", seq);
            }
        }
        let effective_referer = referer
            .map(|s| s.to_string())
            .or_else(|| self.config.stealth_params._referer_for(seq));
    ...
    pub fn _google_get(&mut self, url: &str) -> ScrapeResult {
...
impl AntiDetect {
    ...
    pub fn _tiny_profile_name(base: &str) -> String {
    ...
    pub fn _new_with_defaults() -> Self {
        Self {
            user_agents: vec![
...
pub struct _DesignTokenExtractor;

impl _DesignTokenExtractor {
    /// 从 HTML 文本提取设计令牌。
    pub fn extract(html: &str) -> _DesignTokenSet {
        let mut tokens = _DesignTokenSet::default();
        // 1. <style> 块
    ...
    /// 扫描 CSS 文本填充令牌。
    fn scan_css(css: &str, tokens: &mut _DesignTokenSet) {
    ...
    /// 去重 + 排序具名色。
    fn dedup(tokens: &mut _DesignTokenSet) {
        tokens.colors.sort();
        tokens.colors.dedup();
        tokens.fonts.sort();
        tokens.fonts.dedup();
    ...
    /// 简单可统计性: 令牌计数摘要。
    pub fn summarize(tokens: &_DesignTokenSet) -> String {
        format!(
...
mod tests {
    ...
    fn test_anti_detect_random_ua() {
        #[allow(deprecated)]
        let ad = AntiDetect::_new_with_defaults();
        let ua = ad._random_ua();
    ...
    fn test_anti_detect_tiny_profile() {
        let name = AntiDetect::_tiny_profile_name("test-profile");
        assert!(name.starts_with("tiny_"));
        assert_eq!(name.len(), 5 + 16);
        let name2 = AntiDetect::_tiny_profile_name("test-profile");
        assert_eq!(name, name2);
        let name3 = AntiDetect::_tiny_profile_name("other-profile");
    ...
    fn test_nt_world_browse_nt_world_scrape_new() {
        ...
        let result = bs._human_get("https://example.com");
    ...
    fn test_stealth_params_rotation_and_interval() {
        ...
        assert!(sp._should_rotate_proxy(10));
        assert!(!sp._should_rotate_proxy(9));
        assert!(!sp._should_rotate_proxy(0));

        // 禁用后无等待
        let disabled = _StealthParams {
            enabled: false,
            ..Default::default()
        };
        assert!(disabled._interval_for().is_zero());
        assert!(!disabled._should_rotate_proxy(10));
    }

    #[test]
    fn test_stealth_headers_for_empty_returns_none() {
        let sp = _StealthParams::default();
        assert!(sp._headers_for(0).is_none(), "default empty header variants -> None");
        let mut sp2 = _StealthParams::default();
        sp2.header_variants = vec![{
            let mut m = HashMap::new();
            m.insert("X-Test".to_string(), "v1".to_string());
            m
        }];
        let h = sp2._headers_for(0).unwrap();
    ...
    fn test_design_token_extract_colors_fonts_spacings() {
        ...
        let tokens = _DesignTokenExtractor::extract(html);
    ...
    fn test_design_token_dedup_and_summarize() {
        ...
        let s = _DesignTokenExtractor::summarize(&tokens);
...
#[derive(Debug, Clone)]
pub struct _ExtractionPlan {
    pub fit_score: f64,
    pub keep_blocks: Vec<String>,
    pub pruned: Vec<String>,
    scored: Vec<_BlockScore>,
}

impl _ExtractionPlan {
    /// 全部块的评分 (含剪枝块), 供下游细粒度决策。
    pub fn _block_scores(&self) -> &[_BlockScore] {
        &self.scored
    }
}
...
impl FitExtractor {
    ...
    pub fn analyze(&self, html_blocks: &[String], keywords: &[&str]) -> _ExtractionPlan {
        ...
        let scored: Vec<_BlockScore> = html_blocks
            .iter()
            .map(|b| _BlockScore {
                block: b.clone(),
    ...
    pub fn prune(&self, plan: &_ExtractionPlan, threshold: f64) -> Vec<String> {
        plan.scored
            .iter()
    ...
    pub fn _fit_markdown(&self, blocks: &[String], keywords: &[&str], threshold: f64) -> String {
        let plan = self.analyze(blocks, keywords);
        self.prune(&plan, threshold).join("\n\n")
    }

    /// 差异摘要: 保留块 vs 原块列表。
    pub fn diff(original: &[String], plan: &_ExtractionPlan) -> _ExtractionDiff {
        _ExtractionDiff {
...
impl crate::core::nt_core_self_test::SelfTest for FitExtractor {
    ...
    fn self_test(&self) -> Result<(), Vec<String>> {
        ...
        let md = extractor._fit_markdown(&blocks, &["machine"], 0.001);
        if md.split("\n\n").count() < 2 {
...
mod fit_extraction_tests {
    ...
    fn test_prune_threshold_edges() {
        ...
        let exact = plan._block_scores()[0].score; // ln(2) 附近
        // 阈值 0.0: 全部保留
    ...
    fn test_fit_markdown_join() {
        ...
        let extractor = FitExtractor::new(0.5);
        let md = extractor._fit_markdown(&blocks, &["scaling", "law"], 0.001);
    ...
    fn test_fit_extraction_deterministic() {
        ...
        assert_eq!(a._block_scores(), b._block_scores());
    }
