...
fn test_locate_fuzzy_match_modified_element() {
    ...
    let dom = DomSnapshot::new(vec![
        make_dom_element("div", "content-wrapper", "Hello World!", Some("main"), 1, 2),
    ]);
...
fn test_similarity_with_data_attributes() {
    ...
    a.data_attrs.insert("data-id".to_string(), "123".to_string());
    a.data_attrs.insert("data-type".to_string(), "product".to_string());

    let mut b = make_fingerprint("div", "card", "Item", Some("main"), 0, 2);
    b.data_attrs.insert("data-id".to_string(), "456".to_string());
    b.data_attrs.insert("data-type".to_string(), "product".to_string());
...
fn test_hit_count_tracking() {
    ...
    let dom = DomSnapshot::new(vec![
        make_dom_element("div", "content", "Hello", Some("main"), 0, 1),
    ]);
...
fn test_empty_tracker_locate() {
    let mut tracker = AdaptiveTracker::new();
    let dom = DomSnapshot::new(vec![
        make_dom_element("div", "content", "Hello", None, 0, 1),
    ]);
...
fn test_element_snapshot_creation() {
    ...
    let snapshot = ElementSnapshot::new(
        "div", attrs, "Hello World",
        "/html/body/div[1]", vec!["div.content".to_string(), "#main-content".to_string()],
        Some("body"), vec!["container".to_string()], 0, 2,
    );
...
fn test_element_snapshot_to_fingerprint() {
    ...
    let snapshot = ElementSnapshot::new(
        "div", attrs, "Item",
        "/div[2]", vec!["div.card".to_string()],
        Some("section"), vec![], 0, 2,
    );
...
fn test_smart_selector_prefers_stable() {
    ...
    let dom = DomSnapshot::new(vec![
        make_dom_element("section", "main-wrap", "Main", Some("body"), 0, 1),
    ]);
