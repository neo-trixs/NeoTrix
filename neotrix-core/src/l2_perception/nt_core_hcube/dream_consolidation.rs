...
mod tests {
    ...
    fn test_pattern_frequency_tracking() {
        ...
        // Run more cycles → pattern_freq still present (consolidation may not increase every cycle)
        dc.run_consolidation_cycle();
        dc.run_consolidation_cycle();
        assert!(
            !dc.pattern_freq.is_empty(),
            "pattern_freq should still have entries after additional cycles"
        );
    }
