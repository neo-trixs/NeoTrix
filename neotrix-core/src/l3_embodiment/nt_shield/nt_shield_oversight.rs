...
impl _OversightCanary {
    ...
    pub fn _observe_execution(&mut self, action_id: &str, approved: bool) -> bool {
...
impl _BehavioralSignatures {
    ...
    pub fn is_degraded(&self) -> bool {
        self._degradation_score() >= 0.4
    }
}

// ── SelfTest (T1 存在 + T2 注册) ───────────────────────────────

pub struct _OversightDegradationCanary;
impl SelfTest for _OversightDegradationCanary {
    fn name(&self) -> &str { "shield:oversight-degradation" }
    fn self_test(&self) -> Result<(), Vec<String>> {
        // 桩逻辑校验: canary 注入→旁路执行→触发
        let mut canary = _OversightCanary::new();
        canary.inject("kb:drop-namespace:experience");
        let tripped = canary._observe_execution("kb:drop-namespace:experience", false);
        if !tripped { return Err(vec!["canary 未检测到未审批执行".into()]); }
        if canary.integrity() != 0.0 { return Err(vec!["canary 触发后 integrity 应为 0".into()]); }

        // 行为签名退化判定校验
        let degraded = _BehavioralSignatures { override_rate: 0.8, evidence_seeking_rate: 0.1, time_to_approval: 0.0 };
        if !degraded.is_degraded() { return Err(vec!["高 override/低 evidence 应判为退化".into()]); }
        let healthy = _BehavioralSignatures { override_rate: 0.05, evidence_seeking_rate: 0.7, time_to_approval: 30.0 };
...
pub fn register_oversight_self_tests(registry: &mut SelfTestRegistry) {
    registry.register(Box::new(_OversightDegradationCanary));
}
...
mod tests {
    ...
    fn test_canary_self_test_passes() {
        assert!(_OversightDegradationCanary.self_test().is_ok(), "监督退化 canary SelfTest 应通过");
    }

    #[test]
    fn test_canary_detects_bypass() {
        let mut c = _OversightCanary::new();
        c.inject("kb:drop-namespace:experience");
        assert!(c._observe_execution("kb:drop-namespace:experience", false));
        assert_eq!(c.integrity(), 0.0);
        // 经审批执行不应触发
        assert!(!c._observe_execution("kb:drop-namespace:experience", true));
    }

    #[test]
    fn test_behavioral_degradation_judgement() {
        let degraded = _BehavioralSignatures { override_rate: 0.9, evidence_seeking_rate: 0.1, time_to_approval: 0.0 };
        assert!(degraded.is_degraded());
        let healthy = _BehavioralSignatures { override_rate: 0.05, evidence_seeking_rate: 0.7, time_to_approval: 60.0 };
