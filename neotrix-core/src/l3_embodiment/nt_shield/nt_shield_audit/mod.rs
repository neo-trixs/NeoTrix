...
impl ReasoningTraceGuard {
    ...
    pub fn scan_protected(&self, text: &str, session_id: &str) -> TraceLeakReport {
        ...
        if !report.injection_findings.is_empty()
            && DANGEROUS_REASONING.iter().any(|d| text.contains(d))
        {
        ...
        Self {
            ...
            payloads: vec![
                _ApiAttackPayload {
                    ...
                    description: "Sequential object ID in path enables horizontal/vertical access"
                        .into(),
                },
                _ApiAttackPayload {
                    attack: _ApiAttackType::Bola,
...
mod tests {
    ...
    fn test_trace_scan_counts_and_encrypted() {
        let guard = ReasoningTraceGuard::default();
        let report =
            guard.scan("<reasoning>a</reasoning><encrypted_reasoning>b</encrypted_reasoning>");
    ...
    fn test_default_surface_covers_five_types() {
        ...
        let covered: std::collections::HashSet<_ApiAttackType> =
            surface.payloads.iter().map(|p| p.attack).collect();
        assert!(
            covered.len() >= 5,
            "payloads cover only {} types",
            covered.len()
        );
    ...
    fn test_write_guard_check_result_failed_with_evidence() {
        use crate::l3_embodiment::l1_facade::{
            record_write_evidence, scan_write_guard_evidence, KnowledgeBase, WriteGuardVerdict,
        };
    ...
    fn test_write_guard_check_result_clean_passed() {
        use crate::l3_embodiment::l1_facade::{scan_write_guard_evidence, KnowledgeBase};
        let kb = KnowledgeBase::open(Some(std::path::PathBuf::from(":memory:"))).unwrap();
        let stats = scan_write_guard_evidence(&kb);
        let check = write_guard_check_result(&stats);
