...
impl TurnMessage {
    ...
    pub fn encode(&self) -> Bytes {
        ...
        let method_num = match self.method {
            TurnMethod::Allocate => 0x0003,
            TurnMethod::Refresh => 0x0004,
            TurnMethod::Send => 0x0006,
            TurnMethod::Data => 0x0007,
            TurnMethod::CreatePermission => 0x0008,
            TurnMethod::ChannelBind => 0x0009,
        };
    ...
    pub fn decode(data: &[u8]) -> Result<Self, super::stun::StunError> {
        ...
        let method = match msg_type & 0x3EEF {
            0x0003 => TurnMethod::Allocate,
            0x0004 => TurnMethod::Refresh,
            0x0006 => TurnMethod::Send,
            0x0007 => TurnMethod::Data,
            0x0008 => TurnMethod::CreatePermission,
            0x0009 => TurnMethod::ChannelBind,
        ...
        Ok(TurnMessage { method, transaction_id, attributes: vec![] })
    }
}
...
impl TurnClient {
    ...
    pub fn new(server: SocketAddr, local_addr: SocketAddr) -> Self {
        ...
        Self {
            state: TurnAllocationState::Initial,
            server,
            local_addr,
            relay_addr: None,
            lifetime: 600,  // 10分钟
    ...
    /// 生成分配请求
    pub fn create_allocation_request(&mut self) -> Bytes {
        use rand::Rng;
        ...
        let msg = TurnMessage {
            ...
            attributes: vec![
                TurnAttribute::RequestedTransport(17),  // UDP
                TurnAttribute::DontFragment,
            ],
        };

        self.state = TurnAllocationState::Allocating;
        msg.encode()
    }

    /// 处理分配响应
    pub fn handle_allocation_response(&mut self, data: &[u8]) -> bool {
        if let Ok(msg) = TurnMessage::decode(data) {
            if msg.transaction_id == self.transaction_id {
                // 查找中继地址
                for attr in &msg.attributes {
                    if let TurnAttribute::XorRelayedAddress(addr) = attr {
                        self.relay_addr = Some(*addr);
                        self.state = TurnAllocationState::Allocated;
    ...
    /// 生成刷新请求
    pub fn create_refresh_request(&mut self) -> Bytes {
        use rand::Rng;
        rand::thread_rng().fill(&mut self.transaction_id);
        ...
        let msg = TurnMessage {
            ...
            transaction_id: self.transaction_id,
            attributes: vec![
                TurnAttribute::Lifetime(self.lifetime),
            ],
        };

        self.state = TurnAllocationState::Refreshing;
        msg.encode()
    }

    /// 处理刷新响应
    pub fn handle_refresh_response(&mut self, data: &[u8]) -> bool {
        if let Ok(msg) = TurnMessage::decode(data) {
            if msg.transaction_id == self.transaction_id {
                self.state = TurnAllocationState::Allocated;
                self.last_refresh = Some(Instant::now());
    ...
    /// 生成创建权限请求
    pub fn create_permission_request(&mut self, peer_addr: SocketAddr) -> Bytes {
        use rand::Rng;
        rand::thread_rng().fill(&mut self.transaction_id);
        ...
        let msg = TurnMessage {
            ...
            transaction_id: self.transaction_id,
            attributes: vec![
                TurnAttribute::XorMappedAddress(peer_addr),
            ],
        };
    ...
    /// 生成ChannelBind请求
    pub fn create_channel_bind_request(&mut self, peer_addr: SocketAddr, channel: u16) -> Bytes {
        use rand::Rng;
        rand::thread_rng().fill(&mut self.transaction_id);

        let msg = TurnMessage {
            method: TurnMethod::ChannelBind,
            transaction_id: self.transaction_id,
            attributes: vec![
    ...
    pub fn reset(&mut self) {
        self.state = TurnAllocationState::Initial;
...
mod tests {
    ...
    fn turn_client_creation() {
        ...
        let client = TurnClient::new(server, local);
        assert_eq!(client.state, TurnAllocationState::Initial);
    ...
    fn allocation_request_encoding() {
        ...
        let mut client = TurnClient::new(server, local);
        let req = client.create_allocation_request();

        assert!(!req.is_empty());
        assert_eq!(client.state, TurnAllocationState::Allocating);
    }
}
