...
pub(super) fn proxy_config_from_url(url: &str) -> ProxyConfig {
    if url.is_empty() { ProxyConfig::None }
    else { ProxyConfig::Static(url.to_string()) }
}
...
mod tests {
    ...
    fn test_proxy_url_string_tor() {
        ...
        assert_eq!(url, "socks5://127.0.0.1:9050");
    }
    ...
    fn test_stealth_user_agent_contains_chrome() {
        ...
        assert!(STEALTH_USER_AGENT.contains("Safari/537.36"));
    }
    ...
    fn test_doh_servers_all_https() {
        for server in DOH_SERVERS {
            assert!(server.starts_with("https://"), "DOH server should start with https://: {}", server);
        }
    }
