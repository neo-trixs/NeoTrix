...
impl StealthHttpClient {
    async fn send_request(&self, client: &reqwest::Client, parsed: &Url) -> Result<Response, String> {
        ...
        if let Some(persona) = *self.comm_persona.read().await {
            ...
            let ordered = crate::l3_embodiment::nt_shield::nt_shield_comm::build_headers(&persona, parsed.as_str(), &extra_ref);
            for (k, v) in ordered {
                req = req.header(k, v);
            }
        }
        ...
        let headers: HashMap<String, String> = resp
            ...
            .collect();
        let body = resp.bytes().await.map_err(|e| format!("Read body failed: {}", e))?.to_vec();
    ...
    pub fn compute_reward(status: u16, latency_ms: f64, success: bool) -> f64 {
        ...
        let latency_bonus: f64 = if latency_ms < 800.0 { 0.15 }
            else if latency_ms < 2000.0 { 0.05 }
            else if latency_ms > 8000.0 { -0.15 }
            else { 0.0 };
    ...
    pub fn compute_reward_with_body(status: u16, latency_ms: f64, body: &[u8]) -> f64 {
        ...
        if status == 200 {
            ...
            let captcha_signals = [
                "/recaptcha/", "/challenge-platform", "verify you're human",
                "cf-ray", "cf-nt_world_browse-verification", "turnstile",
                "x-served-by: cloudflare", "waf-blocked",
                "why do i have to complete a captcha", "_cf_chl_opt",
            ];
            let detected = captcha_signals.iter()
                .any(|s| body_lower.contains(s));
            if detected {
                return 0.05;
            }
        }
    ...
    pub(super) async fn record_detection(&self) -> bool {
        let streak = self.detection_streak.fetch_add(1, std::sync::atomic::Ordering::Relaxed) + 1;
        if streak >= 5 {
            log::warn!("[detection] streak={}, triggering seal rotation", streak);
            self.detection_streak.store(0, std::sync::atomic::Ordering::Relaxed);
            ...
            if let Some(ref coord) = *self.coordinator.read().await {
                ...
                coord.set_domain_params(RotationDomain::TlsFingerprint, fast_mean, fast_std).await;
                coord.set_domain_params(RotationDomain::HttpHeaders, fast_mean, fast_std).await;
            }
            return true;
        }
        false
    ...
    pub async fn fetch(&self, url: &str) -> Result<Response, String> {
        ...
        match action {
            ...
            Some(OutboundAction::Tor) => {
                let client = self.get_or_create_client("socks5://127.0.0.1:9050").await?;
                let start = Instant::now();
                let result = self.send_request(&client, &parsed).await;
                let elapsed = start.elapsed().as_secs_f64() * 1000.0;
                let reward = match &result {
        ...
        let (chain, exit_url) = match &proxy_config {
            ...
            ProxyConfig::Tor => (None, "socks5://127.0.0.1:9050".into()),
    ...
    async fn fetch_via_proxy(&self, parsed: &Url) -> Result<Response, String> {
        log::warn!("[geo] {} timeout, retrying via Tor proxy", parsed.host_str().unwrap_or("?"));
        if let Some(host) = parsed.host_str() {
            if let Ok(is_cn) = super::super::geo_proxy::domain_resolves_to_china(host).await {
                if is_cn {
                    log::warn!("[geo] domain {} resolves to China IPs, keeping direct", host);
                    let client = self.get_or_create_client("").await?;
                    let start = Instant::now();
                    let result = self.send_request(&client, parsed).await;
                    let elapsed = start.elapsed().as_secs_f64() * 1000.0;
        ...
        self.report_reward_for_host(reward, parsed.host_str()).await;
        {
            ...
            if budget.is_exhausted() {
                log::warn!("[entropy] budget exhausted ({:.1} bits), forcing full rotation", budget.consumed);
                self.force_clear_fingerprint().await;
                budget.reset();
            }
        }
...
mod tests {
    ...
    fn test_response_text_utf8() {
        ...
        assert_eq!(resp.text().expect("text() on UTF-8 body should succeed"), "hello world");
    }
    ...
    fn test_response_empty_body_text() {
        ...
        assert_eq!(resp.text().expect("text() on empty body should return empty string"), "");
    }
