...
impl IpResource {
    pub fn to_proxy_node(&self) -> ProxyNode {
        ProxyNode {
            ...
            label: format!("{}://{}:{}", self.protocol.as_url_scheme(), self.ip, self.port),
...
impl NetworkResourcePool {
    ...
    pub async fn effective_dns_count(&self) -> usize {
        self.dns_servers.read().await.iter().filter(|d| d.effective).count()
    }
    ...
    pub async fn get_effective_dns(&self) -> Vec<DnsServer> {
        self.dns_servers.read().await.iter().filter(|d| d.effective).cloned().collect()
    }
    ...
    pub async fn effective_route_count(&self) -> usize {
        self.route_nodes.read().await.iter().filter(|r| r.effective).count()
    }
    ...
    pub async fn get_effective_routes(&self) -> Vec<RouteNode> {
        self.route_nodes.read().await.iter().filter(|r| r.effective).cloned().collect()
    }
    ...
    pub async fn effective_ip_count(&self) -> usize {
        self.ip_resources.read().await.iter().filter(|r| r.effective).count()
    }
    ...
    pub async fn get_effective_ips(&self) -> Vec<IpResource> {
        self.ip_resources.read().await.iter().filter(|r| r.effective).cloned().collect()
    }
    ...
    pub async fn random_effective_ip(&self) -> Option<IpResource> {
        let effective: Vec<_> = self.ip_resources.read().await.iter()
            .filter(|r| r.effective)
            .cloned()
            .collect();
    ...
    async fn check_dns(server: &DnsServer) -> bool {
        ...
        if tokio::time::timeout(Duration::from_secs(2), stream.write_all(&len_prefixed)).await.is_err() {
            return false;
        }

        // 读响应头 (至少 2 字节长度 + 12 字节响应头)
        let mut header = [0u8; 14];
        if tokio::time::timeout(Duration::from_secs(2), stream.read_exact(&mut header)).await.is_err() {
            return false;
        }
    ...
    pub async fn start_auto_refresh(self: Arc<Self>) {
        ...
        loop {
            sleep(Duration::from_secs(self.refresh_interval_secs.load(Ordering::Relaxed))).await;
    ...
    /// 获取当前池快照
    pub async fn snapshot(&self) -> NetworkPoolSnapshot {
        NetworkPoolSnapshot {
...
#[derive(Debug, Clone)]
pub struct NetworkPoolSnapshot {
    pub dns_total: usize,
...
// ── Unified ResourcePool trait impl ──

impl PooledResource for IpResource {
    ...
    fn effective_score(&self) -> f64 {
        ...
        let reliability = if self.success_count + self.fail_count > 0 {
            self.success_count as f64 / (self.success_count + self.fail_count) as f64
        } else {
            0.5
        };
        latency_factor * reliability * 1000.0
    }
...
impl ResourcePool for NetworkResourcePool {
    type Resource = IpResource;

    async fn total(&self) -> usize {
        self.total_ip_count().await
    }

    async fn effective_count(&self) -> usize {
        self.effective_ip_count().await
    }

    async fn register(&self, resource: Self::Resource) {
        let proto = resource.protocol;
        self.add_ip(&resource.ip, resource.port, &resource.geo, proto).await;
    }

    async fn select_for(&self, _context: Option<&str>) -> Option<Self::Resource> {
        self.random_effective_ip().await
    }

    async fn report_success(&self, resource_id: &str) {
        let mut resources = self.ip_resources.write().await;
        if let Some(r) = resources.iter_mut().find(|r| r.ip == resource_id) {
            r.success_count += 1;
            r.last_verified = Some(Instant::now());
        }
    }

    async fn report_failure(&self, resource_id: &str) {
        let mut resources = self.ip_resources.write().await;
        if let Some(r) = resources.iter_mut().find(|r| r.ip == resource_id) {
            r.fail_count += 1;
            if r.fail_count > 3 {
    ...
    async fn health_check(&self) -> PoolHealthReport {
        let start = Instant::now();
        let snap = self.refresh_all().await;
        PoolHealthReport {
            total_checked: snap.ip_total,
            healthy: snap.ip_effective,
            failed: snap.ip_total.saturating_sub(snap.ip_effective),
    ...
    async fn snapshot(&self) -> PoolSnapshot {
        ...
        PoolSnapshot {
            total: snap.ip_total,
            effective: snap.ip_effective,
            strategy: PoolSelectionStrategy::Fastest,
            healthy_count: snap.ip_effective,
            stale_count: snap.ip_total.saturating_sub(snap.ip_effective),
        }
    }
}
...
mod tests {
    ...
    async fn test_ip_pool() {
        let pool = NetworkResourcePool::new();
        pool.add_ip("1.2.3.4", 8080, "US", ProxyProtocol::Http).await;
        pool.add_ip("5.6.7.8", 1080, "JP", ProxyProtocol::Socks5).await;
    ...
    async fn test_effective_counts_start_at_zero() {
        let pool = NetworkResourcePool::new();
        pool.add_dns_list(&[("1.1.1.1", DnsProtocol::Udp), ("8.8.8.8", DnsProtocol::Udp)]).await;
        pool.add_route_list(&[("10.0.0.1:80", "local")]).await;
        pool.add_ip_list(&[("203.0.113.1", 8080, "US", ProxyProtocol::Http)]).await;
    ...
    async fn test_effective_rates() {
        let pool = NetworkResourcePool::new();
        pool.add_ip("10.0.0.1", 8080, "US", ProxyProtocol::Http).await;
    ...
    async fn test_random_effective_ip() {
        let pool = NetworkResourcePool::new();
        pool.add_ip("10.0.0.1", 8080, "US", ProxyProtocol::Http).await;
        pool.add_ip("10.0.0.2", 8080, "DE", ProxyProtocol::Http).await;
