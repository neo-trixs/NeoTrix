...
use super::{ProxyHealth, ProxyNode, ProxyProtocol, ProxyPool, DynamicProxyChain};
use std::sync::Arc;
...
fn test_proxy_node_display_url() {
    ...
    let node = ProxyNode::new(ProxyProtocol::Socks5, "127.0.0.1", 9050);
    assert_eq!(node.display_url(), "socks5://127.0.0.1:9050");
}
...
fn test_proxy_node_geo_tag() {
    let node = ProxyNode::new(ProxyProtocol::Socks5, "tor-exit.us", 9050)
        .with_geo("US");
...
fn test_dynamic_chain_creation() {
    ...
    chain.add_layer(vec![
        ProxyNode::new(ProxyProtocol::Socks5, "middle1", 1080),
    ]);
    chain.add_layer(vec![
...
async fn test_custom_rotation_interval() {
    let chain = DynamicProxyChain::new("custom-interval")
...
fn test_parse_url_to_node() {
    let node = parse_url_to_node("http://proxy.example.com:3128").expect("parse_url_to_node should succeed for valid http URL");
    assert_eq!(node.protocol, ProxyProtocol::Http);
    assert_eq!(node.host, "proxy.example.com");
    assert_eq!(node.port, 3128);

    let node = parse_url_to_node("socks5://127.0.0.1:9050").expect("parse_url_to_node should succeed for valid socks5 URL");
