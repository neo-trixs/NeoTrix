...
use super::rules::{RuleEngine, OutboundRule, OutboundAction, RuleCondition, RuleOrigin};
...
impl RulesApiServer {
    ...
    async fn write_port(port: u16) {
        ...
        let _ = std::fs::write(dir.join(RULES_API_PORT_FILE), port.to_string());
    }
    ...
    fn parse_rule(req: RuleRequest) -> Result<OutboundRule, String> {
        let condition = match req.condition_type.to_lowercase().as_str() {
            ...
            "url_path_prefix" | "urlpathprefix" => RuleCondition::UrlPathPrefix(req.condition_value),
        ...
        let mut rule = OutboundRule::external(&req.label, condition, action,
    ...
    async fn handle_request(body: &str, engine: &RwLock<RuleEngine>) -> String {
        ...
        match method {
            "register_rule" | "POST" => {
                let rule_req: RuleRequest = match serde_json::from_value(payload.cloned().unwrap_or(serde_json::Value::Null)) {
                    Ok(r) => r,
            ...
            "remove_rule" | "DELETE" => {
                let label = payload.and_then(|p| p.get("label")).and_then(|l| l.as_str()).unwrap_or("");
                if label.is_empty() { return Self::json_resp(false, "Missing label".into(), None); }
                engine.write().await.remove_rule(label);
                Self::json_resp(true, format!("Rule '{}' removed", label), None)
            }
            "list_rules" | "GET" => {
                let rules = engine.read().await.export();
                let external: Vec<&OutboundRule> = rules.iter().filter(|r| matches!(r.origin, RuleOrigin::External(_))).collect();
                ...
                Self::json_resp(true, "ok".into(), Some(serde_json::json!({"rules": all, "total": rules.len()})))
            }
            "clear_all" => { engine.write().await.clear_external(); Self::json_resp(true, "Cleared external rules".into(), None) }
            "status" => {
                let rules = engine.read().await.export();
                let external = rules.iter().filter(|r| matches!(r.origin, RuleOrigin::External(_))).count();
                Self::json_resp(true, "ok".into(), Some(serde_json::json!({"active_rules": rules.len(), "external_rules": external})))
            }
    ...
    pub async fn start(self: Arc<Self>) -> Result<(), String> {
        ...
        println!("[rules-api] listening on 127.0.0.1:{}", port);
        loop {
            let (mut stream, _) = match listener.accept().await {
                Ok(s) => s,
                Err(e) => { log::error!("[rules-api] accept: {}", e); continue; }
            };
    ...
    pub async fn stats(&self) -> RulesApiStatus {
        ...
        let external_count = rules.iter().filter(|r| matches!(r.origin, RuleOrigin::External(_))).count();
        RulesApiStatus {
            running: self.ready.load(Ordering::Relaxed),
            port: 0,
...
mod tests {
    ...
    fn test_parse_rule_domain_suffix() {
        ...
        let rule = RulesApiServer::parse_rule(req).expect("parse_rule should succeed for hardcoded valid RuleRequest");
    ...
    fn test_rule_request_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&req).expect("serde_json to_string on RuleRequest should succeed");
        let deserialized: RuleRequest = serde_json::from_str(&json).expect("serde_json from_str of roundtripped JSON should succeed");
    ...
    fn test_handle_request_remove_rule_with_label() {
        let engine = tokio::sync::RwLock::new(RuleEngine::new_empty());
        {
            ...
            eng.add_rule(OutboundRule::new("to-remove", RuleCondition::Always, OutboundAction::Block));
        }
    ...
    fn test_handle_request_list_rules() {
        ...
        {
            let mut eng = engine.blocking_write();
            eng.add_rule(OutboundRule::external("ext1", RuleCondition::Always, OutboundAction::Block, "app"));
        }
    ...
    fn test_handle_request_status() {
        ...
        {
            let mut eng = engine.blocking_write();
            eng.add_rule(OutboundRule::new("local", RuleCondition::Always, OutboundAction::Direct));
            eng.add_rule(OutboundRule::external("ext", RuleCondition::Always, OutboundAction::Block, "test"));
        }
    ...
    fn test_handle_request_delete_shorthand() {
        ...
        {
            let mut eng = engine.blocking_write();
            eng.add_rule(OutboundRule::new("del-me", RuleCondition::Always, OutboundAction::Block));
        }
