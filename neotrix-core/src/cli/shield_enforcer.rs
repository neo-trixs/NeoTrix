...
use crate::cli::sandbox::{CliSandboxMode, SandboxEnforcer};
...
impl ShieldEnforcer {
    pub fn new() -> Self {
        Self {
            ...
            sandbox: SandboxEnforcer::new(CliSandboxMode::Disabled),
    ...
    /// Set sandbox mode.
    pub fn set_sandbox_mode(&mut self, mode: CliSandboxMode) {
...
mod tests {
    ...
    fn test_shield_enforcer_new() {
        ...
        assert_eq!(s.sandbox.mode(), CliSandboxMode::Disabled);
    ...
    fn test_check_all_sandbox_read_only() {
        ...
        s.set_sandbox_mode(CliSandboxMode::ReadOnly);
        let result = s.check_all("file_read", "/tmp/test.txt", None, None);
    ...
    fn test_set_project_root() {
        ...
        assert!(s
            .guard
            .check("file_read", "/project/src/lib.rs")
            .unwrap_or(false));
    }
    ...
    fn test_guardrail_blocks_long_input() {
        ...
        let result = s.check_all(
            "file_read",
            "/tmp/test.txt",
            Some("very long input that exceeds the limit"),
            None,
        );
        assert!(result.is_err());
    }
    ...
    fn test_e2e_check_laws_integration() {
        ...
        let violations = s.check_laws(content, Some("src/main.rs"));
        assert!(
            violations.iter().any(|v| v.code == "L001"),
            "should detect L001"
        );
        assert!(
            violations.iter().any(|v| v.code == "L002"),
            "should detect L002"
        );
        assert!(
            violations.iter().any(|v| v.code == "L003"),
            "should detect L003"
        );
    }
    ...
    fn test_e2e_policy_profile_switch_reconfigures_chain() {
        ...
        s.set_sandbox_mode(CliSandboxMode::ReadOnly);
        assert_eq!(s.sandbox.mode(), CliSandboxMode::ReadOnly);
    }
    ...
    fn test_e2e_project_laws_describe_all() {
        for code in &[
            ...
            assert!(
                ProjectLaws::describe(code).is_some(),
