...
pub enum BudgetStatus {
    ...
    Exceeded { action: BudgetAction, message: String },
}
...
impl CostTracker {
    ...
    pub fn total_cost(&self) -> f64 {
        ...
        let current = self.current.as_ref().map(|c| c.estimated_cost).unwrap_or(0.0);
        completed + current
    }
    ...
    pub fn current_session_cost(&self) -> f64 {
        self.current.as_ref().map(|c| c.estimated_cost).unwrap_or(0.0)
    }

    /// Model name of the current session (or "—").
    pub fn current_model(&self) -> String {
        self.current.as_ref().map(|c| c.model.clone()).unwrap_or_else(|| "—".to_string())
    }

    /// Provider name of the current session.
    pub fn current_provider(&self) -> String {
        self.current.as_ref().map(|c| c.provider.clone()).unwrap_or_else(|| "—".to_string())
    }
    ...
    pub fn summary(&self) -> CostSummary {
        ...
        let top = self.sessions.iter()
            .max_by(|a, b| a.estimated_cost.partial_cmp(&b.estimated_cost).unwrap_or(std::cmp::Ordering::Equal))
            .map(|s| s.model.clone())
    ...
    pub fn status_line(&self) -> String {
        ...
        let parts = format!("Cost: ${:.4} | Tokens: {} in/{} out | Model: {}", cost, tin, tout, model);
        if let Some(remaining) = self.budget_remaining() {
            format!("{} | Budget: ${:.2} remaining ({})", parts, remaining, self.budget_period_label())
        } else {
            parts
        }
    }
    ...
    fn load_budget_config(&self) {
        ...
        if let Ok(data) = std::fs::read_to_string(&path) {
            if let Ok(cfg) = serde_json::from_str::<BudgetConfig>(&data) {
                *self.budget.lock().expect("budget lock") = cfg;
            }
        }
    }
    ...
    pub fn save_budget_config(&self) {
        ...
        if let Ok(data) = serde_json::to_string_pretty(&*self.budget.lock().expect("budget lock")) {
    ...
    pub fn set_budget_config(&self, config: BudgetConfig) {
        *self.budget.lock().expect("budget lock") = config;
    ...
    pub fn check_budget(&self, session_cost: f64) -> BudgetStatus {
        let cfg = self.budget.lock().expect("budget lock");
        if !cfg.enabled {
        ...
        let pct = |current, limit| -> f64 {
            if limit > 0.0 { current / limit } else { 0.0 }
        };
        ...
        if session_pct >= 0.8 {
            warnings.push(format!("Session at {:.0}% of ${:.2} limit", session_pct * 100.0, max_session));
        }
        if daily_pct >= 0.8 {
            warnings.push(format!("Daily at {:.0}% of ${:.2} limit", daily_pct * 100.0, max_daily));
        }
        if monthly_pct >= 0.8 {
            warnings.push(format!("Monthly at {:.0}% of ${:.2} limit", monthly_pct * 100.0, max_monthly));
        }

        if warnings.is_empty() {
            BudgetStatus::Ok
        } else {
            BudgetStatus::Warning { message: warnings.join("; ") }
        }
    }
    ...
    pub fn check_budget_and_warn(&self, session_cost: f64) -> Option<String> {
        ...
        match self.check_budget(session_cost) {
            ...
            BudgetStatus::Exceeded { action, message } => match action {
                BudgetAction::Stop => {
                    Some(format!("🛑 BUDGET EXCEEDED: {}. Session will stop.", message))
                }
                BudgetAction::Pause => {
                    Some(format!("⏸️ BUDGET WARNING: {}. Processing paused.", message))
                }
                BudgetAction::Warn => {
                    Some(format!("⚠️ BUDGET WARNING: {}", message))
                }
            },
        }
    }
    ...
    pub fn reset_budget_period(&self, _period: BudgetPeriod) {
        *self.budget.lock().expect("budget lock") = BudgetConfig::default();
        self.save_budget_config();
    }
    ...
    pub fn budget_status(&self) -> String {
        let cfg = self.budget.lock().expect("budget lock");
        if !cfg.enabled {
            return "Budget Status: DISABLED\n  Use `/budget enable` to activate budget limits.".to_string();
        }
...
mod tests {
    ...
    fn test_check_budget_disabled_returns_ok() {
        let t = CostTracker::new();
        *t.budget.lock().unwrap() = BudgetConfig::default();
    ...
    fn test_check_budget_session_exceeded() {
        ...
        assert!(matches!(status, BudgetStatus::Exceeded { action: BudgetAction::Warn, .. }));
    }
