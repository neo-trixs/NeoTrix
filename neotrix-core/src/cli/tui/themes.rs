...
impl<'de> Deserialize<'de> for Theme {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        let bg = parse_color(&h.bg)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid color: {}", h.bg)))?;
        let accent = parse_color(&h.accent)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid color: {}", h.accent)))?;
        let primary = parse_color(&h.primary)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid color: {}", h.primary)))?;
        let secondary = parse_color(&h.secondary)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid color: {}", h.secondary)))?;
        let highlight = parse_color(&h.highlight)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid color: {}", h.highlight)))?;
        Ok(Theme {
            name: h.name,
            bg,
            accent,
    ...
    fn test_all_theme_colors_non_reset() {
        for theme in builtin_themes() {
            ...
            assert_ne!(
                theme.accent,
                Color::Reset,
                "theme {} accent is Reset",
                theme.name
            );
            assert_ne!(
                theme.primary,
                Color::Reset,
                "theme {} primary is Reset",
                theme.name
            );
            assert_ne!(
                theme.secondary,
                Color::Reset,
                "theme {} secondary is Reset",
                theme.name
            );
            assert_ne!(
                theme.highlight,
                Color::Reset,
                "theme {} highlight is Reset",
                theme.name
            );
        }
    }
