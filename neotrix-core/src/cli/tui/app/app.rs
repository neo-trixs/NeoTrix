...
use crossterm::event::{KeyCode, KeyModifiers, KeyEventKind, MouseButton};
use crate::cli::commands::{CommandRegistry, default_registry, CommandOutput};
...
use super::types::{Session, ChatMessage, GoalDisplay, SideMessage, ask_side_llm};
use crate::cli::tui::themes::{self, Theme};
use crate::cli::sandbox::{SandboxMode, global_sandbox};
use base64::Engine;
...
impl TuiApp {
    pub fn new(ephemeral: bool) -> Self {
        ...
        Self {
            ...
            diff_viewer: None,
            workspace_name: {
                let mgr = crate::core::WORKSPACE_MANAGER.lock().unwrap();
                mgr.active().map(|w| w.name.clone()).unwrap_or_else(|| "default".to_string())
            },
            workspace_count: {
                let mgr = crate::core::WORKSPACE_MANAGER.lock().unwrap();
                mgr.list().len()
            ...
            sandbox_mode: global_sandbox().lock().unwrap().mode(),
            session_store: store,
            ephemeral,
        ...
        if role == "user" {
            if let Some((name, data)) = self.image_attachment.take() {
                self.active_session_mut().messages.push_back(
                    ChatMessage::with_image(role, content, Some(data), Some(name))
                );
                return;
            }
        }
        self.active_session_mut().messages.push_back(ChatMessage::new(role, content));
    }
    ...
    pub fn handle_fork(&mut self, text: &str) {
        ...
        let new_id = format!("s-{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap_or("0"));
    ...
    pub fn handle_key(&mut self, key: KeyCode, modifiers: KeyModifiers) -> Option<String> {
        match key {
            KeyCode::Char('n') if modifiers == KeyModifiers::CONTROL => {
                ...
                self.sessions.push(Session { id, name, messages: VecDeque::new() });
                self.active_session = self.sessions.len() - 1;
                self.auto_save();
            }
            ...
            KeyCode::Char('r') if modifiers == KeyModifiers::CONTROL => {
                if self.command_history.search_active {
                    ...
                    if let Some(&idx) = self.command_history.search_results.get(self.command_history.search_selection) {
    ...
    fn build_completions(&mut self) {
        ...
        if let Some(cmd_end) = trimmed.find(' ') {
            ...
            match cmd {
                ...
                "/approval" | "/approve" => {
                    let subs = ["mode", "status", "list", "approve", "deny", "approve-all", "deny-all"];
                    for s in &subs {
                        if s.starts_with(after) {
                            self.completions.push(format!("{} {}", cmd, s));
                        }
                ...
                "/session" => {
                    let subs = ["list", "save", "load", "delete", "fork", "export", "import", "share"];
                    for s in &subs {
                        if s.starts_with(after) {
                            self.completions.push(format!("{} {}", cmd, s));
                        }
                ...
                "/mem" => {
                    let subs = ["view", "list", "search", "edit", "tag", "delete", "pin", "checkpoint", "rollback", "dream", "stats"];
                    for s in &subs {
                        if s.starts_with(after) {
                            self.completions.push(format!("{} {}", cmd, s));
                        }
    ...
    pub fn process_submitted_text(&mut self, text: &str, brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> Result<CommandOutput, String> {
        if text.starts_with('/') {
    ...
    fn handle_vim_toggle(&mut self, text: &str) {
        ...
        match subcmd {
            "on" => { self.vim_mode.set_enabled(true); self.status_text = "Vim: ON".to_string(); }
            "off" => { self.vim_mode.set_enabled(false); self.status_text = "Vim: OFF".to_string(); }
            "toggle" | "" => {
                self.vim_mode.toggle();
                self.status_text = if self.vim_mode.is_enabled() { "Vim: ON".to_string() } else { "Vim: OFF".to_string() };
            }
            _ => { self.push_message("system", format!("Unknown /vim subcommand: {subcmd}. Available: toggle, on, off, status")); }
        }
    }
        ...
        match self.process_submitted_text(&text, Some(agent)) {
            Ok(output) => {
                ...
                self.push_message("system",
                    if output.success { output.message } else { format!("Error: {}", output.message) });
            }
            Err(msg) => {
                self.agent_busy = true;
                ...
                self.active_session_mut().messages.push_back(
                    ChatMessage::with_image("user", msg.clone(), image_data.clone(), image_name)
                );
                if let Some(ref mut engine) = a.reasoning_engine {
                    {
                        let tracker = crate::cli::cost_tracker::COST_TRACKER.lock().expect("COST_TRACKER lock");
                    ...
                    match engine.reason_stream(&msg, image_data).await {
                        Ok((_full_response, mut rx)) => {
                            ...
                            while let Some(token) = rx.recv().await {
                                ...
                                token_count += 1;
                                let elapsed = stream_start.elapsed().as_secs_f64().max(0.001);
                    ...
                    self.push_message("assistant", format!("Evolved: {:.3} → {:.3}", result.score_before, result.score_after));
                    if let Some(ref kb) = a._nt_memory_kb {
                        self.save_and_distill(kb);
                    } else {
                        self.auto_save();
        ...
        match ask_side_llm(rest).await {
            Ok(answer) => {
                ...
                let msg = format!(
                    ...
                    rest, answer, duration.as_secs_f64()
                );
    ...
    pub async fn run(&mut self, agent: Arc<RwLock<SelfIteratingBrain>>) {
        ...
        while self.running {
            ...
            {
                let mgr = crate::core::WORKSPACE_MANAGER.lock().expect("WORKSPACE_MANAGER lock");
                self.workspace_name = mgr.active().map(|w| w.name.clone()).unwrap_or_else(|| "default".to_string());
                ...
                Ok(crossterm::event::Event::Key(key_event)) => {
                    if key_event.kind == KeyEventKind::Press {
                        ...
                        if key_event.code == KeyCode::Char('p') && key_event.modifiers == KeyModifiers::CONTROL {
                            self.status_text = "Switch Provider: type provider name and press Enter".to_string();
                        ...
                        if self.vim_mode.is_enabled() {
                            let action = self.vim_mode.handle_key(key_event.code, key_event.modifiers);
                            let vim_submitted = match &action {
                                VimAction::PassThrough => false,
                                VimAction::Search(cmd) => self.handle_vim_command(cmd, &agent, &mut terminal).await,
                                VimAction::Quit => { self.running = false; true }
                                VimAction::SwitchSession(n) => {
                                    if *n == usize::MAX {
                                        if self.active_session > 0 { self.active_session -= 1; }
                                ...
                                VimAction::MoveDown => { self.scroll_offset += 1; true }
                                VimAction::MoveUp => { self.scroll_offset = self.scroll_offset.saturating_sub(1); true }
                                VimAction::MovePageDown => { self.scroll_offset += 10; true }
                                VimAction::MovePageUp => { self.scroll_offset = self.scroll_offset.saturating_sub(10); true }
                                VimAction::MoveLineStart => { self.scroll_offset = 0; true }
                                VimAction::MoveLineEnd => { self.scroll_offset = usize::MAX; true }
                                VimAction::EnterInsertMode | VimAction::EnterNormalMode | VimAction::EnterVisualMode
                                    | VimAction::None | VimAction::DeleteChar | VimAction::InsertChar(_)
                                    | VimAction::Yank | VimAction::Paste | VimAction::Undo
                                    | VimAction::MoveLeft | VimAction::MoveRight
                                    | VimAction::MoveWordForward | VimAction::MoveWordBack => true,
                            };
                            if vim_submitted { continue; }
                        }
                        ...
                        if let Some(text) = self.handle_key(key_event.code, key_event.modifiers) {
                            ...
                            if trimmed.starts_with("/image ") {
                                ...
                                match load_image_attachment(path) {
                                    Ok((name, data)) => {
                                        ...
                                        self.push_message("system", format!("已加载图片: {}", name));
                            ...
                            if !trimmed.is_empty() && is_image_path(trimmed) && std::path::Path::new(trimmed).exists() {
                            ...
                            match self.process_submitted_text(&text, Some(&agent)) {
                                Ok(output) => {
                                    ...
                                    self.push_message("system", if output.success { msg } else { format!("错误: {}", msg) });
                                }
                                Err(msg) => {
                                    self.agent_busy = true;
                                    let pending_image = self.image_attachment.take();
        ...
        execute!(std::io::stdout(), LeaveAlternateScreen, DisableMouseCapture).expect("Failed to leave alternate screen");
