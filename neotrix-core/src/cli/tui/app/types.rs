...
pub async fn ask_side_llm(question: &str) -> Result<String, String> {
    ...
    use crate::neotrix::nt_shield_prompt::default_output_screener;
    let config = ProviderConfig::from_env();
    let model =
        std::env::var("NEOTRIX_MODEL").unwrap_or_else(|_| "claude-sonnet-4-20250514".to_string());
    let llm = create_provider(config);
    let request = LlmRequest::new(&model, question);
    let response = llm.complete(&request).await.map_err(|e| format!("{}", e))?;
    Ok(default_output_screener().sanitize(&response.content))
}
