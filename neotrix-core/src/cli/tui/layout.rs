...
use super::output::{render_markdown, render_thinking_block, role_style};
...
pub fn render_session_list(frame: &mut Frame, area: Rect, app: &TuiApp, theme: &Theme) {
    let items: Vec<ListItem> = app
        ...
        .enumerate()
        .map(|(i, s)| {
            let prefix = if i == app.active_session {
                "▶ "
            } else {
                "  "
            };
...
pub fn render_input_panel(frame: &mut Frame, area: Rect, app: &TuiApp, theme: &Theme) {
    ...
    let hist_indicator = match app.command_history.position {
        Some(pos) => format!(
            " (history {}/{})",
            pos + 1,
            app.command_history.entries.len()
        ),
        None => String::new(),
    };
    let title = if app.multi_line {
        format!("输入 [多行]{}", hist_indicator)
    } else {
        format!("输入{}", hist_indicator)
    };
...
fn render_history_search(frame: &mut Frame, area: Rect, app: &TuiApp, theme: &Theme) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("  ({} matches | Ctrl+R cycle | Esc cancel)", total),
            Style::default().fg(theme.secondary),
        ),
    ]));
    ...
    let (mut lines, title) = if app.goal_display.has_goal {
        ...
        let mut lines = vec![
            ...
            Line::from(vec![
                ...
                Span::raw(format!(
                    "{}/{} ({:.0}%)",
                    g.iterations, g.max_iterations, pct
                )),
        ...
        let extra = if app.goal_display.queue_count > 0 || app.goal_display.completed_count > 0 {
            format!(
                " (Q:{}, C:{})",
                app.goal_display.queue_count, app.goal_display.completed_count
            )
        } else {
            String::new()
        };
...
pub fn render_diff_viewer(frame: &mut Frame, area: Rect, app: &TuiApp, theme: &Theme) {
    let dv = match &app.diff_viewer {
        Some(dv) => dv,
        None => {
            let paragraph = Paragraph::new("No diff loaded").block(
                Block::default()
                    .title(" Diff Viewer ")
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(theme.secondary)),
    ...
    let paragraph = Paragraph::new(Text::from(visible))
        .block(
            Block::default()
                .title(format!(
                    " Diff Viewer — {} file(s) (j/k scroll, q close) ",
                    file_count
                ))
                .borders(Borders::ALL)
                .border_style(Style::default().fg(theme.accent)),
...
pub fn render_status_bar(frame: &mut Frame, area: Rect, app: &TuiApp, theme: &Theme) {
    ...
    let is_sandbox = app.sandbox_mode != crate::cli::sandbox::CliSandboxMode::Disabled;
    let status = Paragraph::new(Line::from(Span::raw(status_text))).style(
        Style::default()
            .bg(if app.streaming {
                theme.accent
            } else if app.agent_busy {
