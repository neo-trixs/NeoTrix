        ...
        match ch {
            ...
            '"' | '\'' => {
                ...
                while let Some(c) = chars.next() {
                    ...
                    } else if (triple && s.ends_with(&format!("{0}{0}{0}", quote)) && s.len() > 3) || (!triple && c == quote) {
                        break;
                    }
                }
                spans.push(Span::styled(s, Style::default().fg(Color::Green)));
            ...
            c if c.is_alphabetic() || c == '_' => {
                ...
                if keywords.contains(&word.as_str()) {
                    spans.push(Span::styled(word, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)));
                } else if builtins.contains(&word.as_str()) || word.chars().next().is_some_and(|c| c.is_uppercase()) {
                    spans.push(Span::styled(word, Style::default().fg(Color::Cyan)));
                } else {
                    spans.push(Span::raw(word));
                }
    ...
    while let Some(ch) = chars.next() {
        match ch {
            ...
            c if c.is_alphabetic() || c == '_' => {
                ...
                if keywords.contains(&word.as_str()) {
                    spans.push(Span::styled(word, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)));
                } else if line.split_whitespace().next().is_some_and(|cmd| word == cmd) {
                    spans.push(Span::styled(word, Style::default().fg(Color::Cyan)));
                } else {
                    spans.push(Span::raw(word));
                }
