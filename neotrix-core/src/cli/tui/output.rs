...
fn highlight_rust_line(line: &str) -> Vec<Span<'static>> {
    let keywords = [
        "fn", "let", "mut", "if", "else", "for", "while", "loop", "match", "return", "true",
        "false", "Some", "None", "Ok", "Err", "pub", "use", "mod", "struct", "enum", "impl",
        "trait", "async", "await", "move", "ref", "where", "type", "const", "static", "unsafe",
    ...
    while let Some(ch) = chars.next() {
        match ch {
            ...
            c if c.is_ascii_digit()
                || (c == '-' && chars.peek().is_some_and(|p| p.is_ascii_digit())) =>
            {
                let mut num = if c == '-' {
                    "-".to_string()
                } else {
...
fn highlight_python_line(line: &str) -> Vec<Span<'static>> {
    let keywords = [
        ...
        "yield", "import", "from", "as", "with", "try", "except", "finally", "raise", "pass",
        "None", "True", "False", "and", "or", "not", "in", "is", "lambda", "async", "await",
        "self", "print",
    ];
    let builtins = [
        "len",
...
fn highlight_javascript_line(line: &str) -> Vec<Span<'static>> {
    ...
    let builtins = [
        ...
        "Boolean", "Promise", "Map", "Set", "Symbol", "RegExp", "Date", "Error",
    ];
    ...
    while let Some(ch) = chars.next() {
        match ch {
            '"' => {
                ...
                let is_key =
                    s.len() > 2 && trimmed.starts_with('"') && trimmed[1..].contains("\":");
                spans.push(Span::styled(
                    s,
                    Style::default().fg(if is_key { Color::Yellow } else { Color::Green }),
                ));
            }
...
fn highlight_shell_line(line: &str) -> Vec<Span<'static>> {
    let keywords = [
        ...
        "function", "return", "exit", "export", "local", "source", "echo", "printf", "read", "set",
        "unset", "declare", "typeset", "select", "until",
    ];
...
mod tests {
    ...
    fn test_render_inline_code() {
        ...
        assert!(
            code_span.is_some(),
            "Could not find `std::fs` in spans: {:?}",
            spans
        );
    }
