...
impl CliCommand for WorkSpaceCmd {
    ...
    fn execute(&self, args: &[String], _brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        let mut mgr = WORKSPACE_MANAGER.lock().expect("WORKSPACE_MANAGER lock");
        let sub = clean_args[0];

        match sub {
            "create" | "new" => {
                if clean_args.len() < 2 {
                    return CommandOutput::err("用法: /workspace create <name> [--path <dir>] [--desc <text>]");
                }
                ...
                let project_root = path_idx.and_then(|i| clean_args.get(i + 1)).map(|s| std::path::PathBuf::from(s));
                let description = desc_idx.and_then(|i| clean_args.get(i + 1)).map(|s| s.to_string()).unwrap_or_default();
            ...
            "list" | "ls" => {
                let spaces = mgr.list();
                if spaces.is_empty() {
                    return CommandOutput::ok("📂 No workspaces. Create one with /workspace create <name>");
                }
                let mut s = format!("📂 WorkSpaces ({}):\n", spaces.len());
                for ws in spaces {
                    let active_mark = if Some(ws.id.as_str()) == mgr.active_id.as_deref() { " *" } else { "  " };
                    let root = ws.project_root.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
                ...
                if want_json {
                    ...
                    out.with_json(serde_json::json!({"workspaces": json_list, "count": json_list.len()}))
                } else { out }
            }
            "switch" | "use" | "activate" => {
                if clean_args.len() < 2 {
                ...
                match mgr.switch(clean_args[1]) {
                    Ok(()) => {
                        let _ = mgr.save();
                        let out = CommandOutput::ok(&format!("➡️ Switched to workspace: {}", clean_args[1]));
                        if want_json { out.with_json(serde_json::json!({"switched": true, "id": clean_args[1]})) } else { out }
                    }
            ...
            "delete" | "rm" => {
                ...
                match mgr.delete(clean_args[1]) {
                    Ok(()) => {
                        let _ = mgr.save();
                        let out = CommandOutput::ok(&format!("🗑️ Deleted workspace: {}", clean_args[1]));
                        if want_json { out.with_json(serde_json::json!({"deleted": true, "id": clean_args[1]})) } else { out }
                    }
                    Err(e) => CommandOutput::err(&e),
                }
            }
            "rename" | "mv" => {
                ...
                match mgr.rename(clean_args[1], clean_args[2]) {
                    Ok(()) => {
                        let _ = mgr.save();
                        let out = CommandOutput::ok(&format!("✏️ Renamed {} → {}", clean_args[1], clean_args[2]));
                        if want_json { out.with_json(serde_json::json!({"renamed": true, "id": clean_args[1], "new_name": clean_args[2]})) } else { out }
                    }
                    Err(e) => CommandOutput::err(&e),
                }
            }
            "status" | "info" | "current" => {
                match mgr.active() {
                    ...
                    None => {
                        let out = CommandOutput::ok("📌 No active workspace. Create one with /workspace create <name>");
                        if want_json { out.with_json(serde_json::json!({"active": false})) } else { out }
                    }
                }
            }
            _ => CommandOutput::err(&format!("未知子命令: {}. 可用: create, list, switch, delete, rename, status", sub)),
        }
    }
}
...
impl CliCommand for RouterCmd {
    ...
    fn execute(&self, args: &[String], _brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        if plain_args.is_empty() {
            ...
            let out = CommandOutput::ok(msg);
            return if want_json { out.with_json(serde_json::json!({"subcommands": ["status", "enable", "disable", "reset", "set", "classify"]})) } else { out };
        }

        let sub = plain_args[0];
        match sub {
            "status" | "stats" => {
                let router = SMART_ROUTER.lock().expect("SMART_ROUTER lock");
            ...
            "enable" | "on" => {
                let mut router = SMART_ROUTER.lock().expect("SMART_ROUTER lock");
            ...
            "set" | "rule" => {
                if plain_args.len() < 4 {
                    return CommandOutput::err("用法: /route set <complexity> <provider> <model> [cost_in cost_out]");
                }
                let complexity = match TaskComplexity::from_str(plain_args[1]) {
                    Some(c) => c,
                    None => return CommandOutput::err(&format!("无效复杂度: {}。可用: trivial, simple, moderate, complex, critical", plain_args[1])),
                };
                ...
                let msg = format!("🔀 路由规则已设置: {} → {} / {} (${:.4} in, ${:.4} out)",
                    complexity.label(), provider, model, cost_in, cost_out);
                let out = CommandOutput::ok(&msg);
                if want_json {
                    out.with_json(serde_json::json!({
    ...
    fn execute(&self, args: &[String], _brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        match subcmd {
            ...
            "task" => {
                ...
                match task_subcmd {
                    "add" => {
                        let desc: String = task_args.iter().skip(1).cloned().collect::<Vec<_>>().join(" ");
                        if desc.is_empty() { return CommandOutput::err("Usage: /background task add <description>"); }
                        let clean_desc: String = desc.split("--interval").next().unwrap_or(&desc).trim().to_string();
                        let mut engine = ALWAYS_ON_ENGINE.lock().expect("ALWAYS_ON_ENGINE lock");
                    ...
                    "list" => {
                        ...
                        for t in &tasks {
                            ...
                            let interval = if t.interval_secs > 0 { format!("every {}s", t.interval_secs) } else { "oneshot".into() };
                            msg.push_str(&format!("  [{}] {} (runs={}, {})\n", t.id, t.description, runs, interval));
                        }
                        CommandOutput::ok(msg.trim())
                    }
                    "remove" => {
                        let id = task_args.get(1).map(|s| s.as_str()).unwrap_or("");
                        if id.is_empty() { return CommandOutput::err("Usage: /background task remove <id>"); }
                        let mut engine = ALWAYS_ON_ENGINE.lock().expect("ALWAYS_ON_ENGINE lock");
            ...
            "cycle" => {
                ...
                match engine.full_cycle() {
                    Ok(report) => {
                        let _ = engine.save();
                        CommandOutput::ok(&format!("Cycle complete: scanned={}, executed={}, took={}ms",
