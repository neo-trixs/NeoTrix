...
impl CliCommand for AgentCmd {
    ...
    fn description(&self) -> &str {
        "Agent management: /agent team <roles...> | /agent list | /agent status"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        if args.is_empty() || (args.len() == 1 && args[0] == "--json") {
            return CommandOutput::err(
                "用法: /agent team <roles...> | list | status | create <role>",
            );
        }
        ...
        match cmd {
            "team" => {
                ...
                if roles.is_empty() {
                    return CommandOutput::err(
                        "需要角色名: /agent team researcher,writer,reviewer",
                    );
                }
                ...
                let msg = format!(
                    "🤖 AgentTeam 'cli-team' created: {} agents",
                    team.agents.len()
                );
                if want_json {
            ...
            "list" | "ls" => {
                let msg = if let Some(b) = brain {
                    ...
                    format!(
                        "🤖 Agents: default (main) | iterations: {} | brain_version: {}",
                ...
                if want_json {
                    return CommandOutput::ok(&msg)
                        .with_json(serde_json::json!({"agents": ["default"]}));
                }
                CommandOutput::ok(&msg)
            }
            "status" => {
                if let Some(b) = brain {
                    ...
                    let msg = format!(
                        "🧠 Brain: {} absorbs | {} memories | capability: {:.3} | iterations: {}",
            ...
            "create" => {
                if args.len() < 2 {
                    return CommandOutput::err("需要角色名: /agent create researcher");
                }
                CommandOutput::ok(&format!("🤖 Agent '{}' created", args[1]))
            }
            _ => CommandOutput::err(&format!(
                "未知子命令: {}. 可用: team, list, status, create",
                cmd
            )),
        }
    }
...
impl CliCommand for DiscoverCmd {
    ...
    fn description(&self) -> &str {
        "Discover NeoTrix agents on the network: /discover [--json] [--port <port>] [--duration <ms>]"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        let mut discovery =
            match crate::neotrix::nt_agent_protocol::discovery::AgentDiscovery::new(port) {
                Ok(d) => d,
        ...
        match discovery.discover(duration) {
            Ok(agents) => {
                ...
                for (i, a) in agents.iter().enumerate() {
                    let id_trunc = if a.id.len() > 22 {
                        format!("{}…", &a.id[..21])
                    } else {
                        a.id.clone()
                    };
                    let host_trunc = if a.host.len() > 20 {
                        format!("{}…", &a.host[..19])
                    } else {
                        a.host.clone()
                    };
...
impl CliCommand for McpCmd {
    ...
    fn description(&self) -> &str {
        "MCP server management. Sub: list, add, auth, debug"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        match cmd {
            ...
            "status" | "stat" => {
                if let Some(b) = brain {
                    ...
                    let msg = format!(
                        "🔌 MCP Bridge: {} tool calls | {} traces cached",
                        tool_calls,
                        a.tool_traces.len()
                    );
                    if want_json {
            ...
            "publish" | "add" => {
                if args.len() < 3 {
                    return CommandOutput::err(
                        "用法: /mcp publish <name> <command> [args...] [--description <desc>]",
                    );
                }
                let name = &args[1];
                let command = &args[2];
            ...
            _ => CommandOutput::err(&format!(
                "未知子命令: {}. 可用: list, status, discover, search, publish",
                cmd
            )),
        }
    }
...
mod tests {
    ...
    fn test_placeholder() {
        ...
        eprintln!("TODO: implement test assertions for agent commands");
    }
}
