...
use crate::cli::sandbox::{global_sandbox, init_sandbox, SandboxMode};
...
impl CliCommand for SandboxCmd {
    ...
    fn aliases(&self) -> Vec<&str> { vec!["/sb"] }
    ...
    fn execute(
        ...
        match sub {
            ...
            "set" => {
                ...
                let mode = SandboxMode::from_str(&args[1]);
                init_sandbox(mode);
                let descr = match mode {
                    SandboxMode::Disabled => "commands run unsandboxed",
                    SandboxMode::ReadOnly => "mutating commands blocked",
                    SandboxMode::Docker => "exec via Docker (network=none, 512m, 1 cpu)",
                };
                let msg = format!(
            ...
            "run" => {
                if args.len() < 3 {
                    return CommandOutput::err("Usage: /sandbox run <python3|node18|rust|go|linux> <code>");
                }
            ...
            "runtimes" | "rt" => {
                let names: Vec<&str> = CloudRuntime::variants().iter().map(|r| r.as_str()).collect();
...
impl SandboxCmd {
    fn status(&self, want_json: bool) -> CommandOutput {
        let current = global_sandbox().lock().unwrap_or_else(|e| e.into_inner()).mode();
