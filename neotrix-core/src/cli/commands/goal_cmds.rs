...
impl CliCommand for GoalCmd {
    ...
    fn description(&self) -> &str {
        "Autonomous goal pursuit: /goal <desc> | status | list | pause | resume | cancel | history"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        if args.is_empty() || (args.len() == 1 && args[0] == "--json") {
            ...
            return if want_json {
                out.with_json(serde_json::json!({"subcommands": ["<desc>", "status", "list", "pause", "resume", "cancel", "history"]}))
            } else {
                out
            };
        }
        ...
        match cmd {
                ...
                if args.len() >= 2 {
                    ...
                    if let Some(ref g) = gl.active_goal {
                        if g.id == *id {
                            ...
                            return if want_json {
                                out.with_json(serde_json::json!({"goal_id": id, "state": format!("{:?}", g.state)}))
                            } else {
                                out
                            };
                        }
                    ...
                    if let Some(g) = gl.completed_goals.iter().find(|g| g.id == *id) {
                        ...
                        let out =
                            CommandOutput::ok(&format!("Goal '{}' ({}): {}", desc, id, state_str));
                        return if want_json {
                            out.with_json(serde_json::json!({"goal_id": id, "state": state_str, "description": desc}))
                        } else {
                            out
                        };
                    }
            ...
            "pause" => {
                ...
                if gl.active_goal.as_ref().map(|g| g.id == id).unwrap_or(false) {
                    ...
                    if want_json {
                        out.with_json(serde_json::json!({"action": "pause", "goal_id": id}))
                    } else {
                        out
                    }
                } else {
            ...
            "resume" => {
                ...
                if gl.active_goal.as_ref().map(|g| g.id == id).unwrap_or(false) {
                    ...
                    if want_json {
                        out.with_json(serde_json::json!({"action": "resume", "goal_id": id}))
                    } else {
                        out
                    }
                } else {
            ...
            "cancel" | "clear" => {
                ...
                if gl.active_goal.as_ref().map(|g| g.id == id).unwrap_or(false) {
                    ...
                    if want_json {
                        out.with_json(serde_json::json!({"action": "cancel", "goal_id": id}))
                    } else {
                        out
                    }
                } else {
            ...
            "history" => {
                ...
                if want_json {
                    ...
                    out.with_json(
                        serde_json::json!({"completed_goals": goals, "count": goals.len()}),
                    )
                } else {
                    out
                }
            ...
            _ => {
                ...
                let mut tracker =
                    GoalTracker::new(id.clone(), description.clone(), GoalConfig::default());
                ...
                if want_json {
                    out.with_json(
                        serde_json::json!({"goal": description, "id": id, "state": "pursuing"}),
                    )
                } else {
                    out
                }
