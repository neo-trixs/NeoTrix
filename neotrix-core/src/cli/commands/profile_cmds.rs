...
impl CliCommand for ProfileCmd {
    ...
    fn aliases(&self) -> Vec<&str> { vec!["/prof"] }
    ...
    fn execute(&self, args: &[String], _brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        match cmd {
            ...
            "show" | "info" => {
                let name = if plain_args.len() > 1 { plain_args[1] } else { &active_profile_name() };
                match get_profile_info(name) {
                    Ok(info) => {
                        let active_flag = if name == active_profile_name() { " (active)" } else { "" };
                        let mut msg = format!("Profile: {}{}\n", info["name"].as_str().unwrap_or(name), active_flag);
                        ...
                        let out = CommandOutput::ok(&msg);
                        if want_json { out.with_json(info) } else { out }
                    }
                    Err(e) => CommandOutput::err(&e),
                }
            }
            "create" => {
                ...
                let parent_idx = plain_args.iter().position(|a| *a == "--parent" || *a == "-p");
            ...
            "set" => {
                if plain_args.len() < 4 {
                    return CommandOutput::err("Usage: /profile set <profile> <action> <allow|deny|ask>");
                }
            ...
            _ => CommandOutput::err(&format!("Unknown subcommand: {}. Available: list, switch, show, create, rm, set", cmd)),
        }
    }
}
