...
impl CliCommand for SwapCmd {
    ...
    fn execute(
        ...
        with_crypto(|crypto| {
            ...
            let gas_price = client.get_gas_price().unwrap_or(10.0);
            ...
            if execute {
                ...
                let client_clone = match crypto.evm_clients.get(&chain) {
                    Some(c) => c,
                    None => return CommandOutput::err(&format!("❌ 无法连接到 {chain}")),
                };
                ...
                lines.push("\n💡 添加 --exec 或 --send 来执行交换".to_string());
            }
