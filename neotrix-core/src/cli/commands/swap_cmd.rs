    ...
    fn execute(
        ...
        if token_out.is_empty() {
            return CommandOutput::err(
                "❌ 缺少输出代币。Usage: /swap <chain> <token_in> <token_out> <amount>",
            );
        }
        ...
        with_crypto(|crypto| {
            let wallet = match crypto.wallet_manager.active_wallet() {
                ...
                None => {
                    return CommandOutput::err(
                        "❌ 没有活跃钱包。使用 /wallet create 或 /wallet import",
                    )
                }
            };
            ...
            if execute {
                ...
                let client_clone = crypto
                    .evm_clients
                    .get(&chain)
                    .expect("evm client for chain must exist");
        ...
        with_crypto(|crypto| {
            ...
            let (contract_addr, data, gas_limit, note) = match token.to_lowercase().as_str() {
                "native" | "eth" | "bnb" | "matic" | "avax" => (
                    ...
                    vec![],
                    21000u64,
                    format!("发送 {amount_str} {}", chain.native_currency()),
                ),
                token_addr if token_addr.starts_with("0x") => {
                    ...
                    if let Some(info) = reg.get(&chain, token_addr) {
                        ...
                        (
                            ...
                            data,
                            80000u64,
                            format!("发送 {} {}", amount_str, info.symbol),
                        )
                    } else {
                        let data = TxBuilder::encode_erc20_transfer(token_addr, to_addr, amount).1;
                        (
                ...
                sym => {
                    ...
                    let found = reg
                        .all_tokens()
                        .iter()
                        .find(|t| t.symbol.to_lowercase() == sym);
                    match found {
                        Some(info) => {
                            let data =
                                TxBuilder::encode_erc20_transfer(&info.address, to_addr, amount).1;
                            (
                                info.address.clone(),
                                data,
                                80000u64,
