...
impl CliCommand for ConfigCmd {
    ...
    fn execute(
        ...
        if args.is_empty() || (args.len() == 1 && args[0] == "--json") {
            ...
            return if want_json {
                let parsed: serde_json::Value = config_str
                    .parse()
                    .unwrap_or(serde_json::json!({"note": "parse failed"}));
        ...
        match sub {
            ...
            "set" => {
                ...
                if config_str.lines().any(|l| l.trim().starts_with(&key_line)) {
                    // Replace existing
                    let mut new_lines: Vec<String> = config_str
                        .lines()
                        .map(|l| {
                ...
                match std::fs::write(&tmp, &config_str) {
                    Ok(()) => match std::fs::rename(&tmp, &config_path) {
                        ...
                        Err(re) => CommandOutput::err(&format!("Failed to rename config: {}", re)),
                    },
                    Err(e) => CommandOutput::err(&format!("Failed to write config: {}", e)),
                }
            }
            _ => CommandOutput::err(&format!(
                "Unknown subcommand: {}. Available: show, set",
                sub
            )),
        }
    }
...
impl CliCommand for HelpCmd {
    ...
    fn execute(
        ...
        if args.len() >= 1 && args[0] != "--json" {
            let cmd_name = if args[0].starts_with('/') {
                &args[0]
            } else {
                args[0].as_str()
            };
            return match cmd_name {
                ...
                "/stats" => {
                    CommandOutput::ok("Show reasoning statistics: capabilities, iterations, memory")
                }
                "/cost" => {
                    CommandOutput::ok("Track token usage and costs. Sub: detail, budget, reset")
                }
                ...
                "/profile" => CommandOutput::ok(
                    "Permission profiles. Sub: list, switch, show, create, rm, set",
                ),
                "/clear" => CommandOutput::ok("Clear the terminal screen"),
                "/version" => CommandOutput::ok("Show version and build info"),
                "/completions" => {
                    CommandOutput::ok("Generate shell completions (bash|zsh|fish|powershell)")
                }
                ...
                "/approval" => CommandOutput::ok(
                    "Approval mode: suggest|auto-edit|full-auto. Sub: status, list, approve, deny",
                ),
                ...
                "/session" => CommandOutput::ok(
                    "Session management. Sub: list, switch, delete, fork, export, import",
                ),
        ...
        for (n, d) in &[
            ...
            ("/stats", "Reasoning statistics"),
            ("/cost", "Cost tracking"),
            ("/save", "Save brain state"),
            ("/absorb", "Absorb knowledge"),
            ("/evolve", "Self-improvement evolution loop"),
            ("/mem", "Memory nt_world_browse"),
            ("/agent", "Sub-agent management"),
            ("/mcp", "MCP server management"),
            ("/profile", "Permission profiles"),
            ("/clear", "Clear screen"),
...
impl CliCommand for StatsCmd {
    ...
    fn execute(
        ...
        if let Some(b) = brain {
            ...
            let msg = format!(
                "Capabilities: {:.3} | Iterations: {} | Absorb: {} | Memory: {}",
                stats.capability_sum,
                a.iteration,
                a.brain.total_absorb_count,
    ...
    fn execute(
        ...
        match shell {
            ...
            "fish" => {
                ...
                for cmd in &cmds {
                    s.push_str(&format!(
                        "complete -c neotrix -a '{}' -d '{} command'\n",
                        cmd, cmd
                    ));
                }
