...
use crate::neotrix::nt_mind_background_loop::always_on::{ScheduleExpr, ALWAYS_ON_ENGINE};
        ...
        match subcmd {
            ...
            "list" => {
                ...
                for t in &tasks {
                    ...
                    let next_run = t
                        .last_run
                        .and_then(|lr| t.schedule.as_ref().map(|s| s.next_run(lr).to_rfc3339()))
            ...
            _ => CommandOutput::err("Usage: /schedule list|add|remove|pause|resume"),
        }
    }
}
