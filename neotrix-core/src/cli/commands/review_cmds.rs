//! Code review command — staged / unstaged / file / all / audit
...
impl CliCommand for ReviewCmd {
    ...
    fn aliases(&self) -> Vec<&str> { vec!["/r", "/code-review", "/audit"] }
    fn description(&self) -> &str {
        "Code review: staged|unstaged|file <path>|all|audit [--full] [--mode security,testing,...]"
    }
    fn execute(&self, args: &[String], _brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        match subcmd {
            ...
            "all" => self.review_all(args.contains(&"--full".to_string())),
            "help" => CommandOutput::ok("Usage: /review staged|unstaged|file <path>|all|audit [--full] [--mode security,testing,...]"),
            _ => CommandOutput::err("Usage: /review staged|unstaged|file <path>|all|audit [--full] [--mode security,testing,...]"),
        }
    }
}
...
impl ReviewCmd {
    fn review_staged(&self) -> CommandOutput {
        let output = Command::new("git").args(["diff", "--cached", "--no-color"]).output();
        match output {
            Ok(out) => {
    ...
    fn review_all(&self, _full: bool) -> CommandOutput {
        let staged = Command::new("git").args(["diff", "--cached", "--no-color"]).output();
    ...
    fn analyze_content(&self, content: &str, label: &str) -> String {
        ...
        let nt_shield_patterns: &[(&str, &str)] = &[
            (r#"(?i)(api[_\-]?key|secret|token|password|passwd)\s*[:=]\s*(['"]?[A-Za-z0-9_\-]{16,})"#, "Possible hardcoded secret/key"),
            (r"(?i)AKIA[0-9A-Z]{16}", "Possible AWS Access Key"),
            (r"(?i)sk-[a-zA-Z0-9_\-]{32,}", "Possible OpenAI API key"),
            (r"(?i)(ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{36}", "Possible GitHub token"),
            (r"\bunsafe\b", "Usage of `unsafe` block — verify safety invariants"),
            (r"format!\s*\(\s*[^)]*\b(String|str)\b.*\)", "Potential format string injection risk"),
            (r"(?i)cmd\.(Run|Exec|Output|CombinedOutput).*\$\{", "Possible command injection via shell expansion"),
        ];
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if trimmed.starts_with("fn ") && trimmed.contains('(') && trimmed.contains(')') && trimmed.contains('{') {
        ...
        for &(start, size) in &func_lines {
            if size > 50 {
                quality_issues.push(format!("  L{}: Large function ({} lines, threshold 50) — consider refactoring", start + 1, size));
            }
        }
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if trimmed.ends_with(".unwrap()") || trimmed.ends_with(".unwrap();") {
                quality_issues.push(format!("  L{}: `.unwrap()` — prefer proper error handling with `?` or match", i + 1));
            }
        }
        ...
        if max_indent > 40 {
            quality_issues.push(format!("  Deep nesting detected (max indent: {} spaces) — consider flattening", max_indent));
        }

        // --- Style checks ---
        for (i, line) in lines.iter().enumerate() {
            if line.len() > 100 && line_is_code(i) {
                style_issues.push(format!("  L{}: Line too long ({} chars > 100)", i + 1, line.len()));
            }
        }
        ...
        for line in &lines {
            let trimmed = line.trim();
            if trimmed.starts_with("// ") && (trimmed.contains("fn ") || trimmed.contains("struct ") || trimmed.contains("impl ") || trimmed.contains("if ") || trimmed.contains("for ") || trimmed.contains("while ")) {
                commented_lines += 1;
            }
        }
        if commented_lines > 3 {
            style_issues.push(format!("  Found {} lines of commented-out code — clean up before commit", commented_lines));
        }
    ...
    /// Run the full 25-dimension L7 audit on the project
    fn run_audit(&self, args: &[String]) -> CommandOutput {
        let path = args.get(1).map(|s| s.as_str()).unwrap_or(".");
        let mode_flag = args.iter().position(|a| a == "--mode")
            .and_then(|i| args.get(i + 1));

        let mode = if let Some(mode_str) = mode_flag {
            let dims: Vec<AuditDimension> = mode_str.split(',')
                .filter_map(|s| match s.trim() {
                    "security" => Some(AuditDimension::Security),
                    "stability" => Some(AuditDimension::Stability),
                    "testing" => Some(AuditDimension::Testing),
                    "architecture" => Some(AuditDimension::Architecture),
                    "performance" => Some(AuditDimension::Performance),
                    "maintainability" => Some(AuditDimension::Maintainability),
                    "design" => Some(AuditDimension::Design),
                    "release" => Some(AuditDimension::Release),
                    "documentation" => Some(AuditDimension::Documentation),
                    "configuration" => Some(AuditDimension::Configuration),
                    "observability" => Some(AuditDimension::Observability),
                    "data-integrity" => Some(AuditDimension::DataIntegrity),
                    "privacy" => Some(AuditDimension::Privacy),
                    "accessibility" => Some(AuditDimension::Accessibility),
                    "supply-chain" => Some(AuditDimension::SupplyChain),
                    "cost" => Some(AuditDimension::Cost),
                    "ai-safety" => Some(AuditDimension::AiSafety),
                    "fallback" => Some(AuditDimension::Fallback),
                    "testing-authenticity" => Some(AuditDimension::TestingAuthenticity),
                    "type-safety" => Some(AuditDimension::TypeSafety),
                    "frontend-state" => Some(AuditDimension::FrontendState),
                    "backend-api" => Some(AuditDimension::BackendApi),
                    "dependency-weight" => Some(AuditDimension::DependencyWeight),
                    "code-consistency" => Some(AuditDimension::CodeConsistency),
                    "comment-coverage" => Some(AuditDimension::CommentCoverage),
                    _ => None,
                })
                .collect();
            if dims.is_empty() {
                return CommandOutput::err(&format!("Unknown audit mode '{}'. Use: security,stability,testing,architecture,performance,maintainability,design,release,documentation,configuration,observability,data-integrity,privacy,accessibility,supply-chain,cost,ai-safety,fallback,testing-authenticity,type-safety,frontend-state,backend-api,dependency-weight,code-consistency,comment-coverage", mode_flag.map_or("", |v| v)));
            }
            AuditMode::Focused(dims)
        } else {
            AuditMode::Full
        };

        let engine = CodeReviewEngine::with_mode(mode).with_linters();
        let sources = engine.load_sources(std::path::Path::new(path));

        if sources.is_empty() {
            return CommandOutput::warn("No source files found — check the project path");
        }

        let report = engine.audit("project", path, &sources);
        let md_report = engine.generate_markdown_report(&report);

        CommandOutput::ok(&md_report)
    }
}
...
mod tests {
    ...
    fn test_analyze_content_detects_unwrap() {
        ...
        assert!(result.contains("looks good") || result.contains("Minor") || result.contains("Needs attention"));
    }

    #[test]
    fn test_analyze_content_detects_api_key() {
        let cmd = ReviewCmd;
        let result = cmd.analyze_content("api_key = \"sk-abcdefghijklmnopqrstuvwxyz1234567890\"\n", "test");
