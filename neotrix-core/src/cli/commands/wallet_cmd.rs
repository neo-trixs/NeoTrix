...
impl CliCommand for WalletCmd {
    ...
    fn execute(
        ...
        match plain[0] {
            "create" | "new" => {
                let label = plain.get(1).copied().unwrap_or("default");
                let result = with_crypto(|crypto| {
                    match crypto.persist_wallet(label) {
                        Ok(label) => {
                            let w = match crypto.wallet_manager.active_wallet() {
                                Some(w) => w,
                                None => return CommandOutput::err("钱包创建后未能获取活动钱包"),
                            };
            ...
            "balance" | "bal" | "b" => {
                let chain_name = plain.get(1).copied();
                with_crypto(|crypto| {
            ...
            "export" | "e" => {
                let label = plain.get(1).copied().unwrap_or("");
                if label.is_empty() {
