    ...
    fn execute(
        ...
        match plain[0] {
            "create" | "new" => {
                ...
                let result = with_crypto(|crypto| match crypto.persist_wallet(label) {
                    Ok(label) => {
                        let w = crypto
                            .wallet_manager
                            .active_wallet()
                            .expect("active wallet must exist after persist");
                        CommandOutput::ok(&format!(
                            "✅ 创建钱包成功\n   Label: {}\n   Address: {}\n   📁 Saved to: {:?}",
                            label,
                            w.address_short(),
                            crypto.wallet_store.dir_path()
                        ))
            ...
            "list" | "ls" | "l" => with_crypto(|crypto| {
                ...
                if want_json {
                    let json_list: Vec<serde_json::Value> = wallets
                        .iter()
                        .map(|w| {
                            serde_json::json!({
            ...
            "export" | "e" => {
                ...
                with_crypto(|crypto| match crypto.wallet_store.load_wallet(label) {
                    Ok(w) => {
                        let msg = format!(
                            "⚠️  安全警告: 私钥可控制你的全部资产, 请勿泄露!\n\n🔑 {} 私钥:\n{}",
                            w.label,
