//! File operation commands — Read / Write / Create / Edit / Patch / Diff
...
use crate::cli::approval::{global_approval, ActionType};
...
impl CliCommand for FileReadCmd {
    ...
    fn description(&self) -> &str {
        "Read and display file contents: /read <path>"
    }
    fn execute(
        &self,
        args: &[String],
...
impl CliCommand for FileWriteCmd {
    ...
    fn description(&self) -> &str {
        "Write to a file: /write <path> <content> [--yes]"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        if path.exists() {
            return CommandOutput::warn(&format!(
                "⚠️ 文件已存在: {}. 如需覆盖请先执行 /read 查看, 或 /create 创建新文件",
                path_str
            ));
        }
        let content = if clean_args.len() > 1 {
            clean_args[1..].join(" ")
        } else {
            String::new()
        };
        let preview = if content.len() > 80 {
            format!("{}… ({} 字节)", &content[..77], content.len())
        } else {
            content.clone()
        };
        if let Some(block) = check_file_approval(
            args,
        ...
        match fs::write(path, &content) {
            Ok(()) => {
                CommandOutput::ok(&format!("✅ 已写入 {} ({} 字节)", path_str, content.len()))
            }
...
impl CliCommand for FileCreateCmd {
    ...
    fn description(&self) -> &str {
        "Create a new file: /create <path> [content] [--yes]"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        match fs::write(path, &content) {
            Ok(()) => {
                CommandOutput::ok(&format!("✅ 已创建 {} ({} 字节)", path_str, content.len()))
            }
...
impl CliCommand for FileEditCmd {
    ...
    fn description(&self) -> &str {
        "Edit a file: /edit <path> [<line>:<content>] [--yes]"
    }
    fn execute(
        &self,
        args: &[String],
...
impl CliCommand for FilePatchCmd {
    ...
    fn description(&self) -> &str {
        "Apply a unified diff patch: /patch <path> (reads from stdin)"
    }
    fn execute(
        &self,
        args: &[String],
        ...
        if args.is_empty() {
            return CommandOutput::err(
                "用法: /patch <path> (从 stdin 读取 patch, 在 TUI 中不可用)",
            );
        }
        ...
        CommandOutput::ok(&format!(
            "📋 /patch 需要从 stdin 读取 patch 数据。请使用 /edit 进行行编辑。"
        ))
    }
}
...
impl CliCommand for FileDiffCmd {
    ...
    fn description(&self) -> &str {
        "Show git diff for a file: /diff <path>"
    }
    fn execute(
        &self,
        args: &[String],
