...
impl CliCommand for CompactCmd {
    ...
    fn description(&self) -> &str { "Compress context window: /compact [now]" }
    ...
    fn execute(&self, args: &[String], brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        if args.is_empty() {
            if let Some(b) = brain {
                let a = b.blocking_read();
                let mem_count = a.reasoning_bank.memories().len();
                let model = std::env::var("NEOTRIX_MODEL").unwrap_or_else(|_| "default".to_string());
                let msg = format!(
                    "📋 上下文使用:\n  消息数: {} (sessions)\n  记忆: {} 条\n  模型: {}\n  窗口限制: 128K tokens\n  当前估计: ~{} tokens",
            ...
            match args[0].as_str() {
                ...
                "model" => {
                    if args.len() < 2 {
                        ...
                        CommandOutput::ok(&format!("📋 模型切换建议: {}. 请在配置文件中修改 default_model。", args[1]))
                    }
                }
...
impl CliCommand for SessionCmd {
    ...
    fn description(&self) -> &str { "Session management: /session list | save | load | delete | fork | export | import | share" }
    fn execute(&self, args: &[String], brain: Option<&Arc<RwLock<SelfIteratingBrain>>>) -> CommandOutput {
        ...
        match cmd {
            ...
            "load" => {
                ...
                match store.load_session(&name) {
                    Ok(data) => {
                        CommandOutput::ok(&format!("📂 会话 '{}' 已加载 (创建于 {})", data.name, data.created_at))
                    }
            ...
            "export" => {
                ...
                if let Some(idx) = output_idx {
                    ...
                    match store.export_to_file(&name, &path) {
                        Ok(()) => CommandOutput::ok(&format!("📤 会话 '{}' 已导出到 {}", name, path)),
                        Err(e) => CommandOutput::err(&format!("导出失败: {}", e)),
                    }
                } else {
                    match store.export_to_json(&name) {
                        Ok(json) => CommandOutput::ok(&format!("📤 会话 '{}' 导出:\n{}", name, json)),
                        Err(e) => CommandOutput::err(&format!("导出失败: {}", e)),
                    }
                }
            }
            ...
            "share" => {
                ...
                let ttl_hours = ttl_idx.and_then(|idx| {
                    args.get(idx + 1).and_then(|s| s.parse::<u64>().ok())
                });
...
impl CliCommand for ForkCmd {
    ...
    fn aliases(&self) -> Vec<&str> { vec![] }
    fn description(&self) -> &str { "Fork the current session: /fork [name]" }
        ...
        match cmd {
            ...
            _ => {
                ...
                let count = std::fs::read_to_string(&path)
                    ...
                    .map(|v| v.len())
                    .unwrap_or(0);
                let out = CommandOutput::ok(&format!("📋 命令历史: {} 条记录\n使用 Ctrl+R 搜索历史 /history clear 清空", count));
                if want_json { out.with_json(serde_json::json!({"count": count})) } else { out }
            }
        }
    }
}
