...
/// 环境诊断(合并进 /stats): 版本/OS/Git/配置/Brain/KB/Provider/Disk/Terminal/Memory
pub fn run_doctor() -> CommandOutput {
    ...
    let hostname = Command::new("hostname").output().ok()
    ...
    let config_path = dirs::config_dir()
        .map(|p| p.join("neotrix").join("config.toml"));
    if let Some(ref path) = config_path {
        if path.exists() {
            report.push_str(&format!("**Config**: ✅ {} ({} bytes)\n\n",
                path.display(),
                std::fs::metadata(path).map(|m| m.len()).unwrap_or(0)));
        } else {
            report.push_str(&format!("**Config**: ❌ not found at {}\n\n", path.display()));
            all_ok = false;
        }
    }
    ...
    if let Some(ref path) = kb_path {
        if path.exists() {
            report.push_str(&format!("**Knowledge DB**: ✅ {} ({} bytes)\n\n",
                path.display(),
                std::fs::metadata(path).map(|m| m.len()).unwrap_or(0)));
        } else {
    ...
    report.push_str(&format!("**Provider**: {} (API key: {})\n\n", provider, if has_key { "✅ configured" } else { "❌ missing" }));

    // 8. Daemon status
    let health_file = Path::new("/tmp/neotrix_daemon.health");
    ...
    if let Some(df_out) = df {
        ...
        if lines.len() > 1 {
            ...
            if parts.len() >= 4 {
                report.push_str(&format!("**Disk**: {} free of {} ({} used)\n\n",
    ...
    report.push_str(&format!("**Terminal**: {} colors, {} unicode\n\n",
        if std::env::var("TERM").unwrap_or_default().contains("256") { "256" } else { "basic" },
        if std::env::var("LANG").unwrap_or_default().contains("UTF") { "✅" } else { "⚠️" }));
