...
fn write_config_toml(cfg: &toml::Value) -> Result<(), String> {
    ...
    crate::core::nt_core_util::atomic_write_text(&path, &output)
...
/// # Safety
///
/// `std::env::set_var` is not thread-safe when called from concurrent async
/// tasks.  However, this helper is only called from `/model set` (interactive
/// CLI), which runs once per user command on the single TUI thread — two
/// concurrent calls are impossible in practice.
