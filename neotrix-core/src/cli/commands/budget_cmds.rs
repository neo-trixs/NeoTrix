    ...
    fn execute(
        ...
        match sub {
            "status" => {
                ...
                if want_json {
                    let cfg = tracker.budget.lock().unwrap_or_else(|e| e.into_inner());
            ...
            "set" => {
                if plain_args.len() < 3 {
                    return CommandOutput::err(
                        "用法: /budget set <session|daily|monthly> <amount>",
                    );
                }
                ...
                let tracker = COST_TRACKER.lock().unwrap_or_else(|e| e.into_inner());
                let mut cfg = tracker.budget.lock().unwrap_or_else(|e| e.into_inner());
                match tier {
                ...
                if want_json {
                    CommandOutput::ok(&msg)
                        .with_json(serde_json::json!({"tier": tier, "amount": amount}))
                } else {
                    CommandOutput::ok(&msg)
                }
            }
