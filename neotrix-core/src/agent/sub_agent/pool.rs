...
use super::execution::simulate_agent_execution;
...
pub struct SubAgentPool {
    ...
    event_rx: Arc<RwLock<Option<mpsc::UnboundedReceiver<SubAgentEvent>>>>,
}
...
impl SubAgentPool {
    ...
    pub fn event_receiver(&self) -> Option<mpsc::UnboundedReceiver<SubAgentEvent>> {
    ...
    pub async fn launch(
        ...
        let result_slot: Arc<RwLock<Option<SubAgentResult>>> =
        ...
        drop(tokio::spawn(async move {
            ...
            let sim_fut = tokio::task::spawn_blocking(move || {
                simulate_agent_execution(&prompt_owned, capabilities2.as_ref())
            });
            ...
            match timeout(deadline, sim_fut).await {
                Ok(Ok(Ok(mut result))) => {
                    ...
                    *result_slot.write().await = Some(result.clone());
                    let _ = event_tx.send(SubAgentEvent::Done {
                        id,
                ...
                Ok(Ok(Err(e))) => {
                    ...
                    let _ = event_tx.send(SubAgentEvent::Failed { id, error: "execution error".into() });
                }
                Ok(Err(join_err)) => {
                    if let Some(old_handle) = handles.write().await.get_mut(&id) {
                        old_handle.status = SubAgentStatus::Failed(join_err.to_string());
                    }
                    let _ = event_tx.send(SubAgentEvent::Failed { id, error: join_err.to_string() });
                }
                Err(_elapsed) => {
                    ...
                    let _ = event_tx.send(SubAgentEvent::TimedOut {
                        id,
                        elapsed_secs,
                    });
                }
    ...
    pub async fn launch_with_timeout(
        ...
        drop(tokio::spawn(async move {
            ...
            let sim_fut = tokio::task::spawn_blocking(move || {
                simulate_agent_execution(&prompt_owned, capabilities.as_ref())
            });
    ...
    pub async fn wait_for(&self, id: Uuid) -> Option<SubAgentResult> {
        ...
        loop {
            ...
            match handle {
                Some(h) => {
                    if matches!(h.status, SubAgentStatus::Completed | SubAgentStatus::Failed(_) | SubAgentStatus::TimedOut)
                    {
