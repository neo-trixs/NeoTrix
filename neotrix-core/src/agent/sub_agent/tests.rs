...
async fn test_cancel_operation() {
    ...
    pool.cancel_all();
    assert_eq!(pool.active_count(), 0, "active count should be 0 after cancel_all");
    assert!(
        pool.status(id2).is_none() || matches!(pool.status(id2), Some(SubAgentStatus::Failed(ref s)) if s == "cancelled by pool"),
...
async fn test_event_emission() {
    ...
    assert!(tag.starts_with("<subagent:done"), "tag should start with <subagent:done");
    assert!(tag.contains(&id.to_string()), "tag should contain sub-agent ID");
}
