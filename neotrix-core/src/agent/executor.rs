...
mod tests {
    ...
    fn test_agent_builder_with_sub_agent_pool() {
        let config = SubAgentConfig { max_concurrency: 5, ..Default::default() };
    ...
    fn test_agent_full_builder_chain() {
        let agent = Agent::new()
            .with_sub_agent_pool(SubAgentPool::new(SubAgentConfig { max_concurrency: 3, ..Default::default() }))
    ...
    fn test_agent_absorb_normalizes_capability() {
        ...
        let max_val = agent.brain.capability.arr.iter().cloned().fold(0.0f64, |acc, x| acc.max(x));
    ...
    fn test_agent_status_partial_eq() {
        ...
        assert_eq!(AgentStatus::Error("a".into()), AgentStatus::Error("a".into()));
        assert_ne!(AgentStatus::Error("a".into()), AgentStatus::Error("b".into()));
    }
}
