...
pub enum MemoryTier { L1, L2, L3, L4, L5 }
...
impl CognitiveMemory {
    pub fn new() -> Self {
        Self { entries: Vec::new(), cross_agent_agreement: HashMap::new() }
    }

    pub fn store(&mut self, content: &str, tier: MemoryTier, source_agent: Option<&str>, tags: Vec<String>) -> String {
        ...
        for entry in &mut self.entries {
            if entry.reward > promotion_threshold && entry.access_count >= 3 {
                let new_tier = match entry.tier {
                    MemoryTier::L1 => { promoted += 1; MemoryTier::L2 }
                    MemoryTier::L2 => { promoted += 1; MemoryTier::L3 }
                    MemoryTier::L3 => { promoted += 1; MemoryTier::L4 }
                    other => other,
                };
                entry.tier = new_tier;
            }
    ...
    pub fn active_consolidation(&mut self, staleness_threshold_secs: u64) -> (usize, usize, usize) {
        ...
        self.entries.retain(|e| {
            if e.tier == MemoryTier::L1 && now.saturating_sub(e.created_at) > staleness_threshold_secs {
                removed += 1;
                return false;
            }
            true
    ...
    pub fn flash_reasoning(&self, start_tag: &str, max_depth: usize) -> Vec<Vec<&MemoryEntry>> {
        ...
        let start: Vec<&MemoryEntry> = self.entries.iter()
        ...
        for entry in start {
            ...
            while depth < max_depth {
                let last = path.last().copied().expect("result");
                let next = self.entries.iter()
                    .filter(|e| e.tags.iter().any(|t| last.tags.contains(t)) && e.id != last.id)
                    .max_by(|a, b| a.reward.partial_cmp(&b.reward).expect("result"));
                if let Some(n) = next { path.push(n); depth += 1; } else { break; }
            }
            paths.push(path);
        }
        paths
...
mod tests {
    ...
    fn test_flash_reasoning() {
        let mut mem = CognitiveMemory::new();
        mem.store("alpha concept", MemoryTier::L3, None, vec!["math".into(), "alpha".into()]);
        mem.store("beta concept", MemoryTier::L3, None, vec!["math".into(), "beta".into()]);
        mem.store("gamma concept", MemoryTier::L3, None, vec!["science".into(), "gamma".into()]);
