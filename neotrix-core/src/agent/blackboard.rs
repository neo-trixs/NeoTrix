...
impl Blackboard {
    ...
    pub fn get_facts_by_source(&self, source: &str) -> Vec<&Fact> {
        self.facts
            .iter()
            .filter(|f| f.source_agent == source)
            .collect()
    }
    ...
    pub fn search_facts(&self, query: &str) -> Vec<&Fact> {
        ...
        self.facts
            .iter()
            .filter(|f| f.content.to_lowercase().contains(&lower))
            .collect()
    }
    ...
    pub fn get_pending_intents(&self) -> Vec<&Intent> {
        self.intents
            .iter()
            .filter(|i| i.status == IntentStatus::Proposed || i.status == IntentStatus::Claimed)
            .collect()
    }
    ...
    pub fn reachable_goal_estimate(&self, goal_tags: &[String]) -> f64 {
        ...
        for tag in goal_tags {
            ...
            let in_intents = self
                .intents
                .iter()
                .any(|i| i.description.to_lowercase().contains(&tag.to_lowercase()));
...
mod tests {
    ...
    fn test_add_and_retrieve_facts() {
        let mut bb = make_bb();
        let id = bb.add_fact(
            "port 443 is open on target",
            "scanner",
            0.95,
            vec!["network".into(), "port".into()],
        );
    ...
    fn test_search_facts_by_tag() {
        ...
        bb.add_fact(
            ...
            "ping",
            0.9,
            vec!["network".into(), "discovery".into()],
        );
        bb.add_fact(
            "ssl cert expired",
            "tls",
            1.0,
            vec!["ssl".into(), "security".into()],
        );
        bb.add_fact(
            "dns resolves",
            "dns",
            0.8,
            vec!["network".into(), "dns".into()],
        );
    ...
    fn test_supersede_intent_with_replacement() {
        ...
        let repl = bb
            .get_intent(&replacement.expect("replacement should be ok in test"))
            .expect("replacement should be ok in test");
