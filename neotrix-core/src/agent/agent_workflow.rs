...
impl AgentWorkflow {
    ...
    pub fn execute_with_hooks(&self, hooks: &HookRegistry) -> WorkflowResult {
        self.run_steps(Some(hooks))
    }

    fn run_steps(&self, hooks: Option<&HookRegistry>) -> WorkflowResult {
        ...
        for step in &self.steps {
            ...
            match step {
                ...
                AgentStep::CustomTool { tool, args } => {
                    match Self::execute_custom(tool, args) {
                        Ok(out) => {
                            final_output = Some(out);
                        }
                        Err(e) => {
...
pub fn planner_workflow() -> AgentWorkflow {
    ...
    wf.add_step(AgentStep::Think {
        thought: "Parsing task requirements and decomposing into actionable steps."
            .to_string(),
    });
    wf.add_step(AgentStep::EndTurn { result: None });
    wf
...
pub fn reviewer_workflow() -> AgentWorkflow {
    ...
    wf.add_step(AgentStep::Think {
        thought: "Evaluating changes for bugs, style issues, and regressions."
            .to_string(),
    });
...
mod tests {
    ...
    fn test_load_from_dir() {
        ...
        std::fs::write(dir.path().join("workflow.json"), wf_def).expect("failed to write workflow test file");

        let mut reg = WorkflowRegistry::new();
        let count = reg.load_from_dir(dir.path()).expect("load_from_dir should succeed");
        assert_eq!(count, 1);
    ...
    fn test_execute_read_file() {
        ...
        std::fs::write(&file_path, "file content").expect("failed to write sample file for read test");
