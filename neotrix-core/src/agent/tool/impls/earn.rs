...
use crate::neotrix::nt_act_earn::{default_registry, EarnAgent};

pub struct EarnTool {
    manifest: ToolManifest,
    agent: Option<EarnAgent>,
}
    ...
    pub fn new() -> Self {
        Self {
            manifest: ToolManifest {
                ...
                description:
                    "Run EarnAgent publish cycle — generate & publish content across platforms"
                        .into(),
...
impl AgentTool for EarnTool {
    ...
    fn start(&mut self, _api: ToolApi) -> Result<(), ToolError> {
        // Validate that the earn agent can be instantiated (dependencies check)
    ...
    fn execute(&self, ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let mode = args.get("mode").and_then(|v| v.as_str()).unwrap_or("cycle");
        ...
        let result = match mode {
            "validate" => {
                ...
                serde_json::json!({
                    ...
                    "brand_name": brand,
                    "count": count,
                    "valid": platform.is_some() || brand.is_some(),
                    "status": "validated",
                })
            }
            "stats" => {
                let json = exec_earn(&args).unwrap_or_else(|e| format!("{{\"error\": \"{}\"}}", e));
