...
impl ToolWatcher {
    ...
    pub fn scan_and_reload(&self) -> Vec<String> {
        ...
        for entry in entries.flatten() {
            ...
            let inserted = loaded.get(&manifest.id)
    ...
    pub fn check_for_update(&self, tool_id: &str, current_version: &str) -> Option<UpdateInfo> {
        let loaded = self.loaded.lock().expect("loaded mutex poisoned");
...
mod tests {
    ...
    fn test_check_for_update_matches() {
        ...
        assert!(info.is_some());
        assert_eq!(info.as_ref().expect("update info should be Some").new_version, "0.3.0");
        assert_eq!(info.expect("update info should be Some").name, "Update Tool");
