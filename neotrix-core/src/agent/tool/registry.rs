...
impl AgentToolRegistry {
    ...
    pub fn execute(&self, id: &str, args: &Value) -> Result<ToolOutput, ToolError> {
        let handler = self.handlers.get(id).ok_or_else(|| ToolError::Runtime {
            id: id.into(),
    ...
    pub fn register_agent_tool(&mut self, tool: Box<dyn AgentTool>) {
        ...
        let id = manifest.id.clone();
        let input_schema = Self::generate_schema(&manifest);
        let tool_arc: Arc<dyn AgentTool> = Arc::from(tool);
        let handler = {
            let t = tool_arc.clone();
            move |args: &Value| {
                t.execute(ToolContext {
...
mod tests {
    ...
    fn test_generate_schema() {
        let manifest = ToolManifest {
            ...
            mcp: None,
            min_runtime: "0.1.0".into(),
            description: "Scrape web pages".into(),
            author: None,
        };
