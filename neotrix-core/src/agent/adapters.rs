...
use crate::core::{
    AgentExecutor, BrainProvider, CapabilityVector, EngineProvider, KnowledgeSource,
    MemoryProvider, RichMemoryProvider, SealResult,
};
...
impl MemoryProvider for ReasoningBank {
    fn store(&mut self, key: &str, value: &str) -> Result<String, String> {
        ...
        let mem = ReasoningMemory::new(&format!("{}: {}", key, value), TaskType::General, &[], 0.5);
        self.store(mem);
        Ok(key.to_string())
    }
...
impl RichMemoryProvider for ReasoningBank {
    ...
    fn recall_similar(
        ...
        query: &str,
        limit: usize,
    ) -> Vec<crate::core::nt_core_bank::ReasoningMemory> {
        self.memories()
            .iter()
            .filter(|m| m.task_description.contains(query))
            .take(limit)
...
impl BrainProvider for ReasoningBrain {
    ...
    ) -> crate::core::nt_core_knowledge::AbsorptionRecord {
        self.absorb(source);
        self.absorption_history.last().cloned().unwrap_or(
            crate::core::nt_core_knowledge::AbsorptionRecord {
                source,
                timestamp: 0,
...
impl EngineProvider for ReasoningEngine {
    fn reason(&mut self, prompt: &str) -> Result<String, String> {
        ReasoningEngine::reason(self, prompt).map_err(|e: NeoTrixError| e.to_string())
    }
}
...
mod tests {
    ...
    fn test_memory_provider_store_and_search() {
        ...
        let id = MemoryProvider::store(&mut bank, "test_key", "test_value")
            .expect("value should be ok in test");
        assert_eq!(id, "test_key");
        let results =
            MemoryProvider::search(&bank, "test_value", 10).expect("value should be ok in test");
    ...
    fn test_memory_provider_delete() {
        let mut bank = ReasoningBank::new(100);
        let id = MemoryProvider::store(&mut bank, "del_key", "del_value")
            .expect("value should be ok in test");
...
impl AgentExecutor for SelfIteratingBrain {
    ...
    fn execute(&mut self, task: &str) -> Result<String, String> {
        ...
        if let Some(ref mut engine) = self.reasoning_engine {
            crate::neotrix::nt_mind::reasoning_engine::ReasoningEngine::reason(engine, task)
                .map_err(|e: NeoTrixError| e.to_string())
        } else {
    ...
    fn status(&self) -> String {
        ...
        format!(
            "迭代:{} 吸收:{} 能力:{:.3} 记忆:{}",
            self.iteration,
            self.brain.total_absorb_count,
            stats.capability_sum,
            self.reasoning_bank.memories().len()
        )
    }
