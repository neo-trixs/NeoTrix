...
impl ExperiencePool {
    ...
    pub fn retrieve_high_value(&self, min_reward: f64, k: usize) -> Vec<&ExperienceTrace> {
        let mut high: Vec<&ExperienceTrace> = self
            ...
            .collect();
        high.sort_by(|a, b| b.total_reward.partial_cmp(&a.total_reward).expect("ordering on non-NaN values"));
        high.truncate(k);
        high
    }
...
mod tests {
    ...
    fn test_store_and_retrieve() {
        ...
        let seg = make_segment("s1", "initial", "move_left", "moved", 0.5, "navigation", 1000);
