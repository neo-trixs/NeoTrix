...
pub enum AgentBusMessage {
    ...
    WorkerStatus { worker_id: String, busy: bool, capabilities: Vec<String> },
    Heartbeat { agent_id: String, timestamp: Instant },
...
impl AgentBus {
        ...
        if let Ok(mut stats) = self.stats.write() {
            match &msg {
                ...
                AgentBusMessage::TaskCompleted(r) => {
                    if r.success { stats.tasks_completed += 1; } else { stats.tasks_failed += 1; }
                }
                AgentBusMessage::WorkerStatus { .. } => { stats.active_workers += 1; }
                _ => {}
            }
        }
    }
    ...
    pub fn subscribe(&self, agent_id: &str, topic: BusTopic) {
        if let Ok(mut subs) = self.subscriptions.write() {
            subs.entry(topic).or_default().push(agent_id.to_string());
        }
    ...
    pub fn dispatch_by_efc(&self, topic: &str, payload: &str, required_capability: &str) -> Result<(), String> {
        let workers = self.workers.read().map_err(|e| format!("lock error: {}", e))?;
            ...
            if let Some(caps) = workers.get(id) {
                if caps.iter().any(|c| c == required_capability) {
                    self.publish(
                        ...
                        AgentBusMessage::Custom { topic: topic.to_string(), payload: payload.to_string() },
                    );
                    return Ok(());
                }
            }
        }
        Err(format!("no worker found with capability: {}", required_capability))
    }
    ...
    pub fn all_workers(&self) -> Vec<String> {
        self.workers.read().map(|w| w.keys().cloned().collect()).unwrap_or_default()
    }

    pub fn worker_capabilities(&self, worker_id: &str) -> Option<Vec<String>> {
        self.workers.read().ok().and_then(|w| w.get(worker_id).cloned())
    }

    pub fn stats(&self) -> BusStats {
        self.stats.read().as_ref().map(|s| BusStats {
    ...
    pub fn clear_old_messages(&self, max_age: Duration) {
        if let Ok(mut msgs) = self.messages.write() {
            msgs.retain(|(_, m)| match m {
                AgentBusMessage::Heartbeat { timestamp, .. } => timestamp.elapsed() < max_age,
                _ => true,
            });
        }
    }
...
impl SupervisorAgent {
    pub fn new(id: &str, bus: Arc<AgentBus>) -> Self {
        ...
        Self { id: id.to_string(), bus, workers: HashMap::new(), worker_efc: HashMap::new() }
    }
    ...
    pub fn update_worker_efc(&mut self, worker_id: &str, avg_efficiency: f64) {
        self.worker_efc.insert(worker_id.to_string(), avg_efficiency);
    }
    ...
    pub fn dispatch_task(&self, description: &str) -> BusTask {
        ...
        self.bus.publish(BusTopic::TaskRequest, AgentBusMessage::TaskAvailable(task.clone()));
        task
    }

    pub fn dispatch_task_with_capabilities(&self, description: &str, capabilities: &[&str]) -> BusTask {
    ...
    pub fn broadcast(&self, directive: &str) {
        self.bus.publish(BusTopic::SupervisorBroadcast, AgentBusMessage::SupervisorDirective(directive.to_string()));
    }

    pub fn poll_results(&self) -> Vec<TaskResult> {
        self.bus.poll_topic(&BusTopic::TaskResult)
            .into_iter()
    ...
    pub fn poll_claims(&self) -> Vec<TaskClaim> {
        self.bus.poll_topic(&BusTopic::TaskClaim)
            .into_iter()
            .filter_map(|m| match m {
...
impl WorkerAgent {
    ...
    pub fn claim_task(&mut self, task: &BusTask) -> bool {
        ...
        let has_all_caps = task.required_capabilities.iter()
        ...
        self.bus.publish(BusTopic::TaskClaim, AgentBusMessage::TaskClaimed(claim));
        true
    }
    ...
    pub fn complete_task(&mut self, success: bool, output: &str, error: Option<&str>) {
        if let Some(task_id) = self.current_task.take() {
            ...
            self.bus.publish(BusTopic::TaskResult, AgentBusMessage::TaskCompleted(result));
        }
    }
    ...
    pub fn send_heartbeat(&self) {
        self.bus.publish(
            ...
            AgentBusMessage::Heartbeat { agent_id: self.id.clone(), timestamp: Instant::now() },
        );
    }
    ...
    pub fn is_busy(&self) -> bool { self.current_task.is_some() }
...
mod tests {
    ...
    fn test_bus_publish_and_poll() {
        ...
        bus.publish(BusTopic::TaskRequest, AgentBusMessage::TaskAvailable(task.clone()));
    ...
    fn test_bus_stats() {
        ...
        bus.publish(BusTopic::TaskResult, AgentBusMessage::TaskCompleted(
            TaskResult { task_id: 0, worker_id: "w1".into(), success: true, output: "ok".into(), duration_ms: 10, error: None }
        ));
        bus.publish(BusTopic::TaskResult, AgentBusMessage::TaskCompleted(
            TaskResult { task_id: 1, worker_id: "w1".into(), success: false, output: "".into(), duration_ms: 5, error: Some("fail".into()) }
        ));
    ...
    fn test_multiple_workers_and_worker_selection() {
        ...
        bus.register_worker("fullstack", vec!["code".into(), "review".into(), "analysis".into()]);
    ...
    fn test_custom_topic_messages() {
        ...
        bus.publish(
            ...
            AgentBusMessage::Custom { topic: "log".into(), payload: "info message".into() },
        );
    ...
    fn test_dispatch_with_capabilities() {
        let bus = Arc::new(AgentBus::new());
        let supervisor = SupervisorAgent::new("sup-1", bus.clone());
        let task = supervisor.dispatch_task_with_capabilities("audit security", &["security", "review"]);
