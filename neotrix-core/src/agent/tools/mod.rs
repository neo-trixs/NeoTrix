...
impl McpRegistry {
    ...
    pub fn register(&mut self, server: McpServer) {
        for tool in &server.tools {
            self.tool_index
                .insert(tool.name.clone(), self.servers.len());
        }
    ...
    pub fn find_tool(&self, name: &str) -> Option<&McpToolDef> {
        self.tool_index.get(name).and_then(|&idx| {
            self.servers
                .get(idx)
                .and_then(|s| s.tools.iter().find(|t| t.name == name))
        })
    }
    ...
    pub async fn health_check(&mut self) -> Vec<(&str, bool)> {
        ...
        for server in &mut self.servers {
            let ok = match &server.transport {
                McpTransport::Http { url, .. } => reqwest::get(url).await.is_ok(),
    ...
    pub fn search(&self, query: &str) -> Vec<&McpToolDef> {
        ...
        if q.is_empty() {
            let mut seen_servers: std::collections::HashSet<usize> =
    ...
    pub fn call_tool(&self, name: &str, args: &serde_json::Value) -> Result<String, String> {
        ...
        if let Ok(shield) = crate::cli::shield_enforcer::global_shield().lock() {
            if let Err(decision) =
                shield.check_all(&format!("mcp_tool:{}", name), "tool_exec", None, None)
            {
                let msg = match decision {
                    crate::cli::ShieldDecision::Block(m) => {
                        format!("Shield blocked MCP tool '{}': {}", name, m)
                    }
        ...
        match &server.transport {
            ...
            McpTransport::WebSocket { .. } | McpTransport::Sse { .. } => Err(format!(
                "Transport '{}' not yet implemented for tool calling",
...
impl McpTransport {
    ...
    pub fn is_available(&self) -> bool {
        match self {
            McpTransport::Stdio { command, .. } => std::process::Command::new("which")
                .arg(command)
                .output()
...
impl McpToolGenerator {
    pub fn new() -> Self {
        Self {
            project_types: vec!["rust".into(), "python".into(), "node".into()],
        }
    }
    ...
    fn extract_rust_tools(&self, fpath: &std::path::Path, content: &str) -> Vec<McpToolDef> {
        ...
        for line in content.lines() {
            ...
            if trimmed.contains("#[derive(Parser)")
                || trimmed.contains("#[derive(Args)")
                ...
                for entry in entries.flatten() {
                    ...
                    if fpath.extension().is_some_and(|e| e == "py") {
                        if let Ok(content) = std::fs::read_to_string(&fpath) {
                            for line in content.lines() {
                                let t = line.trim();
                                if t.starts_with("@app.route")
                                    || t.starts_with("@router.")
...
mod tests {
    ...
    fn test_register_http() {
        ...
        reg.register_http(
            ...
            vec![make_tool_for("api_call", "Make API call", "api")],
        );
        assert_eq!(reg.server_count(), 1);
        let tool = reg
            .find_tool("api_call")
            .expect("find_tool for registered api_call should succeed");
    ...
    fn test_recommend_no_match() {
        ...
        reg.register_stdio(
            ...
            "cmd",
            &[],
            vec![make_tool("image_gen", "Generate images")],
        );
    ...
    fn test_extract_rust_tools_axum() {
        ...
        assert!(tools
            .iter()
            .any(|t| t.name.contains("api") || t.name.contains("cli")));
    }
    ...
    fn test_generate_from_dir_rust_proj() {
        ...
        let tools = gen.generate_from_dir(
            tmp.to_str()
                .expect("tmp path should be valid utf-8 for rust-proj test"),
        );
        assert!(!tools.is_empty());
        let _ = std::fs::remove_dir_all(&tmp);
    }
    ...
    fn test_tool_provider_list_tools() {
        ...
        reg.register_stdio(
            ...
            "echo",
            &[],
            vec![make_tool("tool_a", "Tool A"), make_tool("tool_b", "Tool B")],
        );
    ...
    fn test_tool_provider_call_tool_builtin() {
        let mut reg = McpRegistry::new();
        reg.register_builtin("echo_test", |args| Ok(format!("echo: {}", args)));
        let result =
            ToolProvider::call_tool(&reg, "echo_test", &serde_json::json!({"msg": "hello"}));
        assert!(result.is_ok());
    ...
    fn test_search_case_insensitive() {
        ...
        reg.register_stdio(
            ...
            "echo",
            &[],
            vec![make_tool_for("WebSearch", "PUBLIC search", "alpha")],
        );
