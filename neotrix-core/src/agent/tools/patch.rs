...
pub fn apply_patch(edit: &PatchEdit, dry_run: bool) -> PatchReport {
    ...
    if count > 1 {
        return PatchReport {
            file_path: edit.file_path.clone(),
            success: false,
            error: Some(format!("Found {} occurrences of old_string. Expected exactly 1.", count)),
            lines_changed: 0,
        };
    }
    ...
    let bak_path = path.with_extension(format!("{}.bak", path.extension().and_then(|e| e.to_str()).unwrap_or("")));
...
pub fn patch_handler(args: &serde_json::Value) -> Result<String, String> {
    let edits: Vec<PatchEdit> = serde_json::from_value(args.get("edits").ok_or_else(|| "Missing required field: edits".to_string())?.clone())
        .map_err(|e| format!("Invalid edits format: {}", e))?;

    let dry_run = args.get("dry_run").and_then(|v| v.as_bool()).unwrap_or(false);

    let reports: Vec<PatchReport> = edits.iter().map(|edit| apply_patch(edit, dry_run)).collect();
...
mod tests {
    ...
    fn test_file_not_found() {
        ...
        let report = apply_patch(&edit, false);
        assert!(!report.success);
        assert!(report.error.expect("error should be Some").contains("not found"));
    }
    ...
    fn test_multiple_occurrences() {
        ...
        let report = apply_patch(&edit, false);
        assert!(!report.success);
        assert!(report.error.expect("error should be Some").contains("3 occurrences"));
    }
    ...
    fn test_binary_file_rejection() {
        ...
        let report = apply_patch(&edit, false);
        assert!(!report.success);
        assert!(report.error.expect("error should be Some for binary").contains("Binary"));
    }
    ...
    fn test_readonly_file_rejection() {
        ...
        let mut perm = std::fs::metadata(&path).expect("read metadata").permissions();
    ...
    fn test_multiline_replace() {
        let (_dir, path) = setup_test_file("fn old_func() {\n    let x = 1;\n    println!(\"{}\", x);\n}\n");
        let edit = PatchEdit {
            file_path: path.clone(),
