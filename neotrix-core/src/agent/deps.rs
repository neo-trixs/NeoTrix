...
use crate::core::{RichMemoryProvider, BrainProvider, EngineProvider, CapabilityVector};
...
impl AgentDeps {
    pub fn new() -> Self {
        Self { memory: None, brain: None, engine: None }
    }
...
mod tests {
    ...
    impl RichMemoryProvider for MockMemoryProvider {
        fn store_memory(&mut self, _memory: ReasoningMemory) -> bool { true }
        fn recall_similar(&self, _query: &str, _limit: usize) -> Vec<ReasoningMemory> { vec![] }
        fn stats(&self) -> crate::core::nt_core_bank::ReasoningBankStats {
            crate::core::nt_core_bank::ReasoningBankStats { total_memories: 0, success_count: 0, success_rate: 0.0 }
        }
    }

    struct MockBrainProvider;
    impl BrainProvider for MockBrainProvider {
        fn capability_vector(&self) -> CapabilityVector { CapabilityVector::default() }
        fn absorb_knowledge(&mut self, source: KnowledgeSource) -> AbsorptionRecord {
            AbsorptionRecord { source, timestamp: 0, weight: 1.0 }
        }
        fn run_seal_iteration(&mut self) -> SealResult {
            SealResult { score_before: 0.5, score_after: 0.6, delta: 0.1, iterations: 1 }
        }
    }

    struct MockEngineProvider;
    impl EngineProvider for MockEngineProvider {
        fn reason(&mut self, _prompt: &str) -> Result<String, String> { Ok("ok".into()) }
    }
}
