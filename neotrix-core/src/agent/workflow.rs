...
impl WorkflowEngine {
    ...
    fn execute_step(&mut self, step: &WorkflowStep, _context: &str) -> WorkflowResult {
        match step {
            ...
            WorkflowStep::Parallel { name, steps } => {
                let results: Vec<WorkflowResult> = steps
                    .iter()
            ...
            WorkflowStep::Loop {
                ...
                WorkflowResult {
                    ...
                    output: format!(
                        "循环 {} 次后: {}",
                        last_result.iteration, last_result.output
                    ),
            ...
            WorkflowStep::Repeat {
                ...
                for i in 0..*max_iterations {
                    ...
                    if last_result.success
                        && (i + 1) as f64 / *max_iterations as f64 >= *quality_threshold
                    {
                        break;
                    }
                }
    ...
    pub fn run(&mut self, name: &str, context: &str) -> Vec<WorkflowResult> {
        ...
        let results: Vec<WorkflowResult> = wf
            .steps
            .iter()
    ...
    pub fn from_yaml(yaml: &str) -> Result<Self, String> {
        fn parse_step_list(
            ...
            start: usize,
            outer_indent: usize,
        ) -> Result<(Vec<WorkflowStep>, usize), String> {
            ...
            steps: vec![WorkflowStep::Route {
                ...
                true_branch: Box::new(WorkflowStep::Parallel {
                    ...
                    steps: vec![
                        WorkflowStep::AgentTask {
                            name: "w1".to_string(),
                            task_description: "Work 1".to_string(),
                        },
                        WorkflowStep::AgentTask {
                            name: "w2".to_string(),
                            task_description: "Work 2".to_string(),
                        },
                    ],
                }),
                false_branch: Box::new(WorkflowStep::AgentTask {
        ...
        match &wf.steps[1] {
            WorkflowStep::Parallel { name, steps } => {
                ...
                match &steps[0] {
                    ...
                    step => {
                        return Err(format!(
                            "Expected AgentTask inside parallel, got {:?}",
                            step
                        ))
                    }
                }
