...
impl SkillDiscovery {
    ...
    pub fn discover_local(&mut self) -> Vec<String> {
        ...
        for path in &self.search_paths {
            let expanded = path.replace("~", &std::env::var("HOME").unwrap_or_else(|_| ".".to_string()));
                        ...
                        if fpath.is_dir() {
                            ...
                            if skill_file.exists() {
                                if let Some(name) = fpath.file_name().and_then(|n| n.to_str()) {
                                    ...
                                    self.skills.insert(name.to_string(), Skill::new(
                                        meta,
    ...
    pub fn discover_github(&mut self, owner: &str, repo: &str, path: &str, branch: Option<&str>) -> Result<String, String> {
        ...
        match reqwest::blocking::get(&url) {
            Ok(response) => {
                ...
                let meta = Self::parse_frontmatter(&content);
                let system_prompt = Self::extract_system_prompt(&content);
                self.skills.insert(name.clone(), Skill::new(
                    meta,
                    SkillSource::GitHub {
    ...
    /// 从 ECC 社区注册表发现 Skill
    pub fn discover_ecc_community(&mut self, skill_id: &str, version: &str) -> Result<String, String> {
        ...
        match reqwest::blocking::get(&url) {
            Ok(response) => {
                ...
                self.skills.insert(name.clone(), Skill::new(meta,
                    SkillSource::EccCommunity { skill_id: skill_id.to_string(), version: version.to_string() },
                ...
                if let Some(idx) = line.find(':') {
                    ...
                    match key.as_str() {
                        ...
                        "requires_tools" | "requires-tools" => {
                            meta.requires_tools = val.split(',').map(|s| s.trim().to_string()).collect();
                        }
                        "requires_capabilities" | "requires-capabilities" => {
                            meta.requires_capabilities = val.split(',').map(|s| s.trim().to_string()).collect();
                        }
                        "mitre_attack" | "mitre" | "technique" => {
                            meta.mitre_attack_ids = val.split(',').map(|s| s.trim().to_string()).collect();
                        }
                        _ => {}
                    }
                }
    ...
    pub(crate) fn extract_system_prompt(content: &str) -> String {
        ...
        content.trim().lines().take(10).collect::<Vec<_>>().join("\n")
    }
...
mod tests {
    ...
    fn test_skill_discovery_add_skill() {
        ...
        let meta = SkillMeta {
            name: "my-skill".into(), description: "desc".into(), version: "1.0".into(),
            author: None, origin: None, triggers: vec![], condition: None,
            requires_tools: vec![], requires_capabilities: vec![], mitre_attack_ids: vec![],
        };
        let skill = Skill::new(meta, SkillSource::LocalDir("./".into()), "content".into(), "prompt".into());
        d.add_skill(skill);
        assert_eq!(d.len(), 1);
    }
...
Second line"#;
    ...
    fn test_skill_injector_inject_with_skills() {
        let meta = SkillMeta {
            name: "test".into(), description: "Test skill".into(), version: "1.0".into(),
            author: None, origin: None, triggers: vec![], condition: None,
            requires_tools: vec![], requires_capabilities: vec![], mitre_attack_ids: vec![],
        };
        let skill = Skill::new(meta, SkillSource::LocalDir("./".into()), "content".into(), "system prompt".into());
