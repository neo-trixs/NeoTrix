...
impl SkillActivator {
    ...
    /// 根据任务描述自动选择激活的技能
    pub fn activate_for_task<'a>(
        &mut self,
        task: &str,
        all_skills: &'a [&'a Skill],
    ) -> Vec<&'a Skill> {
        ...
        matched.sort_by(|a, b| {
            b.stats
                .confidence
                .partial_cmp(&a.stats.confidence)
        ...
        all_skills
            ...
            .filter(|s| {
                s.meta
                    .mitre_attack_ids
                    .iter()
                    .any(|id| technique_ids.contains(&id.as_str()))
            })
            .copied()
            .collect()
    }
...
impl SkillsEngine {
    ...
    fn calculate_keyword_match(&self, meta: &SkillMeta, task_vector: &CapabilityVector) -> f64 {
        ...
        let keywords = [
            "code", "design", "security", "test", "deploy", "data", "web", "api", "ui", "agent",
        ];
        let high_dims: Vec<&str> = task_vector
            .arr()
            .iter()
            .enumerate()
            .filter(|(_, &v)| v > 0.7)
            .map(|(i, _)| {
                crate::core::nt_core_cap::FIELD_NAMES
                    .get(i)
                    .copied()
                    .unwrap_or("unknown")
            })
        ...
        for dim in &high_dims {
            if meta.name.to_lowercase().contains(dim)
                || meta.description.to_lowercase().contains(dim)
            {
                score += 0.3;
            }
        }
...
mod tests {
    ...
    fn test_skill_activator_no_match() {
        ...
        let skill = Skill::new(
            SkillMeta {
                name: "nosuchskill".into(),
                description: "Niche skill".into(),
                version: "1.0".into(),
                author: None,
                origin: None,
                triggers: vec!["xyznonexistent".into()],
                condition: None,
                requires_tools: vec![],
                requires_capabilities: vec![],
                mitre_attack_ids: vec![],
