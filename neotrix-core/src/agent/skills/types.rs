...
impl Skill {
    pub fn new(
        ...
        Self {
            stats: SkillStats {
                use_count: 0,
                success_count: 0,
                confidence: if content.is_empty() || system_prompt.is_empty() {
                    0.0
                } else {
                    0.5
                },
    ...
    pub fn update_confidence(&mut self, success: bool, execution_ms: u64) {
        ...
        self.stats.avg_execution_ms = (self.stats.avg_execution_ms
            * (self.stats.use_count as f64 - 1.0)
            + execution_ms as f64)
            / self.stats.use_count as f64;
        let alpha = 0.3;
        let reward = if success { 1.0 } else { -0.5 };
        self.stats.confidence =
            (self.stats.confidence * (1.0 - alpha) + reward * alpha).clamp(0.0, 1.0);
    }
}
...
mod tests {
    ...
    fn test_skill_confidence_clamped() {
        ...
        let mut skill = Skill::new(
            meta,
            SkillSource::Url("https://x.com".into()),
            "c".into(),
            "p".into(),
        );
        for _ in 0..10 {
