...
impl Hook for SessionPersistenceHook {
    ...
    fn events(&self) -> Vec<HookEvent> { vec![HookEvent::SessionStart, HookEvent::SessionEnd, HookEvent::PreCompact] }
    fn execute(&self, ctx: &HookContext) -> HookAction {
        match ctx.event {
...
impl Hook for QualityGateHook {
    ...
    fn execute(&self, ctx: &HookContext) -> HookAction {
        if let Some(ref input) = ctx.tool_input {
            ...
            if lines > 800 {
                return HookAction::Block(format!("File exceeds 800 lines ({} lines). Split into smaller modules.", lines));
            }
        }
        HookAction::Continue
    }
    ...
    fn execute(&self, ctx: &HookContext) -> HookAction {
        if let Some(ref output) = ctx.tool_output {
            if output.contains("TODO") || output.contains("FIXME") || output.contains("HACK") {
                return HookAction::Warn("New TODO/FIXME/HACK found — consider creating an issue".into());
            }
        }
        HookAction::Continue
    }
...
impl Hook for SessionSummaryHook {
    fn name(&self) -> &'static str { "session-summary" }
    fn description(&self) -> &'static str { "在 Stop 事件时记录会话摘要" }
    fn events(&self) -> Vec<HookEvent> { vec![HookEvent::Stop] }
    fn is_async(&self) -> bool { true }
...
impl HookRegistry {
    ...
    pub fn execute_event(&self, ctx: &HookContext) -> Vec<HookAction> {
        ...
        if let Some(indices) = self.event_index.get(&ctx.event) {
            for &idx in indices {
                ...
                if self.disabled_hooks.contains(&self.hooks[idx].name().to_string()) { continue; }
    ...
    pub fn list_hooks(&self) -> Vec<(&str, &str)> {
        self.hooks.iter().map(|h| (h.name(), h.description())).collect()
    }
}
...
mod tests {
    ...
    fn test_custom_hook() {
        ...
        impl Hook for TestHook {
            ...
            fn description(&self) -> &'static str { "test hook" }
            fn events(&self) -> Vec<HookEvent> { vec![HookEvent::Stop] }
            fn execute(&self, _: &HookContext) -> HookAction { HookAction::Warn("test".into()) }
        }
