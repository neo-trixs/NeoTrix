...
impl PlaybookEngine {
    ...
    pub fn all_playbooks(&self) -> &[Playbook] {
        &self.playbooks
    }
...
mod tests {
    ...
    fn test_extract_from_expert_example() {
        ...
        assert_eq!(
            engine
                ...
                .iter()
                .find(|p| p.id == id)
                .expect("playbook should exist after extraction")
                .confidence,
            0.7
        );
    }
    ...
    fn test_record_success_path() {
        ...
        assert!(matches[0]
            .instruction
            .contains("init -> validate -> execute"));
    }
    ...
    fn test_report_use_and_failure() {
        ...
        let initial_conf = engine
            .all_playbooks()
            .iter()
            .find(|p| p.id == id)
            .expect("playbook should exist before use")
            .confidence;
        engine.report_use(&id);
        let after_use = engine
            .all_playbooks()
            .iter()
            .find(|p| p.id == id)
            .expect("playbook should exist after use")
            .confidence;
        assert!(after_use > initial_conf);
        engine.report_failure(&id);
        let after_fail = engine
            .all_playbooks()
            .iter()
            .find(|p| p.id == id)
            .expect("playbook should exist after failure")
            .confidence;
