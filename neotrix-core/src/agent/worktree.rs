...
impl WorktreeManager {
    ...
    pub fn create(&mut self, name: &str, base_branch: &str) -> Result<Worktree, String> {
        ...
        if self.worktrees.len() >= MAX_WORKTREES {
            return Err(format!("max worktrees ({}) reached", MAX_WORKTREES));
        }
    ...
    pub fn run_in(&self, name: &str, command: &str, args: &[&str]) -> Result<String, String> {
        let wt = self
            .get(name)
        ...
        let out = Command::new(command)
            ...
            .current_dir(&wt.path)
            .output()
            .map_err(|e| format!("Failed to run '{}' in worktree '{}': {}", command, name, e))?;
        if out.status.success() {
...
mod tests {
    ...
    fn cleanup_stale_worktree(path: &Path, name: &str) {
        let stale = path.parent().map(|p| p.join(name)).unwrap_or_else(|| { log::warn!("worktree: path has no parent"); PathBuf::new() });
        if stale.exists() {
    ...
    fn test_create_from_branch() {
        ...
        cleanup_stale_worktree(&path, &wt_name);
        let wt = mgr
            .create_from_branch(&wt_name, &branch_name)
            .expect("failed to create worktree from branch");
    ...
    fn test_commit_and_diff() {
        ...
        mgr.commit(&wt_name, "add new_file.txt")
