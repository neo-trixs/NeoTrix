...
pub enum YieldedStep {
    ...
    SubAgent { agent_id: String, prompt: String, capabilities: Vec<String> },
    Return { value: String },
    Branch { condition: String, if_true: Vec<YieldedStep>, if_false: Vec<YieldedStep> },
    Loop { steps: Vec<YieldedStep>, max_iterations: usize },
    Parallel { steps: Vec<Vec<YieldedStep>> },
}
...
impl StepGenerator {
    pub fn new(max_steps: usize) -> Self {
        Self { steps: Vec::new(), position: 0, context: HashMap::new(), max_steps }
    }

    pub fn add_steps(&mut self, steps: Vec<YieldedStep>) { self.steps.extend(steps); }

    pub fn add_step(&mut self, step: YieldedStep) { self.steps.push(step); }
    ...
    pub fn get_context(&self, key: &str) -> Option<&String> { self.context.get(key) }

    pub fn reset(&mut self) { self.position = 0; }

    pub fn remaining(&self) -> usize { self.steps.len().saturating_sub(self.position) }

    pub fn len(&self) -> usize { self.steps.len() }

    pub fn is_empty(&self) -> bool { self.steps.is_empty() }

    pub fn is_done(&self) -> bool { self.position >= self.steps.len() }

    pub fn file_picker(pattern: &str) -> Self {
        let mut g = Self::new(10);
        g.add_steps(vec![
            YieldedStep::Thought { content: format!("Finding files matching {}", pattern) },
            YieldedStep::ToolCall { tool: "glob".into(), args: HashMap::from([("pattern".into(), pattern.into())]) },
            YieldedStep::Return { value: "files".into() },
        ]);
        g
    }
    ...
    pub fn planner(task: &str) -> Self {
        ...
        g.add_steps(vec![
            YieldedStep::Thought { content: format!("Planning steps for: {}", task) },
            YieldedStep::ToolCall { tool: "analyze".into(), args: HashMap::from([("task".into(), task.into())]) },
            YieldedStep::SubAgent { agent_id: "file-picker".into(), prompt: "find relevant files".into(), capabilities: vec!["code_search".into()] },
            YieldedStep::Return { value: "plan".into() },
        ]);
        g
    }
    ...
    pub fn editor(file_path: &str, content: &str) -> Self {
        let mut g = Self::new(10);
        g.add_steps(vec![
            YieldedStep::Thought { content: format!("Editing {}", file_path) },
            YieldedStep::ToolCall { tool: "read_file".into(), args: HashMap::from([("path".into(), file_path.into())]) },
            YieldedStep::ToolCall { tool: "edit_file".into(), args: HashMap::from([("path".into(), file_path.into()), ("content".into(), content.into())]) },
            YieldedStep::Return { value: file_path.into() },
        ]);
        g
    }
    ...
    pub fn reviewer(file_path: &str) -> Self {
        ...
        g.add_steps(vec![
            YieldedStep::Thought { content: format!("Reviewing {}", file_path) },
            YieldedStep::ToolCall { tool: "read_file".into(), args: HashMap::from([("path".into(), file_path.into())]) },
            YieldedStep::ToolCall { tool: "code_review".into(), args: HashMap::from([("path".into(), file_path.into())]) },
            YieldedStep::Branch {
                condition: "has_issues".into(),
                if_true: vec![
                    YieldedStep::ToolCall { tool: "report".into(), args: HashMap::from([("severity".into(), "warning".into())]) },
                ],
                if_false: vec![
                    YieldedStep::Thought { content: "No issues found".into() },
                ],
            },
            YieldedStep::Return { value: "review_done".into() },
        ]);
        g
    }
}
...
mod tests {
    ...
    fn test_basic_step_generation() {
        ...
        gen.add_step(YieldedStep::Thought { content: "step1".into() });
        gen.add_step(YieldedStep::Thought { content: "step2".into() });
        gen.add_step(YieldedStep::Return { value: "done".into() });
        assert_eq!(gen.len(), 3);
    ...
    fn test_peek_next_remaining_done() {
        let mut gen = StepGenerator::new(10);
        gen.add_step(YieldedStep::Thought { content: "peek me".into() });
        assert!(!gen.is_done());
    ...
    fn test_branch_step_structure() -> Result<(), String> {
        ...
        let (condition, if_true, if_false) = match branch {
            Some(YieldedStep::Branch { condition, if_true, if_false }) => (condition, if_true, if_false),
