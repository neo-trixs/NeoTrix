...
impl BanditScorer {
    pub fn new(exploration_constant: f64) -> Self {
        Self { arms: HashMap::new(), total_plays: 0, exploration_constant }
    }
    ...
    pub fn score(&self, name: &str) -> f64 {
        match self.arms.get(name) {
            Some(&(reward, count)) if count > 0 => {
                ...
                let explore = self.exploration_constant * (self.total_plays as f64).ln().sqrt() / count as f64;
                exploit + explore
            }
            _ => f64::MAX,
        }
    ...
    pub fn total_plays(&self) -> usize { self.total_plays }

    pub fn confidence_weighted_score(&self, name: &str) -> f64 {
        match self.arms.get(name) {
            Some(&(reward, count)) if count >= 3 => {
                reward / count as f64
            }
            Some(&(reward, count)) if count > 0 => {
                let exploit = reward / count as f64;
...
impl RewardModel {
    pub fn new(dim: usize) -> Self {
        Self { weights: vec![0.1; dim], bias: 0.0 }
    }
...
impl StalenessDetector {
    pub fn new(decay_rate: f64, age_threshold_secs: u64) -> Self {
        Self { decay_rate, age_threshold_secs }
    }
