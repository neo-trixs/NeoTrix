...
impl AgentTeam {
    ...
    fn reason(&self, prompt: &str) -> String {
        if let Some(ref engine) = self.engine {
            match engine.lock() {
                Ok(mut eng) => eng
                    .reason(prompt)
                    .unwrap_or_else(|e| format!("[Engine error: {}]", e)),
    ...
    fn run_swarm_devils_advocate(&mut self, task: &str) -> Vec<AgentResult> {
        ...
        let challenger = if available > 1 {
            &self.agents[1]
        } else {
            &self.agents[0]
        };
        let resolver = if available > 2 {
            &self.agents[2]
        } else {
            &self.agents[0]
        };
    ...
    pub fn run_debate(&mut self, task: &str) -> Vec<AgentResult> {
        ...
        let all_args: Vec<String> = results
            .iter()
            .map(|r| format!("[{}]: {}", r.agent_name, r.output))
            .collect();
    ...
    pub fn resolve_conflict(&self, outputs: &[AgentResult], task: &str) -> AgentResult {
        ...
        if let Some(ref engine) = self.engine {
            ...
            if let Ok(mut eng) = engine.lock() {
                let resolution = eng
                    .reason(&prompt)
                    .unwrap_or_else(|_| "Resolution failed".to_string());
                return AgentResult {
                    agent_name: self.name.clone(),
                    output: resolution,
                    success: true,
    ...
    pub fn merge_results(&self, results: &[AgentResult], task: &str) -> AgentResult {
        ...
        if let Some(ref engine) = self.engine {
            ...
            if let Ok(mut eng) = engine.lock() {
                let merged = eng
                    .reason(&prompt)
                    .unwrap_or_else(|_| "Merge failed".to_string());
                return AgentResult {
                    agent_name: self.name.clone(),
                    output: merged,
                    success: true,
