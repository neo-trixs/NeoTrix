...
mod tests {
    use super::*;
    use crate::l1_action::nt_memory::nt_memory_historian::nt_evidence_store::EvidenceStore;

    fn make_store() -> Arc<Mutex<EvidenceStore>> {
        let tmp = std::env::temp_dir().join(format!("neotrix_fg_{}.db", rand::random::<u64>()));
        let _ = std::fs::remove_file(&tmp);
        let kb = crate::l1_action::nt_memory::nt_memory_kb::KnowledgeBase::open(Some(tmp.clone().into()))
