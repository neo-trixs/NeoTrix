...
use crate::l1_action::nt_memory::nt_memory_historian::{TemporalFact, TemporalFactLedger};
...
vsa_expander: RwLock::new(VsaAssociativeExpander::default()),
    ...
    fn recency_rerank(&self, results: Vec<SearchResult>) -> Vec<SearchResult> {
        let now = crate::l1_action::nt_memory::nt_memory_kb::nt_memory_diversity::now_unix_secs();
    ...
    pub fn search_permission_aware(
        ...
        query: &str,
        limit: usize,
        permission: crate::l1_action::nt_memory::nt_memory_kb::nt_memory_types::PermissionLevel,
    ) -> Result<Vec<SearchResult>, String> {
        ...
        use crate::l1_action::nt_memory::nt_memory_kb::nt_memory_adaptive_rag::RelevanceGrade;
        ...
        let filtered: Vec<SearchResult> = scored
            .into_iter()
            .filter(|r| {
                let sensitivity = crate::l1_action::nt_memory::nt_memory_kb::nt_memory_types::node_sensitivity(&r.node.node_type);
        ...
        for (id, sim) in scored.into_iter().take(limit) {
            if let Ok(Some(node)) = nt_memory_store::get_node(&conn, &id) {
                results.push(SearchResult {
                    node,
                    score: sim,
                    matched_on: vec![crate::l1_action::nt_memory::nt_memory_kb::nt_memory_types::SearchMatchType::VectorSimilarity],
                    signals: None,
                });
            }
        }
...
impl crate::core::nt_core_traits::MemoryProvider for KnowledgeBase {
    ...
    fn delete(&mut self, key: &str) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock: {}", e))?;
        let node = crate::l1_action::nt_memory::nt_memory_kb::nt_memory_store::find_node_by_title_and_type(
            &conn,
            key,
            &crate::l1_action::nt_memory::nt_memory_kb::nt_memory_types::NodeType::Insight,
