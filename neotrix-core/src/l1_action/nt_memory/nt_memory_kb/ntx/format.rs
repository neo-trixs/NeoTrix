...
impl NtxHeader {
    ...
    pub fn from_bytes(buf: &[u8; HEADER_SIZE]) -> std::io::Result<Self> {
        ...
        Ok(Self {
            ...
            feature_flags: buf[6],
            compression: buf[7],
            footer_offset: u64::from_le_bytes(buf[8..16].try_into().unwrap()),
            wal_offset: u64::from_le_bytes(buf[16..24].try_into().unwrap()),
            wal_size: u64::from_le_bytes(buf[24..32].try_into().unwrap()),
            wal_checkpoint_pos: u64::from_le_bytes(buf[32..40].try_into().unwrap()),
            wal_sequence: u64::from_le_bytes(buf[40..48].try_into().unwrap()),
            frame_count: u64::from_le_bytes(buf[48..56].try_into().unwrap()),
            node_count: u64::from_le_bytes(buf[56..64].try_into().unwrap()),
            edge_count: u64::from_le_bytes(buf[64..72].try_into().unwrap()),
            toc_checksum: buf[72..104].try_into().unwrap(),
            schema_hash: buf[104..136].try_into().unwrap(),
            _reserved: [0u8; 3960],
        })
    }
