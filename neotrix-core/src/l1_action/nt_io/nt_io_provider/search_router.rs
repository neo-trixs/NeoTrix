...
impl Default for SearchConfig {
    fn default() -> Self { Self { extra_sources: 0, fallback: true } }
}
...
impl SearchRouter {
    pub fn new() -> Self {
        Self { slots: Vec::new(), capabilities: HashMap::new(), config: SearchConfig::default() }
    }

    pub fn register(&mut self, name: &str, capability: SearchCapability) {
        let idx = self.slots.len();
        let priority = self.slots.iter().filter(|s| s.capability == capability).count() as u32 + 1;
        self.capabilities.entry(capability).or_default().push(idx);
        self.slots.push(ProviderSlot { name: name.to_string(), capability, priority });
    }
    ...
    pub fn detect_intent(&self, task: &str) -> SearchIntent {
        ...
        SearchIntent {
            needs_docs: l.contains("api") || l.contains("documentation")
                || l.contains("docs") || l.contains("sdk") || l.contains("reference"),
            is_temporal: l.contains("news") || l.contains("today") || l.contains("latest")
                || l.contains("current") || l.contains("update"),
        }
    }
    ...
    pub fn route(&self, task: &str) -> RoutingRecord {
        ...
        if let Some(indices) = self.capabilities.get(&SearchCapability::MainSearch) {
            ...
            if indices.len() > 1 { record.fallback_used = true; }
        }
    ...
    pub fn check_minimum_profile(&self) -> Vec<SearchCapability> {
        let mut m = Vec::new();
        for c in &[SearchCapability::MainSearch, SearchCapability::DocsSearch, SearchCapability::WebFetch] {
    ...
    pub fn stats(&self) -> HashMap<&'static str, usize> {
        ...
        for c in &[SearchCapability::MainSearch, SearchCapability::DocsSearch, SearchCapability::WebFetch] {
            s.insert(c.as_str(), self.capabilities.get(c).map(|v| v.len()).unwrap_or(0));
        }
        s
    }

    pub fn provider_count(&self) -> usize { self.slots.len() }
}
...
mod tests {
    ...
    fn test_stats_format() {
        ...
        let stats = router.stats();
        assert_eq!(*stats.get("main_search").expect("value should be ok in test"), 1);
        assert_eq!(*stats.get("docs_search").expect("value should be ok in test"), 1);
        assert_eq!(*stats.get("web_fetch").expect("value should be ok in test"), 1);
    }
}
