...
pub async fn create_gateway_async() -> GatewayV2 {
    ...
    if !probe_llamacpp().await {
        ...
        match manager.auto_start().await {
            ...
            Err(e) => {
                log::warn!("[gateway] llama.cpp auto-start failed: {} (will try to connect anyway)", e);
            }
        }
    }
    // Register llama.cpp as OpenAI-compatible local provider (Trusted)
    if probe_llamacpp().await {
        let llamacpp = create_provider(ProviderConfig {
            provider_type: LlmProviderType::Vllm,  // OpenAI-compatible API
            base_url: Some("http://127.0.0.1:8080/v1".to_string()),
            ..Default::default()
        });
        gateway.register_provider_with_category("llamacpp", llamacpp, true, ProviderCategory::Local);
        log::info!("[gateway] Auto-registered: llamacpp (local, flash_attn=1, kv=q4_0, mlock)");
    }
