...
use crate::l1_action::nt_io::nt_io_provider::common::factory::{LlmProviderType, ProviderConfig};
...
pub const BUILTIN_FREE_PROVIDERS: &[(&str, &str, &str, LlmProviderType, &str)] = &[
    ...
    ("anthropic", "claude-haiku-3.5", "https://api.anthropic.com/v1", LlmProviderType::Anthropic, "t1-standard"),
    ("gemini", "gemini-2.0-flash", "https://generativelanguage.googleapis.com/v1beta", LlmProviderType::Gemini, "t2-balanced"),
    ("gemini", "gemini-2.0-flash-lite", "https://generativelanguage.googleapis.com/v1beta", LlmProviderType::Gemini, "t0-cheap"),
    ("ollama", "llama3.2", "http://localhost:11434/v1", LlmProviderType::Ollama, "t0-cheap"),
    ("ollama", "qwen2.5", "http://localhost:11434/v1", LlmProviderType::Ollama, "t1-standard"),
];

/// 环境变量到 Provider 映射
const ENV_VAR_MAP: &[(&str, &str, LlmProviderType, &str, bool)] = &[
    ("OPENAI_API_KEY", "openai", LlmProviderType::OpenAI, "https://api.openai.com/v1", true),
    ("ANTHROPIC_API_KEY", "anthropic", LlmProviderType::Anthropic, "https://api.anthropic.com/v1", true),
    ("GOOGLE_API_KEY", "gemini", LlmProviderType::Gemini, "https://generativelanguage.googleapis.com/v1beta", true),
    ("GROQ_API_KEY", "groq", LlmProviderType::OpenAI, "https://api.groq.com/openai/v1", true),
    ("DEEPSEEK_API_KEY", "deepseek", LlmProviderType::OpenAI, "https://api.deepseek.com/v1", true),
    ("CLOUDFLARE_API_TOKEN", "cloudflare", LlmProviderType::Cloudflare, "https://api.cloudflare.com/client/v4/ai", true),
    ("NVIDIA_API_KEY", "nvidia", LlmProviderType::Nvidia, "https://integrate.api.nvidia.com/v1", true),
    ("GITHUB_TOKEN", "github-models", LlmProviderType::GitHubModels, "https://models.inference.ai.azure.com/v1", true),
    ("HF_API_KEY", "huggingface", LlmProviderType::HuggingFace, "https://api-inference.huggingface.co/v1", true),
    ("COHERE_API_KEY", "cohere", LlmProviderType::Cohere, "https://api.cohere.ai/v1", true),
...
impl ModelDiscovery {
    ...
    pub fn discover_local_free_models() -> Vec<DiscoveredModel> {
        ...
        entries.into_iter().map(|e| {
            let tier = if e.tier.is_empty() { "t1-standard".into() } else { e.tier.clone() };
            DiscoveredModel {
    ...
    pub fn discover_from_env() -> Vec<DiscoveredModel> {
        ...
        for (env_var, provider_id, provider_type, base_url, requires_key) in ENV_VAR_MAP {
            if std::env::var(env_var).is_ok() {
                let models_for_provider = match *provider_id {
                    "openai" => vec![
                        Self::make_model(provider_id, "gpt-4o-mini", "GPT-4o Mini", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t1-standard"),
                        Self::make_model(provider_id, "gpt-4o", "GPT-4o", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t3-powerful"),
                    ],
                    "anthropic" => vec![
                        Self::make_model(provider_id, "claude-sonnet-4", "Claude Sonnet 4", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t4-frontier"),
                        Self::make_model(provider_id, "claude-haiku-3.5", "Claude Haiku 3.5", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t1-standard"),
                    ],
                    "gemini" => vec![
                        Self::make_model(provider_id, "gemini-2.0-flash", "Gemini 2.0 Flash", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t2-balanced"),
                        Self::make_model(provider_id, "gemini-2.0-flash-lite", "Gemini 2.0 Flash Lite", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t0-cheap"),
                    ],
                    "groq" => vec![
                        Self::make_model(provider_id, "mixtral-8x7b-32768", "Mixtral 8x7B", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t2-balanced"),
                        Self::make_model(provider_id, "llama-3.3-70b-versatile", "Llama 3.3 70B", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t3-powerful"),
                        Self::make_model(provider_id, "deepseek-r1-distill-llama-70b", "DeepSeek R1 Distill", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t3-powerful"),
                    ],
                    "deepseek" => vec![
                        Self::make_model(provider_id, "deepseek-chat", "DeepSeek V3", *provider_type, base_url, true, Some(env_var), ModelSource::EnvVar, "t3-powerful"),
                    ],
                    _ => vec![Self::make_model(provider_id, "default", provider_id, *provider_type, base_url, *requires_key, Some(env_var), ModelSource::EnvVar, "t1-standard")],
                };
        ...
        if std::env::var("NEOTRIX_API_KEY").is_ok() {
            let provider_name = std::env::var("NEOTRIX_PROVIDER").unwrap_or_else(|_| "custom".to_string());
            let base_url = std::env::var("NEOTRIX_BASE_URL").unwrap_or_else(|_| "https://api.openai.com/v1".to_string());
            let model_name = std::env::var("NEOTRIX_MODEL").unwrap_or_else(|_| "gpt-4o-mini".to_string());
            models.push(Self::make_model(
                &provider_name, &model_name, &format!("{} ({})", model_name, provider_name),
                LlmProviderType::OpenAI, &base_url, true, Some("NEOTRIX_API_KEY"),
                ModelSource::EnvVar, "t2-balanced",
            ));
        }
        models
    }
    ...
    pub fn discover_builtin_free() -> Vec<DiscoveredModel> {
        ...
        // 内置 Provider 注册
        for &(provider_id, model_id, base_url, provider_type, tier) in BUILTIN_FREE_PROVIDERS {
            let (display_name, requires_api_key, api_key_env) = match provider_id {
                "ollama" => (format!("{provider_id}/{model_id} (Local)"), false, None),
                _ => {
                    let env = match provider_id {
                        "openai" => Some("OPENAI_API_KEY"),
                        "anthropic" => Some("ANTHROPIC_API_KEY"),
                        "gemini" => Some("GOOGLE_API_KEY"),
                        _ => None,
                    };
                    (format!("{provider_id}/{model_id}"), true, env)
                }
            };
            models.push(Self::make_model(
                provider_id, model_id, &display_name,
                provider_type, base_url,
                requires_api_key, api_key_env,
                ModelSource::BuiltinFree, tier,
            ));
        }
    ...
    pub fn discover_local_endpoints() -> Vec<DiscoveredModel> {
        ...
        if let Ok(endpoint) = std::env::var("NEOTRIX_LOCAL_ENDPOINT") {
            models.push(Self::make_model(
                "local", "default", "Local Endpoint",
                LlmProviderType::OpenAI, &endpoint, false, None,
                ModelSource::LocalEndpoint, "t1-standard",
            ));
        }
    ...
    fn make_model(
        ...
        provider_type: LlmProviderType, base_url: &str,
        requires_api_key: bool, api_key_env: Option<&str>,
        source: ModelSource, tier: &str,
    ) -> DiscoveredModel {
        DiscoveredModel {
    ...
    pub async fn discover_dynamic(base_url: &str, api_key: Option<&str>) -> Vec<DiscoveredModel> {
        ...
        models.iter().filter_map(|m| {
            ...
            Some(DiscoveredModel {
                ...
                display_name: m.get("display_name").and_then(|v| v.as_str()).unwrap_or(id).to_string(),
                provider_type: LlmProviderType::OpenAI,
                base_url: base_url.to_string(),
...
impl ModelRegistry {
    ...
    pub fn models_by_provider(&self, provider_id: &str) -> Vec<&DiscoveredModel> {
        self.models.iter().filter(|m| m.provider_id == provider_id).collect()
    }

    /// 设置活跃模型
    pub fn set_active(&mut self, provider_id: &str, model_id: &str) {
        self.active_selections.insert(provider_id.to_string(), model_id.to_string());
    ...
    pub(crate) fn _active_config(&self) -> Option<ProviderConfig> {
        ...
        Some(ProviderConfig {
            ...
            api_key: model.api_key_env.as_ref().and_then(|env| std::env::var(env).ok()),
    ...
    pub fn stats(&self) -> HashMap<&'static str, usize> {
        ...
        s.insert("requires_key", self.models.iter().filter(|m| m.requires_api_key).count());
        s.insert("from_env", self.models.iter().filter(|m| m.source == ModelSource::EnvVar).count());
        s.insert("from_builtin", self.models.iter().filter(|m| m.source == ModelSource::BuiltinFree).count());
        s.insert("from_local", self.models.iter().filter(|m| m.source == ModelSource::LocalEndpoint).count());
        s
    }
}
