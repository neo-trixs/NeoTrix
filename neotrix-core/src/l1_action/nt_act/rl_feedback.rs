...
use crate::l1_action::nt_act_goal::behavioral_verifier::VerificationResult;
...
pub enum RewardEvent {
    ...
    CompileFailed { file: String, dim: String, errors: usize },
    TestFailed { file: String, dim: String, failures: usize },
}
...
impl RewardHistory {
    ...
    pub fn add_event(&mut self, event: RewardEvent) {
        ...
        if reward > 0.0 { self.positive_count += 1; } else { self.negative_count += 1; }
...
impl RLFeedbackLoop {
    ...
    /// 处理验证结果, 返回奖励值
    pub fn process_result(&mut self, file: &str, dim: &str, result: &VerificationResult) -> f64 {
        let mut total_reward = 0.0;

        if result.compile_ok {
            let ev = RewardEvent::CompileSuccess { file: file.to_string(), dim: dim.to_string() };
            total_reward += RewardHistory::reward_value(&ev);
            self.history.add_event(ev);
        } else {
            let ev = RewardEvent::CompileFailed {
                file: file.to_string(), dim: dim.to_string(),
        ...
        if result.tests_ok {
            let ev = RewardEvent::TestPassed { file: file.to_string(), dim: dim.to_string() };
            total_reward += RewardHistory::reward_value(&ev);
            self.history.add_event(ev);
        ...
        if result.properties_ok {
            let ev = RewardEvent::PropertyVerified { file: file.to_string(), dim: dim.to_string() };
            total_reward += RewardHistory::reward_value(&ev);
            self.history.add_event(ev);
        }
    ...
    pub fn update_capability(&self, cv: &mut CapabilityVector, dim_name: &str) -> f64 {
        let avg_reward = if self.history.positive_count + self.history.negative_count > 0 {
            self.history.total_reward / (self.history.positive_count + self.history.negative_count) as f64
        } else {
            0.0
        };
    ...
    pub fn summary(&self) -> String {
        format!(
            "RL 反馈: {} 正 / {} 负 | 总奖励 {:.2} | lr={}",
            self.history.positive_count, self.history.negative_count,
            self.history.total_reward, self.learning_rate,
        )
    }
}
...
mod tests {
    ...
    fn passing_result() -> VerificationResult {
        VerificationResult {
            ...
            compile_errors: vec![], test_failures: vec![], duration_ms: 100,
        }
    }

    fn failing_result() -> VerificationResult {
        VerificationResult {
            passed: false, compile_ok: false, tests_ok: false, properties_ok: false,
