...
impl CryptoAgent {
    ...
    pub fn setup_default_wallet(&mut self, label: &str) -> Result<&CryptoWallet, String> {
        ...
        self.wallet_manager.active_wallet().ok_or_else(|| "no active wallet after setup_default_wallet".to_string())
    }
    ...
    pub fn import_wallet(&mut self, private_key: &str, label: &str) -> Result<&CryptoWallet, String> {
        ...
        self.wallet_manager.add_wallet(wallet);
        self.wallet_manager.active_wallet().ok_or_else(|| "no active wallet after import".to_string())
    }
...
mod tests {
    ...
    fn test_setup_default_wallet() {
        ...
        let _wallet = agent.setup_default_wallet("main").unwrap();
    ...
    fn test_scan_opportunities() {
        let mut agent = CryptoAgent::new();
        agent.setup_default_wallet("test_scan_opportunities").unwrap();
    ...
    fn test_strategic_insights() {
        let mut agent = CryptoAgent::new();
        agent.setup_default_wallet("test").unwrap();
        agent.run_iteration();
