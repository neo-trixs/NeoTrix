...
impl AirdropRegistry {
        ...
        self.airdrops
            ...
            .map(|a| AirdropCheckResult {
                ...
                reason: if a.confidence > 0.7 {
                    ...
                    format!("Low confidence ({:.0}%), need more data", a.confidence * 100.0)
                },
            })
            .collect()
    }
    ...
    pub fn total_unclaimed_value(&self) -> f64 {
        self.airdrops.iter().filter(|a| !a.is_claimed).map(|a| a.estimated_value_usd).sum()
    }
}
