...
impl CriticNode {
    ...
    /// 独立能力评估（基于 capability evaluation，但保持独立实例）
    pub fn evaluate(&self, task_type: impl Into<TaskType>, capability: &CapabilityVector) -> f64 {
        let task_type: TaskType = task_type.into();
        let score = evaluate_capability(&task_type, capability);
...
/// Standalone capability evaluator (replaces L8 PerformanceEvaluator dependency)
pub fn evaluate_capability(task_type: &TaskType, capability: &CapabilityVector) -> f64 {
