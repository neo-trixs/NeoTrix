...
impl Context {
    pub fn from_task_description(desc: &str) -> Self {
        ...
        let task_type = if desc_lower.contains("design") || desc_lower.contains("ui") {
            TaskType::Design
        } else if desc_lower.contains("code") || desc_lower.contains("generate") || desc_lower.contains("review") {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AllocationStrategy { CapabilityFirst, LoadBalance, ThroughputFirst, Hybrid }
...
impl OptimalTaskAllocator {
    pub fn new(strategy: AllocationStrategy) -> Self { Self { _strategy: strategy } }
}
...
impl PlannerNode {
    ...
    pub fn decompose(&self, goal: &str) -> Vec<Task> {
        ...
        if lower.contains("prd") || lower.contains("product requirements") || lower.contains("prd generation") {
            return self.plan_prd(goal);
        }
        if lower.contains("competitive") || lower.contains("competitor") || lower.contains("comparison") || lower.contains("compare") {
            return self.plan_competitive_analysis(goal);
        }
        if lower.contains("ux audit") || lower.contains("ux review") || lower.contains("usability") || lower.contains("heuristic") {
            return self.plan_ux_audit(goal);
        }
        if lower.contains("experiment") || lower.contains("a/b test") || lower.contains("hypothesis") || lower.contains("ab test") {
        ...
        let enriched: Vec<String> = if let Some(ref gm) = self.group_manager {
            subtasks.into_iter().map(|desc| {
                let mut extra = Vec::new();
                for word in desc.split_whitespace() {
                    let clean = word.trim_matches(|c: char| !c.is_alphanumeric());
                    let matches = gm.match_cross_repo(clean);
                    if !matches.is_empty() {
                        let repos: Vec<&str> = matches.iter().map(|m| m.to_repo.as_str()).collect();
        ...
        enriched.into_iter().enumerate().map(|(i, desc)| {
    ...
    pub fn plan_prd(&self, goal: &str) -> Vec<Task> {
        let subtasks = vec![
            ...
            format!("Draft PRD with overview, problem statement, and target users: {}", goal),
    ...
    pub fn plan_experiment(&self, goal: &str) -> Vec<Task> {
        let subtasks = vec![
            ...
            format!("Design A/B test (sample size, duration, variants): {}", goal),
    ...
    fn build_tasks_from_subtasks(&self, subtasks: &[String]) -> Vec<Task> {
        subtasks.iter().enumerate().map(|(i, desc)| {
            let input: Vec<f64> = desc.bytes().map(|b| b as f64).collect();
            Task::new(format!("task_{}", i), input, i as i32)
        }).collect()
    }
