        ...
        for group in file_groups {
            if group.len() > self.max_goals_per_file {
                ...
                if has_critical && has_low {
                    if let (Some(critical_goal), Some(low_goal)) = (
                        group.iter().find(|g| matches!(g.priority, GoalPriority::Critical | GoalPriority::High)),
                        group.iter().find(|g| matches!(g.priority, GoalPriority::Medium | GoalPriority::Low)),
                    ) {
                        conflicts.push(GoalConflict {
                            goal_a_id: critical_goal.id.clone(),
                            goal_b_id: low_goal.id.clone(),
                            reason: ConflictReason::SameFile { file: critical_goal.target_file.clone().unwrap_or_default() },
