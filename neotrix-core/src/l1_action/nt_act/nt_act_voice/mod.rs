...
use crate::core::nt_core_sense::{SensoryEvent, SensoryEventKind, Transcription};
...
impl MicCapture {
    pub fn new() -> Self { Self { active: false, duration_secs: 2.0 } }
    ...
    pub fn poll(&mut self) -> Option<MicSample> {
        if !self.active { return None; }
        let mut meta = std::collections::HashMap::new();
        meta.insert("duration_secs".into(), self.duration_secs.to_string());
        Some(MicSample { raw_bytes: vec![], metadata: meta })
    }
}
...
impl VoiceInput {
    ...
    pub fn record(&mut self, duration: Duration) -> Result<VoiceSample, VoiceError> {
        ...
        let old = std::mem::take(&mut self.mic);
    ...
    pub fn transcribe(&self, sample: &VoiceSample) -> Result<String, VoiceError> {
        match &self.engine {
            TranscribeEngine::Mock => {
                MockTranscriber::transcribe(sample)
            }
            TranscribeEngine::Whisper => {
                WhisperTranscriber::transcribe(sample)
            }
            TranscribeEngine::ExternalAPI { .. } => {
                ExternalAPITranscriber::transcribe(sample)
            }
        }
    }
    ...
    pub fn to_event(&self, sample: &VoiceSample, text: &str) -> SensoryEvent {
        ...
        let ts = sample.timestamp.duration_since(UNIX_EPOCH)
    ...
    pub fn record_and_transcribe(&mut self, duration: Duration) -> Result<(VoiceSample, String), VoiceError> {
