                    ...
                    match &progress.status {
                        ...
                        | PipelineStatus::Playing {
                            ...
                            let speed = if elapsed > 0.5 {
                                let s = (*downloaded - last_downloaded) as f64 / elapsed;
                            ...
                            let eta_str = if speed > 0.0 {
                                if let Some(tot) = total {
                                    let rem = (tot - *downloaded) as f64 / speed;
                                    format!(" | 剩余 {}m{:02}s", rem as u64 / 60, rem as u64 % 60)
                                } else {
                                    String::new()
                                }
                            } else {
                            ...
                            let total_str = total_mb
                                .map(|m| format!("/{:.1} MB", m))
                                .unwrap_or_default();

                            print!(
                                "\r\033[K  {:3}% {} {:.1}{} MB | {}{}",
                                pct, bar, down_mb, total_str, speed_str, eta_str
                            );
                            use std::io::Write;
