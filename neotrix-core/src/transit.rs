...
use neotrix::neotrix::nt_shield_stealth_net::circuit_isolation::{
    CircuitIsolationConfig, CircuitIsolationManager,
};
...
use neotrix::neotrix::nt_shield_stealth_net::transit_station::{
    auto_start_transit, global_transit_station, stop_transit,
};
...
fn ensure_config() {
    ...
    match std::fs::write(&path, DEFAULT_CONFIG_TOML) {
        Ok(_) => log::info!(
            "[transit-daemon] created default config at {}",
            path.display()
        ),
...
"##,
    ...
    log::info!(
        "[transit-daemon] plist installed at {}",
        plist_path.display()
    );
    Ok(())
}
...
fn uninstall_launchd_plist() -> Result<(), String> {
    ...
    if plist_path.exists() {
        ...
        log::info!(
            "[transit-daemon] plist removed from {}",
            plist_path.display()
        );
    }
    Ok(())
...
fn setup_sudoers() -> Result<(), String> {
    ...
    match std::fs::write(&sudoers_path, sudoers_line) {
        Ok(_) => log::info!(
            "[transit-daemon] sudoers installed at {}",
            sudoers_path.display()
        ),
        Err(e) => log::warn!(
            "[transit-daemon] sudoers install failed (need sudo?): {}",
            e
        ),
    }
    Ok(())
...
pub async fn run(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    if args.len() > 1 {
        match args[1].as_str() {
            ...
            _ => {
                log::error!("Usage: neotrix transit [--install] [--uninstall]");
                std::process::exit(1);
            }
        }
    }
    ...
    tokio::spawn(async move {
        signal::ctrl_c().await.expect("transit-daemon: failed to register Ctrl+C shutdown handler");
    ...
    let _checker_handle = tokio::spawn(async move {
