...
impl ConnectionManager {
    ...
    pub fn update_state(&self, id: &str, state: ConnectionState) {
        if let Some(conn) = self.connections.lock().unwrap_or_else(|e| e.into_inner()).get_mut(id) {
            conn.state = state;
    ...
    pub fn check_rate_limit(&self, id: &str) -> bool {
        if let Some(conn) = self.connections.lock().unwrap_or_else(|e| e.into_inner()).get(id) {
    ...
    pub fn validate_payload(&self, data: &[u8]) -> L1Result<()> {
        if data.len() > self.max_payload_size {
            return Err(L1Error::Network(format!(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let stale_ids: Vec<String> = {
                    let map = manager.connections.lock().unwrap_or_else(|e| e.into_inner());
                    map.iter()
                        .filter(|(_, c)| {
