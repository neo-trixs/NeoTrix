...
impl LspServerManager {
    ...
    pub async fn spawn_for_lang(
        ...
        if self.active_servers.contains_key(lang) {
            return self.active_servers.get_mut(lang).ok_or_else(|| LspError::ConnectionFailed(format!("language '{}' not in active servers", lang)));
        }
        let config = self
            .servers
            .get(lang)
        ...
        self.active_servers.get_mut(lang).ok_or_else(|| LspError::ConnectionFailed("failed to retrieve client after insert".into()))
    }
