...
impl MetaMonitor {
    ...
    pub fn run_check(&mut self) -> HealthCheck {
        ...
        const MAX_CHECK_HISTORY: usize = 10000;
        if self.check_history.len() > MAX_CHECK_HISTORY {
            self.check_history.drain(0..MAX_CHECK_HISTORY / 5);
        }
        check
    }
    ...
    pub fn generate_alerts(&mut self) -> Vec<MetaAlert> {
        ...
        const MAX_ALERTS: usize = 10000;
        if self.alerts.len() > MAX_ALERTS {
            self.alerts.drain(0..MAX_ALERTS / 5);
        }
        new_alerts
    }
    ...
    pub fn trend_analysis(&self) -> HealthTrend {
        ...
        let last = self
            .check_history
            .last()
            .expect("check_history.len() >= 2 per guard");
