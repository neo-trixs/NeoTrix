...
impl SelfModel {
    ...
    pub fn register_evolution(&mut self, event: EvolutionEvent) {
        ...
        if self.evolution_history.len() > MAX_EVENTS {
            let excess = self.evolution_history.len() - MAX_EVENTS;
            self.evolution_history.drain(0..excess);
        }
    }
                ...
                Ok(None) => {
                    if start.elapsed() > timeout {
                        if let Err(e) = child.kill() {
                            log::debug!("[self_model] child kill after timeout: {e}");
                        }
                        if let Err(e) = child.wait() {
                            log::debug!("[self_model] child wait after timeout: {e}");
                        }
                        health.errors = 1;
                        return health;
                    }
                    tokio::runtime::Handle::current()
                        .block_on(tokio::time::sleep(std::time::Duration::from_millis(50)));
                }
                Err(_) => break None,
            }
        };
...
impl ArchitectureGraph {
    ...
    /// 注册或更新一个子系统
    pub fn register(&mut self, name: &str, path: &str, status: ModuleStatus) {
        let entry = self
            .nodes
    ...
    /// 关联 handler 名称
    pub fn attach_handler(&mut self, name: &str, handler: &str) {
        if let Some(node) = self.nodes.get_mut(name) {
            node.handler_name = Some(handler.to_string());
        }
    }
    ...
    /// 更新节点健康
    pub fn update_health(&mut self, name: &str, success: bool) {
        if let Some(node) = self.nodes.get_mut(name) {
            if !success {
                node.error_count += 1;
                node.health_score = (node.health_score * 0.9).max(0.0);
            } else {
                node.health_score = (node.health_score * 0.9 + 0.1).min(1.0);
            }
        }
    }
    ...
    pub fn active_count(&self) -> usize {
        self.nodes
            .values()
            .filter(|n| n.status == ModuleStatus::Active)
            .count()
    }

    /// 查询孤立模块 (已编译但未接线)
    pub fn isolated_modules(&self) -> Vec<&ArchitectureNode> {
        self.nodes
            .values()
            .filter(|n| n.status == ModuleStatus::Isolated)
            .collect()
    }

    /// 查询存根模块
    pub fn stubs(&self) -> Vec<&ArchitectureNode> {
        self.nodes
            .values()
            .filter(|n| n.status == ModuleStatus::Stub || n.status == ModuleStatus::StubNoImpl)
            .collect()
    }

    /// 查询健康度低于阈值的节点
    pub fn degraded(&self, threshold: f64) -> Vec<&ArchitectureNode> {
        self.nodes
            .values()
            .filter(|n| n.health_score < threshold)
            .collect()
    }
    ...
    pub fn report(&self) -> String {
        ...
        let isolated = self.isolated_modules().len();
        let stubs = self.stubs().len();
        let degraded = self.degraded(0.5).len();
        format!(
            "Architecture: {} total, {} active, {} isolated, {} stubs, {} degraded",
            total, active, isolated, stubs, degraded
        )
    }
}
