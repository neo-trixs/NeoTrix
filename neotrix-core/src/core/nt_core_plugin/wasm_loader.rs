...
mod tests {
    ...
    fn test_from_file_not_wasm() {
        ...
        std::fs::create_dir_all(&dir).expect("failed to create temp dir for wasm test");
        let f = dir.join("not_a_wasm.txt");
        std::fs::write(&f, b"hello, i am not wasm").expect("failed to write test file");
    ...
    fn test_from_stray_bytes_rejected() {
        ...
        let dir = std::env::temp_dir().join(format!("wasm_test_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("failed to create temp dir for stray test");
        let f = dir.join("stray.wasm");
        {
            let mut file = std::fs::File::create(&f).expect("failed to create stray test file");
    ...
    fn test_valid_minimal_wasm_roundtrip() {
        ...
        let dir = std::env::temp_dir().join(format!("wasm_test_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("failed to create temp dir for hello test");
        let f = dir.join("hello.wasm");
        std::fs::write(&f, &wasm_bytes).expect("failed to write wasm test file");
    ...
    fn test_plugin_registry_load_wasm_integration() {
        ...
        let dir = std::env::temp_dir().join(format!("wasm_test_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("failed to create temp dir for integration test");
        let f = dir.join("integration_test.wasm");
        std::fs::write(&f, &wasm_bytes).expect("failed to write integration test wasm");
