...
impl CognitiveLoadMonitor {
    pub fn new() -> Self {
        Self {
            ...
            prev_mode: ThinkingMode::Balanced,
            total_steps: 0,
            deep_steps: 0,
            max_load: 1.0,
        }
    ...
    pub fn record_step(&mut self, load: f64) {
        ...
        self.thinking_budget =
            (self.thinking_budget - clamped * 0.1 + BUDGET_RECHARGE_RATE).clamp(0.0, 1.0);
        self.update_mode();
    }
    ...
    fn update_mode(&mut self) {
        ...
        let target = match self.prev_mode {
            ThinkingMode::Deep => avg > 0.45,
            ThinkingMode::Fast => avg < 0.25 && self.thinking_budget > 0.3,
            ThinkingMode::Balanced => true,
        };
        let new_mode = if self.thinking_budget > DEEP_MODE_BUDGET * 0.5 && avg < 0.4 && target {
            ThinkingMode::Deep
        } else if (self.thinking_budget < FAST_MODE_BUDGET * 0.5 || avg > 0.7) && target {
            ThinkingMode::Fast
        } else {
    ...
    pub fn reset(&mut self) {
        ...
        self.prev_mode = ThinkingMode::Balanced;
        self.total_steps = 0;
        self.deep_steps = 0;
    }
}
