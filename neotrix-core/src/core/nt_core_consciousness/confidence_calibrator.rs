...
impl CalibrationBin {
    fn new(min: f64, max: f64) -> Self {
        Self { min, max, count: 0, correct_count: 0 }
    }

    fn accuracy(&self) -> f64 {
        if self.count == 0 { 0.0 } else { self.correct_count as f64 / self.count as f64 }
    }
...
impl ConfidenceCalibrator {
    ...
    /// Running EMA of ECE
    pub fn ece(&self) -> f64 {
        if self.total_predictions < 5 { 0.0 } else { self.running_ece }
    }

    /// MetaAccuracy KPI: 1.0 - MAE (higher = better calibration)
    pub fn meta_accuracy(&self) -> f64 {
        if self.total_predictions < 5 { 1.0 } else { (1.0 - self.running_mae).clamp(0.0, 1.0) }
    }

    /// Overall accuracy (correct / total)
    pub fn overall_accuracy(&self) -> f64 {
        if self.total_predictions == 0 { 0.5 } else { self.total_correct as f64 / self.total_predictions as f64 }
    }
