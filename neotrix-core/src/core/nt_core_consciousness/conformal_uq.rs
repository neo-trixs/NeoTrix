...
impl ConformalSet {
    fn new(
        ...
        let prediction_set: Vec<usize> = predictions.iter()
            .filter(|(_, score)| *score <= threshold)
            .map(|(idx, _)| *idx)
            .collect();
        let empty = prediction_set.is_empty();
        Self { predictions, threshold, prediction_set, coverage, empty }
    }
}
...
impl ConformalUQ {
    ...
    pub fn predict(&self, query: &[u8], candidates: &[&[u8]]) -> ConformalSet {
        ...
        let predictions: Vec<(usize, f64)> = candidates.iter().enumerate()
    ...
    pub fn predict_single(&self, query: &[u8], candidate: &[u8]) -> f64 {
        ...
        let count_gte = self.calibration_scores.iter()
            .filter(|&&s| s >= nc)
            .count();
    ...
    pub fn estimate_confidence(&self, query: &[u8], candidates: &[&[u8]]) -> f64 {
        ...
        if cs.prediction_set.len() == 1 {
            ...
            let nc_score = cs.predictions.iter()
                .find(|(i, _)| *i == idx)
                .map(|(_, s)| *s)
                .unwrap_or(1.0);
            return 1.0 - nc_score;
    ...
    pub fn apply_to_tagged(
        &self,
        tagged: &mut VsaTagged,
        query: &[u8],
        candidates: &[&[u8]],
    ) {
