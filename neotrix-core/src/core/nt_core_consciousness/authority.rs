...
impl AuthorityResolver {
    pub fn resolve_conflict(
        ...
        if primary_prio > secondary_prio + 10 {
            return ConflictResolution::UseHigherAuthority(
                primary.authority,
                secondary.authority,
            );
        }
        ...
        ConflictResolution::MergeWithWeight(
            primary_priority_weight(primary_prio, secondary_prio),
        )
    }
    ...
    pub fn sort_by_authority(
        items: &mut [super::vsa_tag::VsaTagged],
    ) {
        items.sort_by(|a, b| {
...
mod tests {
    use super::*;
    use crate::core::nt_core_consciousness::vsa_tag::{VsaTagged, VsaOrigin, VsaSelfCategory};
    ...
    fn test_authority_from_vsa_origin_self() {
        ...
        assert_eq!(AuthorityLevel::from_vsa_origin(&thought), AuthorityLevel::Memory);

        let intention = VsaOrigin::Self_(VsaSelfCategory::Intention);
        assert_eq!(AuthorityLevel::from_vsa_origin(&intention), AuthorityLevel::UserIntent);
    }

    #[test]
    fn test_authority_from_vsa_origin_world() {
        let input = VsaOrigin::World(crate::core::nt_core_consciousness::vsa_tag::VsaWorldCategory::UserInput);
        assert_eq!(AuthorityLevel::from_vsa_origin(&input), AuthorityLevel::UserIntent);

        let tool = VsaOrigin::World(crate::core::nt_core_consciousness::vsa_tag::VsaWorldCategory::ToolOutput);
        assert_eq!(AuthorityLevel::from_vsa_origin(&tool), AuthorityLevel::ToolOutput);
    }
    ...
    fn test_authority_tag_with_detail() {
        ...
        assert_eq!(tag.provenance_detail.unwrap(), "cargo check returned warnings");
    }
    ...
    fn test_resolve_conflict_by_authority() {
        ...
        assert!(matches!(resolution, ConflictResolution::UseHigherAuthority(AuthorityLevel::UserIntent, _)));
    }
    ...
    fn test_resolve_conflict_by_confidence() {
        ...
        assert!(matches!(resolution, ConflictResolution::UseHigherConfidence(0.95, _)));
    }
    ...
    fn test_constitution_default_has_ordering() {
        ...
        assert_eq!(constitution.authority_ordering[0], AuthorityLevel::UserIntent);
        assert_eq!(constitution.authority_ordering[9], AuthorityLevel::AmbientNoise);
    }

    #[test]
    fn test_constitution_has_protected_invariants() {
        let constitution = Constitution::neotrix_default();
        assert!(constitution.protected_invariants.contains(&"never_expose_api_keys".into()));
    }
    ...
    fn test_constitution_resolve_sorts_by_authority() {
        ...
        assert_eq!(resolved[0].tag, VsaOrigin::Self_(VsaSelfCategory::Intention));
    }
