...
impl VolitionEngine {
    ...
    pub fn propose(&mut self, candidate: ActionCandidate) {
        if self.candidates.len() >= MAX_CANDIDATES {
            ...
            if candidate.expected_value > self.candidates.last().unwrap().expected_value {
                self.candidates.pop();
            } else {
                return;
            }
...
mod tests {
    ...
    #[test]
    fn test_evaluate_outcome_empty_predictions() {
        let mut ve = VolitionEngine::new();
        let candidate = ActionCandidate::new(vec![1; 10], "test");
        let acc = ve.evaluate_outcome(&candidate, &vec![1u8; 10]);
