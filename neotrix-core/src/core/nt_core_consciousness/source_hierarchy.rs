...
static NULL_KNOWLEDGE_LAYER: KnowledgeLayer = KnowledgeLayer::Raw(PerceptionMeta {
    source_type: PerceptionSource::UserInput,
    raw_confidence: 0.0,
    timestamp: 0,
});
...
impl SourceHierarchy for ProvenanceChain {
    fn layer(&self) -> &KnowledgeLayer {
        if let Some((layer, _)) = self.layers.last() {
            layer
        } else {
            &NULL_KNOWLEDGE_LAYER
        }
    }
