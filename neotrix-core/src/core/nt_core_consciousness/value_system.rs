...
impl ValueSystem {
    ...
    pub fn get_weight(&self, value: CoreValue) -> f64 {
        self.weights.iter().find(|w| w.value == value).map(|w| w.weight).unwrap_or(0.0)
    }

    pub fn get_satisfaction(&self, value: CoreValue) -> f64 {
        self.weights.iter().find(|w| w.value == value).map(|w| w.satisfaction).unwrap_or(0.0)
    }
    ...
    pub fn record_negentropy_satisfaction(&mut self, delta_n: f64) {
        ...
        if delta_n > 0.0 {
            self.record_satisfaction(CoreValue::Curiosity, (self.get_satisfaction(CoreValue::Curiosity) + 0.05).min(1.0));
        }
    }
...
mod tests {
    ...
    fn test_unsatisfied_values() {
        ...
        let unsatisfied = vs.unsatisfied_values(0.5);
    ...
    fn test_default_weights_are_ordered() {
        ...
        assert!(curiosity > helpfulness, "curiosity should outweigh helpfulness");
    }
}
