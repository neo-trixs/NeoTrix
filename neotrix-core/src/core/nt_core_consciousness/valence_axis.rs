...
use super::caa_steering::CaaDirection;
...
impl NamedEmotion {
    pub fn from_valence_arousal(valence: f64, arousal: f64) -> Self {
        match () {
            ...
            _ if valence > 0.3 && valence < 0.7 && arousal > 0.2 && arousal < 0.6 => {
                NamedEmotion::Gratitude
            }
...
impl ValenceAxis {
    ...
    pub fn shift(&mut self, valence_delta: f64, arousal_delta: f64) {
        ...
        let v = if v.abs() > 0.6 {
            v * (1.0 - reg_strength) + v.signum() * 0.6 * reg_strength
        } else {
            v
        };
        let a = if a > 0.8 {
            a * (1.0 - reg_strength) + 0.8 * reg_strength
        } else {
            a
        };
    ...
    pub fn coherence(&self) -> f64 {
        ...
        let var_v: f64 = last_5
            .iter()
            .map(|(v, _)| (v - mean_v).powi(2))
            .sum::<f64>()
            / last_5.len() as f64;
        let var_a: f64 = last_5
            .iter()
            .map(|(_, a)| (a - mean_a).powi(2))
            .sum::<f64>()
            / last_5.len() as f64;
        (1.0 - (var_v.sqrt().min(1.0)) * 0.5 + (1.0 - var_a.sqrt().min(1.0)) * 0.5).clamp(0.0, 1.0)
    }

    /// Convert current valence state to a CAA direction vector.
    pub fn to_caa_direction(&self, context_vsa: &[u8]) -> CaaDirection {
        let emotion = self.current_emotion();
        let emotion_name = format!("{:?}", emotion);
        let intensity = self.emotional_intensity();
        let seed = emotion_name
            .bytes()
        ...
        for i in 0..VSA_DIM {
            ...
            let mixed = (e_val as f64 * intensity + c_val as f64 * (1.0 - intensity)).round() as u8;
...
mod tests {
    ...
    fn test_negentropy_calibrates_emotion() {
        ...
        assert!(matches!(
            emotion,
            NamedEmotion::Excitement | NamedEmotion::Curiosity | NamedEmotion::Satisfaction
        ));
    }
    ...
    fn test_negentropy_frustration() {
        ...
        assert!(
            v.valence < -0.1,
            "sustained negative ΔN should cause negative valence"
        );
    }
    ...
    fn test_emotion_mapping() {
        assert_eq!(
            NamedEmotion::from_valence_arousal(0.8, 0.9),
            NamedEmotion::Excitement
        );
        assert_eq!(
            NamedEmotion::from_valence_arousal(0.3, 0.6),
            NamedEmotion::Curiosity
        );
        assert_eq!(
            NamedEmotion::from_valence_arousal(0.7, 0.2),
            NamedEmotion::Satisfaction
        );
        assert_eq!(
            NamedEmotion::from_valence_arousal(-0.5, 0.6),
            NamedEmotion::Frustration
        );
        assert_eq!(
            NamedEmotion::from_valence_arousal(0.0, 0.2),
            NamedEmotion::Calm
        );
    }
