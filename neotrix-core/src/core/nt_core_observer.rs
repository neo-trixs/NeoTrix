...
impl OneObserver {
    ...
    fn detect_stuck(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        ...
        let Some(last) = modes.last().copied() else { return; };
        let mut repeat = 0;
    ...
    fn detect_efficiency(modes: &[ReasoningHexagram], patterns: &mut Vec<TrajectoryPattern>) {
        ...
        let Some(start) = modes.first().copied() else { return; };
        let Some(end) = modes.last().copied() else { return; };
...
mod tests {
    ...
    fn test_observer_recommends_meta_on_critical() {
        ...
        assert!(report.recommended_meta.expect("recommended_meta should be set").is_reflecting());
    }
