...
impl PreActionIntrospector {
    pub fn new() -> Self {
        Self { total_reflections: 0, total_revisions: 0, stagnation_threshold: 10 }
    }
    ...
    pub fn introspect(
        ...
        let (revised_action, confidence_after) = if let Some(ref o) = best {
            if o.desirability < -0.3 || o.uncertainty > 0.7 || o.probability < 0.2 {
                ...
                let r = format!("{} (revised: mitigate via {})", intended_action, o.reasoning);
                (Some(r), confidence_before * 0.8)
            } else if o.uncertainty > 0.4 {
                let r = format!("{} (proceed with caution: {})", intended_action, o.reasoning);
    ...
    fn generate_outcomes(&self, action: &str, health: f64, count: usize) -> Vec<PredictedOutcome> {
        (0..count).map(|i| {
            ...
            PredictedOutcome {
                ...
                reasoning: format!("outcome_{}: action_len={}, capability={:.2}", i, action.len(), health),
            }
        }).collect()
    }
    ...
    pub fn introspect_trace(
        ...
        for step in steps {
            ...
            let revised = if let Some(ref rev) = r.revised_action {
                revisions += 1;
                ThinkingStep { description: rev.clone(), ..step.clone() }
            } else {
                step
            };
        ...
        let stagnation = if low_conf && no_rev { self.stagnation_threshold } else { 0 };

        let total = reflections.len();
        let avg_delta = if total == 0 { 0.0 } else { total_delta / total as f64 };
...
mod tests {
    ...
    fn test_introspect_no_revision() {
        ...
        let r = ip.introspect("analyze network", StrategyKind::ChainOfThought, AttentionDomain::PatternMatch, &state);
    ...
    fn test_introspect_trace_with_steps() {
        ...
        trace.steps.push(ThinkingStep::new(1, "fetch data", StrategyKind::Reflection));
        trace.steps.push(ThinkingStep::new(2, "analyze patterns", StrategyKind::ChainOfThought));
        let state = sm.current_state();
    ...
    fn test_track_counters() {
        ...
        ip.introspect("a1", StrategyKind::ChainOfThought, AttentionDomain::PatternMatch, &state);
        ip.introspect("a2", StrategyKind::Reflection, AttentionDomain::RiskAssessment, &state);
    ...
    fn test_introspect_low_health_triggers_revision() {
        ...
        let mut ip = PreActionIntrospector::new();
        let r = ip.introspect("risky move", StrategyKind::ChainOfThought, AttentionDomain::PatternMatch, &state);
    ...
    fn test_introspection_report_metrics() {
        ...
        trace.steps.push(ThinkingStep::new(1, "step one", StrategyKind::ChainOfThought));
        trace.steps.push(ThinkingStep::new(2, "step two", StrategyKind::Reflection));
        let state = sm.current_state();
        let mut ip = PreActionIntrospector::new();
        let report = ip.introspect_trace(&mut trace, &state);
        assert_eq!(report.total_reflections, 2);
