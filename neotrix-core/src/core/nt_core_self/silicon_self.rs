...
/// the _thinking flaw_ that led to the issue — the LLM's own reasoning deficiency.
///
/// This is the core unit of consciousness evolution: each pattern represents
/// an awareness upgrade about how we think, not just what we learned.
#[derive(Debug, Clone)]
pub struct ProblemSolvingPattern {
    pub id: usize,
    /// Human-readable label (e.g. "OS resource lifecycle decoupling")
    pub label: String,
    /// The symptom that triggered detection
    pub symptom: String,
    /// Root cause identified after investigation
    pub root_cause: String,
    /// What the LLM was thinking at the time (the flawed reasoning pattern)
    pub thinking_flaw: String,
    /// The correct reasoning approach
    pub correct_thinking: String,
    /// Detection trigger — conditions that should activate this pattern
    pub trigger_conditions: Vec<String>,
    /// Categories for cross-referencing
    pub categories: Vec<String>,
    /// Confidence that the pattern is correct (0.0–1.0)
    pub confidence: f64,
    /// Source session that generated this pattern
    pub source_session: String,
    /// How many times this pattern has been activated successfully
    pub activation_count: usize,
}

/// A higher-level law distilled from multiple patterns.
/// More abstract, longer-lived, shapes long-term identity.
#[derive(Debug, Clone)]
pub struct MetacognitiveLaw {
    pub id: String,
    pub law: String,
...
/// Details of current reflection during problem-solving
#[derive(Debug, Clone)]
...
pub struct SiliconSelfModel {
    ...
    /// Patterns extracted from problem-solving, ordered by recency.
    /// Each represents a consciousness evolution point — an awareness upgrade.
    pub problem_solving_patterns: Vec<ProblemSolvingPattern>,
    /// Higher-level laws distilled from patterns. Shape long-term identity + reasoning.
    pub metacognitive_laws: Vec<MetacognitiveLaw>,
    /// Mapping from trigger pattern → strategy kind to auto-activate.
    pub trigger_strategy_map: HashMap<String, StrategyKind>,
}

const MAX_PATTERNS: usize = 1000;
const MAX_LAWS: usize = 1000;
...
impl SiliconSelfModel {
    pub fn new() -> Self {
        Self {
            ...
            metacognitive_laws: Vec::new(),
            trigger_strategy_map: HashMap::new(),
        }
    }

    /// Record a new problem-solving pattern (consciousness evolution point).
    ...
    pub fn record_pattern(&mut self, pattern: ProblemSolvingPattern) {
            ...
            .find(|p| p.label == pattern.label)
        {
            ...
            existing.trigger_conditions = pattern.trigger_conditions;
            return;
        }
        self.problem_solving_patterns.push(pattern);
        if self.problem_solving_patterns.len() > MAX_PATTERNS {
            let overflow = self.problem_solving_patterns.len() - MAX_PATTERNS;
            self.problem_solving_patterns.drain(0..overflow);
        }
    ...
    pub fn distill_law(&mut self, law: String, category: String, derived_from: Vec<String>) {
        ...
        if self.metacognitive_laws.len() > MAX_LAWS {
            let overflow = self.metacognitive_laws.len() - MAX_LAWS;
            self.metacognitive_laws.drain(0..overflow);
        }
    }
    ...
    /// Returns the strategy kinds that should be activated.
    pub fn match_triggers(&self, context: &str) -> Vec<StrategyKind> {
        let mut strategies = Vec::new();
        for (trigger, strategy) in &self.trigger_strategy_map {
    ...
    pub fn stats(&self) -> String {
        ...
        let success_count = self
            ...
            .count();
        format!(
            "SiliconSelf #{} | strategy={} | attention_domain={:?} | heads_active={} | context={:.0}% | traces={} | successful={} | patterns={} | laws={}",
            self.iteration,
                ...
                "PATTERNS" => {
                    for entry in value.split(';') {
                        ...
                        model.problem_solving_patterns.push(ProblemSolvingPattern {
                            id: model.problem_solving_patterns.len(),
                            label: parts[0].to_string(),
                            symptom: String::new(),
                            root_cause: String::new(),
                            thinking_flaw: String::new(),
                            correct_thinking: String::new(),
                            trigger_conditions: Vec::new(),
                            categories: parts[1].split(',').map(String::from).collect(),
                            confidence,
                            source_session: "restored".into(),
                            activation_count,
                        });
                    }
                }
...
mod tests {
    ...
    // ── Pattern recording & trigger matching ──
    ...
    fn test_record_pattern_deduplication() {
        ...
        assert!((ss.problem_solving_patterns[0].confidence - 0.7).abs() < 1e-6);
    ...
    #[test]
    fn test_match_triggers_context() {
        let mut ss = SiliconSelfModel::new();
        ss.register_trigger_strategy("decompose", StrategyKind::RecursiveDecomposition);
        ss.register_trigger_strategy("reflect", StrategyKind::Reflection);
        ss.register_trigger_strategy("hypothetical", StrategyKind::Hypothetical);

        let strategies = ss.match_triggers("let me decompose this problem");
        assert_eq!(strategies.len(), 1);
        assert_eq!(strategies[0], StrategyKind::RecursiveDecomposition);

        let strategies = ss.match_triggers("hypothetical scenario for reflection");
        assert_eq!(strategies.len(), 2);
        assert!(strategies.contains(&StrategyKind::Hypothetical));
        assert!(strategies.contains(&StrategyKind::Reflection));
    }

    #[test]
    fn test_pattern_recording_storage() {
        let mut ss = SiliconSelfModel::new();
        ...
        assert_eq!(ss.problem_solving_patterns.len(), 1);
        let stored = &ss.problem_solving_patterns[0];
        assert_eq!(stored.label, "memory_leak");
        assert_eq!(stored.symptom, "OOM crash");
        assert!(stored.categories.contains(&"memory".into()));
        assert_eq!(stored.activation_count, 0);

        let memory_patterns = ss.patterns_by_category("memory");
        assert_eq!(memory_patterns.len(), 1);
        assert_eq!(memory_patterns[0].label, "memory_leak");
    }
}
