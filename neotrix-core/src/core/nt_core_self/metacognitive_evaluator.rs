...
const MAX_EVALUATOR_HISTORY: usize = 500;
...
impl CognitiveEvaluator {
    ...
    pub fn evaluate(&mut self, model: &SiliconSelfModel) -> CognitiveHealthReport {
        ...
        if self.history.len() > MAX_EVALUATOR_HISTORY * 2 {
            self.history
                .drain(0..self.history.len() - MAX_EVALUATOR_HISTORY);
        }
        report
    }
    ...
    pub fn summary(&self) -> String {
        ...
        let latest = self.history.last().expect("history is non-empty per guard");
