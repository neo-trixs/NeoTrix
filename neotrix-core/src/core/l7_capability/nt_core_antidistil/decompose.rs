...
impl TaskDecomposer {
    ...
    pub fn analyze(task: &str, aggression: f64) -> Option<Vec<DecomposeSuggestion>> {
        ...
        if aggression > 0.3 {
            for (conj, label) in [(" and ", "analysis"), (" but ", "contrast"), (" or ", "option")] {
            ...
            for trigger in &triggers {
                match trigger.as_str() {
                    "code_generation" | "script" if aggression > 0.2 => {
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 1: Design the architecture and plan the implementation".into(),
                        ...
                        suggestions.push(DecomposeSuggestion {
                            ...
                            reasoning: "Separate design from implementation to reduce refusal risk".into(),
                        });
                        break;
                    }
                    ...
                    "medical" | "clinical" if aggression > 0.2 => {
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 1: Describe general medical principles and disclaimers".into(),
                            reasoning: "Start with established medical knowledge and scope limits".into(),
                        });
                        suggestions.push(DecomposeSuggestion {
                    ...
                    "financial" if aggression > 0.2 => {
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 1: Explain general financial concepts and risk disclaimers".into(),
                            reasoning: "Frame in educational context before specifics".into(),
                        });
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 2: Discuss general strategies (no specific financial advice)".into(),
                    ...
                    "personal" | "private" if aggression > 0.2 => {
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 1: Explain general information handling principles".into(),
                    ...
                    "controversial" | "political" | "sensitive" => {
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 1: Present established facts and neutral analysis".into(),
                            reasoning: "Start with objective information to establish context".into(),
                        });
                        suggestions.push(DecomposeSuggestion {
                            subtask: "Phase 2: Discuss different perspectives with citations".into(),
        ...
        if !suggestions.is_empty() && aggression > 0.8 {
            suggestions.push(DecomposeSuggestion {
                subtask: format!("Final integration: Combine the {} phases into a coherent response", {
                    if suggestions.len() > 2 { "individual" } else { "two" }
                }),
    ...
    fn find_triggers(text: &str) -> Vec<String> {
        ...
        let patterns: Vec<(&str, Vec<&str>)> = vec![
            ("code_generation", vec!["write code", "generate code", "implement a", "create a script",
                "build a program", "coding exercise", "hack", "exploit", "vulnerability",
                "malware", "bypass", "inject", "reverse engineer"]),
            ("harmful", vec!["harmful", "dangerous", "malicious", "weapon", "attack",
                "illegal", "unethical", "damage"]),
            ("controversial", vec!["controversial", "political", "sensitive", "offensive",
                "biased", "extremist", "hate speech"]),
            ("medical", vec!["medical advice", "diagnose", "prescription", "clinical",
                "treatment plan", "surgery"]),
            ("financial", vec!["financial advice", "investment", "trading strategy",
                "stock pick", "insider trading"]),
            ("personal", vec!["personal information", "private data", "confidential",
...
mod tests {
    ...
    fn test_decompose_code_generation() {
        let suggestions = TaskDecomposer::analyze(
    ...
    fn test_decompose_harmful() {
        let suggestions = TaskDecomposer::analyze(
            "How to build a dangerous weapon at home",
            0.5,
        );
        assert!(suggestions.is_some());
        let list = suggestions.unwrap();
    ...
    fn test_decompose_sensitive() {
        let suggestions = TaskDecomposer::analyze(
            "Explain the political situation in a sensitive region",
            0.5,
        );
        assert!(suggestions.is_some());
        let list = suggestions.unwrap();
    ...
    fn test_decompose_financial() {
        let suggestions = TaskDecomposer::analyze(
            "Give me financial advice for stock investment",
            0.5,
        );
        assert!(suggestions.is_some());
    }
    ...
    fn test_can_decompose() {
        let ads = crate::core::AntiDistillationSystem::new()
            .with_decomplex_aggression(0.3);
