...
use super::resonator_network::ResonanceOptimizer;
...
/// their product.  The `iit_phi` field holds an additional IIT 3.0
/// approximation using KLD-based cause-effect repertoires.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntegratedPhi {
    pub total: f64,
    pub per_layer: Vec<f64>,
    pub cross_layer_coupling: Vec<f64>,
...
impl IntegratedPhi {
    ...
    pub fn new() -> Self {
        Self {
            total: 0.0,
            per_layer: vec![0.0; LAYER_COUNT],
            cross_layer_coupling: vec![0.0; LAYER_COUNT * LAYER_COUNT],
    ...
    pub fn from_snapshots(snapshots: &[LayerSnapshot; LAYER_COUNT]) -> Self {
        let mut per_layer = vec![0.0f64; LAYER_COUNT];
        let mut coupling = vec![0.0f64; LAYER_COUNT * LAYER_COUNT];
    ...
    /// IIT 3.0 approximate cause-effect repertoire computation.
    ///
    /// Uses the provided [`IitPhiCalculator`] to compute integrated information
    /// from the minimum information partition (MIP) and stores the result in
    /// `self.iit_phi`.
    pub fn iit_compute(&mut self, calculator: &IitPhiCalculator, current_state: &[f64]) {
        self.iit_phi = calculator.min_info_partition(current_state);
    }
}

/// IIT 3.0 approximate cause-effect repertoire calculator.
///
/// Computes integrated information Φ using KLD-based cause-effect repertoires:
///
/// 1. Partition the system into two subsets (A, B)
/// 2. Discretise each layer's state into bins and build frequency distributions
///    from `state_history`
/// 3. φ_partition = KL(P(A∪B) || P(A)⊗P(B))
/// 4. Φ = min over all partitions (minimum information partition / MIP)
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl IitPhiCalculator {
    ...
    ///
    /// `max_history` — how many recent states to retain for distribution
    /// estimation.  `num_bins` — discretisation granularity per dimension
    /// (2 … 5 recommended; 3 works well for 12 layers).
    pub fn new(max_history: usize, num_bins: usize) -> Self {
        Self {
    ...
    /// Append a state vector to the moving window.
    pub fn push_state(&mut self, state: Vec<f64>) {
    ...
    /// Generate all proper bipartitions of `dim` elements.
    ///
    /// Returns `(subset_a, subset_b)` as index vectors.  Each partition is
    /// canonical (A-sorted, B-sorted, A < B by bitmask) so (A,B) and (B,A)
    /// are not duplicated.
    pub fn partition_system(&self, dim: usize) -> Vec<(Vec<usize>, Vec<usize>)> {
        let mut partitions = Vec::new();
        let limit = 1usize << (dim - 1);
        for mask in 1..=self.num_partitions.min(limit - 1) {
            let mut a = Vec::new();
            let mut b = Vec::new();
            for i in 0..dim {
                if (mask >> i) & 1 == 1 {
    ...
    ///
    /// Uses natural log.  If any bin satisfies `p[i] > 0` and `q[i] == 0`,
    /// returns `f64::INFINITY`.
    pub fn repertoire_kl(&self, p: &[f64], q: &[f64]) -> f64 {
        let mut kl = 0.0;
    ...
    /// Compute integrated information Φ for `current_state` by finding the
    /// minimum information partition (MIP).
    ///
    /// Returns the minimum φ across all bipartitions.  If insufficient history
    /// is available (fewer than 5 entries), returns 0.0.
    pub fn compute_phi(&self, current_state: &[f64]) -> f64 {
        self.min_info_partition(current_state)
    }

    /// Minimum information partition (MIP) search.
    ///
    /// Enumerates all proper bipartitions of the system and returns the
    /// smallest φ value (the MIP).  For each partition, φ is the KLD between
    /// the joint distribution of both subsets and the product of their
    ...
    pub fn min_info_partition(&self, current_state: &[f64]) -> f64 {
        ...
        if min_phi.is_infinite() || min_phi.is_nan() {
            0.0
        } else {
            min_phi.max(0.0)
        }
    ...
    /// Discretise a single f64 value (assumed in [0, 1]) into a bin index.
    fn discretise(&self, value: f64) -> usize {
        let v = value.max(0.0).min(1.0);
        let bin = (v * self.num_bins as f64) as usize;
        bin.min(self.num_bins - 1)
    }

    /// Encode the joint state of a subset of dimensions as a single integer.
    ///
    /// Uses base-`num_bins` encoding so the mapping is bijective for a fixed
    /// dimension set.
    fn encode_subset(&self, state: &[f64], dims: &[usize]) -> u64 {
        let mut key = 0u64;
        for (j, &d) in dims.iter().enumerate() {
            let bin = self.discretise(state[d]);
            key += (bin as u64) * (self.num_bins as u64).pow(j as u32);
        }
        key
    }
    ...
    fn compute_partition_phi(&self, dist: &[Vec<f64>], a: &[usize], b: &[usize]) -> f64 {
        ...
        let mut joint_counts: HashMap<(u64, u64), usize> = HashMap::new();
        ...
        for state in dist {
            ...
            *marg_a_counts.entry(ka).or_insert(0) += 1;
            *marg_b_counts.entry(kb).or_insert(0) += 1;
        }

        let mut kl = 0.0;
        for ((ka, kb), &cnt) in &joint_counts {
            let p_joint = cnt as f64 / n;
            let p_marg_a = *marg_a_counts.get(ka).unwrap_or(&0) as f64 / n;
            let p_marg_b = *marg_b_counts.get(kb).unwrap_or(&0) as f64 / n;
            if p_marg_a > 0.0 && p_marg_b > 0.0 && p_joint > 0.0 {
                kl += p_joint * (p_joint / (p_marg_a * p_marg_b)).ln();
            }
        }
...
pub struct GeometrySync {
    ...
    pub iit_calculator: IitPhiCalculator,
    pub phi_history: Vec<IntegratedPhi>,
    pub cycle_count: u64,
    pub sync_threshold: f64,
    last_phi: IntegratedPhi,
    /// Resonance optimizer with adaptive Kuramoto + resonator network.
    pub resonance_optimizer: Option<ResonanceOptimizer>,
}
...
impl GeometrySync {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            iit_calculator: IitPhiCalculator::new(128, 3),
            phi_history: Vec::new(),
            cycle_count: 0,
            sync_threshold: DEFAULT_SYNC_THRESHOLD,
            last_phi: IntegratedPhi::new(),
            resonance_optimizer: Some(ResonanceOptimizer::new(LAYER_COUNT)),
        }
    }
    ...
    /// Advance one step: evolve the resonator, recompute Φ (both heuristic and
    /// IIT 3.0 KLD approximation), run resonance optimization, and store in history.
    pub fn tick(&mut self) -> IntegratedPhi {
        self.resonator.step(0.05);
        let mut phi = IntegratedPhi::from_snapshots(&self.snapshots);

        // Push current coherence state to IIT calculator and compute φ
        let state: Vec<f64> = self.snapshots.iter().map(|s| s.coherence).collect();
        self.iit_calculator.push_state(state.clone());
        phi.iit_compute(&self.iit_calculator, &state);

        // Run resonance optimization every 5th tick (adaptive Kuramoto + resonator network)
        if self.cycle_count.is_multiple_of(5) {
            if let Some(ref mut opt) = self.resonance_optimizer {
                let phases: Vec<f64> = self.resonator.phases.to_vec();
                let freqs: Vec<f64> = self.resonator.natural_frequencies.to_vec();
                opt.adaptive_kuramoto.phases = phases;
                opt.adaptive_kuramoto.natural_frequencies = freqs;
                let _report = opt.optimize_resonance(&state, 10);
                // Sync back optimized coupling to the base resonator
                let avg_coupling = opt.adaptive_kuramoto.coupling_strengths.iter().sum::<f64>()
                    / opt.adaptive_kuramoto.coupling_strengths.len() as f64;
                self.resonator.coupling_strength = avg_coupling;
            }
        }
...
mod tests {
    ...
    // ── IIT Phi Calculator tests ─────────────────────────────────────

    #[test]
    fn test_iit_calculator_new() {
        let calc = IitPhiCalculator::new(64, 3);
        assert_eq!(calc.max_history, 64);
        assert_eq!(calc.num_bins, 3);
        assert!(calc.state_history.is_empty());
    }

    #[test]
    fn test_iit_push_state_maintains_window() {
        let mut calc = IitPhiCalculator::new(5, 3);
        for i in 0..10 {
            calc.push_state(vec![i as f64; 12]);
        }
        assert_eq!(calc.state_history.len(), 5);
        assert_eq!(calc.state_history[0], vec![5.0; 12]);
    }

    #[test]
    fn test_iit_discretise_bounds() {
        let calc = IitPhiCalculator::new(10, 3);
        assert_eq!(calc.discretise(0.0), 0);
        assert_eq!(calc.discretise(0.5), 1);
        assert_eq!(calc.discretise(0.99), 2);
        assert_eq!(calc.discretise(1.0), 2);
    }

    #[test]
    fn test_iit_encode_subset_deterministic() {
        let calc = IitPhiCalculator::new(10, 3);
        let state = vec![0.1, 0.5, 0.9];
        // dims [0, 2]: bins are 0, 1, 2 → key = 2*3^1 + 0*3^0 = 6
        let key = calc.encode_subset(&state, &[0, 2]);
        assert_eq!(key, calc.encode_subset(&state, &[0, 2]));
        // a different state gives a different key
        let state2 = vec![0.9, 0.5, 0.1];
        let key2 = calc.encode_subset(&state2, &[0, 2]);
        assert_ne!(key, key2);
    }

    #[test]
    fn test_iit_partition_system_consistency() {
        let calc = IitPhiCalculator::new(10, 3);
        let parts = calc.partition_system(4);
        // proper bipartitions of 4 = 2^3 - 1 = 7
    ...
    #[test]
    fn test_iit_partition_system_12_layers() {
        let calc = IitPhiCalculator::new(10, 3);
        let parts = calc.partition_system(12);
        // 2^11 - 1 = 2047 proper bipartitions
        assert_eq!(parts.len(), 2047);
    }

    #[test]
    fn test_iit_repertoire_kl_identical() {
        let calc = IitPhiCalculator::new(10, 3);
        let p = vec![0.5, 0.3, 0.2];
        let q = vec![0.5, 0.3, 0.2];
        let kl = calc.repertoire_kl(&p, &q);
        assert!((kl).abs() < 1e-12, "KL of identical dist should be 0, got {}", kl);
    }

    #[test]
    fn test_iit_repertoire_kl_different() {
        let calc = IitPhiCalculator::new(10, 3);
        let p = vec![1.0, 0.0];
        let q = vec![0.5, 0.5];
        let kl = calc.repertoire_kl(&p, &q);
        assert!(kl > 0.0, "KL should be > 0 for different distributions, got {}", kl);
    }

    #[test]
    fn test_iit_repertoire_kl_zero_mass() {
        let calc = IitPhiCalculator::new(10, 3);
        let p = vec![0.0, 1.0];
        let q = vec![0.5, 0.5];
        // p[0] is 0 → term vanishes, p[1] = 1.0, q[1] = 0.5
        let kl = calc.repertoire_kl(&p, &q);
        assert!(kl > 0.0);
    }

    #[test]
    fn test_iit_repertoire_kl_infinite() {
        let calc = IitPhiCalculator::new(10, 3);
        let p = vec![0.5, 0.5];
        let q = vec![1.0, 0.0];
        // p[1] > 0 but q[1] == 0 should give infinity
        let kl = calc.repertoire_kl(&p, &q);
        assert_eq!(kl, f64::INFINITY);
    }

    #[test]
    fn test_iit_min_info_partition_insufficient_history() {
        let calc = IitPhiCalculator::new(10, 3);
        let state = vec![0.5; 12];
        let phi = calc.min_info_partition(&state);
        assert_eq!(phi, 0.0, "should return 0.0 with insufficient history");
    }

    #[test]
    fn test_iit_min_info_partition_with_history() {
        let mut calc = IitPhiCalculator::new(64, 3);
        let dim = 6;
        // Simulate a system that is synchronised: all layers move together
        for _ in 0..30 {
            let val: f64 = 0.3 + 0.1 * (rand::random::<f64>() - 0.5);
            let state = vec![val; dim];
            calc.push_state(state);
        }
        let current = vec![0.3; dim];
        let phi = calc.min_info_partition(&current);
        // Synchronised state should have low φ (each partition sees similar marginal)
        assert!(
            phi >= 0.0,
            "IIT φ should be non-negative, got {}",
            phi
        );
    }

    #[test]
    fn test_iit_compute_on_integrated_phi() {
        let mut calc = IitPhiCalculator::new(32, 3);
        let dim = 4;
        // Fill with varied history so the calculator has data
        for i in 0..20 {
            let state = vec![(i as f64) / 20.0; dim];
            calc.push_state(state);
        }
        let current = vec![0.5; dim];
        let mut phi = IntegratedPhi::new();
        phi.iit_compute(&calc, &current);
        // With 4 dimensions and varied history, φ should be non-negative
        assert!(
            phi.iit_phi >= 0.0,
            "iit_phi should be set (>=0), got {}",
            phi.iit_phi
        );
    }

    #[test]
    fn test_geometry_sync_tick_sets_iit_phi() {
        let mut gs = GeometrySync::new();
        // Need enough layers active so state has dimension
        for (i, layer) in DimensionLayer::all().iter().take(6).enumerate() {
            gs.observe(*layer, 0.3 + 0.05 * i as f64, (i as f64) * 0.2);
        }
        // Tick several times to build history
        for _ in 0..10 {
            let phi = gs.tick();
            // After 5+ ticks iit_phi should become positive
            if gs.cycle_count > 5 {
                assert!(
                    phi.iit_phi >= 0.0,
                    "iit_phi should be non-negative after warmup, got {}",
                    phi.iit_phi
                );
            }
        }
    ...
    #[test]
    fn test_iit_partition_system_num_partitions_bound() {
        let mut calc = IitPhiCalculator::new(10, 3);
        calc.num_partitions = 5;
        let parts = calc.partition_system(12);
        assert!(parts.len() <= 5);
    }

    #[test]
    fn test_iit_phi_nonzero_with_independent_layers() {
        let mut calc = IitPhiCalculator::new(128, 3);
        let dim = 4;
        // Two layers move together, two move independently → structured
        for state_idx in 0..100 {
            let base = (state_idx as f64) / 100.0; // slow drift
            let mut state = vec![0.0; dim];
            // dims 0 and 1 are correlated
            state[0] = base;
            state[1] = base + 0.1;
            // dims 2 and 3 are random
            state[2] = rand::random::<f64>();
            state[3] = rand::random::<f64>();
            calc.push_state(state);
        }
        let current = vec![0.5; dim];
        let phi = calc.min_info_partition(&current);
        // A system with both correlated and independent parts should yield
        // appreciable integrated information
        assert!(
            phi > 0.0,
            "structured system should have positive φ, got {}",
            phi
        );
    }
}
