...
pub struct ResonanceMatrix {
    ...
    /// Per-axis discriminative weights (Phase 8.3 E8 Adaptive Modulation).
    /// Higher weight = axis more useful for distinguishing specialists.
    pub axis_weights: [f64; 6],
}

/// Compute per-axis variance-based weights from specialist hexagram states.
/// Higher variance = more discriminative axis.
pub fn compute_axis_weights(states: &[ReasoningHexagram; MODULE_COUNT]) -> [f64; 6] {
    let mut weights = [0.0; 6];
    for axis in 0..6 {
        let values: Vec<f64> = states.iter().map(|s| s.axis(axis) as f64).collect();
        let mean = values.iter().sum::<f64>() / MODULE_COUNT as f64;
        let variance = values.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / MODULE_COUNT as f64;
        weights[axis] = variance;
    }
...
impl ResonanceMatrix {
    /// Build from a slice of 13 hexagram assignments (one per specialist).
    pub fn from_states(states: &[ReasoningHexagram; MODULE_COUNT]) -> Self {
        ...
        let axis_weights = compute_axis_weights(states);
        Self {
            strengths,
            axis_weights,
        }
    ...
    /// Axis-weighted effective salience (Phase 8.3).
    /// Resonance strength is computed as weighted sum of matching axes.
    pub fn effective_salience_weighted(
        &self,
        raw: &[f64; MODULE_COUNT],
        ...
        for (i, item) in eff.iter_mut().enumerate() {
            ...
            for (j, &raw_j) in raw.iter().enumerate() {
                ...
                let weighted: f64 = (0..6)
                    .map(|axis| {
                        if states[i].axis(axis) == states[j].axis(axis) {
                            self.axis_weights[axis]
                        } else {
                            0.0
                        }
                    })
                    .sum();
                resonance_boost += weighted * raw_j * 0.1;
            }
            *item = (*item + resonance_boost).min(1.0);
        }
        eff
    ...
    /// Update axis weights from broadcast history (Phase 8.3).
    /// Axes that distinguish recent winners from non-winners get higher weight.
    pub fn update_axis_weights(
        &mut self,
        broadcast_history: &[usize],
        states: &[ReasoningHexagram; MODULE_COUNT],
    ) {
        if broadcast_history.is_empty() {
            return;
        }
        let winner = broadcast_history[broadcast_history.len() - 1];
        for axis in 0..6 {
            let winner_val = states[winner].axis(axis);
            let same_count = states.iter().filter(|s| s.axis(axis) == winner_val).count();
            // Fewer modules sharing this axis value = more discriminative
            let discriminative = 1.0 - (same_count as f64 - 1.0) / (MODULE_COUNT as f64 - 1.0);
            self.axis_weights[axis] = 0.5 * self.axis_weights[axis] + 0.5 * discriminative;
        }
        let sum_w: f64 = self.axis_weights.iter().sum();
        if sum_w > 0.0 {
...
pub fn resonate_cycle_with_diversity(
    ...
    let injected = diversity_inject(
        raw_salience,
        entropy,
        DIVERSITY_MIN_ENTROPY,
        DIVERSITY_NOISE_AMPLITUDE,
    );
...
// ============================================================
// Phase 8.3 — E8 Adaptive Modulation Report
// ============================================================

/// Report from E8 modulation tick — axis weights and their entropy.
#[derive(Debug, Clone)]
pub struct E8ModulationReport {
    pub axis_weights: [f64; 6],
    pub dominant_axis: usize,
    pub modulation_entropy: f64,
    pub exploration_boosted: bool,
}

/// Modulated axis weights after exploration boost.
pub fn boost_low_weight_axes(axis_weights: &mut [f64; 6], entropy: f64, threshold: f64) -> bool {
    if entropy >= threshold {
        return false;
    }
    let avg = axis_weights.iter().sum::<f64>() / 6.0;
    let mut boosted = false;
    for w in axis_weights.iter_mut() {
        if *w < avg * 0.5 {
            *w = (*w + avg * 0.3).min(1.0);
            boosted = true;
        }
...
// ============================================================
// Phase 10.2 — Entropy-Driven Intrinsic Motivation
// ============================================================

/// Shannon entropy of broadcast winner distribution over history.
/// Low entropy = same module winning repeatedly.
pub fn compute_broadcast_entropy(broadcast_history: &[usize], num_modules: usize) -> f64 {
    if broadcast_history.is_empty() {
        return 0.0;
    }
    let mut counts = vec![0usize; num_modules];
    for &winner in broadcast_history {
        if winner < num_modules {
            counts[winner] += 1;
        }
    }
    let total = broadcast_history.len() as f64;
    let entropy: f64 = counts
        ...
        .sum();
    entropy / (num_modules as f64).log2() // normalized 0-1
}

/// Shannon entropy of a salience distribution.
/// Low entropy = one module dominating.
pub fn compute_content_entropy(saliences: &[f64]) -> f64 {
    let total: f64 = saliences.iter().sum();
    if total <= 0.0 {
        return 0.0;
    }
    let n = saliences.len() as f64;
    let entropy: f64 = saliences
        ...
        .sum();
    entropy / n.log2() // normalized 0-1
}

/// Curiosity signal from content and broadcast entropy.
/// Returns 0.0-1.0 representing "need for novelty".
pub fn curiosity_signal(content_entropy: f64, broadcast_entropy: f64) -> f64 {
    let content_curiosity = if content_entropy < 0.3 {
        (0.3 - content_entropy) / 0.3
    } else {
        0.0
    };
    let broadcast_curiosity = if broadcast_entropy < 0.3 {
        (0.3 - broadcast_entropy) / 0.3
    } else {
        0.0
    };
    (content_curiosity * 0.6 + broadcast_curiosity * 0.4).clamp(0.0, 1.0)
}
