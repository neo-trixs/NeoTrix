...
impl ResonanceMatrix {
    ...
    /// Build using continuous VSA similarity instead of discrete Hamming distance.
    /// Previously VsaContentScorer (68 lines) was completely orphaned — wrapped a
    /// 384-dim E8VsaEmbedding but was never imported or constructed in any production
    /// code. Now wired as an alternative resonance constructor: VSA cosine similarity
    /// gives continuous [0,1] scores (mapped to u32 0-6) that are differentiable,
    /// unlike the previous Hamming-distance-based discrete strengths.
    pub fn from_states_with_vsa(
        states: &[ReasoningHexagram; MODULE_COUNT],
        scorer: &VsaContentScorer,
    ) -> Self {
        let mut strengths = [[0u32; MODULE_COUNT]; MODULE_COUNT];
        for i in 0..MODULE_COUNT {
            for j in 0..MODULE_COUNT {
    ...
    /// Find a complementary module using Hamming distance ≤ 3.
    /// Previously used exact bit flip (all 6 bits) — too strict for 13 hand-picked
...
/// If a `MoERouter` is provided, the winner is determined by learned routing
/// instead of hardcoded argmax of effective salience.
...
pub fn resonate_and_select(
    ...
    router: Option<&mut MoERouter>,
) -> (usize, [f64; MODULE_COUNT], f64) {
    let eff = matrix.effective_salience(raw_salience);

    // Winner-take-most: use learned router or fallback to argmax
    let winner = if let Some(r) = router {
        let selected = r.select_experts(raw_salience, 1);
        selected[0]
    } else {
        eff.iter()
            .enumerate()
...
pub struct ResonanceReport {
    ...
    /// Whether a complementary module (Hamming dist ≤ 3 of true complement) was found.
...
/// If a `MoERouter` is provided, learned routing replaces hardcoded selection.
pub fn resonate_cycle(
    ...
    resonate_cycle_with_matrix(raw_salience, states, &matrix, router)
}

/// Run a full resonance-aware competition cycle using a pre-built ResonanceMatrix.
/// This variant allows the caller to supply a custom matrix (e.g. built with VSA
/// cosine similarity via `from_states_with_vsa()`) instead of the default Hamming
/// distance matrix. All other logic (resonance boost, cluster detection, complement)
/// is identical to `resonate_cycle()`.
pub fn resonate_cycle_with_matrix(
    raw_salience: &[f64; MODULE_COUNT],
    states: &[ReasoningHexagram; MODULE_COUNT],
    matrix: &ResonanceMatrix,
    router: Option<&mut MoERouter>,
) -> ResonanceReport {
    let (winner, eff, entropy) = resonate_and_select(raw_salience, matrix, router);
...
mod tests {
    ...
    fn test_resonate_and_select_picks_highest() {
        ...
        let (winner, _, _) = resonate_and_select(&raw, &matrix, None);
        assert_eq!(winner, 3);
    }
    ...
    fn test_entropy_focused_vs_distributed() {
        ...
        focused_raw[0] = 0.99;
        let (_, _, focused_entropy) = resonate_and_select(&focused_raw, &matrix, None);

        // Distributed: all equal
        let distributed_raw = [0.5; MODULE_COUNT];
        let (_, _, distributed_entropy) = resonate_and_select(&distributed_raw, &matrix, None);
    ...
    fn test_resonance_with_oscillation_enhancement() {
        ...
        let (winner, eff, entropy) = resonate_and_select(&raw, &matrix, None);
