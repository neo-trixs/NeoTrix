...
impl E8Lattice {
    ...
    /// Decode a single root + coefficient back to an 8D physical-space vector.
    /// Each component = `coef × root[i]`.
    /// Returns `None` if `idx >= root_count()`.
    pub fn decode_single(&self, idx: usize, coef: f64) -> Option<[f64; 8]> {
        if idx >= self.roots.len() {
            return None;
        }
        let root = self.roots[idx].as_f64();
        let mut result = [0.0f64; 8];
        for i in 0..8 {
            result[i] = coef * root[i];
        }
        Some(result)
    }
