...
pub struct ContextBudget {
    ...
    allocation: HashMap<BudgetSourceType, f64>,
}

#[derive(Debug, Clone, Hash, PartialEq, Eq)]
pub enum BudgetSourceType {
    KnowledgeBase,
    Stream,
    Prompt,
    System,
...
impl BudgetSourceType {
    pub fn name(&self) -> &'static str {
        match self {
            BudgetSourceType::KnowledgeBase => "knowledge_base",
            BudgetSourceType::Stream => "stream",
            BudgetSourceType::Prompt => "prompt",
            BudgetSourceType::System => "system",
            BudgetSourceType::Tools => "tools",
        }
    }
}
...
pub struct CompactionIntent {
    ...
    pub current_source: BudgetSourceType,
    pub reserve_tokens: usize,
...
impl ContextBudget {
    ...
    pub fn default_allocation() -> HashMap<BudgetSourceType, f64> {
        let mut map = HashMap::new();
        map.insert(BudgetSourceType::KnowledgeBase, 0.35);
        map.insert(BudgetSourceType::Stream, 0.25);
        map.insert(BudgetSourceType::Prompt, 0.25);
        map.insert(BudgetSourceType::System, 0.10);
        map.insert(BudgetSourceType::Tools, 0.05);
        map
    }
    ...
    pub fn token_budget_for(&self, source: &BudgetSourceType) -> usize {
    ...
    pub fn assemble(&self, sources: &[(BudgetSourceType, String)]) -> AssembledContext {
...
mod tests {
    ...
    fn test_assemble_trims_oversized_content() {
        ...
        let sources = vec![
            (BudgetSourceType::KnowledgeBase, kb_content),
            (BudgetSourceType::System, "hello".to_string()),
        ];
    ...
    fn test_remaining_budget() {
        let budget = ContextBudget::new(100);
        let ctx = budget.assemble(&[(BudgetSourceType::System, "test".to_string())]);
    ...
    fn test_set_allocation_updates() {
        ...
        budget.set_allocation(BudgetSourceType::KnowledgeBase, 0.5);
        assert_eq!(
            budget.token_budget_for(&BudgetSourceType::KnowledgeBase),
            500
        );
        budget.set_allocation(BudgetSourceType::KnowledgeBase, 0.25);
        assert_eq!(
            budget.token_budget_for(&BudgetSourceType::KnowledgeBase),
            250
        );
    ...
    fn test_overflow_redistribution() {
        ...
        budget.set_allocation(BudgetSourceType::KnowledgeBase, 0.5);
        budget.set_allocation(BudgetSourceType::System, 0.5);
        let long_kb = "x".repeat(1600);
        let short_sys = "short".to_string();
        let ctx = budget.assemble(&[
            (BudgetSourceType::KnowledgeBase, long_kb),
            (BudgetSourceType::System, short_sys),
        ]);
        let kb_slice = ctx
            .slices
            .iter()
    ...
    fn test_token_budget_for_missing_source_returns_zero() {
        let budget = ContextBudget::new(100);
        let tools = budget.token_budget_for(&BudgetSourceType::Tools);
        assert_eq!(tools, 5);
    }
    ...
    fn test_set_allocation_clamps_above_one() {
        ...
        budget.set_allocation(BudgetSourceType::Prompt, 2.0);
        assert_eq!(budget.token_budget_for(&BudgetSourceType::Prompt), 100);
    }

    #[test]
    fn test_set_allocation_clamps_below_zero() {
        let mut budget = ContextBudget::new(100);
        budget.set_allocation(BudgetSourceType::Prompt, -0.5);
        assert_eq!(budget.token_budget_for(&BudgetSourceType::Prompt), 0);
    }
    ...
    fn test_source_type_names() {
        assert_eq!(BudgetSourceType::KnowledgeBase.name(), "knowledge_base");
        assert_eq!(BudgetSourceType::Stream.name(), "stream");
        assert_eq!(BudgetSourceType::Prompt.name(), "prompt");
        assert_eq!(BudgetSourceType::System.name(), "system");
        assert_eq!(BudgetSourceType::Tools.name(), "tools");
    }
    ...
    fn test_assemble_respects_reserve() {
        ...
        let ctx = budget.assemble(&[
            (BudgetSourceType::KnowledgeBase, kb_content),
            (BudgetSourceType::System, "small".to_string()),
        ]);
        let kb_slice = ctx
            .slices
            .iter()
    ...
    fn test_compaction_intent_critical() {
        let intent = CompactionIntent {
            ...
            current_source: BudgetSourceType::Stream,
            reserve_tokens: 50,
    ...
    fn test_assemble_with_intent_preserves_critical() {
        ...
        let sources = vec![
            (BudgetSourceType::KnowledgeBase, "small kb".to_string()),
            (BudgetSourceType::Stream, "x".repeat(800)),
        ];
        let ctx = budget.assemble_with_intent(&sources, &[intent]);
        let stream_slices: Vec<_> = ctx
            .slices
            .iter()
            .filter(|s| matches!(s.source, BudgetSourceType::Stream))
            .collect();
        assert_eq!(
            stream_slices.len(),
            2,
    ...
    fn test_intent_for_source_found() {
        let intents = vec![
            CompactionIntent {
                ...
                target_cursor: 0,
                summary_blocks: vec![],
                current_source: BudgetSourceType::KnowledgeBase,
                reserve_tokens: 10,
        ...
        let budget = ContextBudget::new(100);
        let found = budget.intent_for_source(&intents, &BudgetSourceType::Stream);
        assert!(found.is_some());
        assert_eq!(found.unwrap().session_id, "b");
        let not_found = budget.intent_for_source(&intents, &BudgetSourceType::Tools);
    ...
    fn test_assemble_with_intent_low_priority_not_reserved() {
        ...
        let sources = vec![(BudgetSourceType::Stream, "content".to_string())];
