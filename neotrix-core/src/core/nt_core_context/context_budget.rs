...
mod tests {
    ...
    fn test_overflow_redistribution() {
        ...
        let kb_slice = ctx
            ...
            .unwrap_or_else(|| panic!("KnowledgeBase slice should exist when overflow > 0"));
        assert!(
            kb_slice.tokens > 200,
    ...
    fn test_assemble_respects_reserve() {
        ...
        let kb_slice = ctx
            ...
            .find(|s| matches!(s.source, SourceType::KnowledgeBase))
            .unwrap();
        // Each source raw budget = 200, each reduced by 100 reserve → 100 effective.
        // System only uses 1 token (5 chars/4), leaving 99 unused that gets redistributed to KB.
        // KB effective = 100 + 99 = 199 tokens
        assert!(
            kb_slice.tokens >= 190 && kb_slice.tokens <= 210,
            "KB should get ~199 tokens after redistribution, got {}",
            kb_slice.tokens
        );
    }
