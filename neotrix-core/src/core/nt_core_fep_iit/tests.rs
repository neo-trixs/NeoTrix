...
fn test_vsa_from_scalar_produces_deterministic_hv() {
    ...
    assert!(
        (b.vsa().similarity(&hv_a, &hv_b) - 1.0).abs() < 1e-10,
...
fn test_fep_to_iit_produces_valid_phi() {
    ...
    assert!(
        phi_report.phi >= 0.0 && phi_report.phi <= 1.0,
        "FEP → IIT phi must be in [0,1], got {}",
        phi_report.phi
    );
...
fn test_fep_iit_empty_state() {
    ...
    assert!(
        (fe - 1.0).abs() < 1e-6,
        "Empty state FE should be ~1.0, got {fe}"
    );
    let phi = b.compute_phi(&[]);
    assert!(
        (phi - 0.0).abs() < 1e-10,
        "Empty system Φ should be 0, got {phi}"
    );
    // Single element also Φ=0
    let phi_single = b.compute_phi(&[zero.clone()]);
    assert!(
        (phi_single - 0.0).abs() < 1e-10,
        "Single element Φ should be 0, got {phi_single}"
    );
}
...
fn test_fep_iit_single_element_phi_zero() {
    ...
    assert!(
        (phi - 0.0).abs() < 1e-10,
        "Single element must have Φ=0, got {phi}"
    );
}
...
fn test_fep_iit_two_element_integration() {
    ...
    assert!(
        phi_same > 0.0,
        "Identical pair should have Φ > 0, got {phi_same}"
    );
    ...
    assert!(
        phi_diff < 0.1,
        "Dissimilar pair should have Φ ≈ 0, got {phi_diff}"
    );
}
...
fn test_fep_iit_action_selection_picks_min_fe() {
    ...
    assert_eq!(
        selected,
        Some(1),
        "Action 1 (matching belief) should be selected"
    );
...
fn test_fep_iit_identical_vectors_edge() {
    ...
    assert!(
        (fe - 0.0).abs() < 1e-6,
        "Identical vectors should have FE=0, got {fe}"
    );
    ...
    assert!(
        (fe_zero - 1.0).abs() < 1e-6,
        "Zero-vector pair should have FE=1.0, got {fe_zero}"
    );
    ...
    assert!(
        (sim_r - 1.0).abs() < 1e-6,
        "After normalize, self-sim should be 1, got {sim_r}"
    );
}
