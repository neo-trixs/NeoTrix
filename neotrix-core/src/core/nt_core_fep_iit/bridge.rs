...
impl FEPIITBridge {
    ...
    fn encode_phi_to_vsa(&self, report: &PhiReport) -> Vec<f64> {
        ...
        self.vsa()
            .bundle(&[&phi_hv, &resonance_hv, &energy_hv, &dims_hv, &trend_hv])
    }
    ...
    pub fn bridge_cycle(
        ...
        let bounded_fe = self.iit_bounded_free_energy(fe_report.variational_fe, phi_report.phi);
        let fe_bound = self.free_energy_bound(phi_report.phi);
        let (fe_improvement, phi_improvement) = self.bidirectional_reward(fe_report, phi_report);
        ...
        BridgeReport {
            ...
            state_classification: self.classify_state(fe_report.variational_fe, phi_report.phi),
        }
    }
    ...
    pub fn project_to_fep_iit(&self, project_score: f64, free_energy: f64, phi: f64) -> f64 {
