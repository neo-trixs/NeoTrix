...
// ─── E₈ Root System ─────────────────────────────────────────────────
// (Re-exported from neotrix-mind::scheduler::e8 above)
// Re-exported types: E8Weight, E8Root, e8_root_system
...
pub fn trigram_to_su3_root(trigram: u8) -> (i8, i8) {
    match trigram {
        ...
        7 => (-1, 1),  // 乾
        _ => {
            log::warn!("[e8] trigram_to_su3_root called with out-of-range value: {trigram}, clamping to 0");
            (-1, -1) // 坤 (trigram 0)
        }
    }
}
...
pub fn fermion_states_for_generation(_gen: usize) -> Vec<FermionState> {
    ...
    for hex in 0..64 {
        ...
        let (color, r, g, b) = match color_bits {
            ...
            6 => ("white", 0, 0, 0),
            7 => ("black", 0, 0, 0),
            _ => unreachable!("unexpected color_bits: {color_bits}"),
        };
        ...
        let (i3, q) = match weak_bits {
            ...
            _ => unreachable!("unexpected weak_bits: {weak_bits}"),
        };
...
// ─── Re-exports from neotrix-mind E8 module ──────────────────────────
/// E8 weight vector in 8D: coords are half-unit encoded (i8).
pub use neotrix_mind::scheduler::e8::E8Weight;

/// E8Root type alias (E8Weight) for API consistency with E8 notation.
pub use neotrix_mind::scheduler::e8::E8Root;

/// E8 root lattice: 240 unit roots in 8D.
pub use neotrix_mind::scheduler::e8::E8Lattice;

/// Generate all 240 non-zero roots of E₈.
pub use neotrix_mind::scheduler::e8::e8_root_system;

/// Block-diagonal E8 weight matrices; generated from Killing form.
pub use neotrix_mind::scheduler::e8::E8BlockDiagonal;

/// Compute E8 structure constants from root system.
pub use neotrix_mind::scheduler::e8::compute_e8_structure_constants;

// ─── E8Projector ───────────────────────────────────────────────────────

/// Projects a VSA buffer (f64 or u8) into E8 root space.
pub struct E8Projector;

impl E8Projector {
    pub fn project_vsa(vsa: &[u8]) -> E8Root {
        // Convert bytes to i8 components (interpret as 8D weight)
        let mut coords = [0i8; 8];
        for (i, chunk) in vsa.chunks(64).take(8).enumerate() {
            let sum: u32 = chunk.iter().map(|&b| b as u32).sum();
            coords[i] = ((sum as f64 / chunk.len() as f64) - 128.0).round() as i8;
        }
        E8Root::new(coords)
    }
}

/// Killing form on the E8 Lie algebra: K(a, b) = sum(ai * bi)
/// Evaluated in half-unit space (i8 coords) and returned as f64.
pub fn killing_form(a: &E8Root, b: &E8Root) -> f64 {
    let dot: i32 = a
        .coords
        .iter()
        .zip(b.coords.iter())
