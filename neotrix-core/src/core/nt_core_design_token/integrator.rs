use super::token_types::{TokenRegistry, DesignToken, TokenType, TokenValue, HierarchyLevel};
use super::renderer::{TokenRenderer, FilterChain};
    ...
    pub fn register_scene_tokens(&mut self) {
        let scene_tokens: [(&str, &str, TokenType, TokenValue); 5] = [
            (
                ...
                TokenType::Motion,
                TokenValue::Motion { duration_ms: 350, stiffness: 180.0, damping: 15.0 },
            ),
            (
                "easing-scene-zoom",
                "Scene zoom easing",
                TokenType::Easing,
                TokenValue::Easing { x1: 0.4, y1: 0.0, x2: 0.2, y2: 1.0 },
            ),
            (
                "color-scene-bg",
                "Scene background color",
                TokenType::Color,
                TokenValue::Color { r: 0.04, g: 0.04, b: 0.08, a: 1.0 },
            ),
            (
                "shadow-scene-overlay",
                "Scene overlay shadow",
                TokenType::Shadow,
                TokenValue::Shadow { offset_x: 0.0, offset_y: 0.0, blur: 16.0, spread: 0.0, r: 0.0, g: 0.53, b: 0.82, a: 0.08 },
            ),
            (
        ...
        for (name, desc, ttype, value) in &scene_tokens {
            if self.registry.resolve(name).is_none() {
                ...
                self.registry.register(token, HierarchyLevel::Semantic("scene".to_string()));
            }
        }
    }
}
