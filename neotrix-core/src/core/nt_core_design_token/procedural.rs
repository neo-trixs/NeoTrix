...
pub fn extract_vsa_params(vsa_vec: &[u8]) -> Vec<(String, f64)> {
    vec![
        ...
        ("saturation".into(), 0.3 + 0.7 * _bits_to_float(vsa_vec, 28, 6)),
        ("brightness".into(), 0.2 + 0.8 * _bits_to_float(vsa_vec, 34, 6)),
        ("contrast".into(), 0.3 + 0.7 * _bits_to_float(vsa_vec, 40, 6)),
        ...
        ("pattern_type".into(), _bits_to_int(vsa_vec, 103, 4, 15) as f64),
        ("num_layers".into(), (_bits_to_int(vsa_vec, 107, 3, 7) + 1) as f64),
