...
/// Temporal Difference dynamics for JEPA. Implemented but not yet wired into the consciousness pipeline (F2.5).
...
impl TDDynamics {
    ...
    pub fn td_error(
        ...
        reward + self.gamma * self.value(z_t_plus_n, critic_weights)
            - self.value(z_t, critic_weights)
    }
