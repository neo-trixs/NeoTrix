use super::*;
use crate::neotrix::nt_core_signal::SIGReg;
...
fn test_multiscale_loss_computation() {
    ...
    assert!(
        loss_bad > 0.0,
        "Mismatched predictions should have positive loss"
    );
...
fn test_sigreg_random_gaussian_low_loss() {
    ...
    let sigreg = SIGReg::new(32, 256.0, 42);
    let n = 500;
    ...
    assert!(
        loss < 0.1,
        "Random Gaussian latents should have low SIGReg loss: {}",
        loss
    );
}
...
fn test_sigreg_collapse_high_loss() {
    ...
    assert!(
        loss > 0.5,
        "Collapsed latents should have high SIGReg loss: {}",
        loss
    );
}
...
fn test_sigreg_zero_projections_zero_loss() {
    let sigreg = SIGReg::new(32, 0.0, 42);
...
fn test_sigreg_deterministic_projections() {
    let sigreg1 = SIGReg::new(32, 256.0, 42);
    let sigreg2 = SIGReg::new(32, 256.0, 42);
    ...
    assert!(
        (loss1 - loss2).abs() < 1e-10,
        "Same seed should produce same loss: {} vs {}",
        loss1,
        loss2
    );
}
...
fn test_sigreg_matrix_variant_detects_collapse() {
    ...
    let matrix_loss = sigreg.compute_loss(&collapsed);
    assert!(
        matrix_loss > 0.1,
        "Loss should detect collapse: {}",
        matrix_loss
    );
    ...
    let random: Vec<Vec<f64>> = (0..100)
        ...
        .collect();
    let random_loss = sigreg.compute_loss(&random);
    assert!(
        random_loss < matrix_loss,
        "Loss on random data should be lower than on collapsed: {} vs {}",
        random_loss,
        matrix_loss
    );
}
...
fn test_sigreg_integration_with_model() {
    ...
    let batch_x: Vec<Vec<f64>> = (0..10)
        .map(|i| (0..64).map(|j| ((i * 64 + j) as f64) / 640.0).collect())
        .collect();
    let batch_y: Vec<Vec<f64>> = (0..10)
        .map(|i| (0..64).map(|j| ((i * 64 + j + 1) as f64) / 640.0).collect())
        .collect();
    ...
    assert!(
        loss_with_sigreg >= loss_without * 0.5,
...
fn test_sigreg_matrix_variant_single_embedding() {
    ...
    let embeddings = vec![vec![0.5; 32]];
    let loss = sigreg.compute_loss(&embeddings);
    assert_eq!(loss, 0.0, "Single embedding should give 0 loss");
}
...
fn test_sigreg_both_variants_agree_on_collapse() {
    ...
    let loss = sigreg.compute_loss(&collapsed);
    assert!(loss > 0.5, "Loss should detect collapse: {}", loss);
}
