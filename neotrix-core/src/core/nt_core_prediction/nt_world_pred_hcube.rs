...
impl HyperCoordEncoder {
    ...
    pub fn latent_to_coord(latent: &[f64]) -> HyperCoord {
        ...
        let step = if latent.len() > CUBE_STORE_DIM {
            latent.len() / CUBE_STORE_DIM
        } else {
            1
        };
    ...
    pub fn retrieve_similar(
        ...
        entries
            ...
            .map(|entry| {
                ...
                let latent_state: Vector =
                    entry.coord.dims().map(|(_dim, val)| *val / 100.0).collect();
