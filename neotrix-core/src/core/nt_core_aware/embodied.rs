...
impl Proprioception {
    ...
    pub fn overall_discomfort(&self) -> f64 {
        ...
        let pain_component = if self.pain_signals.is_empty() {
            ...
            self.pain_signals.iter().map(|(_, i)| *i).sum::<f64>() / self.pain_signals.len() as f64
        };
...
impl TemporalExtension {
    ...
    pub fn anticipate(&mut self, event: AnticipatedEvent) {
        if self.anticipated_future.len() >= DEFAULT_MAX_ANTICIPATIONS {
            if let Some((idx, _)) = self.anticipated_future.iter().enumerate().min_by(|a, b| {
                a.1.confidence
    ...
    pub fn temporal_phi(&self) -> f64 {
        let past_ratio = (self.personal_past.len() as f64 / self.max_history as f64)
            .max(0.0)
            .min(1.0);
...
impl EmbodiedSelf {
    ...
    pub fn integrate(&mut self, foundation: &ConsciousnessFoundation) {
        ...
        for i in 0..n {
            self.proprioception.update_tension(i, intensity * 0.5);
        }
        ...
        self.time_extension.duration = self.time_extension.duration.saturating_add(extra);
