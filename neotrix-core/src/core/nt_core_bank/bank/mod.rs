...
impl ReasoningBank {
    ...
    pub fn store(&mut self, memory: ReasoningMemory) {
        ...
        self.task_type_index
            .entry(memory.task_type)
            .or_default()
            .push(new_idx);
        if let Some(ref mut wh) = self.wh_index {
            let mut text = format!("{} {:?}", memory.task_description, memory.task_type);
            if let Some(ref v) = memory.t3_views.struct_view {
                text.push_str(&format!(" struct:{}", v));
            }
            if let Some(ref v) = memory.t3_views.semantic_view {
                text.push_str(&format!(" semantic:{}", v));
            }
            if let Some(ref v) = memory.t3_views.reflect_view {
    ...
    pub fn quality_score(&self) -> f64 {
        ...
        let max_access = self
            .memories
            .iter()
            .map(|m| m.lifecycle.access_count)
            .max()
            .unwrap_or(1)
            .max(1) as f64;
        let access_health: f64 = self
            .memories
            .iter()
            .map(|m| {
