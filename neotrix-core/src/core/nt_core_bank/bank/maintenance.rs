...
impl ReasoningBank {
    ...
    pub fn initialize_with_design_knowledge(&mut self) {
        ...
        let design_knowledge = vec![
            ...
            ("Chat UI: user/assistant/system message types, fade-in animation, thinking state (pulse/spin animation), status dot indicator, message counting, evolution badge display", TaskType::UIDesign, 0.87),
        ];
