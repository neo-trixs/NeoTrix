...
use super::designer::{ArchitectureDesign, ModuleBlueprint, TypeKind, RefactoringPlan, CodeAction};
...
impl CodeImplementer {
            ...
            match change.action {
                ...
                ChangeAction::Modify => {
                    let mut file = fs::OpenOptions::new()
                        .append(true)
                        .open(path)
                        .map_err(|e| format!("Failed to open file {} for append: {}", path.display(), e))?;
                    file.write_all(change.content.as_bytes())
        ...
        for change in changes.iter().rev() {
            ...
            match change.action {
                ChangeAction::Create => {
                    if path.exists() {
                        fs::remove_file(path)
                            .map_err(|e| format!("Failed to rollback file {}: {}", path.display(), e))?;
                    }
                }
                ChangeAction::Modify => {
                    // 无法精确回滚追加操作 — 打印警告
                    log::warn!("Cannot rollback modification to {} (content was appended)", path.display());
                }
                ChangeAction::Delete => {
                    // 无法恢复删除 — 打印警告
    ...
    fn create_module(&self, blueprint: &ModuleBlueprint, project_root: &str) -> FileChange {
        let dir_path = format!("{}/src/{}/{}", project_root, blueprint.parent_module, blueprint.name);
        let file_path = format!("{}/{}.rs", dir_path, blueprint.file_name.trim_end_matches(".rs"));

        let mut content = String::new();
        content.push_str(&format!("//! {} — auto-generated by ArchitectAgent\n\n", blueprint.description));
        ...
        for ty in &blueprint.types {
            ...
            let prefix = if ty.fields.iter().any(|f| f.is_pub) { "pub " } else { "" };
            match &ty.kind {
                TypeKind::Struct => {
    ...
    fn create_parent_registration(&self, blueprint: &ModuleBlueprint, project_root: &str) -> FileChange {
    ...
    fn apply_action(&self, action: &CodeAction, _plan: &RefactoringPlan, _project_root: &str) -> Option<FileChange> {
        match action {
            CodeAction::AddTests { test_file, test_functions } => {
...
mod tests {
    use super::*;
    use crate::core::nt_core_arch::designer::{ModuleBlueprint, TypeBlueprint, TraitBlueprint, MethodBlueprint, TypeKind, FieldBlueprint, RefactoringPlan, CodeAction};
    ...
    fn test_create_module_generates_rust_code() {
        ...
        let module_change = changes.iter().find(|c| c.path.ends_with("test_mod.rs")).expect("module change for test_mod.rs should exist");
    ...
    fn test_generates_parent_registration() {
        ...
        let changes = impler.plan_changes(&design, "/tmp/test_project");
        assert!(changes.iter().any(|c| c.path.contains("mod.rs")), "should register in parent mod.rs");
    }
    ...
    fn test_write_changes_creates_files() {
        ...
        impler.write_changes(&changes).expect("write changes should succeed");
        let content = impler.read_file_content(&changes[0].path).expect("should read back file content");
