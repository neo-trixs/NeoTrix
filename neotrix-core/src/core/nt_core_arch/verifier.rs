...
/// 变更验证结果
#[derive(Clone, Debug)]
pub struct CompileVerificationResult {
    pub success: bool,
...
impl ChangeVerifier {
    ...
    pub fn verify_compilation(&self, project_root: &str) -> CompileVerificationResult {
    ...
    pub fn verify_tests(&self, project_root: &str, test_filter: &str) -> CompileVerificationResult {
    ...
    /// 全流程验证：编译 + 测试
    pub fn verify_all(&self, project_root: &str) -> CompileVerificationResult {
    ...
    fn run_cargo_check(&self, root: &str) -> String {
        if cfg!(test) {
            return "skipped during test".into();
        }
        ...
        match output {
            Ok(o) => {
                ...
                log::info!("cargo check took {}.{:03}s", dur.as_secs(), dur.subsec_millis());
    ...
    fn run_cargo_test(&self, root: &str, filter: &str) -> String {
        ...
        match output {
            Ok(o) => {
                let dur = start.elapsed();
                log::info!("cargo test took {}.{:03}s", dur.as_secs(), dur.subsec_millis());
                let mut result = String::from_utf8_lossy(&o.stdout).to_string();
                result.push_str(&String::from_utf8_lossy(&o.stderr));
                result
            }
