...
mod tests {
    ...
    #[test]
    fn test_default_embedder_no_panic() {
    ...
    fn test_encode_decode_similarity() {
        ...
        let sim_same = e.similarity("cat", "cat");
        let sim_diff = e.similarity("cat", "quantum");
        assert!(
            sim_same >= sim_diff,
            "same words should be at least as similar as different words"
        );
    }
}
