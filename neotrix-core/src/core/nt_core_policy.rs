...
impl E8Policy {
    ...
    /// Get the learning rate for SGD updates
    ...
    pub fn learn_from_scores(&mut self, trajectory: &AgentTrajectory, scores: &[ProcessScore]) {
        for score in scores {
            if let Some(step) = trajectory.steps.get(score.step_idx) {
                ...
                for tag in score.attribution_tags.iter() {
                    let delta = match tag.as_str() {
                        "good" | "ok" | "productive" => 0.1,
                        "fail" | "stuck" | "inefficient" => -0.1,
                        "efficient" => 0.15,
                        _ => 0.05,
                    };
                    let spread = delta / NUM_E8_FACTORS as f64;
                    for d in factor_deltas.iter_mut() {
                        *d += spread;
                    }
                }
...
// ─── Manual serde for E8Policy (large arrays >32 elements) ─────────

impl serde::Serialize for E8Policy {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        use serde::ser::SerializeStruct;
        let mut state = serializer.serialize_struct("E8Policy", 10)?;
        state.serialize_field("epsilon", &self.epsilon)?;
        state.serialize_field("epsilon_decay", &self.epsilon_decay)?;
        state.serialize_field("min_epsilon", &self.min_epsilon)?;
        state.serialize_field("learning_rate", &self.learning_rate)?;
        state.serialize_field("discount", &self.discount)?;
        state.serialize_field("mode_values", &self.mode_values[..])?;
        state.serialize_field("mode_counts", &self.mode_counts[..])?;
        state.serialize_field(
            "factor_energies",
            &AsRef::<[[f64; NUM_E8_FACTORS]]>::as_ref(&self.factor_energies),
        )?;
        state.serialize_field("factor_control", &self.factor_control[..])?;
        state.serialize_field("previous_mode", &self.previous_mode)?;
        state.end()
    }
}

impl<'de> serde::Deserialize<'de> for E8Policy {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        #[derive(serde::Deserialize)]
        struct E8PolicyHelper {
            epsilon: f64,
            epsilon_decay: f64,
            min_epsilon: f64,
            learning_rate: f64,
            ...
            mode_values: Vec<f64>,
            mode_counts: Vec<u64>,
            factor_energies: Vec<Vec<f64>>,
            factor_control: Vec<f64>,
            previous_mode: Option<ReasoningHexagram>,
        }

        let help = E8PolicyHelper::deserialize(deserializer)?;

        if help.mode_values.len() != 64 {
            return Err(serde::de::Error::custom(
                "E8Policy.mode_values must have exactly 64 elements",
            ));
        }
        if help.mode_counts.len() != 64 {
            return Err(serde::de::Error::custom(
                "E8Policy.mode_counts must have exactly 64 elements",
            ));
        }
        if help.factor_energies.len() != 64 {
            return Err(serde::de::Error::custom(
                "E8Policy.factor_energies must have exactly 64 inner arrays",
            ));
        }
        if help.factor_control.len() != NUM_E8_FACTORS {
            return Err(serde::de::Error::custom(
                "E8Policy.factor_control must have exactly 6 elements",
            ));
        }

        let mut mode_values = [0.0; 64];
        mode_values.copy_from_slice(&help.mode_values);

        let mut mode_counts = [0u64; 64];
        mode_counts.copy_from_slice(&help.mode_counts);

        let mut factor_energies = [[0.0; NUM_E8_FACTORS]; 64];
        for (i, energies) in help.factor_energies.iter().enumerate() {
            if energies.len() != NUM_E8_FACTORS {
                return Err(serde::de::Error::custom(format!(
                    "E8Policy.factor_energies[{}] must have exactly {} elements",
                    i, NUM_E8_FACTORS
                )));
            }
            factor_energies[i].copy_from_slice(energies);
        }

        let mut factor_control = [0.0; NUM_E8_FACTORS];
        factor_control.copy_from_slice(&help.factor_control);

        Ok(E8Policy {
            epsilon: help.epsilon,
            epsilon_decay: help.epsilon_decay,
            min_epsilon: help.min_epsilon,
            learning_rate: help.learning_rate,
            discount: help.discount,
            mode_values,
            mode_counts,
            factor_energies,
            factor_control,
            previous_mode: help.previous_mode,
        })
    }
}
...
// ─── Manual serde for E8TransitionLearner (mode_rewards: 64-element array) ─

impl serde::Serialize for E8TransitionLearner {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        use serde::ser::SerializeStruct;
        let mut state = serializer.serialize_struct("E8TransitionLearner", 6)?;
        state.serialize_field("outcomes", &self.outcomes)?;
        state.serialize_field("mode_rewards", &self.mode_rewards[..])?;
        state.serialize_field("matrix_rewards", &self.matrix_rewards)?;
        state.serialize_field("max_outcomes", &self.max_outcomes)?;
        state.serialize_field("min_samples", &self.min_samples)?;
        state.serialize_field("improvement_threshold", &self.improvement_threshold)?;
        state.end()
    }
}

impl<'de> serde::Deserialize<'de> for E8TransitionLearner {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        #[derive(serde::Deserialize)]
        struct E8TransitionLearnerHelper {
            outcomes: Vec<E8Outcome>,
            mode_rewards: Vec<(f64, u64)>,
            matrix_rewards: [[(f64, u64); 8]; 8],
            max_outcomes: usize,
            min_samples: u64,
            improvement_threshold: f64,
        }

        let help = E8TransitionLearnerHelper::deserialize(deserializer)?;

        if help.mode_rewards.len() != 64 {
            return Err(serde::de::Error::custom(
                "E8TransitionLearner.mode_rewards must have exactly 64 elements",
            ));
        }

        let mut mode_rewards = [(0.0, 0u64); 64];
        mode_rewards.copy_from_slice(&help.mode_rewards);

        Ok(E8TransitionLearner {
            outcomes: help.outcomes,
            mode_rewards,
            matrix_rewards: help.matrix_rewards,
            max_outcomes: help.max_outcomes,
            min_samples: help.min_samples,
            improvement_threshold: help.improvement_threshold,
        })
    }
}
