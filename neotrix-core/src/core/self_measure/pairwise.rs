use super::{PIDScores, NUM_SUBSYSTEMS};
...
pub(crate) fn compute_pairwise_pid(
    ...
    let i_xt = if r_xt.abs() < 1.0 {
        -0.5 * (1.0 - r_xt * r_xt).ln()
    } else {
        10.0
    };
    let i_yt = if r_yt.abs() < 1.0 {
        -0.5 * (1.0 - r_yt * r_yt).ln()
    } else {
        10.0
    };
    ...
    let det_xyt = c_xx * (c_yy * c_tt - c_yt * c_yt) - c_xy * (c_xy * c_tt - c_yt * c_xt)
