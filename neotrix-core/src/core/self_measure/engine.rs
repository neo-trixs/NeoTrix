use super::{
    compute_pairwise_pid, eigenvector_centrality, spectral_phi, RingBuffer, SubsystemId,
    SystemSnapshot, DEFAULT_WINDOW_SIZE, NUM_SUBSYSTEMS,
};
...
impl SelfMeasure {
    ...
    fn recompute_all(&mut self) {
        ...
        let mut count_syn = 0;
        for i in 0..NUM_SUBSYSTEMS {
            for syn_val in syn_matrix[i]
                .iter()
                .skip(i + 1)
                .take(NUM_SUBSYSTEMS - i - 1)
            {
                total_syn += syn_val;
                count_syn += 1;
            }
        }
        self.usk_current = if count_syn > 0 {
            total_syn / count_syn as f64
        } else {
            0.0
        };
    }
...
mod tests {
    ...
    fn dummy_snapshot(timestamp: i64, v: f64) -> SystemSnapshot {
        SystemSnapshot {
            timestamp,
            mood: [v; 6],
            persona: [v; 5],
    ...
    #[test]
    fn test_self_measure_new() {
        let sm = SelfMeasure::new();
        assert_eq!(sm.phi_current, 0.0);
        assert_eq!(sm.fcs_current, 0.0);
        assert_eq!(sm.usk_current, 0.0);
        assert_eq!(sm.tick_count(), 0);
        assert!(sm.trajectory.is_empty());
    }

    #[test]
    fn test_snapshot_tick_count() {
        let mut sm = SelfMeasure::new();
        sm.snapshot(dummy_snapshot(100, 0.5));
        assert_eq!(sm.tick_count(), 1);
        assert_eq!(sm.trajectory.len(), 1);
    }

    #[test]
    fn test_full_cycle_computes_phi() {
        let mut sm = SelfMeasure::new();
        for i in 0..15 {
            let v = i as f64 * 0.1;
            sm.snapshot(dummy_snapshot(i as i64, v));
        }
        assert_eq!(sm.tick_count(), 15);
        assert!(
            sm.phi_current > 0.0,
            "phi should be > 0 after 15 correlated snapshots"
        );
        assert!(sm.fcs_current > 0.0, "fcs should be > 0");
        assert!(sm.usk_current > 0.0, "usk should be > 0");
        let (b1, b2) = sm.weakest_link();
        let b1_idx: usize = b1 as usize;
        let b2_idx: usize = b2 as usize;
        assert!(b1_idx < NUM_SUBSYSTEMS);
        assert!(b2_idx < NUM_SUBSYSTEMS);
    }

    #[test]
    fn test_awakening_speed_empty() {
        let sm = SelfMeasure::new();
        assert_eq!(sm.awakening_speed(), 0.0);
    }

    #[test]
    fn test_awakening_speed_after_snapshots() {
        let mut sm = SelfMeasure::new();
        for i in 0..20 {
            let v = (i as f64).sin() * 0.5 + 0.5;
            sm.snapshot(dummy_snapshot(i as i64, v));
        }
        let speed = sm.awakening_speed();
        assert!(speed.is_finite(), "awakening_speed should be finite");
    }
    ...
    fn test_generate_report_defaults() {
        ...
        assert_eq!(report.fcs, 0.0);
        assert_eq!(report.window_used, 0);
        assert!(report.timestamp > 0);
    }

    #[test]
    fn test_weakest_link_default() {
        let sm = SelfMeasure::new();
        let (b1, b2) = sm.weakest_link();
        // default pair is (Mood, Persona) when all synergy_matrix entries are 0
        assert_eq!(b1, SubsystemId::Mood);
        assert_eq!(b2, SubsystemId::Persona);
    }
}
