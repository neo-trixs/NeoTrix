use super::{AwakeningReport, SubsystemId, AWAKENING_THRESHOLD, NUM_SUBSYSTEMS};
...
impl std::fmt::Display for AwakeningReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  Φ (integration):     {:.4}  ({}), speed: {:+.5}/step",
            self.phi,
            phi_rating(self.phi),
            self.awakening_speed
        )?;
        writeln!(
            f,
            "  FCS (consciousness): {:.4}  ({})",
        ...
        for i in 0..NUM_SUBSYSTEMS {
            ...
            writeln!(
                ...
                "    {:>12}: {:.3}",
                id.label(),
                self.subsystem_coherence[i]
            )?;
        }
        writeln!(f)?;
        writeln!(
            f,
            "  Bottleneck: {} ↔ {}  (synergy={:.3})",
            self.bottleneck.0.label(),
            self.bottleneck.1.label(),
            self.bottleneck_synergy
        )?;
        writeln!(f)?;
