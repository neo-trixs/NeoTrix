...
pub trait Coach: Send + Sync {
    ...
    ///
    /// Default implementation computes TD(0)-style prediction error:
    /// δ = external_reward - score, averaged across all steps.
    /// Stateful coaches (e.g. learned PRM with internal weights) should override
    /// this to implement their own update rule (policy gradient, temporal-difference).
    fn learn(&mut self, trajectory: &AgentTrajectory, scores: &[ProcessScore]) {
        let n = trajectory.steps.len().min(scores.len());
        if n == 0 {
            return;
        }
        ...
        let mut count = 0u64;
        for i in 0..n {
            if let Some(reward) = trajectory.steps[i].external_reward {
                let error = reward - scores[i].score;
                total_abs_error += error.abs();
                count += 1;
            }
        }
        if count > 0 {
            let mae = total_abs_error / count as f64;
            log::info!(
                "[Coach::learn] tid={} steps={} steps_with_reward={} mae={:.4}",
            ...
            log::debug!(
                "[Coach::learn] tid={} steps={} (no external rewards)",
                trajectory.trajectory_id,
                n
            );
        }
...
pub struct HeuristicCoach {
    ...
    /// TD(0) bias term updated by learn(). Adjusts all scores by this amount.
    pub bias: f64,
    /// Learning rate for TD(0) bias updates.
    pub learning_rate: f64,
    /// Running mean absolute error over all learn() calls.
    pub running_mae: f64,
    /// Number of learn() invocations processed.
    pub learn_count: u64,
}
...
impl Coach for HeuristicCoach {
    ...
    fn score_step(&self, step: &TrajectoryStep, context: &CoachContext) -> ProcessScore {
        ...
        // Apply learned bias
        score = (score + self.bias).max(0.0).min(1.0);
    ...
    fn learn(&mut self, trajectory: &AgentTrajectory, scores: &[ProcessScore]) {
        ...
        for i in 0..n {
            if let Some(reward) = trajectory.steps[i].external_reward {
                let predicted = scores[i].score;
                let error = reward - predicted;
                // TD(0) update: bias += lr * δ
        ...
        if count > 0 {
            let mae = total_abs_error / count as f64;
            // EMA smoothing for running average
            let alpha = 0.1;
            self.running_mae = if self.learn_count == 1 {
                mae
            } else {
                (1.0 - alpha) * self.running_mae + alpha * mae
            };
            log::info!(
                "[HeuristicCoach::learn] {} tid={} steps={} mae={:.4} bias={:.4} n_updates={}",
                self.name_label,
                trajectory.trajectory_id,
                n,
                mae,
...
impl ProcessRewardLearner {
        ...
        F: FnOnce(&mut TrajectoryCollector),
    {
        ...
        for traj in &trajectories {
            ...
            const MAX_SCORE_HISTORY: usize = 10000;
            if self.score_history.len() > MAX_SCORE_HISTORY {
                self.score_history.drain(0..MAX_SCORE_HISTORY / 5);
            }
