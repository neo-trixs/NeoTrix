...
impl ReputationTracker {
    ...
    pub fn record_evaluation(
        ...
        if accepted {
            ...
            let raw = compute_raw_bonus(entry.consecutive_high, true, bonus_per_step, decay_per_step);
...
fn compute_raw_bonus(streak: u32, is_positive: bool, bonus_per_step: f64, decay_per_step: f64) -> f64 {
...
mod tests {
    ...
    fn test_accepted_improves_reputation() {
        ...
        assert!(rep > 0.5, "accepted packets should improve reputation, got {}", rep);
    }
    ...
    fn test_persistent_low_quality_penalizes() {
        ...
        let rep = tracker.reputation(&id);
        assert!(rep < 0.5, "persistent rejection should lower reputation, got {}", rep);
    }
    ...
    fn test_weight_factor_range() {
        ...
        assert!(wf >= 0.5 && wf <= 1.5, "weight factor should be in [0.5, 1.5], got {}", wf);
    }
    ...
    fn test_acceptance_rate() {
        ...
        assert!((rate - 2.0 / 3.0).abs() < 0.01, "acceptance rate should be ~0.667, got {}", rate);
    }
