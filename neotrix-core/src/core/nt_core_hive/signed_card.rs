...
impl SignedAgentCard {
    ...
    pub fn extract_card<T: serde::de::DeserializeOwned>(&self) -> Result<T, String> {
        serde_json::from_slice(&self.card_json)
            .map_err(|e| format!("deserialize AgentCard: {}", e))
    }
...
mod tests {
    ...
    fn test_trust_registry_verify() {
        ...
        let signed =
            SignedAgentCard::sign(sample_card_json(), &signing_key, "trusted").unwrap();
    ...
    fn test_trust_registry_rejects_unknown() {
        ...
        let signing_key = SigningKey::random(&mut OsRng);
        let signed =
            SignedAgentCard::sign(sample_card_json(), &signing_key, "unknown").unwrap();
        assert!(!registry.verify_signed_card(&signed).unwrap());
    }
    ...
    fn test_pubkey_fingerprint_deterministic() {
        let signing_key = SigningKey::random(&mut OsRng);
        let signed1 =
            SignedAgentCard::sign(sample_card_json(), &signing_key, "a").unwrap();
        let signed2 =
            SignedAgentCard::sign(sample_card_json(), &signing_key, "b").unwrap();
        assert_eq!(
