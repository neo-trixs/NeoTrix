...
fn ecdh_shared_secret(signing_key: &SigningKey, peer_pub: &[u8]) -> Result<[u8; 32], String> {
    ...
    let peer_pk = PublicKey::from_sec1_bytes(peer_pub)
...
impl NaclChannel {
    ...
    pub fn seal_knowledge(
        ...
        let mut channel = NaclChannel::from_signing_key(
...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        alice_ch
            .derive_shared_secret(
                &*VerifyingKey::from(&bob_ch.local_signing_key)
                    .to_sec1_bytes(),
            )
            .unwrap();
        bob_ch
            .derive_shared_secret(&alice_ch.local_pubkey)
            .unwrap();
    ...
    fn test_encrypt_produces_different_ciphertexts() {
        ...
        let mut alice_ch = NaclChannel::from_signing_key(SigningKey::random(&mut rand::rngs::OsRng));
        alice_ch
    ...
    fn test_ratchet_init_encrypt_decrypt() {
        ...
        let encrypted = alice_ch.encrypt_ratchet(plaintext, Some(eph_pubkey)).unwrap();
